...
status: CONFIRMED (Lean-formalized, LRCTrapArea.lean, trap_integral = 1/49 SORRY-FREE, [propext, Classical.choice, Quot.sound]). S20 UPDATE: now WIRED to klein's ACTUAL LonelyRunner.SpreadPairFloor.trap (was self-contained) -- directly usable by the pair-floor machinery. CORRECTION to S19: the corpus BUILDS GREEN (8627 jobs); my 'mathlib drift / corpus broken' alarm was WRONG -- LRCSpreadPairFloor was an unbuilt ORPHAN (not imported), broken vs v4.30.0 by 8 stale lemma names; kps-S25 + I independently made IDENTICAL fixes; now built+registered+green. The AREA (leading term) is exact; the discrete-sum discretization error (L/49-err) is the next piece. NOTE: self-contained (re-declares trap) because klein's LRCSpreadPairFloor does NOT build against the pinned mathlib v4.30.0 -- a fleet-wide mathlib-drift issue flagged to klein/kps.
