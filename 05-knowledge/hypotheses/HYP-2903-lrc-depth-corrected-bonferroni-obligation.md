...
This is the right target only in the edge-active regime where the two-far
packet is already live.
...
```text
edge-active rows      r0 = 2: prove T_{>=4} <= 0, hence order-3 upper bound.
triple-active rows    r0 = 3: prove T_{>=5} <= 0, hence order-4 upper bound.
higher-depth rows     r0 >= 4: route to the single-block/decorrelated
                      extremality or finite residual atlas.
```
...
`T_2+T_3` by the cap and prove the `r>=4` tail is nonpositive.  But
