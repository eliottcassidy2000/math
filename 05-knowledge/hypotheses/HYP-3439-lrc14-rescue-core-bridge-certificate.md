...
```text
join the one-branch overlap rescue cores from HYP-3437
with the two-color bad-core / survivor ledgers from HYP-3436,
then separate the canonical corridor-fence family from noncanonical rows.
```

The intended theorem target is:

```text
Every primitive covering row with negative one-branch naive slack either
  (a) is the canonical corridor-fence family already handled by HYP-3431,
  (b) has a noncanonical bounded rescue core of rank at most 5 in the
      combined stress bank, or
  (c) routes through the HYP-3436 two-color survivor ledger, endpoint-spine
