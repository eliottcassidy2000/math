...
- **HYP-9065 (ELLIPTIC PART CORRECTED/PROVED ON `D(a)`; LRC TRANSFERS OPEN):** `-4p^3-27q^2=-2^4 3^6a^2K`, so globally the singular-pencil set is `V(a) union V(K)` and `V(p,q)=V(a,b) union E`; only on `a!=0` do `3/1/0` match smooth/nodal/cuspidal type. MISTAKE-287/THM-2566 give the repair. Six clock/rank/Gram/window/Fejer/residual transfer probes remain claimed.
