...
| HYP-463 | SPECTRAL VARIATIONAL PRINCIPLE (CONJECTURE): H on the spectral simplex {(y_1^2,...,y_m^2): sum=p(p-1)/8} has a unique global max at center y_k^2=p/4. When achievable (p≡3 mod 4): Paley wins. When not (p≡1 mod 4): farthest point from center (max Sum_y^4) wins. OPEN — needs proof or larger prime tests. | Based on p=7 (center=max) and p=13 (farthest=max). Non-monotone at intermediate distances. | kind-pasteur-S56 |
