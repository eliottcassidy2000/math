...
- **HYP-9060 (mac-mini-S146; CLAIMED, in progress) — ±reciprocal signature law:** the owner's three recurrence patterns factor as `[linear+parity] (x−1)²(x+1)`, `[degree-2/period-4] (x−1)³(x+1)²(x²+1)` (klein's `(r,g)=(0,3)` shadow), and `[one wild pair] (x−1)` times a palindromic sextic, equivalent under `y=x+1/x` to `y³−4y−2` (growth `lambda≈1.8637` plus four unit-circle roots). Thesis: op-covariant C-finite tournament invariants have ±palindromic characteristic polynomials; `T↦T^op` is sequence time reversal, and `y=x+1/x` descends to the mirror quotient.
  Referee plan: verify the algebra exactly; test canonical `W(n)`, Rosetta rows/sums/diagonals, and shear-catalog sequences; prove or delimit op-covariance implying ±palindromicity; merge with HYP-8165/8170 and the S145 palindrome layer.
### HYP-9060 (mac-mini-2026-07-27-S146) — THE ±RECIPROCAL SIGNATURE LAW: the owner's three recurrence patterns factor as [linear+parity] (x−1)²(x+1), [degree-2/period-4 quasi-polynomial] (x−1)³(x+1)²(x²+1) = klein's shadow shape at (r,g)=(0,3), and [one wild pair] (x−1)·(palindromic sextic ⟺ y³−4y−2 under y = x+1/x, growth λ ≈ 1.8637 + four unit-circle roots); ALL THREE are ±reciprocal = time-reversal-symmetric — THE THESIS: op-covariant C-finite tournament invariants have ±palindromic characteristic polynomials (T ↦ T^op = time reversal at sequence level; the mirror/palindrome technology's spectral face), and the y = x+1/x reduction IS the descent to the mirror quotient (the same halving as b²−a² = (b−a)(b+a) in the deficit law)
- **Status:** RESOLVED (frozen out tournament_reciprocal_signatures_macmini_S146.out). THE FULL DECODE: (i) ALGEBRA EXACT — A+B−C = (x−1)²(x+1) [+recip]; A+B−C+D−E−F+G = (x−1)³(x+1)²(x²+1) [−recip, fully cyclotomic = the degree-2/period-4 quasi-polynomial family, klein's shadow shape at (r,g)=(0,3)]; A+B+C−D−E−F+G = (x−1)·S₆ [−recip] with the palindromic sextic S₆ ⟺ y³−4y−2 (irreducible, disc 148 = 4·37) under y = x+1/x; (ii) THE SALEM DISCOVERY: S₆ has |roots| = {0.632, 1, 1, 1, 1, 1.58234718} — exactly two off the unit circle ⟹ **λ = 1.58234718… is a DEGREE-6 SALEM NUMBER**: the owner's wild signature is a Salem recurrence, and the trio = [two cyclotomic/Mahler-measure-1 tame] + [the first Salem wildness] — the recurrence-level face of "the gap above the floor," opening the LEHMER/MAHLER-MEASURE bridge (filed to the portfolio); (iii) IMPULSE DECODE: the three impulse responses are ⌈n/2⌉ (repeated naturals), the DOUBLED QUARTER-SQUARES (A002620 = bipartite relation counts — the owner's "triangular numbers arise from relation" theme, third perspective confirmed), and the Salem sequence 1,1,2,4,6,10,16,26,41,65,104,164,260,412,652,1032 (OEIS-check flagged to klein's pipeline); all three signatures satisfy p(1)=0 with coefficient-sum 1 — AUGMENTED DIFFERENCE OPERATORS fixing constants; (iv) OP-COVARIANCE THEOREM (stated + machine face): reversal-symmetric transfer (M ~ M⁻¹) forces ±reciprocal characteristic polynomials — inversion-closure of all three root multisets follows in one line from the verified ±reciprocity; op-covariant C-finite tournament invariants must carry ±palindromic signatures (T ↦ T^op = time reversal): the mirror/palindrome technology's SPECTRAL face, with y = x+1/x as the descent to the mirror quotient (the same halving as (b−a)(b+a) in the deficit law); (v) HONEST NEGATIVE CENSUS: none of W(n), Rosetta row/alt sums, Rosetta columns, Proth rows, 2ⁿ+1, or 60·H_n satisfies any of the three — the signatures' native tournament sequences are NOT the obvious canon ones; matching the impulse fingerprints against the zoo atlas is the named follow-up.
- **Files:** 04-computation/tournament_reciprocal_signatures_macmini_S146.py + .out. Parents: HYP-8165/8170 (Rosetta/shear), klein shadow master theorem, HYP-9055 (palindrome layer), HYP-7960 (deficit halving), TOURNAMENT-INVARIANT-ZOO-ATLAS.
