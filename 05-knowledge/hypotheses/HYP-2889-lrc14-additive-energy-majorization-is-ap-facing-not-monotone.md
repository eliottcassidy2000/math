...
## S105 addendum: labelled compression has a Bruhat carrier

HYP-2891 turns the local-compression warning into a concrete graph target.  The
truncated-octahedral graph is the Bruhat graph on `S4` generated by adjacent
swaps.  It has two edge orbits: outer swaps `s1/s3` and middle swaps `s2`.
Therefore local compression should be audited by Bruhat face type, not by an
unlabelled "move toward AP" scalar.  The companion Clebsch folded-cube quotient
gives the tangent residual-mask covariance carrier for the signed sector
remainder.
