...
status: PARTIALLY REFUTED / CORRECTED by MISTAKE-497 and MISTAKE-498. The exact Dedekind and margin identities survive, but eta_sig=-4s up to orientation, not s; 1/2 is the twisted value L_7(-1,omega^2), not the trivial-character zeta_7(-1); the exact B2-remainder wording is retracted; and the f14/local-global synthesis remains an OPEN analogy. THM-2057 separately proves the exact LRC ray; THM-4089 is unrelated.
...
> **CORRECTION (MISTAKE-497/498).** The historical text below is
...
> `182/549`, not `-91/1098`. The rational value `1/2` is
> `L_7(-1,omega^2)=(1-7)zeta(-1)`, on the Teichmuller-twisted branch;
> the standard trivial-character `zeta_7(-1)` has 7-adic valuation `-1`
> and lies in `4/7+Z_7`. The exact `B_2` Euler--Maclaurin-remainder wording
> is also retracted. Only the exact Dedekind/margin formulas, their corrected
> spectral factor, and the asymptotic relation to `zeta(-1)` survive. The
> claim that their mismatch is exactly the `f_14` residual is **OPEN**.
