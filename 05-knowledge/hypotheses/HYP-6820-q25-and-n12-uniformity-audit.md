...
progress_note: >-
  ...
  joint-cut and scalar-owner scale bound c<=840, and a finite exact recursion for every
  ...
  THM-957 classifies the c=4 sheet stalk into 64 directed-cycle supports and
  ...
  also empty. THM-958 then exhausts all 14,414,400 admissible scale-five
  order/unit contexts and finds no common-sheet survivor. Independently, sheet
  ...
  Hereditary lcm supplies at least two such owners.  THM-1072 closes c=28 by
  retaining the order-two/order-four masks as whole fibres of
  `Z/28 -> Z/4` and independently maximizing every transverse contribution
  outside that anchor.  The complete scalar census leaves 3,170 rows on 206
  supports, but the sound relaxed owner histogram is `0:2018,1:912,2:240`;
  no row can cover all six owners.  An independently developed C++ literal-CRT
  referee agrees with two Python implementations on every decisive count and
  on all 6,628,500 exact reachable-mask incidences.  Scale 29 is
  prime-excluded by THM-983.  THM-1090 then closes c=30 with complementary
  quotient flags.  Among 171,118,332 labelled support/order contexts, scalar
  capacity leaves 54,050 rows.  Retaining the masks with `D|6` reduces these
  to 120 all-owner rows; retaining `D|10` on that residual gives all 720 owner
  bounds in `23..28`, strictly below 30.  The exact DP maxima lie in `23..27`.
  An independently developed standard-library C++ literal-CRT referee agrees
  with the Python primary on the grammar, scalar bank, both quotient passes,
  and all 3,401,088 exact reachable-mask incidences.  Scale 31 is
  prime-excluded.  THM-1096 closes c=32: its two-adic grammar has 12,281 words
  and 11,347,644 labelled support/order contexts; scalar capacity leaves 3,450
  rows on 284 supports.  Retaining every `D|8` mask as an exact thick-fibre
  anchor gives live-owner histogram `0:2802,1:456,2:192`.  An independently
  developed literal-CRT C++ referee agrees on every theorem-bearing count and
  additionally constructs all 403,733,376 exact reachable masks.  THM-1124
  then closes c=33.  Its squarefree grammar has 3,249 words and 3,002,076
  labelled support/order contexts representing 1,171,996,056,000 raw unit
  states.  Scalar capacity leaves 1,344 rows on 150 supports.  Retaining
  every `D|3` mask exactly gives a sound `Z/3` owner bound at most 29 whenever
  the owner's own order carries eleven; hereditary lcm supplies at least two
  such owners.  The quotient preserves owner-local upper bounds but destroys
  the shared nonanchor unit word and mutual needle overlaps; those losses are
  safe only because the certificate is an upper relaxation.  The frozen
  Python primary and a separately developed standard-library C++20
  bounded-literal-CRT referee agree on every proof-facing count and replay
  byte-for-byte in normal/`-O` and O3/O0 modes.  THM-1125 then closes c=34.
  Its squarefree grammar again has 3,249 words and 3,002,076 labelled
  support/order contexts, now representing 1,271,272,375,296 raw unit states.
  Scalar capacity leaves 552 rows on 36 supports.  Retaining every `D|2`
  mask as an exact union of the two fibres of `Z/34 -> Z/2`, then maximizing
  every order-17/order-34 contribution independently outside that anchor,
  gives all 3,312 owner bounds in `25..29`, strictly below 34.  Thus every
  scalar survivor fails at every owner.  The algebraic/literal-CRT Python
  primary and separately developed bounded-literal-CRT C++20 referee agree on
  every proof-facing count and replay byte-for-byte in normal/`-O`, O3/O0,
  and sanitized modes.  THM-1249 then closes c=35.  Its squarefree grammar
  has 3,249 hereditary words and 3,002,076 labelled support/order contexts;
  scalar capacity leaves 216 rows on 24 supports.  Retaining every order-one
  and order-five mask as an exact thick-fibre anchor over Z/5, then maximizing
  every order-seven/order-thirty-five contribution independently outside the
  anchor, gives all 1,296 surviving owner bounds in 27..31, strictly below 35.
  Normal and optimized exact runs agree, and the generic terminal ceiling is
  a sorry-free Lean consumer.  The next untreated common scale is c=36.
...
sharpens this to `min D_i<=72`.  Joint enumeration of all 63 cuts leaves 8,449
normalized order words and 18,405 multiplier rows, with order-only cap 1,120.
All four scale-1,120 extremals, all twelve scale-1,008 rows, and all twenty-three
scale-882 rows fail the exact scalar owner-capacity condition across 2,162,160
normalized label assignments.  The next arithmetic scale is 840; hence
`c<=840`.  Range zero at a
large prime means a common factor, not prime absence; individual H6 orders are
not asserted smooth.  Every fixed
...
THM-957 closes the next bank as well.  At `c=4`, exact common-sheet exhaustion
...
common-scale face is killed even earlier: THM-958 scans all `14,414,400`
admissible `c=5` order/unit contexts and finds zero common-sheet survivors.
The independent reduction says why.  Sheet capacity excludes at most four
order-five colours, the five-colour case would require a five-clique in
`K_6,6`, and the six-colour case reduces to 64 signed-doubling supports.  On
...
- in the shallow packet, missing-owner splice sheets joined to replacement
  ...
  prime-power upper-set layer, jointly optimizes all 63 relative cuts, excludes
  every arithmetic row at scales 1,120, 1,008, and 882 by scalar owner capacity,
  ...
  every remaining labelled ray. THM-957 then classifies the `c=4` sheet fibre
