...
depends_on:
  ...
  - THM-776   # exact scale-free f=4 transverse-fragmentation falsifier
  - THM-777   # invariant-level good-set-state transverse-tooth cap
...
## Exact infinite-family falsifier (THM-776)
...
3. **It occurs inside the covering endgame.** Every `V_N` is primitive because it
   ...
   LRC counterexample: THM-776 proves the top peel fires THM-755's capped-envelope
...
## A terminal transverse face (THM-777)
...
the theorem-facing load `r/(aN|G'|)` bounded. For THM-776, `L=1/1540`,
...
- THM-776 proves that the entire chart-native prime family has an empty top band;
