...
depends_on:
  ...
  - THM-794   # exact scale-free f=4 transverse-fragmentation falsifier
...
## Exact infinite-family falsifier (THM-794)
...
3. **It occurs inside the covering endgame.** Every `V_N` is primitive because it
   ...
   LRC counterexample: THM-794 proves the top peel fires THM-755's capped-envelope
...
## A terminal transverse face (THM-795)
...
`r_top/(aN|G'|)`. For THM-794, `L=1/1540`,
`a=1092`, and the exact crossing is `11734415/9278<1265`.

More globally, THM-795 proves a uniform `412`-lacunary terminal cone in every
...
- THM-794 proves that the chosen top peel `1092N` lies above its band edge for
...
The scale statements now form a precise logical staircase. THM-794 shows
that component topology is not compact modulo `c*`. THM-795 shows that a fixed
