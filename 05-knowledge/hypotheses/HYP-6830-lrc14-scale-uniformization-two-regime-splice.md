...
depends_on:
  ...
  - THM-798   # exact scale-free f=4 transverse-fragmentation falsifier
  - THM-799   # invariant-level good-set-state transverse-tooth cap
...
## Exact infinite-family falsifier (THM-798)
...
3. **It occurs inside the covering endgame.** Every `V_N` is primitive because it
   ...
   LRC counterexample: THM-798 proves the top peel fires THM-755's capped-envelope
...
## A terminal transverse face (THM-799)
...
`r_top/(aN|G'|)`. For THM-798, `L=1/1540`,
`a=1092`, and the exact crossing is `11734415/9278<1265`.

More globally, THM-799 proves a uniform `412`-lacunary terminal cone in every
...
- THM-798 proves that the chosen top peel `1092N` lies above its band edge for
...
The scale statements now form a precise logical staircase. THM-798 shows
that component topology is not compact modulo `c*`. THM-799 shows that a fixed
