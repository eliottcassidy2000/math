---
id: HYP-2115
status: DESIGN + PARTIAL RESULT, core FORMALIZED in Lean (math-lean 460e772, sorry-free) —
  bounded CRT residue automaton cross-checked 3 ways; resonance bound |D|<=u_b K_a+u_a K_b
  (generalizing Lemma C) + rigidity + Lemma C + lemmaC_no_fit machine-checked; SHARPENING — the
  isolated owner-congruence system is satisfiable, so looseness needs config-level constraints;
  clarity exposed that the bound is j-free (integrality-refined bound = the next extension)
source: claudebox-2026-06-03-S581 (formal follow-up S581b)
related:
  - HYP-2105
  - THM-398
  - HYP-2107
...
# HYP-2115: the large-owner residual is a bounded CRT residue automaton
...
## FORMALIZED in Lean (math-lean `460e772`, sorry-free)

`Math/LonelyRunner/OwnerCongruence.lean` machine-checks the arithmetic core, all over `ℤ`:

- `feasible_elim` — the `j`-elimination `w·D = u_b r_a − u_a r_b`.
- `resonance_bound` — `|D| ≤ u_b·K_a + u_a·K_b` (`K_u = ⌊(u−1)/n⌋`); `resonance_bound_clean`
  — `n·|D| < 2 u_a u_b`.
- `rigidity` — small owner (`u ≤ n`) ⇒ `r = 0` (endpoint at centre).
- `lemmaC` — both owners small ⇒ `D = 0` (the cross-relation `a = b`); `lemmaC_no_fit` — a
  *nondegenerate* (`a < b`) small-owner component fits **no** arc (full Lemma C modulo
  connectivity).

**What the clarity exposed (the extension).** The Lean proof of `resonance_bound` uses ONLY
`w ≥ 1`, the two window bounds, and `u_a,u_b > 0` — it **never uses that `j` is an integer**,
nor any congruence. So the headline bound is a **`j`-free / congruence-blind** necessary
condition; it cannot, even in principle, see the integrality that separates a real `G(S')`
component from an arbitrary owner tuple (exactly why §[E]'s 1590 isolated tuples are "feasible").
And `lemmaC_no_fit` made precise that Lemma C's only non-arithmetic input is the nondegeneracy
`a < b`, with just "fits-no-arc ⇒ loose" (connectivity) left informal. **Two concrete next
formalizations follow:** (i) the **integrality-refined bound** — add the congruences
`w(k_a n+1) ≡ r_a (mod u_a)`, `w(k_b n−1) ≡ r_b (mod u_b)` to get a strictly sharper necessary
condition (the CRT content the resonance bound discards); (ii) the **valid-config predicate** as
a Lean structure, toward `accept ∩ valid = ∅`. The formalization shows these are the *only* two
missing arithmetic ingredients — the rest of the residual proof is done.

## Open / next

- **(formal, new)** the integrality-refined bound + the valid-config predicate (above).
...
**Artifacts:** formal — `claude-monad/math-lean` `Math/LonelyRunner/OwnerCongruence.lean`
(commit `460e772`); informal — `04-computation/lrc_largeowner_crt_automaton_s581.py` (+`.out`),
