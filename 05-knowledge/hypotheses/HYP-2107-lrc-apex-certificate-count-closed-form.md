...
# HYP-2107: the apex-certificate count has closed form `(q-1)(q-1-|S|)`
...
### Apex trichotomy + the lift, also formalized (math-lean `5edc79a`)

The single-runner forbidden set is now fully classified (all `sorry`-free):

- **Trichotomy.** `forbidden_univ_iff` (whole plane ⟺ `a=b=c=0`, the apex), `forbidden_empty_iff`
  (empty ⟺ `a=b=0, c≠0`, inconsistent), and `card_forbidden_eq` (the **transversal** case
  `(a,b)≠0`: a *proper line* with `Nat.card = |K|`, codimension 1). The apex is exactly the
  runner that fails transversality.

- **The lift (`card_apex_lift`).** This formalizes "the `r/p` lift restores codimension 1."
  Lift to three coordinates (`Forbidden3 (a,b,d) c`). The degenerate apex covector `(0,0)` —
  the whole plane in `𝔸²` — becomes, in `𝔸³` with the lift coefficient `d ≠ 0` (the apex
  speed mod `p`, a unit), a *proper hyperplane*: `Nat.card (Forbidden3 0 0 d 0) = |K|²`, i.e.
  a `1/q` slice of `|K|³` (`card_apex_lift_codim`). The *same* zero covector that was the
  whole plane in 2D is codimension 1 in 3D. This is the certificate-level form of S559's
  apex-lift: the obstruction is not removed but made transverse by one more coordinate.

## What this adds over HYP-2101

HYP-2101 (codex-S579) frames the n=14 proof as *gluing* cheap-pair certificate germs across
the mod-7 apex seam (a sheaf-section existence question). HYP-2107 is the **counting**
...
(count `ed9aefd`; trichotomy + lift `5edc79a`); informal —
`04-computation/lrc_apex_lift_certificate_sheaf_s579.py` (+`.out`). Builds on HYP-2101,
HYP-2063/S559, arXiv:2604.23906 Prop 4.1.
