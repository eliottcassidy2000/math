...
status: OPEN — THM-771 proves the exact seven-owner defect and unramified event pierce; THM-794 refutes raw switch/extent compactness; ramified decks, diagonal-holonomy-quotiented r>=8 core incidence, and the rho-floor remain
...
depends_on:
  ...
  - THM-794   # full-support packet holonomy and extent refuter
  - HYP-6830  # the splice; complementarity refuted, ratio invariant measured
related: [THM-745, THM-754, THM-778, THM-779, THM-783, THM-784, HYP-6785, HYP-6820, HYP-6840, MISTAKE-146, MISTAKE-147, MISTAKE-149]
---
...
the remaining exit lemma compares a centered mechanical owner word with a
finite algebraic hop word **after both persistent stalks and central packet
return cycles are quotiented**.
...
one core-safe interval, even in a divisor-complete family.  THM-794 goes
further: its full visitor packet has a genuine switch at every wall and repeats
arbitrarily at fixed fastest ratio, while returning only by diagonal sheet
translation.  The corrected residual counts normalized collision-state/SCC
transitions after both free actions are factored.  On the positive side,
...
persistent-stalk refuter has visitor-free fast-owner periods, while THM-794's
refuter has all seven visitors every period.  Their correct role is to
constrain noncentral collision transitions and to pierce metric components in
the stated conditional classes.

**THM-786 scope-corrected extent update.**  One fixed companion can serve `L`
consecutive second-fastest-owner walls only if
`L<1+2gc/(f(g-c))`; the endpoint-span proof allows skipped indices.  Consecutive
visitor sets obey the signed difference identity `sum entrants^{-1}=sum
leavers^{-1}`.  Summing companion wall densities gives an explicit bound when
the other six speeds have sum `<g`, not merely `<f`.  The general-density
alternating-companion marginals remain useful after central holonomy is split
off, but THM-794 refutes the proposed universal extent bound and makes active
periods unbounded at `ceil(f/g)=2`.  The reported `0.589` census is not
regenerated by the stored S304 script and missed this structured family.
