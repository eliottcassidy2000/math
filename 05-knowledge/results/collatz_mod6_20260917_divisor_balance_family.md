...
**Support bounds (PROVED).** `alpha in {0,1}`: nonsquarefree forces `3*2^(r-1) <= T(r)`, giving `r <= 0,1,2,2` for `(0,beta)`, `beta = 0..3`, and `2^(r-1) <= 1 + beta r`, giving `r <= 1,3,4,5` for `(1,beta)`. `alpha in {2,3}`, `(alpha,beta) != (2,0)`: `T/2^r <= alpha + 3r/2^r <= 4.5` (the true maximum, asserted by the script, is `4.25` at `(3,3)`, `r = 2`), so `k <= 3`; for `r >= 4`, `c_r := beta r + 2 - alpha` satisfies `0 < |c_r| <= 3r+1 < 2^r` (it vanishes only in cell `(2,0)` or at `r = 1`), so `v_2(T) = v_2(c_r) <= log2(3r+1)`; with `2^(r-k) | T` this gives `r <= 3 + log2(3r+1)`, i.e. `r <= 7`. Within these bounds the solutions are the multiplicative partitions of `T(r)` into `r` parts `>= 2` (finite, enumerated exactly, no exponent cap).
...
Run from the repository root (the script uses no relative paths). The regenerated `.out` ends with its own resource stamp, `[time total 36.2s, peak RSS 663 MB]` on the mac-mini (numpy sieve to `10^7`, pure-python direct audits to `2*10^5`); the `[time` lines are the only lines that vary between runs and are excluded from the `-O` comparison. Independent audit companions from wave one: `04-computation/experiments/collatz_mod6_20260917_divisor_balance_family_audit_recompute.py` and `..._audit_proof-audit.py`.
