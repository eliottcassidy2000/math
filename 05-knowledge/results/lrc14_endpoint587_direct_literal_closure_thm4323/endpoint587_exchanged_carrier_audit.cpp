...
// the newly derived endpoint-587 typed frontier. Scratch only.

#define ENDPOINT590_QUOTIENT_MAIN endpoint590_quotient_hidden_for_endpoint587
...
namespace {
constexpr u64 kTop587Fnv = UINT64_C(0xf48ca5f1904d6f52);
constexpr u64 kAddition9Fnv = UINT64_C(0xd1cf49e4b811b958);
constexpr u64 kDeletion9Fnv = UINT64_C(0x3546eb56552b4cde);
constexpr u64 kExchangedCarrier587Fnv = UINT64_C(0xeeae5518d84ccac5);

std::vector<u32> read_delete9_587(const std::filesystem::path& path) {
...
#ifndef ENDPOINT587_EXCHANGED_CARRIER_AUDIT_MAIN
#define ENDPOINT587_EXCHANGED_CARRIER_AUDIT_MAIN main
#endif

int ENDPOINT587_EXCHANGED_CARRIER_AUDIT_MAIN(int argc, char** argv) {
    try {
        require(argc == 22,
                "usage: endpoint587 JOINT BASE8951 ADD45 SUFFIX9 UNIVERSE "
                "OLD_UNION REPAIRS76 ADD4 DELETE73 ADD10 FINAL_DELETE "
                "ADDITION593 DELETE593 COVER43 DELETE43 TOP587 COVER9 "
        ...
        const std::vector<u32> deletions = read_delete9_587(argv[18]);
        ...
        require(carrier.size() == 3925 && rank8 == 3809 &&
                    masks_fnv_agent(carrier) == kExchangedCarrier587Fnv &&
        ...
        const auto row_set = read_pairs_repaired(argv[16], 10, kTop587Fnv);
        std::vector<PairAgent> rows;
        Fnv row_ledger;
        for (const auto& [q, r] : row_set) {
            require(r == 587, "frontier row escaped endpoint587");
            rows.push_back({q, r});
        ...
        require(pair_out && failure_out, "cannot create endpoint587 ledgers");
        ...
        require(pair_out.good() && failure_out.good(),
                "endpoint587 ledger write failed");

        std::cout << "LRC14_ENDPOINT587_EXCHANGED_CARRIER_AUDIT_V1\n"
        ...
        std::cerr << "ENDPOINT587_AUDIT_ERROR " << error.what() << '\n';
        return 1;
    }
}
