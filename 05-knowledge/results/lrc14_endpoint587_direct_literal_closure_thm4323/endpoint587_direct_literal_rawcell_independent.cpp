// Independent raw-cell replay of the endpoint-587 direct literal audit.
...
constexpr int kR = 587;
constexpr u64 kExpectedFailureFnv = UINT64_C(0xbae65dc3d3bd34d0);
constexpr u64 kExpectedBodyCount = UINT64_C(41);
...
WallModel make_wall_model(int q) {
    ...
    model.grid = lcm_exact(model.grid, i64{14} * kR);
    ...
    for (std::size_t i = 1; i < cut.size(); ++i) {
        ...
        if (!is_safe(q, model.grid, a, b) ||
            !is_safe(kR, model.grid, a, b)) continue;
        u32 failed = 0;
...
FailureInput load_failures(const std::filesystem::path& name) {
    ...
    while (std::getline(file, record)) {
        ...
        demand(r == kR && wide < (u64{1} << 30), "failure escaped universe");
    ...
    demand(file.eof() && input.count == kExpectedBodyCount &&
               fingerprint.value == kExpectedFailureFnv,
...
int main(int argc, char** argv) {
    try {
        demand(argc == 4, "usage: independent FAILURES DETAIL_CSV THREADS");
        const unsigned threads = std::stoul(argv[3]);
        demand(threads >= 1 && threads <= 64, "thread count escaped 1..64");
        const FailureInput failures = load_failures(argv[1]);
        ...
        Fnv64 global;
        std::cout << "LRC14_ENDPOINT587_DIRECT_LITERAL_RAWCELL_INDEPENDENT_V1\n"
        ...
        for (const auto& [q, bodies] : failures.row) {
            ...
            for (std::size_t i = 0; i < bodies.size(); ++i) {
                ...
                ledger.word(q); ledger.word(kR); ledger.word(i);
        ...
        std::cerr << "ENDPOINT587_RAWCELL_INDEPENDENT_ERROR " << error.what()
                  << '\n';
        return 1;
    }
}
