...
* **UPDATE (HARD-class lane, 2026-09-23; [note](collatz_procgen_20260922_hard_class.md)):**
  * Theorem R's exact reach is `Dio(w) > eta(w)` (Theorem D).
  * With Bugeaud–Kim's `Dio >= 2.50994`, Theorem S extends to every map with `eta < 2.50994`, including **every slope of `5x+1`** (not only `alpha < 0.804`), and to quasi-Sturmian words.
  * The description of HARD as "positive entropy" is REFUTED. The zero-entropy square-swap word has `Dio = 1`, is out of reach of periodic approximants, and falls to a 2-adic Tschakaloff–Padé argument (Theorem Y). The cube-swap word `Y3` is the smallest open instance.
