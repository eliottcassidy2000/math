// Independent finite-exact audit of the rank-at-most-two wall skeleton.
//
...
i64 exact_lcm(i64 a, i64 b) {
    ...
    demand(candidate <= std::numeric_limits<i64>::max(), "LCM overflow");
...
std::vector<Pair> read_pairs(const std::string& path, u64& raw_fnv) {
    ...
    demand(pairs.size() == 22647, "pair-universe cardinality changed");
    return pairs;
}
...
class PairEvents {
  public:
    PairEvents(int speed, i64 grid) : speed_(speed), grid_(grid) {
    ...
    i64 next() const {
        ...
        const i64 numerator = 14LL * tooth_ + (entering_ ? 1 : 13);
        return numerator * unit_;
    }

    void cross(i64 position, bool& safe) {
...
struct Graph {
    i64 grid = 0;
    i64 rank0 = 0;
    std::array<i64, 30> rank1{};
    std::array<std::array<i64, 30>, 30> rank2{};
    std::array<i64, 30> degree{};
    i64 total = 0;
    u64 cells0 = 0;
    u64 cells1 = 0;
    u64 cells2 = 0;
...
Graph event_sweep_graph(int q, int r,
    ...
    const i64 scale = graph.grid / kBaseGrid;
    ...
    while (left < graph.grid) {
        const i64 pool_next = pool_cursor < base_events.size()
                                  ? base_events[pool_cursor].position * scale
                                  : graph.grid;
        const i64 right = std::min({graph.grid, pool_next,
    ...
    for (unsigned u = 0; u < 30; ++u) {
        for (unsigned v = u + 1; v < 30; ++v) {
            const i64 weight = graph.rank2[u][v];
            graph.total += weight;
...
Coarse coarse_bound(const Graph& graph) {
    ...
    coarse.ticks = 63 * coarse.mass - 4 * static_cast<i128>(graph.grid);
    return coarse;
}
...
int main(int argc, char** argv) {
    try {
        ...
        std::vector<std::pair<Pair, Graph>> hostile;
        ...
        for (const Pair pair : pairs) {
            ...
            for (i64 weight : graph.rank1) mass1 += weight;
            for (unsigned u = 0; u < 30; ++u)
                for (unsigned v = u + 1; v < 30; ++v)
            ...
            if (!positive) {
                ...
                hostile.push_back({pair, graph});
            }
            std::ostringstream row;
            row << pair.q << ',' << pair.r << ',' << graph.grid << ','
                << graph.cells0 << ',' << graph.cells1 << ',' << graph.cells2
                << ',' << graph.rank0 << ',' << dec(mass1) << ',' << dec(mass2)
                << ',' << graph.total << ',' << dec(coarse.degree_sum) << ','
        ...
        for (const auto& item : hostile) {
            const Pair pair = item.first;
            const Graph& graph = item.second;
            const Coarse coarse = coarse_bound(graph);
            ...
            const i128 minimum_ticks =
                63 * minimum_mass - 4 * static_cast<i128>(graph.grid);
        ...
        std::cout << "LRC14_RANK2_CLEANROOM_EVENT_SWEEP_AUDIT_V1\n";
        ...
        std::cout << "SCOPE FINITE_EXACT_22647_PAIR_RANK2_WALL_AUDIT_NO_PHYSICAL_ENTRY_NO_LRC14\n";
        std::cout << "VERDICT "
                  << ((pairs.size() == 22647 && coarse_positive == 22540 &&
                       hostile.size() == 107 && exact_positive == 107)
                          ? "PASS"
                          : "FAIL")
                  << "\n";
        return (pairs.size() == 22647 && coarse_positive == 22540 &&
                hostile.size() == 107 && exact_positive == 107)
                   ? 0
                   : 2;
