...
**n = 24 (FINITE-EXACT; former forcing proof RETRACTED).** There is only one original leaf, `18`. The earlier claim of 21 forced edges in round 1, the ensuing seven deletions, and the second-round three-leaf contradiction relied on false R2. They do not constitute a proof. The corrected reduction forces only `7-18` and returns `OPEN`. No cut with `|S| <= 3` was found. Nonexistence is still established in the explicit finite universe by the independent exhaustive searches S2 and A2, both giving zero paths. A short structural certificate remains OPEN.
...
[CORRECT (data)]         N=24 stalled                  -> zero paths by independent exhaustive searches; forcing proof RETRACTED
