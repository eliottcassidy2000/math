...
* **Not claimed.** THM-4476's `eps` is now a power of the logarithm (addendum 1.6b of the thin-divergence note: `N(X) <= K X^(h*) (log_2 X)^a` for every `a > 1 + lambda*/h* = 1.5138`, the same recursion with `theta = (1+eta) log_2 log_2 X/(h* log_2 X)` and `h(rho) <= h* + lambda* theta`); a better power needs a uniform ballot factor at a moving barrier. The `gamma < 1` polynomial (expected `log^(-1/2)`) is open. The observed constants (`W_k k^(3/2) 2^(-hk)` about `10-11`) are far inside `[0.26, 545]`.
