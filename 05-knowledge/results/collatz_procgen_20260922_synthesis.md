...
* **Not claimed.** THM-4476's `X^(h+eps)` keeps its `eps` (a dichotomy-bootstrap loss; a polylog version is a target). The `gamma < 1` polynomial (expected `log^(-1/2)`) is open. The observed constants (`W_k k^(3/2) 2^(-hk)` about `10-11`) are far inside `[0.26, 545]`.
