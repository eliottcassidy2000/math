...
## 2n. Wave 18 (2026-09-26, opus session `collatz-crossings-20260926`): the orbit-coupled potential on crossings, the 1/3-2/3 analogy, and three decoded seeds

* **The attack and its result.** Three orbit-coupled quantities were made exact for a single divergent orbit: the phases `{log_2 m_l}` are an irrational rotation by `log_2 3` up to a convergent shift (Benford along every non-eventually-periodic orbit; equidistribution discrepancy `0.005` on `980` iterates); the two-place clock `Q_l = m_l 2^(Delta_l) = 2^(d_l) m_l/3^l` is monotone with `|Q_l|_2 = 2^(-d_l)`, `|Q_l|_3 = 3^l` and real increments `Q_l/(3 m_l)`; and `m_l = 2^(-d_l) S_(l-1) mod 3^l` once `2^(d_l) > Q_l` (the past is the `3`-adic address as the future is the `2`-adic one). Stays below `X` of length `>= log_2 X` are ballot-thin (`<= 2 M_k(1.6)`, exits lemma), record orbits cross each dyadic level only `2`-`5` times, and the whole difficulty is the *length* of a stay, which no residue-class count controls (a class realises any finite stay) and no two-place potential sees (it moves by `O(1/m)` per step). Missing: an oscillation lemma for one orbit, stay length `<= C (log X)^beta (leaders inside + 1)`; with leaders ballot-thin (S7), `beta = 0` gives the floor. THM-4506 (procgen, same day) proved the recursion saturates at `a*(mu) = mu lambda*/h* - 3/2`, so this lemma is the whole remaining content of the one-window method.
