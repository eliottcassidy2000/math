...
**1c. THM-4499 (PROVED): thin divergence is `o(X^(h*))`.** The moving-barrier count `M_k(y) = #{words staying above -y}` is at most `D_s 2^(hk) k^(-3/2) 2^(lambda* y) e^(sy)` (decompose at the minimum into a negative word with total `> -y` and a positive word; count negative words by the weighted Spitzer identity, whose bijective proof tolerates multiplicative letter weights, and THM-4495's convolution lemma; the tilt pays `2^(lambda* y)`). THM-4476's counting lemma then carries the ballot factor `(log X)^(-3/2)` at the barrier `theta_X log_2 X`, and the bootstrap with `theta_X = (1+eta) log_2 log_2 X/(h* log_2 X)` gives `N(X) <= K X^(h*) (log_2 X)^a` for every `a > lambda*/h* - 3/2 = -0.9862`. So no divergent orbit (and no injective invariant set) attains the Terras exponent; the expected `O(log X)` is still out of reach (one free window per element, landing multiplicity `L`).
...
## 2n. Wave 18 (2026-09-26, opus; corrected by crossroads integration): crossings and adjacent constructions

* **The attack and its result.** Under THM-4476, phases of successive odd values shadow a fixed irrational rotation. The carry clock Q_l is monotone and bounded on a nonperiodic orbit; its3-adic norm is3^l for l>=1, with the initial valuation retained separately. The correct reciprocal comparison is3 log(Q_infinity/n)<sum1/m_l<=4 log(Q_infinity/n). The past residue address is exact. Completed exits on an injective orbit admit the ballot count; the endpoint-corrected whole-orbit oscillation estimate remains OPEN. See the corrected crossing note for quantifiers.
* **1/3--2/3.** Positive-prefix-multiplier words, a sufficient subset of actual no-descent words, form width-two posets (THM-4503). The finite census has varying balance constants:1/3 at(5,4),(6,4),3/7 at(7,5),7/15 at(10,7), and1/2 in some cells. Neither all-large-cell balance1/2 nor an information-optimal order-comparison code is proved. Spitzer's entropy formula concerns the SUM of cell counts. The law on one fixed integer remains the missing coordinate.
* **Seeds.** Wythoff AA/B/AB is a candidate matching the first34 supplied colours and disagreeing at35. The user's sequence is authoritative; it is not silently repaired. Reported mutual information is finite evidence. The prime set{2,3,11} has the exact odd-square-bracket mechanism. The angle-doubling coordinate applies specifically to x^2-2. The correct maximal-rise invariant is log_2(n+1)+log_2(3/2)v_2(n+1), with reset costs still present.
* **Subsequent audited extension.** [Crossroads crossing synthesis](crossroads_crossing_20260926_board.md) gives finite-height crossing certificates, exact binary sibling reductions, and THM-4507's obstruction for arbitrary nonlinear finite polynomial-valuation potentials. Its [integration audit](crossroads_crossing_20260926_integration.md) preserves the correction lineage.
...
## 2o. Wave 19 (2026-09-26, opus session `collatz-oscillation-20260926`): the oscillation lemma as shell revisits, and Gilbreath's triangle as an automaton
