...
**Status:** PROVED (scoped, elementary): the corrected core local identity and the unique solution of the two flow equations (section 2); the d-dimensional simplex convolution law containing the tetrahedral and pentatope laws (section 3); the unique symmetric reading of the g-operator (section 4); the double-counting identity `sum_T h(T) = n! 2^{T(n-2)}` and the resulting cost bound for the proposed tournament code (section 5); the residue law "every shortcut peak of an odd seed is 2 mod 6" and the ladder `C^j(a 2^k - 1) = a 3^j 2^{k-j} - 1` (section 8). FINITE-EXACT: all grids, censuses and tables below (`[0,11]^2` simplex checks through `d=8`; labelled tournaments `n<=6`; Bang/Zsigmondy tables `n<=40`; Cipolla through `p=23`; 49,999 odd seeds `<=10^5`). CITED: Bang 1886 / Zsigmondy 1892, Mihailescu 2004 (Catalan), Cipolla 1904, Redei 1934, Shannon 1948, Poonen 1998, Bott 1959. REFUTED (minimal witnesses in the table): the stated core identity, the g-operator readings, "path edges carry 0 bits", the 341 bottleneck, the peak endpoints 1/26/80, the 4k+1 square tracking, "196 = sum of the first 12 primes", and "growth bound by the area of squares". SCOPE (no map found): every "physical insight", the horizons, the unified cycle, Bott/tetration, the quasicrystalline Fourier transform, the Wythoff diffraction. OPEN: exactly what the [blueprint audit](collatz_blueprint_20260921_synthesis.md) left open, a positive-margin word for every fixed `n>1`. No novelty claim for any of the surviving identities (Chu-Vandermonde, Redei, Bang, Cipolla, the inherited descent certificate); the one small new exact observation is the ladder explanation of the square-minus-one peaks (section 8), which is elementary. Results note of session collatz-mod6-20260917 (machine mac-mini), lane `scaffolding_audit`, written 2026-09-21; not a reserved canon ID.

## Inheritance and concept board

The pasted block is the scaffolding of the geometric blueprint whose proof claim was REFUTED in the [blueprint synthesis](collatz_blueprint_20260921_synthesis.md), with the exact affine/carry model in its [affine companion](collatz_blueprint_20260921_affine.md) and the [source document](../reference/COLLATZ-BLUEPRINT-2026-09-21-SOURCE.md). Everything Collatz-arithmetic here is inherited and cited, not re-derived: the inverse-fibre braid `R(n)=4n+1`, `(B1)-(B3)` of the [first Collatz note](arithmetic_braids_20260917_collatz.md); the descent certificate `2^K T^L(n) = 3^L n + B` of the blueprint synthesis, section 6; the Catalan reading of Bang's exception `2^6-1=7*9` from the wave-one [zsigmondy lane](collatz_mod6_20260917_zsigmondy_triad.out); the rational 3-cycle of `x^2-29/16` from [THM-4139](../../01-canon/theorems/THM-4139-rational-three-cycle-order-six-lift-and-horizontal-carrier.md) and [THM-4146](../../01-canon/theorems/THM-4146-rational-three-cycle-order-six-lift-horizontal-divisor-fibre-firewall.md); the tournament `h`-spectrum holes `{7,21}` and `|Aut| | h` from [THM-1745](../../01-canon/theorems/THM-1745-leaf-graded-arborescence-filtration-721-shadow.md). **Closest proved mechanism:** the descent certificate itself, which the paste restates verbatim as `3^L n + B_L < 2^{K_L} n` and then asserts without argument. **Canonical hostile:** the ladder seed `2^k-1`, which climbs to `3^k-1` (section 8); it refutes "peaks bound by area" and, at the same time, is the whole explanation of the only real signal in the paste's square-tracking data. **Corrected near miss:** the core local identity is off by exactly one in both triangular arguments; with both shifted it is a true polynomial identity and the paste's own flow equations are its partial differences. **Least-used sidecar:** the residue `p = 2 mod 6` of every shortcut peak, which halves the square windows the paste can even hit and forces `3|s` in the `s^2-1` window. **Typed analogy (ladder -> square-minus-one peaks):** source = seeds `a 2^k - 1` with `a` odd; target = shortcut peaks; map = `C^k`; preserved = the exact value `a 3^k - 1`, a square minus one when `a=m^2` and `k` is even; lost = whether that value is the orbit maximum (needs the later descent, checked per seed); sidecar = `v_3(s)`; test = the `s^2+1` window, which has no ladder and is at chance level. **No map found (SCOPE):** from "shear as swept volume", "Bott periodicity", "quasicrystalline Fourier transform", "Wythoff diffraction", "unified arithmetic cycle", "inverse square horizon" to any defined mathematical object.
...
| II peaks track squares of `4k+1` | FALSE | bases 1 mod 4 and 3 mod 4 equally represented (46/47 and 49/50 distinct peaks); the only excess is the ladder (section 8) |
...
**Lemma 4.1.** Any `f` with `f(A,B)=AB f(A-1,B-1)` and `f(1,B)=B!` has `f(0,k)=k!` and `f(A,A)=(A!)^2`; the symmetric solution is `f(A,B)=A!B!`, which satisfies all three of the paste's statements at once. The alternative recursion `AgB=AB*(Ag(B-1))` gives `A^B B!`, which has `1gB=B!` but `AgA=A^A A!` (`A=2`: 8 vs 4); `(AB)^{AB}` satisfies neither recursion.
...
Every `h` is odd (Redei 1934) and 7, 21 never occur (THM-1745's holes, re-seen here for `n<=6`).
...
**341 (REFUTED).** `341=11*31=(4^5-1)/3=4^4+4^3+4^2+4+1`. `4^5-1=3*11*31` and both 11 and 31 have `ord_p(4)=5`: two primitive primes, so 341 does not "stall primitive prime generation". Zsigmondy's exceptions are `n=1` with `a-b=1`, `n=2` with `a+b` a power of 2, and `(2,1,6)`; since `4-1=3` and `4+1=5`, every `4^n-1` has a primitive prime (table for `n<=40` in the output; none missing). What is true about 341 is Cipolla (1904): for prime `p>=5`, `N_p=(4^p-1)/3` is a composite base-2 pseudoprime, `2^{N_p-1}=1 mod N_p`; verified for `p=5,7,11,13,17,19,23`, giving 341, 5461, 1398101, 22369621, 5726623061, 91625968981, 23456248059221 with their factorisations (proof sketch in the output: `N_p-1=4(4^{p-1}-1)/3` is divisible by `2p`, and `2^{2p}=3N_p+1=1 mod N_p`). Under `R(n)=4n+1` the orbit of 0 is `0,1,5,21,85,341,1365=(4^j-1)/3`, and `3(4^j-1)/3+1=4^j`: these are the odd predecessors of the powers of two, the trunk of 1 in the inherited inverse-fibre braid `(B1)-(B2)`. "A zero-friction state in `G_4`" is SCOPE.
...
## 8. Second part: Collatz peaks and squares (REFUTED, with the one real mechanism)
...
The `+1` window is at chance (99 distinct vs 81.8, Poisson `z=1.91`). The `-1` window has a real excess (93 vs 40.6), split 46/47 between bases 1 and 3 mod 4, so nothing selects `4k+1`. The excess is entirely Lemma 8.2: all 93 distinct `s^2-1` peaks are `a 3^k-1` for a ladder seed `a 2^k-1` (some `k<=2 v_3(s)`) whose orbit has that value as its maximum (explained 93, unexplained none). Examples: `8=3^2-1`, `80=9^2-1=3^4-1` (seed 15; also `53=27*2-1` on the orbit of 23), `6560=3^8-1` (21 seeds), `59048=3^10-1` (48 seeds), `164024=25*3^8-1=405^2-1` (17 seeds), `4782968=3^14-1` (17 seeds). The shifted-window control `{(4k+1)^2+s-1,+s,+s+1}` gives 23, 0, 24, 115, 91, 152, 248, 96, 0, 150, 150, 21, 21 for `s=-12,-10,...,12`: shifts 6 and 8 each score 150 seeds, so the window at `s=0` is not distinguished by anything except the `-1` ladder cell. Verdict: "the accelerated Syracuse paths track the boundaries of perfect squares as they scale" is REFUTED; the surviving exact content is Lemma 8.2, which is the hostile to the paste's own bound.
...
- The ladder `C^j(a 2^k-1)=a 3^j 2^{k-j}-1`, which both refutes bounded growth and accounts for every square-minus-one peak in the census.

## 12. What this means for a Collatz proof

Nothing in the paste advances the descent problem. The two arithmetic facts it states about Collatz, the certificate `3^L n+B_L<2^{K_L} n` and the trunk `(4^j-1)/3`, are already in the repo and are exactly the objects the blueprint audit identified as the correct replacements for the geometric language. The quadratic-horizon picture is refuted at the level of its own data (bases 1 and 3 mod 4 indistinguishable; the only structure is the `3^k-1` ladder, which is a growth mechanism, not a bound), and the "area" argument is not an argument about any specific `n`. Consistent with the [blueprint audit](collatz_blueprint_20260921_synthesis.md), the open obligation is unchanged and precise: for each fixed odd `n>1`, a word of its own orbit with `2^K>3^L` and `n>B/(2^K-3^L)`. Residue coverage, finite-word completion, and any density statement supply witnesses for prefixes, not this statement for a given `n`.
...
Stdlib plus sympy (factorisation only; every factorisation is re-multiplied). Exact integer, Fraction and polynomial arithmetic; all checks are explicit `raise`s and the `python3 -O` run is byte-identical to the normal run. Runtime about 20 s, memory far below 1 GB. Two checks in the first draft were corrected after computing the truth: the crude seed-weighted null of section 8 (replaced by the distinct-peak cell table, which shows the excess and its cause), and a stale assertion that 27 first descends at `L=41` (the truth is `L=37`; 41 is its total number of odd steps).

## Stopping boundary / next question

Stopped once every sentence of the paste had a verdict with a witness or a proof pointer and the only signal in its data was explained exactly. The next real question is the small one that came out of section 8: for which ladder seeds `a 2^k-1` is `a 3^k-1` actually the orbit maximum (equivalently, when does the orbit of `a 3^k-1` never exceed it)? This is a descent question in disguise and is left OPEN; no claim is made that it helps the global obligation.
