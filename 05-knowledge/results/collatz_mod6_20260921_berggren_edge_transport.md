...
|b|<=15; hypotenuse-<=10^6 triangle counts; 1,849,203 consecutive orbit triangles)
+ CITED (THM-3756, odd_square edges/synthesis notes, arithmetic braids)
+ REFUTED (the lead's "depth <= 1 above every k=1 edge": one witness, 3->5)
+ SCOPE (negative multipliers).  Nothing here bears on Collatz convergence or
cycle completeness; no literature-priority claim.**  Lane `berggren_edge_transport`
...
children all leave the multiplier 3 (x-type children shift a by +-2^(k+1),
the y-type child acquires the odd cofactor 2^(k-1)+3). Corrected near miss:
"the parent of 7->11 is not a legal edge" is orientation-dependent; the parent
pair (7,3) is the halving edge 7->3 of the multiplier-1 family E_{1,-1,1}, and in
the opposite orientation the 3x+5 edge 3->7, with the affine parameter law
b''=(x+3b)/2. Least-used sidecar: the B1-ray {(x,y): t=y fixed}, on which the
whole inverse fibre of a target y sits at the B1-heights 2^(k0-1)(4^j-1)/3.
...
| Inverse fibre of y | a B1-ray with t=y | the k=1 element needs one B2 |
...
An x-type child is always an E-edge (source x kept, exponent k kept, multiplier
shifted by +-2^(k+1), parameter b or -b). A y-type child keeps the target y and
...
a u - b = (2a - 2^k) y. An identity a' y + b' = 2^j u with y as source is
impossible: it would force a | 2^j (2^k +- 2a) hence a | 2^k, so a=1... and
even then u is not an integer-affine function of y unless a=1 divides; for a>=3
no such identity exists. The unordered child pairs agree between the cases
({x, y+2x}, {x+2y, y}, {2y-x, y} or {2x-y, x}), as they must. QED

The lead's table for b=1, k=1, x=3,7,...,31 is printed in S2: B1 gives x->y+2x
...
B3 -> (3,-b,2); from k=1 with y<x (possible only for b<=-3), B1 -> (3,b,3); and
from k=2 with y>x (possible only when x<b, so b>=3), B3 -> (3,-b,1). For
...
the edge 3->1 of E_{9,-23,2} has B3 child (5,3) with -3+23=20=4*5; and M=-2
...
the only p->q exponent, and q->p would give s = -2t-b' < 0. For B3, 3p/q in
(3,6) with q>=3 (q=1 would force p>=3>2q), so j=2 only; q->p with j=1 gives
s = 2t+b'. For B1, q->p gives 2s = -t-b' < 0, so only p->q occurs.
...
* the lead's statement "depth <= 1 above any k=1 edge" is **REFUTED** by the
...
Collatz k=1 edge is never a (3,+-1) edge and always a halving edge; the parents
of k=2 and k=3 edges are the k=1 edges of their own fibre or of the mirrored
parameter; the parents of k>=4 edges are generalized edges with an odd cofactor.
...
(1665 directed, 1664 pairs: the overlap is (7,5)); per multiplier the totals are
...
1013 directed edges (overlap: the single triangle (7,5)); the union over
...
k0+2j it is B1^((2^(k0+2j)-2^k0)/6). The fibre is the subset of the B1-ray
{(x,y): t=y} at B1-heights m_j = 2^(k0-1)(4^j-1)/3; R is a tree step (B2) only
once, at k=1, and afterwards a power of B1 whose exponent doubles-squared
(4, 16, 64, ...). Verified on 8002 fibre steps (y<=2001, k<=12, b=+-1). The
lead's example in Euclid coordinates, (9,2)->(20,9)->(64,53), is the root chain
(11,7)->(29,11)->(117,11), i.e. 7->11 (k=1), 29->11 (k=3), 117->11 (k=5);
(29,11)->(117,11) is B1^4 and not a child (the children of (29,11) are (51,11),
(69,29), (47,29)), which is exactly what the lead observed.
...
consecutive orbit-triangle pairs (this reproduces the lead's count) and 0 of
them are Berggren parent/child in either direction. **PROVED sketch.** The
shared coordinate y must be the retained coordinate of the child, so
z in {x+2y, 2y-x} or x in {z+2y, 2y+z, 2y-z}; with y=(3x+1)/2^k and
z=(3y+1)/2^j each case is one linear equation in x whose coefficient dominates
its right-hand side for all (j,k) except finitely many small cases, and those
only produce the fixed edge x=y=1 (the script's S5 machinery is the same
computation for the sibling question; the orbit case is stated with its
inequalities in the script text and checked exhaustively here).
...
`ALL CHECKS PASSED`. Every number quoted above appears in the `.out`.
