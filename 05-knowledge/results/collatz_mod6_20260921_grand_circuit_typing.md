...
**Equivalence (Giuga 1950; restated in Borwein, Borwein, Borwein, Girgensohn, Amer. Math. Monthly 103 (1996); CITED).** A composite `n` satisfies the congruence iff `n` is squarefree and, for every prime `p | n`, `(p-1) | (n/p-1)` (Korselt, so `n` is Carmichael) and `p | (n/p-1)` (Giuga).
...
**S5 (FINITE-EXACT).** The trunk numbers `t_j = (4^j-1)/3 = R^j(0) = R^(j-1)(1)` of the inherited inverse-fibre braid (the odd predecessors `3 t_j + 1 = 4^j` of the powers of four; the `N_j` of the [wild_typing lane](collatz_mod6_20260917_wild_typing.md)) are never Korselt and never Giuga for `3 <= j <= 40` (0 hits). (Audit fix: the first version wrote `R^j(1)`, which is `t_{j+1}`; `R^j(1) = 1, 5, 21, 85, 341, ...` starts one step later than `t_j = 0, 1, 5, 21, 85, ...`.) Table for the small cases:
...
**S12 (HEURISTIC, float64).** `min_{n <= 10^6} n ||n log_2 3|| ||n phi|| = 0.000209` at `n = 10946` (the Fibonacci number of index 21, as expected: Fibonacci `n` make `||n phi||` small); `min n ||n log_2 3|| = 0.017734` (a float64 artefact of the exact `q ||q alpha|| = 0.017732` at `q = 190537` in the S10 table; the float64 error at that size is about `q^2 2^-53 = 4.0e-06`, audit item A5), `min n ||n phi|| = 0.381966 = 1/phi^2`, attained at `n = 1` (the liminf, Hurwitz's constant for `phi`, is `1/sqrt 5 = 0.447214`, and `n = 10946` attains exactly `n ||n phi|| = 0.447214` to six places). This is numerology, not evidence.

**S13 (SCOPE / NO MAP).** The Collatz discrepancy `K_j - j log_2 3` involves one irrational, an inhomogeneous one-number problem; Littlewood is a two-number simultaneous problem. Wythoff, Zeckendorf and the golden ratio occur in the audited notes only as SCOPE tokens of the same paste family: the [blueprint energy note](collatz_blueprint_20260921_energy.md) types and dismisses the lower Wythoff sequence `floor(k phi)`, the [scaffolding audit](collatz_mod6_20260917_scaffolding_audit.md) and the [mod-6 synthesis](collatz_mod6_20260917_synthesis.md) carry "Wythoff diffraction" as SCOPE, the [braids summand note](arithmetic_braids_20260917_summand.md) mentions Zeckendorf and the [guards valves note](collatz_guards_20260921_valves.md) the golden ratio; no audited note attaches a golden-ratio object to a Collatz orbit, and in the [pythagorean_semicircle lane](collatz_mod6_20260917_pythagorean_semicircle.md) `phi` is an angle. (Audit fix: the first version said "in none of the audited notes", which is false as stated.) "Bragg peaks `q_{m,n} = (2 pi/phi^2)(m + n phi)`" is the diffraction module of the Fibonacci chain (Levine-Steinhardt 1984, CITED as a formula family; the paste's normalisation is not load-bearing); "`5 pi/6` phase from `5 mod 6`": no object.
...
**S15 (PROVED + FINITE-EXACT; disjointness is FALSE on `Z_2`).** Since `Q` is a bijection, there is a 2-adic `x` with `parity(x) = 0` and `parity(T^k x) = [mu(k) = 1]` for all `k >= 1`. The truncation `x = 1781862506 mod 2^32` realises `parity(x) = 0` followed by the first 31 values of `[mu = 1]`, the parity vector `01000010001000110000011000100000` (verified by direct iteration). For that `x` and `f = parity`, `f(T^n x) = [mu(n) = 1]` for `n >= 1`, so `(1/N) sum_{n <= N} mu(n) f(T^n x) = (1/N) #{n <= N : mu(n) = 1} -> 3/pi^2 = 0.303964`, not 0. So the Collatz system on `Z_2` is not Mobius-disjoint, exactly as any full shift is not. The point `x` is not in `Q`: rational 2-adic integers have eventually periodic parity vectors (Lagarias 1985, CITED), and `[mu = 1]` is not eventually periodic, since every residue class `r mod P` contains infinitely many `n` with `mu(n) = 1` and infinitely many with `mu(n) = 0` (PROVED, one line by CRT). (Audit fix: the first version indexed the point as `parity(T^k x) = [mu(k+1) = 1]`, `x = 890931253 mod 2^32`, for which `sum mu(n) f(T^n x) = sum mu(n) [mu(n+1) = 1] = (1/2) sum mu(n) mu(n+1)^2 + (1/2) sum mu(n) mu(n+1)`, whose second term is the two-point Chowla sum, OPEN for natural density; the claimed value `#{mu = 1}/N` was wrong for that point. On 2000-bit truncations at `N = 1999` the two correlations are `-0.002001` for the old point and `+0.305153 = #{mu(n) = 1}/N` for the corrected point, audit item A6.)
...
PROVED: `parity(T^j n)` depends only on `n mod 2^(j+1)` (the first `j+1` parities determine and are determined by `n mod 2^(j+1)`; Terras 1976 and Everett 1977, CITED; the modulus is sharp, the parity is not periodic mod `2^j` for any `j <= 6`, audit item A7), so `(-1)^{parity(T^j n)}` is a finite combination of residue-class indicators, each of which is Mobius-orthogonal by the prime number theorem in arithmetic progressions (Landau; CITED). Hence `c_j -> 0` for every fixed `j`. This is the zero-entropy odometer on `Z/2^(j+1)`, not the Collatz dynamics, and it proves nothing about Collatz.
...
**S22 (FINITE-EXACT; repository grep).** 182 canon theorems have "tournament" in their title; 10 of them have "17" in the title. In each the 17 is a coefficient of the 7-tournament spectrum `x(x^2+7)(x^4+14x^2+17)` (THM-1450, THM-1455, THM-1575), the Fermat prime 17 (THM-871), the fingerprint twin `(17, 13)` (THM-893), or a mod-16 / 2-adic statement (THM-1780, THM-1862, THM-1865, THM-1870, THM-1980). Tournament-titled theorems whose body mentions a 17-vertex, order-17 or `n = 17` tournament: 2, [THM-868](../../01-canon/theorems/THM-868-e8-bridge-score-lattice.md) (the sedenion rung `n = 17` in a list) and [THM-871](../../01-canon/theorems/THM-871-fermat-rung-rigidity.md), which is a theorem about 17-vertex tournaments: for the Fermat prime `n = 17` every rotational tournament has `Aut = Z_17` and there are exactly 16 rotational classes, a free `Z_16`-torsor. Neither defines a "capacity". (Audit fix: the first version of this note said 0 while its own output said 2.) Results notes matching `S^6`, `S6 monodromy` or `S_2 x S_3`, with this lane's own files excluded so that the count is stable: 5, namely the two `planar_jc48_sep06` notes, where `S^6` is the monomial in projective coordinates `[S:T]` (not a sphere, not this paste family), the two concurrent [arithmetic_seams](arithmetic_seams_20260921_synthesis.md) notes, which build an explicit `S_2 x S_3` action on a six-vector lift of a rational three-point cycle, and the [blueprint synthesis](collatz_blueprint_20260921_synthesis.md), which refutes the `S6` monodromy. (Audit fix: the first version counted 5 "all audits of the same paste family", a count that included this note itself once it existed and two non-paste monomial hits.)

**S23 (SCOPE).** The 17s in tournament context in this session are the `h`-value 17 attained at `n = 6` (scaffolding audit, section 5; that 7 and 21 are omitted from the `h`-spectrum for all `n` is [THM-1370](../../01-canon/theorems/THM-1370-h-spectrum-omits-7-21-all-n.md), whose completeness statement "odds minus `{7, 21}`" is a conjecture there, while [THM-1745](../../01-canon/theorems/THM-1745-leaf-graded-arborescence-filtration-721-shadow.md) is the arborescence shadow of `{7, 21}`; audit fix of the citation), the class 17 of the mod-30 wheel, and canon THM-871's 16 rigid rotational classes at `n = 17` (S22). No "capacity" of a 17-vertex tournament is defined anywhere: no object.

**S24 (SCOPE).** `S^6` (the 6-sphere) appears in no note of this paste family. `S_2 x S_3 = C_2 x S_3` (order 12) appears as the actual automorphism action on the fruit-curve triple (catalan_elliptic synthesis, section 4, written `C2 x S3` there) and as the arithmetic_seams lanes' explicit action on the six-vector lift of a rational three-point cycle (S22); a "projective lift of `S^6`" is undefined in all of them, and the blueprint audit already found no `S6` monodromy. No object. (Audit fix: the first version said `S_2 x S_3` "appears once".)
...
CITED (shape only; the exponent `C` is not asserted here): Baker-type lower bounds `|2^K - 3^L| > 2^K / K^C` (Pillai's problem; Stroeker-Tijdeman 1982 for this pair) are the standard formal input that bounds cycle lengths through the gate; that is the concurrent [pillai lane's](collatz_mod6_20260921_pillai_convergents_cycle_gates.md) object, not this lane's.
...
| "17-vertex capacity" | no object | 2 body hits among 182 tournament theorems, THM-868 and THM-871 (16 rigid rotational classes at `n = 17`), no capacity (S22) | tournament compression | SCOPE (S23) |
| "`S^6` / `S_2 x S_3` lift" | no object | 5 notes: 2 projective monomials, 3 of this paste family (blueprint, arithmetic_seams x2) (S22) | projective lift | SCOPE (S24) |
...
## 9. Adversarial audit record (2026-09-22)

Independent recomputation ([audit script](../../04-computation/experiments/collatz_mod6_20260921_grand_circuit_typing_audit.py), [audit output](collatz_mod6_20260921_grand_circuit_typing_audit.out), items A1-A10): the AAC data for all 211 primes from a Python continued-fraction solver of `x^2 - p y^2 = -1` plus the cube-root test for the half-integral unit (agrees on every histogram, the mod-8 census 101/110/79/31, and `p = 2689`, 38 digits, `u mod p = 2646`; small units `(1 + sqrt 5)/2`, `(3 + sqrt 13)/2`, `4 + sqrt 17`); the Giuga sum by pure-Python `pow` for `n <= 5000` and the criteria by `factorint` for `n <= 10^5` (same 16 Carmichael and 4 Giuga numbers, 0 disagreements including primes); the prime-power lemma of S2 at `k = -1 mod q`, the case that occurs; the 60 partial quotients by mpmath at 220 digits with all 25 convergent qualities to `10^-6`; the correlations `c_j` by a linear sieve and direct iteration (all seven agree to `10^-6`, squarefree count 607926 to `10^6`); the coprimality counts; the greps; the S25 table.

| edit | what was wrong | fix |
|---|---|---|
| S5 | `t_j = R^j(1)` | `t_j = R^j(0) = R^(j-1)(1)` |
| S15 | point indexed `parity(T^k x) = [mu(k+1) = 1]`, so the correlation was `sum mu(n)[mu(n+1) = 1]`, not `#{mu = 1}` | point re-indexed, `x = 1781862506 mod 2^32`; irrationality now argued (Lagarias 1985 + CRT) instead of asserted |
| S16 | periodicity uncited | Terras 1976 / Everett 1977 CITED; sharpness (not periodic mod `2^j`) checked |
| S22 | "0 body hits" (own output said 2); `S^6` count unstable, included this note and two monomial hits | 2 hits, THM-871 named; own-lane files excluded, 5 hits classified |
| S23 | `h`-spectrum cited to THM-1745 | THM-1370 (omission proved, completeness a conjecture there); THM-871 added |
| S24 | "`S_2 x S_3` appears once" | catalan_elliptic (`C2 x S3`) and the two arithmetic_seams notes |
| S13 | "Wythoff, Zeckendorf, golden ratio in none of the audited notes" | present as SCOPE tokens in six notes (blueprint energy defines `floor(k phi)`); statement rewritten |
| S12 | `0.017734` presented as a minimum | float64 artefact of the exact `0.017732`; `0.381966 = 1/phi^2` at `n = 1` |
| S25 | "the only formal input"; Stroeker-Tijdeman exponent implied | "standard"; CITED shape only |
| board | "Pell hypotenuses are units of `Z[sqrt 2]`" | the negative-Pell rows `X^2 - 2m^2 = -1` are the norm `-1` units `X + m sqrt 2` |
| S1 | BBBG credited with the equivalence | Giuga 1950, restated in BBBG 1996 |

Confirmed unchanged: S1-S4, S6-S11, S14, S17-S21, the 182/10 title counts, the S25 table, every verdict. The verdict table and the answer to the closing question are unaffected by any of the edits.
...
python3 04-computation/experiments/collatz_mod6_20260921_grand_circuit_typing_audit.py > 05-knowledge/results/collatz_mod6_20260921_grand_circuit_typing_audit.out
```

Dependencies: PARI/GP (`gp -q` on stdin, for `quadunit` and the 400-digit continued fraction), numpy (vectorised power sums and the Mobius sieve), sympy (`factorint`, `mobius`, `primerange`; every factorisation is re-multiplied); the audit uses only sympy and mpmath. Every check is an explicit `raise`; the `python3 -O` run is identical. Runtime about 20 seconds for the lane script and 15 seconds for the audit, memory far below 1 GB. Every number quoted in this note appears in the `.out` or, where marked "audit item", in the audit `.out`.
