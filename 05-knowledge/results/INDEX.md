...
| `lonely_runner_endpoint_protection_s359.py` | [STORED] | Exact endpoint-protection incidence graph for LRC; known tight examples have unprotected endpoints exactly `{0}` plus unit residues mod `k+1`, while near-tight positive-gap sets still leave explicit unprotected endpoints | `lonely_runner_endpoint_protection_s359.out` |
