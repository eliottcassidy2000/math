...
- So the induction reduces to chains of hostile transfers. An adversarial `m` can arrange such chains digit by digit, since each 1/2-transfer consumes `k>=3` digits of `w`. With the prices of the known threads, a chain can raise the value by at most about `m^{0.104}` (`0.114` nats per digit over `log_3 m` digits) before the digits of `m` run out. The infinitely many other dyadic threads have not been priced.
  **REFUTED (endgame lane, 2026-09-23):** L1 links do not consume the digits of `m`. Each link's next precision is bounded only by `log_3(2 m_t)`. For example, `829812238225934675` (37.6 digits) runs 15 consecutive L1 links over 60 digits, and `6082250` reaches excursion `6.57 > m^0.104 = 5.08`. The correct statement is the canonical-chain budget `m_T/m_0 <= exp(c* D_L1)`, `c* = ln(128/81)/4 = 0.1144` (sharp, all threads). In terms of `m` there is no budget; the random-model worst excursion is `X^0.1157` ([q2_endgame](collatz_procgen_20260922_q2_endgame.md) §3).
