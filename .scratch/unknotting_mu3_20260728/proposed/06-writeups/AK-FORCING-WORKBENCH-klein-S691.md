...
3. **Identification-gluing (independent AK hypothesis):** merging classes
   can manufacture a hub that inherits every member's incident species.
   Measured: merge = 12/7 versus edge+seed = 7/4 on twin [2,2,2]
   witnesses.  This remains an AK design hypothesis, but it is **not**
   evidence extracted from the owner's depth-two fragment.  That fragment
   is now exactly resolved by THM-2473 as a Keller-map Jelonek fiber drop:
   `3+1+3=7`, with two sheets escaping to infinity, no coincidence, an
   order-two fixed branch, and generic `S3` monodromy.  See the corrected
...
vertices and lowers n without touching m, r.  **This is the leading AK
hypothesis for where 1.75 and 1.675 certificates actually live, but it no
longer claims support from the unrelated Keller-map 9-to-7 fragment.**
