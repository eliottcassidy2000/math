...
profiles. The S225-S291 refinements add the geometry-regime, lattice-fence,
...
Chebyshev_equioscillation_unit_group, and Q_sqrt_minus_7_residue_field to
...
n4_einheit_torsor_status, or Q_sqrt_minus_7_residue_field.
```
...
Chebyshev_equioscillation_unit_group unit, and the Q_sqrt_minus_7_residue_field
unit. The S235-S291 sequence prevents spectral leakage by requiring
route-triple centers to be unique and promoted only after legal closure,
named debt exits, validated collision arity, a named modular cusp
...
Chebyshev_equioscillation_unit_group, or Q_sqrt_minus_7_residue_field are
attached.
```
...
ledger. The S229-S291 transition preserves the Toeplitz square-peg scale gate
...
Chebyshev_equioscillation_unit_group law, and Q_sqrt_minus_7_residue_field law
...
Chebyshev_equioscillation_unit_group unit, and Q_sqrt_minus_7_residue_field unit
...
### 1. Residue Layer Organization
Organized via $\mathbb{Q}(\sqrt{-7})$. The three binding pairs are 
characterized as three Galois-conjugate quadratic residue/non-residue 
(QR/NQR) pairs.

### 2. Magnitude-Level Divergence
Establishes that "tight-vs-loose" is a magnitude-level property 
(residue-blind). Loose sets (e.g., $12 \to 26$) may share the AP signature 
but jump via "un-killing" the $t=1/12$ peak.

### 3. Census Rigidity
Frames the census as **exact-minimal-cover rigidity**. Tightness is the 
unique state where the minimal cover of the unit group is rigid, preventing 
sliding.

### 4. Chiral Guard & Resurrection
Governed by Galois action on QR/NQR pairs. Evaluation reveals a **MIXED** 
signature. Identifies **Galois-conjugate QR/NQR pairs** as Resurrection 
Sheaf anchors.

---

## codex-S290 -- Chebyshev Unit Unification (checkpoint)

Integrated fd72ab. Formalized the Lonely Runner Problem as a Chebyshev 
equioscillation problem on the unit group $(\mathbb{Z}/14)^\times$, 
unifying the combinatorial case splits.
