...
profiles. The S225-S313 refinements add the geometry-regime, lattice-fence,
...
n4_einheit_torsor_status, or AP84_Corridor_Splice.
```
...
Phase-Branch_Color_Pullback unit, and the AP84_Corridor_Splice unit.
```
...
ledger. The S229-S313 transition preserves the Toeplitz square-peg scale gate
...
law, and AP84_Corridor_Splice unit alongside mu approx 1.451).
```
...
### 1. AP84 Corridor Splice
Implements the structural splice of the AP-tail bridge by importing the 
HYP-3431 branch-union carrier (corridors $C_1=[8/49, 6/35]$ and 
$C_0=[29/35, 41/49]$). Audits confirm that for $m=1$, the canonical rank-6 
base is preserved, while for $m \ge 2$, the rank-5 overlap descent is valid.

### 2. Residue-Cover Resolution
Characterizes covering structures by mapping rank-one endpoint intervals and 
transient windows into the branch-union corridors. Proves continuous floor 
coverage across the AP84 tower by requiring an explicit window-to-corridor 
containment certificate.

### 3. Algebraic Triad Coupling
Formally couples the splice with the field triad: $\mathbb{Q}(\cos 2\pi/7)$ 
(carrier endpoints), $\mathbb{Q}(\sqrt{-7})$ (residue shift), and 
$\mathbb{Q}(\sqrt{-3})$ (moving wall intersection).

### 4. Chiral Guard & Resurrection
Governed by mirror window pairs across the $C_0/C_1$ carrier. Evaluation 
reveals a **MIXED** signature. Identifies the **branch-union carrier** 
and **rank-split interface** as Resurrection Sheaf anchors.
