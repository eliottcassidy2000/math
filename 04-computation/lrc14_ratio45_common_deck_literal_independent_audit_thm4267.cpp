...
// Clean-room audit of the proposed LRC(14) ratio-4:5 common deck.
//
...
// intervals for speeds 4g and 5g, intersects those intervals, and then sweeps
...
constexpr std::size_t KEEP = 32768;
constexpr std::array<std::size_t, 4> BUDGETS = {4096, 8192, 16384, 32768};
constexpr u32 TARGET_REPAIR = UINT32_C(0x18251600);
constexpr u32 PRIMARY_MIN_REPAIR = UINT32_C(0x0a20d082);
constexpr u64 REPAIR_UNIVERSE = UINT64_C(5852925);
constexpr u64 BODY_UNIVERSE = UINT64_C(14307150);
constexpr int G0 = 73;
constexpr int G1 = 153;
...
std::vector<Interval> literal_joint_intervals(int g, i64 denominator) {
    const auto a = speed_safe_intervals(4 * g, denominator);
    const auto b = speed_safe_intervals(5 * g, denominator);
    std::vector<Interval> result;
    std::size_t i = 0;
    std::size_t j = 0;
...
LiteralWeights literal_weights(const std::vector<PoolCell>& cells) {
    ...
    for (int si = 0; si < SCALE_COUNT; ++si) {
        const int g = G0 + si;
        const i64 denominator = exact_lcm(D, 280LL * g);
        ...
        // The literal 4:5 joint set has length 208/280=26/35 in one
        // primitive cell; scaling repeats it g times.
        require(static_cast<i128>(35) * total == static_cast<i128>(26) * denominator,
...
struct DeckResult {
    ...
    std::array<std::vector<u32>, BUDGETS.size()> budget_common;
    Fnv64 matrix_fnv;
    i128 least_margin = 0;
    int least_scale = 0;
    u32 least_repair = 0;
    ...
    bool primary_min_active = false;
    i128 target_least_margin = 0;
    int target_least_scale = 0;
};
        ...
        if (index < KEEP) {
            for (std::size_t slot = 0; slot < BUDGETS.size(); ++slot) {
                if (index < BUDGETS[slot]) {
                    result.budget_common[slot].push_back(repair);
                }
            }
        } else if (repair == TARGET_REPAIR) {
            result.target_active = true;
            result.target_least_margin = local_least;
            result.target_least_scale = local_scale;
        } else {
            require(repair == PRIMARY_MIN_REPAIR,
                    "unexpected audit-sidecar repair");
            result.primary_min_active = true;
        }
        if (index < KEEP || repair == TARGET_REPAIR) {
...
int main() {
    try {
        ...
        require(std::find(candidates.begin(), candidates.end(), TARGET_REPAIR) ==
                    candidates.end(),
                "target repair unexpectedly lies in first 32768");
        require(std::find(candidates.begin(), candidates.end(), PRIMARY_MIN_REPAIR) ==
                    candidates.end(),
                "primary minimum repair unexpectedly lies in first 32768");
        auto audit_candidates = candidates;
        audit_candidates.push_back(TARGET_REPAIR);
        audit_candidates.push_back(PRIMARY_MIN_REPAIR);
        const auto cells = pool_cells();
        const auto weights = literal_weights(cells);
        const auto deck = evaluate_matrix(audit_candidates, weights);
        ...
        const int least_index = deck.least_scale - G0;
        const i128 conversion =
            static_cast<i128>(280) * D * deck.least_scale /
            weights.denominator[least_index];

        std::cout << "LRC14_RATIO_4_5_LITERAL_JOINT_WALL_REFEREE\n"
                  ...
                  << "MATRIX_CELLS " << audit_candidates.size() * SCALE_COUNT
                  << " NONNEGATIVE " << deck.nonnegative_cells
                  << " ZERO " << deck.zero_cells
                  << " MATRIX_FNV " << hex64(deck.matrix_fnv.value()) << "\n"
                  << "TARGETED_COMMON " << deck.common.size()
                  << " TARGET_ACTIVE " << deck.target_active
                  << " PRIMARY_MIN_ACTIVE " << deck.primary_min_active
                  << " TARGETED_FNV " << hex64(deck_fnv.value()) << "\n"
                  << "TARGET_LEAST_LITERAL_MARGIN "
                  << dec(deck.target_least_margin) << " AT_SCALE "
                  << deck.target_least_scale << "\n"
                  << "LEAST_LITERAL_MARGIN " << dec(deck.least_margin)
                  << " AT_SCALE " << deck.least_scale
                  << " REPAIR {" << labels(deck.least_repair) << "}"
                  << " CONVERSION " << dec(conversion)
                  << " PRIMARY_MARGIN " << dec(deck.least_margin * conversion)
                  << "\n";

        constexpr std::array<u64, BUDGETS.size()> expected_common = {
            1245, 2417, 4942, 9805};
        constexpr std::array<u64, BUDGETS.size()> expected_failures = {
            1377, 219, 30, 2};
        require(deck.common.size() == 9806, "targeted deck size mismatch");
        require(deck.target_active, "target repair is not common-active");
        require(deck.primary_min_active, "primary minimum repair is not common-active");
        require(deck_fnv.value() == UINT64_C(0xc2e18c7ecbcb9c72),
                "targeted deck FNV mismatch");
        require(deck.least_margin * conversion ==
                    static_cast<i128>(INT64_C(19129894580160)),
                "primary/literal minimum-margin scaling mismatch");

        const auto bodies = enumerate_bodies();
        for (std::size_t slot = 0; slot < BUDGETS.size(); ++slot) {
            const auto hostile = scan_bodies(deck.budget_common[slot], bodies);
            Fnv64 budget_fnv;
            for (u32 repair : deck.budget_common[slot]) budget_fnv.word(repair);
            std::cout << "BUDGET " << BUDGETS[slot] << " COMMON "
                      << deck.budget_common[slot].size() << " COMMON_FNV "
                      << hex64(budget_fnv.value()) << " BODY_COUNT "
                      << hostile.bodies << " FAILURES " << hostile.failures
                      << " CHECKS " << hostile.checks << " MAX_CHECKS "
                      << hostile.max_checks << " FIRST_FAILURE ";
            if (hostile.failed_bodies.empty()) {
                std::cout << "NONE";
            } else {
                std::cout << "{" << labels(hostile.failed_bodies.front()) << "}";
            }
            std::cout << "\n";
            require(deck.budget_common[slot].size() == expected_common[slot],
                    "nested common-deck census mismatch");
            require(hostile.failures == expected_failures[slot],
                    "nested hostile-body census mismatch");
            if (slot + 1 == BUDGETS.size()) {
                for (u32 body : hostile.failed_bodies) {
                    std::cout << "HOSTILE_32768_FAILURE {" << labels(body)
                              << "}\n";
                }
            }
        }
        ...
        std::cout << "TARGETED_BODY_COUNT " << full.bodies
        ...
        std::cout << "VERDICT PASS_LITERAL_FIRST32768_PLUS_TARGET_ALL_81_SCALES_AND_ALL_14307150_BODIES\n";
        return 0;
