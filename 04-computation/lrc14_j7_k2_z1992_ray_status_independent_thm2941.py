#!/usr/bin/env python3
"""Independent exact referee for the fixed projected z1=1992 state.

This audit does not import the primary z2060 closure.  It independently
reconstructs the four all-label ray maxima, audits the separate crude kill,
enumerates the 125 five-vertex trees through Pruefer codes, recomputes all
...
BODY = (1, 4, 8, 10, 12, 14)
FIRST = 1992
LABELS = (1992, 2060, 2142, 2172, 2534)
DS = (280, 490, 588, 840, 980)
D = 5880
Q = 840
M = 7
CRUDE_STATES = (
...
def main():
    ...
    require(
        tuple(marginals) == (120, 0, 0, 120, 0)
        and tuple(sorted(set(capacities))) == (3, 4, 5, 6, 7)
        and capacities[0] == 3,
    ...
    crude_arcs = U.fibre.projected_support_arcs(CRUDE_D, ranges)
    crude_histogram = U.fibre.residue_load_histogram(crude_arcs, CRUDE_Q)
    crude_target = max(load for load, count in crude_histogram if count)
    crude_capacities = tuple(
        sum(U.fibre_cap(CRUDE_D, d, CRUDE_Q) for d in ds)
        for ds in CRUDE_STATES
    )
    require(
        (CRUDE_D // CRUDE_Q, crude_target, crude_capacities)
        == (7, 6, (5, 5, 5)),
        ("independent crude kills changed", crude_histogram, crude_capacities),
    )
    ...
    # Farkas certificate for the threshold-four row.  With alpha=1/60 and
    # z_total=0,z_i=1/60, every one of the 32 status-cell slacks is exact.
    alpha = F(1, 60)
    z = (F(0),) + (F(1, 60),) * 5
    slacks = []
    ...
    require(
        ...
        and contradiction == -1
        and heavy_four == 300
        and exception_events == 240,
        ("exact Farkas audit changed", slacks, contradiction),
    )
    require(
        all(
            int(capacities[pattern] >= 4)
    ...
    lines = [
        "LRC14 projected k=2 z1=1992 independent fixed-state referee",
        f"uniform_ray_status_source_sha256={file_sha256(UNIFORM)}",
        "primary_z1992_source_imported=False;tree_engine=Pruefer;LP_used=False",
        (
        ...
        (
            f"independent_crude_kills=states:{CRUDE_STATES};D:{CRUDE_D};q:{CRUDE_Q};"
            f"M:{CRUDE_D//CRUDE_Q};target:{crude_target};"
            f"capacities:{crude_capacities};histogram:{crude_histogram}"
        ),
        (
        ...
        (
            f"transparent_threshold4=heavy_fibres:{heavy_four};"
            f"exception_events:{exception_events};300>240"
        ),
        (
            f"farkas=alpha:1/60,z:(0,1/60,1/60,1/60,1/60,1/60);"
        ...
        "conclusion=fixed projected z1=1992 scalar state is impossible",
