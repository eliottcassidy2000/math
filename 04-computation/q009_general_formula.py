"""
Q-009: GENERAL formula for ΔI, valid for all n.

ΔI = -2·Σ_x s_x · H(B_x) + 2·Σ_{L≥5,odd} (DL - CL)

The -2·Σ s_x·H(B_x) term encodes:
  - 3-cycle changes: -2·Σ s_x (the "1" in H(B_x) = 1 + 2c(B_x))
  - VD pair changes: -4·Σ s_x·c(B_x) (the "2c(B_x)" part)

The cycle_term must only include L≥5 cycles to avoid double-counting
with the 3-cycle contribution already in -2·Σ s_x·H(B_x).
...
if __name__ == "__main__":
    ...
    for n in [4, 5, 6, 7]:
        n_trials = {4: 500, 5: 300, 6: 200, 7: 50}[n]
        n_match = 0
        n_total = 0
        ...
        for _ in range(n_trials):
            ...
            # Cycle terms: L≥5 ONLY (3-cycles already in sx_hbx term)
            ...
            if formula == delta_H:
                n_match += 1
            elif n_total - n_match <= 3:
                print(f"  n={n} MISS: formula={formula}, ΔH={delta_H}, "
                      f"sx_hbx={sx_hbx}, longD={long_D}, longC={long_C}")

        print(f"n={n}: {n_match}/{n_total}")

    print("\nIf all match: ΔH = -2·Σ s_x·H(B_x) + 2·Σ_{L≥5}(DL-CL)")
    print("is the GENERAL arc-flip OCF formula for all n!")
