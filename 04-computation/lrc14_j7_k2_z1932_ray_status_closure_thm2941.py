#!/usr/bin/env python3
"""Close the projected k=2 z1=1932 row for all later labels.

Body E=(1,4,8,10,12,14), first drift 1932, and four later drifts.  The
...
PRIOR_ENGINE = (
    ROOT
    / "04-computation"
    / "lrc14_j7_k2_z1940_ray_status_closure_thm2941.py"
)
BAND_OUTPUT = (
    ROOT
    / "05-knowledge" / "results"
    / "lrc14_j7_k2_scalar_band_1932_1939_thm2941.out"
)
OUTPUT_PATH = (
    ROOT
    / "05-knowledge"
...
EXPECTED_FRONTIER_ROW = (
    ...
    "lower=1049/267540;upper=442380823/112512089655;"
    "gap=12804017/1170125732412"
)
BODY = (1, 4, 8, 10, 12, 14)
FIRST = 1932
K = 2
...
def main():
    ...
    require(feasible_control, "360-heavy-fibre positive control rejected")
    heavy_three = sum(count for load, count in histogram if load >= 3)
    require(
        heavy_three == 720
        and sum(marginals) == 360
        and heavy_three > sum(marginals),
        "transparent threshold-three obstruction changed",
    )
    ...
    lines = [
        "LRC14 projected k=2 z1=1932 all-label ray/status closure",
        ...
        (
            "scope=inherited projected k=2 z1=1932 body row;"
        ...
        (
            "fixed_state_transparent_obstruction=720 load>=3 fibres;"
            "only 360 exceptional-status events;EXACT_FARKAS_INFEASIBLE"
        ),
        (
        ...
        "conclusion=projected k=2 z1=1932 row is empty uniformly",
