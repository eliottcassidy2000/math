#!/usr/bin/env python3
"""Clean-room referee for the p^9 lower-memory row-fourteen repair.

Only the audited THM-4308/4315 source-normal row operators are imported.
In particular, neither scout in ``.scratch/row14_incoming_bridge_20260905``
is read or imported.  The certificate reconstructs rows 4--14 for

    H_14 + z*p^9 + lambda*p^3*y^4 + nu*y^6
         + kappa*p*y^6 + rho*p^2*y^6,

normalizes the row-fourteen unpaid class against THM-4415's frozen hash,
and verifies the boundary projected-depth survivors in the exact quadratic
quotient rather than by floating-point substitution.
"""
...
z, lambda18, nu18, kappa20, rho22 = sp.symbols(
    "z lambda18 nu18 kappa20 rho22"
)
CHECKS = 0
...
def main() -> None:
    ...
    source = sp.expand(
        R8.H + face13 + face14 + memory9 + late11 + late12 + late13 + paid14
    )
    ...
    xi_candidates = [
        ...
        if proportional(value.subs(z, 0), xi_expected)
    ]
    ...
    beta_candidates = [
        ...
        if proportional(value.subs(z, 0), beta_expected)
    ]
    ...
    base5 = {R8.Phi, R8.eta, R8.alpha11, c51, z}
    check(
    ...
    origin4 = {R8.Phi: 0, R8.eta: 0, R8.alpha11: 0, c51: 0, z: 0}
    ...
    check(linear_coefficient == expected_linear, "unpaid linear coefficient")
    ...
    j_boundary = primitive(j18.subs(boundary), positive_in=c51)
    d_c51 = exact(sp.diff(j_boundary, c51))
    check(
        d_c51 == 45455611113114234086880000000,
        "boundary c51 pivot",
    )
    boundary_poly = sp.Poly(j_boundary, z, c51, domain=sp.QQ)
    check(
        boundary_poly.degree(c51) == 1
        and boundary_poly.degree(z) == 2,
        "boundary unpaid bidegree",
    )
    a0 = exact(sp.expand(j_boundary).coeff(z, 2))
    b0 = exact(sp.expand(j_boundary).coeff(z, 1))
    c0 = exact(j_boundary.subs({z: 0, c51: 0}))
    check(
        (a0, b0, c0)
        == (
    ...
    check(all(value == 0 for value in bracket_after), "boundary bracket coefficientwise")
    ...
    scale_q = exact(a0 / sp.Poly(q_raw, z, domain=sp.QQ).coeff_monomial(z**2))
    check(not scale_q.free_symbols and scale_q != 0, "depth quadratic normalization")
    q = sp.expand(scale_q * q_raw)
    q_poly = sp.Poly(q, z, domain=sp.QQ)
    check(q_poly.degree() == 2, "depth source condition quadratic")
    c1 = exact(q.subs(z, 0))
    check(
        (q_poly.coeff_monomial(z**2), q_poly.coeff_monomial(z), c1)
        == (
            a0,
            b0,
    ...
    constant_gap = exact(c1 - c0)
    fixed_c51 = exact(constant_gap / d_c51)
    check(
        fixed_c51 == sp.Rational(1087, 135),
        "depth condition fixes the same rational c51 on both conjugate roots",
    )
    check(
        remainder_mod_q(c51_graph - fixed_c51, q) == 0,
        "c51 graph reduces to 1087/135 modulo Q",
    )
    rho_mod_q = remainder_mod_q(rho_graph, q)
    check(
        sp.Poly(sp.fraction(sp.together(rho_mod_q))[0], z, domain=sp.QQ).degree()
        <= 1,
        "rho coordinate lies in the same quadratic field",
    )
    discriminant = int(sp.discriminant(q, z))
    expected_discriminant = int(
        "548570569425327391286894008779398603921084860366624411440000000000"
    )
    check(discriminant == expected_discriminant, "depth discriminant")
    factors = sp.factorint(discriminant)
    expected_factors = {
        2: 13,
        3: 19,
        5: 10,
    ...
    check(factors == expected_factors, "depth discriminant factorization")
    squarefree = math.prod(prime for prime, exponent in factors.items() if exponent % 2)
    square_part = math.isqrt(discriminant // squarefree)
    check(
        square_part**2 * squarefree == discriminant
        and squarefree
        == 31983397604359613083640070627484254,
        "quadratic field squarefree reduction",
    )
    check(discriminant > 0 and math.isqrt(discriminant) ** 2 != discriminant,
          "two distinct real nonrational roots")

    # A fixed 5x5 pivot minor remains a unit at both conjugate roots, so the
    # depth rank is exactly five on each fibre, not merely generically five.
    pivot_minor = matrix_depth14.extract(row_pivots14, pivots_depth14).det()
    pivot_minor_mod_q = remainder_mod_q(pivot_minor, q)
    check(pivot_minor_mod_q != 0, "depth pivot survives quadratic quotient")
    check(
        len(theta14) - len(pivots_depth14) == 10,
        "terminal fibre dimension ten",
    )

    # Substitute the terminal selector and reduce every residual modulo Q.
    # This is coefficientwise verification simultaneously at both roots.
    depth_after = [exact(value.subs(terminal14)) for value in depth14]
    depth_remainders = [remainder_mod_q(value, q) for value in depth_after]
    check(all(value == 0 for value in depth_remainders),
          "both quadratic survivors coefficientwise depth")

    # Rational hostile z=1: both bracket equations have rational solutions,
    # but the final depth class is nonzero.
    hostile_z = sp.Integer(1)
    hostile_q = exact(q.subs(z, hostile_z))
    check(hostile_q != 0, "rational z=1 depth hostile")
    hostile_raw_values = [exact(value.subs(z, hostile_z)) for value in raw_depth14]
    nonzero_hostile_raw = [value for value in hostile_raw_values if value != 0]
    check(nonzero_hostile_raw, "hostile leaves a raw depth residual")
    ...
    for name, value in graph_order:
        poly_degree = sp.Poly(
            sp.fraction(sp.together(value))[0], z, domain=sp.EX
        ).degree()
        graph_data.append((name, poly_degree, denominator(value)))

    print("row14_p9_memory_clean_room_independent_referee")
    print("imports=audited_THM4308_THM4315_row_operators;scratch_scout_import=no")
    print("field_for_symbolic_chain=characteristic_zero")
    print("universe=complete_weight_le_14_plus_z_p9_plus_lambda_p3y4_plus_nu_y6_plus_kappa_py6_plus_rho_p2y6")
    print("p9_rows_9_to_14=" + sp.sstr(expected_p9))
    ...
    print("prefix_source_projection=A5(Phi,eta,alpha11,c51,z);row13_terminal=A9")
    print("prefix_graph_z_degrees_and_denominators=" + sp.sstr(tuple(graph_data)))
    print("row14_bracket=15x9_rank9;raw_cokernel6;active_conditions2")
    print(f"J18_hash={expression_hash(j18)};J14_at_z0_hash={expression_hash(j14)}")
    print(f"J18_quadratic_coefficient={quadratic_coefficient}")
    print("J18_linear_coefficient=" + sp.sstr(linear_coefficient))
    print(f"rho_pivot={frozen_rho_pivot};rho_denominator={denominator(rho_graph_global)}")
    print(f"boundary_bracket=D*c51+A0*z^2+B0*z+C0;D={d_c51};A0={a0};B0={b0};C0={c0}")
    print("boundary_bracket_coefficients=15/15_zero_after_c51_rho_graphs")
    print("row14_depth=P2(150x353,rank108),P3(165x564,rank129),joint(78x15,rank5,pivots10,11,12,13,14),conditions1")
    print(f"Q(z)={sp.sstr(q)}")
    print(f"Q_discriminant={discriminant}")
    print("Q_discriminant_factorization=" + sp.sstr(tuple(sorted(factors.items()))))
    print(f"Q_square_part={square_part};Q_squarefree_part={squarefree}")
    print(f"boundary_depth_constant_gap={constant_gap};c51_at_both_survivors={fixed_c51}")
    print(f"rho_at_survivors_mod_Q={sp.sstr(rho_mod_q)}")
    print("survivor_field=QQ(sqrt(Q_squarefree_part));roots=2_distinct_real_quadratic_conjugates")
    print(f"depth_pivot_minor_mod_Q={sp.sstr(pivot_minor_mod_q)};terminal_fibre=A10_over_each_quadratic_point")
    print("quadratic_survivor_residuals=all_78_depth_coefficients_zero_mod_Q")
    print(f"rational_hostile_z1_Q={hostile_q};raw_nonzero_count={len(nonzero_hostile_raw)};first_raw={sp.sstr(nonzero_hostile_raw[0])}")
    print("field_quantifier=survivors_exist_over_the_real_quadratic_field_and_any_char0_field_containing_it;not_over_every_char0_field")
    print("scope=finite_row14_partial_source_bracket_and_projected_depth_only;not_full_B2;not_termination;not_chart_entry;not_JC2_or_DC2")
