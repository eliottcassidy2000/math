...
BASELINE_SHA='66f903430fd804074e6c8adb330aca4f6bb6b8f97d955e014a73309c39d17a8d'
GATES=0;INF=10**10
def need(ok,why):
    global GATES
    GATES+=1
...
def main():
    ...
    baseline=json.loads((ROOT/'05-knowledge/results/continuing12_20260908_lrc_depth_wedge_certificate.json').read_bytes())['new_scales']
    need(sha256(canonical(baseline)).hexdigest()==BASELINE_SHA,'inherited complete necessary scale array')
    need(len(baseline)==7620 and max(baseline)==11935,'declared inherited scope')
    upper=[9240,11088]
    targets=[t for t in upper if t in baseline]
    need(targets==upper,'both predeclared composite clocks survive in array')
    sub={}
    ...
    for t in targets:
        ...
        record=dict(cheap_uniform_margin=cheap_margin,coarse_edge_floor=coarse_floor,uniform_excess_cap=excess_cap,t=t,domain=list(D),words=S.tolist(),word_count=len(S),event_evaluations=events,compatible_atlas_edges=compatible,
        ...
        if len(bad):break
    need(len(records)<=2 and (len(records)==2 or records[-1]['survivor_count']>0),'stop only after first complete residual or both clocks')
    ...
    cert=dict(status='PROVED minimum-tree consumer; FINITE-EXACT selected complete domain',profile_sha256=PROFILE_SHA,baseline_semantic_sha256=BASELINE_SHA,
      declared_clocks=upper,atlas_count=len(AT),domains=sorted(domain_cert,key=lambda d:d['domain']),clocks=records,controls=controls,removed_scales=removed,new_scales=new,
