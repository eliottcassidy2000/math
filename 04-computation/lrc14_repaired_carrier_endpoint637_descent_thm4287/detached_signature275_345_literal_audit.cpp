// Detached literal-wall and exhaustive-body audit of the THM-4287
// signature-{275,345} fibre surgery.  This translation unit imports no
// project source.  It independently reads the frozen joint deck, exact
// 33-row fibre, and two-mask witness; rebuilds the deck; exhausts every
...
constexpr std::size_t kFibreCount = 33;
constexpr u64 kFibreFnv = UINT64_C(0xf11425c1e1b17094);
constexpr std::size_t kWitnessCount = 2;
...
constexpr std::array<u32, 2> kExpectedWitness{
    UINT32_C(0x02229124), UINT32_C(0x1c030206)};
constexpr std::size_t kRebuiltCount = 421;
constexpr u64 kRebuiltFnv = UINT64_C(0x9781de64dff60cee);
constexpr u64 kBodyCount = UINT64_C(14307150);
constexpr u64 kBodyChecks = UINT64_C(405170443);
constexpr u64 kBodyRowFnv = UINT64_C(0x3aa84f98dae6429e);
constexpr i64 kFixedGrid = INT64_C(18241159416480);
constexpr u64 kEmptyFnv = UINT64_C(0xcbf29ce484222325);
constexpr u64 kTotalCells = UINT64_C(267981);
constexpr u64 kTotalFailureClasses = UINT64_C(77940);
constexpr u64 kActivityAggregateFnv = UINT64_C(0x7ae6c304327ea64e);
constexpr int kWeakestQ = 103;
constexpr int kWeakestR = 519;
constexpr u32 kWeakestMask = UINT32_C(0x1e808042);
constexpr i128 kWeakestMargin =
    static_cast<i128>(UINT64_C(224226244060434));
constexpr i64 kWeakestGrid = INT64_C(325039219642257120);
...
std::vector<Pair> read_fibre(const std::string& path) {
    ...
    require(pairs.size() == kFibreCount && ledger.state == kFibreFnv &&
                std::find(pairs.begin(), pairs.end(), Pair{520, 637}) !=
                    pairs.end(),
...
int main(int argc, char** argv) {
    try {
        ...
        require(argc == 4,
                "usage: detached-signature275-345 JOINT421 FIBRE33 WITNESS2");
        ...
        require(joint[kDeletedIndices[0]] == kDeletedMasks[0] &&
                    joint[kDeletedIndices[1]] == kDeletedMasks[1],
        ...
        std::cout << "THM4287_SIGNATURE275_345_DETACHED_LITERAL_AUDIT_V1\n"
                  ...
                  << kWitnessFnv << std::dec << '\n'
                  << "DELETE_INDICES 275 345 DELETE_MASKS " << std::hex
                  << std::setw(8) << std::setfill('0') << kDeletedMasks[0]
                  << ' ' << std::setw(8) << kDeletedMasks[1]
                  << " APPEND " << std::setw(8) << witness[0] << ' '
                  << std::setw(8) << witness[1] << std::dec
                  << std::setfill(' ') << '\n'
        ...
        std::cout << "ACTIVITY PAIRS " << fibre.size() << " MASKS_PER_PAIR "
                  ...
                  << weakest_grid << '\n'
                  << "VERDICT PASS DETACHED_EXACT_33_ROW_COMMON_DECK_SURGERY\n"
                  << "SCOPE LITERAL_ACTIVITY_AND_EXHAUSTIVE_BODY_SCAN_"
                     "FIXED_POOL_NO_PHYSICAL_ENTRY_NO_LRC14\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "THM4287_DETACHED_SIGNATURE_SURGERY_ERROR "
                  << error.what() << '\n';
        return 1;
    }
}
