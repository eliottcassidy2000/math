#!/usr/bin/env python3
"""r4_finite_horn_tail_kps_S128c62.py -- kind-pasteur S128 cont.61 (background).
...
KB=400
CORES=[sorted(c) for c in itertools.combinations(range(1,13),9)][160:]  # cont.62: remaining 60
...
for ci,P in enumerate(CORES):
    ...
    if ci%10==0: print("  ... core %d/%d  tested=%d  bad=%d"%(ci,len(CORES),tested,len(bad)))
print()
