#!/usr/bin/env python3
"""
a000568_parallel.py — A000568 via parallel C mod-p DP + CRT.

Runs multiple C mod-p computations in parallel using concurrent.futures,
then reconstructs the exact integer via CRT.
...
_lib_path = os.path.join(_dir, "a000568_c_core.so")

def _compute_one_prime(args):
    """Worker: compute a(n) mod p using the C library."""
    n, p = args
    lib = ctypes.CDLL(_lib_path)
    lib.a000568_mod_p.argtypes = [ctypes.c_int, ctypes.c_int64]
    lib.a000568_mod_p.restype = ctypes.c_int64
    return int(lib.a000568_mod_p(n, p))
...
def a000568_parallel(n, verbose=True, num_workers=None):
    """Compute A000568(n) using parallel C mod-p DP + CRT."""
    if n <= 1:
        return 1
    ...
    if verbose:
        print(f"  n={n}: {num_primes} primes, {num_workers} workers, est {est_bits} bits", flush=True)

    t0 = perf_counter()
    args_list = [(n, p) for p in primes]

    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        residues = list(executor.map(_compute_one_prime, args_list))

    t_par = perf_counter() - t0
    if verbose:
        print(f"  Parallel mod-p: {t_par:.1f}s ({t_par/num_primes:.2f}s/prime avg)", flush=True)
...
if __name__ == "__main__":
    print("=" * 70)
    print("A000568 via parallel C mod-p DP + CRT")
    ...
    for n_val in range(13):
        t0 = perf_counter()
        result = a000568_parallel(n_val, verbose=False, num_workers=1)
        dt = perf_counter() - t0
    ...
    for n_test in [20, 30, 40, 50, 60, 70, 76, 80]:
        t0 = perf_counter()
        result = a000568_parallel(n_test, verbose=True)
        dt = perf_counter() - t0
        s = str(result)
        extra = ""
        if n_test == 76:
