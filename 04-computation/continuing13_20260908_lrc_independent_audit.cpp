...
int main(int argc,char**argv){
    ...
    // Independent second pass: every native compatible ratio, with no shortcut.
    I native_compatible=0,native_zeros=0;
    for(auto&E:ex){
        auto&D=domains[E.domain];set<int> states(D.begin(),D.end()),es;
        for(int a:D)for(int b:D)es.insert(gcd(a,b));
        array<array<I,91>,91> allW;for(auto&row:allW)row.fill(INF);
        ofstream zfile(dir+"/zeros_"+to_string(E.t)+".txt");
        I local=0,zeros=0;
        for(int e:es)for(auto&G:AT){
            int n=E.t/e,dp=e*gcd(n,G.p),dq=e*gcd(n,G.q);
            if(!states.count(dp)||!states.count(dq))continue;
            local++;native_compatible++;
            auto cp=capacity(G,n);need(cp.first>=0,"unpruned native capacity nonnegative");
            allW[dp][dq]=allW[dq][dp]=min(allW[dp][dq],e*cp.first);
            if(cp.first)continue;
            native_zeros++;zeros++;
            zfile<<e<<' '<<G.p<<' '<<G.q<<' '<<dp<<' '<<dq<<'\n';
            set<I>walls={0};for(auto ab:G.intervals){walls.insert(mod(n*ab.first,G.L));walls.insert(mod(n*ab.second,G.L));}
            I best=INF;
            for(I wall:walls)for(I phase:{2*wall,2*wall+1})best=min(best,count_at(G,n,phase));
            need(best==0,"every zero-bank phase wall and chamber has the correct minimum");
            need(literal(G,n,cp.second)==0,"literal entire quotient grid at every native zero attainer");
            need(gcd(E.t,e*G.p)==dp&&gcd(E.t,e*G.q)==dq,"separate actual realization of each zero ratio");
        }
        for(auto [ab,v]:E.weights)need(allW[ab.first][ab.second]==v[0],"all-ratio unpruned minimum agrees with full table");
        if(E.t==9240)need(local==53195&&zeros==2235,"entire9240 native bank");
        else need(E.t==11088&&local==48641&&zeros==1849,"entire11088 native bank");
        cerr<<"FULL_NATIVE "<<E.t<<" ratios="<<local<<" zeros="<<zeros<<"\n";
    }
    need(native_compatible==101836&&native_zeros==4084,"complete two-clock native universe");
    cout<<"NATIVE_BANKS "<<native_compatible<<" COMPLETE_RATIOS; ZERO_ARMS "<<native_zeros<<"\n";
    ...
    cout<<"UPPER_CLOCKS_EXCLUDED "<<excluded<<"; MINIMUM_MARGIN "<<minupper<<"; BOTH_DECLARED_CLOCKS_COMPLETE\n";
    need(excluded==2&&hostiles==0,"both declared clocks close with no residual");
