#!/usr/bin/env python3
"""
beta2_delta_n7.py — Test δ-injectivity at n=7 (sampled)

Verify HYP-249 (interior v gives injective δ) and HYP-250 (some v works) at n=7.
...
def check_delta_fast(A, n, v):
    """Check δ-injectivity for (T, v). Returns (is_injective, h2_rel)."""
...
print("=" * 70)
print("δ-INJECTIVITY AT n=7 (SAMPLED)")
print("=" * 70)
...
m = n*(n-1)//2
total = 1 << m
n_samples = 200  # Keep small due to n=7 being slow
...
for idx, bits in enumerate(samples):
    ...
    # Pick ONE interior vertex (fastest check)
    interior_v = [v for v in range(n) if 1 <= scores[v] <= n-2]

    has_good = False
    for v in interior_v[:2]:  # Check up to 2 interior vertices
        inj, h2_rel = check_delta_fast(A, n, v)
        if h2_rel > 0:
            total_interior += 1
            if inj:
                has_good = True
                ...
                print(f"  INTERIOR FAIL! bits={bits} scores={sc} v={v} d+={scores[v]}")
        else:
            has_good = True  # H₂(T,T\v)=0

    some_v_works += (1 if has_good else 0)

    if (idx + 1) % 50 == 0:
        elapsed = time.time() - t0
        print(f"  {idx+1}/{n_samples} ({elapsed:.0f}s)")

elapsed = time.time() - t0
print(f"\nn=7: {n_samples} samples in {elapsed:.0f}s")
print(f"  Interior checks: {total_interior}")
print(f"  Interior failures: {fail_interior}")
print(f"  Some v works: {some_v_works}/{n_samples}")

if fail_interior == 0:
    print(f"  ✓ δ injective for ALL tested interior vertices at n=7")
else:
    print(f"  ✗ {fail_interior} interior failures!")
