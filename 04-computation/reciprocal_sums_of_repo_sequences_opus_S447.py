...
of the poly/#P tower (THM-1970/1975).  Historical script corrected by
MISTAKE-209/213: census rows are finite term-multiset prefixes, no arithmetic
type follows from rapid convergence, and global H-support divergence is open.
Use THM-2000/2005 as the canonical support-semantic referees.
"""
from fractions import Fraction as F
from math import comb, log, e, pi

print("CORRECTION=MISTAKE-209/213;historical_finite-prefix_demo;canonical_referee=THM-2000/2005")
print("="*72)
...
print(f"    true c3-max parity splice                       -> sum 1/c3max=75/4-24log(2)={75/4-24*log(2):.10f}")
...
print("\n" + "="*72)
print("PART 2 -- COUNTING sequences: finite term-multiset prefixes (not constants)")
print("="*72)
SEQS = {
...
def cf(x, n=8):
    ...
    for _ in range(n):
        ...
        x=1/x
    return a
print("\n  continued fractions of finite prefixes (diagnostic only; no inverse-symbolic inference):")
...
print("\n" + "="*72)
print("PART 4 -- ABEL-DINI: the exact exponent-one boundary diverges")
print("  divergent S=sum a_n: sum a_n/S_n DIVERGES but sum a_n/S_n^(1+eps) CONVERGES (any eps>0)")
print("="*72)
# Model calculation on all odd numbers; this is not the unproved global H-value support.
...
print(f"  odd-number MODEL sum 1/odd (DIVERGES, harmonic-slow); partial(2e5)={partial:.4f}")
...
print(" k/(k-1) (arc->2, triple-slots->3/2, var->3/4); COUNTING rows above are finite prefixes;")
print(" global H-VALUE support divergence is OPEN -- odds are only the conditional comparison model")
print(" under the positive-density conjecture H's value-set would sit at divergence; this is not proved.")
print(" Abel-Dini says exponent one still diverges, while every exponent 1+epsilon converges.")
