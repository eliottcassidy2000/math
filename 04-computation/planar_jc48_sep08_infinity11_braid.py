#!/usr/bin/env python3
"""Exact infinity-eleven two-cusp configuration witnesses; companion proof required.

Default verifies five rational moving-centre witnesses; --produce NAME proposes and certifies
...
COEFS={(6, 0):Q(1,1),
 ...
 (3, 2):Q(-2,1),
 (3, 1):Q(-70343,1458),
 (3, 0):Q(1147892116,531441),
 (2, 2):Q(-1355,54),
 (2, 1):Q(-2891327,19683),
 (2, 0):Q(-199929248,177147),
 (1, 3):Q(14,3),
 (1, 2):Q(-492908,2187),
 (1, 1):Q(-9758176,19683),
 (1, 0):Q(-1999270400,59049),
 (0, 4):Q(1,1),
 (0, 3):Q(-16772,729),
 (0, 2):Q(-206228,2187),
 (0, 1):Q(-43462400,6561)}
PATHS={
 'smooth':(C(Q(8,3)),[1,2,-1]),
 'cusp_plus':(C(Q(13,3)),[1,1,1]),
 'cusp_minus':(C(Q(-19,3)),[2,1,3,3,3,-1,-2]),
 'node_real':(C(Q(-8835419,2**20)),[2,1,3,3,1,1,-3,-3,-1,-2]),
 'node_upper':(C(Q(-9133949,2**20),Q(3934729,2**20)),[2,1,3,2,2,-3,-1,-2]),
 'node_lower':(C(Q(-9133949,2**20),Q(-3934729,2**20)),[2,-3,2,2,3,-2])}
SELECTED=['smooth','cusp_plus','cusp_minus','node_real','node_lower']
BASE=C(4,3)
...
def certpath(name):
 return Path(__file__).resolve().parents[1]/('05-knowledge/results/planar_jc48_sep08_infinity11_braid_'+name+'_certificate.json.gz')
def proposals():
 import numpy as np
 def roots(u):
  ts=np.roots([1,-8/3,-2,8,-u.floating()])
  zs=[t**6-4*t**5-1.5*t**4+548*t**3/27-368*t/9 for t in ts]
...
def geometry():
 ...
 V=t**6-4*t**5-S.Rational(3,2)*t**4+S.Rational(548,27)*t**3-S.Rational(368,9)*t
 ...
 H=59049*u**3+1526283*u*u+13980187*u+44759481
 disc=-S.Rational(104857600000000,328256967394537077627)*(3*u-13)**3*(3*u-8)*(3*u+19)**3*H**2
 ...
 check(S.Poly(S.gcd(S.diff(U,t),S.diff(V,t)),t).monic().as_expr()==t*t-1,'exact cusp critical locus')
 for e in [-1,1]:
...
def algebra():
 def inverse(word):return [-i for i in word[::-1]]
 splits={
  'smooth':([1],[2]),
  'cusp_plus':([],[1,1,1]),
  'cusp_minus':([2,1],[3,3,3]),
  'node_real':([2,1,3,3],[1,1]),
  'node_lower':([2,-3],[2,2])}
 for name,(prefix,core) in splits.items():
  check(PATHS[name][1]==prefix+core+inverse(prefix),'literal conjugated word decomposition')
 check(free_action([1])==[(1,2,-1),(1,),(3,),(4,)],'smooth prefix exposes a,c')
 check(free_action([2,1])==[(1,2,3,-2,-1),(1,),(2,),(4,)],'second cusp prefix exposes b,d')
 check(free_action([2,1,3,3])[:2]==[(1,2,3,-2,-1),(1,)],'real node first pair')
 check(free_action([2,-3])[1:3]==[(2,3,-2),(4,)],'lower node pair')
 # a=c and braid(a,b) simplify the real node pair to (b,a), as proved in prose.
 # Following its commutation, a=b=c; the lower node then exposes (a,d).
 check(free_action([2,-3],[(1,),(1,),(1,),(4,)])[1:3]==[(1,),(4,)],'lower node after first three equalities')
 for name in SELECTED:
  check(free_action(PATHS[name][1],[(1,)]*4)==[(1,)]*4,'cyclic converse control')
 return 'analytic necessity: a=c, braid(a,b), [a,b]=1, [a,d]=1, braid(b,d); hence a=b=c=d'
...
def main():
 ...
 print('FINITE-EXACT INFINITY11 TWO-CUSP RATIONAL BRAID PASS; analytic consumer proved separately')
