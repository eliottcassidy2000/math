// Exact fixed-50 fifth-petal Haar chart scout.
//
// Starting from the four-petal set {10,15,20,30}, this program tests each
// possible fifth petal.  For the selected chart it then audits every triple,
// quadruple, and quintuple face, with core sizes 6, 5, and 4 respectively.
// The limiting pass records exact Haar surplus, circular component count, and
// the resulting THM-4170 tail cutoff; the literal pass exhausts each finite
// head.  Lower faces are inherited from THM-4211/4229/4234.
...
constexpr int Q=50;
constexpr std::array<int,4> BASE={0,1,2,3}; // 10,15,20,30 in O.
...
int main(int argc,char** argv) {
    ...
    const std::uint16_t base=static_cast<std::uint16_t>(
        bit(BASE[0])|bit(BASE[1])|bit(BASE[2])|bit(BASE[3]));
    std::uint64_t total_profiles=0;
    int candidates=0;
    bool best_set=false;
    i128 best_floor=0;
    int best_extra=-1,best_cutoff=0;
    bool cheapest_set=false;
    i128 cheapest_floor=0;
    int cheapest_extra=-1,cheapest_cutoff=0;
    for (int extra=0;extra<12;++extra) {
        if (base&bit(extra)) continue;
        ++candidates;
        i128 candidate_floor=0;
        bool floor_set=false;
        int candidate_cutoff=0;
        int candidate_nonstrict=0;
        for (int first=0;first<4;++first) for (int second=first+1;
             second<4;++second) {
            const std::uint16_t required=static_cast<std::uint16_t>(
                bit(extra)|bit(BASE[first])|bit(BASE[second]));
        ...
        for (int omitted=0;omitted<4;++omitted) {
            ...
            for (int j=0;j<4;++j) if (j!=omitted) required|=bit(BASE[j]);
            const BaseStats stats=base_stats(arrangement,required,5);
            require(stats.profiles==8568,"quadruple profile count changed");
            total_profiles+=stats.profiles;
            candidate_cutoff=std::max(candidate_cutoff,stats.max_cutoff);
        ...
        const std::uint16_t required=static_cast<std::uint16_t>(base|bit(extra));
        const BaseStats stats=base_stats(arrangement,required,4);
        require(stats.profiles==3060,"quintuple profile count changed");
        total_profiles+=stats.profiles;
        candidate_cutoff=std::max(candidate_cutoff,stats.max_cutoff);
        candidate_nonstrict+=stats.nonstrict;
        if (!floor_set || stats.min_delta<candidate_floor) {
        ...
        std::cout << "LAYER extra=" << O[extra]
                  << " kind=quintuple labels=" << o_labels(required)
                  << " profiles=" << stats.profiles
                  << " nonstrict=" << stats.nonstrict
                  << " min_delta=" << decimal(stats.min_delta)
                  << " min_body=" << c_labels(stats.min_body)
        ...
        if (candidate_nonstrict==0 &&
            (!cheapest_set || candidate_cutoff<cheapest_cutoff ||
             (candidate_cutoff==cheapest_cutoff &&
              candidate_floor>cheapest_floor))) {
            cheapest_set=true;
            cheapest_floor=candidate_floor;
            cheapest_extra=extra;
            cheapest_cutoff=candidate_cutoff;
        }
    }
    require(candidates==8,"candidate count changed");
    require(total_profiles==UINT64_C(1189728),"total profile count changed");
    std::cout << "SUMMARY denominator=" << arrangement.denominator
              << " cells=" << arrangement.cells.size()
              << " candidates=" << candidates
              << " profiles=" << total_profiles
              << " strict_candidates=" << (best_set ? "positive" : "zero")
              << " best_extra=" << O[best_extra]
              << " best_min_delta=" << decimal(best_floor)
              << " best_max_cutoff=" << best_cutoff
              << " cheapest_extra=" << O[cheapest_extra]
              << " cheapest_min_delta=" << decimal(cheapest_floor)
              << " cheapest_max_cutoff=" << cheapest_cutoff << '\n';

    if (!(argc==2 && std::string(argv[1])=="--selected-five-petal")) return 0;
    require(best_set,"no strict fifth-petal candidate");

    const int EXTRA=best_extra;
    struct Lane {
    ...
    const std::array<int,5> chart={BASE[0],BASE[1],BASE[2],BASE[3],EXTRA};
    std::array<Lane,16> lanes{};
    int lane_count=0;
    for (int first=0;first<5;++first) for (int second=first+1;second<5;++second)
        for (int third=second+1;third<5;++third) {
        const std::uint16_t required=static_cast<std::uint16_t>(
            bit(chart[first])|bit(chart[second])|bit(chart[third]));
        const BaseStats stats=base_stats(arrangement,required,6);
        lanes[lane_count].required=required;
        lanes[lane_count].core_size=6;
        lanes[lane_count].cutoff=stats.max_cutoff;
        ++lane_count;
    }
    for (int omitted=0;omitted<5;++omitted) {
        std::uint16_t required=0;
        for (int j=0;j<5;++j) if (j!=omitted) required|=bit(chart[j]);
        const BaseStats stats=base_stats(arrangement,required,5);
        lanes[lane_count].required=required;
        lanes[lane_count].core_size=5;
        lanes[lane_count].cutoff=stats.max_cutoff;
        ++lane_count;
    }
    lanes[lane_count].required=static_cast<std::uint16_t>(base|bit(EXTRA));
    lanes[lane_count].core_size=4;
    lanes[lane_count].cutoff=
        base_stats(arrangement,lanes[lane_count].required,4).max_cutoff;
    ++lane_count;
    require(lane_count==16,"selected lane count changed");

    int literal_stop=0;
    for (const Lane& lane:lanes)
        literal_stop=std::max(literal_stop,lane.cutoff);
    ...
    for (int outsider=1;outsider<literal_stop;++outsider) {
        ...
        std::array<i128,16> row_min_delta{};
        std::array<i64,16> row_min_mass{};
        std::array<u32,16> row_min_body{};
        std::array<std::uint64_t,16> row_failures{},row_equalities{};
#pragma omp parallel for schedule(static)
            ...
            for (u32 body=ALL;;body=(body-1)&ALL) {
                if (std::popcount(body)==lane.core_size) {
                    ...
                    if (!initialized || delta<least) {
                        ...
                        least=delta;
                        least_body=body;
                        row_min_mass[lane_index]=mass[ALL^body];
                    }
                    bad+=delta<0;
                    equal+=delta==0;
                }
        ...
        for (int lane_index=0;lane_index<16;++lane_index) {
            ...
            if (outsider>=lane.cutoff) continue;
            ++lane.rows;
            lane.checks+=lane.core_size==6 ? UINT64_C(18564) :
                         lane.core_size==5 ? UINT64_C(8568) : UINT64_C(3060);
            ...
            if (!lane.min_set ||
                ...
                lane.min_mass=row_min_mass[lane_index];
                lane.min_r=outsider;
    ...
    for (const Lane& lane:lanes) {
        ...
        std::cout << "LITERAL kind="
                  << (lane.core_size==6 ? "triple" :
                      lane.core_size==5 ? "quadruple" : "quintuple")
                  ...
                  << " min_mass=" << lane.min_mass
                  << " min_r=" << lane.min_r
                  << " min_body=" << c_labels(lane.min_body) << '\n';
    }
    std::cout << "LITERAL_SUMMARY labels="
              << o_labels(static_cast<std::uint16_t>(base|bit(EXTRA)))
              << " rows=" << total_rows
              << " checks=" << total_checks
