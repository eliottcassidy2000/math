#!/usr/bin/env python3
"""Independent exact referee for the fixed projected z1=2004 state.
...
OUTPUT_PATH = (
    ...
    / "05-knowledge"
    / "results"
    / "lrc14_j7_k2_z2004_ray_status_independent_thm2941.out"
)
EXPECTED_UNIFORM_SHA256 = (
    "dfa4788297b8c31fc9b5dce1afadf29d20b267cb4159fa95dadb9346b1980b36"
)
EXPECTED_SEMANTIC_SHA256 = (
    "5cead518b79fb5f992d70fa894f7cf2914c2403bb5d5a0af4893efc2e0cce67d"
)
BODY = (1, 4, 8, 10, 12, 14)
FIRST = 2004
LABELS = (2004, 2060, 2142, 2172, 2534)
DS = (280, 588, 840, 980, 980)
D = 5880
Q = 840
M = 7
CRUDE_DS = (196, 588, 980, 980, 2940)
CRUDE_D = 2940
CRUDE_Q = 420
...
def main():
    ...
    crude_capacity = sum(
        U.fibre_cap(CRUDE_D, d, CRUDE_Q) for d in CRUDE_DS
    )
    require(
        (CRUDE_D // CRUDE_Q, crude_target, crude_capacity) == (7, 6, 5),
        ("independent crude kill changed", crude_histogram, crude_capacity),
    )
    ...
    lines = [
        "LRC14 projected k=2 z1=2004 independent fixed-state referee",
        f"uniform_ray_status_source_sha256={file_sha256(UNIFORM)}",
        "primary_z2004_source_imported=False;tree_engine=Pruefer;LP_used=False",
        (
        ...
        (
            f"independent_crude_kill=ds:{CRUDE_DS};D:{CRUDE_D};q:{CRUDE_Q};"
            f"M:{CRUDE_D//CRUDE_Q};target:{crude_target};"
            f"capacity:{crude_capacity};histogram:{crude_histogram}"
        ),
        (
        ...
        "conclusion=fixed projected z1=2004 scalar state is impossible",
