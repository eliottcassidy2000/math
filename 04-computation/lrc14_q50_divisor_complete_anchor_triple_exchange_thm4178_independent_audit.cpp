...
Hypergraph build_hypergraph(const Geometry& geometry,
    ...
    require(arity == 5 || arity == 6, "unsupported deletion arity");
...
void run_audit() {
    ...
    std::cout << "AUDIT q50_anchor_exchange_cleanroom_cpp20\n";
    std::cout << "POOL";
    ...
    for (const auto& anchor : anchors) {
        ...
        for (int arity : {5, 6}) {
            ...
            if (anchor == ORIGINAL_ANCHOR && arity == 5) {
                require(hypergraph.edges.size() == 2063 &&
                            hypergraph.equality_count == 0 && cover.tau == 5,
                        "original-anchor d=5 control failed");
            }
            if (anchor == ORIGINAL_ANCHOR && arity == 6) {
                require(hypergraph.edges.size() == 46261 &&
                            hypergraph.equality_count == 0 && cover.tau == 8,
                        "original-anchor d=6 control failed");
            }
