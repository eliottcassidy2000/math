// Exact post-B-gate puncture certificate for THM-839 (using THM-828's join).
//
...
// word are then checked directly.  This replay additionally retains every
// B-compatible base, proves the local two-toggle parity criterion for raw S2,
// and computes the exact reflection action on the candidate bases.  It is a
// deliberately independent full replay rather than an analysis of a stored
// witness subset.
...
struct BBase { uint32_t D,u; };

int main(int argc,char**argv) {
    std::string atlas="/tmp/n8_merged_node_rank_u16.bin", witnesses="/tmp/n9_puncture_witnesses.tsv",summary,json;
    ...
    // THM-839: retain the entire post-B bank and compress every active raw-S2
    // layer to two parity tests.  On this bank D toggles exactly two low
    // cells and their two reflected high cells in each active layer.
    using Masks = std::vector<std::tuple<int,uint32_t,uint32_t>>;
    std::map<uint32_t,Masks> active_masks;
    std::map<uint32_t,std::map<std::string,uint64_t>> chi_words;
    std::map<uint32_t,std::array<uint64_t,7>> prefix_by_d;
    std::map<uint32_t,std::map<int,uint64_t>> first_fail_by_d;
    std::map<uint32_t,std::set<uint32_t>> bases_by_d;
    uint64_t active_layer_tests=0,chi_pair_equal=0,upper_black_bases=0;
    for(const auto& base:b_bases) bases_by_d[base.D].insert(base.u);
    for(const auto& [D,bases]:bases_by_d) {
        Masks masks;
        for(int layer=0;layer<6;++layer) {
            uint32_t lo=0,hi=0;
            for(int i=0;i<(int)t9.size();++i) if(t9[i].a+t9[i].b-1==layer+3) {
                if((D>>i)&1u) lo|=1u<<i;
                if((D>>s9[i])&1u) hi|=1u<<s9[i];
            }
            if(lo||hi) {
                if(__builtin_popcount(lo)!=2||__builtin_popcount(hi)!=2)
                    throw std::runtime_error("active S2 layer is not a two-toggle pair");
                if(reflect_mask(lo,s9)!=hi)
                    throw std::runtime_error("active low/high toggle sets are not reflected mates");
                masks.push_back({layer,lo,hi});
            }
        }
        active_masks[D]=masks;
        for(uint32_t u:bases) {
            uint32_t v=u^D;
            upper_black_bases+=(reflect_mask(u,s9)!=u&&reflect_mask(v,s9)!=v);
            auto wu=s2_word(u,t9,s9),wv=s2_word(v,t9,s9);
            bool prefix=true; int first=-1;
            for(int k=0;k<7;++k) {
                bool equal=wu[k]==wv[k];
                prefix=prefix&&equal;
                if(prefix)++prefix_by_d[D][k];
                if(!equal&&first<0)first=k+3;
            }
            if(first>=0)++first_fail_by_d[D][first];
            std::string word;
            bool all_chi=true;
            for(auto [layer,lo,hi]:masks) {
                int chi_lo=__builtin_parity(u&lo),chi_hi=__builtin_parity(u&hi);
                ++active_layer_tests;chi_pair_equal+=(chi_lo==chi_hi);
                if(chi_lo!=chi_hi)throw std::runtime_error("low/high chi disagree");
                bool layer_equal=wu[layer]==wv[layer];
                if(layer_equal!=(chi_lo==1&&chi_hi==1))
                    throw std::runtime_error("two-toggle S2 criterion failed");
                word.push_back(char('0'+chi_lo));all_chi=all_chi&&chi_lo;
            }
            if((wu==wv)!=all_chi)throw std::runtime_error("chi word does not decide full S2");
            ++chi_words[D][word];
        }
    }

    // Reflection acts on canonical A-oriented bases.  A fixed point below
    // means that reflection swaps the two endpoints; it does not imply raw
    // ordered-S2 equality, because states 01 and 10 are interchanged.
    struct OrbitCounts {uint64_t pointwise=0,swap_fixed=0,cycle_points=0,cycles=0,raw_equal=0;};
    std::map<uint32_t,OrbitCounts> reflection_orbits;
    uint64_t pointwise_total=0,swap_fixed_total=0,cycle_points_total=0,cycle_total=0;
    uint64_t raw_equal_fixed=0,raw_equal_cycles=0;
    for(const auto& [D,bases]:bases_by_d) {
        uint32_t dA=restrict_face(D,0,fm);
        std::set<uint32_t> seen;
        for(uint32_t u:bases) {
            uint32_t ru=reflect_mask(u,s9);
            uint32_t rA=restrict_face(ru,0,fm);
            if(rA>(rA^dA))ru^=D;
            if(!bases.count(ru))throw std::runtime_error("reflection left B-base fibre");
            auto wu=s2_word(u,t9,s9),wv=s2_word(u^D,t9,s9);
            bool raw_equal=wu==wv;
            if(ru==u) {
                if(reflect_mask(u,s9)==u) {
                    ++reflection_orbits[D].pointwise;++pointwise_total;
                } else if(reflect_mask(u,s9)==(u^D)) {
                    ++reflection_orbits[D].swap_fixed;++swap_fixed_total;
                } else throw std::runtime_error("unknown fixed reflection type");
                raw_equal_fixed+=raw_equal;
            } else {
                ++reflection_orbits[D].cycle_points;++cycle_points_total;
                raw_equal_cycles+=raw_equal;
            }
            reflection_orbits[D].raw_equal+=raw_equal;
            if(!seen.count(u)){seen.insert(u);seen.insert(ru);if(ru!=u){++reflection_orbits[D].cycles;++cycle_total;}}
        }
    }

    const uint32_t hole_face=0x1026286,hole_8=0x4dd3c9e,hole_big=0x54a5692,hole_7=0x5537214;
    auto rb=range_for(restrict_face(hole_face,1,fm));
    uint64_t hole_face_LB=rb.second-rb.first;
    if(b_bases.size()!=636||active_layer_tests!=1810||chi_pair_equal!=active_layer_tests||upper_black_bases!=636)
        throw std::runtime_error("post-B local parity census mismatch");
    if(hole_face_LB!=4144||bases_by_d.count(hole_face))
        throw std::runtime_error("face-supported puncture mismatch");
    if(chi_words[hole_8]!=std::map<std::string,uint64_t>{{"1110",4}}||
       chi_words[hole_big]!=std::map<std::string,uint64_t>{{"000",500},{"100",4}}||
       chi_words[hole_7]!=std::map<std::string,uint64_t>{{"100",2},{"101",2}})
        throw std::runtime_error("puncture chi-word mismatch");
    if(first_fail_by_d[hole_8]!=std::map<int,uint64_t>{{8,4}}||
       first_fail_by_d[hole_big]!=std::map<int,uint64_t>{{5,500},{7,4}}||
       first_fail_by_d[hole_7]!=std::map<int,uint64_t>{{7,4}})
        throw std::runtime_error("puncture first-failure mismatch");
    if(prefix_by_d[hole_8]!=std::array<uint64_t,7>{4,4,4,4,4,0,0}||
       prefix_by_d[hole_big]!=std::array<uint64_t,7>{504,504,4,4,0,0,0}||
       prefix_by_d[hole_7]!=std::array<uint64_t,7>{4,4,4,4,0,0,0})
        throw std::runtime_error("puncture prefix mismatch");
    auto killer_mask=[&](uint32_t D,int tau){for(auto [k,lo,hi]:active_masks[D])if(k+3==tau)return std::pair{lo,hi};throw std::runtime_error("killer layer inactive");};
    if(killer_mask(hole_8,8)!=std::pair<uint32_t,uint32_t>{0x0010002,0x0100080}||
       killer_mask(hole_big,7)!=std::pair<uint32_t,uint32_t>{0x0020400,0x1080000}||
       killer_mask(hole_7,7)!=std::pair<uint32_t,uint32_t>{0x0020004,0x1002000})
        throw std::runtime_error("puncture killer mask mismatch");
    if(pointwise_total!=0||swap_fixed_total!=388||cycle_points_total!=248||cycle_total!=124||
       raw_equal_fixed!=58||raw_equal_cycles!=0||
       reflection_orbits[hole_big].swap_fixed!=256||reflection_orbits[hole_big].cycle_points!=248||
       reflection_orbits[hole_big].cycles!=124||reflection_orbits[hole_8].swap_fixed!=4||
       reflection_orbits[hole_7].swap_fixed!=4)
        throw std::runtime_error("reflection-orbit puncture census mismatch");
    const std::array<uint32_t,4> punctures{hole_face,hole_8,hole_big,hole_7};
    auto killer_tau=[&](uint32_t D){if(D==hole_face)return 0;if(D==hole_8)return 8;return 7;};
    uint64_t puncture_gauge_flips=0;
    for(int i=0;i<4;++i)for(int j=i+1;j<4;++j) {
        uint32_t a=punctures[i],b=punctures[j];
        auto sa=std::tuple{a==hole_face?0:1,killer_tau(a),a};
        auto sb=std::tuple{b==hole_face?0:1,killer_tau(b),b};
        auto ea=std::tuple{-int64_t(bases_by_d[a].size()),a};
        auto eb=std::tuple{-int64_t(bases_by_d[b].size()),b};
        puncture_gauge_flips+=((sa<sb)!=(ea<eb));
    }
    if(puncture_gauge_flips!=4)throw std::runtime_error("puncture Tournament Analysis mismatch");
    ...
    report<<"THM-839 N=9 DEFECT PUNCTURE CERTIFICATE / THM-828 FULL REPLAY\n"
    ...
    report<<"\n"
             <<"post-B parity bases/tests/chi-low-high-equal/upper-black="<<b_bases.size()<<'/'
             <<active_layer_tests<<'/'<<chi_pair_equal<<'/'<<upper_black_bases<<"\n"
             <<"reflection action pointwise/swap-fixed/cycle-points/cycles="<<pointwise_total<<'/'
             <<swap_fixed_total<<'/'<<cycle_points_total<<'/'<<cycle_total
             <<" raw-S2-equal fixed/cycles="<<raw_equal_fixed<<'/'<<raw_equal_cycles<<"\n"
             <<"face puncture 1026286 dA/dB/dC="<<std::hex<<restrict_face(hole_face,0,fm)<<'/'
             <<restrict_face(hole_face,1,fm)<<'/'<<restrict_face(hole_face,2,fm)<<std::dec
             <<" L_A/L_B/L_C=0/"<<hole_face_LB<<"/0\n"
             <<"puncture 4dd3c9e B/prefix/chi/killer="<<bases_by_d[hole_8].size()<<"/[4,4,4,4,4,0,0]/1110x4/tau8"
             <<" lo=0010002 hi=0100080 fixed/cycle=4/0\n"
             <<"puncture 54a5692 B/prefix/chi/killer="<<bases_by_d[hole_big].size()<<"/[504,504,4,4,0,0,0]/000x500+100x4/tau7"
             <<" lo=0020400 hi=1080000 fixed/cycle-points=256/248\n"
             <<"puncture 5537214 B/prefix/chi/killer="<<bases_by_d[hole_7].size()<<"/[4,4,4,4,0,0,0]/100x2+101x2/tau7"
             <<" lo=0020004 hi=1002000 fixed/cycle=4/0\n"
    ...
    report<<"\nraw-codec fibres singletons/doubletons/cells/excess/max="
             ...
             <<"PUNCTURE TOURNAMENT ANALYSIS vertices=4 proof obligations\n"
             <<"  observable=(gate kind,killer layer,B-mass); switches=structural/empirical; tie-path=hex D\n"
             <<"  both gauges: score histogram 0..3 once, C3=0, SCC=1^4, Hamiltonian paths=1; edge flips="<<puncture_gauge_flips<<"\n"
             <<"  preserves=B-gate and raw-S2 truth; destroys=node fibre, future continuation, LRC metric\n"
             <<"  challenged assumption=punctures are not absent nodes; use local parity obligations as vertices\n";
    std::cout<<report.str();
    ...
    if(!json.empty()) {
        ...
        o<<"],\n  \"collision_pairs\": "<<found.size()<<",\n  \"distinct_endpoints\": "<<endpoints.size()<<",\n"
         ...
         <<"  \"puncture_certificate\": {\n"
         <<"    \"post_B_bases\": "<<b_bases.size()<<", \"active_layer_tests\": "<<active_layer_tests<<", \"chi_low_high_equal\": "<<chi_pair_equal<<", \"upper_black\": "<<upper_black_bases<<",\n"
         <<"    \"reflection_action\": {\"pointwise_fixed\": "<<pointwise_total<<", \"endpoint_swap_fixed\": "<<swap_fixed_total<<", \"cycle_points\": "<<cycle_points_total<<", \"two_cycles\": "<<cycle_total<<", \"raw_S2_equal_fixed\": "<<raw_equal_fixed<<", \"raw_S2_equal_cycle_points\": "<<raw_equal_cycles<<"},\n"
         <<"    \"holes\": {\n"
         <<"      \"0x1026286\": {\"gate\": \"A/C face support\", \"dA\": \"0x084883\", \"dB\": \"0x001946\", \"dC\": \"0x0204c5\", \"L_A\": 0, \"L_B\": "<<hole_face_LB<<", \"L_C\": 0},\n"
         <<"      \"0x4dd3c9e\": {\"B_bases\": 4, \"prefix\": [4,4,4,4,4,0,0], \"chi_words\": {\"1110\": 4}, \"killer_tau\": 8, \"low_mask\": \"0x0010002\", \"high_mask\": \"0x0100080\", \"swap_fixed\": 4, \"cycle_points\": 0},\n"
         <<"      \"0x54a5692\": {\"B_bases\": 504, \"prefix\": [504,504,4,4,0,0,0], \"chi_words\": {\"000\": 500, \"100\": 4}, \"first_fail\": {\"5\": 500, \"7\": 4}, \"killer_tau\": 7, \"low_mask\": \"0x0020400\", \"high_mask\": \"0x1080000\", \"swap_fixed\": 256, \"cycle_points\": 248, \"two_cycles\": 124},\n"
         <<"      \"0x5537214\": {\"B_bases\": 4, \"prefix\": [4,4,4,4,0,0,0], \"chi_words\": {\"100\": 2, \"101\": 2}, \"killer_tau\": 7, \"low_mask\": \"0x0020004\", \"high_mask\": \"0x1002000\", \"swap_fixed\": 4, \"cycle_points\": 0}\n"
         <<"    },\n"
         <<"    \"tournament_analysis\": {\"vertices\": \"four proof obligations\", \"observable\": [\"gate kind\", \"killer layer\", \"B mass\"], \"switches\": [\"structural\", \"empirical\"], \"tie_hamiltonian_path\": \"hex D\", \"directed_triangles\": 0, \"scc_sizes\": [1,1,1,1], \"hamiltonian_paths\": 1, \"edge_flips\": "<<puncture_gauge_flips<<"}\n"
         <<"  },\n"
