...
The two already-certified bodies E_57 and E_67 therefore close every final
family containing either anchor.  For a final family containing five labels
from {1,...,7}, 14 of the 21 possible five-sets contain an anchor.  For each
of the seven residual five-sets K, put P=H union K.  P has twelve speeds, so
...
seven finite ranges contain 456 values in total and all are positive.  With
the one unanchored six-set checked directly, this proves the strict shadow:
...
the seven residual five-label bases.  Its pair observable is the difference
...
ANCHORS = (frozenset((5, 7)), frozenset((6, 7)))
...
def main() -> None:
    ...
    require((len(five_sets), len(anchored_five), len(residual_five)) == (21, 14, 7), "bad five-set split")
    ...
    require(total_sweeps == 456, f"unexpected sweep total {total_sweeps}")
    ...
    require((len(six_sets), len(anchored_six), len(residual_six)) == (7, 6, 1), "bad six-set split")
    residual_six_family = tuple(sorted(H | residual_six[0]))
    require(len(residual_six_family) == 13, "residual six-set is not a final family")
    _, residual_six_r, residual_six_m = core.good_norm(residual_six_family)
    require(residual_six_m > 0, "unanchored six-small family is not strict")
    ...
    print("five-small sets total/anchor-shadow/residual=21/14/7")
    ...
    print(
        "six-small sets total/anchor-shadow/residual=7/6/1 ; "
        f"residual_K={tuple(sorted(residual_six[0]))} r={residual_six_r} "
        f"measure={residual_six_m}"
    )
    print("Tournament Analysis vertices: seven residual five-label bases (not runners or root edges)")
    print("pair observable: C(B)-C(A), C=(99/70)r/(6m); gauge: lower C -> higher C; tie: lex")
    print(
        f"fingerprint: score_hist={{0..6:1}}, directed_3cycles=0, SCC_sizes=7x1, "
