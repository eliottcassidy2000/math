...
# A general element c+a*x+b*w can generate only if the determinant of
# {1,theta,theta^2} in the free basis {1,x,w} is a unit.  Its reduction
# modulo p factors into three nonunits, proving global nonmonogenicity.
a_gen, b_gen = sp.symbols("a_gen b_gen")
theta2_x = (a_gen**2 * Ctar
            - 2 * a_gen * b_gen * (1 + Ptar**2)
            + b_gen**2 * Ptar * Ctar)
theta2_w = (-a_gen**2 * Ptar
            + 2 * a_gen * b_gen * Ctar
            - b_gen**2 * (1 + Ptar**2))
generator_det = sp.expand(a_gen * theta2_w - b_gen * theta2_x)
expected_det = (
    -a_gen**3 * Ptar + a_gen**2 * b_gen * Ctar
    + a_gen * b_gen**2 * (1 + Ptar**2)
    - b_gen**3 * Ptar * Ctar
)
zero(generator_det - expected_det,
     "finite cubic general-generator determinant")
zero(expected_det.subs(Ptar, 0)
     - a_gen * b_gen * (a_gen * Ctar + b_gen),
     "finite cubic nonmonogenic mod-p factor")
...
summary = {
    ...
    "finite_cubic": "B_2 finite free rank3 over k[p,x+y], globally nonmonogenic by the general-generator determinant mod p; residual interior ramification",
