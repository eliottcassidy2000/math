#!/usr/bin/env python3
"""Consume the repaired endpoint-593 layer into the inherited typed partition."""
...
def main() -> None:
    ...
    prior_union = read_rows(args.prior_union, 2036)
    prior_residual = read_rows(args.prior_residual, 20611)
    top593 = read_rows(args.top593, 16)
    if set(prior_union).isdisjoint(prior_residual) is False:
    ...
    if not set(top593).issubset(prior_residual):
        raise RuntimeError("top593 escaped prior residual")
    if any(r != 593 for _, r in top593):
        raise RuntimeError("top593 endpoint changed")

    prefix391 = read_audit(args.prefix391_audit, 391)
    top594 = read_audit(args.top594_audit, 25)
    expected_audited = set(prefix391) | set(top594) | set(top593)
    if len(expected_audited) != 432:
        raise RuntimeError("inherited/new audit targets overlap unexpectedly")
    audited = read_audit(args.pair_audit, 432)
    if set(audited) != expected_audited or any(audited.values()):
        raise RuntimeError("exchange pair audit did not close exact full432 target")
    with args.failures.open(newline="", encoding="ascii") as handle:
        reader = csv.DictReader(handle)
        if list(reader):
            raise RuntimeError("exchange failure ledger is nonempty")

    new_union = sorted(set(prior_union) | set(top593))
    new_residual = sorted(set(prior_residual) - set(top593))
    if len(new_union) != 2052 or len(new_residual) != 20595:
    ...
    union_sha = write_rows(args.output_dir / "typed_union2052.csv", new_union)
    residual_sha = write_rows(args.output_dir / "final_residual20595.csv", new_residual)
    top_sha = write_rows(args.output_dir / f"residual_top{max_endpoint}.csv", new_top)
    print("LRC14_ENDPOINT593_TYPED_CONSUMER_V1")
    ...
    print(f"CONSUMED_TOP593 {len(top593)} FNV {fnv_rows(top593):016x}")
