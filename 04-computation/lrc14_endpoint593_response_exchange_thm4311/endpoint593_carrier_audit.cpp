...
// THM-4310 endpoint-593 residual layer. This is the hostile baseline audit.
...
namespace {
constexpr u64 kTop593Fnv = UINT64_C(0x5424c07fa724011f);
...
std::vector<PairAgent> read_target_rows(const std::filesystem::path& path) {
    const auto row_set = read_pairs_repaired(path, 16, kTop593Fnv);
    std::vector<PairAgent> rows;
    for (const auto& [q, r] : row_set) {
        require(r == 593, "target escaped endpoint 593");
...
int main(int argc, char** argv) {
    try {
        require(argc == 16 || argc == 17,
                ...
                "FINAL_DELETE TARGET16 PAIR_CSV FAILURE_CSV WORKERS "
                "[HOSTILE_DELETE_ONE]");
        ...
        // though the exact 16-row target is supplied by the typed consumer.
        ...
        require(static_cast<bool>(pair_out) && static_cast<bool>(failure_out),
                "cannot create frontier ledgers");
        ...
        require(pair_out.good() && failure_out.good(),
                "frontier ledgers failed to write");

        std::cout << "LRC14_ENDPOINT593_C3925_RAW_SCOUT_V1\n"
                  ...
                  << (hostile_delete == 0 ? std::string("NONE")
                                          : hex8(hostile_delete)) << '\n'
                  << "ROWS " << rows.size() << " ENDPOINT 593 ROW_FNV "
                  << std::hex << kTop593Fnv << std::dec << " WORKERS "
