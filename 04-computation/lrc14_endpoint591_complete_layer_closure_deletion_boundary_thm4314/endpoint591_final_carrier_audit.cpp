// Frontier-only exact replay of THM-4313's final size-3925 carrier on the
// canonical 13-row endpoint-591 layer.  This is a finite fixed-pool scout;
// it asserts neither a physical entry nor LRC(14).

#define ENDPOINT593_RESPONSE_CAPACITY_MAIN endpoint593_capacity_hidden_for_endpoint591
...
int main(int argc, char** argv) {
    try {
        require(argc == 20,
                "usage: endpoint591 JOINT BASE8951 ADD45 SUFFIX9 UNIVERSE "
                "OLD_UNION REPAIRS76 ADD4 DELETE73 ADD10 FINAL_DELETE "
                "ADDITION593 DELETE593 TOP591 COVER43 DELETE43 PAIR_CSV "
                "FAILURE_CSV WORKERS");
        ...
        for (const auto& [q, r] : row_set) {
            require(r == 591, "frontier row escaped endpoint591");
            rows.push_back(PairAgent{q, r});
            row_ledger.add(q); row_ledger.add(r);
        }
        require(rows.size() == 13 && row_ledger.state == kTop591Fnv,
                "endpoint591 row identity changed");
        ...
        require(pair_out && failure_out, "cannot create endpoint591 ledgers");
        ...
        require(pair_out.good() && failure_out.good(),
                "endpoint591 ledger write failed");
        require(total_exposed == 28791 && total_hits == 1031512 &&
                    total_failures == 0 && failed_rows == 0,
                "endpoint591 aggregate census changed");
        require(failure_ledger.state == UINT64_C(0xcbf29ce484222325) &&
                    pair_ledger.state == UINT64_C(0x8191899a3e142a2c),
                "endpoint591 failure/pair ledger identity changed");

        std::cout << "LRC14_ENDPOINT591_FINAL_CARRIER_AUDIT_V1\n"
                  << "CARRIER " << carrier.size() << " FNV " << std::hex
                  << masks_fnv_agent(carrier) << std::dec << " RANK8 " << rank8
                  << " RANK9 " << carrier.size() - rank8 << " JOINT_RETAINED "
                  << joint.size() << '\n'
                  << "ROWS " << rows.size() << " ENDPOINT 591 ROW_FNV "
        ...
        std::cout << "SUMMARY EXPOSED " << total_exposed
                  ...
                  << "SCOPE FINITE_EXACT_FIXED_POOL_ENDPOINT591_LAYER_"
                     "THM4313_FINAL_CARRIER_ONLY_NO_PHYSICAL_ENTRY_NO_LRC14\n"
                  << "VERDICT "
                  << (total_failures == 0 ? "PASS" : "HOSTILE_FAIL") << '\n';
        return total_failures == 0 ? 0 : 2;
    } catch (const std::exception& error) {
        std::cerr << "ENDPOINT591_AUDIT_ERROR " << error.what() << '\n';
        return 1;
    }
}
