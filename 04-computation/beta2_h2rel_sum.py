#!/usr/bin/env python3
"""
beta2_h2rel_sum.py — Sum of h₂_rel over interior vertices

If Σ_v h₂_rel(T, T\v) < #(interior vertices), then SOME v has h₂_rel=0.

Look for a bound: Σ h₂_rel ≤ f(n, T) < n for all T.

Also: the Euler characteristic approach.
χ(T,T\v) = Σ (-1)^p dim Ω_p^rel = Σ (-1)^p β_p^rel
This gives: β₂^rel - β₁^rel + β₀^rel - β₃^rel + ... = χ^rel
So h₂_rel is constrained by other relative Betti numbers.
...
print("=" * 70)
print("SUM OF h₂_rel OVER ALL VERTICES (including source/sink)")
print("=" * 70)
...
for n in [5, 6, 7]:
    ...
    # Key: for HYP-258 to hold, the interior sum must be < n_interior for all T
    n_interior = n - 2  # minimum interior vertices (when there's exactly one source and one sink)
    # Actually, n_interior varies. But if Σ interior h₂_rel ≤ n - 3 always, then SOME is 0.
    if max_sum < n:
        print(f"  ✓ Σ h₂_rel < n for all T (trivially gives HYP-258)")

# Also check: is there a connection to β₁(T)?
print(f"\n{'='*70}")
print("Connection: Σ h₂_rel vs β₁(T)")
print("=" * 70)
...
for bits in range(1 << m):
    ...
    total_h2 = sum(compute_h2_rel(A, n, v) for v in range(n))
    ...
    sum_vs_beta1.setdefault(beta1_T, Counter())[total_h2] += 1

for b1 in sorted(sum_vs_beta1):
    print(f"  β₁(T)={b1}: Σ h₂_rel distribution = {dict(sorted(sum_vs_beta1[b1].items()))}")
