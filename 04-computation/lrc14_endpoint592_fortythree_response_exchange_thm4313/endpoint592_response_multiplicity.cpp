...
#ifndef ENDPOINT592_RESPONSE_POOL_SCOUT_MAIN
#define ENDPOINT592_RESPONSE_POOL_SCOUT_MAIN main
#endif

int ENDPOINT592_RESPONSE_POOL_SCOUT_MAIN(int argc, char** argv) {
    try {
        require(argc == 18,
                "usage: pool JOINT BASE8951 ADD45 SUFFIX9 UNIVERSE OLD_UNION "
                "REPAIRS76 ADD4 DELETE73 ADD10 FINAL_DELETE ADDITION1 DELETE1 "
                "FAILURES2468 COVER_OUT RESPONSE_OUT MULTIPLICITY_OUT");
        ...
        std::array<std::array<u64, kTotalFailures592>, 2>
                ...
                for (std::size_t row = 0; row < 7; ++row) {
                    ...
                    for (std::size_t index = 0; index < kCounts592[row]; ++index) {
                        ...
                        if ((candidate.response[base_word + index / 64] >>
                             (index % 64)) & 1U) {
                            ++response_multiplicity[slot][global_index];
                            if (first_response[global_index] == 0) {
        ...
        std::ofstream multiplicity_out(argv[17]);
        require(cover_out && response_out && multiplicity_out,
                "cannot create response outputs");
        ...
        multiplicity_out << "q,r,body_hex,rank8_responders,rank9_responders,"
                            "total_responders\n";
        Fnv multiplicity_ledger;
        std::array<u64, 2> minimum_multiplicity{
            std::numeric_limits<u64>::max(),
            std::numeric_limits<u64>::max()};
        std::array<u64, 2> maximum_multiplicity{};
        u64 minimum_total = std::numeric_limits<u64>::max();
        u64 maximum_total = 0;
        std::size_t response_ordinal = 0;
        for (std::size_t row = 0; row < 7; ++row) {
            for (std::size_t index = 0; index < kCounts592[row]; ++index) {
                const std::size_t global_index = response_ordinal + index;
                const u64 rank8_count = response_multiplicity[0][global_index];
                const u64 rank9_count = response_multiplicity[1][global_index];
                const u64 total_count = rank8_count + rank9_count;
                require(total_count != 0, "zero-multiplicity obligation");
                minimum_multiplicity[0] =
                    std::min(minimum_multiplicity[0], rank8_count);
                minimum_multiplicity[1] =
                    std::min(minimum_multiplicity[1], rank9_count);
                maximum_multiplicity[0] =
                    std::max(maximum_multiplicity[0], rank8_count);
                maximum_multiplicity[1] =
                    std::max(maximum_multiplicity[1], rank9_count);
                minimum_total = std::min(minimum_total, total_count);
                maximum_total = std::max(maximum_total, total_count);
                multiplicity_ledger.add(kQ592[row]);
                multiplicity_ledger.add(592);
                multiplicity_ledger.add(failures.bodies[row][index]);
                multiplicity_ledger.add(rank8_count);
                multiplicity_ledger.add(rank9_count);
                multiplicity_out << kQ592[row] << ",592,"
                                 << hex8(failures.bodies[row][index]) << ','
                                 << rank8_count << ',' << rank9_count << ','
                                 << total_count << '\n';
            }
            response_ordinal += kCounts592[row];
        }
        require(response_ordinal == kTotalFailures592,
                "multiplicity ordinal partition changed");
        require(cover_out.good() && response_out.good() &&
                    multiplicity_out.good(),
                "output write failed");
        ...
        std::cout << "OBLIGATION_MULTIPLICITY_FNV " << std::hex
                  << multiplicity_ledger.state << std::dec
                  << " RANK8_MIN_MAX " << minimum_multiplicity[0] << ' '
                  << maximum_multiplicity[0] << " RANK9_MIN_MAX "
                  << minimum_multiplicity[1] << ' ' << maximum_multiplicity[1]
                  << " TOTAL_MIN_MAX " << minimum_total << ' '
                  << maximum_total << '\n';
        for (const Candidate592& candidate : cover) {
