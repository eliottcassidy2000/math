#!/usr/bin/env python3
"""Top twenty inherited clocks: complete minimum-tree grid consumer. Exact integer NumPy operations only."""
...
BASELINE_SHA='8ffc6d14b3883cf7e02c3ab02ddca5339d909a8051411def9096dee83b0aaed7'
GATES=0;INF=10**10
def need(ok,why):
    global GATES
    GATES+=1
...
def main():
    ...
    baseline=json.loads((ROOT/'05-knowledge/results/continuing8_20260906_lrc_minimum_tree_certificate.json').read_bytes())['new_scales']
    need(sha256(canonical(baseline)).hexdigest()==BASELINE_SHA,'inherited complete necessary scale array')
    need(len(baseline)==7646 and max(baseline)==11995 and 7200 in baseline,'declared inherited scope')
    baseline=[t for t in baseline if t!=7200]
    need(sha256(canonical(baseline)).hexdigest()=='f8c42793c4a5081d40a3937c9a79d9cd307e6c08c8b9ea3e94bac5388e47d16c','independently audited clock7200 deletion')
    upper=sorted(baseline,reverse=True)[:20]
    targets=upper;need(len(upper)==20 and upper[-1]==11940,'entire top twenty clocks')
    sub={}
    ...
    for t in targets:
        ...
        for e in sorted({gcd(*pair) for pair in weights}):
            ...
            for p,q,L,ints in AT:
                ...
                coarse[pair]=min(coarse[pair],max(0,sep))
        ...
        coarse_floor=min(coarse.values())
        excess_cap=max(d*((-t//d)%7) for d in D)
        cheap_margin=6*coarse_floor-excess_cap
        print('CHEAP',t,'EDGE_FLOOR',coarse_floor,'EXCESS_CAP',excess_cap,'MARGIN',cheap_margin,flush=True)
        S=words_for(D)
        ...
        record=dict(cheap_uniform_margin=cheap_margin,coarse_edge_floor=coarse_floor,uniform_excess_cap=excess_cap,t=t,domain=list(D),word_count=len(S),event_evaluations=events,compatible_atlas_edges=compatible,
    ...
    need(removed==upper,'whole declared top twenty closes')
    new=[t for t in baseline if t not in set(removed)]
    cert=dict(status='PROVED minimum-tree consumer; FINITE-EXACT selected complete domain',profile_sha256=PROFILE_SHA,baseline_semantic_sha256=BASELINE_SHA,baseline_after_7200_semantic_sha256='f8c42793c4a5081d40a3937c9a79d9cd307e6c08c8b9ea3e94bac5388e47d16c',
    ...
    print('DOMAIN_TOTAL',len(domains),'WORDS',sum(len(v) for v in domains.values()),'MIN_UPPER_MARGIN',min(r['minimum_margin'] for r in records))
