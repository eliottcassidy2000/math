// Independent closed-danger-union replay for the scale-two Hamming-six tree.
//
...
// THM-815 next-speed recursion on the 64 signed-doubling roots and their six
// labelled step-26 rays.  Candidate shortcuts are verified by directly
...
// full-safe-gap witnesses, early-cap witnesses, and terminal verdicts.
...
public:
  ...
  RootResult run() {
    ...
    header << "LRC-C2-H6-CLOSED-DANGER|v=1|delta=1/13|root=" << root_index_
           << "|R=" << missing_word() << '\n';
...
void check_diagnostic(const RootResult &result) {
  if (result.root_index == 0) {
    const array<unsigned long long, 7> nodes =
        {1, 70, 5004, 207720, 3611, 0, 0};
    const array<unsigned long long, 7> dead =
        {0, 0, 0, 206243, 3611, 0, 0};
    const array<unsigned long long, 7> full =
        {0, 0, 0, 182782, 0, 0, 0};
    const array<unsigned long long, 7> early =
        {0, 0, 0, 23461, 3611, 0, 0};
    if (result.nodes != nodes || result.dead != dead ||
        result.full_prunes != full || result.early_prunes != early ||
        result.candidate_edges != 216405 || result.covers != 0 ||
        result.loose != 0)
      throw runtime_error("c=2 root 0 diagnostic census mismatch");
  }
  if (result.root_index == 923) {
    const array<unsigned long long, 7> nodes =
        {1, 62, 3616, 125060, 1336, 6, 1};
    const array<unsigned long long, 7> dead =
        {0, 0, 0, 124544, 1331, 5, 0};
    const array<unsigned long long, 7> full =
        {0, 0, 0, 114769, 0, 0, 0};
    const array<unsigned long long, 7> early =
        {0, 0, 0, 9775, 1331, 5, 0};
    const string cover =
        "missing=7,8,9,10,11,12, ordered=7:1,8:3,9:5,10:7,11:9,12:11,";
    if (result.nodes != nodes || result.dead != dead ||
        result.full_prunes != full || result.early_prunes != early ||
        result.candidate_edges != 130081 || result.covers != 1 ||
        result.loose != 0 || result.cover_rows != vector<string>{cover})
      throw runtime_error("c=2 root 923 diagnostic census mismatch");
  }
}
...
void check_all_root_aggregate(
  ...
  if (nodes != expected_nodes || dead != expected_dead ||
      ...
      edges != 41882918 || covers != 1 || loose != 1 ||
      cover_rows != expected_cover_rows)
    throw runtime_error("all-root independent c=2 replay census mismatch");
}
...
int main(int argc, char **argv) {
  ...
  cout << "C2_H6_CLOSED_DANGER_UNION_REPLAY\n";
          ...
          for (int e = d + 1; e <= 11; ++e)
            for (int f = e + 1; f <= 12; ++f) {
              ...
              if (!c2_sheet_survivor(missing))
                continue;
              RootResult result =
                  RootReplay(this_root, missing).run();
  ...
  cout << "PASS: independent closed-danger-union c=2 H6 replay completed\n";
}
