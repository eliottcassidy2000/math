#!/usr/bin/env python3
"""
LRC(14) DISPROVE - FAST breadth (bounded). Skips the pathological HCN-divisor
sets (huge candidate-tau sets) that stalled the prior run; keeps the fast,
disprove-relevant families: random sweeps in several ranges, near-multiples-of-14,
geometric/mixed structured, and a CAPPED small-lcm probe (only sets whose lcm is
moderate, so candidate-tau count stays bounded).
Float screen + exact confirm of any sub-1/14 hit and of the float champion.
"""
...
random.seed(11)
print("(f) broad random several ranges",flush=True)
for hi in [25,30,40,60,100,150]:
    for _ in range(120000): consider(random.sample(range(1,hi+1),13))
...
for _ in range(250000): consider(random.sample(pool,13))
print(f"  bestf={bestf[0]:.6f} ce={len(ce)}",flush=True)
print("(c') multiples of small base + extra (small lcm, BOUNDED)",flush=True)
for bm in range(1,7):
    for extra in range(1,80): consider([bm*k for k in range(1,13)]+[extra])
# small-lcm: random 13-subsets of {1..24} (lcm bounded -> bounded cand set)
for _ in range(150000): consider(random.sample(range(1,25),13))
print(f"  bestf={bestf[0]:.6f} ce={len(ce)}",flush=True)
