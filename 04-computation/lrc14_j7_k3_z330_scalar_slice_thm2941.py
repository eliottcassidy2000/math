#!/usr/bin/env python3
"""Exact projected ``k=3`` scalar slice at first drift 330.
...
one body survives the scalar necessary condition.  The complete 2,891-row
crude ledger, not merely that survivor, is frozen by a semantic digest.
"""
...
EXPECTED_FIRST = 330
EXPECTED_CRUDE_ROWS = 2_891
EXPECTED_COMPLETE_RECORD_SHA256 = (
    "605ef66742a97dc334fad29de6d057e56242a75d4f48fc4f08557d82d0250e86"
)
EXPECTED_BODIES = ((1, 2, 6, 8, 12, 14),)
...
base = load_module("z330_global_scalar_scanner", BASE_PATH)
require(
    base.atlas.FIRST_MIN <= EXPECTED_FIRST <= base.atlas.FIRST_MAX,
    "z1=330 lies outside the canonical atlas",
)
...
def main():
    ...
    require(
        ...
        "this fixed-purpose transcript only permits --first 330",
    )
    crude_count, _records, survivors, digest = base.scan_fixed_first(
        args.first, args.processes
    )
    require(crude_count == EXPECTED_CRUDE_ROWS, "z1=330 crude count changed")
    require(
        tuple(record[0] for record in survivors) == EXPECTED_BODIES,
        "z1=330 body ledger changed",
    )
    require(digest == EXPECTED_COMPLETE_RECORD_SHA256, "z1=330 digest changed")
