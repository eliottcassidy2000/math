#include <filesystem>
// Exact carrier-via-joint-exposure scan for THM-4283.
//
...
int main(int argc, char** argv) {
    try {
        require(argc == 4, "usage: primary JOINT BASE ADDITIONS");
        init_choose8_local();
        const std::vector<u32> joint =
        ...
        require(carrier.size() == 8996 &&
                ...
                "inherited 8996-mask carrier changed");
        constexpr u32 endpoint644_repair = UINT32_C(0x014c9084);
        const std::unordered_set<u32> joint_set(joint.begin(), joint.end());
        const std::vector<Cell> cells = build_pool_cells();
        // The detached literal source proves all 63 consequence rows.  This
        // primary response source isolates the two decisive quotient stages:
        // the endpoint-644 repair before/after insertion and the first
        // resistant row under the repaired carrier.
        const std::array<Pair, 3> pairs{{
            {256,644}, {256,644}, {256,638}}};
        const std::array<const char*, 3> stages{{
            "PRIOR8996", "REPAIRED8997", "REPAIRED8997"}};
        FnvLocal ledger;
        u64 total_failures = 0;
        for (std::size_t pair_index = 0; pair_index < pairs.size();
             ++pair_index) {
            if (pair_index == 1) {
                require(std::popcount(endpoint644_repair) == 8 &&
                            carrier_set.insert(endpoint644_repair).second,
                        "endpoint-644 repair invalid/duplicate");
                carrier.push_back(endpoint644_repair);
                require(carrier.size() == 8997 &&
                            mask_fnv(carrier) ==
                                UINT64_C(0x8e1860a25d0fcf87),
                        "repaired carrier identity changed");
            }
            const Pair pair = pairs[pair_index];
            const PairAudit a =
                audit_pair(cells, joint, carrier, joint_set, pair);
            ledger.add(pair_index); ledger.add(pair.q); ledger.add(pair.r);
            ledger.add(a.active_carrier); ledger.add(a.active_carrier_fnv);
            ledger.add(a.active_joint); ledger.add(a.inactive_joint);
            ledger.add(a.inactive_joint_fnv); ledger.add(a.exposed);
            ledger.add(a.exposed_fnv); ledger.add(a.failures);
            ledger.add(a.failure_fnv); ledger.add(a.hit_incidences);
            ledger.add(a.minimum_hits); ledger.add(a.maximum_hits);
            ledger.add(a.response_fnv); ledger.add(a.complete_response_classes);
            ledger.add(a.complete_response_fnv);
            ledger.add(a.complete_active_response_fnv);
            ledger.add(a.full_response_masks); ledger.add(a.least_full_response);
            ledger.add(a.exact_minimum_replacements);
            for (u32 mask : a.exact_witness_masks) ledger.add(mask);
            total_failures += a.failures;
            std::cout << "STAGE " << stages[pair_index]
                      << " PAIR " << pair.q << ',' << pair.r
                      << " ACTIVE_CARRIER " << a.active_carrier
                      << " ACTIVE_CARRIER_FNV " << std::hex << a.active_carrier_fnv
                      << std::dec << " ACTIVE_JOINT " << a.active_joint
                      << " INACTIVE_JOINT " << a.inactive_joint
                      << " INACTIVE_FNV " << std::hex << a.inactive_joint_fnv
                      << std::dec << " EXPOSED " << a.exposed
                      << " EXPOSED_FNV " << std::hex << a.exposed_fnv
                      << std::dec << " FAILURES " << a.failures
                      << " FAILURE_FNV " << std::hex << a.failure_fnv
                      << std::dec << " HIT_INCIDENCES " << a.hit_incidences
                      << " HIT_RANGE " << a.minimum_hits << ".." << a.maximum_hits
                      << " RESPONSE_FNV " << std::hex << a.response_fnv
                      << std::dec << '\n';
            if (!a.failure_bodies.empty()) {
                std::cout << "FAILURE_BODIES";
                for (u32 body : a.failure_bodies)
                    std::cout << ' ' << std::hex << std::setw(8)
                              << std::setfill('0') << body;
                std::cout << std::dec << std::setfill(' ') << '\n';
                std::cout << "QUOTIENT CLASSES " << a.complete_response_classes
                          << " CLASS_FNV " << std::hex << a.complete_response_fnv
                          << " ACTIVE_RESPONSE_FNV " << a.complete_active_response_fnv
                          << std::dec << " FULL " << a.full_response_masks
                          << " LEAST_FULL " << std::hex << std::setw(8)
                          << std::setfill('0') << a.least_full_response
                          << std::dec << std::setfill(' ')
                          << " MINIMUM " << a.exact_minimum_replacements
                          << " WITNESS";
                for (u32 mask : a.exact_witness_masks)
                    std::cout << ' ' << std::hex << std::setw(8)
