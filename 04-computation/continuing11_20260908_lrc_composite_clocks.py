#!/usr/bin/env python3
"""Declared composite clocks: complete minimum-tree consumer, stopping at first residual. Exact integer NumPy operations only."""
...
BASELINE_SHA='51c00db52c18732ec6128e4d9e1ed197813ab107bfd3dec7dc7ace9c5c3c67c7'
GATES=0;INF=10**10
def need(ok,why):
    global GATES
    GATES+=1
...
def main():
    ...
    baseline=json.loads((ROOT/'05-knowledge/results/continuing10_20260908_lrc_upper_twenty_certificate.json').read_bytes())['new_scales']
    need(sha256(canonical(baseline)).hexdigest()==BASELINE_SHA,'inherited complete necessary scale array')
    need(len(baseline)==7625 and max(baseline)==11935,'declared inherited scope')
    upper=[10080,10800,11520]
    targets=[t for t in upper if t in baseline]
    need(targets==upper,'all three predeclared composite clocks survive in array')
    sub={}
    ...
    for t in targets:
        ...
        if len(bad):break
    need(len(records)<=3 and (len(records)==3 or records[-1]['survivor_count']>0),'stop only after first complete residual or all three clocks')
    ...
    print('COMPLETE',len(removed),'declared composite clocks excluded;',len(new),'necessary clocks remain; maximum',max(new))
