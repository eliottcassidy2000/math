/*
 * A052283: Triangle T(n,k) = number of directed graphs (digraphs) on n nodes with k arcs.
 *
 * Uses Burnside/Polya: T(n,k) = [x^k] (1/n!) sum_{sigma in S_n} GF_sigma(x)
 *
 * For cycle type lambda = [(r_a, m_a)]:
 *   GF_sigma(x) = prod of (1 + x^{orbit_size}) over all ORDERED pair orbits
 *
 * Orbit structure for DIRECTED edges (ordered pairs):
 *   Within single r-cycle: (r-1) orbits of size r
 *     (each ordered pair (i,j) with i!=j in the cycle has orbit size r)
 *   Within m copies of r-cycle: C(m,2)*2*r + m*r*(r-1)...
 *     Actually: m*m*r orbits total for ordered pairs across/within m copies
 *     But more precisely: for m copies, same-part cross = C(m,2)*2*r orbits of size r
 *   Cross (r_a, m_a) and (r_b, m_b): m_a*m_b*2*gcd(r_a,r_b) orbits of size lcm(r_a,r_b)
 *
 * From PARI: edges(v,t) = prod_{i<j} t(lcm)^{2*gcd} * prod_i t(v_i)^{v_i-1}
 *
 * Compile: gcc -O3 -I/opt/homebrew/include -L/opt/homebrew/lib \
 *          -o a052283_gmp a052283_gmp.c -lgmp
 *
 * Usage: ./a052283_gmp <max_n>
 *
...
static void compute_gf(int *pk, int *pm, int depth, mpz_t *poly, int max_deg) {
    ...
    for (int idx = 0; idx < depth; idx++) {
        ...
        /* Within-cycle terms for directed graphs: (1+x^r)^{m*(r-1)} */
        int within_exp = m * (r - 1);
        if (within_exp > 0)
            poly_mul_1_plus_xd(poly, max_deg, r, within_exp);

        /* Same-part cross-cycle terms: C(m,2)*2*r orbits of size r */
        int cross_same = m * (m - 1) * r;  /* = C(m,2)*2*r */
        if (cross_same > 0)
            poly_mul_1_plus_xd(poly, max_deg, r, cross_same);
    }

    /* Cross-part terms: 2*gcd orbits of size lcm per pair of cycles */
    for (int i = 0; i < depth; i++) {
        ...
        for (int j = i + 1; j < depth; j++) {
            ...
            int g = gcd(ri, rj);
            int l = ri / g * rj;  /* lcm */
            int exp = mi * mj * 2 * g;  /* 2*gcd for directed */
            if (exp > 0 && l <= max_deg)
...
int main(int argc, char **argv) {
    ...
    printf("A052283: Triangle T(n,k) = digraphs on n nodes with k arcs\n");
    printf("Computing rows 1..%d\n\n", max_n);

    /* Open b-file for output */
    FILE *bfile = fopen("b052283.txt", "w");
    int bfile_idx = 0;

    for (int n = 1; n <= max_n; n++) {
        global_n = n;
        global_max_edges = n * (n - 1);  /* directed: n*(n-1) arcs max */
        partition_count = 0;
