/* Exact bulk scan for disconnected high edges with gcd<=3 and 264<=p<=454.
...
static const char EXPECTED_TASK_SHA256[]="e967030c1566b49a580692c712d0ab1def306db52e5f598009a6198074893f36";
...
static void make_tasks(void){
 int cap=400000;tasks=malloc((size_t)cap*sizeof(*tasks));if(!tasks)die("malloc tasks");ntasks=0;
 for(int p=264;p<=454;p++)for(int q=p+1;q<8*p;q++)
  if(gcd64(p,q)<=3)tasks[ntasks++]=(Task){p,q};
 if(ntasks!=397502){fprintf(stderr,"task count %d\n",ntasks);exit(3);}
...
int main(int argc,char **argv){
 ...
 printf("universe 264<=p<=454;p<q<8p;gcd(p,q)<=3;all 2530 L<4592 contexts\n");
