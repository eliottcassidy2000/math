...
[[maybe_unused]] static uint64_t label_from_adj(const std::array<uint16_t,N>& out) {
  uint64_t z=0; int k=0;
  for(int i=0;i<N;i++) for(int j=i+1;j<N;j++,k++) if(out[i]&(1u<<j)) z|=1ull<<k;
  return z;
}
...
int main(int argc,char**argv){
  ...
  if(argc>1 && std::string(argv[1])=="--stdin") {
    std::string s; uint64_t count=0, strong_count=0, failures=0, coset_failures=0, gz=0,mz=0; bool have_max=false; std::vector<uint64_t> max_labels,min_labels; P global,minp; minp.coset_margin=std::numeric_limits<int64_t>::max();
    while(std::cin>>s){ if((int)s.size()!=E) { std::cerr<<"bad label length\n"; return 2; } ++count; uint64_t z=parse_bits(s); P p=eval(z); strong_count+=p.strong; if(!p.strong) continue; failures += 2*__int128(std::llabs(p.Chdx4)) >= __int128(p.D4x4); coset_failures+=p.coset_margin<=0; __int128 left=__int128(std::llabs(p.Chdx4))*global.D4x4,right=__int128(std::llabs(global.Chdx4))*p.D4x4; if(!have_max || left>right){global=p;gz=z;max_labels.assign(1,z);have_max=true;} else if(left==right){max_labels.push_back(z);} if(p.coset_margin<minp.coset_margin){minp=p;mz=z;min_labels.assign(1,z);} else if(p.coset_margin==minp.coset_margin){min_labels.push_back(z);} }
    std::cout<<"count="<<count<<" strong="<<strong_count<<" rational_failures="<<failures<<" coset_failures="<<coset_failures<<"\n"; show("MAX_RHO",gz,global); std::cout<<"MAX_RHO_EXACT numerator="<<2*std::llabs(global.Chdx4)<<" denominator="<<global.D4x4<<" max_ties="<<max_labels.size()<<" min_ties="<<min_labels.size()<<"\n"; for(uint64_t z:max_labels)std::cout<<"MAX_TIE label="<<bits(z)<<"\n"; show("MIN_COSET",mz,minp); for(uint64_t z:min_labels)std::cout<<"MIN_TIE label="<<bits(z)<<"\n"; return 0;
  }
