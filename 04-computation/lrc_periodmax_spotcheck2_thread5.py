#!/usr/bin/env python3
"""THREAD 5: EXACT period-max for skipped k=8 bases with period <= 70000 (feasible subset).
Tests whether 'huge-period => safe (margin covers it)' holds where it was never checked."""
import sys
...
def period_max(arcs,P):
    ...
    return mx,argw
cap=F(2243,5880); k=8
# the skipped bases with period<=70000 from the audit list
test_bases=[
 (0,1,2,10,11,12,13),(0,1,3,5,9,11,13),(0,4,5,8,9,10,13),
 (0,1,3,9,11,12,13),(0,3,4,5,8,9,13)]
print(f"k={k} cap={float(cap):.5f}: EXACT period-max for SKIPPED bases (P<=70000)")
print(f"{'B':<28} {'margin':>8} {'P':>8} {'period-max':>11} {'pm/margin':>10} {'argw':>7} verdict")
worst=F(0)
for B in test_bases:
    plat,arcs=plat_and_arcs(B); margin=cap-plat; P=period_of(B)
    pm,argw=period_max(arcs,P)
    ratio=pm/margin
    if ratio>worst: worst=ratio
    verdict="CLOSE" if pm<15*margin else "*** FAIL ***"
    print(f"{str(B):<28} {float(margin):>8.4f} {P:>8} {float(pm):>11.4f} {float(ratio):>10.3f} {argw:>7} {verdict}")
