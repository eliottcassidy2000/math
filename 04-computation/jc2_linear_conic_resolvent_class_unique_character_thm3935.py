...
linear_generators = sp.Matrix([
    [sp.diff(X, variable).subs({t: 0, X: 0, W: 0}) for variable in (t, X, W)],
    [sp.diff(a, variable).subs({t: 0, X: 0, W: 0}) for variable in (t, X, W)],
])
gate(linear_generators.rank() == 2,
     "D+ has two independent local generators at the singular origin")
...
summary = {
    ...
    "units": "k*",
    "cartier": "Pic(S)=0; Pic(Sreg)=Z/3; cover quasi-etale but not etale at origin",
...
print("PIC_AFFINE=0;PIC_SMOOTH=Z/3;CARDANO=QUASIETALE_NOT_ETALE_AT_ORIGIN")
