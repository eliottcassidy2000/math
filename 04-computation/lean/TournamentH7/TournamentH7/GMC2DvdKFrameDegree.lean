...
theorem xCoeff0_logDeriv_eq_zero (u : PowerSeries (LaurentSeries F)) (hu : IsUnit u)
  ...
  have hLu : logDeriv u * u = derivativeFun u := logDeriv_mul_self hu
  ...
  have key : ∀ k, (coeff (R := LaurentSeries F) k (logDeriv u)).support ⊆ Set.Iio 0 := by
    ...
    | _ k ih =>
      ...
      rw [PowerSeries.coeff_mul, coeff_derivativeFun] at hmul
...
theorem xCoeff0_logDeriv_eq_zero_of_monic (φ : PowerSeries (LaurentSeries F)) (hφ : IsUnit φ)
  ...
  have hlogxM : logDeriv xM = 0 := by
    rw [GMC2DvdKFrame.logDeriv, hxM, PowerSeries.derivativeFun_C, zero_mul]
  ...
  rw [hφeq, GMC2DvdKFrame.logDeriv_mul huunit hxMunit, hlogxM, add_zero]
  exact xCoeff0_logDeriv_eq_zero u huunit hu0 huneg

#print axioms xCoeff0_logDeriv_eq_zero
#print axioms xCoeff0_logDeriv_eq_zero_of_monic
