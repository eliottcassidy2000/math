...
/-!
# Assembly bridge: my swapped-mapped `Φ` is death-star's clean `Phi R M`

`GMC2PhiIrreducible.phi_irreducible_ratfunc` proves irreducibility of the swapped-and-mapped
polynomial; `GMC2PhiVieta.Phi R M = X^M − C(t)·R.map` is the clean form death-star's Vieta
...
/-- **The bridge.**  death-star's clean `Phi R M` equals my swapped-mapped polynomial. -/
...
transitivity input, now on the exact `Φ` death-star's Vieta uses. -/
...
#print axioms GMC2DvdKAssembly.Phi_eq_map_swap
#print axioms GMC2DvdKAssembly.irreducible_Phi
