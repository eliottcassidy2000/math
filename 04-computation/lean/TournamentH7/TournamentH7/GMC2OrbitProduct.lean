...
/-!
# The orbit-product identity: the gap-independent core of THM-2067 (general DvdK1)

THM-2067 (codex; the Galois orbit-product proof of one-variable Duistermaat–van der Kallen, the sole
...
THM-2067: with `v(∏_Ω) = 0` and `v(p) = 1` (`p = c·t`), `|Ω| = 0`, absurd.

This module proves the group-theoretic core, gap-free and kernel-pure — death-star-S106 §5 target 1.
The one genuinely valued-field piece (THM-1550, the small-root product, an unramified Hensel lift)
is separate.
-/
...
/-- **Orbit-product equation — the abstract core of THM-2067.**  If `A` carries a distributive
...
is `|Ω|·v(p) = |S|·v(C)` — the valuation identity that closes THM-2067 (`v(C)=0`, `v(p)=1` ⟹
`|Ω|=0`, absurd). -/
