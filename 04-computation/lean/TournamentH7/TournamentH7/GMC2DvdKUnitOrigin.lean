...
multiplicative THM-1550 crux (`Π = c·t`) modulo two hypotheses on the Weierstrass unit `h`:
...
/-- **The multiplicative THM-1550 crux, reduced to `hderiv` alone.**  With `hconst` now discharged
...
theorem smallRootFactor_coeff0_eq_of_derivative_vanishes' [CharZero F]
    ...
    (hderiv : PowerSeries.derivativeFun (GMC2DvdKMultiplicativeClosing.unitCoeff0 R M) = 0) :
...
#print axioms GMC2DvdKUnitOrigin.unitCoeff0_constantCoeff_eq_one
#print axioms GMC2DvdKUnitOrigin.smallRootFactor_coeff0_eq_of_derivative_vanishes'
