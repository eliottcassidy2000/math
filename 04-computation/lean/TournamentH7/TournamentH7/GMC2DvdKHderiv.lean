...
Building on `GMC2DvdKFrame`, this closes the DvdK log-derivative lemma `hderiv` (`d_t(h(0,t)) = 0`
...
* **`hderiv_of_frame`**: with `Φ = P·h` (units), `xCoeff0(logDeriv P) = 0` (the degree lemma (c)),
  ...
  vanishing, boxeph's `generatingFunction_eq_one`), the master identity forces `t·g' = 0`, hence
...
theorem derivativeFun_X_eq_one :
    PowerSeries.derivativeFun (PowerSeries.X : PowerSeries (LaurentSeries F)) = 1 := by
  ext n
...
theorem derivativeFun_PhiFrame (Rl : LaurentSeries F) (M : ℕ) :
    PowerSeries.derivativeFun (PhiFrame Rl M) = - PowerSeries.C Rl := by
...
`xCoeff0(logDeriv h) = g'/g` with `g := xCoeff0 h` a unit, and `F = 1`, we get `derivativeFun g = 0`
...
theorem hderiv_of_frame (Rl : LaurentSeries F) (M : ℕ)
    ...
    (hc : xCoeff0 (GMC2DvdKFrame.logDeriv Pfr) = 0)
    (ha : xCoeff0 (GMC2DvdKFrame.logDeriv hfr)
        = PowerSeries.derivativeFun (xCoeff0 hfr) * Ring.inverse (xCoeff0 hfr))
    (hg : IsUnit (xCoeff0 hfr))
    (hF1 : xCoeff0 (PowerSeries.C ((HahnSeries.single (1 : ℤ) (1 : F)) ^ M)
        * Ring.inverse (PhiFrame Rl M)) = 1) :
    PowerSeries.derivativeFun (xCoeff0 hfr) = 0 := by
  have hstar : xCoeff0 (GMC2DvdKFrame.logDeriv (PhiFrame Rl M))
      = PowerSeries.derivativeFun (xCoeff0 hfr) * Ring.inverse (xCoeff0 hfr) := by
    rw [hfact, xCoeff0_logDeriv_mul hPu hhu, hc, ha, zero_add]
  have hmaster := master_identity Rl M
  rw [hstar, hF1, sub_self] at hmaster
  have hg1 : PowerSeries.X * (PowerSeries.derivativeFun (xCoeff0 hfr)
...
#print axioms GMC2DvdKHderiv.master_identity
#print axioms GMC2DvdKHderiv.hderiv_of_frame
