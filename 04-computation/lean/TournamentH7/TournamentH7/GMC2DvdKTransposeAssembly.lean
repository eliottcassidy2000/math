...
death-star's transpose `GMC2DvdKTranspose.phi : F⟦t⟧⟦x⟧ →+* (F⸨x⸩)⟦t⟧` (the swap `τ` followed by
...
death-star flagged as "`phi_Phi` connector remaining".  This module provides it:
...
/-- **The frame identity** (death-star's flagged connector).  `φ` sends `Φ = xᴹ − t·R` to
`PhiFrame (Rl R) M`. -/
...
#print axioms GMC2DvdKTransposeAssembly.phi_Phi
#print axioms GMC2DvdKTransposeAssembly.hderiv_final
