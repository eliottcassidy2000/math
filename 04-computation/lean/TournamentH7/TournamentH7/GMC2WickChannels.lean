...
finite-support multinomial channels used in `THM-2022`.  For a multiplicity
...
/-- Rewrite the support-sum definition as the native `Finsupp.sum`. -/
theorem E_eq_finsupp_sum (P : MvPolynomial (Fin 2) ℂ) :
    E P = P.sum (fun s c => c * wt s) := by
...
theorem E_add (P Q : MvPolynomial (Fin 2) ℂ) : E (P + Q) = E P + E Q := by
  rw [E_eq_finsupp_sum, E_eq_finsupp_sum, E_eq_finsupp_sum]
...
name used by the concurrent `THM-2022` arithmetic engine. -/
...
`THM-2022` development.  Keeping the theorem here makes the modular library
...
/-- Equation (1) of `THM-2022` in its explicit balanced-factorial form.
