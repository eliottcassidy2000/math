...
/-!
# THM-1550 closed on the Weierstrass objects, modulo the single derivative identity (exp/log-free)

This wires the **char-0 back half** (`GMC2DvdKCharZeroClosing`) onto mac-mini's **algebraic front
half** (`GMC2DvdKWeierstrass`) to close the multiplicative DvdK crux (THM-1550) on the concrete
Weierstrass factorization `Φ = P·h`, *without* exp, log, Puiseux, or a Fredholm-determinant.

mac-mini's `coeff_zero_smallRootFactor_mul_unit` established
...
`eq_one_of_derivativeFun_eq_zero` turns `h(0,t) = 1` into the **exp/log-free** pair
...
log-derivative identity `d_t(h(0,t))/h(0,t) = −∑_{m≥1} D_m t^{m-1}` (root-free, mac-mini/death-star's
frame lane) — plus the normalization `hconst`, which is a direct consequence of the distinguished
...
/-- **The multiplicative DvdK crux (THM-1550), closed exp/log-free modulo the derivative identity.**
...
theorem smallRootFactor_coeff0_eq_of_derivative_vanishes
    ...
    (hderiv : PowerSeries.derivativeFun (unitCoeff0 R M) = 0) :
  ...
  GMC2DvdKCharZeroClosing.factorCoeff0_eq_of_unit_eq_one
    (GMC2DvdKCharZeroClosing.eq_one_of_derivativeFun_eq_zero hconst hderiv)
    (coeff_zero_smallRootFactor_mul_unit R M hM)

end GMC2DvdKMultiplicativeClosing

#print axioms GMC2DvdKMultiplicativeClosing.smallRootFactor_coeff0_eq_of_derivative_vanishes
