...
/-!
# The t-adic closing of THM-2067 over `F(t)`

The orbit-product equation (`GMC2OrbitProduct.prod_pow_card_group_eq`) instantiated at the Galois
group of `Φ(X) = X^M − t·R(X)` yields, in the base field `F(t) = RatFunc F`, an equation

  `Π ^ |G| = C_Φ ^ (|S|·|Stab|)`,

where (THM-1550) the small-root product is `Π = c·t` (`c ≠ 0`) and (Vieta) `C_Φ = (−1)^d r_0/r_d ∈ F`
is a nonzero *constant*.  This module proves that such an equation is impossible for `|G| ≥ 1`: a
monomial `c·tᴺ` (`N ≥ 1`) is never a constant in `F(t)`.  This is the concrete valuation-free closing
step — the `t`-adic contradiction realized by a degree comparison in `F[t]` after pulling back along
the injection `F[t] ↪ F(t)`.  No splitting-field valuation is needed; the only missing input is
THM-1550 (that `Π = c·t`), the death-star-S106 unramified-Hensel gap.
-/

open Polynomial

namespace GMC2RatFuncClosing

variable {F : Type*} [Field F]

/-- In `F(t)`, a monomial `a·tᴺ` with `a ≠ 0` and `N ≥ 1` is never equal to a constant `b`.  Pulling
back along the injective `F[t] ↪ F(t)` turns this into `C a * Xᴺ = C b` in `F[t]`, refuted by degree. -/
...
end GMC2RatFuncClosing

#print axioms GMC2RatFuncClosing.monomial_pow_ne_const
