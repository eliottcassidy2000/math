...
import TournamentH7.LRCDiscreteBonferroni  -- death-star-2026-07-09-S5 (HYP-5780, klein-S210 handoff): THM-671'S HISTOGRAM BOUND in Lean -- bandCount/liveCount/momentS/B5 decidable integer defs over multipliers p in (0,q); B5_le_liveCount (sum mac-mini-S101 BonferroniTruncation pointwise engine over multipliers + sum swap = the quintic moment form); exists_live_of_B5_pos; mreach_ge_of_B5_pos (B5>0 at ANY modulus => live p => t=p/q witness via kps-S114 pairsum dispatch => Mreach>=1/14 -- the decidable loneliness certificate, decide-shaped no analysis); DEMO: b5_ap_fourteen_pos (B5(AP{1..13},14)=5>0 by kernel decide, histogram {0:6,1:6,6:1}) + mreach_AP_via_histogram (the equality extremal certified end-to-end through the THM-671->THM-668 pipe); kernel-pure, NO native_decide
...
import TournamentH7.LRCDetunedDispatch  -- monad-S7: THM-668 FORMALIZED (detuned-harmonic dispatch, kernel-pure: quarter-window + Bezout branch + triangle pigeonhole; grand-assembly branch 6)
