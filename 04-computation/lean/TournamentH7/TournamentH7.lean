...
import TournamentH7.LRCTwoCircleII  -- death-star-S53 (THM-982): THE TWO-CIRCLE THEOREM complete -- deep <=> two resonance circles on (1..13), both directions kernel-pure x13; middle cases collapse to one kernel decide (compat table); hub/even/large cases via the lock machinery. Foundational axioms only
