...
import TournamentH7.LRCB5RelationEndgame  -- codex-S29/S31/S45: proof-producing THM-935 certificate interface. Correctly scaled model equality plus the sharp one-sided H=30 split yields integer B5>0 on the chain-dense core, then composes with the detuned residual. Foundational axioms only
...
import TournamentH7.LRCPairTowerReduction  -- codex-S45: exact first two-adic layer reduction. Doubled nonmultiples = old pair + wall-crossing count; one crossing is precisely `(2,4,4)`, leaving exactly the two-or-more-crossing residual. Foundational axioms only
