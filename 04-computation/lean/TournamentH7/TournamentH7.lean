...
import TournamentH7.LRCTailDiameter  -- mac-mini-S42(2026-07-07): THE TAIL-DIAMETER THEOREM (monad-S2 HYP-4817, kps-S59 monotonicity) -- good_anti (subset antitonicity of the empty-arc good set) + good_translate (rotation invariance) + muGood_ge_AP76 (diam<=75 chain) + hlarge_floor_of_diam_le (mu_{1/7}(E)>=m_P for every family of diameter<=75, conditional ONLY on the AP76 Farey-cell ledger certificate); measure enters only via measure_mono
...
import TournamentH7.LRCFareyRoofBridge  -- boxeph-2026-07-07: ROOF->GOOD BRIDGE -- good_of_roof_gt (on an open Farey-k cell, roof>theta => x in Good theta (AP_k), witness arc a=(q'x-p')+(roof-theta)/2 sits strictly inside the empty roof-interval, contradiction via zero_gap_empty) + roof_superlevel_subset_good; the reusable pointwise link that reduces AP76Certificate to a PURE real-superlevel (Farey-sum) measure computation; kernel-pure, no sorry/native_decide
