...
import TournamentH7.LRCTwoCircle  -- death-star-S52 (THM-979 Part I): the two-circle deep certificate for the canonical family (1..13) -- divisor descent, witnessed circle failures, circles => deep in full; recon-exact iff over 1185 moduli; Part II case analysis structured. Foundational axioms only
import TournamentH7.LRCTwoCircleConverse  -- codex-S66: begins THM-979 Part II in-kernel. Six canonical failures plus failure of speed one force the integer resonance circle; depth six also forces some failing speed <=8, eliminating the k0>=9 endpoint. Foundational axioms only
...
import TournamentH7.LRCPairOverlapArcs  -- boxeph-S74: arc-measure rendering of the pair overlap (LEM-042(A) Lean face): explicit Bezout components, exact 0-term, tent lower bounds, disjointness, assembled trapezoid credit <= volume(D_k cap D_{k+1} cap window). No sorry/native_decide
