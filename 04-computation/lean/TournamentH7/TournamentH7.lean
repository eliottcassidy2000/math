...
import TournamentH7.LRCRungLock  -- death-star-S46 (THM-960): the rung lock at the lonely-runner threshold -- exact witness locking for integer ratios <= 13 (sharp at 14, kernel-checked), ray propagation, all-integer Farey collapse. Foundational axioms only
...
import TournamentH7.LRCRationalLock  -- death-star-S48 (THM-963): rational-ratio lock -- reduced ratio i'+j' <= 13 locks witnesses on the Bezout ray; <= 3 branches through 27 (covers all 78 pairs of {1..13}); witness-mod bridge; exact pair count 2*floor((q-1)/(14*max)). Discrete face of boxeph LEM-044. Foundational axioms only
...
import TournamentH7.LRCB5WrappedToothOverlap  -- codex-S54: exact eight-case overlap formula for normalized circular teeth, plus a cyclic-to-linear comb permutation reducing strict-comb intersections to circular tooth sums. Foundational axioms only
...
import TournamentH7.LRCPairCovarianceReindexer  -- codex-S54: the exact circle covariance identity now follows from one named finite equality between merged-comb length and the gcd-scaled overlap ledger. Foundational axioms only
