...
import TournamentH7.LRCMod13Blocking  -- boxeph-S115/S116: mod-13 pair-blocking, a PROVED necessary condition for the n=12 tightness M(C)=1/13 (sharpens HYP-4382). sieve13_middle_witness: if every speed's residue (c_i*b)%13 is in the middle band [2,11] then t=b/13 puts all runners >=2/13 from Z (=> M>=2/13>1/13); contrapositive => M=1/13 forces {+-c_i mod 13}={1..12}. mod13_middle_far is the integer core (r in [2,11] => |13k+r|>=2). Standard axiom trio
import TournamentH7.LRCFinsetBridge  -- boxeph-S109, corrected codex-2026-07-18: exports both official Finset bridges. LRC14_finset_of_INVcov is the useful sufficient reduction; LRC14_finset_of_residual_INV closes the representation gap for the exact residual target. Enumerates W via equivFinOfCardEq and normalizes time by Int.fract. Standard axiom trio
import TournamentH7.LRCMSplit  -- boxeph-S108, corrected codex-2026-07-18: the covering M-split derives Covering(2..14) from no Lonely14 and no Lonely13 by band monotonicity. INVcov (covering + no Lonely13 => dominance) is a noncircular sufficient open target. ResidualINV (covering + no Lonely14 => dominance) is exact and, with LRC(≤13), equivalent to working LRC(14), not an identification with Tao n=12. The omitted-covering premise is false (MISTAKE-166). Standard axiom trio
