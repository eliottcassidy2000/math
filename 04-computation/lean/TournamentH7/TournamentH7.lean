...
import TournamentH7.LRCDetunedOverlap  -- codex-S34/S35: saturated three-detuned Zarankiewicz sharpening. Any pairwise bad-row intersection saves one branch and closes a weak total-degree budget; the tuple-specific clearing now feeds the harmonic reduction to an actual LRC witness. A uniform (3,3,3) failure is an exact pairwise-disjoint partition into three full g/3 rows. Foundational axioms only
