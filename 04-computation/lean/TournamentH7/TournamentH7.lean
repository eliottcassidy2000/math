...
import TournamentH7.LRCDilatedSieve  -- boxeph-S86 (THM-1013): the dilated-AP sieve. If every speed is >=d from n*d*Z then t=1/(nd) is 1/n-lonely (dilated generalization of sieve_frac). n=13 gives the WITNESS the compact LRC(14) minimizers d*{1..12}u{killer} were missing (they defeat every perturbation tool): M>=1/13>1/14. Standard axiom trio
import TournamentH7.LRCFinsetBridge  -- boxeph-S109 (connect the Lonely-form to the ledger's Finset target): LRC14_finset_of_INV -- LRC(<=13)[cited] + INV[open] => LRC14.LRC14 (the official Finset ℕ, t in [0,1] target of LRC14Ledger). Enumerate W via equivFinOfCardEq as v:Fin 13->Z; reduce t to [0,1) by Int.fract (loneliness is integer-shift invariant: lonely_fract). So the OFFICIAL LRC(14) target now reduces, kernel-checked, to LRC(<=13) + the single open INV. Standard axiom trio
import TournamentH7.LRCMSplit  -- boxeph-S108 (the M-split + the COMPLETE reduction of LRC(14) to INV): M<1/n <=> not exists t Lonely n, so the split 'M>=1/14 immediate vs M<1/13 crux' is Classical by_cases on (exists t Lonely 14) + band-shrink Lonely 13 => Lonely 14, NO sup needed. M_split: (not-exists-Lonely13 => exists-Lonely14) => exists-Lonely14. crux_of_dominance: INV(M<1/13=>rho>=13) + ap_core_bridge => the crux. LRC14_of_INV: forall v pos, (INV: not-exists-Lonely13 => rho>=13 dominance) => exists t Lonely 14 -- LRC(14) <= LRC(<=13)[cited] + INV[open], KERNEL-CHECKED. Non-covering absorbed (not-exists-Lonely13 entails covering). Standard axiom trio
...
import TournamentH7.LRCSieveDispatch  -- boxeph-S106 (the non-covering => sieve dispatch): Covering v := every n in {2..14} divides some speed. sieve_dispatch: not Covering => some n<=14 divides no speed => t=1/n is n-lonely (lonely_of_no_multiple) => 1/14-lonely (n<=14). lonely14_dispatch/lrc14_of_covering: LRC(14) reduces to the COVERING case alone; non-covering discharged here. With S105: LRC(14) <= LRC(<=13)[cited] + INV[open] + {sieve dispatch, descent}[Lean]. Standard axiom trio
import TournamentH7.LRCAPCoreBridge  -- boxeph-S105 (THM-1017(II), the elementary half of LRC(14)): ap_core_bridge -- rho=v_max/v_2nd>=13 (13*v_i<=v_max) + LRC(<=13) citation => Lonely 14 (via descent_dominant on the 12 non-max speeds, reindexed through vstar.succAbove). ap_core_bridge_of_shape: dilated-AP core (v_i<=12d) + far element >=156d=lcm(13,14) mechanism => Lonely 14. lonely14_of_INV: records LRC(14) <= LRC(<=13)[cited] + INV[open, dominance form] + {sieve,descent}[proved]. Standard axiom trio
