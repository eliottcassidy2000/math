...
import TournamentH7.LRCEndgameUniformThreePhase  -- exact uniform-(3,3,3) cyclic normal form: primitive modulus g=3; failure iff the three unit speeds permute the three branch classes; normalized sum-frequency clearance 3/14 is sufficient. No sorry/native_decide
...
import TournamentH7.LRCB5RelationBudget  -- codex-S28: exact algebraic consumer for THM-935's support-2/3/4/5 relation-mass identity. Absolute debt below 2052/16807 forces positivity; the proved H=30 pair tail spends strictly less than one quarter, leaving a checked three-quarter socket for T_s(H), s=3,4,5. Does not assume the analytic identity with concrete B5. Foundational axioms only
import TournamentH7.LRCB5RelationEndgame  -- codex-S29/S31: proof-producing THM-935 certificate interface. Exact model equality plus the quarter/three-quarter tail split yields integer B5>0 on the chain-dense core, then composes with the sharp detuned residual: nonterminating pair tower, q=2 with a distinct q<=8 companion, or uniform (3,3,3). Foundational axioms only
import TournamentH7.LRCDetunedOverlap  -- codex-S34: saturated three-detuned Zarankiewicz sharpening. Any pairwise bad-row intersection saves one branch and closes a weak total-degree budget; a uniform (3,3,3) failure is therefore an exact pairwise-disjoint partition of the branch circle into three full g/3 rows. Foundational axioms only
...
import TournamentH7.LRCRationalRegionProvider  -- codex: THM-933 rational-region analytic provider -- real half-open union, fract periodization, exact rational-length density, centered-primitive/eta-q duality, and unconditional rational-circle survivor/chart instantiations. No sorry/native_decide
