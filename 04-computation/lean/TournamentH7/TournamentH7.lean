...
import TournamentH7.LRCParallelClassZarankiewicz  -- codex-S46: exact three-row incidence ledger E <= |union| + sum pair-codegrees. At codegree zero z(3,g;2,1)=g, with equality iff the rows partition the carrier; all surviving detuned failures attain equality, so static incidence has no slack. Foundational axioms only
...
import TournamentH7.LRCB5NormalizedBridge  -- codex-S46/S47: THM-940 aggregate deviations Möbius-invert exactly to the THM-935 model. The pair socket is exactly the second factorial-moment floor 1703/1470; the harmful depth average must be < -65218/84035. Depths <=6 give credit and depths >=7 give debt. Foundational axioms only
import TournamentH7.LRCB5RelationEndgame  -- codex-S29/S31/S45/S46/S47: proof-producing dense-core consumer. The concrete certificate now chooses only a clean-modulus height and proves the explicit pair-depth floor plus harmful depth budget; modulus, coprimality, singleton sign, and THM-940 identification are automatic. Foundational axioms only
...
import TournamentH7.LRCSelectedWitnessCommon  -- codex-S45: common selected-witness infrastructure. Ten quotient citations supply a harmonic-good phase; selected witness is exactly LRC on a fixed three-row decomposition, and the unit-budget route reaches only <=6 residual speeds at gap 1/14. Foundational axioms only
