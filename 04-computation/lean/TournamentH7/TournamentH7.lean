...
import TournamentH7.LRCZeroColorGluing  -- codex-S58/S66: zero overlap determinant is equality of rational witness slopes, so every connected zero-color stalk glues to one primitive witness parameter. Exact resonance carries q/gcd(p,q) into its denominator; at coprime p,q the modulus divides every stalk speed once the primitive window fires. Foundational axioms only
