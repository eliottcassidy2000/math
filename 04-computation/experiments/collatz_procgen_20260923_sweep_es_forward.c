// collatz_procgen_20260923_sweep_es_forward.c  (HYP-9121)
// Partial-choice game E_S: the excursion x -> 3x+1 -> 9x+4 is allowed only at even x with x mod 2^J in S.
// Adapted from collatz_procgen_20260922_dim_forward.c (dimension lane); only the excursion rule changes.
...
static int A[400];
int cmpu(const void*a,const void*b){ uint64_t x=*(const uint64_t*)a,y=*(const uint64_t*)b; return x<y?-1:(x>y?1:0); }
static int JJ; static uint8_t ALLOW[1<<12];   // excursion allowed at even y iff ALLOW[y mod 2^JJ]
static inline int allowed(uint64_t y,int p){ return p>=JJ && ALLOW[y & ((1ULL<<JJ)-1)]; }
...
static int dfs(word y, int p, int a){
  ...
  for(;;){
    ...
    if(!allowed((uint64_t)y,p)) return 0;
...
static int dfs2(word y, int p, int a, int need){
  ...
  for(;;){
    ...
    if(!allowed((uint64_t)y,p)) return got;
...
int main(int argc,char**argv){
  if(argc<5){ fprintf(stderr,"usage: %s P0 MMAX J r1 [r2 ...]   (S = {r_i mod 2^J})\n",argv[0]); return 1; }
  P0=atoi(argv[1]); int MMAX=atoi(argv[2]); const char*dump=NULL;
  JJ=atoi(argv[3]); if(JJ<1||JJ>12){fprintf(stderr,"bad J\n");return 1;} memset(ALLOW,0,sizeof ALLOW);
  for(int i=4;i<argc;i++) ALLOW[atoi(argv[i])&((1<<JJ)-1)]=1;
  ...
  // optional dump of the final exceptional list (env ES_DUMP=file)
  if(getenv("ES_DUMP")){ FILE*g=fopen(getenv("ES_DUMP"),"w"); for(size_t i=0;i<nbad;i++){ print_word(g,bad[i]); fputc('\n',g);} fclose(g); }
  // projected counts P_m(MMAX) = |Bad_MMAX mod 2^m|  (upper bounds for the covering numbers of Bad_inf)
  {
    word *tmpw=malloc(sizeof(word)*(nbad+1));
    printf("# projected counts P_m(%d) = |Bad_%d mod 2^m|\n",MMAX,MMAX);
    for(int m=10;m<=MMAX;m++){
      word mk=MASKP(m);
      for(size_t i=0;i<nbad;i++) tmpw[i]=bad[i]&mk;
      // sort (simple radix on 64-bit low part)
      size_t n=nbad; 
      // insertion into hash-free counting: sort with qsort on uint64
      uint64_t *u=(uint64_t*)malloc(sizeof(uint64_t)*(n+1)); for(size_t i=0;i<n;i++) u[i]=(uint64_t)tmpw[i];
      int cmpu(const void*a,const void*b);
      qsort(u,n,sizeof(uint64_t),cmpu);
      size_t d=0; for(size_t i=0;i<n;i++) if(i==0||u[i]!=u[i-1]) d++;
      printf("P m=%d %zu\n",m,d); free(u);
    }
    free(tmpw);
  }
