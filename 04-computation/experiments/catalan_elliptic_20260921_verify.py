"""Bounded master replay for four Catalan/elliptic lanes and the Lean package.

Usage from any working directory:
  python /path/to/repo/04-computation/experiments/catalan_elliptic_20260921_verify.py
...
STEM = "catalan_elliptic_20260921"
...
LEAN = Path("04-computation/lean/CatalanEllipticAudit")
LANES = ("catalan", "statistics", "elliptic", "polynomial")
LEAN_INPUTS = (
    "lean-toolchain", "lakefile.toml", "lake-manifest.json", ".gitignore",
    "CatalanEllipticAudit.lean", "CatalanEllipticAudit/Certificates.lean", "AxiomAudit.lean",
    "verify.py", "README.md",
)
ALLOWED_AXIOMS = set()
...
def marker_present(value: dict, lane: str) -> bool:
    if lane == "catalan":
        return (str(value.get("status", "")).startswith("FINITE-EXACT")
                and value.get("universe", {}).get("ordered_words") == 250952
                and value.get("universe", {}).get("integer_words_each_parameter") == 37
                and value.get("universe", {}).get("distinct_cycles_each_parameter") == 4)
    if lane == "statistics":
        return (str(value.get("status", "")).startswith("FINITE-EXACT")
                and value.get("universe", {}).get("all_integers") == [1, 1000000]
                and bool(value.get("prefixes")) and bool(value.get("source_sha256")))
    return value.get("status") == "PASS" or bool(
        re.match(r"^PASS(?:\b|;)", str(value.get("checks", ""))))


def run_python(script: Path, optimized: bool, cwd: Path, record: dict, *, json_stdout: bool = False) -> str:
...
def check_embedded_source_hashes(value: dict, script: Path) -> list[str]:
    ...
    for key in ("source_sha256", "script_sha256"):
        if key in value:
            ...
            checked.append(key)
    if "source_sha256_lf" in value:
        require(value["source_sha256_lf"] == lf_hash(script), f"Stale LF source hash: {script}")
        checked.append("source_sha256_lf")
    return checked
...
def run_lane(lane: str, record: dict) -> list[Path]:
    ...
    for optimized in (False, True):
        ...
        stdout = run_python(script, optimized, Path("."), record, json_stdout=(lane == "catalan"))
        if lane == "catalan":
            # This lane deliberately emits fresh JSON to stdout rather than writing a file.
            fresh = json.loads(stdout)
            require(isinstance(fresh, dict), "Catalan output is not a JSON object")
            (ROOT / output).write_text(json.dumps(fresh, indent=2, sort_keys=True)+"\n",
                                      encoding="utf-8", newline="\n")
        value = decoded_json(output)
        require(marker_present(value, lane), f"Lane JSON lacks a PASS/check marker: {lane}")
    ...
    record["lanes"][lane] = {
        ...
        "embedded_source_hash_fields_checked": markers[0],
...
def run_lean(record: dict) -> list[Path]:
    ...
    require(report.get("public_root") == "CatalanEllipticAudit", "Unexpected Lean root")
    ...
    record["lean"] = {
        "status": "PASS", "root_import": "CatalanEllipticAudit", "external_packages": [],
...
def main() -> None:
    ...
    record = {
        ...
        "scope": "Finite exact replay of four Catalan/elliptic lanes and twelve concrete Lean witnesses.",
        "global_collatz_convergence_proved": False, "all_signed_cycles_classified": False,
        "hash_basis": "SHA-256 of UTF-8 file bytes after CRLF and lone CR are normalized to LF.",
        "embedded_source_hash_basis": "Raw source_sha256/script_sha256 and LF-normalized source_sha256_lf are checked according to their declared basis.",
    ...
    try:
        ...
        require((ROOT / SYNTHESIS).is_file(), "Missing synthesis")
