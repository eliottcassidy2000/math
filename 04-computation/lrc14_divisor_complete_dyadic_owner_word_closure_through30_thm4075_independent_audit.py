#!/usr/bin/env python3
"""Independent literal two-lift census for THM-4075."""
...
def circular_magnitude(value: int, modulus: int) -> int:
    residue = value % modulus
    return min(residue, modulus - residue)


def build_literal_tables(maximum: int) -> tuple[dict[int, list[int]], dict[int, list[tuple[int, int]]]]:
    pack_masks: dict[int, list[int]] = {}
    lift_danger_masks: dict[int, list[tuple[int, int]]] = {}
    for clock in CLOCKS:
        masks = [0] * (maximum + 1)
        for speed in range(1, maximum + 1):
            mask = 0
            for residue in range(clock):
                if 14 * circular_magnitude(speed * residue, clock) >= clock:
                    mask |= 1 << residue
            masks[speed] = mask
        pack_masks[clock] = masks

        tail_masks = []
        modulus = 2 * clock
        for tail in range(1, modulus, 2):
            label_zero = 0
            label_one = 0
            for residue in range(clock):
                if 14 * circular_magnitude(tail * residue, modulus) < modulus:
                    label_zero |= 1 << residue
                if 14 * circular_magnitude(tail * (residue + clock), modulus) < modulus:
                    label_one |= 1 << residue
            tail_masks.append((label_zero, label_one))
        lift_danger_masks[clock] = tail_masks
    return pack_masks, lift_danger_masks


def core_packet(core: tuple[int, ...], clock: int, pack_masks: dict[int, list[int]]) -> int:
    packet = (1 << clock) - 1
    for speed in core:
        packet &= pack_masks[clock][speed]
    return packet


def literal_lift_certificate(packet: int, tail_masks: list[tuple[int, int]]) -> bool:
    if packet == 0:
        return False
    for left_index, (left_zero, left_one) in enumerate(tail_masks):
        for right_zero, right_one in tail_masks[left_index:]:
            zero_covered = ((left_zero | right_zero) & packet) == packet
            one_covered = ((left_one | right_one) & packet) == packet
            if zero_covered and one_covered:
                return False
    return True
...
def census() -> tuple[list[dict[str, object]], list[tuple[int, ...]]]:
    pack_masks, lift_danger_masks = build_literal_tables(max(MAXIMA))
    ...
    for maximum in MAXIMA:
        ...
        for base in combinations(range(1, maximum), 10):
            ...
            first_clock = 0
            for clock in CLOCKS:
                packet = core_packet(core, clock, pack_masks)
                cached = caches[clock].get(packet)
                if cached is None:
                    cached = literal_lift_certificate(packet, lift_danger_masks[clock])
...
def main() -> None:
    ...
    print("LRC14 DIVISOR-COMPLETE CLOSURE THROUGH 30 -- INDEPENDENT LITERAL LIFTS")
