...
// signature-{20,50,399,401} fibre surgery.  This translation unit imports no
// project source.  It independently reads the frozen joint deck, exact
// 54-row fibre, and four-mask witness; rebuilds the deck; exhausts every
...
constexpr std::size_t kFibreCount = 54;
constexpr u64 kFibreFnv = UINT64_C(0x47ab2af18f07ff59);
constexpr std::size_t kWitnessCount = 4;
constexpr u64 kWitnessFnv = UINT64_C(0x2f59f31ad0512a75);
constexpr std::array<std::size_t, 4> kDeletedIndices{20, 50, 399, 401};
constexpr std::array<u32, 4> kDeletedMasks{
    UINT32_C(0x19801c80), UINT32_C(0x19104884),
    UINT32_C(0x3880b200), UINT32_C(0x118c0540)};
constexpr std::array<u32, 4> kExpectedWitness{
    UINT32_C(0x19801608), UINT32_C(0x11842450),
    UINT32_C(0x01180c85), UINT32_C(0x11803211)};
constexpr std::size_t kRebuiltCount = 421;
constexpr u64 kRebuiltFnv = UINT64_C(0xf63671c303bbafe4);
constexpr u64 kBodyCount = UINT64_C(14307150);
constexpr u64 kBodyChecks = UINT64_C(406361173);
constexpr u64 kBodyRowFnv = UINT64_C(0x7239db05659ba187);
constexpr i64 kFixedGrid = INT64_C(18241159416480);
constexpr u64 kEmptyFnv = UINT64_C(0xcbf29ce484222325);
constexpr u64 kTotalCells = UINT64_C(435652);
constexpr u64 kTotalFailureClasses = UINT64_C(127144);
constexpr u64 kActivityAggregateFnv = UINT64_C(0xe00f64d1ceceb6c8);
constexpr int kWeakestQ = 372;
constexpr int kWeakestR = 636;
constexpr u32 kWeakestMask = UINT32_C(0x18402a88);
constexpr i128 kWeakestMargin = static_cast<i128>(UINT64_C(10382682544542));
constexpr i64 kWeakestGrid = INT64_C(29970224921276640);
...
std::vector<Pair> read_fibre(const std::string& path) {
    ...
    require(pairs.size() == kFibreCount && ledger.state == kFibreFnv &&
                std::find(pairs.begin(), pairs.end(), Pair{372, 636}) !=
                    pairs.end(),
...
int main(int argc, char** argv) {
    try {
        ...
        require(argc == 4,
                "usage: detached-signature372 JOINT421 FIBRE54 WITNESS4");
        ...
        std::cout << "LRC14_SIGNATURE372_DETACHED_LITERAL_AUDIT_V1\n"
                  ...
                  << kWitnessFnv << std::dec << '\n'
                  << "DELETE_INDICES 20 50 399 401 DELETE_MASKS " << std::hex
                  << std::setw(8) << std::setfill('0') << kDeletedMasks[0]
                  << ' ' << std::setw(8) << kDeletedMasks[1] << ' '
                  << std::setw(8) << kDeletedMasks[2] << ' '
                  << std::setw(8) << kDeletedMasks[3] << " APPEND";
        for (u32 mask : witness)
        ...
        std::cout << "ACTIVITY PAIRS " << fibre.size() << " MASKS_PER_PAIR "
                  ...
                  << weakest_grid << '\n'
                  << "VERDICT PASS DETACHED_EXACT_54_ROW_COMMON_DECK_SURGERY\n"
                  << "SCOPE LITERAL_ACTIVITY_AND_EXHAUSTIVE_BODY_SCAN_"
                     "FIXED_POOL_NO_PHYSICAL_ENTRY_NO_LRC14\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "LRC14_SIGNATURE372_DETACHED_AUDIT_ERROR "
                  << error.what() << '\n';
        return 1;
    }
}
