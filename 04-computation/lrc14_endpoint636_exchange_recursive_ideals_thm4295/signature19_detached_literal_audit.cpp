...
// signature-{19} fibre surgery.  This translation unit imports no project
// source.  It independently reads the frozen joint deck, exact 36-row fibre,
// and two-mask witness; rebuilds the deck; exhausts every
...
constexpr std::size_t kFibreCount = 36;
constexpr u64 kFibreFnv = UINT64_C(0x5c8af37cf2f002e7);
constexpr std::size_t kWitnessCount = 2;
constexpr u64 kWitnessFnv = UINT64_C(0xcfc985695c621eca);
constexpr std::array<std::size_t, 1> kDeletedIndices{19};
constexpr std::array<u32, 1> kDeletedMasks{UINT32_C(0x1804aa01)};
constexpr std::array<u32, 2> kExpectedWitness{
    UINT32_C(0x0000e649), UINT32_C(0x0184a205)};
constexpr std::size_t kRebuiltCount = 422;
constexpr u64 kRebuiltFnv = UINT64_C(0xdc50478119bc6c12);
constexpr u64 kBodyCount = UINT64_C(14307150);
constexpr u64 kBodyChecks = UINT64_C(406270061);
constexpr u64 kBodyRowFnv = UINT64_C(0xef356b1d97405826);
constexpr i64 kFixedGrid = INT64_C(18241159416480);
constexpr u64 kEmptyFnv = UINT64_C(0xcbf29ce484222325);
constexpr u64 kTotalCells = UINT64_C(301512);
constexpr u64 kTotalFailureClasses = UINT64_C(87264);
constexpr u64 kActivityAggregateFnv = UINT64_C(0x1b0e43e7cc4c77d7);
constexpr int kWeakestQ = 6;
constexpr int kWeakestR = 511;
constexpr u32 kWeakestMask = UINT32_C(0x30960500);
constexpr i128 kWeakestMargin = static_cast<i128>(UINT64_C(1741559221584));
constexpr i64 kWeakestGrid = INT64_C(1331604637403040);
...
std::vector<Pair> read_fibre(const std::string& path) {
    ...
    require(pairs.size() == kFibreCount && ledger.state == kFibreFnv &&
                std::find(pairs.begin(), pairs.end(), Pair{338, 636}) !=
                    pairs.end(),
...
int main(int argc, char** argv) {
    try {
        ...
        require(argc == 4,
                "usage: detached-signature19 JOINT421 FIBRE36 WITNESS2");
        ...
        std::cout << "LRC14_SIGNATURE19_DETACHED_LITERAL_AUDIT_V1\n"
                  ...
                  << kWitnessFnv << std::dec << '\n'
                  << "DELETE_INDEX 19 DELETE_MASK " << std::hex
        ...
        std::cout << "ACTIVITY PAIRS " << fibre.size() << " MASKS_PER_PAIR "
                  ...
                  << weakest_grid << '\n'
                  << "VERDICT PASS DETACHED_EXACT_36_ROW_COMMON_DECK_SURGERY\n"
                  << "SCOPE LITERAL_ACTIVITY_AND_EXHAUSTIVE_BODY_SCAN_"
                     "FIXED_POOL_NO_PHYSICAL_ENTRY_NO_LRC14\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "LRC14_SIGNATURE19_DETACHED_AUDIT_ERROR "
                  << error.what() << '\n';
        return 1;
    }
}
