...
BASELINE_SHA='832714c179ab4f425a76172c41d7d8eecf05dc15adfdd3c5e08bc7eb5a13e48b'
GATES=0;INF=10**10
def need(ok,why):
    global GATES
    GATES+=1
...
def main():
    ...
    baseline=json.loads((ROOT/'05-knowledge/results/continuing11_20260908_lrc_composite_clocks_certificate.json').read_bytes())['new_scales']
    need(sha256(canonical(baseline)).hexdigest()==BASELINE_SHA,'inherited complete necessary scale array')
    need(len(baseline)==7622 and max(baseline)==11935,'declared inherited scope')
    upper=[6930,7560,9240,11088]
    targets=[t for t in upper if t in baseline]
    need(targets==upper,'all four predeclared composite clocks survive in array')
    sub={}
    ...
    for t in targets:
        ...
        if len(bad):break
    need(len(records)<=4 and (len(records)==4 or records[-1]['survivor_count']>0),'stop only after first complete residual or all four clocks')
