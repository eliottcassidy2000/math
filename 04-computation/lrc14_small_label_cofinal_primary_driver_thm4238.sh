...
binary=/tmp/lrc14-small-label-primary
jobs=${JOBS:-3}

"$cxx" -O3 -std=c++20 -Wno-unknown-pragmas \
...
xargs -P "$jobs" -n 1 sh -c '
  binary="$1"
  q="$2"
  "$binary" "$q" > "/tmp/lrc14-small-label-primary-q${q}.out"
' sh "$binary"
...
' /tmp/lrc14-small-label-primary-q*.out | sort -n \
  > /tmp/lrc14-small-label-primary-ledger.out
