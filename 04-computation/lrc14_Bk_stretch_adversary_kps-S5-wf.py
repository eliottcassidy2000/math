#!/usr/bin/env python3
"""
lrc14_Bk_stretch_adversary_kps-S5-wf.py  (kind-pasteur-2026-06-18-S5)

Lean follow-up: the STRETCH test (B) and LARGE-SPREAD ADVERSARY (C) for ALL k=4..13,
plus a trimmed cap-push for k=6,7 (the large-k stability checks that were the bottleneck).
This isolates the decisive 'can large spread beat the bounded-spread minimum?' question.
...
print("="*90)
print("(A') TRIMMED CAP-PUSH for k=6,7 (large-k stability): does mu_min stay flat as cap grows?")
print("="*90)
for k, caps in [(6,[8,12,16,20]),(7,[9,12,15,18])]:
    print(f"\n  k={k}:")
    prev = None; vals=[]
    for cap in caps:
    ...
    print(f"     -> {'STABLE (last 3 caps equal).' if stable else 'NOT flat over last 3 caps.'}")

print()
print("="*90)
print("(B) STRETCH TEST: same-k large-spread DILATIONS of the cap-14 minimizer never beat mu_min(k).")
print("    Also L1 scalar-multiply sanity (must be EQUAL).")
print("="*90)
for k in range(4, 14):
    ref, Estar = REF[k]
    eqfail = False
    for c in (2,3,5,7,11):
        if mu(tuple(c*e for e in Estar)) != ref: eqfail = True
    worst = (F(2), None); tried = 0
    for num,den in [(3,2),(2,1),(5,2),(3,1),(7,2),(4,1),(9,2),(5,1),(11,2),(6,1),(7,1),(8,1),(10,1),(13,1)]:
        lam = F(num,den)
    ...
    print(f"  k={k:2d}: mu_min={str(ref):>14s}={float(ref):.6f}  L1-eq={'OK' if not eqfail else 'FAIL!'}  "
          f"dilations={tried}  min-stretched-mu={float(worst[0]):.6f} "
          f"{'-> BEATS!!' if beat else '-> none below mu_min (OK)'}  at {worst[1]}")

print()
print("="*90)
print("(C) LARGE-SPREAD ADVERSARY: exact random search, spread in [3k,140]; can it beat mu_min(k)?")
print("="*90)
overall_beat = False
for k in range(4, 14):
    ref = REF[k][0]
    best = (F(2), None)
    ntr = 8000 if k <= 9 else 4000
    for _ in range(ntr):
        ...
        if m < best[0]: best = (m, E)
    beat = best[0] < ref
    overall_beat = overall_beat or beat
    print(f"  k={k:2d}: bounded mu_min={float(ref):.6f}  large-spread min found={float(best[0]):.6f}  "
          f"spread={max(best[1])}  -> {'BEATS mu_min!!' if beat else 'OK (>= mu_min)'}")
print(f"\n  ==> large-spread adversary {'FOUND a shape below the bounded min (reduction in DOUBT)' if overall_beat else 'NEVER beat the bounded-spread mu_min (reduction corroborated)'}.")
print("\nDONE.")
