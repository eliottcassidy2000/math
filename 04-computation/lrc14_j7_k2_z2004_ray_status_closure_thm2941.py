#!/usr/bin/env python3
"""Close the projected k=2 z1=2004 row for all later labels.

Body E=(1,4,8,10,12,14), first drift 2004, and four later drifts.  The
periodic/antipodal ray law gives exact all-label maxima in every denominator
multiset.  The exact quotient leaves twenty denominator states.  The crude
all-q screen rejects one; the all-arity common-table transport of THM-2928,
with five-needle Hunter caps, rejects the remaining nineteen by exactly
checked rational Farkas certificates.  No finite label horizon occurs.
The inherited projected suffix is one of the nineteen status kills.
"""
...
Z2060_ENGINE = (
    ROOT
    / "04-computation"
    / "lrc14_j7_k2_z2060_ray_status_closure_thm2941.py"
)
BAND_OUTPUT = (
    ROOT
...
EXPECTED_FRONTIER_ROW = (
    ...
    "lower=1049/267540;upper=6622358579/1633871214990;"
    "gap=1123710587/8496130317948"
)
BODY = (1, 4, 8, 10, 12, 14)
FIRST = 2004
K = 2
...
EXPECTED_FIXED_SCALAR = (
    (280, 588, 840, 980, 980),
    F(6622358579, 1633871214990),
    (2004, 2060, 2142, 2172, 2534),
)
EXPECTED_SCALAR_COUNT = 20
EXPECTED_SCALAR_SHA256 = (
    "073aa1a3c9644dba411d11f5c942923c85874e69fa7fff64ca361b7487dec5fa"
)
EXPECTED_CRUDE_KILL = (
    (196, 588, 980, 980, 2940),
    F(4089542274241, 1017068812986030),
...
require(
    file_sha256(Z2060_ENGINE) == EXPECTED_Z2060_ENGINE_SHA256,
    "z2060 K5/full32 referee dependency changed",
)
...
zspec = spec_from_file_location("k2_z2060_referee_control", Z2060_ENGINE)
require(zspec is not None and zspec.loader is not None, "cannot load z2060 referee")
...
def main():
    ...
    require(
        tuple(crude_kills) == (EXPECTED_CRUDE_KILL,)
        and len(crude_survivors) == EXPECTED_SCALAR_COUNT - 1,
    ...
    require(
        (
            ...
            control_capacities5[10],
            control_capacities5[26],
    ...
    lines = [
        "LRC14 projected k=2 z1=2004 all-label ray/status closure",
        f"uniform_ray_status_source_sha256={file_sha256(UNIFORM)}",
        f"z2060_referee_source_sha256={file_sha256(Z2060_ENGINE)}",
        f"scalar_band_output_sha256={file_sha256(BAND_OUTPUT)}",
        (
            "scope=inherited projected k=2 z1=2004 body row;"
        ...
        "conclusion=projected k=2 z1=2004 row is empty uniformly",
        f"semantic_sha256={semantic_hash}",
        "all_exact_controls=PASS",
    ]
    crude_ds, crude_upper, crude_labels, crude_witness = crude_kills[0]
    lines[10:10] = [
        (
            f"state[1]={crude_ds};upper={ftext(crude_upper)};"
