#!/usr/bin/env python3
"""Finite Rule 30 shell/history and explicit easy-hard-history controls."""
...
def shell_row(center: bytearray, scale: int):
    ...
    return scale, stop - start, running, minimum, minimum_at, maximum, maximum_at, amplitude
...
def main() -> None:
    ...
    controls = (
        (
            "alternating",
            2 * sum(alternating) - len(alternating),
            tuple(history_profile(alternating, h)[2:5] for h in (0, 1, 8, 32)),
        ),
        (
            "thue_morse",
            2 * sum(thue_morse) - len(thue_morse),
    ...
    print("RULE30_SHELL_HISTORY_BALANCE_THM4048")
    ...
    print(
        "scope=FINITE-EXACT through 2^18;universal shell/predictor identities "
        "proved in THM4048;no balance limit or complexity lower bound"
    )
    print("RESULT=PASS")
