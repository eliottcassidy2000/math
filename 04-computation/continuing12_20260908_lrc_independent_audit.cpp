...
int main(int argc,char**argv){
    ...
    for(auto&E:ex){
        ...
        if(bad.empty())excluded++;else hostiles++;minupper=min(minupper,worst);
        cerr<<"CLOCK "<<E.t<<" words="<<bank.size()<<" margin="<<worst<<" survivors="<<bad.size()<<"\n";
    }

    // Fresh complete native banks: no incumbent/profile shortcut in this scan.
    int native_compatible=0,native_zeros=0;set<int> firstdepth,seconddepth;
    auto vp=[](int x){int v=0;while(x%2==0){v++;x/=2;}return v;};
    for(int end:{4,18}){
        int middle=24,e=gcd(end,middle),n=7560/e,seen=0,positive=0;
        set<pair<int,int>> zero;
        for(auto&G:AT){
            int dp=e*gcd(n,G.p),dq=e*gcd(n,G.q);
            if(!((dp==end&&dq==middle)||(dp==middle&&dq==end)))continue;
            seen++;native_compatible++;
            auto cap=capacity(G,n);need(cap.first>=0,"complete native capacity nonnegative");
            if(cap.first){positive++;continue;}
            native_zeros++;
            int numerator=(dp==end?G.p:G.q),denominator=(dp==end?G.q:G.p);
            zero.insert({numerator,denominator});
            need(vp(end)<vp(7560),"native endpoint depth is actually unsaturated");
            int depth=vp(end)-vp(numerator)+vp(denominator);
            (end==4?firstdepth:seconddepth).insert(depth);
            need(gcd(7560,e*numerator)==end&&gcd(7560,e*denominator)==middle,"every native zero arm separately realizes its roles");
            set<I> walls={0};for(auto ab:G.intervals){walls.insert(mod(n*ab.first,G.L));walls.insert(mod(n*ab.second,G.L));}
            I best=INF;
            for(I wall:walls)for(I ph:{2*wall,2*wall+1}){
                I brute=literal(G,n,ph);need(brute==count_at(G,n,ph),"every zero-bank phase chamber and wall on literal native grid");best=min(best,brute);
            }
            need(best==cap.first&&best==0,"complete literal zero minimum");
        }
        if(end==4){need(seen==454&&positive==453,"complete 4-24 native bank");need(zero==set<pair<int,int>>{{307,48}},"entire 4-24 oriented zero set");}
        else{need(seen==227&&positive==223,"complete 18-24 native bank");need(zero==set<pair<int,int>>{{309,8},{303,16},{219,128},{201,152}},"entire 18-24 oriented zero set");}
    }
    need(native_compatible==681&&native_zeros==5,"entire native scope");
    need(firstdepth==set<int>{6}&&seconddepth==set<int>{4,5,8},"same physical middle requires incompatible exact dyadic depths");
    cout<<"NATIVE_BANKS 681 COMPLETE_RATIOS; ZERO_ARMS 5; MIDDLE_DEPTHS {6} VERSUS {4,5,8}\n";
    ...
    cout<<"UPPER_CLOCKS_EXCLUDED "<<excluded<<"; MINIMUM_MARGIN "<<minupper<<"; COMPLETE_FIRST_RESIDUAL_RETAINED\n";
    need(excluded==1&&hostiles==1,"one minimum-tree closure and one complete residual clock");
