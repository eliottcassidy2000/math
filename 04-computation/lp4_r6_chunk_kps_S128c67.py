#!/usr/bin/env python3
"""lp4_r6_chunk_kps_S128c67.py -- kind-pasteur S128 cont.67 (background).
...
def moms(ms,K=4):
    ...
    return out
import os
A=int(os.environ.get('C0','0')); B=int(os.environ.get('C1','792'))
C7=[sorted(c) for c in itertools.combinations(range(1,13),7)][A:B]
print("### S4 fixed-sextuple certificate: chunked sampled-sextuple audit ###")
pos3=[]; pos4=[]; worst3=None; worst4=None
for ci,P in enumerate(C7):
    global_ci=A+ci
    rng=random.Random((67<<16)+global_ci)
    M=max(P); lo=13*M+1
    ...
    if global_ci%100==0: print("  ... core %d/792  positive-sample(LP3)=%d positive-sample(LP4)=%d"%(global_ci,len(pos3),len(pos4)))
print()
...
path='05-knowledge/results/r6_lp4_positive_sample_cores_%d_%d_kps_S128c67.json'%(A,B)
json.dump([list(p) for p in pos4],open(path,'w'))
print("  positive-sample list written to "+path)
if pos4[:6]:
