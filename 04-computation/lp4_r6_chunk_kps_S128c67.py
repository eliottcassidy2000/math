...
for ci,P in enumerate(C7):
    ...
    if ci%100==0: print("  ... core %d/792  surv(LP3)=%d surv(LP4)=%d"%(ci,len(surv3),len(surv4)))
print()
...
json.dump([list(p) for p in surv4],open('05-knowledge/results/r6_surv_%d_%d.json'%(A,B),'w'))
print("  survivor list written to 05-knowledge/results/r6_survivors_%d_%d_kps_S128c67.json'%(A,B)+'")
if surv4[:6]:
