#!/usr/bin/env python3
"""orbit_decode_n9_burnside_kps_S128c18.py -- kind-pasteur S128 cont.18.
...
def slot(a, b):
    if b - a == 1:
        return None
    return tidx[(b + 1, a + 1)]
print("m=%d; sweeping %d perms..." % (m, 362880))
sols = Counter()
np_ = 0
...
print("gridsym-qf at n=9: %d (expect 0, odd n)" % gs)
...
for t in X:
    ...
    if iso(B1, BT):
        ...
        oNS += 1
print("n=9: orbits=%d = orbitsSC %d + orbitsNS %d" % (oSC + oNS, oSC, oNS))
print("SEQUENCES: orbitsSC = 2,0,9,3,%d ; orbitsNS = 0,3,13,98,%d" % (oSC, oNS))
NS9 = (191536 - 2752) // 2
print("CORRECTED NS rates: 3/22=%.3f (n=6), 13/184=%.3f (n=7), 98/3352=%.3f (n=8), %d/%d=%.4f (n=9)"
      % (3/22, 13/184, 98/3352, oNS, NS9, oNS / NS9))
print("DONE")
