/* Exact bulk scan for disconnected high edges with gcd<=3 and 455<=p<=678.
...
static const char EXPECTED_TASK_SHA256[]="3eea5e3450a6f188c9ac8574f57c8ef5f04e64595af148096df42cfe3bbd9561";
...
static void make_tasks(void){
 int cap=1400000;tasks=malloc((size_t)cap*sizeof(*tasks));if(!tasks)die("malloc tasks");ntasks=0;
 for(int p=455;p<=678;p++)for(int q=p+1;q<8*p;q++)
  if(gcd64(p,q)<=3)tasks[ntasks++]=(Task){p,q};
 if(ntasks!=734566){fprintf(stderr,"task count %d\n",ntasks);exit(3);}
...
int main(int argc,char **argv){
 ...
 printf("universe 455<=p<=678;p<q<8p;gcd(p,q)<=3;all 2530 L<4592 contexts\n");
