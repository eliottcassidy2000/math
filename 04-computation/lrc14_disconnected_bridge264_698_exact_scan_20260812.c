/* Exact bulk scan for disconnected high edges with gcd<=3 and 264<=p<=698.
...
static const char EXPECTED_TASK_SHA256[]="079e54a95dd55135dddcb9093bb98fba261939ce9cd9c9aeb7b38c8fe45866b7";
...
static void *worker(void *unused){(void)unused;
 for(;;){
  ...
  for(int ci=0;ci<ncontexts;ci++){
   Result x=mass_one(contexts[ci],t.p,t.q);x.ci=ci;
...
static void make_tasks(void){
 int cap=1300000;tasks=malloc((size_t)cap*sizeof(*tasks));if(!tasks)die("malloc tasks");ntasks=0;
 for(int p=264;p<=698;p++)for(int q=p+1;q<8*p;q++)
  if(gcd64(p,q)<=3)tasks[ntasks++]=(Task){p,q};
 if(ntasks!=1211966){fprintf(stderr,"task count %d\n",ntasks);exit(3);}
...
int main(int argc,char **argv){
 ...
 Result global={-1,1,-1};int failures=0;
 ...
 printf("global_weakest %d %d mass %lld/%lld context %d %d %d %d margin ",t.p,t.q,x.num/gg,x.den/gg,c.L,c.j,c.e,c.f);print128(stdout,mn/mg);putchar('/');print128(stdout,md/mg);putchar('\n');
 return failures?1:0;
}
