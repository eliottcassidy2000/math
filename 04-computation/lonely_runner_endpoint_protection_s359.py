...
Endpoint-protection graph for the reduced Lonely Runner Conjecture.
...
def endpoint_records(speeds: tuple[int, ...]) -> list[EndpointRecord]:
    ...
    endpoints = sorted({p % ONE for I in intervals for p in (I.lo, I.hi)})
    ...
    for t in endpoints:
        ...
        records.append(
            EndpointRecord(
                t=t,
                incident=len(touching),
                protection=len(protecting),
                is_witness=S356.is_lonely_witness(speeds, t),
                speeds_touching=tuple(sorted(set(touching))),
                speeds_protecting=tuple(sorted(set(protecting))),
            )
        )
    return records
...
def protection_report(raw_speeds: tuple[int, ...]) -> ProtectionReport:
    ...
    records = endpoint_records(speeds)
...
def main() -> None:
    ...
    print("  4. The next obstruction should be a protection-peeling lemma:")
    print("     full open covers would require an impossible all-protected core.")
