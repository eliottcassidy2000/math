/*
 * Compute A122082: Number of unlabeled bicolored graphs on 2n nodes
 * which are invariant when the two color classes are interchanged.
 *
 * a(n) = (1/n!) * sum_{lambda partition of n}
 *            permcount(lambda) * 2^{edges(lambda)}
 *
 * where edges(lambda) = sum_{i<j} gcd(c_i,c_j) + sum_i floor((c_i+1)/2)
 *   c_1,...,c_m are cycle lengths
 *
 * Also useful: A007139(n) = (A002724(n) + A122082(n)) / 2
 *   (bicolored bipartite graphs)
 *
 * OEIS: A122082, had ~50 terms
 *
 * Compile: gcc -O3 -I/opt/homebrew/include -L/opt/homebrew/lib \
 *          -o a122082_gmp a122082_gmp.c -lgmp
 *
 * Usage: ./a122082_gmp <max_n>
 *
...
static void process_partition(int *pk, int *pm, int depth) {
    ...
    /* edges = sum_{i<j} gcd(c_i,c_j) + sum_i floor((c_i+1)/2) */
    int e = 0;
    for (int i = 0; i < num_cycles; i++)
        for (int j = i + 1; j < num_cycles; j++)
            e += gcd_func(cycles[i], cycles[j]);
    for (int i = 0; i < num_cycles; i++)
        e += (cycles[i] + 1) / 2;
...
/* Enumerate ALL partitions of 'remaining' with parts <= max_part */
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "Computing A122082 (bicolored graphs invariant under color swap), n=0..%d\n\n", max_n);
