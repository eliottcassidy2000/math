// Finite-exact direct literal audit for every endpoint-588 body missed by
// the unchanged THM-4318 carrier.  The truncated mass uses only pair-safe
...
Failures read_failures(const std::filesystem::path& path) {
    ...
    while (std::getline(input, line)) {
        ...
        fields >> q >> r >> token;
        require(fields && !(fields >> extra) && r == kEndpoint,
...
int main(int argc, char** argv) {
    try {
        require(argc == 3, "usage: endpoint588_direct FAILURES DETAIL_CSV");
        ...
        for (const auto& [q, bodies] : failures.rows) {
            ...
            for (std::size_t ordinal = 0; ordinal < bodies.size(); ++ordinal) {
                ...
                detail << q << ',' << kEndpoint << ',' << ordinal << ','
                       ...
                       << '\n';
                ledger.add(q); ledger.add(kEndpoint); ledger.add(ordinal);
        ...
        Fnv summary_ledger;
        std::cout << "LRC14_ENDPOINT588_DIRECT_LITERAL_PRIMARY_V1\n"
        ...
        std::cerr << "ENDPOINT588_DIRECT_LITERAL_PRIMARY_ERROR "
                  << error.what() << '\n';
        return 1;
    }
}
