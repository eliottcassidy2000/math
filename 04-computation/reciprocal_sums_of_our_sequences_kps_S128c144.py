...
CORRECTION (MISTAKE-209/213): this is a historical indexed-term/multiset
prefix calculator, not the canonical support referee.  In particular the
odds-minus-{7,21} row is only the conjectural H-spectrum model; global
H-support divergence is open.  Use THM-2000/2005 for support semantics.
"""
import mpmath as mp
mp.mp.dps = 40
...
SEQS["conditional odds\\{7,21} model"] = ([x for x in (2*n-1 for n in range(1,4000)) if x not in (7,21)], "n>=1", "linear")
...
print("CORRECTION=MISTAKE-209/213;historical_term-multiset_prefixes;canonical_referee=THM-2000/2005")
print("H_SPECTRUM_ROW=CONDITIONAL_ODDS_MINUS_7_21_MODEL;GLOBAL_DIVERGENCE=OPEN")
print("="*84)
print("RECIPROCAL SUMS  sum(1/a_n)  — indexed term-multiset prefixes")
print("="*84)
...
KNOWN = {
 ...
 "conditional odds\\{7,21} model": ("DIVERGES as a model; actual H-support status OPEN"),
}
