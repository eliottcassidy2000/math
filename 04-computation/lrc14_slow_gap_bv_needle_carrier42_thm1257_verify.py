#!/usr/bin/env python3
"""Independent standard-library verifier for THM-1257's c=42 certificate.

This replay does not import the LP generator or its clipped-tooth integrator.
It recomputes every finite comb load from the cumulative periodic
danger-length function and recomputes the BV tail with exact rationals.
"""
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise RuntimeError(f"independent THM-1257 verification failed: {message}")
...
def main() -> None:
    ...
    require(
        set(keys) == expected and len(keys) == len(set(keys)),
    ...
    print("THM-1257 CARRIER-42 INDEPENDENT CUMULATIVE REPLAY")
    print(
    ...
    print("RESULT=no six-comb cover at carrier 42")
