#!/usr/bin/env python3
"""Exact W=0 hidden-E0 degree-12 attachment certificate for THM-4247.

This is a deliberately scoped follow-up to THM-4230/4241. On the normalized
W=0 curve

    C0: x^6 + y^4 = 1,       t=(1+y^2)/x^3,

it audits the degree-six hidden map f, its tau translate g=T f, the saturated
rank-two Eisenstein lattice L=O f + O g (O=Z[zeta_3]), and the degree-twelve
shell against the twelve attachment points. Saturation is inherited from
THM-4241; this program does not reprove it.
...
root of d_h(t) and t^deg(d_h) d_h(-1/t). The script exhausts the 24-map
degree-twelve shell, proves its two symmetry orbits, and checks that the
reciprocal gcd is exactly the excluded wall factor `t^2-1` in four good
...
def enumerate_degree_42() -> tuple[list[LatticeVector], list[LatticeVector], Counter[int]]:
    """Inherited THM-4230 hostile control; not called by THM-4247's main."""
...
def denominator_degree_budget() -> dict[str, int]:
    """Inherited degree-42 ledger; not called by THM-4247's main.
...
def finite_field_attachment_audit(
    ...
    for a_coefficient, b_coefficient in representatives:
        ...
        require(h_point is not None, "degree-12 vector became zero")
        ...
        # Good-reduction/pole-degree controls: d=t*D(t^2), deg D=1,
...
def rank_one_resultant_certificate() -> dict[str, int | str]:
    """Inherited norm-seven control; not called by THM-4247's main."""
...
def main() -> None:
    ...
    print("THM-4247 W=0 HIDDEN-E0 DEGREE-12 ATTACHMENT AUDIT")
