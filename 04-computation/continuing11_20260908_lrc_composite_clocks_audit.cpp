...
int main(int argc,char**argv){
    ...
    for(auto&E:ex){
        ...
        for(auto [ab,v]:E.weights){
            ...
            if(v.size()>1){int e=v[1],p=v[2],q=v[3];auto G=geometry(p,q);int n=E.t/e;
                ...
                if(true){auto cp=capacity(G,n);need(cp.first==literal(G,n,cp.second),"complete hostile/minimum-clock attainers on literal native grids");}
        ...
        need(bad.empty(),"selected upper clock closes");minupper=min(minupper,worst);excluded++;
    ...
    cout<<"UPPER_CLOCKS_EXCLUDED "<<excluded<<"; MINIMUM_MARGIN "<<minupper<<"; ALL_TARGETS_CLOSED\n";
    need(excluded==3&&hostiles==0,"declared complete selected scope");
