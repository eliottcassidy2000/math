...
therefore close whenever
    ...
    m > (13392/35)D + 65224/3185.

In particular ``m>=383D+21`` closes, so failure of this sufficient local
tree certificate forces ``m<=383D+20``.  This is again not a physical
survivor claim.
...
EXPECTED_SEMANTIC_SHA256 = "388806f23a9b52c7f0bcaeecf58595f460b6564af596548376d7b88d23dc4da0"
FINITE_MAX_Q = 5
...
def main() -> None:
    ...
    cone_slope = F(9, 2) / TENT_FLOOR
    cone_intercept = (F(9, 2) * label_tax + F(1, 39)) / (5 * TENT_FLOOR)
    require(cone_slope == F(13392, 35), ("tree cone slope changed", cone_slope))
    require(
        cone_intercept == F(65224, 3185),
        ("tree cone intercept changed", cone_intercept),
    )
    require(F(383) > cone_slope, "clean tree cone slope failed")
    require(
        F(383 + 21) > cone_slope + cone_intercept,
        "clean tree cone intercept failed",
    )
    require(F(383 + 20) <= cone_slope + cone_intercept, "clean intercept is not sharp at D=1")
    ...
    for diameter in (1, 2, 7, 64, 1000):
        minimum = 383 * diameter + 21
        require(
            5 * TENT_FLOOR
            - F(9, 2) * F(5 * diameter, minimum)
            - F(9, 2) * label_tax / minimum
            > F(1, 39 * minimum),
            ("clean double-star cone failed", diameter, minimum),
        )
    ...
    lines = [
        ...
        f"double_star_cone=m>(13392/35)D+65224/3185;clean=m>=383D+21;uncertified_wedge=m<=383D+20;partition_checks={partition_rows};digest={partition_digest.hexdigest()}",
