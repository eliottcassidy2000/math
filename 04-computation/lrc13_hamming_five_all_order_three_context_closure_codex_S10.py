#!/usr/bin/env python3
"""Exact all-context order-three Hamming-five closure for THM-844.

THM-823 leaves 96 common-sheet all-order-three contexts: three quartet
...
count and measure and leaves every relevant ratio unchanged.  No floating
point, sampled circle, heuristic cutoff, or bounded-height assumption occurs.
"""
...
EXPECTED_CERTIFICATE = "9bb065fe6ffafc348ada19c5f3ef30f0b28a5066bfd905a72107202ed13cdb2a"

RunWord = tuple[F, ...]
Context = tuple[tuple[int, ...], int, tuple[int, int, int]]
...
def global_kl_bound(runs: RunWord, length: F, remaining_count: int) -> int:
    """THM-837's total-residual K/L next-speed bound."""
...
def longest_component_bound(runs: RunWord, remaining_count: int) -> int:
    """THM-815/842's stronger one-component next-speed bound."""
...
def all_contexts() -> tuple[Context, ...]:
    ...
    for quartet in cosets:
        flags = sorted({(2 * r) % P for r in quartet})
        assert len(flags) == 4 and not (set(flags) & set(quartet))
        for flag in flags:
            for bits in product((1, 2), repeat=3):
                contexts.append((quartet, flag, bits))
    assert len(contexts) == 3 * 4 * 8 == 96
    return tuple(contexts)


def context_data(context: Context) -> tuple[tuple[int, ...], dict[int, int], tuple[int, ...]]:
    quartet, flag, bits = context
    ...
    bases = {label: crt_base(label, units[label]) for label in missing}
...
def recurse(
    *,
    ...
    long_bound = longest_component_bound(runs, len(remaining))
    global_bound = global_kl_bound(runs, length, len(remaining))
...
def comparison_edge(
    ...
    """Orient larger values first and use the declared tie path."""
...
def tournament_fingerprint(edge: Sequence[Sequence[bool]]) -> tuple[object, ...]:
    ...
    while unseen:
        ...
        unseen -= block
    # Both telemetry gauges are scalar total orders, so a transitive result
    # must have exactly one Hamiltonian path.
...
def context_tournament(
    ...
    """Compare total-mass and longest-component marginal erosion."""
    ...
    return (
        tournament_fingerprint(raw),
        tournament_fingerprint(switched),
        flips,
        tuple(total_values),
        tuple(longest_values),
    )
...
def main() -> None:
    ...
    assert not any(row[14] <= 0 for row in context_rows)
    ...
    lines = [
        "THM-844 ALL ORDER-THREE HAMMING-FIVE CONTEXT CLOSURE",
        ...
        "contexts=3_quartet_cosets*4_forward_flags*4_quartet_pair_words*2_fifth_units=96",
        "carrier=active_open_endpoint_runs+remaining_labelled_CRT_progressions+last_speed",
        ...
        "THM837.context=(C=(1,5,8,12),b=10,bits=(1,1,1))",
    ]

    thm837_row = next(
        row
        for row in context_rows
        if row[1] == (1, 5, 8, 12) and row[2] == 10 and row[3] == (1, 1, 1)
    )
    lines.extend(
        (
            f"THM837.root_global_bound={thm837_row[10]}",
            f"THM837.root_longest_bound={thm837_row[11]}",
            f"THM837.longest_replay_states={thm837_row[14]}",
            "THM837.published_global_replay_states=75371 published_depth5_terminals=57",
            "CONTEXT_ROWS",
        )
    )
    for row in context_rows:
        lines.append(
            "context[{:02d}] C={} b={} bits={} bases={} root=(K={},L={},Lmax={},global={},long={}) "
    ...
    lines.extend(
        (
            "TOURNAMENT_ANALYSIS",
            "vertices=five_labelled_least_CRT_comb_obligations",
            "pair_observable=root_total_measure_marginal_erosion",
            "switch=root_longest_component_marginal_erosion",
            "tie_hamiltonian_path=increasing_(least_CRT_speed,label)",
            ...
            "assumption_challenge=comb_vertices_are_not_the_state;the_bipartite_component_obligation_carrier_is_faithful",
            "scope=all_THM823_all_order3_contexts_not_mixed_D1_D3_not_other_orders_not_global_H5",
            f"certificate_sha256={certificate}",
            f"source_sha256={sha256(Path(__file__).read_bytes()).hexdigest()}",
            "PASS: all 96 arbitrary-height all-order-three contexts are strictly loose",
        )
    )
    payload = "\n".join(lines) + "\n"
    output_hash = sha256(payload.encode()).hexdigest()
    print(payload, end="")
