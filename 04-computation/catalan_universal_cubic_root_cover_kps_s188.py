...
def main() -> None:
    ...
    require(sp.expand(jac_vr + sp.Rational(2, 3) * r) == 0,
            "polynomialized Jacobian")
    ...
    require(jac_vw == kappa, "Catalan chain-rule cancellation")
    ...
    require(source_change_jac == 2, "source affine change")
    ...
    require(target_change_jac == -6, "target affine change")
    require(sp.expand(p - (2 - 3 * P)) == 0, "target p identity")
    require(sp.expand(q + t**3 + p * t) == 0, "universal root equation")
    ...
    require(universal_jac == pp + 3 * tt**2,
            "universal-cover Jacobian")
    require(sp.expand(universal_jac.subs({tt: t, pp: p}) - 2 * r) == 0,
            "ramification coordinate pullback")

    X = sp.symbols("X")
    cubic = X**3 + pp * X + sp.symbols("qq")
    cubic_disc = sp.factor(sp.discriminant(cubic, X))
    require(cubic_disc == -4 * pp**3 - 27 * sp.symbols("qq")**2,
            "cubic discriminant")
    pulled_disc = sp.factor(cubic_disc.subs({pp: p, sp.symbols("qq"): q}))
    require(sp.expand(pulled_disc + 4 * r**2 * (8 * r - 9 * v**2)) == 0,
            "discriminant pullback")

    target_cusp = sp.factor((3 * P - 2)**3 - 27 * Q**2)
    require(sp.expand(4 * target_cusp - pulled_disc) == 0,
            "target cusp normalization")
    branch_P = sp.factor(P.subs(r, 0))
    branch_Q = sp.factor(Q.subs(r, 0))
    require(sp.expand(branch_P - (v**2 + sp.Rational(2, 3))) == 0,
            "branch P parametrization")
    require(branch_Q == v**3, "branch Q parametrization")
    require(sp.expand((3 * branch_P - 2)**3 - 27 * branch_Q**2) == 0,
            "branch maps to cusp")
    ...
    triple_jacs = [sp.factor(jac_vr.subs({v: vv, r: rr}))
                   for vv, rr in triple]
    require(triple_images == [(1, 0), (1, 0), (1, 0)],
            "three-point collision")
    require(triple_jacs == [sp.Rational(1, 3),
                            -sp.Rational(2, 3),
                            -sp.Rational(2, 3)],
            "simple collision Jacobians")
    ...
    require(sp.expand(corrected_jac_on_R0 + 6 * T * (T * a0 + b0)) == 0,
            "fixed-ramification-line first jet")
    require(corrected_jac_on_R0.subs(T, 0) == 0,
            "cusp-point Jacobian obstruction")
