...
**Status:** CURRENT SNAPSHOT / NOT A PROOF SOURCE.  Follow the linked theorem
files and their literal statuses.  Corrected and promoted canon overrides this
synthesis.

## Executive answer

The current beach search produced a coordinated set of frontier advances and
structural tools, with later exact audits promoted directly into the map below.

1. For **LRC(14)**, THM-3171 turns the earlier reflected two-star scout into an
   all-width theorem for the fixed body `H=(1,2,3,4,6,12)`, cell `90`, and the
   inherited cap-two sufficient family.  Every ordinary high label-edge lane
   has overlap at least `1/105`; the sole weak `{1,6}`, primitive `3:5` lane
   has sharp floor `2030/280393`, but its occurrence forces three additional
   regular high edges.  The uniform nine-edge law therefore closes every
   `D>=6`, including every gcd cone.  This is not a physical-survivor
   ...
   formula is a Bernoulli-quadratic boundary potential: the signed `1/g`
   correction is its exact coboundary.  It telescopes to zero on every complete
   reflected-cell orbit, whose total bulk is `T/49`.  THM-3224 now evaluates
   the complete orbit exactly at every dilation: the first surviving term is
   a nonzero, residue-periodic gcd-carry coefficient at order `g^-2`, its
   normalized magnitude has sharp bound `9/49`, and the remainder is exactly
   `O(g^-3)`.  Cellwise, that curvature splits canonically into its orbit
   average plus a cyclic potential difference.  The potential retains the
   cell owner and residue class; finite heads still need their sidecars.
   Independently, THM-3218 now lowers the projected `k=3` cap to `219`.
2. For the **planar Jacobian response lane**, THM-3151 excludes every balanced
   ...
   polynomial `A2` source, and proves that no bounded marked jet decides the
   first gate.  THM-3172 supplies a different, shear-invariant differential-
   ...
   centered source has five forced Morse critical points.  More strongly, the
   entire constant family `(Vz^2+bz+c)^2+A_src z+kx` is obstructed.  Its finite
   clutch walls are explicit; after they pass, the sole `S`-escape wall moves
   at most three resultant units, leaving at least `49` (`k!=0`) or `25`
   (`k=0`) critical multiplicity away from `ST`.  Generic counts are exactly
   `52` and `28` Morse points.  THM-3225 extends the obstruction from constant
   accessories to every affine `B=b_0+b_1x` in
   `(Vz^2+Bz)^2+A_src z+x`: the finite clutch is exact, no `T`-escape is
   possible, the sole `S`-boundary has multiplicity at most two, and at least
   `50` residual critical intersection multiplicity remains.  A generic
   affine accessory has exactly `52` squarefree Morse points.  None of these
   families supplies the marked inverse pair required by the owner
   filtration, a mate, or `JC(2)`.
3. For **quadratic factorial windows**, the prime/prime-power exits at offsets
   ...
   seven-exit rows in `2201<=d<=2400`, and arithmetic exits close the next
   five values. Independent recurrence/Fraction-hull and determinant-hull
   engines reproduce the full multi-prime trace. Thus the exact-support
   `{0,1,2}` window cannot vanish for `1<=r<=2498`.  A dual exact extension
   closes all `35` seven-exit residuals in `2401<=d<=2500`; the former
   boundary `r=2404,d=2406` dies at prime `7`.  The first unaudited residual
   is now exactly `r=2499,d=2501`, not a survivor.
   THM-3182/3183 independently lift the moment recurrence to two commuting
   integral frames: the prime reset has two transverse Smith bands and an
   oriented graph-distance-two continuant.  A same-Smith hostile proves that
   these invariants do not yet imply the empirical Euclidean-depth staircase.
   THM-3186 gives the whole signed exit-time convolution, proves that a
   ...
   even though its whole sequence has a closed recurrence.  Full-depth,
   growing-offset, and exceptional-prime alternate-chart selection remain
   open.  THM-3215 now identifies the
   local carrier behind the exterior reflections in every polynomial degree:
   a trace-zero two-jet Hamiltonian, affine-dihedral alternating shear, flat
   `dlog` root-chart cocycle, and primitive projective `p`-return with a
   nonzero first carry.  THM-3220 identifies the next exact layer: tangent
   four-jets are the Heisenberg central extension of the abelian three-jet
   quotient, with Schwarzian second coordinate and covariant fourth-jet
   central filler.  On quadratic factorial germs the commutator is an
   oriented cubic discriminant with the exact quadratic Frobenius character.
   THM-3221 identifies every first unequal derivative-normalized coefficient
   as a coordinate-covariant osculating tensor.  After reduction with nonzero
   coefficient in residue characteristic `p`, its minimal quotient has exact
   order `p`; over a mixed-characteristic DVR with unit first coefficient,
   the divided carry returns that coefficient.  The
   selected root, derivative anchor, and normalized tower through a supplied
   degree `D` reconstruct a polynomial of degree at most `D`, but no fixed
   ...
   finite exception integer `Xi_s` for each fixed offset, but no arbitrary-
   ...
   vanishes identically.  THM-3227 is now independently hostile-audited proved
   canon: selected-root residue contacts form a labelled `q`-ary prefix trie
   with exact-order-`p`
...
4. For **tournaments and exact sequences**, THM-3181 proves pure parity
   ...
   `Q_T(1),...,Q_T(ceil(n/2))` recover the complete path-cover profile.  For
   repeated order joins, each fixed extreme coefficient has an explicit
   polynomial--exponential formula and a minimal constant recurrence of order
   `k(k+1)/2`.  THM-3202 supplies the complementary cyclic boundary: a
   quadratic-time moving-jet evaluator is exact, but every fixed positive
   coordinate has a factorial-cube floor and is not C-finite.  THM-3213
   accelerates the complete factor jet to one polynomial product, gives an
   exact trivariate diagonal for transitive cyclic lifts, and proves every
   twice-normalized fixed coordinate is still non-C-finite.  A prime shift
   now closes the singly normalized sequence at every fixed depth and, more
   generally, produces `p^q` denominators for every fixed `q`-block quotient.
   A transcendental-radius obstruction now proves that every fixed-depth C3
   normalization is non-P-recursive.  The same conclusion holds for every
   strongly connected Perron-balanced quotient on at least three blocks,
   hence every nontrivial regular tournament quotient.  THM-3226 solves the
   analytic half of the unbalanced problem: every fixed strongly connected
   tournament quotient on at least three vertices has a unique positive saddle, strict complex
   minimality, and a nondegenerate coefficient asymptotic.  For the smallest
   strong unbalanced `Q4`, its parameter `p` and scale ratio `q=s/t` are
   transcendental.  Transcendence of its product radius would prove the
   normalized sequence non-P-recursive; both unconditional conclusions remain
   open, while Schanuel implies them.  SCC order, run data, and the arithmetic
   of general unbalanced radii remain open.
...
with exact failure mechanisms and cheap next tests.

## Continuation synthesis: four operation laws

The productive reframe was the same in four apparently unrelated settings:
retain the whole operation until its boundary term is visible, and scalarize
only afterward.

| lane | operation retained | new exact law | information still lost |
|---|---|---|---|
| LRC channel | complete reflected phase orbit plus gcd carry | the `1/g` boundary coboundary cancels; the exact nonzero `g^-2` harmonic curvature is residue-periodic, has sharp normalized bound `9/49`, and the cellwise remainder is a canonical potential difference | the harmonic scalar forgets the owner potential and physical finite head |
| planar JC | full constant and affine-`B` parameter families plus boundary saturation | finite clutch walls and the `S,T` valuations exhaust boundary escape channels; no affine clutch can remove all critical points | a source obstruction supplies neither a mate nor global chart entry; nonlinear accessories remain |
| factorial | complete seven-exit degree profile in two independent engines | all `35` residuals through `d=2500` have matching empty final intersections | finite exact support does not cover translated supports or `FC(3)` |
| tournament sequences | quotient resolvent and its variational saddle | every fixed strong tournament quotient on `q>=3` has one positive nondegenerate saddle; balance forces a transcendental radius, while unbalanced `Q4` has transcendental `p` and ratio `s/t` | the unbalanced product radius can remain arithmetically unclassified even after the analytic saddle is solved |

This is a reusable research rule, not a theorem transfer between conjectures:
boundary cancellation, saturation, multi-place intersection, and saddle
selection are different maps, but each fails when its intermediate data are
collapsed to a single local count too early.

The sharper common pattern is a **controlled-forgetting hierarchy**.  In LRC,
the first coboundary disappears but its second harmonic curvature and owner
potential survive.  In the root-jet lane, the abelian three-jet projection
forgets a half-symplectic area that the central fourth jet restores.  In the
tournament lane, solving the variational geometry leaves the arithmetic of one
product radius.  In the Jacobian lane, deforming the accessory changes a
boundary wall but not the unique top bulk degree.  These are not literal
theorem transfers: they say which sidecar must accompany a quotient before a
downstream obstruction can be trusted.

There is also one exact cardinality hostile across the LRC/root-jet boundary.
The largest reflected owner orbit has `168` cells.  THM-3228 says every
permutation action of the full mod-`13` four-jet group on fewer than
...
| LRC(14), projected `k=3` | [THM-3218](../01-canon/theorems/THM-3218-projected-k3-z220-valuation-product-terminal-descent-and-cap219.md) | projected layers close through `z1=220`; cap `219`, ledger `373427` | next `16` rows at `z1=219`; arbitrary `k<=1`, the rung, and LRC(14) remain open |
| LRC(14), reflected levels | [THM-3135](../01-canon/theorems/THM-3135-directed-cycle-weak-order-lane-cover-and-reflected-h-boundary.md), [THM-3171](../01-canon/theorems/THM-3171-global-high-channel-cell90-floor-and-all-width-uniform-two-star-law.md), [THM-3200](../01-canon/theorems/THM-3200-fixed-lrc-channel-cleared-overlap-quasipolynomial-and-mass-recurrence-boundary.md), [THM-3211](../01-canon/theorems/THM-3211-uniform-lrc-channel-limit-bernoulli-cubic-and-sharp-floor.md), [THM-3224](../01-canon/theorems/THM-3224-complete-lrc-orbit-bernoulli-gcd-carry-and-owner-hodge-splitting.md) | exact cycle/DAG boundary; all-width cell-90 theorem; sharp `1/105` bulk; exact first coboundary; nonzero periodic second curvature with sharp `9/49` bound and canonical owner Hodge split | physical owner identification, certified finite heads, other cells, the rung, and physical entry remain open |
...
| selected-root residue contact trie | [THM-3227](../01-canon/theorems/THM-3227-selected-root-residue-contact-trie-primitive-carry-and-delayed-resplitting.md) | **PROVED + VERIFIED-EXACT + INDEPENDENTLY HOSTILE-AUDITED:** coordinate-invariant `q`-ary contact trie, leaves-minus-one labels, exact-order-`p` edges, and unramified divided carries under `p`-divisible lower jets | no global root/owner selector or physical carrier; reduction can delay and recreate splits |
...
| lane | closest proved mechanism | canonical hostile | corrected near miss | least-used useful sidecar |
|---|---|---|---|---|
| LRC reflected | THM-3211 Bernoulli bulk/coboundary and THM-3224 gcd-carry/Hodge split | `H=(1,2,3,4,6,12)`; a fixed phase orbit can flip the second-order sign with `g mod 31` | MISTAKE-347 duplicated one order; a finite 168-owner scout was later mistaken for an eventual branch certificate | exact owner/residue potential, physical boundary clipping, and a certified finite head |
...
| planar JC | THM-2241 transverse response tail, THM-3172 owner filtration, and THM-3225 affine clutch saturation | the punctured cubic and its diagonal control have identical bounded marked jets; every affine-`B` heptic clutch is critical even after the only two boundary valuations | abstract response reconstruction, local clutch passage, or a reduced generic count was mistaken for global source entry | differential-owner ring; affine covariant `J_B`; finite clutch product; `S,T` valuations; saturated resultant and inverse-minimal-polynomial divisibility |
| factorial | THM-3186 continuant, THM-3217 three-row atlas, and THM-3215/3220 selected-root jet groupoid | nonempty barcodes, identical Smith types, or a flat two-jet cocycle need not select a global root; quotienting the four-jet group to three jets loses central area | MISTAKE-350 confused ambient dimension with support size | arithmetic exits, multi-place intersection, signed amplitude, independent lower hull, root permutation, and the fourth-jet central coordinate |
| tournaments | THM-3121 walk-content kernel, THM-3134 endpoint jet, and THM-3226 variational saddle | same profile can hide SCC order; fresh denominator primes do not imply non-P-recursiveness; transcendental `p` and `s/t` do not classify `R_Q4` | finite fitting, fresh primes, or factorial normalization was mistaken for a polynomial recurrence | complete moving jet, run-content resolvent, quotient variational equations, Hessian, and singular-radius arithmetic ledger |
...
The current board has eight objects.  Every new idea should be compared with
all eight rather than only with its home conjecture.
...
1. **Weak-order / two-star channel graph:** remembers which literal
   ...
   bulk limit, a Bernoulli-quadratic boundary potential, roots-of-unity residue
   modes, and a finite head.  A complete cell orbit is a closed phase path:
   its `1/g` coboundary cancels, while THM-3224's nonzero `g^-2` harmonic
   curvature and canonical owner potential survive.  The gcd carry can reverse
   the harmonic sign without changing the phase orbit.
...
3. **Local Newton flag / oriented exit amplitude:** a polygon forgets root
   residues but can separate factors by slopes at a chosen prime; the exterior
   transfer restores a signed path sum.  Support and Smith data do not control
   its cancellations, although the complete selected amplitude is
   P-recursive and linearly evaluable.  At a selected simple root, THM-3215's
   universal two-jet Hamiltonian turns every pair transition into an exact
   `dlog` shear cocycle; a primitive mod-`p` return reappears as first carry.
   THM-3220 adds the four-jet Heisenberg central coordinate: its commutator is
   ...
   THM-3227's proved contact trie and the audit-pending THM-3229 root-free
   contact-gcd flag are precise finite-family probes; only the latter is not
   canon.  Root permutation, the
...
4. **Pole valuation fan:** forgets other response families but can isolate the
   ...
   entry at infinity.
5. **Endpoint jet / quotient resolvent:** remembers precisely the derivatives
   consumed by substitution; its zeroth coordinate is scalar `H`.  A full
   repeated-join jet is one exponential-series binomial convolution.  For
   transitive blocks, the quotient resolvent exposes a variational saddle:
   balanced quotients have a transcendental OGF radius and fail every
   P-recursive law; every fixed strong tournament quotient on `q>=3` has a unique positive saddle; and
   the strong unbalanced `Q4` moves it off the equal point while leaving the
   arithmetic of its product radius open.
...
8. **Differential-owner filtration:** adjoins all target derivatives to the
   ...
   lift, the cheaper preceding observers are the degree-five residue clutch,
   the finite wall product `Lambda`, the single escape wall `tau`, and only
   then the saturated resultant.  THM-3225's affine covariant and `S,T`
   boundary valuations close every affine `B=b_0+b_1x` clutch as well.  Finite
   entry alone still does not decide the inverse-different gate.
...
coverage.  And both `p`-adic slopes and pole valuations are useful only when
the chosen local observer separates the faces that the target predicate
consumes.

## New exact mathematics

### 1. LRC fixed-grid response is inclusion--exclusion plus death

For `m|q`, define the principal response tail

```text
T_q(m)=sum_(m|d|q) phi(d).
```

If `q=prod p^a` and `m=prod p^b`, then THM-3130 proves

```text
T_q(m)=prod_(p^a||q) t(p,a,b),
t(p,a,0)=p^a,
t(p,a,b)=p^a-p^(b-1)  for b>=1.                              (1)
```

For an unhit-divisor upset with minimal antichain `M`, its complete grid
witness count is

```text
|W_q|=sum_(empty!=A subset M)(-1)^(|A|+1)T_q(lcm(A)).        (2)
```

Inserting a speed `v` kills exactly the part inside
`Div(gcd(v,q))`.  The count is decreasing and supermodular, and each divisor
layer is a bar of multiplicity `phi(d)` with death time
`min{i:d|v_i}`.  This is an exact operation law.  Its hostile boundary is
equally exact: after normalization, `q=6,S={1}->{1,2}` decreases the upset
mass of `{2,6}` from `3/5` to `1/2`.  A cemetery state can conserve the lost
mass, but that state is an added observer rather than an LRC witness.

### 2. LRC reflected assignment cover is cycle versus potential

For a directed edge define

```text
U_(u->v)={q>0:q_u<=q_v}.                                     (3)
```

THM-3135 proves

```text
union_(e in E)U_e covers all positive assignments
        iff E contains a directed cycle.                     (4)
```

Missing every lane means strict descent on every edge.  A directed cycle
makes this impossible; a DAG admits a strictly decreasing topological
potential.

The orientation is load-bearing.  For pair `(i,j)`, the tail ratio is
`Q/P=q_j/q_i`: low gives edge `j->i`, high gives edge `i->j`.  Keeping
`(0,1)` fixed and using both halves repairs

```text
H2=(1,3,4,6,8,12)
```

at caps `7/3` and `5/2`.  All `43+16` and `87+27` unequal-channel heads are
positive, and equality uses the inherited same-level mechanism.
...
all `30` ordered pairs and all low/high/full sides yield only

```text
cap 2,7/3: 0->1->2->3,          cap 5/2: 0->1->2.            (5)
```

The admissible assignment `(9,8,7,6,10,11)` misses every edge.  The three
complementary reverse-lane asymptotic limits are negative, so merely moving
...
A transverse finite-exact scout first replaced one uniform pair by the two-star

```text
01,02,03,04,05,12,13,14,15.                                (5a)
```

At safe upper-median cell `90`, that scout was positive.  THM-3150 extracted
the cap-two skeleton and common-dilation tail; THM-3156 proved the uniform law
through `D=100`; THM-3171 now completes the infinite channel audit.  Every
ordinary high lane has exact overlap at least `1/105`.  The sole weak
label-`{1,6}`, primitive-`3:5` lane has sharp floor `2030/280393`, but an exact
ratio graph forces at least three additional regular high edges whenever it
appears.  Thus the uniform nine-edge law has expected overlap at least
...
The barycenter itself is a boundary potential.  For a phase `phi`, integration
by parts gives

```text
B(phi)=-1/2 sum_(xi in boundary S_phi) sigma_xi Bbar_2(xi),
c_j=B(phi_(j+1))-B(phi_j).
```

Thus a complete reflected-cell phase path telescopes: `sum_j c_j=0`, and its
correction word is palindromic.  Put `d=gcd(168,e,f)`, `T=168/d`,
`n_g=gPT-e/d`, `m_g=gQT-f/d`, `h_g=gcd(n_g,m_g)`, and
`K=|Qe-Pf|/d`.  Concatenating the cells gives the exact aggregate

```text
J_g=T integral_0^1 chi(n_g u)chi(m_g u)du,
|J_g-T/49|<=T h_g^2/(3n_gm_g)<=T K^2/(3n_gm_g).
```

THM-3224 evaluates the formerly bounded term exactly.  With
`A_g=n_g/h_g`, `B_g=m_g/h_g`,

```text
Xi_g=Bbar_2((A_g-B_g)/14)-Bbar_2((A_g+B_g)/14),
J_g=T/49+T h_g^2 Xi_g/(n_gm_g)
   =T/49+Omega_g/g^2+O(g^-3).
```

Here `h_g|K`; `h_g`, `Xi_g`, and `Omega_g` are residue-periodic.  Across all
eighteen ordered lanes, `49Xi_g` lies in
`{-8,-6,-5,-3,1,2,3,4,5,8,9}`.  It is never zero and the `9/49` bound is
sharp.  Even the sign is genuinely arithmetic: the fixed orbit
`(P,Q;e,f)=(3,5;1,12)` reverses sign exactly on `g=4 mod 31`.

The cellwise eventual second coefficients have the canonical cyclic Hodge
split

```text
q_(j,r)=Omega_r/T+D_(j+1,r)-D_(j,r).
```

Thus the complete orbit retains a harmonic gcd-carry curvature while the
exact part retains owner and residue.  A fixed cell-90 hostile takes three
different second coefficients on three residue classes, and a zero-first-
correction hostile still has a mixed-sign second word.  The separate
168-owner sign scout is only `FINITE-EXACT`: no stabilization bound promotes
that finite word to an eventual theorem.  The live semantic debt is now
physical owner entry and certified finite heads, not derivation of the
...
point were a common simple zero of `V` and `M`.  THM-3133 splits by the source
orders `a=ord(A_src)`, `b=ord(B_src)`:

- `a=b=0`: the top polar row is strictly more singular than every retained
  lower row;
- `a=0,b>=1`: the unique pure-`q` face cycles through `phi`, `psi`, and `K`,
  and each congruence class of `R mod 3` contradicts a flux order;
- `a>=1`: the Faber polynomial ring is regular while the divisor identity
  requires the opposite order for `K`.

Thus common simple zeros do not occur for `R>=7`, without setting
`d_F=s_F=0`.  In the balanced response normal form this forces

```text
s_resp=0,       R=3k+2,       D=4k+3,       N>=2D.           (6)
```

Equality forces the two-pole passport `(D,D)`.  The first formal cell is
`R=8,D=11,N=22,(11,11)`.  The odd-bipole family with `T=x^2-1` and `E` the
polynomial part of `T^(d/2)` realizes the abstract response boundary for
every odd `d`; its constant first-order recurrence proves nonvanishing.  It
...
THM-3151 subsequently excludes every balanced `(D,D)` equality cell and
raises the balanced chart floor to `N>=4D`.  THM-3167 then separates diagonal
inverse-Jacobian response, constant-field membership, and ownership by one
polynomial `A2` source; no bounded marked jet decides the first gate.
THM-3172 attacks the owner gate instead.  Its shear-invariant filtration

```text
B_0=k[u,v,x,y],      B_(n+1)=B_n[partial_u B_n,partial_v B_n]
```

freezes at `k[x,y]` for a polynomial Keller owner.  A presentation
`v-H(u)=Phi(z)` with `deg(Phi)>1` forces the nonconstant unit `Phi'(z)` in
`B_1`, excluding the punctured cubic hostile without pretending to decide its
diagonal gate.  The same theorem gives the exact transverse recurrence

```text
A_1=-f_v,
A_(n+1)=f_T Lambda_f(A_n)-(2n-1)A_n Lambda_f(f_T),
partial_v^n xi=A_n(xi)/f_T(xi)^(2n-1).
```
...
The parameter discriminant is now complete for the entire constant family

```text
P_(b,c,k)=(Vz^2+bz+c)^2+A_src z+kx.
```

After `y=Vz`, its gradient equations have a universal resultant `V^3K`.
Every parameter coefficient of `K` contains the passport boundary
`S^3T^8x^9` or `S^3T^8x^6(x-1)^3`, leaving a residual `H` of exact degree
`52` for `k!=0` and `28` for `k=0`.  The finite-fibre walls are

```text
(bk-2c)(bk+c/2)(bk+2c)^3,
(bk-2c)(bk+2c/3)(bk+c)(bk+2c)^2.
```

If one vanishes, it is an actual critical point.  If they all pass, no
`T`-root remains and the only possible boundary escape is
`tau=bk+2c-2eta b^2=0` at `S`.  The next three exact jet coefficients reduce
to a cubic/quintic pair with Euclidean profile
`(5,3)->(3,2)->(2,1)->(1,0)`, so at most three resultant units escape.
Therefore at least `49` or `25` units remain away from `ST`, proving a
critical point for every constant triple.  Generic members have exactly
`52` (`k!=0`) or `28` (`k=0`) distinct Morse points; tuned escape controls
still leave `51` or `27`.

No member may be fed to THM-3172's `B_1`, which requires a complete marked
inverse pair; its critical point precludes any Keller mate.  The next honest
experiment must use nonconstant `B,C_0,E_0` data that alter the universal
gradient resultant while retaining all formal Faber fluxes.

THM-3225 closes the first nonconstant accessory direction.  For every affine
`B=b_0+b_1x`, put `y=Vz`, `L=y^2+By`, and
...
These generate the localized gradient ideal and their `y`-resultant is
`V^3K_B`.  If `B` meets `g=ST`, the common root already carries the explicit
critical point `(alpha,-1/A_src'(alpha))`.  Otherwise the passport boundary
leaves a residual degree-52 resultant.  Its leading `T` coefficient is
nonzero, so nothing escapes there.  At the sole `S` wall, an exact cubic/
quintic PRS has profile `(5,3)->(3,2)->(2,1)->(1,0)` in both accessory fields,
so at most two multiplicities escape and at least `50` intersection units remain.  The control
`B=1+x` has `52` squarefree Morse points.  The next deformation must therefore
be genuinely nonlinear in `B`, or move `C_0` or `E_0`; the affine family is no
longer a live search lane.
...
M_(n+1)=a^n(a+(n+1)b)
       +2(n+1)(2n+1)c M_n
       +n(n+1)(b^2-4ac)M_(n-1).                             (7)
```

Three consecutive zero moments beginning at `r` force
`b/a=-1/(r+2)`.  Put `p=r+2`, `v=p(c/a)`, and

```text
A_n(v)=L((p-t+vt^2)^n).
```

When `p` is prime, THM-3131 proves the exact lower Newton slopes

```text
A_(p-2): {0,1/(p-3)},       A_(p-1): {1/(p-1)}              (8)
```
...
The later Euclidean-flag program makes these local exits composable.
THM-3146/3153/3170/3176 add the prime offsets `d-3` through `d-6`; THM-3161
closes every four-exit row through `d=2000`. THM-3180 applies six exits before
the THM-3152 first full Euclidean row and closes all `79` residual rows in
`2001<=d<=2200`. THM-3201 applies all seven exits and closes all `50` residual
rows in `2201<=d<=2400`; the next five `d` values exit arithmetically.  A dual
extension then checks all `35` seven-exit residuals in `2401<=d<=2500` with
the primary Fraction-hull and independent determinant-hull engines.  Their
complete progressive degree traces agree.  The current exact boundary is
therefore

```text
1<=r<=2498 closed;      first unaudited r=2499, d=2501;
d,d-2,d-3,d-4,d-5,d-6 composite;       d-1 not a prime power. (9)
```

The `d-6` exit is load-bearing only for THM-3201's new range; no Gauss--Manin
Smith-band implication is used. A nonempty barcode remains only a necessary
observer. Continue the sequential exit/flag census from `d=2501`, changing
Euclidean row or local place only on actual survivors.

The structural transfer has also reached the actual offset-six PRS chart.
THM-3186's reciprocal continuant gives a P-recursive selected tail but permits
signed cancellations; THM-3210 makes the nonmonotone
visible/invisible/invisible/visible window exact.  THM-3192 repairs the former
typing gap by reversing coefficients: fixed top jets propagate homogeneously,
the hidden wedge returns at the next `z`-adic layer, and the maintained
offset-six factors `H,J,K` are exactly two Toeplitz Pluecker charts and one
...
both bounds sharp while keeping all earlier pivots regular.  The proved gap is
atlas selection, not an unknown information budget.  THM-3217 is now
independently hostile-audited canon: its eight universal coordinates and
nonzero fixed-offset exception integer `Xi_s` settle the displayed first
three rows, but not arbitrary depth or growing offsets.

THM-3223 is a **PROVED CANDIDATE + VERIFIED-EXACT awaiting independent
audit**.  It continues the same fraction-free atlas through rows four and
five, factoring those pivots into inherited factors and new primitive
...
```text
J_a(f)=[f' f'';-2f -f'],             J_a(f)^2=(f'^2-2ff'')I.
```

At a simple root this normalizes to the reflection
`R(kappa)=[1 kappa;0 -1]`, with `kappa=f''/f'`.  An arbitrary word has one
alternating shear; composable pair transitions are the flat cocycle
`d log(f_i'/f_j')`.  The shear is the next Pluecker jet
`f''g'-g''f'`.  In odd residue characteristic a primitive pair shear has
exact projective order `p`, and its scalar mod-`p` return lifts to a nonzero
rank-one first carry.  This is a literal bridge from the factorial exterior
reflection to arbitrary-degree root germs, but it is local: a global branch,
root permutation, multiple-root seams, infinity, and higher jets remain.

THM-3220 computes the first noncommutative higher layer.  Tangent-to-identity
four-jets in logarithmic coordinates `(A,B,C)` satisfy
...
At a selected simple root, `A` is half the two-jet shear, `B` is one-sixth
the Schwarzian, and `C` is its covariant fourth-jet derivative.  Thus the
three-jet projection loses a half-symplectic triangle and the fourth jet fills
it exactly.  Quadratic factorial germs lie on a twisted cubic: the central
commutator is an oriented cubic discriminant, its two-root deck line has the
quadratic Frobenius character, and an odd-prime reset produces a primitive
central carry.  This strengthens the local carrier, but supplies neither a
global root selector nor a physical factorial/JC carrier.

THM-3221 now makes the variable-depth statement exact.  The first unequal
coefficient of two derivative-normalized germs is precisely the first live
transition coefficient and transforms as a weighted cotangent tensor.  Its
minimal associated-graded quotient has additive order `p` in residue
characteristic `p`; over a mixed-characteristic DVR, a unit first coefficient
returns undivided after division by `p`.  A selected root, derivative anchor,
and full normalized tower through `D` reconstruct a polynomial of degree at
most `D`; there is no degree-independent finite jet.  THM-3228 then closes the
cardinality question for the four-jet center: every permutation action of the
full mod-`p` Heisenberg jet group that is faithful on its center has at least
`p^2` points, sharply.  A `p`-root permutation chart is
...
THM-3227 is **PROVED + VERIFIED-EXACT + INDEPENDENTLY HOSTILE-AUDITED**.
...
only claimed over an unramified `p`-adic lift when every lower jet is
...
Let `F_T(d)=d!pc_T(d)` count ordered spanning path covers, and let `b_T(pi)`
count backward consecutive adjacencies of a permutation.  THM-3134 defines

```text
E_T(u)=sum_pi (u-1/2)^b_T(pi)(u+1/2)^(n-1-b_T(pi))           (10)
```

and proves

```text
sum_d F_T(d)z^d=z E_T(z+1/2),
pc_T(d)=E_T^(d-1)(1/2)/(d!(d-1)!).                          (11)
```

Thus the full substitution sidecar is exactly an endpoint Taylor jet; the
Hamiltonian count `H=E_T(1/2)` is only its value.  Masks `40` and `76` both
have `H=15` but different first derivatives, producing `C3` counts `123` and
`135` after substitution with two singleton factors.

For `C3[S_1,S_2,S_3]`, the THM-3121 quotient walk series is

```text
W=(U+2xyz)/(1-xyz),       U=(1+x)(1+y)(1+z)-1.              (12)
```

Expanding `(12)` and applying finite differences gives the exact transform

```text
F_C3(d)=sum_(j<=min(d,q)) 2^j binom(d,j)
        sum_(t<=q-j) binom(d+t-1,t) D_(t+j)^(d-j),           (13)
```

where `D_s^m` is the `m`th forward difference at zero of the product of three
binomial profile transforms.  Equal `N`-vertex factors cost `O(N^3)`
arithmetic and `O(N^2)` memory.  Iteration gives exact Hamiltonian counts
through `3^6=729` vertices; the canonical cyclic-wreath subgroup divides
every ordered path-cover count.  The profile dimension still grows, so this
is an efficient exact transfer rather than a fixed-order scalar closed form.

THM-3181 finds a fixed-order law on the operation where the state really
closes: order join.  Permutation reversal gives
`Q_T(-t)=(-1)^nQ_T(t)` and

```text
sum_(m>=0)Q_T(m)x^m=x B_T(x)/(1-x)^(n+1),
```

so `ceil(n/2)` positive values recover the complete profile.  For
`T_r=A^(▷r)`, fixed extreme depth `k`, and `lambda_m=Q_A(m)`,

```text
A_(T_r)(k-1)=sum_(m=1)^k(-1)^(k-m)binom(ar+1,k-m)lambda_m^r,
```

with minimal recurrence
`product_(m=1)^k(E-lambda_m)^(k-m+1)` of order `k(k+1)/2`.  This does not
contradict the growing-state warning for cyclic substitution: SCC order is
lost, and `H(C3[C3,C3,C3])=3159` refutes the naive plethysm value `33`.

THM-3202's quadratic factor-jet table admits a sharper operational
conjugacy.  For `B_r=A^(join r)`, `N=ar`,

```text
F_r(c)/c!=[z^c]
 (sum_(j=0)^N Q_A(j)^r z^j/j!)
 (sum_(ell=0)^N (-z)^ell/ell!).
```

Thus one truncated polynomial product computes the complete moving jet in
`O(M(N)+N log r)` arithmetic and `O(N)` storage.  Any fixed `C3` output depth
then adds only `O_d(N)`.  This is a genuine fast exact algorithm, even though
the required state length still grows.

For the transitive seed `T_r=K1^(join r)` and
`U_r=C3[T_r,T_r,T_r]`, THM-3213 also gives the exact diagonal

```text
F_(U_r)(d)/(r!)^3
 =[x^r y^r z^r]W(e^x-1,e^y-1,e^z-1)^d.
```

The unique smooth point is `(log 2,log 2,log 2)`.  Its pole and Gaussian
Hessian yield an explicit leading constant times
`r^(d-2)(log 2)^(-3r)`.  Dividing again by `binom(r-1,d-1)` leaves a nonzero
`(log 2)^(-3r)/r` tail, which no C-finite sequence can have.  A shifted-prime
congruence puts a new `p^3` in the reduced denominator at every fixed depth;
for a fixed `q`-block quotient the same mechanism gives `p^q`.  Hence fast
evaluation, C-finiteness, and P-recursiveness are three different questions.
The last is now decided on a large exact scope: a rational D-finite germ has
algebraic finite radius, whereas this diagonal has radius `(log 2)^3`.
Consequently all three fixed-depth C3 normalizations are non-P-recursive.
For a strongly connected quotient `Q`, the equal saddle persists exactly
when its left/right Perron vectors satisfy `w_i v_i=constant`; then the radius
is `(log(1+1/rho(A_Q)))^q`, again transcendental.  Every nontrivial regular
tournament is in this Perron-balanced class.

THM-3226 solves the unequal-saddle geometry for the smallest strong
unbalanced quotient and, variationally, for every fixed strong tournament
quotient on `q>=3`.  For
the arcs `1->2->3->1`, `3->4`, `4->1`, `4->2`, the unique diagonal saddle is
`(s,t,t,s)`, where, for the unique positive root `p` of

```text
p(1+p^2)log(1+p^-2)-2(1+p)log(1+p)=0,
t=log(1+p),                    s=log(1+p^-2)/2.
```

It is strictly minimal and nondegenerate, and the normalized coefficient has

```text
C_d r^(d-5/2) R_Q4^(-r),      R_Q4=s^2t^2.
```

Both `p` and `q=s/t` are unconditionally transcendental.  This does not yet
classify `R_Q4`: unconditional non-P-recursiveness of the
normalized `Q4` sequence remains open, while Schanuel would imply it.  The raw
integer sequence is nevertheless non-C-finite by its superexponential
factorial factor.  Analytic saddle existence is no longer the unbalanced
bottleneck; arithmetic of product radii is.

### 6. Signed lift is not positive lift

THM-3132 proves for the projection of the `(r+1)`-forest boundary onto
rank-`r` component partitions that

```text
image over Q = all partition currents,          r even;
image over Q = augmentation-zero currents,      r odd.       (14)
```

An endpoint swap realizes every one-point transfer, and parity supplies the
last direction exactly when `r` is even.  This explains earlier rank-four
surjectivity without a large matrix computation.

But the lift is signed.  In the active product-Gamma pole-prefix probe,
plethystic pole stripping commutes exactly with labelled deletion, yet the
first active coarsest upset has mass `-2324160` at
`N=5,(a,b)=(1,3),I2,j=5`.  The whole deletion fibre cannot repair it because
an invariant upset remains negative.  Commutation, boundary lifting, and
...
THM-3219 closes one larger directed region exactly.  Above THM-3209's complete
reset, every nonempty physical completion `tau` has

```text
G_5^(Q+tau)((5))=-1440 sum_(r in tau) r^5<0.
```

One common-prefix invariant facet therefore excludes all 63 nonreset states
in the 64-state principal filter, through depth sixteen.  This is a proved
...
| complete LRC phase orbit | second owner coefficients | orbit average plus cyclic Hodge potential | exact residue-periodic `g^-2` coefficient | harmonic projection forgets cell owner; finite scout forgets stabilization | physical owner map, residue label, certified finite head |
...
| affine heptic source clutch | critical locus | localized gradient pair, resultant, then `S,T` valuation | critical intersection multiplicity and generic Morse count | resultant alone forgets a mate and inverse owner | marked inverse pair; nonlinear-`B` or `C_0,E_0` hostile |
| selected root four-jet | abelian three-jet quotient and permutation model | Heisenberg projection `(A,B,C)->(A,B)` | two-jet shear and Schwarzian coordinates | projection kills central area; fewer than `p^2` permutation points also kill it | fourth-jet center; selected root; a lawful carrier with at least `p^2` points |
| tournament factor | substitution state | profile `-> E_T` jet `->` exponential-series binomial product | every ordered path-cover count, with near-linear factor-jet evaluation | scalar loses derivatives; substitution output also forgets cyclic input root | same-`H` masks `40,76`; rooted labelled triple; prime-denominator ledger |
| strong tournament quotient | diagonal coefficient asymptotic | positive variational saddle and product radius | unique strict saddle, Hessian, exponential growth | analytic solution need not classify radius arithmetic | `Q4` product `s^2t^2`; algebraic/transcendental hostile |
...
that makes its operation well defined.

## Bold reframes that survived hostile tests

### A. Acyclicity has opposite values for different consumers

A forest has maximal independent boundary information, so it is ideal for
reconstructing signed differences.  The same absence of cycles leaves a
global potential, so it is fatal for covering all weak-order assignments.
The transferable lesson is not “use forests” or “use cycles”; it is to compute
the circuit rank seen by the named operation.

### B. Choose a portfolio of local observers

The factorial and JC arguments both win by making one face uniquely visible
at a local place.  In the factorial problem, the place is a prime and the
observer is the lower Newton polygon.  In the JC problem, the place is a
response zero and the observer is a valuation fan.  Their shared workflow is
legitimate:

```text
choose place -> normalize -> list face slopes/orders -> retain units or
coefficients -> prove separation -> test the first overlap.                  (15)
```
...
The useful sequence formulas here are not fitted scalar recurrences.  The
quadratic factorial recurrence comes from integration by parts, the tournament
transform from a rational quotient-walk kernel conjugated by finite
differences (accelerated by exponential-series convolution), and the LRC
totient formula from divisor inclusion--exclusion.
Each formula names the state it must carry and the operation under which it
closes.  “Closed form” is therefore best treated as a finite or controlled
state conjugacy, with the first discarded derivative/profile as the hostile.
Factorial normalization may reveal a diagonal asymptotic without producing a
fixed recurrence; THM-3213's `lambda^r/r` tail is the mandatory control.

### D. Death, transport, and refinement are different operations

Speed insertion deletes LRC divisor layers.  Pole stripping is virtual-letter
subtraction.  Forest boundary is a signed lift.  None is automatically a
positive mass transport.  Before invoking monotonicity, add a cemetery state,
...
| reflected LRC channel | Bernoulli-cubic bulk, first potential, and exact gcd-carry curvature | complete phase orbit cancels `1/g`; the nonzero periodic `g^-2` harmonic has exact `O(g^-3)` remainder | harmonic projection forgets the owner potential; the finite 168-owner scout lacks stabilization |
| cyclic tournament diagonal | one-product moving jet and unique strong-quotient variational saddle | balanced quotients are non-P-recursive; unbalanced `Q4` has a solved saddle but arithmetically open product radius | scalar `H` loses the jet; transcendental `p` and `s/t` do not settle `R_Q4` |
...
### G. Closed-path cancellation demands an owner or branch sidecar

THM-3211 and THM-3215 expose a sharper cross-domain pattern.  The LRC first
correction is the exact phase coboundary `B(phi_1)-B(phi_0)`; the selected-root
transition is the exact cocycle `d log(f_i'/f_j')`.  A closed complete cell
orbit cancels the former, while a composable root-chart triangle cancels the
latter.  In both cases, local first-order data cannot carry a global
obstruction after the path closes.

The missing coordinates are different and must not be conflated.  LRC now has
the exact residue-periodic `g^-2` functional but still needs its physical cell
owner and finite head.  The root-jet problem now has the fourth-jet central
filler but still needs a selected global branch, root permutation/monodromy,
multiple-root or infinity seams, and a physical carrier.  Moreover,
an arbitrary nonzero reflection word is not automatically a nontrivial Cech
loop: only composable chart transitions telescope.  The procedural payoff is
exact—when the first obstruction is a coboundary, search the quotient sidecar
that prevents the physically relevant path from closing trivially.
...
| Projected `k=3` descent | **PROVED + VERIFIED-EXACT + INDEPENDENTLY HOSTILE-AUDITED** | [THM-3218](../01-canon/theorems/THM-3218-projected-k3-z220-valuation-product-terminal-descent-and-cap219.md) closes `z1=220`, giving cap `219`, ledger `373427`, and only `16` next rows | attack the exact `z1=219` layer; THM-3230 is only a reserved empty stub, not a result |
...
| Selected-root residue contact trie | **PROVED + VERIFIED-EXACT + INDEPENDENTLY HOSTILE-AUDITED** | [THM-3227](../01-canon/theorems/THM-3227-selected-root-residue-contact-trie-primitive-carry-and-delayed-resplitting.md) gives a `q`-ary labelled trie, exact-order-`p` charged edges, and an unramified divided carry under `p`-divisible lower jets | test whether any physical owner map realizes the trie; retain delayed resplitting and ramified failure |
...
2. Use THM-3224's exact gcd-carry curvature and Hodge split instead of another
   dilation scan.  Map the canonical owner potential to the physical residual
   and certify its finite head; do not infer an eventual word from the finite
   168-owner scout.
3. Continue the projected `k=3` descent at the exact `z1=219` layer of 16 rows;
   THM-3230 is a reserved empty stub, not a result.  Keep exact wall universes
   and semantic hashes; never read the ledger as a physical-cover census.
4. Use THM-3228 only as a carrier hostile: a mod-`13` four-jet action on the
   168 owner cells cannot retain its center.  Any bridge must type an extra
   state and specify the preserved LRC predicate before arithmetic testing.

### Niche -- factorial sequences

1. Start at the exact boundary `r=2499,d=2501`, run the seven arithmetic exits,
   and extend the first Euclidean flag in deterministic chunks with the
...
3. Use THM-3186's linear-time tail, THM-3192's reciprocal top-jet map, and
   ...
   three PRS rows; audit the THM-3223 row-four/five continuation.  Use the
   audited THM-3227 trie as a hostile for any physical owner map; audit THM-3229
   before use.  THM-3229 is not the factorial PRS: identify an input-pair
   bridge before comparison, then target all-depth chart selection.

### Wildcard -- planar Jacobian

1. Treat both the constant family of THM-3212 and every affine-`B` family of
   THM-3225 as closed.  Search a genuinely nonlinear `B`, or a nonconstant
   `C_0,E_0` deformation that changes the global resultant, rather than tuning
   a clutch or boundary wall already exhausted.
...
1. Use THM-3181's half-grid/minimal recurrence on order joins.  For cyclic
   substitution, compute the complete moving jet with THM-3213's one-product
   transform; do not replace it by scalar `H`.
2. For a fixed quotient, test Perron balance first.  Balanced quotients are
   non-P-recursive by transcendental radius.  For an unbalanced quotient use
   THM-3226's variational saddle theorem, then attack the arithmetic of its
   product radius; start with `R_Q4=s^2t^2`.
3. Classify small quotient kernels by operational state width and diagonal-band
   geometry, with same-`H`/different-jet factors and the strong Q4 unequal-
   saddle example as mandatory hostiles; then isolate growing-depth failure.
...
5. Keep the degree-ten one-/two-pole exclusions as the first hostile; do not
...
For every new object or analogy:

1. name the source, target, map, and exact predicate consumed downstream;
2. list what the map forgets and the smallest plausible restoring sidecar;
3. run the cheapest hostile at an equality, composite, DAG, same-scalar, or
   coarsest-upset boundary;
4. if it passes, extend until a mechanism appears, not merely more data;
5. if it fails, record the minimal witness, first failed implication,
...
```bash
python3 04-computation/lrc_grid_witness_antichain_response_thm3130.py
python3 -O 04-computation/lrc_grid_witness_antichain_response_thm3130.py
...
python3 04-computation/gmc_projected_forest_boundary_parity_thm3132.py
python3 -O 04-computation/gmc_projected_forest_boundary_parity_thm3132.py
...
python3 04-computation/arbitrary_degree_root_jet_affine_dihedral_carry_thm3215.py
python3 -O 04-computation/arbitrary_degree_root_jet_affine_dihedral_carry_thm3215.py

python3 04-computation/factorial_universal_resonant_prs_wall_atlas_thm3217.py
python3 -O 04-computation/factorial_universal_resonant_prs_wall_atlas_thm3217.py
python3 04-computation/gmc_complete_reset_upper_filter_thm3219.py
python3 -O 04-computation/gmc_complete_reset_upper_filter_thm3219.py

python3 04-computation/root_four_jet_schwarzian_heisenberg_thm3220.py
python3 -O 04-computation/root_four_jet_schwarzian_heisenberg_thm3220.py
python3 04-computation/selected_root_osculating_separation_prime_carry_thm3221.py
python3 -O 04-computation/selected_root_osculating_separation_prime_carry_thm3221.py
...
python3 04-computation/lrc14_j7_k3_z220_valuation_product_terminal_descent_cap219_thm3218.py
python3 -O 04-computation/lrc14_j7_k3_z220_valuation_product_terminal_descent_cap219_thm3218.py
...
transcript verifies only its encoded finite claim; theorem promotion also
requires the type, implication, symmetry, equality, and scope audits stated
in canon.
