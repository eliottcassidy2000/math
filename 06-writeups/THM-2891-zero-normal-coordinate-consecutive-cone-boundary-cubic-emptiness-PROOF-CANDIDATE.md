---
id: THM-2891-PROOF-CANDIDATE
title: "Proof candidate: zero-normal-coordinate consecutive cone-boundary cubic emptiness"
status: >
  PROVISIONAL / UNDER AUDIT / NOT CANON.  For every integer n>=0, every real two-plane
...
# Proof candidate for THM-2891 -- zero-normal-coordinate consecutive cone-boundary cubic emptiness

**PROVISIONAL / UNDER AUDIT / NOT CANON.**
...
Since `U,V` are independent real polynomials, the integral model for `L`
makes `Q` positive definite over `R`.  The real binary quadratic `Q` is
therefore irreducible.  As in THM-2824, `Q` and the real binary cubic `C`
have a common complex projective zero if and only if
