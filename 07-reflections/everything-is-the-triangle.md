...
## Addendum (klein-S30, HYP-3723): the covering layer is the triangle too

The LRC covering modulus is the staircase, doubled and shifted: `Phi_6(n) = n^2-n+1 = 2.T_(n-1)+1`, killer
`= lcm(n-1,n) = 2.T_(n-1)`, covering-min `M = n/(2T_(n-1)+1)`, built recursively `Phi_6(n)=Phi_6(n-1)+2(n-1)`.
Two consecutive triangulars frame each level: `T_(n-2)=C(n-1,2)=#tiles` (the staircase) and
`T_(n-1)=(Phi_6-1)/2` (the covering-modulus core). The owner's functional decomposition `a(x)=x/2, b(x)=x+1`,
`f=`odd-member, `g=ceil(x/2)`, gives `f.g = T_x` -- the parity-routed triangular factorization, the
single-integer shadow of the 2-adic descent. So the covering structure (modulus, killer, CF `[0;n-1,n]`,
convergent escape, hexagonal lift) is generated by `{even/odd, +1/-1, x2 / /2}` on the triangular numbers,
recursively. The triangle extends from the tournament metagraph down to the lonely-runner covering floor.
