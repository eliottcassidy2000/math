...
| KCL at a node | the absorption law: every exit must meet a partner entry; capacity gcd(w_a,w_b)·[14·gcd \| w_a + w_b] |
...
distinct, and the enemy needs the tiling to PERSIST. The KCL is then a rigidity
obstruction, not a design tool: a maintained exact tiling of the deck demands
mirror-partner absorption capacity ≥ current at every node, and random distinct
7-sets fail this 1399/1400 times. In BSST the network's solvability produces the
squared square; here the network's INFEASIBILITY produces the lonely runner. The
same mathematics, run in reverse: **their existence theorem is our impossibility
theorem.**
...
- **THM-767**: balance identity (∫|B_a| = c/7 exactly, every stratum), two-value
  ...
  the explicit shape bound w_max > 7·Σ(P), and the KCL absorption law with exact
  gcd capacities. Referee battery all-pass; `deck_event_witness()` in the library
  (self-test 16/16).
- **Regime 2 corrected and measured**: the raw complementarity `r_P ≤ B(c*)` died
  ...
  the B5 lane.
- **The program** (HYP-6835): KCL-feasible packet census (the deck's AP/GW
  corners), the s-threshold decks (7∤c) via the closed Kronecker geodesic, the
  r ≥ 8 alignment residue, Lean targets (all counting, decide-friendly).
...
MAINTAINED covers under a phase flow, and their persistence is governed by a
conservation law. The circle carries arcs; the arcs carry currents; the currents
must balance at switches; and where they cannot — which is generically everywhere —
the runner is lonely. Squaring the square was always about when a rectangle can be
filled with no slack. The Lonely Runner's covering case is about when a circle
cannot be. The two problems share their bookkeeping, and the bookkeeping is
Kirchhoff's.
