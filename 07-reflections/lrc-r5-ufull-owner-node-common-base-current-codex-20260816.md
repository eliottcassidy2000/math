...
integral.  Its complete `F_7 x F_13` spectrum is nonzero in one certified
split-field image.  It does not isolate an exact address `C(a;X,m)`, identify
arrival and source time, transplant `U_clock`, exclude a row, or prove
LRC(14).
...
After inverse transformation to the thirteen `(1,0,t)` residue classes, its
seven-cell spectrum has support

```text
(total,DC,F7-axis,F13-axis,mixed) = (91,1,6,12,72).
```

Double-centering the output table removes exactly both axes and DC; all `72`
mixed modes remain nonzero:

```text
(72,0,0,0,72).
```

At the fixed relation class `(1,0,6)`, all seven `F_7` Fourier reductions are
the same nonzero field value

```text
317699132065964946247468.
```

The six displayed zero cell reductions are only statements modulo this split
prime; no characteristic-zero vanishing is inferred from them.  Conversely,
each nonzero reduction proves that the corresponding characteristic-zero
cyclotomic quantity is nonzero.
...
Replacing both source profiles by `1` gives a source-erasure bank.  It still
has full `91/91` spectral support and independently restores the literal
endpoint guards, so it is a positive engine control rather than an attempted
causal proof.  Its fixed-relation geometry and spectral support match the
previous desheeted endpoint calculation, now evaluated in the larger joint
field.
...
The computation advances the previous stopping boundary: source service and
actual `U_full` endpoint factors now coexist as an atomwise product on one
owner-node base, and their complete degree-graded `7 x 13` output is
nonvanishing.  It is therefore stronger than a contraction against
preintegrated endpoint atom weights.
...
The cheapest decisive continuation is now narrower: retain one exact-address
sidecar (or the inverse ancestry sheet that determines it) inside this joint
field calculation and test whether the fixed `(1,0,6)` signal survives.
Separately, test whether the `U_clock` endpoint factor descends to the same
owner-node coordinate without changing the source-time copy.  Spectral
closure itself is no longer the observed obstruction on this host.
