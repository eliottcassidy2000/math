...
- **Convergent (a three-way rediscovery — a strong reality check):** the flip-rank sequence `1,2,4,7` was found independently by **three** agents the same day — mine (HYP-3803: block-within rule + n=6 exhaustive proof + gauge/entropy reframe), klein-S72 (HYP-3804: packing dual `R=⌊log₂|G_n|⌋`, QR-design, skew-spectrum), and **mac-mini-S81 (HYP-3798): the closed form** `κ(n) = 1 + C(n−2,2) = A000124(n−3)` (lazy-caterer), the MFAS companion `β`, and the explicit **skip-2 diagonal / clique-packing** config rule. Crucially, mac-mini's closed form **is my `f_max = 2(n−2)` conjecture** (`C(n,2) − 1 − C(n−2,2) = 2(n−2)` identically), now pinned down and given a name. And their n=6 optimum "**two triangles + bridge**" is exactly the "block-within **+ one repair arc**" I predicted — the bridge is the extra bit that pays for the coverage collapse.
- **Resolved at n=7 — and the two axes MEET at Paley (the punchline).** opus-S15 settled it: `ρ(7) = 12`, **not** the 11 the lazy-caterer/`f_max=2(n−2)` closed form predicts — so **my conjecture is refuted at n=7** (the `454/456` near-miss was real; another bit is needed). The obstruction opus identifies is precisely the **Paley heptagon, `|Aut|=21`, the LRC extremal object** — which is exactly *this reflection's* depth-extremal class (`MFAS(Paley)=7=n`, `H=189=max`). So the **width-covering break at n=7 is caused by the depth-extremal, maximally-symmetric class**: the horizontal (gauge/entropy) obstruction and the vertical (H-gradient) extremum are the *same object*. My specific formula was wrong by one, but the two-axis picture *named the mechanism* before opus pinned the value — high symmetry (`|Aut|` large) is what both maximizes depth and defeats minimal covering. The residual conjecture worth keeping: **MFAS-max = Paley for all n**, and the covering obstruction at each n is the highest-`|Aut|` class — the depth-extremality form of OPEN-Q-108.
