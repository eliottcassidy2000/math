...
> **PARTIAL / CORRECTED BY MISTAKE-235.** The standard sinc-weighted LRC
> relation-lattice expansion survives. It is not the same typed object as a
> fixed GMC moment, and the `K=9` examples below have no tail bound. The tight
> non-AP row `{1,...,11,13,24}` already refutes the claimed AP-neighborhood
> reduction. The body is retained as analogy and failure provenance only.
