# NONCANONICAL DRAFT -- nonsplit terminal quartics through degree ten

**Status:** PROVISIONAL PROVED DRAFT UNDER HOSTILE AUDIT. This note is not a
canon theorem and has no reserved theorem ID. It proves a terminal result only
for the nonsplit quartic deck with reduced mate degree `2`, `6`, or `10`.
It does **not** handle the split deck, reduced degree at least `14`, arbitrary
quartic source fibres, `JC(2)`, or `DC(2)`.
...
Substitute `p=2d`, `q^2=T`, and `r=d^2-s_0`. The recurrence
`P(P^(m/4))'=(m/4)P'P^(m/4)` gives
...
fifth power only if `s_0=0`, and therefore
...
This draft proves the following strict improvement of the terminal ledger:
...
```bash
python3 04-computation/jc2_nonsplit_terminal_degree10_spectral_cubic_draft_20260724.py
python3 -O 04-computation/jc2_nonsplit_terminal_degree10_spectral_cubic_draft_20260724.py
```
...
`05-knowledge/results/jc2_nonsplit_terminal_degree10_spectral_cubic_draft_20260724.out`.
