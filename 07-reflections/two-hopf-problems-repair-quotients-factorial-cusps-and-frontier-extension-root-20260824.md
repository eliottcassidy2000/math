...
8. [THM-3997](../01-canon/theorems/THM-3997-reduced-two-three-hasse-repair-and-zero-residual-no-go.md)
...
9. [THM-3998](../01-canon/theorems/THM-3998-reduced-two-three-three-by-at-most-three-source-weight-support-obstruction.md)
...
- [Brendle--Hung `S2 x S2` ledger](../05-knowledge/reference/CORE-PAPERS-BRENDLE-HUNG-S2XS2-2026-08-24.md):
  the omitted `P1(L(h_a))=0` branch, generic Hessian positive definiteness, and hence
  `z(h_a)=0` are now **VERIFIED EXACT SYMBOLIC** relative to the reconstructed
  formulas. The global identities `V_ac=V_bc=0` are also verified exactly
  relative to that implementation, independently of saved notebook state,
  with a sensitive generic-point hostile. A separate moving-frame
  reconstruction verifies all four Sylvester minors. The other
...
`deg_s H_m<=m`. Moreover the residual fifth-root quotient is already scalar:
with `A5=a^5`, `b=beta/gamma`, and `d=delta/gamma`, one has

```text
G/gamma=u+(6/A5)p+R/gamma,
[p^2](R/gamma)=-16/(3A5^2),
[p^3](R/gamma)=b^2/4+2752/(135A5^3).                    (8b)
```

Thus THM-3999's companion-divisor atlas can work directly over `(A5,b,d)`;
no residual five-orbit enumeration is needed. This does not quotient later
target automorphisms. THM-3998's conic/ODE gives another independent hostile in a
...
literal polynomial without imposing any algebraic `SO(3)` constraint on the
final `q`-components. The background reconstruction still uses the Lie-frame
brackets. A separate independent reconstruction proves all four generic
Sylvester minors positive (but degenerating at both deleted endpoints), while
the global audit reconciles pullback-plus with pushforward-minus symmetry
conventions.
...
`Phi_(-k u_bar)`. The lattice tuple is not itself a rational coboundary; the
torus exponential is essential. Conditional on compatibility with the punctured-torus
exponential sequence, on zero-winding overlap differences being exactly the
Cech cocycles of the completed vertical-field bundle `V=f_*T_(X/B)`, and on
the manuscript's displayed vertical bundle

```text
0 -> O -> V -> O(-1) -> 0,
```

the sequence splits because `Ext^1(O(-1),O)=H^1(O(1))=0`; hence
`V=O direct_sum O(-1)` and `H^1(P1,V)=0` kills the remaining additive class.
Thus, within the prescribed
