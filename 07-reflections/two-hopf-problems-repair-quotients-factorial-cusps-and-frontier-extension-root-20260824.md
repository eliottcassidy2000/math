...
`H^1(dual graph,mu_3)`.  A forest kills clutch holonomy; a cycle is the only
place a new three-primary class can live.  Existence of that class would
still not make it a Keller boundary basis.
...
Equations `(12)--(17)` are **FINITE-EXACT SCOUT RESULTS** pending canon
promotion and independent audit.  They do not close the scale-two row or
`LRC(14)`.  The faithful next object is the signed exposed-event matrix, not
the individual address matrix.  On equality rows that survive, the next
scalar should be the cubic falling-factorial response
...
presentation moves.  Thus raw coordinate noninjectivity does not exhibit
analytic moduli.  Compute the action of global sections and fibre translations
on the actual holomorphic gluing cocycle, then quotient.  Only after that may
one ask whether `p` is complete analytically.  Independently reconstruct the
oriented attaching maps and nearby-cycle extensions; Smith forms of displayed
matrices cannot certify their geometric provenance.
...
cover in characteristic zero.  A forest is then expected to pay either extra
node addresses or a missing/nonproper normalization end.  This is a precise
completion dichotomy to prove, not yet a canon claim.
