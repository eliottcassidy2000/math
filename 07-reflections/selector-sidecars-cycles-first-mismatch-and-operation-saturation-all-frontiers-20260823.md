# Selector sidecars, cycles, first mismatch, and operation saturation

**Status: CURRENT RESEARCH SYNTHESIS, updated 2026-08-24.**  This note compares proved
canon, cited imports, reproducible finite computations, provisional candidates,
and hostile controls.  It creates no object-level reduction.  LRC(14), JC(2),
DC(2), FC(3), HFC(3), the Arithmetic--Kakeya target, the strong tournament
inequality, and every Rule 30 prize remain **OPEN**.

## Outcome

Four mechanisms survived comparison across the active portfolio.

1. A **filtering selector** merely discards candidates.  A
   **mechanism-changing selector** makes its chosen object eligible for a new
   theorem or operation.  The distinction explains why another residue
   barcode did not finish the factorial frontier, while an LRC multiplier
   divisible by one coefficient can genuinely remove a runner and invoke an
   inherited lower-rank theorem.
2. **Cycles carry second-round debt.**  Forest structure makes the
   Arithmetic--Kakeya `k=1` floor easy; the first triangle changes the sound
   score.  In LRC interval atlases, orbit closure and connected components
   likewise matter after every pairwise width has been computed.  Pairwise
   data are not automatically composable.
3. The **first mismatch** is more informative than another positive replay.
   It identifies the missing coordinate: the prime `19` adaptive killer in a
   factorial row, the `(3,7,2)` LRC pair-only hostile, the polynomial/formal
   split in Jacobian normal strips, or the lower filtered bucket after a
   leading gauge cancellation.
4. A complete static universe should be **saturated before it is optimized**.
   Rule 30's positive-word monoid saturates the odd half of the address space;
   the two-cube elementary layers saturate a Bernoulli Euler product.  In both
   cases the exact complete mass is clearer than a sequence of local lower
   bounds.  Neither saturation theorem supplies the missing physical or
   arithmetic asymptotic by itself.

These are reusable research moves, not a common mathematical object.

## Live concept board

| Lane | Object / representation | Preserved invariant | Native operation | Missing sidecar |
|---|---|---|---|---|
| LRC(14) | rank-eleven star plus a two-runner packet | exact interval widths and coefficient ratio | delete/absorb a runner, then invoke lower-rank loneliness | relative scale, component owner, phase, arrival |
| Jacobian | rank-two residual `(T,f)` and its degree filtration | discriminant residual and leading homogeneous symbol | additive lift gauge / strict transform | lower residual debt, branch place divisor, index form |
| Factorial | exact-support coefficient rows | exit/divisor/rho barcodes | choose the next prime specialization | actual surviving polynomial and first adaptive killer |
| Arithmetic Kakeya | sound forcing certificate | normalized score under the declared rules | add a forcing row / close a dependency cycle | intrinsic cycle incidence and future-round debt |
| Rule 30 | affine address words and ray images | exact static orbit image | compose positive words or route an adaptive query | physical chronology, owner, projective carry |
| Two cubes | inert-prime singleton rows and elementary layers | singleton fibre and critical weight | add a distinct inert prime to the product | collision law outside the singleton family |

Every new object was compared with all six lanes.  The strongest connections
occur at the level of information loss and operation compatibility.

## 1. Filtering is not the same as changing the proof mechanism

The exact extension of
[THM-3483](../01-canon/theorems/THM-3483-factorial-nondivisor-residue-digit-pair-compiler.md)
through `r<=10028` gives a clean hostile.  In the window `9980<=d<=10030`,
the final ledger is

```text
9 exit rows, 36 divisor rows, 6 rho rows, no survivors.
```

At `d=9996`, however, the inherited divisor barcodes still leave `3998`
coefficient candidates.  The first ordered adaptive specialization killing
the residual is `p=19`; it is not a unique magical prime.  This refutes only
barcode completeness.  A barcode filters the existing row, but it does not
change which theorem applies to a survivor.

The LRC support-two operation is different.  If a chosen multiplier is
divisible by coefficient `p` or `q`, the corresponding runner is absorbed and
the problem changes rank.  The selector has paid for a new implication.  Its
legitimacy therefore depends on the divisibility witness and the inherited
lower-rank theorem, not on whether the selector looks canonical.

This yields a practical test:

```text
selector output -> which new hypothesis becomes true?
```

If the answer is “none,” the selector is only a filter.  If it changes the
applicable theorem, record the exact source, target, and destroyed runners or
coordinates.

## 2. Cycles are the first place local certificates can acquire debt

[THM-3875](../01-canon/theorems/THM-3875-arithmetic-kakeya-k1-forest-one-round-score-floor.md)
proves that every normalized `k=1` forest certificate has sound score at least
`2`.  A five-row triangle reaches `5/3`, so cycles are necessary for the first
interesting improvement.  This is a structural boundary, not the
...
[THM-3903](../01-canon/theorems/THM-3903-arithmetic-kakeya-strict-two-by-two-cyclic-score-seven-four.md)
...
[THM-3966](../01-canon/theorems/THM-3966-lrc14-signed-endpoint-cross-phase-and-fixed-scale-two-family.md)
...
collision and THM-3966's Graver/half-translate hostiles are complementary
...
does not recover its sign partition, owner, or endpoint phase.  THM-3966's
...
```text
local edge certificate + acyclic incidence -> composable by peeling;
cycle -> retain a second-round compatibility or circulation sidecar.
```

This suggests the next AK search should enumerate intrinsic minimal cycles at
`k>=2`, with every candidate replayed under the sound rule set, rather than
merely enlarging a forest bank.

## 3. First-mismatch hostiles locate the missing coordinate
...
- In the Jacobian normal-strip program, a formal inverse can be unique while
  polynomial termination fails.  The polynomial/formal comparator must be
  carried explicitly.
- The Russell sextic scouts shear `(6,1)--(6,3)` into known quintic cells and
  kill the principal `(6,5)` branches, but the nonreduced `(6,4)` cell needs a
  strict transform.  The top pole cone forgot the lower principal part.

The correct record for a false method is:
...
This prevents a method hostile from being advertised as a counterexample to
the underlying open conjecture.

## 4. The Jacobian front is now a sequence of typed invoices

The marked-root problem is no longer an undifferentiated conductor search.
[THM-3880](../01-canon/theorems/THM-3880-marked-root-opposite-sign-normalization-fibre-nondescent.md)
and
[THM-3883](../01-canon/theorems/THM-3883-marked-root-complete-local-square-cube-descent-criterion.md)
separate the exact local invoices:

```text
A=0:  sign compatibility;
z!=0: common nonzero branch residue;
z=0:  the missing cube z^3 in the completed branch ring.
```

The global sign-kernel/class-group obstruction of
[THM-3877](../01-canon/theorems/THM-3877-quadratic-resolvent-cyclic-quotient-degree-four-branch-obstruction.md)
is a different object.  No canonical map has been proved between its
permutation sign and the local marked-root sign.

At the three-cusp point,
[THM-3881](../01-canon/theorems/THM-3881-cusp-ideal-residual-transport-rank-two-matrix-factorization.md)
contracts all polynomial cusp coefficients and lift ambiguity to the pair
`(T,f)`.  Its residual has sidecars `L^2f` and `Pf^2-T^2`.  The full `T=0`
lane and every affine-slope lane `T=hf`, `deg h<=1`, now have only the base
square survivor.  The constant edge is essential: it is detected by the
noncancelling coefficient `243c^4`.

[THM-3884](../01-canon/theorems/THM-3884-cusp-residual-total-degree-leading-gauge-filtration.md)
is the first all-pair filtration.  For nonconstant `f`, a square requires

```text
deg T>=deg f+1.
```

At equality the leading pair is a lift-gauge symbol.  Two hostile audits show
this over every integral domain of characteristic different from three.  The
gauge fixes one coordinate but changes the mixed lift by a `Delta`-multiple;
...
forces `deg_y T>=deg_y f`; THM-3901 retains the strict-degree fan and THM-3902
the positive-equality two-jet response.  New
...
but those jets can lift.  THM-3905 extends the color product through the third
...
and
[THM-3882](../01-canon/theorems/THM-3882-rational-dual-one-place-wronskian-projection-criterion.md)
settle the place invoice for the best `C3` packet.  For every immersed plane
normalization, one-place dual support forces genus zero, projection degree
one, and the smooth-conic boundary.  Without immersion the exact reduced
...
cap.  Its unanchored conjugate-color gain stays constant, while the affine-
infinity anchor detects the transition as a simple tie.  This is the same typed
...
THM-3922 adds a different global carrier.  If the actual normal affine
completion `X` contains `A2`, its omitted prime divisors form a `Z`-basis of
`Cl(X)`.  Torsion on a quadratic resolvent is not transferable: the test must
be run on `X` itself.  THM-3924 does so for THM-3921, finding

```text
Cl(B)=Z<g>,                  [E_ram]=5g.
```

The mandatory ramification prime is nonprimitive, so no extra deletion can
repair the rank-one boundary basis.  The same integer five is the `A^5`
power-index exponent and the difference `6-1` of its two pole valuations.
Here integral index debt has become actual boundary-class divisibility; no
general index-to-class transfer is claimed.
...
THM-3927 constructs precisely that sharper hostile. Its unit-ideal normal
nonmonogenic cubic has a rational sextic branch, two affine collision fibres,
two addresses in each, primitive boundary basis, and scalar units. Yet the
target normalization is `G_m`, so THM-3841 excludes it; independently its
ramification curve identifies both affine pairs and violates THM-3920. The
natural deletion has `chi_c=5`, and compressing the two infinity places to a
one-place octic makes the maximal order monogenic. MISTAKE-467 now records the
repeated projective-versus-affine carrier repair.

THM-3928 closes the most obvious singular-conic escape with a different
currency.  On a one-place branch the Cardano parameter `h=3q/(2p)` is
polynomial.  If `p` is a product of two distinct affine lines, parallel lines
are impossible; nonparallel line degrees satisfy

```text
max(e_1,e_2)=N,             e_1+e_2=2 deg(h).
```

Hence a sextic pays fold degree `4`, `5`, or `6`, with packets `(6,2)`,
`(6,4)`, or `(6,6)`—never a birational coefficient map. Under the full
classical sextic hypothesis all three rows fail: respectively multiple
infinity support, a missing weighted-degree-15 monomial, and an impossible
pure-sixth binary form. Its universal
resolvent divisor lattice is only `Z direct_sum Z/3`: splitting a component
does not manufacture a second three-class, just a free anti-diagonal needing
new global data.  This and THM-3925 form a pole-versus-fold trade: the smooth
marked seam pays two Jacobian-zero punctures, while a chosen one-place split
...
paying THM-3801's generator debt.

THM-3934 and THM-3935 then close the most natural attempt to turn that class
into a different completion. THM-3934 proves that every conic restricting to
a square on the branch normalization is either a double line or `XZ`; the
unique reduced support recovers the same Cardano divisor. THM-3935 supplies
the global sidecar that the simple-elliptic tangent cone forgot. Every vertical
fibre is irreducible and principal, the generic elliptic curve has

```text
MW=ZQ,            height(Q)=1/4,            O^--O^+=3Q,
```

and therefore the affine resolvent has scalar units and class group `Z/3`.
The Cardano prime is Weil but not Cartier: `Pic(S)=H1(S,mu_3)=0`, whereas on
the smooth locus both groups are `Z/3`. Thus the cover is quasi-etale, not
etale across the singular origin, and the two nonzero smooth-locus characters
are inverse presentations of one cyclic field. The local `E~8` exceptional
curve advertises four lines in `E[3]`; the complete vertical/infinity ledger
...
THM-3939 isolates the invariant that those computations were measuring. If
an elliptic generic fibre deletes exactly two sections with infinite
difference `D`, integral vertical fibres give

```text
Cl(B)=E(k(t))/<D>,
0 -> E(k(t))[3] -> Cl(B)[3] -> I_D subset F_3 -> 0.
```

One boundary relation can create at most one new three-line, regardless of
the free Mordell--Weil rank. THM-3940 tests that distinction rather than only
stating it. For

```text
q=t^3+t(r+beta t)X-X^2,                 beta!=0,
```

the elliptic surface has `I7` at infinity and, on a nonempty parameter open,
`5I1` and free rank two. Its three-division polynomial still has no rational
root, so the Cardano divisor remains the only class-group three-line and the
natural normal cubic remains monogenic. Free rank was visible in the elliptic
surface and then destroyed by the one-relation quotient; rational `E[3]`
would survive.

The
[double-torus near miss](jc-double-torus-two-character-costs-a-second-infinity-place-20260824.md)
hits precisely that missing coordinate. The identity

```text
q_0^2-4X^3=q_1^2-4(X+t)^3
```

creates intrinsic rational three-torsion from the difference of its two
Cardano decompositions. It has scalar units and
`Cl(B)[3]=(Z/3)^2`, attaining the two-boundary bound as one boundary-
divisibility line plus one inherited `E[3]` line. But its rational quartic
branch normalizes to `G_m`, not `A1`: the translated-cube ansatz forces a
conic parameter with two poles. This is the sharp new tradeoff. A second
character is algebraically cheap once rational `E[3]` is present; making it
coexist with one branch infinity place is the expensive coordinate.

[THM-3942](../01-canon/theorems/THM-3942-affine-linear-double-torus-factor-split-one-place-obstruction.md)
turns that example into a complete affine-linear theorem.  In independent
coordinates, UFD allocation of the three lines in `p_1^3-p_0^3` has only two
orbits: the `1|2` conic with two ends and the `0|3` Fermat cubic with three.
Both really carry two independent Cardano characters, so character scarcity
is not the failure.  Parallel coefficients give only line components or
duplicate data.  More strongly, a nonconstant `A1` coefficient carrier cannot
retain each whole cube factor on one side: any escape must split an
irreducible factor internally or use gcd/multiplicity overlap.

The first such escape makes the missing sidecar literal.
[THM-3944](../01-canon/theorems/THM-3944-repeated-factor-double-torus-one-place-square-conductor-collapse.md)
splits the two copies in `p_1-p_0=G^2` between the complementary factors.  It
does buy a smooth one-place parabola, but

```text
H=-P^2(4P+3G^2).
```

The doubled line makes the quadratic order nonnormal.  Its normalization is
`A2`; one Cardano row becomes an exact cube, while the other is the boundary
class `(2,1)` on the original regular locus `Gm^2` and ramifies when extended
across the conductor.  MISTAKE-471 records why the original smooth locus and
the normalization's smooth locus cannot be interchanged.

[THM-3947](../01-canon/theorems/THM-3947-scalar-weighted-repeated-square-split-trichotomy.md)
then exhausts every scalar weight in that repeated-square allocation.  The
generic discriminant is the reduced union of three distinct smooth one-place
...
now supplies that audit for every natural nonconstant-ratio same-field row.
Every prime boundary curve of a normal surface containing `A2` has rational
normalization, while the equianharmonic residual prime has positive genus;
arbitrary common debt contributes only a square to its discriminant. Zariski
Main would force that ramified prime into the boundary of a same-field Keller
chart, an immediate contradiction. Under `gcd(c,RU0V0)=1`, there is also an
independent forest certificate: the graph and residual meet over at least two
finite colors, giving two disjoint paths and hence a cycle on a common
resolution. This extends THM-3920 from two branches of one prime to repeated
incidence of two primes. The genus gate preserves normalization type and the
forest gate preserves prime labels and incidence; the latter forgets
tangency/contact order. Both need the sidecar that the ramification prime is
deleted in the same normal function-field completion. They do not prove that
an arbitrary normalization parameter descends to a polynomial target
coordinate.

[THM-3952](../01-canon/theorems/THM-3952-minimal-mobius-internal-split-carriers-are-four-critical-colors-and-nonentry.md)
classifies the smallest polynomial-carrier response. For unit extra debt and
degree-one `R/S`, the two assigned factors generate `k[t]` exactly when source
infinity is one of the four critical colors of `phi`. At every other color
the optimal degree pair is `(3,2)`, forbidden for a polynomial `A1` embedding
by degree divisibility. Each of the four critical gauges has a linear pencil
member, but retains two or three clean graph/residual incidences and is
excluded by THM-3951. The Riemann--Hurwitz comparison explains the bridge to
THM-3941: `phi` has a `C2^4` genus-one closure, while replacing the rational
map by a polynomial cubic moves one branch point to infinity and yields the
genus-zero `C3,C2,C2` ledger. This preserves the inertia colors but changes
the map and does not transport polynomial color solutions between the lanes.
MISTAKE-475 restricts this dichotomy to rational-source covers and the
THM-3941 handoff to its centered trace-zero grammar; positive-genus sources
can leave `C2^4` without acquiring `C3` inertia.

The complementary rationally split hidden cubic is now
[THM-3953](../01-canon/theorems/THM-3953-rationally-split-hidden-cubic-ramification-triangle-nonentry.md).
Its missing linear root row forces three pairwise collision carriers, which
become a forbidden triangle of ramification primes on the already normal
cubic. The independent audit passed after MISTAKE-474 repaired a 3/3
formal-factor-type count that had been overread as a 3/3 realized-point count;
the triangle needs only one zero of each nonconstant coprime carrier. Its
promoted constant-ratio addendum identifies the surface with `xy=c(t)^3` and
uses either a nonconstant unit or exact Nagata 3-torsion.

[THM-3954](../01-canon/theorems/THM-3954-extra-common-debt-creates-A-singularities-and-nonunibranch-residual-boundary.md)
then resolves the local information destroyed by the clean forest quotient.
When extra debt absorbs a color with order `m`, the completed normal surface
is `xy=t^(3m)` and one irreducible residual prime has two normalization
addresses. This is an independently proved non-unibranch mechanism, although
THM-3951's stronger genus gate already gives all-`c` same-field nonentry.

[THM-3956](../01-canon/theorems/THM-3956-split-hidden-cubic-integrality-and-repeated-root-trichotomy.md)
closes the rest of the split hidden-cubic lane: monicity eliminates rational
denominators and repeated roots reduce to ramified quadratic or total-
ramification forbidden-unit packets. THM-3958 closes the exactly-one-root
case by making the reduced graph-plus-residual different principal. Finally,
independently audited THM-3960 observes that this monogenic-different gate
works before factorization: every integral natural `T^3-3PT-(E+CP)` surface
...
different grammar—not another hidden-root factorization. Proved THM-3961 now
...
global different. Its repeated-factor and `P^2` nonnormal conductor debts are
the only survivors in that monogenic grammar; THM-3962/3963 only reserve their
...
`r`-fold coordinate crossings, where normalized differentials acquire a
branch-mismatch cokernel. These results explain what normalization forgets;
they do not globalize a twisted section or prove the audited Hopf/S6
manuscript. Proved THM-3959 closes all seven of THM-3941's degree-five color
rows: L/M/O are empty and J1/J2/K/N/P end at a scalar seam or folded
non-unibranch arm.
...
classifications.  (The displayed obstruction identities themselves are
algebraic under characteristic-zero base change.)
[THM-3943](../01-canon/theorems/THM-3943-rational-weight-eight-four-torus-sextics-have-no-one-place-line.md)
closes every rational simple configuration, and
...
coefficient to divide the other endpoint coefficients, so a unit coefficient
ideal makes the cubic monogenic. THM-3930 crosses that tariff with a centered
degree-two root map, producing a normal nonmonogenic `S3` order whose branch is
an aligned line plus rational quintic, both one-place and within the affine
...
with finite `t(infinity)`, exact trace first forces that value to zero without
a root translation. The finite Riemann--Hurwitz budget then says every pole
partition either shares an `A`-address or is one triple pole over a simple
critical point. Shared addresses coalesce on the ramification curve and
violate unibranchness. The sole collision-free normal form
...
THM-3938 advances the same finite-grammar method through centered root degree
four. Trace and the two finite Riemann--Hurwitz units leave five rows. Exact
color division gives two scalar index-form endpoints, already monogenic, and
five nonscalar families whose irreducible one-place branch components occur
to discriminant exponent one. Every nonscalar family has a two- or
three-address target fibre; rank-three flatness coalesces those addresses on
one source ramification arm, making it non-unibranch. Thus the fixed centered,
linear-color grammar is closed through root degree four. The next root-degree
threshold is five, not four.

[THM-3941](../01-canon/theorems/THM-3941-all-degree-centered-cubic-pole-carrier-routing.md)
separates what really persists at arbitrary repeated-root degree from what was
special to degrees three and four.  Completed-local trace leaves exactly
three outcomes: no finite pole, a shared `A`-address, or a collision-free
carrier of type `C3`, `C2`, or `C2 x C2`.  The shared-address row forces only
`a(A_0)=C=0` in the bare centered grammar.  It becomes a source
non-unibranch obstruction only after adding genuine exact-double, reduced
tame `(2,1)`, maximal-normal Keller hypotheses.  MISTAKE-470 records the
...
cubic, and THM-3933's index-cleared octic all turn an apparently rich
one-place branch packet into a globally monogenic maximal order; THM-3935
proves uniqueness of the normal cubic field for the exact THM-3932
branch/resolvent, THM-3937 makes that uniqueness uniform in the fold
parameter, and THM-3938 closes degree four by scalar endpoints or source
folding.  THM-3941 saturates the **carrier** grammar at all degrees, but does
not saturate color existence or maximal-order geometry. THM-3959 now solves
the seven `N=5` carrier signatures; the next centered unit is degree at least
six, not another degree-five pole partition. Arbitrary root gauges, other
color directions, rational-`E[3]` one-place resolvents, and altered
branch/resolvent families also remain open.

Concurrent JC promotions now put centered color canon through THM-3959 and
normal monogenic canon through arbitrary-`q` THM-3961. THM-3962/3963 remain
reserved. THM-3955/57 are proved local conductor theorems with no global Hopf
consequence. The proved LRC endpoint theorem moved to collision-free
THM-3966; the namespace move changes no mathematical scope.
...
filtration and the
[sextic filtered-kernel scout](jc2-sextic-filtered-kernel-principal-parts-cross-frontier-scout-20260823.md).
In both lanes a leading symbol may lie in the kernel while no genuine lift is
known.  Abstractly,
...
need not be onto.  The source and target maps are different, but the failure
anatomy is identical: the associated graded object preserves the top
cancellation and destroys the lower residual/principal-part debt.  The needed
sidecar is the next filtered bucket, and the cheapest decisive test is to lift
one symbol by one layer.

The
[alternative Russell cubic-seed audit](jc2-russell-cubic-seed-quotient-density-cocycle-scout-20260823.md)
adds a determinant version of the same warning.  Constant density on a chosen
section is an artifact; the full source-addition quotient carries a
determinant cocycle, and the raw affine cubic cell is empty.  Rational
corrections remain open.  The determinant is not a decorative Jacobian scalar:
it is the predicate forgotten by the quotient.

The
[composite-depth ordinal synthesis](jc2-composite-depth-ordinals-filtered-liftability-and-hidden-coordinate-portfolio-root-20260823.md)
provides a useful negative control.  Triangular or odd-square ordinals can
...
identifies the positive affine-word monoid exactly: ambient active words send
each even ray onto the entire odd half.  Consequently the least literal
two-stage bank for depth `D` and gap budget `B` has
...
native normalization uses independent Bernoulli variables with
...
Williams' cited fixed-modulus Mertens theorem then gives the proved bound

```text
liminf H(X)sqrt(log log X)/sqrt(log X)
 >=(2/5)sqrt(2/3)E_32
 =0.25659588... .
```
...
```text
identify the complete finite operation monoid/product;
normalize its whole mass exactly;
only then optimize the cutoff.
```

They do not share states, probability, chronology, or a reduction.

## 7. Sidecars must be typed by the consumer
...
| LRC projected polyhedron | short Graver row | take a minimum-width flatness covector | necessary relation with `l1<=356` | sign partition, owner and physical phase | labelled endpoint word plus cross-phase | THM-3966 equal-Graver opposite-covariance hostile |
...
| Four-ray tangent cone | cubic Keller branch | pass from completed branches to normalization addresses | per-component affine address count | grouping among reducible global components | reduced irreducible branch label | THM-3923 fixed-packet `4>3` hostile |
...
1. **LRC anchor:** combine auxiliary-center and endpoint cross-phase uniformly
   ...
   keeping `t<U` separate.  THM-3966 closes only the 1,365 fixed bodies
...
2. **Jacobian anchor:** leave `f=0`, x-only, every radial chart, the named
   ...
   THM-3959 now closes THM-3941's seven `N=5` carrier signatures by exact
   color division and a maximal-source audit. THM-3951 closes every natural nonconstant-ratio
   same-field cubic, with a second forest proof in clean rows; THM-3952
   ...
   irreducible arbitrary `q(P,t)` and leaves exactly two nonnormal conductor
   debts. Normalize those debts (THM-3962/3963 are reserved targets), then
...
3. **Nonmonogenic niche:** build a genuinely nonradial unit-ideal order with
   ...
   THM-3930, THM-3932/33/35/37/38 and THM-3940 show that rationality, the cap,
   one-place alignment, higher free rank, and even a genuine `C3` class can
   still leave Jelonek, unibranch, class, unit, Euler, monogenic, or uniqueness
   ...
   irreducible arbitrary-`q` row. Normalize its repeated-factor or `P^2`
   conductor debt, or start from a different nonmonogenic field/order; then
...
5. **Factorial wildcard:** learn a local rule predicting the first adaptive
   ...
   as hostile controls.
6. **Rule 30 / two-cube wildcard:** ask for asymptotics only after identifying
   which adaptive chronology or collision fibre is still absent from the
   complete static mass.

The session's broadest lesson is modest but operational: the useful bridge is
usually not a shared formula.  It is a shared diagnosis of what a quotient,
selector, cycle, or leading symbol forgot, followed by the smallest sidecar
that makes the next native operation lawful.
