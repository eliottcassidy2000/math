...
- MISTAKE-466 blocks replacing the original regular locus by the full
  normalization.  THM-3955 shows the same operation can erase a cotangent
  class, while THM-3944 involves a Kummer class.
...
| **Niche -- conductor algebra** | can the claimed failure of a normalization reduction be isolated without trusting the manuscript? | compute `Omega` at `xy=0` before and after torsion-free normalization | **THM-3955 PROVED**, characteristic-free |
...
This is the strongest unconditional mathematical progress of the session.  It
proves the local mechanism asserted in the S6 manuscript's response to CDP20,
but it neither globalizes `x dy` nor verifies the claimed threefold.
...
## 8. Incoming signal: THM-3950--3954 changes the sidecar
...
All three are **PROVED + VERIFIED-EXACT + INDEPENDENTLY HOSTILE-AUDITED** in
...
incidence pattern consumed by the ambient recognition theorem.  The newest
split-hidden-cubic namespace THM-3956 remains a **RESERVED / UNPROVED EMPTY
STUB** and supplies no result or dependency here.
...
2. **Conductor nearby-cycle complex.**  Construct a chain model retaining the
   normalization, three double curves, two triple points, and all incidence
   maps; compare it integrally with the displayed pushout matrix.
3. **Global cotangent audit.**  Starting from THM-3955, compute the conductor
   sheaf on the claimed `W` and determine whether the local generator
   globalizes with the required twist.  Keep local nonzero, global nonzero,
   and ambient extension as separate predicates.
...
7. **JC three-site census.**  For each surviving boundary grammar after
   THM-3942, 3946, 3949, and incoming 3950--3954, record original regular locus,
...
- THM-3955's characteristic-free node cotangent sequence;
- the general two-by-two Smith conclusion (11) under its stated coprimality
  and admissibility assumptions;
- the repo theorems cited with proved status, including incoming THM-3950--3954.
