...
> **CORRECTION (2026-08-24; MISTAKE-468).**  The first version treated a
> nonzero kernel vector of the scalar `p` as evidence that `p` loses analytic
> gluing data and proposed a kernel representative as a sidecar.  For coprime
> `(a,b)`, that kernel is generated by the two standard Seifert presentation
> changes.  It proves noninjectivity only on raw labelled triples.  The
> holomorphic gauge quotient is **OPEN**; all affected passages below are
...
native gluing operation
  ...
  -> quotient by section/fibre-translation gauge,
...
clutch gauge control: for (a,b)=(3,4), ell and ell+(1,3,0)
                      have the same p, and the move is a standard Seifert
                      presentation change rather than a proved moduli change;
...
| ordinal evaluator | `p=ab ell0-b ell1-a ell2` | unit increment in a gluing coordinate | selected group order | analytic gauge quotient and sign |
...
- the ordinal is a lawful response, but its raw fibre must be quotiented by
  gauge before it is treated as moduli;
...
What is not lawful is replacing a raw labelled presentation by `n` without
first identifying its gauge.  For coprime `(a,b)`,

```text
phi(ell0,ell1,ell2)=ab ell0-b ell1-a ell2,
ker(phi)=Z(1,a,0) direct_sum Z(1,0,b).                  (12)
```

The proof is exact: reduce the kernel equation modulo `a` and `b`, write
`d1=a r`, `d2=b s`, and obtain `d0=r+s`.  In the manuscript's Seifert
coordinates these two generators are standard presentation changes.  Hence
`p` is complete for the displayed Seifert data modulo standard gauge; the
same-`p` kernel does not prove distinct analytic threefolds.

The correct analytic task is to compute

```text
(raw twist lattice)/(global section and fibre-translation gauge),
then record (|p|, residual analytic class, ordered gluing word). (13)
```

Whether a nontrivial residual analytic class remains is **OPEN**.
...
This is a procedural task generator.  It creates neighboring raw presentations
with a known response delta.  Before asking whether their analytic
realizations, homology, or moduli differ, quotient them by the actual gauge
sheaf; do not manufacture a kernel ordinal from presentation redundancy.
...
- ordinal plus the residual class after analytic gauge quotient.
...
4. quotient admissible twists by section/fibre-translation gauge and only then
   enumerate any residual classes inside `|p|` fibres;
