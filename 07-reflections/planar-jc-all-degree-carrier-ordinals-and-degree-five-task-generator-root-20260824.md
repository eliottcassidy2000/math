...
(infinity order m, finite inertia carrier, finite pole orders)
    ...
    v
one of C3, selected C2, C2 x C2, or the root-regular exit.
```
...
For a non-root-regular degree `N` row the possibilities are:
...
The wildcard is therefore a four-coordinate conjunction rather than a vague
request for "more characters": a **normal** quadratic surface, a **reduced
irreducible one-place** branch, at least **two extendable** cubic characters,
and compatibility with the polynomial/Keller side.  The affine-linear split,
repeated-square split, scalar-weighted split, and classified high-weight
sextics each lose a different coordinate.  The next honest generator should
enumerate unequal or multi-factor internal splits by the first lost
coordinate and kill that coordinate cheaply before attempting a full atlas.
...
gcd/multiplicity overlap, or a different affine boundary line.  THM-3950 is
only a reserved empty stub and contributes no proved consequence.
...
the 2,858-gate carrier companion, its period-twelve identity, and its ordinal
map.  **OPEN:** color survival for the seven degree-five tasks, arbitrary
