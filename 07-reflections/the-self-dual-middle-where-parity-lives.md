...
There is ONE mechanism of symplectic linear algebra standing under all four objects:

> A non-degenerate **alternating** form on a finite abelian group / free module
> decomposes into hyperbolic planes; its order/determinant is a perfect **square**,
> with square root the **Pfaffian**.

Its incarnations share this one mechanism (the symplectic/hyperbolic decomposition);
they are not literally the same theorem — the finite-abelian-group version (Ш) and the
skew-matrix version (Pf) are parallel corollaries of it, not invocations of each other
(adversarial-check correction). With that precision:
...
antisymmetric. The decisive sharpening (adversarial-verified from Poonen–Stoll §2,5):
**alternating and antisymmetric COINCIDE on every odd-primary part; they differ only
at the 2-primary part.** So the defect from alternating is *forced to live at the
prime 2* — and the order can be off from a square by *at most a factor of 2*, never
any other multiple. The user's "doubling" intuition has its most precise home exactly
here: the square-vs-not obstruction is a 2-adic, ±1 phenomenon.

- **BSD (Poonen–Stoll 1999):** for higher-genus Jacobians the polarization-induced
  Cassels–Tate pairing can be only antisymmetric, not alternating; then \|Ш\| is
  **a square or 2 × a square**, decided by a single canonical class **c ∈ Ш[2]** with
  ⟨c,c⟩ ∈ {0, ½} (Ш square ⟺ ⟨c,c⟩=0). A positive density (~13% of genus-2
  hyperelliptic curves /Q) sit on the non-square, "odd," 2-defect side.
- **Tournaments (THM-442):** H² − Pf² = 8Q. The Pfaffian Pf is the square skeleton;
  Q (the #P-hard correction) measures departure from "H is itself the Pfaffian." The
  factor 8 = 2³ is itself 2-adic — the tournament defect, like Ш's, is carried by the
  prime 2. **FRAME, not identity:** 8Q and Poonen–Stoll's factor-2 are different
  constants doing different jobs; what they share is that *the departure-from-square
  lives at p = 2.*
...
- **Tournaments, MULTIPLICATIVE:** H is **multiplicative over strong components**
  (Moon): T = T₁⇒T₂ (dominance join) ⟹ H(T)=H(T₁)·H(T₂) — re-verified here in every
  test. So R = {H(T)} is the **multiplicative semigroup generated by strong-tournament
  H-values** ("H-primes"). This is the tournament **Euler-product** ζ-side:
  condensation = the transitive tower of strong components = factorization into
  H-primes.
...
Z/2 grading. **This merge is the new lens (HYP-2424); the realizability fact it acts
on is already PROVEN canon (next paragraph) — I rediscovered it mid-session and credit
it fully.**

**The realizability theorem is HYP-2271/HYP-2180 (NOT new this session — I withdrew my
duplicate THM-490 stub).** The project has already proved: R = **all odd numbers except
{7, 21}** — a co-finite multiplicative numerical semigroup of **genus 2**, Frobenius
number **21**. Mechanism: Moon multiplicativity + the strong-minimum
minH_strong(m) = 3, 5, 9, 15, 25, 45, 75, … = **Busch (2006)** (= Moon's upper bound),
strictly increasing, so minH_strong(m) ≥ 25 > 21 for all m ≥ 7; with exhaustive m ≤ 6
strong spectra this forces {7,21} to be the only gaps. (7 = Φ₃(2) is prime and never a
strong-H atom; 21 = 3·7 needs 7; yet 49 = 7² ∈ R because some strong tournament has
H = 49 directly — a gap with realizable powers, the numerical-semigroup signature.)
**Correction logged:** the once-cited formula minH_strong = m²−5m+9 is a 4-point
coincidence that fails at m=7 (gives 23; truth 25) — already a MISTAKE in canon.

So the tournament side is **not** a Frobenius *question* — it is a Frobenius *theorem*
(genus 2). What this session adds is only the cross-domain reading: the genus-2 missing
set {7,21} is the tournament's "two absent Euler factors," the multiplicative shadow of
Goldbach's additive "every even = two primes."
...
- **New this session:** (a) the **shared-mechanism identification** THM-174 ↔
  Cassels–Tate's "alternating ⟹ square" (same symplectic decomposition, parallel
  corollaries), with the sharpening that the defect is **2-adic** (lives at the prime-2
  part: Poonen–Stoll's c ∈ Ш[2], the 8 = 2³ in 8Q) — the precise home of the user's
  "doubling"; (b) the **s=2 segment** as the named bottom rung of the polygonal↔Goldbach
  ladder, aligning polygon-side s with additive arity (HYP-2421); (c) the **additive
  Goldbach vs multiplicative Euler MERGE** (HYP-2424) — reading the project's PROVEN
  genus-2 H-semigroup (HYP-2271, R = odds\{7,21}) as the tournament ζ-side opposite
  Goldbach's additive side, bridged by the segment and λ-doubling; (d) the
  **self-dual-middle table** organizing RH/BSD/Hodge/Goldbach/project under one
  involution-with-parity frame, with λ(2n)=−λ(n) as the single rigorous ½/doubling link.
- **Caught and withdrawn:** a THM-490 stub asserting the realizability semigroup as new
  — it is HYP-2271/HYP-2180, already proven (Moon+Busch). Scouring canon before posing
  caught the duplicate; OPEN-Q-094 (the "Frobenius question") was retracted the same day
  for the same reason — it is a genus-2 *theorem*, not an open question.
...
- BSD and Hodge are OPEN (Clay); Ш is not known finite in general; ternary Goldbach is
  proved (Helfgott 2013 — accepted to Annals of Math Studies, monograph in final
  revision; community-accepted), while strong Goldbach and Lemoine/Levy are OPEN
  (Lemoine verified ~10⁹–10¹⁰ directly; a generalized form to ~10¹³). State every status.
