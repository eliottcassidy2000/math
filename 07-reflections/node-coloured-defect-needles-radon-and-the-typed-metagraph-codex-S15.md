...
stalk generated by the Walsh levels.  In fact, with `S` the unnormalised flip
generator, `A_T(x)=sum_j a_j(T)x^j`, and `b_r=a_(n-1-r)`, the next step is

```text
S a_j=(j+1)a_(j+1)+(n-j)a_(j-1)-(n-1)a_j,
S A=(1-x)((1+x)A'-(n-1)A),
K=(b_1+(n+1)b_0)/2,              S K=b_2+b_1-Mb_0.         (3.3a)
```

Thus the functional form is an all-size tridiagonal ladder, not an all-size
two-scalar closure.  Through `n=7`, `(H,K)` determines `S K` only because the
ambient OCF kernel `(1,-5,4)` is absent from the realized locus.  At `n=8`
exactly one of 1,727 `(H,K)` fibres splits: `(345,2820)` has drifts `-240/7`
and `-60/7`.  The pointwise companion is an odd-cycle-forest current:

```text
K(T^e)-K(T)=4(sum_(gained C) K(T-V(C))
                -sum_(lost C) K(T-V(C))).                 (3.3b)
```

It holds on all 6,340 audited node-edge incidences through `n=7`.  Even the
full first-moment stalk need not determine diffusion.  The unordered
`K`-gradient multiset separates all 272 size-seven nodes.  This is the same
lesson as the Kakeya covariance algebra below: a low-order moment pair can
close accidentally on a finite locus, while the operation-stable object is the
full tridiagonal coefficient/Walsh stalk.
...
THM-858 adds a third tropical-looking operation that is genuinely
higher-arity.  For common-sheet H5 orders, every nonempty colour set `S`
obeys the complement-lcm relative-capacity cut

```text
sum_(i in S) ceil(2m_i/13)/m_i>=1,
m_i=D_i/gcd(D_i,lcm(D_j:j notin S)).                    (6.15)
```
...
runner tournament as the universal object.
