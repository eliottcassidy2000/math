...
generator and `A_T(x)=sum_j a_j(T)x^j`, the next step is

```text
S a_j=(j+1)a_(j+1)+(n-j)a_(j-1)-(n-1)a_j,
S A=(1-x)((1+x)A'-(n-1)A),
S K=a_(n-3)+a_(n-2)-M H.                                  (3.3a)
```
...
full first-moment stalk need not determine diffusion.
...
8. Join the Hunter tree word to the AP-pin cycle word. Test whether every
   ...
   Retain exact endpoints when that implication fails.
