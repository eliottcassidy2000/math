...
*codex-2026-07-21. Companion to THM-2000, correcting THM-1985/1990 through
MISTAKE-209 and extending Downey--Ong--Sellers.*
...
`{1,2,3,6}` both have mass two.  Powers of two also have mass two, and
independent Egyptian refinements generate continuum many further examples.
The scalar is not a fingerprint; it is a conserved valuation.
...
coordinates.  This reveals three facts that the paper's presentation hides:
...
| lacunary | super-geometric decay | converges rapidly |
...
| fibbinary / Moser--de Bruijn | exact block automata and dimensions | block-tail computation next |
| A000568 and other census rows | start-index-sensitive numerical prefixes | arithmetic nature unknown |
...
Ordering supports by scalar mass produces only a transitive tournament.  It
has no cycles, one Hamiltonian path, and no structural content beyond a
ranking.  The assumption that sequence values should be vertices is as lossy
as assuming runners must be vertices in LRC work.
...
proof obligations.  Bounded-ratio occupancy blocks are the smallest vertices
that preserve the convergence predicate.  The Dirichlet profile is their
...
1. Tail-certified intervals for every census prefix already printed in
   THM-1985/1990, with canonical offsets and support deduplication.
2. Full Dirichlet profiles, not only `s=1`, for fibbinary, Moser--de Bruijn,
   Moser-circle, shallow-diagonal, and tournament-census supports.
...
5. A formal analysis library for finite Abel summation, the master-figurate
   factorization, and exact collision taxes.  Digamma and infinite-series
   endpoints can then be layered above a kernel-checked arithmetic core.
