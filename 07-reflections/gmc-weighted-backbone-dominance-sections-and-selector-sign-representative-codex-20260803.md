...
**Status:** FINITE-EXACT PROVISIONAL SYNTHESIS NOTE. The companion and stored
transcript agree in normal and optimized mode, but this note has no theorem ID
and is not a proved canonical dependency.
...
Every reset-link state occurring here has one Q-directed physical edge, and
that edge ends at `Q`. None of the dominance-relation arrows is such a
physical transition. Consequently the result is a Cech-style dominance
handoff only. It is not response composition, a positive current, a Gaussian
moment functional, an owner phase, or an LRC decrement.
