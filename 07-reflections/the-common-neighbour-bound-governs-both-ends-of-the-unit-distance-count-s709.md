...
status: REFLECTION on THM-420. One geometric fact — two unit circles meet in ≤2 points —
  ...
  threshold (κ/2)N AND the floor C(κ,2)+2; the gap 17→39 is the cost of Euclidean realizability.
tags: [unit-distance, kovari-sos-turan, common-neighbours, K23-free, K4-free, kissing-number,
  harborth, erdos, eisenstein, additive-vs-multiplicative, lrc-worry-set, two-circles, THM-420]
---
...
`U ≤ N(1+√(8N−7))/4`, which exceeds `3N` only once `N ≥ 17` (THM-420). So:

```
   the SAME inequality        Σ_v C(d_v,2) ≤ 2 C(N,2)
   read as an upper bound  →  asymptotic CEILING  U = O(N^{3/2})   [Erdős/KST]
   read as a lower bound   →  moderate-N FLOOR    U > 3N ⟹ N ≥ 17  [THM-420]
```
...
general statement (THM-420(A)): in any ≤2-common-neighbour graph, average degree
`> D` forces
...
## The gap 17 → 39 = the cost of Euclidean realizability
...
so the realisable minimum is pushed up to `N* ≤ 39` (the edge-midpoint-centred √7
Eisenstein patch, THM-420(B); improves HYP-2267's 43). The interval `[17, 39]`
measures exactly **how much Euclidean geometry costs over pure combinatorial
design** for this extremal task. Closing it (HYP-2284) is the live question.
...
- **Sharpens the S702/S703 lattice story.** S702 (HYP-2262) separated the
  ...
  N=43 disk. THM-420 adds the missing *lower* bound — and it lives entirely on the
  ...
  combinatorial (CN/KST). They meet in `[17, 39]`.
...
- **Same rosette skeleton as the LRC worry-set (HYP-2170, THM-401/403).** The UD
  ...
  **additive energy** maximised by a lattice/AP. The "3" in 3N is `κ/2 = 6/2`, the
  ...
  worry-set's third-roots (27 = 3³ at n=14) and the dichromatic χ=3. THM-420 says:
...
- **K₄-free is the unused second lever.** Planar unit-distance graphs are also
  ...
  triangle). THM-420's floor uses only (CN); folding in K₄-freeness (bounding
