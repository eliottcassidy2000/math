...
status: >
  ...
  pair, and 7033 sparse low-filtration rows have no `F_4` mate. On the
  minimal member, (p,x+y) does give a finite degree-three polynomial map,
  but its ramification curve meets the affine-plane open. A
...
audit: >
  ...
  rational compression, finite cubic, discriminants, ramification, and
  bounded-search scope. A general-generator determinant modulo p was added
  to prove global nonmonogenicity, and the 7033-row search was correctly
  scoped to F_4 mates. Normal and optimized runs give CHECKS=7494 and
  LF-normalized output matching the frozen file; all hashes pass.
depends_on:
...
script_sha256: 38cee174ab543d7565c0a14155179accb82446e6fde6b3273a96718787acebc1
output_sha256: 68fe1b77494757aa754a344c1c6b731797379338c1de21dd822eb6da17bad9b3
semantic_sha256: a975db791a77fa913af642a33dcc3ef36dd13da846af3342b9335a5b60659d53
...
normal basis. Put `w=z-1`. The exact multiplication table on
`{1,x,w}` is
...
closure of `k[p,C]` in the cubic field.

The word nonmonogenic is global, not only a statement about the chosen
element `x`. Write any possible generator as

```text
theta=c+ax+bw,                 a,b,c in k[p,C].           (F7c)
```

Subtracting `c` does not change the generated algebra. Using `(F7a)`, the
determinant of `{1,theta,theta^2}` in the basis `{1,x,w}` is

```text
D(a,b)=-a^3p+a^2bC+ab^2(1+p^2)-b^3pC.                  (F7d)
```

If `B_2=k[p,C][theta]`, Cayley--Hamilton makes
`{1,theta,theta^2}` span the finite free rank-three algebra, so `D(a,b)`
must be a unit. Modulo `p` this becomes

```text
a_0 b_0(a_0 C+b_0) in k[C]^*.                           (F7e)
```

Hence `a_0`, `b_0`, and `a_0C+b_0` would all be units of `k[C]`. The first
two are nonzero constants, making `a_0C+b_0` nonconstant, a contradiction.
Thus `B_2/k[p,C]` is not monogenic.

For the natural element `x`, the change from `{1,x,w}` to the
monogenic rows `{1,x,x^2}` has determinant `-p`, and correspondingly

```text
Disc(F3)=p^2 Delta_2,
Delta_2=4C^4p-8C^2p^4+20C^2p^2+C^2
        +4p^7+12p^5+12p^3+4p.                           (F7f)
```
...
not branch; `(F7d)--(F7e)` prove the stronger global nonmonogenic claim.
...
`E` meets `U` in a nonempty ramification locus (and meets `L_1` at
`p^2=-1`). The map `(F1)` is therefore the exact positive finiteness
...
Both runs must print `CHECKS=7494` and, after LF normalization, byte-match the
frozen output.
