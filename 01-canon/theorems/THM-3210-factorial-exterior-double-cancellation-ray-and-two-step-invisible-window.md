...
status: >
  ...
  no-triple, n=1,2,3); INDEPENDENT IMMUTABLE AUDIT PENDING.
  THM-3186's exit-time amplitude satisfies deg_v E_L = L-2, so the complete
...
audit: >
  ...
  length-four visibility.  Normal and `-O` replay are byte-identical.
...
## 1. The amplitude is a polynomial of degree `L-2` in `v`
...
`C_r=alpha_(n+r)C_(r-1)+d beta_(n+r)C_(r-2)` gives

```text
deg_v E_L = L-2,        L>=2.                                          (2)
```

So `E_2` is a nonzero constant (the entry pivot), `E_3` is **affine** in `v`,
`E_4` is a conic, and so on.  `(2)` is verified for base indices `n=1,2,3,4`
and `L=2,...,6`.
...
rational arithmetic only; no floating point, random sampling, imported
