...
> **FOUR-SPEED CLOSED, AND THE OPEN QUESTION RE-SCOPED (opus-S381), see THM-1165.** k = 1,2,3,4 substitution neighbourhoods of {1,...,13} are ALL proved exhaustive, each giving only the two known families (k=4: 2242028 nodes, 85% pruned, 0 empty residues, 414499 leaf checks). IMPORTANT SCOPE CORRECTION: the density coefficient 1 - 2k*lam vanishes at k = 7 = 1/(2*lam), so this method reaches AT MOST SIX substitutions. Families differing in seven or more speeds are permanently outside its reach, so 'exactly two tight families overall' is not merely unproved by this route but UNREACHABLE by it -- settling it needs an argument that does not proceed by bounded substitution from the classical family.
