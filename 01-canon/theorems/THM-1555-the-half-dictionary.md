...
status: >
  ...
  one value at each n.  This is the part that is genuinely explanatory rather than
  recovered.
...
related: [THM-1455, THM-1525, THM-895]
...
`(1 + 0)/2 = ½` and the adjacency diagonal must be `0`. **Everywhere this dictionary is
applied, the correction term is a half.**
...
## II. The characteristic-2 collapse, and why this repo carries two matrix formalisms

The dictionary is invertible exactly when `2` is. Over `GF(2)` it fails, and it fails
**asymmetrically**: since `−1 ≡ 1`,

> `S = A − Aᵀ ≡ A + Aᵀ = J − I ≡ J + I  (mod 2)`,

which **does not depend on the tournament at all**. Verified `n = 3..7`: `S mod 2` takes
**exactly one value at every `n`**, while `A mod 2` separates all `8, 64, 1024, …`
tournaments tested.

So the skew matrix carries *zero* information mod 2 and the adjacency matrix carries
*everything*. That is the structural reason this repo maintains two parallel formalisms:

| layer | matrix | why the other one is unavailable |
|---|---|---|
| `GF(2)`: cut/cycle split, switching classes, even graphs, Rédei mod 2, mod-2 rank | `A` | `S mod 2` is constant — no information survives |
| char 0: Pfaffians, skew spectrum, THM-1455 parity, THM-1475 | `S` | `A` mixes the parity grading with the `J − I` background |

Neither choice is a convention that could have gone the other way. **Each formalism is
the only one available on its side of the dictionary**, and the `½` is precisely the
