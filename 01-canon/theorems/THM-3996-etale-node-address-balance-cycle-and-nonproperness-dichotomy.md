...
audit: >
  ...
  a condensation-DAG balance argument. A second audit repaired only the
  contrapositive's graph-definition scope and supplied disconnected,
  incomplete, nonproper, and ramified hostiles. The companion verifies the
  THM-3992 normalization, node addresses, cycles of lengths 2..12, the
  deleted-address path, Riemann-Hurwitz degree-one control, and an embedded
  two-cycle model. Normal and optimized runs match the frozen output after LF
  normalization.
depends_on:
