...
**Prime squares — also clean (PROVED).** When `C=q²` the only proper nontrivial subgroup is `K_q`
(order `q`), so there is *no nesting and no combination*: every collision is a single `H_q` flip,
classes have size 2, and `m=C/q=q` gives
> **`C=q²`:  deficiency `= 2^{(q−1)/2+(q−1)/2−2} = 2^{q−3}`.**

Verified `C=9→2⁰=1`, `C=25→2²=4`, `C=49→2⁴=16` (`signed_lrc_fast_orbit_s707g.py`, a fast
`Φ²`-matmul orbit counter agreeing with brute force for all `C≤39`).

**Higher prime powers and mixed `q^a·p` need the full subgroup lattice — and blow up.**
`C=27=3³` has deficiency `69` (a size-4 class plus size-2 classes) because the nested subgroups
`K_3⊂K_9` produce *combined* half-system flips (`H_3`, `H_9`, `H_3⊕H_9`). The mixed case is far
worse: `C=45=3²·5` has subgroups of orders `3,5,9,15` with `K_3⊂K_9` nesting (and `K_q-coset`
unions automatically refine `K_{q'}-coset` unions when `K_{q'}⊂K_q`), giving
**deficiency `= 8728 = 2³·1091`** — not a clean power of two, and far above the naive
single-subgroup sum (`160`). The uniform count over all factorizations remains **open**: it is the
count of the *group of silent moves* generated by `{H_Q : Q a proper subgroup order}` under
symmetric difference, weighted by how many cuts each move (and each composite move) is silent for —
an inclusion–exclusion over the subgroup lattice of `ℤ/C`. See HYP-2270/HYP-2273.
...
`signed_lrc_count_law_s707e.py` (full-coset = exact silent set; per-prime counts),
`signed_lrc_re_zero_s707f.py` (Lemma 1 `Re≡0`; count formulas), `signed_lrc_fast_orbit_s707g.py`
(fast `Φ²` orbit counter: `C=49→16`, `C=45→8728`), and outputs in `05-knowledge/results/`. Reflection:
