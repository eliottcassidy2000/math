...
status: >
  ...
  THM-3947 makes the full discriminant a reducible union of parabolas.  For
  ...
  and constant -2, explaining the uniform no-one-place gate.  A constant
  factor reduces to THM-3942.  Thus no irreducible full discriminant in this
  entire affine internal-split grammar is one-place.
source: jc-degree6-place / full slope-ratio repair of the THM-3942--3947 internal-split lane, 2026-08-24
audit: >
  INDEPENDENT HOSTILE AUDIT PASS (jc-cohn3709, 2026-08-24). The first
  ...
  charts. The end counts, frozen output, and all hashes agree.
depends_on:
...
algebraically closed field `k` of characteristic zero.  Fix
...
1. If one of `A,B` is a nonzero constant and the other is nonconstant, `(3)`
   is a whole-factor singleton split.  THM-3942 gives two infinity places.
   A zero factor gives duplicate torus data.
2. If both are nonconstant and have the same zero, the full discriminant is
   reducible by THM-3947: generically it is three distinct parabolas, and at
   the two exceptional scalar weights it is a doubled parabola plus another
   parabola.
3. If both are nonconstant and coprime, the full discriminant is irreducible.
...
That theorem proves the full discriminant is always reducible into three
parabolas counted with multiplicity.  The endpoint `m=1` is THM-3944:
...
The companion verifies the full `m,c` coefficient grammar, both torus rows,
...
corrected THM-3944 character ledger.
