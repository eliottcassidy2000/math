...
status: (A), (B) and the odd-n corollary of (C) PROVED (derivation below; machine-verified EXACT over every tiling at n=4..7 for (A), over all gridsym tilings at n=4..9 for (B), 16/16 and 512/512 direct c3-parity checks at n=5,7). Even-n self-loop count 2^{n/2−2} VERIFIED (n=4,6,8; refinement-canon iso checks; each loop in a distinct class) — NOT yet proved: the multiplicative twist family ⟨2,−1⟩ mod n+1 accounts for all loops at n=4 (×2 and ×2⁻¹, each a unique GF(2) solution), HALF at n=6, NONE at n=8 (remaining witnesses are non-affine permutations); the full witness classification / Burnside count is open.
...
- **Even n (verified, count open):** the `D = 0` pool is the central column
  (`C(n−2,(n−2)/2)·2^{…}`; n=8: 1280, census-exact), and the self-loop count is
  **1, 2, 4 at n = 4, 6, 8** (`2^{n/2−2}`), each loop in a distinct class. The witness
