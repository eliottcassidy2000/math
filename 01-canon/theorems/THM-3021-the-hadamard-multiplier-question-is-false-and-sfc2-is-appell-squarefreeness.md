---
id: THM-3021
title: "Settling the Hadamard multiplier question: it is FALSE as posed, and SlotSFC_1(2) is squarefreeness of an Appell sequence"
status: >
  PROVED + VERIFIED-EXACT / AWAITING INDEPENDENT HOSTILE AUDIT. THM-3020 (K6)
  reduced SlotSFC_1(2) at p = 0 to: can a positive-coefficient A = sum a_j lam^j
  ...
  giving gcd(A,B) = gcd(A,A'): SlotSFC_1(2) at s = 1 IS squarefreeness.
  ...
  Polya-Schur / multiplier-sequence argument can prove SlotSFC_1(2).
  ...
  SlotSFC_1(2) at window k (p = 0)  <=>  Phi_{k+2} is SQUAREFREE.
  ...
  EITHER level fails, and SlotSFC_1(2) is irreducibly a statement about the SPECIFIC
  ...
  by Appell squarefreeness; SlotSFC_1(2) for s >= 2 remains OPEN in that form.
...
THM-3020 (K6) reduced `SlotSFC_1(2)` at `p = 0`, window `k`, to a resultant of two
...
SlotSFC_1(2) at window k   <=>   Res(A,B) != 0,   m = k+1.
```
...
`gcd(A,B) = gcd(A,A')`: SlotSFC_1(2) at `s = 1` *is* squarefreeness of `A`, which
...
increasing or to `{w_j}` being a multiplier sequence -- can prove SlotSFC_1(2).**
...
(m+1)Phi_m(z_0) = 0` is precisely the SlotSFC_1(2) failure condition. Hence

```text
SlotSFC_1(2) at window k, support {0,s}   <=>   Phi_{k+2} is SQUAREFREE.      (H5b)
```
...
specific measure `dnu = (1/s) u^{1/s-1} e^{-u^{1/s}} du`. SlotSFC_1(2) is a theorem
...
i.e. windows `k <= 24`, extending THM-3019's SlotSFC_1(2) box (`s <= 8`, `k <= 8`)
in both directions.
...
`Phi_n`. `SlotSFC_1(2)` for `s >= 2` remains **OPEN** in that form, with the
