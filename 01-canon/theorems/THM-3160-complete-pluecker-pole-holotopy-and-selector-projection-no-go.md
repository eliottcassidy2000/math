...
status: >
  ...
  but neither Hasse positivity nor a stochastic stopping law.
...
the full sidecar is positive or physically stochastic.
...
In any coefficient basis, if `K_M` is the matrix of `T_M`, then

```text
Omega^M=K_M Omega K_M^T.                                   (6)
```

Thus the full truncated exterior square is an exact linear sidecar for every
prefix edge.  Commutativity of the `T_M` makes `(6)` path-independent around
...
## 5. The honest two-parameter selector bifiltration
...
does not provide a probability law, a sequential stopping process, an
original-response decomposition, arbitrary-radial NC2, or the Gaussian
Moment Conjecture.
