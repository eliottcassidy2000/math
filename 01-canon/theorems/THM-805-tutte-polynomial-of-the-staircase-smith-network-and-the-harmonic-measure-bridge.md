...
status: PROVED (closed form; 5-line rank–nullity proof below) + REFEREED (closed form == brute-force subset expansion as polynomials, n=4..7; all specializations exact: κ=(n−2)!, forests=(n−1)!, acyclic 2^{n−2}, chromatic 3·2^{n−2} at q=3). THE HARMONIC LADDER (ii) — CORRECTED by the referee: the identity m({1..k};1/(k+2)) = H_k/C(k+2,2) is NOT universal; it holds EXACTLY iff n = k+2 is even with n ≢ 4 (mod 6) (verified exhaustively k=2..20: EQUAL at n ∈ {4,6,8,12,14,18,20}, differ at all odd n and at n ∈ {10,16,22}; ratios 0.46–0.82, no simple correction). **The LRC(14)-relevant case n=14 (≡ 2 mod 6) HOLDS**: m({1..12};1/14) = H₁₂/91 = 6617/194040 exact — the electrical identity behind mac-mini THM-736's deep-well base measure. The mod-6 characterization is a verified conjecture (HYP-6885). Sudler part (i) SHARPENED: the unit-circle zeros of T(N_n;1,·) are exactly the primitive q-th roots of unity for 2 ≤ q ≤ n−2 — i.e. the CLEAN-WITNESS DENOMINATOR GRID of klein-S296/THM-523 (t=j/q kills the core ⟺ T vanishes); good-set θ rank HIGH in |T| (away from the Sudler zeros), the danger topography is |T|'s valleys (float stats in HYP-6885)
