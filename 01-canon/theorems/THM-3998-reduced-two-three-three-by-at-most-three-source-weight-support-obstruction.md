...
status: >
  ...
  killed by an endpoint valuation. No such Darboux pair exists. This is an
  independent elementary proof, in the fixed node gauge, of a cell already
  excluded abstractly by THM-3974/3987; it does not enlarge the closed
  support region or settle the full reduced 2:3 case.
...
audit: >
  ...
  The audit also repaired a false novelty claim against THM-3974/3987 and
  identified hostiles showing that the simple negative jet, deleted-line
  boundary, and characteristic-zero hypotheses are essential. Normal and
  optimized outputs agree with the frozen output after LF normalization.
  Independent finite direct Groebner replays provide hostile controls but are
  not the all-degree proof.
depends_on:
...
script_sha256: cf0b8c1227ddd5db4a0cc3f26df6993c3365b7d8d1d7307b9b5491ff35fb7e2e
output_sha256: 8343196741f99780a34b87642252820bfb7eff04e277066e6a4dda6f43527d26
...
This cell is already excluded abstractly by THM-3974 and THM-3987. In
particular THM-3987 requires, beyond the weight `-2` row, a distinct surviving
even negative `A` tail of weight at most `-4`; it is absent here. The theorem
is retained because the fixed-node-gauge conic/ODE mechanism below is a new
independent explanation and a reusable hostile for wider cells. The apparent
three-versus-two/three raw gauge count should not be confused with the
canonical retained-support count of THM-3974/3987: the weight-zero row here
contains the nonzero boundary constant `a`, which their normalization deletes.
...
The independent hostile audit also checks why the hypotheses cannot be
dropped. If the forced simple jet is replaced by a double zero, then with
`H=u(1+u)` the choices `m=H^2`, `ell=H` at `r=-1` and `ell=H^3` at `r=-3`
satisfy the lowest-row equation `(10)`. If the deleted-line condition
`ell(0)=0` is removed, `f=ell=1` satisfies the highest-row equation `(11)`
for `r=5,7`. Positive characteristic can likewise erase the derivative
coefficients used at both endpoints.
...
The normal and optimized executions agree with the frozen outputs after LF
normalization. The first certificate ends with `ALL THM-3998 EXACT CHECKS
PASSED`; the second gives four independent direct-source Groebner hostiles and ends with
`ALL THM-3998 DIRECT GROEBNER REPLAYS PASSED`.

Not treated by this argument or certificate:
...
Several such support subcells are already closed elsewhere in canon. The
first support sizes not excluded by the general retained-support floors remain
`3x4` and `4x3`; nothing here closes either one.
