...
status: >
  ...
  length-count separator. Each of those nineteen holes is nevertheless filled
  in a power of the Hafnian support of degree at most twenty; this algebraic
  dilation does not implement a 13-adic depth transition. The unit-annulus law
  also eliminates
...
### 4.4. Every nonlinear hole has a bounded support-dilation pump

The identities (21o) have an additional exact consequence. For the nineteen
hole words in their displayed order, take

```text
(d_h)=(4,4,4,4,4,2,20,5,5,2,4,4,4,20,12,2,4,5,5). (21q)
```

Multiplying the identity for `h` by `d_h` writes the integer vector
`d_h h` as a sum of exactly `d_h` feasible matching profiles. Since every
coefficient of `P_S` is nonnegative,

```text
x^(d_h h) has positive coefficient in P_S^(d_h),
2<=d_h<=20.                                           (21r)
```

Thus the nineteen profiles are degree-one support holes but not persistent
holes in the semigroup generated by matching profiles. For example,

```text
2(12333)=11333+22333,
```

so `x^(2(12333))` occurs in `P_S^2` although `x^(12333)` is absent from
`P_S`.

This is a genuine bounded algebraic image pump, but not the reserved
all-depth pump. A product `P_S^d` places matchings on `d` independent copies
of the ten-vertex carrier. Multiplication by thirteen instead deletes and
inserts labelled endpoints on the same rooted fibre. Turning (21r) into a
depth transition therefore requires the cross-depth owner transport and
event word; the convex certificate alone supplies neither.

The split from Section 4.3 is now exact at every static power. Each of the
seventeen endpoint-separated holes violates one inequality in (21m).
Every monomial of `P_S^d` satisfies the corresponding right side multiplied
by `d`, so `x^(d h)` is absent from `P_S^d` for every `d>=1`. In contrast,
(21r) fills each of the other nineteen by `d<=20`.

There is also a finite-place shadow of THM-2022's whole-layer mechanism. For
every prime `p`, Frobenius gives

```text
P_S(x_1,...,x_6)^p
 =P_S(x_1^p,...,x_6^p)                 modulo p.      (21s)
```

Hence the coefficient of `x^(p h)` in `P_S^p` is divisible by `p` whenever
`h` is a degree-one hole, even if `p h` lies in ordinary Minkowski support.
For `h=12333`, the two ordered products of the witnesses `11333` and
`22333` already make the positive coefficient of `x^(2h)` even. This
separates ordinary support saturation from finite-place survival. It becomes
an LRC depth mechanism only if an actual depth-transition polynomial is
identified; no such identification is claimed here.
...
thirteen-speed relation space `Lambda(V)_Q` have dimension twelve. Consider
an injective lift
...
Such a lift always exists over `Q`; its bare existence contains no
arithmetic information. Write `w=hA`, choose indices `i,j_0` with
`V_i w_(j_0)!=0`, and choose eight distinct `i_l!=i`. If `j_l` runs through
the other eight scalar coordinates, put

```text
f_l=e_(j_l)-(w_(j_l)/w_(j_0))e_(j_0) in ker(w),
z_l=e_(i_l)-(V_(i_l)/V_i)e_i             in ker(V).  (27a)
```

The nine vectors `e_(j_0),f_1,...,f_8` form a basis of `Q^9`. Define

```text
T f_l=z_l,
T e_(j_0)=(w_(j_0)/V_i)e_i.                           (27b)
```

The `z_l` are independent in `ker(V)`, while the last image is not in
`ker(V)`. Thus `T` is injective and `VT=w`. The ratios in (27a)--(27b),
...
7. the exact split of those exclusions into `17` endpoint-potential
   shadows and `19` nonlinear holes in the matching/Hafnian support; and
8. a degree-at-most-twenty support-dilation pump for every nonlinear hole.
