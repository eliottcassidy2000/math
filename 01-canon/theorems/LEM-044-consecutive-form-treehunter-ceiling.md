---
id: LEM-044
title: THE CONSECUTIVE CLOSED FORM, THE c = 8 THEOREM, THE EDGE CAP, AND THE TREE-HUNTER CEILING (the wall program past c = 7). (A) THE CONSECUTIVE CLOSED FORM: μ(D_k ∩ D_{k+1}) = 1/49 + r(6−r)/(49k(k+1)), r = k mod 7 — the LEM-042 integer sum collapses to 49Σ − 14k(k+1) = 14r(6−r) exactly; zero excess iff r ∈ {0,6}, with maximum at r = 3; it specializes to 1/(7(k+1)) for k ≤ 6. Refereed exact k = 1..400. (B) THE c = 8 CONSECUTIVE THEOREM: every consecutive block [v..v+7] crosses the full-period wall — the seven adjacent starts cover every residue mod 7, so exactly the r = 0 and r = 6 credits equal 1/49 and the other five are strict. Hence the seven path credits sum to more than 7/49 = 1/7 and Hunter gives good > 0 for every v. Positivity is pointwise in v; there is no v-independent positive margin because the excess tends to zero. THE c = 9 BOUNDARY: the pure consecutive route needs excess > 6/49 and dies for ALL v (referee: no crossing v ≤ 100). (C) THE EDGE CAP: μ(D_a ∩ D_b) ≤ 1/14 for every distinct pair, equality iff reduced (1,2) — proof: each LEM-042 term ≤ 2a and there are ≤ (a+b)/7 + 1 terms, giving μ ≤ (a+b+7)/(49b) ≤ 1/14 outside the finite set a, b ≤ 4, checked directly; scan to 300 confirms uniqueness. (D) THE TREE-HUNTER AND ITS CEILING: Hunter's inequality holds for ANY spanning tree T (leaf-plucking induction; klein's kernel-pure Lean lemma is the path case, the tree case named): μ(∪A_i) ≤ Σμ − Σ_{T} μ(pairs) — refereed exact on 60 random family/tree instances. Crossing at block size c needs tree credits > (c−7)/7 = (2c−14)/14 while (C) caps them at (c−1)/14: the tree-hunter can cross ONLY for c ≤ 12 — AT c = 13 IT IS IMPOSSIBLE FOR EVERY TREE AND FAMILY (12/14 = 12/14, not strict). Feasibility is real: doubling families {3·2^i} cross at c = 9, 10, 12 with credits exactly (c−1)/14 (brute-verified against true good)
status: PROVED ((A) algebraic collapse, verified; (B) two equality residues and five strict residues per seven-edge window; (C) finite-check + termwise bound; (D) leaf-plucking + (C)) + REFEREED EXACT throughout
...
8-block, pointwise in its integer start), c = 9..12 fall per-family to tree-hunter credits
