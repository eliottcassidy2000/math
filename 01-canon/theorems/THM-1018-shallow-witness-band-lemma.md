...
status: (II) PROVED (elementary: the band containment is an implication with no hypotheses beyond 2 ≤ eᵢ, e_max ≤ 13 e_min, and an integer in the interval); (I) exact (residue arithmetic); **(III) NOW CLOSED — see THM-1032 (kind-pasteur cont.57): the divisor count is UNNECESSARY, because q = v₁ + max(P) is an explicit always-good modulus whenever max(P) ≤ 12·min(P) and the killer spread is ≤ max(P) − min(P), and both hold automatically on this stratum. The whole theorem is therefore unconditional here.** (The (III) text below is retained as the superseded route — its width-7 at-most-one-divisor mechanism is still correct, just no longer needed.)
