...
status: >
  ...
  Independently audited addendum: the positive monoid generated by the Rule
  30 states A,B is level-transitive on every finite binary-tree level, and the
  active images of each even ray are exactly the odd half.  Consequently the
  least literal uniform two-stage point-query bank for target rays (0,1,2),
  target depth D>=2 and gap budget B>=1 has exactly 2^(D+B-1)+B+2 points.
...
audit: >
  ...
  hostile.  A second, independent wreath-recursive audit verified the
  self-replicating root stabilizer, all tested forward-monoid orbits, and the
  universal static-bank formula.  Ordinary, optimized, and stored transcripts
  are byte-identical.
depends_on:
...
static_closure_script: 04-computation/rule30_universal_static_closure_thm3511.py
static_closure_output: 05-knowledge/results/rule30_universal_static_closure_thm3511.out
static_closure_script_sha256: 2f6121c536f5cc07d9567260693c0b099dd626dd9423df9b5d5547564ccbe049
static_closure_output_sha256: 5eb01f74c0be5ec7f448a4a07948a4b47839d4f872484841007a70f8b51b24f5
static_closure_semantic_sha256: 7b7b977eedf82a3798015e9e0d6be50f07ac0efd33b01bc6696b0be2a2a55a8d
static_closure_independent_script: 04-computation/rule30_universal_static_closure_independent_audit_thm3511.py
static_closure_independent_output: 05-knowledge/results/rule30_universal_static_closure_independent_audit_thm3511.out
static_closure_independent_script_sha256: 3cbb3732f6cac106b69edf206433c6515ee06cd54b64cedd6f37a0a361f110db
static_closure_independent_output_sha256: ea3b2112f1796862ae2896c9c222984b540445c1a87b3fc58780f7ccaadbf571
static_closure_independent_semantic_sha256: 2654e5dd189e0bc70eedc82fdc9a57b810af0122c47314606042e2d96296a613
...
## 9. Universal static two-stage closure

There is one all-depth consequence for the **ambient** finite-tree action.
Let `K=<A,B>`.  The state `B` swaps the two root children, while `A` fixes the
root and has left section `A|_0=A`.  The conjugate

```text
h=B^(-1) A B
```

also fixes the root.  In the right-action convention `(2)`, the recursion
`B=(C,B)sigma` gives `B^(-1)|_0=B^(-1)`, and hence

```text
h|_0=B^(-1) (A|_1) (B|_1)=B^(-1)BB=B.               (38)
```

Thus the section map `Stab_K(0)->K`, `g|->g|_0`, is onto: its image contains
both generators.  Root transitivity and `(38)` inductively make `K`
transitive on every finite level.  At a fixed level, inverses of `A,B` are
positive powers, so the positive `A,B` monoid has the same orbits as `K`.
Root activity is the parity of the number of `B` letters.  Therefore, for
every even depth-`H` ray `x`,

```text
{x^w : w is a positive active word in A,B}
  = {all odd depth-H rays}.                            (39)
```

Let `X` be any nonempty set of even depth-`H` inputs.  A literal uniform bank
that evaluates `pi^2(x)` for every `x in X` and every active ambient word
`pi` must contain `X` and, by `(39)`, the entire odd half.  Their union is
also sufficient, so it is the unique least bank under inclusion.

For target rays `(0,1,2)`, target depth `D>=2`, and positive gap budget `B`,
the possible first-stage inputs are

```text
X_B={0,2,4,...,2^(B+1)}.
```

They are `B+2` distinct even rays at source depth `D+B`.  Consequently the
least literal static two-stage bank is exactly

```text
S_(D,B)=X_B union {all odd depth-(D+B) rays},
|S_(D,B)|=2^(D+B-1)+B+2.                              (40)
```

The earlier `D=4`, `B=1,...,6` sizes
`19,36,69,134,263,520` are finite instances of `(40)`, not the basis of its
proof.  The mechanism is the self-replicating stabilizer `(38)`: a frozen
point bank pays for every possible routed address, whereas the adaptive
compiler waits for the realized address before its second query.

This statement ranges over all ambient active words and literal point banks.
It is not a lower bound for the physical signalizer path, and it does not
exclude symbolic compression or a richer Mealy-state observer.  It also stops
after the declared two-stage composition.  Thus it proves no bounded physical
gap, finite signalizer graph, center behavior, balance, density, prediction
bound, or other Rule 30 prize.

The primary addendum checker imports the byte-pinned action above, verifies
`(38)` through depth `16` in `131,068` section gates, and exhausts
`1,966,082` orbit-value gates.  An independent checker reconstructs `(1)`
without imports and verifies `105` starting-state orbits through depth `14`.
Both check `(40)`, a single-generator hostile, their semantic transcripts,
and normal/optimized determinism.
...
python3 -B 04-computation/rule30_universal_static_closure_thm3511.py
python3 -B -O 04-computation/rule30_universal_static_closure_thm3511.py
python3 -B 04-computation/rule30_universal_static_closure_independent_audit_thm3511.py
python3 -B -O 04-computation/rule30_universal_static_closure_independent_audit_thm3511.py
```

and compare each stream byte-for-byte with its stored output.  The companions
use no `assert` gates.
