...
status: >
  PROVED + VERIFIED-EXACT + INDEPENDENTLY HOSTILE-AUDITED AFTER MISTAKE-475
  COMPARISON-SCOPE REPAIR.
...
audit: >
  ...
  and documentation checks pass. A further audit (audit_mobius_3952) found
  that Section 5 had omitted the rational-source hypothesis and the centered
  trace-zero scope of the THM-3941 handoff. MISTAKE-475 repairs only that
  comparison; the four-color classification and exact certificate are unchanged.
depends_on:
...
Thus, among connected tame degree-three covers with rational source `P1`,
escaping the `C2^4` elliptic ledger requires creating an index-three inertia
point. Moving that point and its value to infinity is precisely the
rational-to-polynomial carrier transition. Within THM-3941's centered
trace-zero polynomial grammar, its carrier theorem then routes the resulting
support/order data. This comparison is a design map, not a construction of a
Keller atlas or a claim about positive-genus source covers.
