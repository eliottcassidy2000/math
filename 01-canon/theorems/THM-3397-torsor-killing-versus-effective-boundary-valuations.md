...
status: >
  ...
  (v^(mq)(1+cv)^(eq)).  Its poles at v=0 and the oriented X-boundary
  survive the polynomial cyclic cover even though that cover kills the
...
factor of `v` or `L` leaves a pole of order one.
