...
status: >
  ...
  hypothetical Darboux pair on the THM-3785 Russell
...
audit: >
  ...
  jc-cohn3709, 2026-08-23).  The proof derives the gradient syzygy from
  ...
  The audits rederived the unimodular syzygy without a hidden derivative
  case split; checked generic multiplicity one, the completed `z`-uniformizer,
  the scheme intersection `(z,G)`, and every analytic-branch conductor order;
  and confirmed that components of `D` disjoint from `L` remain allowed.
  ...
  optimized replay agree with the frozen transcript.
depends_on:
...
The syzygy module of the unimodular row `(p',q')` is free of rank one,
generated by `(q',-p')`.  Applying this to `(11)` gives `(6)`, including the
edge cases in which one derivative is zero.  (Those edge cases would in fact
make `gamma` injective and so cannot occur here.)  Uniqueness follows from
`(13)`, and `kappa` is nonzero because the irreducible characteristic-zero
curve `Gamma` is generically smooth.  Equation `(13)` also gives the ideal
identity
...
on `Y` is constructed, and `JC(2)` remains open.  **QED.**
