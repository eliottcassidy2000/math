---
id: THM-779
title: The r=8 token-walk blocking criterion at the prime-7 lens — full deck blocking is an integer-decidable walk condition; a universal exit bound remains open
status: PROVED (the criterion and its exact decision procedure; referee 4000-point token check + S301 cross-validation) + VERIFIED (finite adversarial censuses, superseded at height 10^4 by THM-785's working cap 6; the survivor's exact factorization) + OPEN (the unconditional surjectivity-exit bound)
...
owner switch in the schedule imposes a restrictive algebraic coincidence. The
census below suggests rapid decay of long runs, but no exponential bound is
proved.
...
## (4) The conditional pierce consequence

> **If a universal run bound `K0` is proved, any core-safe component containing more than K0 walls cannot be fully
...
> more than K0 walls. Empirically (S301 + this census) that is
> every family tested; HYP-6840's Q1 is resolved-negative in practice and Q2
> (the positive r=8 pierce) is supported on the tested rows. THM-785 later
> updates the working empirical cap to 6; neither value is universal.

## (5) What remains (honest)

- **The unconditional exit lemma -- ADVANCED by THM-785 (opus-S303):** the
