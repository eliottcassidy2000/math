...
> **LANDED IN LEAN (opus-S354): THM-1012 IS KERNEL-PURE.** TournamentH7.LRCPairIndependence, all four at [propext, Classical.choice, Quot.sound]: `consecutive_cells_ge` (THE GENERAL INDUCTION -- if every cell of a uniform partition contributes >= X to vol(S n .), then n consecutive cells contribute >= n*X; this SUBSUMES S353's aligned_count_ge and performs the OUTER sum over the a-arcs, so one lemma does both sums and no Finset-indexed disjointness family appears anywhere); `aligned_block_in_interval` (THE ALIGNMENT FINDER -- any interval of length >= (m+1)/b contains a half-cell-aligned block of m whole b-cells, via j = ceil(p*b - 1/2), using Int.le_ceil and Int.ceil_lt_add_one); `per_a_cell_ge` (inside one aligned a-cell the a-arc swallows an aligned block of m whole b-cells, each a full b-arc); and `pair_overlap_independence` -- THE THEOREM: over a unit window, vol(badArcs a lam n badArcs b lam) >= a * m * (2*lam/b) whenever (m+1)/b <= 2*lam/a. With m ~ 2*lam*b/a this is the INDEPENDENCE constant 4*lam^2 up to a defect linear in a/b, exactly as the paper proof claims -- and reached with NO sawtooth identity, only containment and counting. The `m` is left as a parameter with its hypothesis, which is the honest sharp form: the caller picks m = floor(2*lam*b/a) - 1.
