...
- **The ladder law (exhaustive through B ≤ 31 below the cap):** `diam ≤ B − 11`,
  tight at every level: (B, max diam) = (24, 13), (25, 14), (26, 15), (28, 17), (29, 18), (30, 19), (31, 20) — bitmask DFS (13 755 sets, 124.6M nodes at B ≤ 31).
  Equivalently **`B ≥ diam + 11 = diam + (k − 2)`** on this range.
- **Tail (PROVED):** any gap > D/2 splits `A` into blocks with disjoint sum ranges:
  `B ≥ (2s − 3) + 12 + (2(13 − s) − 3) = 32` (singleton blocks ≥ 33) — so B ≤ 31 sets
...
If `B(v) ≤ 31`, then by (b) `v ⊂ {a, a + d, …, a + 20d}` for some `a ≥ 1, d ≥ 1`
(AP-containment, 21 terms). Two arms:

- **d ≥ 2:** all speeds ≡ a (mod d) ⟹ the common-residue dispatch (a): `M(v) ≥ 8/17`.
- **d = 1:** `v ⊂ [v_min, v_min + 20]`; gapped gives `13·v_min < v_max ≤ v_min + 20`,
  so `12·v_min < 20`, forcing `v_min = 1` and `v ⊂ {1, …, 21}` — inside **window-22**
...
- **collision-light:** `B ≥ 32` proved (conj. ≥ 33) — the collision route to ruler
  death is capped; and
