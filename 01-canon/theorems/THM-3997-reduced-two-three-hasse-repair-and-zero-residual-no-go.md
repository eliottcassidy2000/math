---
id: THM-3997
title: "Hasse repair selects the nonliftable seam and excludes the zero-residual reduced 2:3 cell"
status: >
...
audit: >
  PASS (bounded_residual, conductor_incidence, and independent root audit,
  2026-08-24). The Laurent
  ...
  audited. The combined THM-3992 seam consequence and every displayed
  residual coefficient were rechecked exactly. One cancellation-unsafe
  hostile sentence was repaired. Normal and optimized outputs agree with the
  frozen output after LF normalization.
depends_on:
...
external: >
  ...
  Annals of Mathematics 88 (1968), 492-517, Section 1,
  https://annals.math.princeton.edu/1968/88-3/p05; imported only for the
...
closes the subcell `R=0`. Its first positive diagonal also removes THM-3992's
square-liftable lower-seam branch and, after the next oriented seam, forces a
nonzero first pure residual coefficient. It does not construct a pair, show that the free
...
Thus `[p^2]R` is not free. Viewed in isolation, before imposing THM-3992's
next seam, the formula may vanish on the resonant divisor
`9a^3+10gamma=0`; so `(18)` alone is not a universal nonvanishing obstruction.
...
### 3.2 Connection to the first seam: the liftable branch disappears

THM-3992 writes `eta=[s]q_0` and proves the exact first-seam dichotomy

```text
eta=0                         or                         eta=a/gamma.       (21a)
```

Equation `(21)` shows that every actual pair satisfying the source-normal
degree cap lies in the second branch. Thus the square-liftable `eta=0` branch
is a sharp lower-seam hostile, not a live branch after the first positive
source-normal diagonal is imposed. THM-3992 then forces the next oriented
seam

```text
a^3+2gamma=0,                 gamma=-a^3/2.              (21b)
```

Substitution in `(18)` gives the stronger combined residual statement

```text
[p^2]R=alpha=8/(3a^7) !=0.                              (21c)
```

Hence the resonant divisor `9a^3+10gamma=0` does not meet the live continued
cell, and `R=0` is already impossible at its first pure coefficient. This
uses THM-3992's next seam in addition to the present diagonal. The second
diagonal and the generic-fibre argument below remain independent checks that
do not rely on this shortcut.
...
1. **Sharp image hostile.** For `c!=0`, the isolated monomial
   `c*s^(m+1)p^m`, or its sum with any element of the image of `k[p,y]`
   whose `p^m` coefficient has degree at most `m`, is polynomial after
   `tau=p-s^2` but has `H_m` of degree `m+1`; it is not in the image. The
   restricted base class prevents cancellation of the displayed top term.
...
The normal and optimized outputs agree with the frozen output after LF
normalization and end with `ALL THM-3997 EXACT CHECKS PASSED`.
...
3. that `beta` is invariant under arbitrary target automorphisms outside the
   fixed normalization;
4. that an arbitrary nonzero residual is impossible, the depths decrease,
