...
status: (I) PROVED (grouping the singular series by exact support; E_s from the binomial partial-sum identity) + refereed exactly at n = 3 (errors 7.5e-9, 9.4e-9 = box truncation only); (II) battery-complete, zero middle; (III)(a),(b) PROVED. UPDATED by THM-2051: the coarse UNIVERSAL alternative is now proved by whole-product Fejer--BV approximation; after THM-965 pays pairs sharply, the explicit residual is a support-three-through-five relation of height at most 2^21. Thus the T_s(H), s=3,4,5, absolute lattice tails are no longer required for that dichotomy. They remain open if one wants termwise mass bounds or a substantially smaller horizon. The structured higher-relation branch remains open.
...
> support-three-through-five relation has all coefficient magnitudes at most
> `2^21`, then continuous `BONF5>0`; THM-965 pays the pair layer exactly. Thus the universal
