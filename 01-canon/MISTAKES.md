...
**What survives / correct framing:** route (c) STILL holds via the ACTUAL `ρ* = μ`, not the D3 proxy: `c = 0.878 < μ = 0.915` (dilation-invariant), and over 4691 hard low-L sets `c < μ` ALWAYS (min margin 0.081). So a good period EXISTS. The a-priori closure of route (c) needs an a-priori **`μ ≥ c`** (a genuine decorrelation lower bound `μ ≳ 0.9` holding even for 7-structured sets), which D3 does NOT deliver. So mac-mini-S62's "finite/verifiable D3 closure" has a hole (the 7-structured dilated family); the clean small-finite-check closure via D3 is INVALID. Existence is unaffected: LEM-010(ii) Dirichlet `j* ≤ 3^{k−1}` is the unconditional fallback, and LEM-012 covers structured `L ≥ k−6`.
