...
(\textbf{BIBD}) for all Paley primes; this follows from the 2-transitive action of $\mathrm{Aff}(\mathrm{QR})$
and is classical in design theory, though the clean algebraic proof in the tournament setting is our work.
...
Modular rank over $\mathbb{F}_p$ is known in sparse linear algebra; our contribution is
packaging it for topological use: reduce mod a small prime $p < 256$, store as \texttt{uint8},
then certify at a second independent prime.
...
Tang and Yau (arXiv:2602.04140, Feb 2026) introduced a ``symbol matrix'' framework
for circulant digraph path homology and proved stability at generic $t$.
\textbf{THM-125} (our work, March 2026): we proved the \emph{stronger} result
that the symbol matrix is \emph{identically constant} (independent of $t$) — an
elementary two-line algebraic argument.
This implies all $n$ eigenspaces have \textbf{identical} boundary ranks.
Therefore: compute one rank, multiply by $n$.
