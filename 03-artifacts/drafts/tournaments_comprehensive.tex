...
approximately 12{,}000 new terms, including tournaments (A000568, $n=76 \to 200$),
...
\midrule
A000568 & Tournaments & 76 & 200+ & 120+ \\
A000273 & Digraphs & 64 & 101 & 37 \\
...
exhaustively through $n = 8$ ($2^{27}$ configurations). Grinberg and Stanley
...
At $n = 9$, this gives a $100\times$ speedup: 0.7ms per tournament versus 70ms
for Held--Karp.
...
\begin{theorem}[THM-069, Walsh diagonalization]\label{thm:walshH}
...
$\beta_1(T) \cdot \beta_3(T) = 0$ for all tournaments on $n \leq 7$.
\end{theorem}
...
\begin{openquestion}
Prove $\beta_1(T) \cdot \beta_3(T) = 0$ for all $n$ (currently proved through $n = 7$).
\end{openquestion}
...
\midrule
OCF: $H = I(\Omega, 2)$ & Exhaustive & $n \leq 8$ ($2^{27}$) & 0 failures \\
