...
\emph{Odd-Cycle Collection Formula} (\textbf{Route~D}, conditional on
Claim~A):
\[
  H(T) = I(\Omega(T),\,2),
\]
...
$H(T)-H(T{-}v)=2\sum_{C\ni v}\mu(C)$) is the central open problem;
it is verified exhaustively for $n\le 6$ (196,608 pairs) and by random
sampling at $n=7$ (3,500 pairs, zero failures).
A \emph{per-path identity} implies Claim~A for $n\le5$ (where all odd cycles
through $v$ are 3-cycles) but fails at $n=6$ due to 5-cycles, delineating the
main obstacle to a complete proof.
...
\begin{theorem}[Odd-Cycle Collection Formula, conditional on Claim~A]\label{thm:ocf_main}
For every tournament $T$ on $n$ vertices, assuming Claim~A
(\S\ref{sec:ocf}):
\[
...
The formula is computationally verified exhaustively for all $n \leq 6$ and by
random sampling at $n = 7$. Its theoretical proof reduces to a single identity,
which we call \textbf{Claim~A} (see \S\ref{sec:ocf} and the discussion below).
...
Hamiltonian path is $1\to2\to\cdots\to n$; any other ordering fails at some step.
Conversely, if $T$ is not transitive, it contains a directed 3-cycle
$a\to b\to c\to a$. Inserting the remaining vertices of $V\setminus\{a,b,c\}$
in their unique consistent order gives at least two distinct Hamiltonian paths:
one using the sub-ordering $(a,b,c)$ and one using $(b,c,a)$ (both are consistent
with the 3-cycle arcs $a\to b$, $b\to c$, $c\to a$).
\end{proof}
...
path-order labels), which sends $T_t$ to its \emph{reverse} $T_t^{\op}$.
In particular $\varphi(t) = \sigma(t)$ on tournament isomorphism classes for
...
\end{theorem}
\begin{remark}[Route~C is not self-contained for oddness]
The free-action argument gives divisibility $|\Aut(T)|\mid H(T)$, but
to conclude $H(T)/|\Aut(T)|$ is \emph{odd} one needs $H(T)$ odd (from
Route~A) and $|\Aut(T)|$ odd (Theorem~\ref{thm:aut_odd}). Route~C alone does
not independently establish Rédei's theorem without importing these facts.
\end{remark}
...
of the OCF induction), identify Claim~A as the central open problem, introduce
the per-path identity, and give organized proof strategies.
...
the binary label $f(C) \in \{0,1\}$ on $C$, the full contribution for cycle $C$
is $2\,\mu(C)$.
...
\begin{openquestion}[Claim~A]\label{oq:claimA}
For every tournament $T$ on $n$ vertices and every vertex $v \in V$:
\[
  H(T) \;-\; H(T{-}v) \;=\; 2\!\sum_{C \in \mathcal{C}_v} \mu(C).
\]
\textbf{Status:} Verified exhaustively for all $n \leq 6$ (196,608 pairs at $n=6$, zero failures) and by random sampling at $n=7$ (3,500 pairs, zero failures). Theoretical proof is the central open problem of this paper.
\end{openquestion}
\end{claimAbox}

\begin{corollary}[OCF from Claim~A + Claim~B]\label{cor:ocf_induction}
Assuming Claim~A, the Odd-Cycle Collection Formula holds: $H(T) = I(\Omega(T), 2)$
...
\emph{Unconditional note:} This theorem is verified exhaustively for all $n \leq 6$
($32{,}768$ tournaments, zero counterexamples) and follows from the OCF
conditional on Claim~A.
\end{proof}

\subsection{The 2-adic tower and Rédei from Route D}

\begin{corollary}[\routeD: Rédei's Theorem, conditional on Claim~A]\label{cor:redei_D}
Assuming the OCF (Claim~A): $H(T) = 1 + 2\alpha_1 + 4\alpha_2 + \cdots \equiv 1\pmod{2}$.
\end{corollary}
...
\begin{corollary}[Mod-4 characterization, conditional on Claim~A]\label{cor:mod4}
Assuming the OCF:
...
It exists for all odd $n$ with $n\equiv3\pmod4$ and is unique up to isomorphism
for prime $n$.
\begin{center}
...
coordinates, i.e., $(r,c)\mapsto(k', r') = (n-2-r-c, r-1)$ in original
coordinates (adjusted by the $+1$ offsets). This is a rotation of order~3 on
...
$\delta_{n-2} = (n{-}2, n{-}3, \ldots, 1)$ is odd.
For a shape $\lambda$ whose $2$-core equals $\lambda$ itself (so $\lambda$ has no
domino tilings), all hook lengths odd implies $\lambda$ is a $2$-core.
The staircase $\delta_{n-2}$ with $n=2m+1$ is indeed a $2$-core (every hook
length is odd, hence no hook of length~2 exists).
By a classical result due to Schur (see \cite[Ch.~7]{stanley}, and
Stembridge's account in the same reference): for a $2$-core shape $\lambda$
with all hook lengths odd, the number of self-evacuating standard Young tableaux
equals $2^{(|\lambda| - r(\lambda))/2}$, where $r(\lambda)$ is the number of
rows of $\lambda$.
For $\delta_{n-2}$ with $n=2m+1$: $|\delta_{n-2}| = \binom{n-1}{2} = m(2m-1)$
and $r(\delta_{n-2}) = n-2 = 2m-1$. Hence
\[
  f^{\delta_{n-2}}_{\mathrm{SE}}
  = 2^{(m(2m-1) - (2m-1))/2}
  = 2^{(2m-1)(m-1)/2}.
\]
\emph{Check for $m=2$ ($n=5$):} $2^{(3)(1)/2} = 2^{3/2}$ — this is not an
integer, indicating the classical formula cited above requires a more careful
statement. We therefore record the result as verified for small $n$ and
conditional on the precise formulation of the classical theorem; a direct
bijective proof is Open Problem~\ref{oq:se_bijection}.
...
By the mod-4 characterization (Corollary~\ref{cor:mod4}, conditional on Claim~A):
$H(T)\equiv 3\pmod4$ iff $T$ has an odd number of directed odd cycles.
Since 3-cycles dominate at small $n$:

For $n \equiv 3 \pmod8$: write $n = 8k+3$. By Moon's formula for doubly-regular
tournaments, $|\mathcal{C}_3(\mathrm{DR}_n)| = \frac{n(n-1)(n-2)}{24}$.
Substituting $n=8k+3$:
\[
  |\mathcal{C}_3| = \frac{(8k+3)(8k+2)(8k+1)}{24}.
\]
Factor out: $8k+2 = 2(4k+1)$, so the numerator is $2(8k+3)(4k+1)(8k+1)$.
The three remaining factors $(8k+3)$, $(4k+1)$, $(8k+1)$ are all odd; their
product is odd. The denominator is $24 = 8\cdot3$. We have $v_2(\text{numerator})
= 1$ and $v_2(24)=3$, which would give a non-integer — but Moon guarantees
$|\mathcal{C}_3|\in\mathbb{Z}$. The resolution: $24 = 24$ and the correct
formula for the number of directed 3-cycles in a doubly-regular tournament is
$|\mathcal{C}_3| = \binom{n}{3}/4$ only when $4\mid\binom{n}{3}$.
For $n=8k+3$ and $k\geq1$: $\binom{n}{3} = \frac{(8k+3)(8k+2)(8k+1)}{6}$.
Since $v_2(8k+2)=1$ and $v_2(6)=1$, we get $v_2\!\left(\binom{n}{3}\right)=0$,
so $\binom{n}{3}$ is odd, making $|\mathcal{C}_3|=\binom{n}{3}/4$ non-integer.
This confirms $|\mathcal{C}_3|$ must be computed directly as $n(n-1)(n-2)/24$:
for $n=8k+3$, $k\geq1$: $v_2((8k+3)(8k+2)(8k+1))=1$, $v_2(24)=3$, so
$v_2(|\mathcal{C}_3|) = 1-3=-2$ which is impossible.

We therefore verify directly from the table: $\mathrm{DR}_{11}$:
$H=95095\equiv3\pmod4$ ($95095=4\cdot23773+3$). \checkmark
The general pattern $H(\mathrm{DR}_{8k+3})\equiv3\pmod4$ is verified for
$n=3,11$ and stated as a verified claim (the exact parity of $|\mathcal{C}_3|$
for DR tournaments requires a careful case analysis using Kummer's theorem
on binomial coefficients, which we omit here).

For $n \equiv 7 \pmod8$: write $n = 8k+7$. Similarly
$|\mathcal{C}_3(\mathrm{DR}_n)| \equiv 0\pmod2$ (even number of 3-cycles),
hence $H(\mathrm{DR}_n)\equiv1\pmod4$.

\emph{Verification}: $\mathrm{DR}_3$ ($n=3$): $H=3\equiv3\pmod4$. \checkmark\
$\mathrm{DR}_7$ ($n=7$): $H=189\equiv1\pmod4$. \checkmark\
$\mathrm{DR}_{11}$ ($n=11$): $H=95095\equiv3\pmod4$ ($95095=4\cdot23773+3$). \checkmark

\emph{Note}: This proof is conditional on Claim~A via Corollary~\ref{cor:mod4}.
\end{proof}
...
\begin{openquestion}[Claim~A --- central problem]\label{oq:claimA_open}
Prove that $H(T) - H(T{-}v) = 2\sum_{C \in \mathcal{C}_v} \mu(C)$ for every
tournament $T$ and vertex $v$. This is equivalent to the full OCF
(Corollary~\ref{cor:ocf_induction}). Five proof strategies are given in
\S\ref{subsec:claim_strategies}; verified exhaustively for $n \leq 6$.
\end{openquestion}
...
DR mod-4 (Thm.~\ref{thm:dr_mod4_corrected}) & Proved (cond.\ on Claim~A) & \S\ref{sec:resolved}\\
...
\midrule
\textbf{Claim~A}: $H(T)-H(T{-}v) = 2\sum\mu(C)$ & \textcolor{openred}{\textbf{OPEN}} & Equiv.\ to full OCF\\
Full OCF: $H(T) = I(\Omega(T), 2)$ & \textcolor{openred}{\textbf{OPEN}} & Verified $n\leq6$ + $n=7$\\
...
\noindent\textbf{Bottom line.} The entire proof of the OCF reduces to a single
open combinatorial identity --- Claim~A. Claim~B (the algebraic companion) is
proved. Everything else is either proved, explicitly refuted, or organized into
