...
## 6. The Erdős–Szekeres tournament form of the finite theorem

Through the THM-453 A dictionary, Q(n,t) UNSAT (t ≥ R(n,2)) reads:

> **Every tournament on the lex-ordered grid [t]^n that contains no order-reversed
> transitive triple contains an order-faithful transitive binary subgrid.**

R(2,2) = 5 is the exact threshold at n = 2: monotone-substructure forcing à la
Erdős–Szekeres, with Specker's ω² theorem as its transfinite limit. (Cross-thread:
kind-pasteur's concurrent Erdős–Moser sessions study trans(T) unaligned; this is the
order-aligned variant, where the grid structure, not just size, is forced.)

## 7. The tower miniature and the Chang shadow (THM-460)

Full-type subsets of ω^(ω^m) are characterized by STACKED TOWERS (THM-460 B2: the
tail/indecomposability argument), with binary miniatures generated by a recursive
shape grammar (B3): pairs at finite scales, M-towers at limit scales, trailing CNF
terms outermost. König bridge (C1): Q^(m)(M; s,C) SAT for all (s,C) at fixed M would
give ω^(ω^m) ↛ (ω^(ω^m),3)². Hence:
* m=1: **Chang's theorem forces finite tower-Ramsey cutoffs** — the "Chang numbers"
  computed this session (results table in THM-460 D).
* m=2: Schipperus's ω^(ω²) theorem forces the same one rank up.
* m=3: the SAT/UNSAT behaviour of Q^(3) is a computational probe of the OPEN $1000
  case; m=4: persistence possible (Schipperus's negative side).
The m ≥ 2 instances need the general-shape enumerator (next session); the m=1
machinery (structural tower finder, cross-validated against brute force) ran today.

## 8. The relation-Schur reading of the invariant cutoffs

THM-453 F1's invariant witnesses are graded families {B_g} with
B_{g1}∘B_{g2} ∩ B_{g1+g2} = ∅ (composition-freeness over (ℕ,+)) under
rectangle-density floors. The invariant cutoff (least t where no such family exists
on [t]-columns) is therefore a **Schur-type number for graded relations** — the
relation-valued analogue of the repo's cauldron boils (S618-621) and Sidon/B_h
rungs (THM-446). The session-1 (buggy-verifier) invariant models, though their
certificates were invalid, consistently showed B_g structure depending on g mod 2 —
a dyadic fingerprint consonant with the repo's 2-adic seam (Erdős 64 thread); the
corrected runs test whether it survives exact verification.
