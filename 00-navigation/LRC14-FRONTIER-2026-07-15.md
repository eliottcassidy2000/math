...
| fixed `(13d,5d)` symmetric radius budget | PROVED ALL-SIZE for compact sets + FINITE-EXACT replay: the folded target is exactly the two radius-`2/169` cells about `5/13,8/13`. For nonempty compact `E` and compact symmetric `R=-R` with `0 in R`, full-union containment is equivalent to `rho_C(E)+rho_0(R)<=2/169`; after the common dilate this is `max_E ||13dt||+13 max_R ||dr||<=2/13`. The proof includes an exact no-switch lemma. Evaluation is linear in the component/cell lists, but individual nonsymmetric satellites do not factor this way and endpoint owners remain necessary for transport. All 214 replayed THM-821 cores fail the global budget, so this is a compression theorem, not branch closure. | THM-824 + exact radius replay |
...
For `(13d,5d)` this becomes
`max_E ||13dt||+13 max_R ||dr||<=2/13`.  It evaluates from the two exact
