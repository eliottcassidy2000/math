...
| scale-one Hamming-five finite reduction | PROVED REDUCTION / FINITE-EXACT SLICE: THM-820 alone gives the recursive box `(x,v,w,y,z)<=(388,776,1552,3104,6208)` or an exceptional `a{1,2,4,8}` top-four SCC with `x<=228`, `v<=1986`, `max_top<=7944`. Intersecting with THM-815's independent `x<=146` component bound sharpens the actual boxes to `(146,292,584,1168,2336)` and `x<=146,v<=1986,max_top<=7944`. All 792 height-one rows are loose, uniquely minimized by `2[11] union {11}` at `1/12`; the next odd lift reaches the doubled AP `2[12]` exactly at radius six. Five-cycle bands have five feasible types, and identical tournament shadows can carry different integer centres and maxima. The two finite boxes and arbitrary-scale radius-five descent remain OPEN. | THM-815/820 + exact replays |
...
| signed return-cell / deep-component stalk purity | PROVED for the fixed folded pair `(13,5)` + FINITE-EXACT: the atomic predicate factors through the exact circular sum arc `C+R_k`. Across 9,974 reconstructed atoms, 492 succeed and 9,482 fail. Cell sign, return interval/owners, deep interval/owners, widths, and coarse selector event all have explicit mixed fibres; disconnected satellites make even an exact deep component insufficient. Exact input intervals or the exact sum arc have zero mixed fibres. Endpoint owners are numerically forgettable only after endpoints are fixed and remain necessary ancestry under descent. This is not an all-size/global separator or a Cech-gluing theorem. | THM-821 + exact stalk replay |
| fixed `(13d,5d)` symmetric radius budget | PROVED ALL-SIZE for compact sets + FINITE-EXACT replay: the folded target is exactly the two radius-`2/169` cells about `5/13,8/13`. For nonempty compact `E` and compact symmetric `R=-R` with `0 in R`, full-union containment is equivalent to `rho_C(E)+rho_0(R)<=2/169`; after the common dilate the intrinsic criterion is `rho_(C,d)(E)+rho_d(R)<=2/169`. The phase inequality `max_E ||13dt||+13 max_R ||dr||<=2/13` is equivalent only together with `dE subset H` and alone is merely a necessary thickness tax (`E=R={0}` is the guardrail counterexample). The proof includes an exact no-switch lemma. Evaluation is linear in the component/cell lists, but individual nonsymmetric satellites do not factor this way and endpoint owners remain necessary for transport. All 214 replayed THM-821 cores fail the intrinsic global budget, so this is a compression theorem, not branch closure. | THM-824 + corrected exact radius replay |
...
| n=12 sporadic branch | OPEN globally. Closed: the bounded shallow slice; the full AP-centred Hamming-one through Hamming-four stars at arbitrary height and scale; the complete height-at-most-two scale-one Hamming-five slice; every two-sheet core in `[1,19]` with unbounded odd exceptions; the full two-sheet speed box through 100; every non-signed-complement `q=13` profile; and the forced-`w=13c` quotient box through height 24. THM-820 makes scale-one Hamming five uniformly finite in two explicit boxes but does not empty them or descend arbitrary AP scale. THM-822 shows why the bounded kernel extremes do not finish it: live-relation fibres mix exact `M`, while literal interval faces are statically injective but unproved under continuation. Scale-one Hamming six is recursively finite, and radius seven is the first chart where this discrepancy argument gives no initial cap. THM-823 reduces the all-scale deck interface to a least-order pivot, refutes an all-order scalar cutoff, and classifies the bounded no-order-one common-sheet survivors as one order-three forward-coset orbit; arbitrary-lift strictness and common sheets above order twelve remain open. THM-817 classifies every return satellite and gives the adaptive exact selector; THM-821 factors every odd-pair atomic stalk exactly and proves the audited coarse projections impure. THM-824 then proves that the full symmetric union for `(13d,5d)` is governed exactly by one two-radius budget, but all 214 replayed cores violate it and the theorem does not cover individual satellites or other odd ratios. Remaining: exact closure of the radius-five boxes, exhaustion of the finite H6 recursion, and unbounded deck languages; for every admissible `(U,x,y)`, a uniform violation/decreasing rule for the radius or exact signed-sum-arc selectors, or an incompatibility/descent; extension beyond the fixed ratio; unbounded dyadic/collar assembly; and general higher-sheet packets. | THM-759/763/765/766/768/769/770/772/774/775/776/782/786/789/792/795/797/800/803/804/806/807/810/815/816/817/820/821/822/823/824; HYP-6820 |
...
arbitrary height.  Thus the remaining deck problem is common-sheet and metric,
not scalar capacity; unbounded orders and strictness of the surviving flags
remain open.
...
necessary thickness tax.  The intrinsic radius criterion evaluates from the two exact
extrema in time linear in the stored components and return cells.  This does
...
either a uniform arithmetic violation of this budget, a controlled extension
to other odd ratios, or a genuinely satellite-level obstruction where the
symmetry collapse is unavailable.
...
> **LRC(14) is open.  The AP-centred Hamming-four star is closed and
> scale-one Hamming five/six is finite; the live obstruction is uniform
...
THM-768--770, THM-794/795/797/800--804/806/807/810/815--817/820--824,
