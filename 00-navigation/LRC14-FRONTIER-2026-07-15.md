...
prefix--comb--comb third moment.  The rank-six form closes every fixed
pairwise-coprime common-scale seven-comb ray once the common scale is explicit
and sufficiently large.  Exact replay of six pilots finds connected positive-credit graphs in
all four Hunter successes, but all 21 endpoint and total edge defects negative
in both consecutive failures despite mostly positive sawtooth terms.  The
remaining state is the incoherent, prefix-coupled edge field.  The four-far
...
| Hunter-tree second-order seven-comb functional | PROVED functional/ideal coefficient + coherent-ray closure + FINITE-EXACT pilots, not full radius-seven closure: single-comb density bounds are information-theoretically empty at seven, while Hunter--Kounias gives ideal coefficient `(165-22m)/169`, positive at `m=7` and negative at `m=8`. The live-pull referee corrected the global pair law to the two-sided sawtooth `4/169+[Q(a+b)-Q(|a-b|)]/(169ab)` for the reduced pair `(a,b)`; in particular `mu(D_6 intersect D_7)=2/91<4/169`, so the original one-sided Bezout floor and raw-speed deficit lemma are false. For a `c_E`-component prefix, the projective pullback error at common scale `g` is at most `2c_E/g`. The exact anomaly is `(165-22m)e/169-sum s_i+MST(c)`. Its node/edge colours form the PSD restricted covariance `G^E_ij=c_ij-(2/13)(s_i+s_j)` and its irreducible endpoint field is a third centered moment; the sawtooth numerator is a rank-six reflection-odd PSD kernel. Deletion--contraction, partition insertion, and the graphic-rank derivative are exact recursive classifiers. The sharp scalar projective floor exactly cancels but cannot cross the ideal seven-comb margin. For seven pairwise-coprime quotients at common scale `g`, `L_H>=(4363/382200)e-26c_E/g`, so every fixed coherent ray closes beyond an explicit scale. Six exact packets verify the full decomposition: the four successes have positive-credit spanning trees, while both consecutive failures have all 21 endpoint/total credits negative despite mostly positive projective terms. Remaining: a uniform maximum-tree lemma on incoherent prefix-coupled edge packets and an AP-window argument for correlated residuals. | corrected THM-856 + exact overlap and node/edge replays |
...
3. **Classify higher deck ramification, finish Hamming six,
   ...
   languages. Classify effective orders above twelve. THM-815 C.1 now proves
   that a nonprimitive radius-six row is either the genuine `2[12]` equality
   orbit or loose.  THM-815 C.2 then closes fourteen of the twenty
   three-antipodal-pair primitive-core rows by oriented AP-germ handoffs and
   ten fixed-coordinate trees.  Continue through the six product-`1/16` rows,
   the 903 `f<=2` rows, and the exceptional odd-label mixed-parity branch;
