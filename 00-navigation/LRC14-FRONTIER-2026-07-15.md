...
portable exact lane closes flood edge `(5,7)` uniformly: all 28,847 residual
bottom sweeps are positive, with minimum `7/858`.  Thus one of the 21 flood
bodies is closed and twenty remain.  They retain an eight-dimensional edge
...
the 2002-body theorem.  Its edge-`(5,7)` addendum is an exact proved bank, not an unconditional
assembly lemma for the other twenty floods.
...
| Hamming-six exact closure / ramification boundary / seven-comb wall | PROVED / FINITE-EXACT, UNIFORM IN LIFT HEIGHT: THM-857 applies THM-815's longest-component cap at every labelled prefix over all `C(12,6)=924` deletion roots and every unbounded proper lift progression. Its logical tree has `580,919,164` nodes and `580,918,240` candidate edges; twenty terminals are certified loose and the unique covering terminal is the doubled AP `2[12]`. THM-859 proves that common dilation conjugates this complete action tree, so `2c[12]` is the unique tight row whenever all six effective orders are one, and identifies order `D` as the exact deck-mask ramification degree. THM-860 now makes the entire primitive proper AP-centred H6 ramified branch finite-decidable: `lcm(D_j:j!=i)=c`, the optimized valuation ranges are `(5,3,1,1)`, the all-six cut gives `min D_i<=72` and hence `c<=2,177,280`, and every fixed order/unit language has a finite exact component tree with at most 37 first heights per labelled ray. The `c=2` face has 64 signed-cycle contexts, but their zero-cover verdict is not yet proved. Seven remaining combs are still the precise first-moment density barrier because `13-2m` changes sign. | THM-815 Part C/C.1/C.2, THM-857/859/860 + exact primary/closed-danger/ramification replays |
...
| n=12 sporadic branch | OPEN globally. Closed: the bounded shallow slice; the full AP-centred Hamming-one through Hamming-four stars at arbitrary height and scale; the proper scale-one Hamming-five chart; every common-sheet H5 survivor language with all effective orders at most 21; the **entire proper scale-one Hamming-six chart and all of its common dilations except the genuine equality family `2c[12]`**; every two-sheet core in `[1,19]` with unbounded odd exceptions; and the other named finite banks. THM-858 proves that the entire remaining common-sheet H5 effective-order bank is finite, with `min D_i<=21<max D_i<=10,584`, all orders `{2,3,7}`-smooth, and no private prime power. THM-857/859 remove the former scale-one residual and identify `D>1` as the exact H6 ramification obstruction; every branch containing `D=1` is now finite-decidable, but its first `336` and `672` ramified context languages have not yet been evaluated. Exact endpoints are Markov only for monotone insertion. On the deep side THM-824/831 classify the radius/switch scalar, while THM-836 excludes shells `s=1,3`, uniformly eliminates `d=11 mod 52` at `s=5`, and proves that no `U`-independent single unit numerator on `q=5d` or `13d` can close the other three classes. The classes `15,37,41 mod 52` remain open and now require a lift-dependent, multicolumn, or non-endpoint certificate. | THM-759/763/765/766/768/769/770/772/774/775/776/782/786/789/792/795/797/800/803/804/806/807/810/815/816/817/820/821/822/823/824/831/836/837/840/844/845/847/857/858/859; HYP-6820 |
...
AP-centred ramified H6 language finite-decidable, with `c<=2,177,280`; metric
evaluation of that finite bank, radius at least seven, the two-sheet selector,
...
window residual. The finite smooth-ramified H5 presentation bank, arbitrary-
ramified common-sheet H6 languages, deep two-sheet, and general higher-sheet
branches remain.
