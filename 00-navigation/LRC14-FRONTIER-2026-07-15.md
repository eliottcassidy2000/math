...
inequality.  The sawtooth numerator is a positive-semidefinite rank-six kernel
on the reflection-odd residue sector.  More generally, the node anomalies and
edge credits complete to the restricted covariance Gram
`G^E_ij=c_ij-(2/13)(s_i+s_j)`; the irreducible endpoint field is a centered
prefix--comb--comb third moment.  A complete ten-ratio threshold table makes
the bad projective-edge graph triangle-free and proves
`MST(h)>=-237/7436`, leaving the strict projective margin `19/572` for every
seven-speed packet.  Consequently every common-dilate seven-comb ray closes
beyond `L_H>=19e/572-(1203/70)c_E/g`, without a pairwise-coprime hypothesis.  Exact
...
| fixed-ratio thin owner shells | PROVED SHARP FOR THIS LOCAL STEP + FINITE-EXACT: for ten distinct speeds with THM-836's exact signed-complement residues and guarded `(13d,5d)` containment, the two directional exit coefficients are both permutations of `(9,20,31,42,53,64,75,86,97,108)`. In the band `B<13d`, the two coefficient-nine owners force `delta(d)<=11s(13d+s)/(2(117d+11s))`, `s=2B-13d`. Modular alignment empties `s=1,3`. At `s=5`, local feasibility is exactly `d mod 52 in {11,15,37,41}`, with omitted signed class `{3,10}` and owners `{B-3,B-2}`. The explicit grid `q=5d`, `p=(45d-1)/26` uniformly eliminates `d=11 mod 52`; a 1,605,632-row exact census eliminates the single value `d=15`, but not its full congruence class. The classes `15,37,41 mod 52` remain open uniformly. | THM-836 + exact shell/divisor-grid replays |
| shell-five universal endpoint-grid template | PROVED NO-GO FOR THIS METHOD: in each open class `d mod 52 in {15,37,41}`, the points deep for every possible free lift and the three forced owners lie in four exact intervals around `4/13` and `9/26`. Neither mandatory endpoint grid `q=5d` nor `q=13d` has a unit point there; the `q=13d` half also has a direct inverse construction showing that every proposed unit column is killed by an allowed raw-class-`1` or `12` lift. Thus the U-independent single-numerator analogue of the `d=11` proof is uniformly impossible. This closes no class: a witness may depend on the actual lift set, combine columns, or use another denominator. | THM-836 §6B + exact skeleton/grid replay |
| shell-five fixed two-column endpoint family | PROVED NO-GO IN THE RELAXED MODEL: every unit `q=13d` column has free danger support at least three; every nonforced unit `q=5d` column has support at least two except `d=41 mod 52`, `p=+/-(45d+1)/26`, whose support is exactly `{4}`. A labelled matching argument therefore jointly kills any fixed pair of unit endpoint columns by one relaxed shell-admissible lift set. The 432-row all-size affine certificate has minimum strict margin `1/6545`. This does not impose THM-772 divisor completeness or THM-803 parity support, so it closes no structured shell class. | THM-836 §6C + exact support/matching replay |
...
| Hamming-five arbitrary-scale deck boundary | PROVED STRUCTURAL / UNIFORMLY FINITE COMMON-SHEET ORDER BANK: every scalar cover has `sum 1/D_i>3/13`, hence some `D_i<=21`, but the family `(1,2,3,5,10):(2,5,13q+1,2,13q+1)` refutes any scalar all-order cutoff. THM-858 adds the relative-ramification cuts `sum_(i in S) ceil(2m_i/13)/m_i>=1`, `m_i=D_i/gcd(D_i,lcm(S^c))`. In the new branch with some `13<D_i<=21`, singleton cuts reduce 2,055 scalar rows to 185 and top-prime cuts reject all 185; a literal replay independently finds zero covers among 51,360 unit words. Applying the same cuts across adjacent valuation levels and resolving the dyadic equality cases gives `range(v_2,v_3,v_7)<=(3,2,1)`, so every common-sheet order is at most `21*2^3*3^2*7=10,584`. Any unclassified row lies in `min D_i<=21<max D_i<=10,584`, is `{2,3,7}`-smooth, has no private prime power, and obeys every relative cut. | THM-816/823/837/844/845/847/858 + exact C++/Fraction replays |
...
| Hamming-six exact closure / ramification boundary / seven-comb wall | PROVED / FINITE-EXACT through AP-centred common scale `c=17`; `c=13` is primitive-impossible. THM-857 closes scale one and its unbounded lifts; THM-859 proves exact dilation conjugacy and identifies the effective order `D`; THM-860 proves the primitive proper AP-centred H6 branch finite with `c<=840`, hereditary leave-one-out lcm, all 63 relative cuts, and a finite metric tree per language. THM-861 closes `c=2`. THM-862 closes all 1,504 `c=3` contexts by a 950,540,566-edge terminal recursion. THM-957 closes `c=4`; THM-958 empties 14,414,400 `c=5` contexts; THM-960 empties 37,710,288 `c=6` contexts; THM-962 empties 108,673,488 `c=7` contexts; THM-963 empties 215,728,128 `c=8` contexts with conflict graph `K3,3`; THM-969 empties 482,294,736 `c=9` contexts with `3K2`/`2K2` intersection nerves; THM-970 empties 821,620,800 `c=10` contexts after reduction to 64 projective sign transversals with triangular-prism intersection graph `K6` minus `C6`; THM-974 empties 1,636,866,000 `c=11` contexts after reduction to 66 all-order-eleven supports and replay of 66,000,000 unit words; THM-976 empties 2,413,458,432 `c=12` contexts after reduction to 64 sign transversals and replay of 262,144 words, with every distinct owner pair disjoint; THM-977 empties 6,194,388,816 `c=14` contexts after all 576 scalar rows fail at each owner, with maximum local union at most 12 of 14 sheets; THM-978 empties 10,320,710,400 `c=15` contexts after 2,184 scalar rows have at most four locally feasible owners; THM-980 empties 13,806,600,192 `c=16` contexts after 2,540 scalar rows have at most two locally feasible owners; THM-981 empties 22,303,024,128 `c=17` contexts after scalar capacity leaves only the QR/NQR rows and all twelve owner-local maxima equal sixteen. THM-860 excludes `c=13` by primitivity. The next AP-centred common-scale face is `c=18`; H5, non-AP/deep sheets, and the positioned seven-wall remain open. | THM-815 Part C/C.1/C.2, THM-857/859/860/861/862/957/958/960/962/963/969/970/974/976/977/978/980/981 + exact component/ramification/sheet certificates |
| primitive-core H6 antipodal AP-germ reduction | PROVED STRUCTURAL PRECURSOR, SUBSUMED FOR SCALE ONE BY THM-857: excluding the exceptional odd-label retained core, the 923 primitive-core missing-label rows split by full antipodal-pair count `f` as `63,480,360,20`. AP-germ cycle products and ten exact fixed-coordinate trees first closed fourteen `f=3` rows; a 2,653,600-state residual recursion closed the other six and left 903 `f<=2` rows. THM-857's literal component recursion then closes those 903 rows and the exceptional primitive branch too. The germ graph remains informative—the min-plus cycle dual of the Hunter max-plus tree—but `f`, cusp products, and binary orientations are lossy shadows of exact component--remaining-comb incidence. | THM-815 C.2, THM-857 + exact AP-pin/slice and component replays |
| primitive-core H6 low-cap independent subcertificate | PROVED / FINITE-EXACT, SUBSUMED BY THM-857: a separate implementation enumerates the 360 `f=2` roots, freezes every one of the 52 roots with first-speed cap at most 156, and closes them in `9,888,159` exact states with zero covering prefixes and 1,323 independent closed-danger checks. This is an implementation-level crosscheck of the full theorem, not a surviving frontier. | THM-815 C.3, THM-857 + exact `f=2` replay |
| Hunter-tree second-order seven-comb functional | PROVED functional/ideal coefficient + universal projective margin + common-dilate closure + FINITE-EXACT pilots, not full radius-seven closure: single-comb density bounds are information-theoretically empty at seven, while Hunter--Kounias gives ideal coefficient `(165-22m)/169`, positive at `m=7` and negative at `m=8`. The live-pull referee corrected the global pair law to the two-sided sawtooth `4/169+[Q(a+b)-Q(|a-b|)]/(169ab)` for the reduced pair `(a,b)`; in particular `mu(D_6 intersect D_7)=2/91<4/169`, so the original one-sided Bezout floor and raw-speed deficit lemma are false. For a `c_E`-component prefix, the projective pullback error at common scale `g` is at most `2c_E/g`. The exact anomaly is `(165-22m)e/169-sum s_i+MST(c)`. Its node/edge colours form the PSD restricted covariance `G^E_ij=c_ij-(2/13)(s_i+s_j)` and its irreducible endpoint field is a third centered moment; the sawtooth numerator is a rank-six reflection-odd PSD kernel. Deletion--contraction, partition insertion, and the graphic-rank derivative are exact recursive classifiers. A complete ten-ratio threshold table makes the bad-edge graph triangle-free and proves `MST(h)>=-237/7436`, hence strict projective margin `19/572` for every seven-speed packet. Every common-dilate ray satisfies `L_H>=19e/572-(1203/70)c_E/g`, with no pairwise-coprime hypothesis. Six exact packets verify the full decomposition: the four successes have positive-credit spanning trees, while both consecutive failures have all 21 endpoint/total credits negative despite mostly positive projective terms. Remaining: classify the prefix-coupled third moments on small-gcd maximizing trees and prove an AP-window argument for correlated residuals. | corrected THM-856 + exact overlap, node/edge, and projective-MST replays |
...
| n=12 sporadic branch | OPEN globally. Closed: the bounded shallow slice; the full AP-centred Hamming-one through Hamming-four stars at arbitrary height and scale; the proper scale-one Hamming-five chart; every common-sheet H5 survivor language with all effective orders at most 21; and the primitive proper AP-centred Hamming-six common-scale faces through `c=17`, with `c=13` primitive-impossible and the ordinary AP equality presentation separated. THM-858 leaves a finite smooth H5 order bank, and THM-860 bounds every primitive AP-centred H6 ramified language by `c<=840`. The proved remaining AP-centred H6 range begins at `c=18`; THM-982's primary C++ certificate gives a provisional scale-eighteen deficit—13,098 scalar rows on 684 supports, with no row feasible at more than four owners—but independent and cross-build replays are pending. Non-AP/deep-sheet continuations remain. At radius seven the full-period pair-credit theory includes the exact consecutive formula, and `LRCC8Consecutive` proves positive safe measure for every consecutive eight-speed block; inherited-window positioning and the other five combs remain open. On the deep side the classes `15,37,41 mod 52` still require a lift-dependent, multicolumn, or non-endpoint certificate. | THM-759/763/765/766/768/769/770/772/774/775/776/782/786/789/792/795/797/800/803/804/806/807/810/815/816/817/820/821/822/823/824/831/836/837/840/844/845/847/857/858/859/860/861/862/863/957/958/960/962/963/969/970/974/976/977/978/980/981; THM-982 provisional; `LRCC8Consecutive`; HYP-6820 |
...
4. **Reproduce and formalize finite tiles.** Independently rerun THM-738's complete bank; port the
   exact edge-`(5,7)` bulk screen across the other twenty flood bodies rather than promote THM-741
   globally; attach machine-verifiable certificates and exact scope metadata.
...
at seven. THM-859/860 make common dilation and every primitive proper
AP-centred ramified H6 language finite-decidable, with `c<=840`;
THM-861 evaluates all 64 `c=2` roots and leaves only the ordinary AP `[12]`.
Metric evaluation from `c=3` onward, radius at least seven, the two-sheet selector,
and other deep colour covers remain open.
...
THM-858 now reaches THM-823's sharp least-order pivot.  For every nonempty
colour set `S`, contract the complement by

```text
m_i=D_i/gcd(D_i,lcm(D_j:j notin S)).
```

Any common-sheet cover must satisfy
`sum_(i in S) ceil(2m_i/13)/m_i>=1`.  Geometrically, the complement misses an
owner sheet and each colour of `S` samples one gcd coset of its affine cyclic
interval on that fibre.  Singleton cuts remove 1,870 of the 2,055 new scalar
rows through order 21; top-prime cuts remove the remaining 185, and a literal
51,360-unit-word replay independently finds zero covers.  Hence the already
closed all-one/all-three/mixed languages are the complete common-sheet list
through order 21.  The unclassified strip is now
`min D_i<=21<max D_i`, with every order `{2,3,7}`-smooth and every maximal
prime power shared.  Applying the same cut at each adjacent valuation level
gives ranges at most `3,2,1` for `v_2,v_3,v_7`.  Therefore
`max_i D_i<=21*2^3*3^2*7=10,584`: this presentation bank is finite, although
its metric lift languages still require erosion.  Its faithful arithmetic carrier is the prime-power
hypergraph plus complement-lcm fibres and affine owner intervals; both tested
pair gauges are transitive on all 185 residuals despite 574 edge flips.
...
and THM-858 then excludes every new common-sheet language with effective
orders through 21 and bounds every remaining order by 10,584, turning the H5
frontier into a finite `{2,3,7}`-smooth shared-prime-power carrier. Two
...
one. THM-857 now completes the radius-six decision tree over all 924 deletion
roots: among 580,919,164 logical states, `2[12]` is the unique covering
terminal. The former AP-cusp reduction remains a useful min-plus chart, but
its intermediate 909-row frontier, later 903-row residual, and exceptional
mixed-parity row are no longer open at scale one. THM-856's strict `19/572`
projective margin moves the radius-seven obstruction to prefix-coupled
centered third moments on small-gcd maximizing trees and the correlated AP-
window residual. The finite smooth-ramified H5 presentation bank, ramified
common-sheet H6 languages at `c>=3`, deep two-sheet, and general higher-sheet
branches remain.
...
> Hamming-five chart, entire effective-order-at-most-21 Hamming-five
> common-sheet bank, and the full proper scale-one Hamming-six chart away
> from the doubled AP equality are closed.  The live obstruction is finite
> ramified H5 language erosion, ramified common-sheet H6 language closure,
> radius-seven endpoint/third-moment and correlated AP-window residuals, and
> signed-cell/component compatibility in the deep and higher-sheet branches.**

*Controlling corrections: HYP-6780, MISTAKE-143, MISTAKE-149, THM-762/764,
THM-768--770, THM-794/795/797/800--804/806/807/810/815--817/820--824/831/836/837/840/844/845/847/857/858/859,
