...
| Hamming-six exact closure / ramification boundary / seven-comb wall | PROVED / FINITE-EXACT, UNIFORM IN LIFT HEIGHT: THM-857 applies THM-815's longest-component cap at every labelled prefix over all `C(12,6)=924` deletion roots and every unbounded proper lift progression. Its logical tree has `580,919,164` nodes and `580,918,240` candidate edges; twenty terminals are certified loose and the unique covering terminal is the doubled AP `2[12]`. THM-859 proves that common dilation conjugates this complete action tree, so `2c[12]` is the unique tight row whenever all six effective orders are one. It also proves that order `D` is exactly the number of deck masks created by insertion: the scale quotient is a continuation congruence iff `D=1`. Every H6 common-sheet branch containing an order-one colour reduces to a finite H5 carrier bank and finite exact component trees; the first ramified subbanks have `336` two-order-one/four-order-three and `672` one-order-one/five-order-three contexts. Their zero-cover verdict is not yet proved. Seven remaining combs are still the precise first-moment density barrier because `13-2m` changes sign. | THM-815 Part C/C.1/C.2, THM-857/859 + exact primary/closed-danger replays |
