...
| scale-one Hamming-six exact closure / seven-comb wall | PROVED / FINITE-EXACT, UNIFORM IN LIFT HEIGHT: THM-857 applies THM-815's longest-component cap at every labelled prefix over all `C(12,6)=924` deletion roots and every unbounded proper lift progression. Its logical tree has `580,919,164` nodes and `580,918,240` candidate edges; twenty terminals are certified loose and the unique covering terminal is the doubled AP `2[12]`, arising from missing odd labels lifted to `14,16,...,24`. A companion implementation reconstructs closed danger unions and hashes each labelled root tree. Every other scale-one H6 packet is strictly loose, including all packets over the 923 primitive retained-core roots and all primitive mixed-height completions over the exceptional root. Arbitrary AP scale/common sheets remain open. Seven remaining combs are still the precise first-moment density barrier because `13-2m` changes sign. | THM-815 Part C/C.1/C.2, THM-857 + exact primary/closed-danger replays |
...
| n=12 sporadic branch | OPEN globally. Closed: the bounded shallow slice; the full AP-centred Hamming-one through Hamming-four stars at arbitrary height and scale; the proper scale-one Hamming-five chart; every common-sheet H5 survivor language with all effective orders at most 21; **all proper scale-one Hamming-six packets other than the genuine equality `2[12]`**; every two-sheet core in `[1,19]` with unbounded odd exceptions; and the other named finite banks. THM-858 proves that the entire remaining common-sheet H5 effective-order bank is finite, with `min D_i<=21<max D_i<=42,336`, all orders `{2,3,7}`-smooth, and no private prime power. THM-857 removes the former 903 `f<=2` primitive-root residual plus the exceptional primitive mixed-height branch, while subsuming all prior `f=3` work; what remains on H6 is transport across arbitrary AP scale/common sheets, not another scale-one label census. Exact endpoints are Markov only for monotone insertion. On the deep side THM-824/831 classify the radius/switch scalar, while THM-836 excludes shells `s=1,3`, uniformly eliminates `d=11 mod 52` at `s=5`, and proves that no `U`-independent single unit numerator on `q=5d` or `13d` can close the other three classes. The classes `15,37,41 mod 52` remain open and now require a lift-dependent, multicolumn, or non-endpoint certificate. | THM-759/763/765/766/768/769/770/772/774/775/776/782/786/789/792/795/797/800/803/804/806/807/810/815/816/817/820/821/822/823/824/831/836/837/840/844/845/847/857/858; HYP-6820 |
...
gives ranges at most `5,2,1` for `v_2,v_3,v_7`.  Therefore
`max_i D_i<=21*2^5*3^2*7=42,336`: this presentation bank is finite, although
...
orders through 21 and bounds every remaining order by 42,336, turning the H5
...
scale/common-sheet H6 transport, deep two-sheet, and general higher-sheet
branches remain.
...
> ramified H5 language erosion, arbitrary-scale/common-sheet H6 transport,
...
THM-768--770, THM-794/795/797/800--804/806/807/810/815--817/820--824/831/836/837/840/844/845/847/857/858,
