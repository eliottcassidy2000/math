...
| connected-return selector compression | CLAIMED / AUDIT IN PROGRESS: the mandatory central return component is proposed to give a linear necessary endpoint/cusp selector, exact when the closed return set is connected. Disconnected return satellites would then be the source of THM-803's quadratic interaction. Long silent `13d` grid ladders are also being constructed as method boundaries, not tight packets. | THM-807 reservation |
