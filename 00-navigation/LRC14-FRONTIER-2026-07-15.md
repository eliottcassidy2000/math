...
| Hamming-six exact closure / ramification boundary / seven-comb wall | PROVED / FINITE-EXACT, UNIFORM IN LIFT HEIGHT through `c=2`; STRUCTURALLY CLASSIFIED at `c=3`: THM-857 applies THM-815's longest-component cap at every labelled prefix over all `C(12,6)=924` deletion roots and every unbounded proper lift progression. Its logical tree has `580,919,164` nodes and `580,918,240` candidate edges; twenty terminals are certified loose and the unique covering terminal is the doubled AP `2[12]`. THM-859 proves that common dilation conjugates this complete action tree, so `2c[12]` is the unique tight row whenever all six effective orders are one, and identifies order `D` as the exact deck-mask ramification degree. THM-860 makes the entire primitive proper AP-centred H6 ramified branch finite-decidable: `lcm(D_j:j!=i)=c`, optimized valuation ranges `(5,3,1,1)`, `min D_i<=72`, a complete 8,449-word joint-cut census, exclusion of every arithmetic row at scales 1,120, 1,008, and 882 by 2,162,160 scalar owner-capacity checks, `c<=840`, and at most 37 first heights per labelled ray. Its fibre-Hunter inequality imports exact max-spanning-tree overlap debt into the remaining sheet banks. THM-861 closes the first ramified face: its 64 signed-cycle roots give a `41,882,982`-node exact tree whose unique cover is the ordinary AP `[12]`; every sporadic `c=2` packet is loose. THM-862 classifies the next sheet stalk as 212 presentations/1,504 contexts, 68 affine matching-code templates, 110 retained roots, and 146,912 exact first metric edges. Literal first-child geometry caches to 22,262 keys, but the five future rays separate all logical lanes. The sheet/cache quotients are not metric, so the `c=3` terminal recursion remains open. Seven remaining combs are still the precise first-moment density barrier because `13-2m` changes sign. | THM-815 Part C/C.1/C.2, THM-857/859/860/861/862 + exact primary/closed-danger/ramification/sheet replays |
