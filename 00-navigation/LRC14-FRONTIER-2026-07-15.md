...
| folded-target no-switch frontier | PROVED SHARP for every reduced odd-pair fold + FINITE-EXACT: for coprime opposite-parity half-frequencies `alpha>beta`, all components have explicit Bezout-offset centres and radii. The nonempty primitive symmetric radius factorization holds exactly for `4<=alpha<=9`, comprising sixteen antipodal two-ball rows. From `alpha=10` onward the ordered component triple `(-1|-3,+1)` has nonpositive weighted switch slack, so even variable-radius clearance factorization fails. A common gcd creates raw deck switches but preserves the quotient-scaled radius theorem. The proof carrier is the weighted ordered centre-switch 3-hypergraph, not a runner/component tournament. | THM-831 + 518-pair/6,160-packet exact replay |
...
| one arbitrary-height order-three Hamming-five context | FINITE-EXACT EMPTY: for `C={1,5,8,12}`, `b=10`, parity bits `(1,1,1)`, the exact active-endpoint five-comb recursion visits 75,371 states and has zero covers, with 57 nonempty terminals and maximum recursive bound 1,025. This closes one of the 96 directed-flag/parity contexts left by THM-823. Both raw and conditioned comb tournaments are transitive on every terminal while one through ten edges flip; active endpoint runs plus remaining CRT progressions carry the proof. The other 95 contexts remain OPEN. | THM-837 + exact Fraction replay |
...
| n=12 sporadic branch | OPEN globally. Closed: the bounded shallow slice; the full AP-centred Hamming-one through Hamming-four stars at arbitrary height and scale; the complete height-at-most-two scale-one Hamming-five slice; one of the 96 arbitrary-height order-three five-comb contexts; every two-sheet core in `[1,19]` with unbounded odd exceptions; the full two-sheet speed box through 100; every non-signed-complement `q=13` profile; and the forced-`w=13c` quotient box through height 24. THM-815/820 reduce scale-one Hamming five to the joint boxes `(146,292,584,1168,2336)` and `x<=146,v<=1986,max_top<=7944`, while scale-one Hamming six is recursively finite and radius seven is the first discrepancy-unbounded chart. THM-840 proves exact endpoints are Markov for monotone comb insertion, but neither `H0/H1` nor endpoints alone support all needed operations. THM-823/837 reduce the all-scale five-deck interface to a least-order pivot and one bounded survivor orbit, then close one of its 96 contexts; the other 95, common sheets above order twelve, and mixed order-one strictness remain open. On the deep side THM-821 factors every odd-pair atomic stalk, THM-824 gives the exact `(13d,5d)` global radius budget, THM-831 classifies the sixteen primitive no-switch folds, and THM-836 excludes the first two owner shells under its exact signed-complement/guarded-containment hypotheses by `13d<=2B-5`; local `s=5` owners survive. Remaining: exact closure of the Hamming-five boxes, the other 95 all-order-three contexts, exhaustion of H6, and unbounded deck languages; uniform violation/decreasing rules for the viable radius/sum-arc selectors; ternary switch-aware treatment elsewhere; unbounded dyadic/collar assembly; and higher sheets. | THM-759/763/765/766/768/769/770/772/774/775/776/782/786/789/792/795/797/800/803/804/806/807/810/815/816/817/820/821/822/823/824/831/836/837/840; HYP-6820 |
