...
| small half-frequency two-ball extension | RESERVED / NOT A RESULT: THM-831 proposes auditing the sixteen coprime opposite-parity pairs with `4<=alpha<=9`. Preliminary arithmetic reports two antipodal components and the needed separation, but exact centres/radii, endpoint-complete no-switch proof, the `alpha=10` boundary, common-dilate corollary, replay, and Tournament Analysis are all explicitly unfinished. No branch deduction may use this stub. | THM-831 reservation only |
