...
THM-2441 makes the delayed-clock part of the surviving graft audit finite. For any fixed rational exclusive source with endpoint denominator `13^K D_0`, clocks in one class modulo `7D_0` differ pointwise by an explicit uniform ancestry vector.
The complete centred signed event word and target-restricted nonflatness therefore have period dividing `ord_(7D_0)(13)`, independently of the fixed target denominator.
This does not supply the genuine global source/terminal packet or prove owner-conditioned landing.
