...
`224`. THM-2239 separately centers a nonnegative signed charge for each blocker core and closes another `58`: all `29` first-depth-two rows, `28/29` first-depth-three rows, and `(4,6,8)`.
This leaves all `165` first-depth-one rows with deepest depth at least five and `(3,4,5)`. THM-2243 centers the latter's composite union; THM-2250 closes every unequal-core partition; and THM-2257's exact `169`-image sieve contradicts the capacity of its common-core branch. The exact scalar ledger is therefore `165`.
...
THM-2323 gives every primitive colour a common bare/positive-word ordinary Fourier atom and extends the mechanism to gcd-compatible arithmetic combs. THM-2326 puts a seven-primitive `c_3` edge through every marked shallow vertex; THM-2327's two-colour triangle upgrades this to a `91`-unit marked edge on every positive shallow-owner word stratum in the `150` strict rows. The `15` repeated-first rows and alternative resonance arms remain outside that closure.
THM-2321 prescribes the root character or projective target-gain label of a positive cubic word current; THM-2329 shows that rerooting the marked `c_3` triangle realizes exactly the two pure axes and mixed gain `-1`, while the other eleven directions require a transverse three-nonzero-character triangle. THM-2325 puts more than `3.1*10^12` exact all-`91`-unit relations in every nonzero target-vector fibre.
THM-2331 embeds every such relation, with at least `234375` two-sided lifts, as a nonzero Abel term. [THM-2333](../01-canon/theorems/THM-2333-abel-target-fibre-sum-landing-and-zero-fibre-boundary.md) proves that some zero-word-mode target aggregate survives, but an exact hostile can leave only the zero target.
[THM-2334](../01-canon/theorems/THM-2334-relation-residue-current-and-character-twist-pushforward.md) retains the full word: each fixed exact-address orbit is ordinarily absolutely convergent, every finite residue pushforward is a coordinate-twist DFT, and nonzero-target survival is exactly the variance of `169` shifted currents.
[THM-2337](../01-canon/theorems/THM-2337-expiration-word-residue-invisibility-and-first-bockstein-sidecar.md) proves that the word is target-neutral modulo `13` but reappears as a factor-coloured first jet; that jet is surjective termwise but gauge-dependent, and word-support landing is a masked squared energy.
[THM-2340](../01-canon/theorems/THM-2340-owner-word-anova-target-landing.md) diagonalizes that energy: pure-`a` and pure-`b` landing are the ordered main-effect norms of the `13 by 13` twist matrix, while fork landing is its two-way interaction norm.
The highest-leverage strict-row obligation is therefore to prove the relevant main effect or interaction nonzero for the canonical current. After that, one must still retain an all-`91`-unit or bounded visible aggregate and transport THM-2303 terminal-component phase.
No scalar profile is excluded by these connectors: the exact ledger remains `165`. THM-2300's `m=7` hostile and THM-2319's negative mixed polarization are the sharp warnings against promoting a term to an aggregate. The other immediate exits are the repeated-first/resonance branches or THM-2304's middle-depth balance; THM-2307 still reconstructs the dual current only at rank eight.
...
- **Knots / operation-ready relations:** [THM-2176](../01-canon/theorems/THM-2176-gordian-continuation-profile-and-interaction-cocycle.md) gives the continuation kernel; [THM-2191](../01-canon/theorems/THM-2191-catalytic-localization-of-the-gordian-metric.md) gives catalytic localization and stable group length. THM-2220 gives fixed-context slopes; THM-2242 makes the kernel embedding isometric and identifies its atoms.
  ...
  [THM-2336](../01-canon/theorems/THM-2336-prime-target-gordian-owner-diagram-and-bypass-split.md) evaluates every prime-target fibre as a weighted owner diagram. For a two-prime packet, landmarks `U,K,J` recover both directional translation/bypass splits; the `T(2,7)`/mirror shortcut has vanishing prime-summand landmarks and is pure bypass.
  [THM-2339](../01-canon/theorems/THM-2339-prime-owner-deletion-and-target-allocation-hypergraph.md) makes composite-target lifts finite prime-token allocations; Möbius gives a canonical coloured hypergraph with global gauge. Deletion identifies the fixed prime-owner face with the complementary unknot face; first collision has an exact bypass/competitor law. Cited sole-cover drops are nonzero and unbounded. Positive catalysis and the exact diagonal remain open.
