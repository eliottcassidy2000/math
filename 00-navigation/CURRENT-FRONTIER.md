...
- **Knots / operation-ready relations:**
  [THM-2176](../01-canon/theorems/THM-2176-gordian-continuation-profile-and-interaction-cocycle.md)
  makes `Pi_K(J)=u(K#J)` universal and its defect a cone-monotone positive
  2-coboundary.
  [THM-2191](../01-canon/theorems/THM-2191-catalytic-localization-of-the-gordian-metric.md)
  now gives the greatest connected-sum-invariant metric below Gordian
  distance, with root norm `u_cat(K)=min_J d_G(K#J,J)` and
  `max{u_hash(K),c*(K)}<=u_cat(K)<=u(K)`. Owens--Strle's
  `c*(9_10)=u(9_10)=3` closes the formerly unresolved reverse `4_1,9_10`
  direction as pure bypass. Positive catalysis is still open; the sharp
  cited-seed boundary is conditional `10_6` (`c*=2`, `u in {2,3}`),
  followed by the uncalibrated pretzel families.
