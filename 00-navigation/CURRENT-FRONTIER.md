...
- **Jacobian/Dixmier:** `JC(2),DC(2)` remain **OPEN**; THM-3543--3991 close named cells, not global entry. THM-3992/3997 reduce the first live `2:3` cell to `R in (p^2,y)`, give the all-row Hasse criterion, force `[p^2]R!=0`, and exclude `R=0`.
  ...
  THM-4008 excludes every entire pure-`p` residual. THM-4016 proves nontorsion of the formal sharp-`5x5` weight-six points `(X^3,Y^2)=(43/84,127/84)`; THM-4017 refutes their realization as attachments because the same sharp data force a `p^4` term with rho-exponent `-2` on that scale. Its truncated `p^4+y^2` lower Newton model has a conditional `j=1728` signal, but `p*y^2` destroys the side facet. THM-4011 finds a class-zero companion-factor kernel invisible to finite observers and applies log Riemann--Hurwitz to actual factors.
