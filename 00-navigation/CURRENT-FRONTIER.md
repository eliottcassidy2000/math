...
THM-2321 prescribes the root character or projective target-gain label of a positive cubic word current. THM-2325 puts more than `3.1*10^12` exact all-`91`-unit full-lattice addresses in every nonzero target-vector fibre, but label abundance does not make one address participate in that current or enter the bounded visible carrier.
Thus the strict-row live object is an address-decorated, target-polarized mixed Fourier circuit: its source leg is word-marked and incident to a unit `c_3` edge, while actual full-lattice coefficient/address participation and THM-2303 terminal-component phase transport are unproved. THM-2300's `m=7` hostile and THM-2319's negative mixed polarization show why separate support statements cannot supply either coordinate.
No scalar profile is excluded by these connectors: the exact ledger remains `165`. The other immediate exits are the repeated-first/resonance branches, visible-carrier incidence, or THM-2304's middle-depth balance; THM-2307 still reconstructs the dual current only at rank eight.
