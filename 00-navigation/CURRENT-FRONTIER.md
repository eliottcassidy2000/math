...
Hence `nu_7(c_3)>M => nu_7(H)=M`. THM-2430 classifies the exact common-`91` tilings; THM-2431 excludes every `t=5,b=0` type, and THM-2432 removes `(2,0,0,2)`. The live deep-`c_3` shapes are `(1,5,1,8),(2,5,2,9)` at `M=0` and `(2,5,1,8)` at `M>0`. THM-2435 proves their actual top blockers fill quotient-stalk gaps over parent mass at least `(1+k)/7`; one fixed label has an equivariant marked section of mass at least `3/1274`, exact energy at least `3/115934` in every quotient `C_91` class, and total unit-class energy at least `108/57967`. This is physical for `M=0`; positive depth retains a `7^M` ancestry kernel. The `c_3<=M` side has an isolated noncirculant top graft. No scalar row is removed.
