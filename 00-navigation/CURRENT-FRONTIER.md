...
THM-2300 forces same-character relation multiples for `m<=6`, leaving `693` no-forced multipliers, but the primitive/base cancellation bank remains `696`; `m=7` is sharp. The missing bridge is lawful relative phase transport plus exact-carry alignment, potentially supplied by a global cover identity; the other `45` lack the anchor, not rank, lift existence, rooted return, or same-character multiples.
