...
### LRC(14) finite-owner/defect-tail tasks generated by THM-4029--4031 (2026-08-24)

- **D4 pack-safe intersection (highest-value anchor).** THM-4030 gives the
  exact certificate-positive spoiled-phase set for each exception triple,
  but forgets the divided pack's safe set `G(H)`. Represent both as unions
  of rational open/closed intervals and decide whether
  `G(H) minus Spoil(2r,a,b)` is nonempty under THM-3818's height and gcd filters.
  Cheapest controls: the typed `H={1,...,10},(2,9,11)` selector hostile and
  its safe phase `21/22`; first search certificate-positive triples only.
- **Small-modulus completion.** Derive labelled affine-lattice laws analogous
  to THM-4030 for the remaining `d=3` partition `1+1+1` and the `d=2`
  equality branch. Preserve assignments and strict endpoints. Do not replace
  them by bad-count totals; the first decisive test is direct-wall versus
  defect-lattice equivalence on all small distinct exception profiles.
- **Sparse deletion response of the twelve AP owners.** THM-4029 solves the
  ...
  Seek a deletion-mask max-min formula for
  `a(N+1)-meas(S7(E))`. Cheap hostile: `E={0,...,6,N}`, where scalar
  containment throws away most of the mass. Target: a cardinality/gap-aware
  bound, not AP extremality by monotone compression.
- **Replace the asymptotically dead owner-relaxed gate.** THM-4031 proves that
  minimization over all `u<=U` has an arithmetic survivor tail on every old
  strip ray. Retain the actual eleven-owner set and simultaneous endpoint
  assignments, then test whether an owner-count/height inequality prevents
  the endpoint sequence `u=kU-1` from appearing often enough. Any proposed
  replacement must survive the scaled-owner hostile `u=kU` and declare its
  scope below `91^6`.
- **Prime-sector tail theorem (orthogonal niche).** Generalize THM-4029 from
  seven sectors to prime `P`: prove eventual finite-owner phase-rationality
  and the candidate constant
  `C_P=(1/P) sum_(q<P) phi(q)(2(P-q)-1)/q`. The existing exact checks through
  `P=31` are regression evidence only; the proof must handle the robust
