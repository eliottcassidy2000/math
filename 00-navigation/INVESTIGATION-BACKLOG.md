...
- **The live target:** the closing sum `Σ_{n∈Λ°(E)} D7(n mod 7)/∏_j n_j` (HYP-2606/2646, conditionally convergent, ruled by support-6 density R6 per corrected THM-538) is a **character-twisted reciprocal-product sum over the relation lattice** — the higher-rank analog of the t2 hyperbola sum ALREADY FORMALIZED in LEM-022 (`Σ 1/(cdist h·cdist wh)`, rank-1 via P(w)). **Next step:** the Minkowski successive-minima count `|K(n)| ≤ c/(λ₁···λ₆)` on Λ°(E) for wide-spread E (HYP-2608a) — a lattice-point count, not a character-sum bound. This is the single highest-value open step.
