...
**Next steps:** (1) state the discrete decorrelation bound precisely (port THM-661's moment-LP to Z_q bands); (2) estimate V0 + the m≠0 divisor constant; (3) exhaustive small-V liveness census up to Vmax ~ 40-60 to locate the true V0 empirically; (4) Lean: the aggregation identity + the dispatch are decide-shaped; the theta port is the one analysis item. Guard-rails: self-hosting refuted (don't design pair-choice heuristics); low-order proxies (E2/E3/R3 truncations) fail — full-ledger only.
