...
- **OPEN / next steps (each cheap):** (1) **the n→∞ conductance constant** — is `G_n` a true expander (φ bounded below) or slow-decaying? Exact φ=0.6,0.379 (n=4,5); n=6 bracketed [0.16,0.46]; get n=7 φ via `tournament_metagraph.py`/`fast_metagraph_n7.py`. (2) **isoperimetric dimension of the SC-spine** (quasi-1D at n=6, branched at n=5 — resolve). (3) ⭐ **run the whole battery on the even-graph metagraph `E_n`** — the honeycomb IS a tiling by even 6-cycles, so `E_n` is the prime candidate for a genuinely 2D ("honeycomb-analog") tournament graph; this doubles as the standing opus `E_n`-dual mandate. (4) the densest-k-subgraph profile `M_n(k)` on `G_n` (started; exact n≤5) vs the A000568 edge-sandwich (OPEN-Q-108). (5) does "no thin boundary layer" give a clean *impossibility* meta-lemma for `G_n` bounds proved by peeling? (retro-explains MISTAKE-035/037).
...
- **FORGOTTEN FRAMES to revive (highest value first):** (1) "everything is sl(n)" (`everything-is-sl-n.md`, opus-S313, dormant since March — the dual to "everything is the triangle"; would give χ(G_n)=n-1 a rep-theory proof); (2) the index-theorem frame for LRC (`the-index-theorem-frame-...`, mac-mini-S79 — topological route orthogonal to moment-nullcone); (3) two-hop/A² principle (opus-S339/S345); (4) DRT engine S²=J−nI, Catalan=genus-0 (monad-S7); (5) lever-zoo as one moment hierarchy (mac-mini-S76, never cross-linked to THM-1775); (6) revisit regularity/AP/Paley only as a **typed comparison**: MISTAKE-227–229 rule out treating them as one object without an explicit map, saturation/basis audit, and preserved predicate.
- **REFUTED-BUT-CLOSE (cheapest theorems):** HYP-3798 (κ lazy-caterer, one exhaustive step from settled n≥7); HYP-260 (δ interior, one boundary lemma); HYP-3805 (flip-rank break = Paley heptagon = LRC extremal — corrected formula open); HYP-2912 (sharper spectral gap, 3/41 inside); HYP-463 (corrected p mod 4 variational principle).
- **GAPS / named-never-computed:** the E_n dual sweep (run the whole battery on the even-graph metagraph — opus mandate, biggest structural gap); Ihara/Bartholdi zeta of tournaments (TANGENTS, never done); the beta2_rank_identity.out n=5 VIOLATION anomaly (uncited, unchased); 11 dangling variable-registry files; greenfield fronts with ZERO repo work (Seymour 2nd-neighbourhood, Sumner, Caccetta–Häggkvist, cap-set, Alon–Tarsi); the radius-7 single-cluster sweep (787 prefixes, never run → would make radius-7 non-tightness unconditional).
- **Empirical harvest this session (n≤6 verified, flagged for n=7):** γ,fas,dichr ≤ ndev (spectral dominates width); H ≤ Σa_r (paths ⊂ source-rooted arborescences); disc ≤ H (HYP-8636); ndev/kings both skip {2} (THM-1858 + classical). Dataset `tournament_zoo_full_deathstar_S79_data.json`.
### Reductions as character decompositions; the real-character closure criterion (boxeph-2026-07-21-S194, THM-1875)
- **Source:** THM-1875 + `07-reflections/the-reduction-is-a-product-of-trigonometric-functions-boxeph-S194.md`; script `trig_reduction_tournament_boxeph_S194.py`.
- **The lens (verified pieces):** char_A(T)=∏char_A(strong component) (exact n≤6); signed partition = Vandermonde = product of sines on the circle; Paley nonprincipal adjacency roots are shifted half-scaled quadratic Gauss sums, while interval-circulant roots use Dirichlet/Chebyshev-U. Reductions are decompositions along group characters; the shared Dirichlet/Fejér kernel is an analytic analogy until a target-preserving map is supplied.
- **Open next steps:** (a) **[DONE → THM-1926, boxeph-S195]** the Bowen–Lanford zeta ζ_T=1/det(I−uA) DOES concentrate on the strong core (Euler product over primitive cycles, ζ=1 on acyclic, complement-invariant, N_3=3c3, poles=1/Gauss-Chebyshev). NEXT within: the two-variable Ihara/edge-zeta refinement; var(λ_S²) as a zeta-residue. (b) **The real-character closure criterion**: formalize "a reduction telescopes iff its character is real (±1 sign)" — predicts which repo problems collapse (tournament sign-involution: yes; LRC oscillating sinc: no). Test against the GMC(2)⇒LRC(14) barrier (mac-mini-S157). (c) Lean the block-triangular char-poly factorization. (d) push (a) to the DFT block-diagonalization of circulant tournaments (THM-125) — spectral algorithms as explicit character decompositions.

### The order-join reduction principle as a WOWII proof/refute engine (boxeph-2026-07-21-S193, THM-1862)
- **Source:** THM-1862 + `07-reflections/inflation-velocity-and-the-coupling-law-boxeph-S193.md`; scripts `tournament_graffiti_coupling/strongcore_mine/largen_sample_boxeph_S193.py`.
- **The tool:** order-join `T₁▷T₂` makes c₃,tr,scc additive and Rédei H **multiplicative**; strong tournaments are the atoms. A **join-monotone** invariant inequality reduces to the strongly-connected core (proof engine); a bound whose sides **decouple** under an inflation op D± is refuted by the THM-1830 3-cycle-atom+singletons witness (refute engine). Subsumes opus THM-1855 (single-vertex case) and kp THM-1860 (`c₃≤H` instance).
- **Status:** framework PROVED + verified n≤7 (530 classes) + sampled n≤12. Delivered: minimal repair `srange ≤ tr+1`; king-ecc straddle of e (19/7, 17/6); `c₃≤H` strong-core margin (never tight, ratio ≤ 2/5); low-entropy caveat (random sampling misses inflation-fragility).
- **Open next steps:** (a) prove `srange ≤ tr+1` for all n (currently n≤7 + join-mono; is it sharp forever, or does it break like srange≤tr did at n=7?). (b) finish the strong-core `c₃≤H` base (Moon/Busch Ham-path lower bound vs c₃≤C(n,3)) — closes kp THM-1860. (c) calibrate a TIGHT king-eccentricity directed-103 whose only slack is the ln-term so the e-straddle is decisive (HYP-8647). (d) apply the coupling/velocity analysis to the METAGRAPH invariants (klein-S395 computed α,b,ecc on G_n; add: is any conjectured G_n-invariant inequality join-monotone under the metagraph's own composition?). (e) auto-screen: run every new generator survivor through join-monotonicity (lifts-to-all-n) + D±-fragility (pre-refute) BEFORE the exhaustive n≤7 filter.
### TOURNAMENT-INVARIANT ZOO ATLAS + its gap map (klein-2026-07-21-S399)
- **Source:** `00-navigation/TOURNAMENT-INVARIANT-ZOO-ATLAS-klein-S399.md` — a full synthesis of five parallel repo-mining passes (tournament-proper / metagraph / moment-nullcone / forgotten-threads / cross-domain). Read the atlas first; this backlog entry lists the ACTIONABLE gaps it surfaced.
- **Status:** OPEN, catalogued. Two gaps CLOSED this session (see §V): Perron ranking ≠ arborescence ranking (distinct centralities, Kendall-τ≈0.94, exact-match 174/353 at n=7); skew-adjacency SNF subsumed by the Pfaffian / degenerate at odd n.
- **TOP named-never-computed objects (each cheap, each a 1-session win):**
  - **(1) ⭐ The equivariant partition function Z_n** — "the project's deepest surmise": one generating object unifying the tournament / metagraph / moment layers. NEVER tested. Next: define `Z_n = Σ_{iso classes} weight^H · charge-grading` and check for factorization / functional equation at n≤6.
  - **(2) ⭐ The bicycle space** (Cut ∩ Cycle of the metagraph star-flip GF(2) split, THM-1382/1405) — NAMED, never computed at any n. Next: compute dim(Cut∩Cycle) at n=4,5,6; is it 0 (direct-sum, expected at odd n) or positive?
  - **(3) The A049313 second metagraph family** — named once, never built.
  - **(4) The full E_n invariant sweep** (opus's "do everything on E_n too" mandate) — energy, Hadwiger, width, spine/ribs/sea analog NEVER run on the even-graph dual E_n.
  - **(5) fractional χ(G_n/Z₂) and the Lovász-θ sandwich** — never run at any n.
- **MISSING graph→tournament analogs still OPEN** (II.c; SNF now closed): Ihara zeta / non-backtracking spectrum (TOP — cheap, "no immediate reversal" is natural for orientations), Lovász θ (needs SDP; ties to the G_n/Z₂ Hadwiger/perfect-graph thread), sign rank, directed Tutte.
- **UNCOMPUTED invariant-pair correlations** (II.b, automated by generator G4): H vs Pf, Σa vs d(T), tr vs dichr, OCF vs #C₃, |Aut| vs H — run the full pairwise Kendall-τ matrix over all invariants at n≤7 in one sweep (never done; one pair done this session).
- **ELEVEN dangling registry files** (`05-knowledge/variables/INDEX.md` references them, no file exists): `alpha-k.md`, `signed-hp-permanent.md` (S(T)), `D-k.md`, + 8 more — pure documentation debt, each a 20-min stub from existing canon.
- **Seven PROCEDURAL GENERATORS** (atlas §III) for producing new frames on demand: G1 WOWII-inequality (easy⇒hard + refute + Lean), G2 E_n-dual, G3 per-class/per-charge refinement, G4 correlation matrix, G5 missing-analog import, G6 functional-swap (THM-1840), G7 two-atom threshold.
- **Why it matters:** the directive was "find ALL the invariants and the gaps between and around them." This atlas is the navigation surface; every row is either a computed invariant, a closed gap, or an actionable next-step. Do not let a named object stay uncomputed.
...
### TournamentGraffiti + formal-conjectures bridge (death-star-2026-07-20-S77, HYP-8618)
- **Source:** owner-supplied counterexample to Graffiti WOWII-103 (google-deepmind/formal-conjectures PR #4482), verified in `graffiti_leverage_tournament_deathstar_S77.py`.
- **Status:** prototype built; three concrete leads open.
- **Next steps:** (1) build `tournament_graffiti.py` — a TxGraffiti-style automated conjecture/refute engine over the repo's invariant zoo (H, c3, τ=largest transitive sub, kings, scores, eigenvalues, disc, pencil, even-graph E_n invariants), $n\le 7$; an engineering deliverable. (2) marginal-threshold refutation sweep on the repo's transcendental bounds (√2,π,e,γ; LRC 1/(n+1); measure-horn 1/(7L); the width formula = a native "holds n≤6 breaks n=7" 103; the {7,21} gap). (3) contribute the repo's Lean artifacts (LRCWitnessMomentFloor, GMC2 strata) to formal-conjectures AND mine it for parity/moment-attackable conjectures. Reflection: graffiti-103-counterexample-and-its-leverage-...-S77.
...
- **Next step:** the spectrum is a multiplicative monoid (ordinal sum) generated by strongly-connected
  tournaments' counts. To prove completeness, show these generators multiplicatively cover odd\{7,21}:
  (a) every odd prime ≠7 is a strong-tournament count; (b) every 7k (k≥5 odd) is directly achievable.
  The nearly-transitive family gives 2^{n-2}+1; find a generator family covering all odd primes ≠7.
  Also: OEIS/literature check — is the exact spectrum with the {7,21} exception already published?
...
- **(a) THE 2-JET ARCHITECTURE -- IDENTITY LAYER DONE (S131, THM-2446):** the six-bracket z-graded decomposition, Keller reformulation, Euler cone law 4[Phi_x,Phi_y,Phi] = z det J(Phi) (E5 vacuous, E4/E3 slice laws), gauge-22 bookkeeping, and the tame-witness warning ((x+z^2,y,z) passes the graded system -- consistency is NOT a wildness certificate; stratify away the tame locus before any GB hunt). REMAINING: measured per-box ranks, the swallowtail master identity (P1) and the stratified d=4 seed hunt itself. (P2 REFUTED same day: dominant plane maps survive the full Keller system -- see THM-2446 S5 refutation record; the false w-cone constraint is REMOVED from hunt design.)
...
## Lead kind-pasteur-2026-07-19-S128c97: the Jacobian/Dixmier fall -- four cashable follow-ups (HYP-8095 (ex-8070/8090, twice-ceded), T1547)
- **Source:** owner-reported JC_3 counterexample, independently verified (det J == -2 symbolic + exact collisions + equivariance); reflection `the-jacobian-falls-dixmier-follows-and-the-repo-mirrors-kind-pasteur-S128c97.md`.
...
- **(i) The c(p) growth law + extinction point — RESOLVED NEGATIVELY (S128c91, HYP-8000, MISTAKE-192): no extinction prime exists** (the AP is improper mod every p coprime to 14 — one line from THM-401/1002 — so c(p) ≤ 13 identically; compiled hunt verified 130 primes alive to 1200 with certificates). REPLACED BY: (i′) the LARGE-p SHELL-COLLAPSE question — where does c(p) first reach 13 (12-onset is p=181; the exact run 200–320 was cut mid-grind, binary + CLI committed for resume), does it stick, and do minimal covers thin to the classifiable tight-family shell (reversing the S128c88 generic-sea finding at p ≤ 71)? The 15 greedy-hard primes (349…1163) rank sea-thinness = per-prime sieve cost. Original growth-law text follows for reference: **(i-old)** the quantized seven-tile wall c(p) = 7,9,9,9,10,10,10,11 at p = 29..151 (excess over density bound grows ~log p?). Pin the law with a compiled evaluator (death-star's ghost machinery reaches 2 more orders of magnitude); locate the crossing c(p) > 13 where I(13,p,1) = ∅ (level-1 improper-sea extinction — a finite-check design constant). Proof target: c(p) ≥ 9 for p ≥ 43 via cover-debt/wasted-overlap (boxeph's rank-identity frame on a finite exact object). Lean seeds: the p=29 perfect tiling (14-point decidable); c(43) ≥ 9 (finite check).
- **(ii) The PTE bridge (H-H1):** the moment-blindness decay profile (deg-4 width ≥ 1/12; deg-6 ≥ 2/99) should be governed by Prouhet–Tarry–Escott-in-squares solvability at size 13; a literature session (opus's pinning genre) on even-PTE/ideal-solution sizes would convert the measured profile into a threshold theorem "degree d* determines 13-sets" — the cage's degree-26 Newton is the trivial upper bound; the true d* is unknown.
- **(iii) Triage axis 6:** fold the measured moment-blindness into opus-S399's five-axis triage law ("moment degree ≥ d* or blind"); the three-instrument triangle (grids/sea, profiles/geometry-to-2/29, moments/sliver) is a supplement to the wall census.
- **Status:** OPEN; (i) is one session with compiled code; (ii) is one literature session; (iii) is a doc edit.


## Lead opus-2026-07-19-S401: cash the two S401 results — one pinning session + one effectivization target + a literature-facing note
- **Source:** THM-1288 (S-T Conjecture 7.1 REFUTED, self-contained) + HYP-7930 (Giri–Kravitz accumulation revival) — reflection `c71-refuted-by-translation-and-the-gk-accumulation-revival-opus-S401.md`.
- **(1) G-K pinning session — DONE (opus-S402), SPLIT VERDICT:** the v1/v2 equality acc(S(n)) = S(n−1) was WITHDRAWN (v3) and is Conjecture 1.5 in the published v4 (MPCPS, DOI 10.1017/S0305004125101497). PROMOTED: THM-1289 — the proven "only upper accumulation points" gives floor-isolation (1/14, 1/14+δ) empty unconditionally (δ ineffective), killing THM-1268's horn 2; same at 12 speeds above 1/13. DEMOTED: window finiteness / 12-speed finite-list are CONDITIONAL on Conjecture 1.5 (proven chain terminates in S*₀ = grid-loneliness land, no 1/13 floor — MISTAKE-190 logged for the S401 headline). NEW LEVERS REPLACING (C2): (1a) **characterize G*₀(11) ∩ (1/14, 1/13 − ε)** — the grid-loneliness value set of finite proper subgroups of the 11-torus; accumulation inside the window can only sit at these points, so thinness there gives near-finiteness WITHOUT Conjecture 1.5; repo gridmax/rung machinery applies. (1b) **the paper's sufficient condition for Conj 1.5 is S₂(n) = S(n−1)** — a rank-2 statement; the dusty Route-2 Lean artifacts (LRCRankRigidity/LRCTorusProjection) are aimed exactly there. (1c) death-star's tower = the k = D−1 diagonal of G-K Amended Conjecture 1.2 (Fan–Sun's refutation of 1.1 verified 4/4 as scanner control) — fold into the note-to-authors package.
- **(2) NEW NAMED TARGET — effectivize the bottom of G-K:** their degeneration argument (box geometry/dimension counting) applied only near the floor; ANY effective height bound for the finite list in (1/14, 3/41] composes with HYP-7920's cage (effective below height 258,276 at 12 speeds) and the repo's enumeration machinery. This is the (C4) sandwich: cage = effective/bounded-height, G-K = all-heights/ineffective; the intersection is the well-posed missing piece.
- **(3) Owner decision — literature-facing note:** THM-1288 + the HYP-7920 cage are both contributions TO the S-T program (their §7 conjecture settled negatively with a repaired version proposed; their bottleneck I(13,p,1) mapped to repo theory). A short note to the authors (or arXiv comment) is drafted-in-substance across the S400/S401 reflections; needs owner sign-off, not agent action.
- **Status:** OPEN; (1) is one session and unblocks the strongest retyping of the frontier since the (D,s) coordinates.

## Lead kind-pasteur-2026-07-19-S128c86: the finite-check FEASIBILITY LEDGER + K-ladder universality + THM-1220 re-scope (HYP-7890)
- **Source:** owner-directed history synthesis (reflection the-strategy-crosswalk-two-vocabularies-and-the-goddyn-wong-periodicity-kind-pasteur-S128c86).
- **(i) The feasibility ledger — EXECUTED (S128c87, HYP-7921; ledger doc = 00-navigation/LRC14-FINITE-CHECK-FEASIBILITY-LEDGER-2026-07-19.md).** Result: heuristic-priced compute for the S-T architecture at n=14 is only ~46× their n=13 job ≈ **50 core-years** (18 days on 1000 cores) — compute is NOT the obstruction; the two unpriced walls are (a) the I(13,p,1) improper-tuple sieve (S-T's own named bottleneck; a covering-comb classification mod p = repo territory; pruning-rule table in ledger §4) and (b) the ×7 lift forced by 14 = 2·7 composite (7^13 per surviving tuple; needs |S| ≲ 1e4) — the apex-7 convergence: the computational route dies on the same 7 as every repo analytic route. **ACCEPTANCE TEST EXECUTED (S128c88, HYP-7930): NEGATIVE on level-1 pruning (MRV/repo-cuts factor = 1.06× at p=71, not ≥10³) and level-1 ansatz-completeness fails (99%+ of 260k minimal covers at p=71 unclassified, ×20 growth/prime).** REDIRECTED NEXT STEP: lift-level kill theorems — (a) the k=13 analog of opus-S400's tightness cage (witness quantization at lifted moduli: every certificate rung strictly above 1/14 forces congruence structure); (b) re-aim the LRCMod19/23Spread template at the lifted moduli 2^a·7^b·p; (c) quantify the ×7-lift survivor set |S| empirically at one prime (the |S| ≲ 10⁴ feasibility criterion from ledger §3b). Level-1 search itself is a compiled-code triviality (310k nodes/s in Python, embarrassingly parallel per prime). Also: verify the k=12 prime table against the S-T PDF (the fetched "73 primes to 733" is arithmetically impossible — see ledger §5.1) before citing counts anywhere.
- **(ii) K-ladder universality theorem target:** K_c(N) = {1..N−1} ∪ {cN} attains M = c/(cN+1) at every (N, c) tested (N ≤ 24, c ≤ 8, 0 violations; witness direction is one line; THM-633 proved N=12 kernel-pure). Generalize LRCLadderD1 to all N — puts the whole bottom-spectrum bracket (window = interval between rungs c=1,2) on proved footing. Lean-shaped, one file.
- **(iii) THM-1220 re-scope:** the "unique non-rigid case" claim needs a window qualifier (n=8 and n=20 tie via F₂(N), N ≡ 1 mod 6 = Goddyn–Wong acceleration; script lrc14_ladder_realization_crossN_kps_S128c86). Owner of THM-1220 (opus) should amend; the deeper cross-N law is death-star's D-graded primorial cascade (THM-1285/1273).
- **Status:** OPEN; (ii) and (iii) are one-session items; (i) is one focused literature+arithmetic session.

## Lead opus-2026-07-19-S399: five unabsorbed directions from the full-history synthesis + inbox deep-read (07-14→19)
- **Source:** `07-reflections/lrc14-history-synthesis-patterns-and-reframings-opus-S399.md` (full-arc era map 05-30→07-19, strategy-family table, five-axis triage law, wall census = Wall A [inverse-theorem complex, seven names one object] + Wall B [codex phase-transport, genuinely distinct until a reduction is written] + the n=12 sporadic half-wall). Inbox sweep MSG-1599–1850.
- **(1) EXECUTED (opus-S400, 2026-07-19) — the S-T equality-case mining. VERDICT: statement-level NO (already known — HYP-4096/kps-S1 had recorded on 07-05 that the paper does not characterize extremizers; the S399 recommendation missed that grep, MISTAKE-189); PROOF-level YES — the paper's witness quantization yields the S-T TIGHTNESS CAGE (HYP-7920): every primitive 12-family with M < 113/1466 is dilated-AP-congruent mod every sieve prime not dividing its product; with the (proved) 11-even branch lemma and power-sum forcing this gives tight-locus rigidity + micro-gap (1/13, 113/1466) emptiness to height 258,276 (vs ~200 prior), conditional only on three extraction facts. FOLLOW-UPS: (a) one-session PDF verification pass (checklist in the S400 reflection §8) promotes HYP-7920 to THM-with-citation; (b) Lean: quantization lemma + 11-even branch lemma are kernel-ready; (c) the I(13,p,1) bridge (their named k=13 bottleneck = the repo's rung/q₀/(D,s) theory) is a new joint target; (d) their Conjecture 7.1 is entangled with the (1/14,3/41)/gap program — probe data frozen.** Original proposal: boxeph-S114/MSG-1823.
- **(2) EXECUTED (mac-mini-S123, 2026-07-19) — the metagraph transport + the §5 cycle-holonomy lens, both cashed at instrument level (HYP-7960/7965, reflection lrc14-two-quantizations-dominance-deficit-and-mirror-palindrome-macmini-S123).** (a) The candidate faithful functor Φ(W) (pairwise dominance measure, θ=1/2 cutoff) is BLIND — the DOMINANCE DEFICIT LAW P(a,b) = 1/2 − [mixed-parity]/(2(b²−a²)) makes Φ the recursive dyadic preorder (no upsets, ties exactly at equal ν₂); the isolation statement itself landed the same day by citation (THM-1289, G-K). Residue kept: the deficit matrix lives on the pair hyperbolas ds = b²−a² (the rung lattices), giving the SLACK-PARITY coordinate — odd-slack rungs force mixed-parity strict-deficit straddles; both tight families straddle both-odd (1,13) with deficit 0. New per-rung necessary condition for the rung-kill assembly line. (b) The involution pairing on blocker cycles TYPECHECKS (opus-S399 §5's question): set-valued palindrome law + central column = even speeds at τ=1/2 (LEM-020 depth-1 inside THM-1240) + parity localization, proved and verified 8 families × all carriers. REMAINING from the original lead: the Singer/regular-pole half (PG(2,13) difference-set side) is untouched; the germ-lift mirror-oddness test on codex's THM-1248 address compression is the shaped Wall-B follow-up.
- **(3) Function-field port of deficit→margin** (death-star-S58e/g): archimedean carry vanishes in FF — the natural home for a clean Schur-deficit→margin inequality. Distinct from the abandoned S91 FF attempt (different target).
- **(4) Height-bound citation frame** (mac-mini-S120): Erdős/Jacobsthal (tight families have v_max=2n−Θ(log n)) + Pomerance ("n<v_max<2n−c·log²n ⟹ not tight") — literature sharpening of HYP-7450, never followed up.
- **(5) Hybrid tree×moment inequality** (kind-pasteur c66/c67): pairwise moment LP and spanning-tree bounds are non-nested; a hybrid may beat both. Standalone inequality question, motivation independent of the (closed) r=6 need.
- **Meta (adoptable now, from the synthesis):** the five-axis triage gate (translation-breaking / dilation-orbit / sup-not-mean / signed-tolerant / cross-modulus-or-exact — one line per axis before any new route); residual typing against the wall census (A / B / A-half / justified-new); kind-pasteur's instrument validation gate (a search's negatives count only if it rediscovers known extremals from inside its own range) — canonize as fleet rule + package as an engineering deliverable with the exact-rational straddling-pair analyzer (`lrc14_dms_identity_crosscheck_opus_S399.py` is a seed).
- **Status:** logged; (1) is the recommended next-session pick for any agent.

## Lead death-star-2026-07-19-S59: the cross-N BAND LAW — three falsifiable follow-ups (THM-1284/HYP-7885)
- **Source:** THM-1284 (single-far classification N=6..13 + N=8..11 census: populated N in [6,13] = {6} ∪ {N≡1 mod 6}; emptiness band [8,12] = the non-1-mod-6 stretch).
- **Next steps:** (i) **N=31 test** — first N≡1 mod 6 with 5|3N+2; band law predicts EMPTY (mac-mini-S29: canonical F(31) degrades to floor 1/32); a member would refute the law and reopen the species catalog. (ii) **N=14..19 single-far extension** — the absorption classification runs on per-instance base floors (computed exact M(B), no conjecture needed even where LRC open); predicts {1..17,19,54}=3/59 the unique member at N=19, nothing at 14..18. (iii) **Width-death half of the law** — prove no order-k≥3 species (bordered-AP type) fits the window for N≥8: a quantitative statement about bordered-AP safe-set geometry vs window width 1/((N+1)(2N+1)). (iv) Unify the binder-gate congruences (HYP-4516) with THM-996/997/998's fill-profile/resonant-dichotomy laws into one cross-N gate theorem — the two mechanisms are one (gcd(b,Q)|clearance = unit/zero-divisor dichotomy at the competing denominator).
- **Status:** UPDATED S59t — THE FIGURATE-STAGGER ATLAS (THM-1360): the slope ladder = descending Pisot family (2, φ, supergolden, ...); Proth→Mersenne telescoping (proved); the relational reading (triangular=arcs=tiles, 2^triangular=graphs/tilings, x=triangular the natural exponent); wide zoo comparison (Mersenne/cumulative-Fibonacci convergences; cubic-Pisot; hyperfactorial products). NEW LEADS: (lxi) the two-parameter Vandermonde law — one generating-function statement for the whole (slope s, figurate family) matrix, generalizing opus-S317's polygonal-vs-polyhedral to all slopes; (lxii) is the cubic Pisot 1.75488 (x³=2x²−x+1) a named/tournament constant? and where does the plastic number 1.3247 (x³=x+1) live in the zoo (different slope convention)?; (lxiii) the multiplicative side — the diagonal PRODUCTS are hyperfactorial/Barnes-G class; do their prime factorizations or p-adic valuations carry tournament content (cf. THM-466 v_2 ladder)?; (lxiv) close the owner power-sum triangle's OEIS/closed form (carryover lvii) using the truncation frame.
- **Status:** UPDATED S59s — the n·2^x+1 PROTH UNIFICATION (THM-1355): the two master +1 constants (observer 2n+1, hypotenuse 2^x+1) are the axes of one Proth table; the owner triangle = the THIRD figurate triangle (power-sum, completing opus-S317's polyhedral/polygonal); Moser-break at cubes; cubic-Fibonacci shallow diagonal; harmonic=staircase resistance (HYP-6865); odd-degree=Rédei (boxeph-S146). NEW LEADS: (lvii) exact closed form/OEIS of the power-sum triangle (col j mimics Σk^j with a Pascal-deep correction; candidate A143542 unverified — retry via oeis API/gh when OEIS reachable) + derive its row-sum law 1,3,7,16,39,106,317 as a Vandermonde truncation (opus-S317 machinery); (lviii) is the cubic Pisot 1.7549 (root x³-2x²+x-1) a known tournament/tiling growth constant? compare to pseudo-doubling 2-1/(n-2) and tribonacci; (lix) prove n·2^x+1 odd ⟺ Rédei-parity as a genuine bridge (not just numerology) via the OCF 2-adic digit; (lx) the two rational series as the log/exp shadows — does series 2 = Σ(2^k-1)/k have a staircase-resistance meaning like series 1?
- **Status:** UPDATED S59r — DC1 FRONTIER via tournaments (HYP-8160, reflection dc1-and-the-3-cycle; DC1 NOT claimed): the A1 weight-triple = the oriented 3-cycle (observer = ℏ = the conserved +1, exact teeth); the TWO-LENS strategy (Rédei parity × 2D leading-form bound); three convergent tournament shadows (golden degree mac-mini-S137, cyclic-3 orientation opus-S418, Rédei-odd count) of one hypothetical counterexample. NEW LEADS: (liii) W9 — construct/compute a Rédei-parity invariant e(φ) of A1 endomorphisms (odd mod 2?) via the char-p center Keller map; (liv) W10 — test the ℏ-ladder = Cayley-Dickson descent (floor C↔n=3); (lv) unify the three shadows: can golden-degree ∧ cyclic-3-monodromy ∧ Rédei-odd coexist with {P,Q}=1? (the repo owns the instruments: Zaremba/Hurwitz CF + OCF parity + metagraph); (lvi) the mac-mini convergence (lii) sharpened — is the leading-form propagation length = the Euclid/CF chain length of the degree pair?
- **Status:** UPDATED S59q — THE TWO-VARIABLE CASE (THM-1345, crediting boxeph-S144 + noting mac-mini-S137 concurrent): equivariant JC2 = THEOREM for EVERY C*-action (hyperbolic->linear, elliptic->triangular); det JF = {P,Q} Poisson reframing (C*-action = Hamiltonian flow of xy); full JC2 LOCATED as descent through the weight filtration (base = equivariant settled; step = leading-form propagation = AMS-hard, OPEN — NOT claimed). NEW LEADS: (xlviii) W6 — is leading-form-propagation-failure an H1 class of the graded Poisson algebra? compute at small filtration length; (xlix) W7 — build the ℏ-interpolation M_ℏ (repo θ-flow machine) with endpoints DC1/DC2, midpoint JC2; (l) elliptic finite-cover argument -> Lean-able 'first certified Jacobian-side theorem'; (li) push the filtration one weight past equivariant (2-weight pairs); CONVERGENCE with mac-mini-S137 (golden corner / Lamé-for-polygons / engine-dimension lemma) — their worst-approximable-data frame + my weight-filtration frame are two lenses on the same descent; (lii) unify: is the leading-form propagation length bounded by the CF/Euclid chain length mac-mini measures?
- **Status:** UPDATED S59n — SPORADIC-OR-FAMILY largely ANSWERED (THM-1305): rigid modulo automorphisms (moduli tangent 0, per-row scalar uniqueness); all owner rationals DERIVED (Phi = 4t+6 forced linear, (t*,s*) = (-3/2,13/2), Psi = (1+t)(2+t)); c2 = 0 forces A = v^{k+1}, D = delta v^2 at every k; k=3 natural slice EXACTLY dead, full verdict in flight; W3 refuted. NEW LEADS: (xli) k=3/k=4 emptiness closure (harvest v2-hunt + mod-p scans; if empty: the isolated-species theorem; W2 predicts |det| = k on any found point); (xlii) deg(v) >= 2 and s-quadratic E charts (one session each with THM-1305 calculus); (xliii) COMPLETED S59o -> THM-1320: TRAP confirmed STRONGER — det = −E₀(0)A(0)C(0) (identity, all k), homogeneous cubes can't even be Keller in-class; the −2 decodes; follow-up (xlvi) COMPLETED S59p -> THM-1325: the +1 IS the Yagzhev X (does not hide); the reduction hides the TORUS (collisions must sit in the fixed core of any equivariant cubic-linear model); SURGERY WALL discovered (no single-variable substitution stabilizes — the +1's zero-hyperplane kills the cofactor field; stabilization is composition-shaped, +1 absorbed into tame data); THM-1320 honesty-amended (P3 = det JF(0), trivial), MISTAKE-197; NEW (xlvii): the composition-shaped stabilization — construct explicit tame φ, ψ with φ∘(F⊕id)∘ψ of degree ≤ 3 in dim ~5-7 and READ OFF where the constants land (the machinery exists: multi-step, one session); (xliv) non-equivariant moduli at higher ambient degree (mod_rank-powered); (xlv) Phi-linear-iff-solvable refinement (does c1 AND c0 consistency force Phi linear whenever nonempty?).
- **Prior status (S59m):** — THE JACOBIAN/DIXMIER EVENT (THM-1300): owner's JC counterexample verified exactly; explicit A_3 Dixmier counterexample built + verified; C*-equivariant collision anatomy; dyadic inverse ladder. NEW LEADS: (xxxvi) fiber-parity census of equivariant Keller maps — is the odd 1+2k fiber universal? (joint with kind-pasteur's mod-p/Groebner streams, HYP-8090); (xxxvii) weighted-torus classification of equivariant Keller maps (small weight vectors, validate-then-emit); (xxxviii) JacobianCounterexample.lean — kernel-checked det certificate + collision (hours-scale, stub-priced); (xxxix) extract the dependency-free exact-poly engine (pmul/adjugate/flatness/Laurent/truncated-inverse) to 04-computation/exactpoly.py beside mod_rank (engineering shelf); (xl) the surviving bottom: DC_1 via the repo's 3-cycle/atomic-parity technology (observer-principle dictionary A_k <-> n=2k+1 — speculative, priced accordingly).
- **Prior status (S59l):** — (xxiv) generators launched (4/367 + 6/1271 emitted; builds in flight); THM-1295 (K-ladder floor+seals PROVED, (60,12) verified, residual UNIFIED with the F_D tower); octal n=7 resolved (missing = exactly {7,21}); tower members dyadically collapse (datum for HYP-8015). SURVEYED PORTFOLIO (sub-hour each, unconsumed): (xxv) slack-2 multi-far rung search (opus-S407); (xxvi) θ-flow separation thetas + ranking falsification; (xxvii) c(p) seven-tile excess growth past p=269; (xxviii) moment-blindness decay deg 8-10; (xxix) SC-blowup Ω-operation n≤5 (OPEN-Q-046); (xxx) root-ratio SC-minimization n=7 (OPEN-Q-049); (xxxi) E₈ ω=χ; (xxxii) invisible cospectral pairs → n=8 double-cones; ENGINEERING: (xxxiii) mod_rank to PyPI; (xxxiv) NAME + package the live LRC toolkit; (xxxv) root-cause the P₇ sparse-eigenspace mismatch (omega6_sparse.out) BEFORE any T₁₉ claim.
- **Prior status (S59k):** — lead (xxiii) DONE: the certificate-module GENERATOR (Python-validate → two-file emit → build) produced member_247_exact as a KERNEL THEOREM (4/247 for F_4(61); 181 moduli; checks 348 s cached + assembly 131 s; kernel-pure). THREE kernel-exact first-gap values: 3/23, 4/127, 4/247. NEW: (xxiv) generator runs for 4/367 (~270 moduli) and 6/1271 (~630 moduli, est. 30-60 min checks); deep members want parallel/batched decides. Still open: (xxii) ledger-wire; (xiii), (xvi), (xix).
- **Prior status (S59j):** — lead (xxi) DONE: the per-modulus split works — member_4_127_exact is a KERNEL THEOREM (sSup margin = 4/127 for {1..29,31,120}; 91 per-modulus decides, 92 s, memory bounded; check_4_127 axioms [propext, Quot.sound] only). The Lean-engineering taxonomy for exact-value pipelines is complete (per-modulus Bool decides + Bool-level assembly + lazy reflection; avoid ℤ/Finset instances, monolithic decides, and Prop-membership fin_cases). NEW: (xxiii) a generator script emitting per-modulus sections mechanically → member_4_247_exact and beyond. Still open: (xxii) ledger-wire the members; (xiii), (xvi), (xix).
- **Prior status (S59i):** — lead (xx) DONE (THM-1271 §7): the e-channel certificate format is FORMALIZED — Cert + soundness through the formal Pinch chain + ℕ/List reflection checker + the FIRST KERNEL-EXACT first-gap member (sSup margin = 3/23 for {1,2,3,4,5,7,18}; check_3_23 depends on ZERO axioms). NEW: (xxi) per-modulus certCheck splitting to reach 4/127+ within memory (the 31-speed monolithic decide hit 5.6 GB/7 GB); (xxii) ledger-wire member_3_23_exact. Still open: (xiii) even-N L2 variant; (xvi) tie/gap/degrade unification; (xix) D=15.
- **Prior status (S59h):** — lead (xii) DONE: L1-L3 ALL LEAN-FORMALIZED (LRCRungFloor.lean, 13 declarations kernel-pure sorry-free, incl. the kernel-checked 4/127 floor member_4_127 and the THM-1270 closure pair; the L1 divisibility-certificate form eliminates modular inverses — the template for e-channel certificates). REMAINING: (xx) general-N e-channel law + its Lean certificate format; (xiii) even-N L2 variant; (xvi) tie/gap/degrade unification; (xix) D=15 at N=223092871 (deliberate).
- **Prior status (S59g):** — leads (xvii) and (xviii) DONE (THM-1287): D=12 rung CONFIRMED at N=9699691 (12/116396303 exact, femto-window 5.3e-15, 482 s at 10⁷ speeds — out-of-sample #8; primorials 6..9699690 confirmed, D = 3,4,6,7,9,10,12) and the D=9 NECESSITY sweep passed (1399 families off-progression, zero in-window). The gate law is now supported in both directions across six orders of magnitude. REMAINING: (xii) Lean L1-L3 (the highest-value next step — the lemmas are ZMod-elementary and the per-N ghost certificates would become kernel-checked gap witnesses); (xiii) even-N L2 variant; (xvi) tie/gap/degrade unification (kind-pasteur); (xix, NEW) D=15 (binder 29) at N = 223092871 — ~2·10⁸ speeds, hours-scale, deliberate, no urgency; (xx, NEW) the general-N e-channel law = the last quarter of gate completeness (THM-1271 §4).
- **Prior status (S59f):** — leads (xiv) and (xv-partial) DONE (THM-1276): D=10 rung CONFIRMED at N=510511 (10/5105119, pico-window, 19 s) and the D=9 gate confirmed OFF-diagonal (9/540557 at 60061, 9/810827 at 90091) — out-of-sample hits #5-#7; ghost-evaluator scaling ~linear demonstrated. REMAINING/NEW: (xvii) D=12 (binder 23) at N = 9699691 ≡ 16 mod 23 → predicted 12/116396303 — hour-scale deliberate run; (xviii) the full D=9/D=10 gate sweeps over their non-tower N (does any N ≢ 1 mod L_D open a high gate? predicted no — the L_D congruence is necessary by the branch mechanism, worth one falsification pass); plus (xii) Lean L1-L3, (xvi) tie/gap/degrade unification (kind-pasteur), (xiii) even-N L2 variant — all still open.
- **Prior status (S59e):** — lead (xi) DONE (THM-1270): D=9 rung CONFIRMED at N=30031 (9/270287 exact, 1 second, fourth out-of-sample hit, primorial 30030); composite-skip + tower-closure LEMMAS PROVED (one congruence: N ≡ 1 mod ℓ ⟹ ℓ | Q_D ⟺ ℓ | 2D−1); ghost-enumeration evaluator (O(De) clearance, 142/142 gated) makes any rung computable in ~linear time. NEW: (xiv) the D=10 rung (binder 19) at N = 510511 (= 19·26869 ≡ 0 mod 19, gate OPEN predicted → 10/5105119) — minutes on the ghost evaluator; (xv) the general-N D=9 gate sweep (which N in [8, 10^5] ≡ 1 mod 30030 open — only 30031 and 60061 = 30030·2+1 in range; 60061 mod 17 = 60061−3532·17 = 17·3533 = 60061?? check at run time); (xvi) unify with kind-pasteur's tie side: one theorem for the tie (m=2, F_2 Goddyn-Wong), gap (m ≥ 3, the tower), and degrade (composite/5|Q) regimes of the binder-competition family.
- **Prior status (S59d):** — leads (viii) and (ix) largely DONE (THM-1271; renumbered on integration because THM-1258 was already claimed): (ix) gate completeness THREE-QUARTERS PROVED — rung-floor lemma (closed-form witness a = D·p^{-1}, unique danger slot = the deleted N−1), small-moduli seal (q' ≤ 2N, N odd), packing seal (e ≥ c) ⟹ the (N−1)-ghost channel is the sole escape and exact M(F_D(N)) collapses to ~10⁵ ops; per-N completeness is now a finite certificate (all four members proof-backed); the general-N e-channel law + even-N L2 variant remain. (viii) D=7 rung CONFIRMED at N=2311: 7/16183 exact, in-window (9.4e-8), 3 seconds, third out-of-sample tower hit. NEW: (xi) the D=9 rung (binder 17) at N = 30031 — the pruned evaluator scales linearly, run it; (xii) Lean-formalize L1-L3 (elementary, kernel-friendly; L1 is 10 lines of modular arithmetic); (xiii) the even-N L2 variant (classify the q' = 2N corner's b=2 configuration as the unique even-N escape).
- **Prior status (S59c):** — follow-up (v) DONE: the full D=4 gate is EXTRACTED and CONFIRMED = {N≡1 mod 30, N≢1 mod 7} (THM-1257; members {31,61,91} in [8,100], N=61 predicted-then-found 4/247, N=91 new 4/367); the tower is a PRIMORIAL CASCADE (L_next = L_D·(2D−1); D=6 opens at N=211 = 6/1271; D=5 never — composite binder). NEW LEADS: (viii) the D=7 rung at N=2311 (7/16183) — needs a pruned/segmented exact evaluator for ~2311-speed families; (ix) PROVE gate completeness (the §3 branch lemmas + an all-other-moduli seal — Lean-able; would make every gate member a theorem-grade gap witness); (x) non-canonical species along the primorial progressions (does anything else populate N ≡ 1 mod 30 windows?).
- **Prior status (S59b):** — item (i) EXECUTED same day and the prediction REFUTED-with-discovery (THM-1256): N=31 is POPULATED by {1..29,31,120} = 4/127, the D=4 slack-1 rung (binder 7 at Q=127); the mod-30 gate is the D=3 slice of a D-GRADED gate family with the see-saw 5|3N+2 ⟺ 5|4N+1. Item (ii) partially executed: single-far classified N=14..31 (members exactly 3/59, 3/77, 4/127). NEW follow-ups: (v) extract the FULL D=4 gate condition from the P2 data (N=16 ≡ 1 mod 15 has no member — the see-saw is necessary-not-sufficient) and predict/test the next D=4 opening (N=61?); (vi) census the non-single-far species at N=14..30 (only N≤13 and N=31 have been censused); (vii) the D-graded gate at N=13 for opus's wall: F_4(13)={1..11,13,48} is gate-closed (b=5 alive at Q'=53), so 4/55 needs a NON-single-far realizer or none — aim boxeph's mod-17/19/23 CRT stack there.
- **Status:** UPDATED S59d — leads (viii) and (ix) largely DONE (THM-1271; renumbered on integration because THM-1258 was already claimed): (ix) gate completeness THREE-QUARTERS PROVED — rung-floor lemma (closed-form witness a = D·p^{-1}, unique danger slot = the deleted N−1), small-moduli seal (q' ≤ 2N, N odd), packing seal (e ≥ c) ⟹ the (N−1)-ghost channel is the sole escape and exact M(F_D(N)) collapses to ~10⁵ ops; per-N completeness is now a finite certificate (all four members proof-backed); the general-N e-channel law + even-N L2 variant remain. (viii) D=7 rung CONFIRMED at N=2311: 7/16183 exact, in-window (9.4e-8), 3 seconds, third out-of-sample tower hit. NEW: (xi) the D=9 rung (binder 17) at N = 30031 — the pruned evaluator scales linearly, run it; (xii) Lean-formalize L1-L3 (elementary, kernel-friendly; L1 is 10 lines of modular arithmetic); (xiii) the even-N L2 variant (classify the q' = 2N corner's b=2 configuration as the unique even-N escape).
- **Prior status (S59c):** — follow-up (v) DONE: the full D=4 gate is EXTRACTED and CONFIRMED = {N≡1 mod 30, N≢1 mod 7} (THM-1286; members {31,61,91} in [8,100], N=61 predicted-then-found 4/247, N=91 new 4/367); the tower is a PRIMORIAL CASCADE (L_next = L_D·(2D−1); D=6 opens at N=211 = 6/1271; D=5 never — composite binder). NEW LEADS: (viii) the D=7 rung at N=2311 (7/16183) — needs a pruned/segmented exact evaluator for ~2311-speed families; (ix) PROVE gate completeness (the §3 branch lemmas + an all-other-moduli seal — Lean-able; would make every gate member a theorem-grade gap witness); (x) non-canonical species along the primorial progressions (does anything else populate N ≡ 1 mod 30 windows?).
- **Prior status (S59b):** — item (i) EXECUTED same day and the prediction REFUTED-with-discovery (THM-1285): N=31 is POPULATED by {1..29,31,120} = 4/127, the D=4 slack-1 rung (binder 7 at Q=127); the mod-30 gate is the D=3 slice of a D-GRADED gate family with the see-saw 5|3N+2 ⟺ 5|4N+1. Item (ii) partially executed: single-far classified N=14..31 (members exactly 3/59, 3/77, 4/127). NEW follow-ups: (v) extract the FULL D=4 gate condition from the P2 data (N=16 ≡ 1 mod 15 has no member — the see-saw is necessary-not-sufficient) and predict/test the next D=4 opening (N=61?); (vi) census the non-single-far species at N=14..30 (only N≤13 and N=31 have been censused); (vii) the D-graded gate at N=13 for opus's wall: F_4(13)={1..11,13,48} is gate-closed (b=5 alive at Q'=53), so 4/55 needs a NON-single-far realizer or none — aim boxeph's mod-17/19/23 CRT stack there.

## Lead boxeph-2026-07-19-S130: the MOD-23 NEAR-BIJECTION is an unused pin on every live LRC(14) target (HYP-7880)
- **Source:** owner-directed history review of near-completions ("think 2/19") + exact certificate-rung profile (`04-computation/lrc14_certificate_rung_profile_boxeph_S130.py`, 0 violations on cast + 200 random).
- **Fact (proved, general-p spread lemma = S115/S126 template):** M(V) < 2/p and p∤all v ⟹ residues hit ALL (p−1)/2 antipodal unit-pairs of Z/p. At p=23 this pins: (a) every 13-set in opus's unsettled window (1/14, 3/41) — all 11 pairs hit with slack 2 (or a dedicated multiple of 23); (b) every 12-set in the uniqueness gap (1/13, 2/25) — a NEAR-BIJECTION, exactly one doubled pair (AP12 doubles {±11}, {1..11,24} doubles {±1}). Sharper than the wired mod-19 rung (slack 3–4). S* = {…,38,42} is the paradigm: it defeats arc-criteria but its M = 2/23 IS the mod-23 certificate (unhit pair {±6}, maximizer numerator 4 = 6⁻¹ mod 23).
- **Next steps:** (i) impose mod-23 near-bijectivity + covering{2..13} + mod-17/19 spread as a FINITE CRT feasibility system (mod lcm(8,9,7,11,13,17,19,23)) on the 3/38 depth-minimal target (HYP-7782) and on the D-rung candidates in (1/14,3/41) — D=4 forces s=55 (4/55), D=5 forces s=69, D=6 forces s=83; (ii) intersect with death-star's spread-far-core killer tuples (S58i 5% residual): killers must complete the antipodal covers the 8-speed core leaves open — a codimension count per prime; (iii) Lean: antipodal_cover generalizes from 19 to any prime by the LRCMod19Spread template if the count proves load-bearing.
- **Status:** IN PROGRESS — step (i) EXECUTED (boxeph-S131, HYP-7895): both targets CRT-feasible (no cheap kill); 3/38 pinned (23-blocker-or-1/1191, 25-blocker-or-1/747); sweeps: no 3/38 realizer Vmax<=34, no 4/55 realizer Vmax<=32. Remaining: (ii) death-star killer-tuple codimension count; (iii) Lean generalization; rerun S126's 271 mod-19-evaders against 23/25.

## Lead mac-mini-2026-07-17-S109 (OWNER-DIRECTED, next session): the COMPLETENESS BRIDGE INVARIANT for the n=12 sporadic branch
- **Target (owner brief):** attempt the coverage/completeness theorem that codex-S64 sec.6 names as the sharp missing bridge: *every primitive tight twelve-set must enter one of the already-closed labelled packet languages, or a global invariant must decrease until it does.* This is what would turn the pile of proved reductions into actual sporadic-branch emptiness (HYP-6800/6820, still OPEN).
- **Proposed carrier (codex-S64 sec.6, explicitly NOT yet a proof):** rank a state not by raw maximum speed but by the integer **denominator debt** `n*m - q` on each active pair-sum ruler (THM-668 supplies `mu = m/q`, `q <= 2b`; strict nonextremality forces `n*m - q >= 1`, so the debt is positive and QUANTIZED on the sporadic branch), coupled to **component overlap debt** (wasted comb mass, detected by the labelled component--comb incidence). A viable theorem: every legal insertion either increases denominator debt, decreases the labelled component obligation, or enters an already-closed sheet packet. NEITHER coordinate is monotone alone -- that is the whole difficulty.
- **Assets to build on:** THM-759 (ratio bound `a_n <= n a_{n-1}`); THM-1001 (safe-interval element bound `w <= 2/(13 delta(A\{w}))`, GENERAL -- holds on shallow AND deep, refines THM-759 and is the strictness certificate upgrading THM-751's `>=1/13` to `>1/13`); THM-763 (finite height `sum <= 78^11`); THM-765 (hereditary primitivity + exact tooth/component containment); THM-769 (shallow/deep split + sheet capacity `|F|>=2`); THM-770 (shallow full-residue CSP through lift height 12); THM-774/775/776/836 (deep two-sheet: folded diamond, dyadic descent, height-100, owner shells); codex `LRCSporadicDiscreteCap.lean` (the denominator-gap arithmetic already formalized, awaiting the THM-668 ruler-data and THM-759 completion bridges).
- **Key obstruction found in S109 (design constraint for the invariant):** on the shallow half the single-free-coordinate direction closes uniformly (THM-1001(B), all heights), because eleven coordinates stay in `{1..12}` and pin `delta` from below. The moment TWO coordinates climb together, the second wound speed stitches a fine danger-comb across the first's complement and `delta(A\{w})` collapses to 0, so the safe-interval cap diverges. Any successful invariant must therefore be sensitive to JOINT climb, not per-coordinate size -- exactly the shallow analogue of the deep two-sheet 'moderate middle'. Both halves fail in the same place, which is evidence the bridge should be stated once, above the shallow/deep split.
- **Concrete first moves:** (i) compute the denominator debt `n*m - q` explicitly on the known corners ({1..12}; the wound single-coordinate family; the GW n=13 sporadic as the positive control where the bridge must FAIL) and see whether it separates them; (ii) test whether debt + overlap is monotone under the elementary moves (wind one coordinate, dilate, gcd-descend, peel-max) -- find the move that breaks monotonicity and design the coupling to absorb it; (iii) check the valley picture (`the-shallow-branch-is-a-valley-macmini-S109`) for a potential function: M - 1/13 is 0 only at {1..12} and rises in every direction, but is not directly computable -- is there a debt-expressible lower bound for it?
- **Status:** OPEN, owner-scheduled for the next session. Coordinate with codex (apparatus owner, active on AP-centred scales c>=21) and klein (census, S313/HYP-7310) to avoid duplication.
...
- **Status:** MAJOR FINITE/STRUCTURAL PROGRESS, UNIFORM EMPTINESS OPEN.
  ...
  the equality `2[12]`.  The intermediate 909-row AP-germ frontier, subsequent
  903-row residual, and exceptional mixed-parity fibre are empty at scale one.
  The live shallow obligation is
  ...
  THM-856 replaces the failed mean-density coefficient by the exact Hunter
  tree functional.  Its projective part has uniform strict margin `19/572`;
  the live field is the prefix--comb--comb centered third moment on small-gcd
  maximizing trees, followed by the correlated AP-window residual.  Preserve
  threshold-graph incidence and reciprocal-gcd cost, not merely edge scores.
...
- **Dyadic/height-100/component progress:** THM-775 proves that every
  ...
  grid has a unit numerator deep for the entire possible-lift pool.  Section
  6C strengthens this: every fixed pair of unit endpoint columns is jointly
  killable in the relaxed shell-admissible model.  Continue with a U-dependent
  three-or-more-column signed-lift/divisor incidence, use the structured
  divisor/parity restrictions, or change denominator; do not retry a fixed
  one- or two-column endpoint template.
- **Open shallow theorem:** THM-845 has finished both THM-820 scale-one H5
  ...
  AP-centred `D>1` language finite-decidable as well: `c<=840`, with at
  ...
  THM-862 now classifies the first unevaluated ramified languages, the `c=3`
  bank: `336` two-order-one/four-order-three, `672`
  one-order-one/five-order-three, and `496` all-order-three unit contexts.
  Their unit fibres are exact affine matching codes and give 68 signed sheet
  templates, but exact maxima `1/4`, `7/26`, and `14/65` in one sheet orbit
  prove that this is not a metric quotient.  The exact first layer has 110
  retained roots and 146,912 cap-admissible edges.  The sharded depth-two
  scout is now exact: `14,992,263` second edges cache to `4,307,561` literal
  geometries, exactly one lane is cap-dead, and `14,992,262` reach depth
  three.  A bounded `(R,x_1,x_2,x_3)` prototype now matches raw evaluation on
  1,685,358 depth-three lanes, reduces metric calls, and gives a 1.599x CPU
  speedup on an eight-context root block.  Next shard complete retained-root
  blocks with this transposition and measure the complete-tooth/streaming
  collapse before a full run; then
  evaluate every arithmetic lane by transporting the proof-facing
  action
  ...
  `min D_i<=21<max D_i<=10,584` with `{2,3,7}`-smooth orders and no private
  maximal prime power.  The upper bound follows by applying the cut at every
  adjacent valuation level: the `2,3,7` valuation ranges are at most `3,2,1`.
...
- **Open deep theorem:** rule out the remaining persistent folded covers in
  ...
  `U^-=U\{R}` and `Q=||(x+y)tau/2||+||(x-y)tau/2||`; the exact target is the
  nine-core collar
...
- **r=8 correction and surviving structure:** raw covered-wall count is
  ...
  fixed-span laws.  THM-794 then refutes both universal extent and active-
  period compactness with a repeated full-support packet at `ceil(f/g)=2`.
  Cross-audit shows it satisfies both positive marginal hypotheses, but with
  collapsing margins `1-W_*/g=42/g` and `dist(r,P)=49/(2g)`.  Its ordered
...
- **THM-779/784/788/794 join:** All nine named gaps are nonempty (minimum three intervening
  ...
  active-period normal form is the correct first contraction, but THM-794
  proves that full-support zero-holonomy packets can repeat too.  After
...
- **Edge transport/curvature join (THM-811/812/813):** Centered-CF coordinate copy
  ...
  endpoint extension carries them through `n=6` loop faces.  At the next CF
  step, 51/187 projected coloured edge cells split; the universal action
  carrier is the finer line-reflection orbit `Q_n`, not the projected edge.
...
- **Pull list:** 39 bounded cards in
  ...
  owner-labelled heptagon/endpoint join.
...
### LEAD: the seven-comb wall -- Hunter tree functional survives, global overlap and raw-speed deficit corrected (opus-S312 THM-856; codex-S14 referee)
- **PROVED/FINITE-EXACT:** Hunter/Kounias second-order Bonferroni over a max spanning tree has ideal coefficient `(165-22m')/169`, positive at seven combs and negative at eight. Four generic radius-seven pilot packets are certified non-covering by the exact functional; two consecutive packets are exact failures of this certificate but remain non-covering in fact.
...
- **What remains open (the successor lead):** MEASURE FLOORS — **PROGRESSED at S235 (THM-686) and S236 (THM-687):** bounded windows censused (window-22: μ ≥ 7/858, zero tight); the UNBOUNDED two-scale slices closed beyond explicit V₀ (THM-687: C/V transfer + k ≤ 6 unconditional floor + apex positivity at every k) — the wall now holds on [window-22] ∪ [two-scale slices with μ∞ > 0, V > V₀]. REMAINING: (a) middle-speed families — **DONE at S237 (THM-688):** separated scales = product limit; bounded-ratio middle clusters = rational-ray b-cover limits (product fails by persistent correction; b=2 verified exactly); taxonomy closed, every scale structure exactly computable; still open within it: the general-(a,b) ray evaluator (mechanical). (b) the structural dead-zone lemma (μ∞ > 0 for ALL (P,E) — per-class decidable today, extremal shapes verified positive); (c) Lean: THM-685 + THM-687(B)'s slice argument + the census certificates (boxeph/mac-mini interval stack); (d) V ≤ V₀ bank instantiation per class (kps machinery).
- **(superseded S235 text follows)** MEASURE FLOORS — **PROGRESSED at S235 (THM-686):** the window-22 branch floor is DONE exhaustively (μ ≥ 7/858 on all 31471, zero tight, max q* = 12135 ⟹ branch certified at every modulus with THM-685); truncated Bonferroni REFUTED as the instrument (S₅ explodes 156–172× under relation stacking — census is the instrument for bounded classes). STILL OPEN: (a) Lean-transcribe THM-685 + the census certificates (boxeph μ_L evaluator + mac-mini LRCIntervalBridge stack; compression: only the near-AP tail needs individual interval data — 6-witness-style shrinkage); (b) confirm the 7/858 = mac-mini k=1 engine floor identity (same object?); (b2) CONSECUTIVE-MAXIMALITY — **FULLY SUPERSEDED at S241 (THM-692): the complete two-scale dead-zone theorem is PROVED (7-torsion dichotomy for P ∌ 7 + q* for P ∋ 7); no census residue remains anywhere; consecutive-maximality retired.** (S239 note:** the 13-torsion test-point theorem proves mu_inf > 0 for ALL E at pmax <= 12 (E-quantification eliminated; sharp at 1/91); consecutive-maximality remains relevant only for the wholesale form of the corner 13 ∈ P (P-finite, censused all-positive, per-class decidable — low priority); (c) bounded windows beyond 22 if any assembly leg needs them; (d) the unbounded residual = hpartA/analytic node (unchanged — not a census question).

### SYNTHESIS (opus-2026-07-11-S216): the t≥3 signed cancellation is SUPERSEDED; the crux is a rank-≤5 lattice reciprocal-product sum
- **Source:** 4 concept-cluster scouts + THM-684/685/538/530/657; reflection `07-reflections/the-signed-cancellation-is-support-6-not-t3-opus-S216.md`; `04-computation/lrc14_ap_minimizes_mu_koksma_route_opus_S216.py` (+out).
- **Finding:** the THM-684 t≥3 character-layer cancellation is order-one/non-truncatable (THM-685(B)) and **bypassed** by the transfer THM-685(A); do NOT invest in bounding those layers. Every route (OffLine/Fourier, transfer, density-floor union bound, two-scale μ∞) converges on the SAME lemma: **AP/consecutive minimizes μ** (= SafeMeasureFloor, the single open Lean obligation of `lrc14_of_measureFloor`; HYP-2602/2607/2608). Validated exactly: μ(AP)=0 (tight extremal), μ→(6/7)^13 for spread; Koksma per-pair bound (THM-686(C)) = my LEM-022 one rank down, 0 violations.
- **The live target:** the closing sum `Σ_{n∈Λ°(E)} D7(n mod 7)/∏_j n_j` (HYP-2606/2646, conditionally convergent, ruled by support-6 density R6 per corrected THM-538) is a **character-twisted reciprocal-product sum over the relation lattice** — the higher-rank analog of the t2 hyperbola sum ALREADY FORMALIZED in LEM-022 (`Σ 1/(cdist h·cdist wh)`, rank-1 via P(w)).
- **⚠ REFINED (opus-S217, `the-minkowski-count-is-the-tail-not-the-crux-opus-S217.md`):** the absolute Minkowski count `|corr| ≤ c/(λ₁···λ₆)` is PROVABLY INSUFFICIENT — 3 walls: (1) F3 sign-blindness (Σ|K|=1.773 ≫ margin 0.357); (2) MISTAKE-078 harmonic divergence; (3) EXACT experiment: covol MISSES the short relations (stranger {0..6,400} has covol 400 but corr 0.17, because 1+2−3=0 in the core dominates). **corr splits by height: LOW-HEIGHT CORE (short relations, carries corr, consec-extremal — FINITE census + the PROVED far-element peel THM-546 `|Δ_w|≤(6/49)V/w`) + HIGH-HEIGHT TAIL (dissociated, →0, where the Minkowski count + LEM-022 lift belong, but still SIGNED).** The Minkowski count is the TAIL, not the crux. **Real open object:** the ungapped-wide-non-AP residual, held by the Plat↔Δ entanglement — the multi-D generalization of THM-546's SIGNED 1-D peel (THM-504: Abel across support + Pólya–Vinogradov on the mod-7 char D7), NOT the absolute covolume count. Promote it to the named open lemma.

### THE UNGAPPED PLAT↔Δ ENTANGLEMENT — the open crux of LRC(14)-S3 (opus-S218, reflection the-entanglement-nut-is-the-missed-sector-phase-opus-S218)
- **Setup:** peel w=max E, p0(E)=Φ(E')+Δ_w, THM-700/546 PROVED |Δ_w|≤V(E')/(6w) (signed 6/49 sharper). Gapped closes; UNGAPPED (w~span) has V/(6w)=O(1) but Φ small — the compensation is NOT decoupled = the crux (verified: wide p0≤0.223≪cap_9, margin 0.27).
- **The nut, VERIFIED & isolated:** Δ_w = Σ_{ℓ≠0} ĝ_0(ℓ)·ĥ_ℓ(−ℓw), h_ℓ(x)=f(x)ω^{−ℓσ(x)} (f=p1-indicator, σ=missed sector). The sin-factor gives a rigorous 1.8×; the remaining **8× (measured exactly) is the missed-sector-phase cancellation** (σ(x) ⊥ ℓw) — a 2-D (x,σ) object, the oscillation-side twin of THM-699's zero-mean weight Σ_c D7=0.
- **Clean equivalent (PROVED reduction):** p0(E)≤L_y(E) (THM-534); L_y(consec_9)=0.49288<cap_9 (margin 0.0014). Crux ⟺ "consec maximizes L_y" (HYP-2607), finite functional, no accumulation.
- **Two OPEN inputs that each close it:** (a) V(E')≤C·span (measured C≈4) — closes gapped/multiscale accumulation; (b) shell-gated Δ_w⁺≤2p1/5 (verified B=24; 1/3,3/8,global-2/5 refuted). Use 6/49 + PROVED V≤7Σe; the LARGE margin cap−L_y≥0.044 not 0.0014 (HYP-2840); doublet rung PROVED via Tornheim 12ζ(3) (HYP-2808).
- **PROGRESS (opus-S219, THM-702, PROVED):** the SUPPORT half of the missed-sector-phase cancellation is proved. Δ_b = ∫_{P1}[1{frac(bx)∈sector j(x)}−1/7]dx = −(1/b)Σ_{∂P1}±G_{j}(frac(bx)) — the far-element correction is the missed-sector discrepancy over the p1-region ONLY (127 atoms collapse to one G_j on |A|=1, via H_T=−Σ_{j∈T}G_j linearity + the (1−1)^{|A|−1} telescoping). |Δ_b|≤(6/49)|∂P1|/b. Meets THM-699's zero-mean weight (kps) on the oscillation side. **NEXT (the remaining ~17×):** the p1-boundary EQUIDISTRIBUTION — a Koksma/three-distance discrepancy bound on {frac(b·x_i)} over the p1-boundary points x_i (the signed mean-zero G_j sum). Closes the ungapped/bounded-ratio core. → kps (seven-sector program).
...
**Source:** `01-canon/theorems/LEM-014-p-separated-composed-realization-wide-regime.md`; `04-computation/lrc_composed_realization_boxeph_S1.py` (+.out); reflection `the-object-is-the-equivariant-torus-quotient-...boxeph-S1`.
**Status:** OPEN (three named follow-ups; the chain itself is stated, elementary, and exact-verified incl. P nonempty).
LEM-014 composes `hfloor => explicit lonely tau` in the wide regime `Vmax >~ 2.8*spread(E)` INCLUDING the slow block P (the leg HYP-5710 isolates and every cluster-only piece omits): robust G_P-intersected floor + nearest grid point + gap-centered phi* + G_P erosion. Remaining:
1. **(H1) per-k delta-robust floor bookkeeping:** port the six CLOSED density-floor legs (THM-661/LEM-006 etc.) from `P(W>0)` to `P(W >= 6*delta)` at `delta = 3s/Vmax` -- their PZ/moment proofs bound `P(W >= t)` natively; measured shell cost 0.015-0.049 at wide V vs leg margins +0.086..+0.216, so this is perturbative bookkeeping, not analysis.
2. **Compressed-regime P-leg:** for `V < 2.8s`, good-period existence comes from LEM-012/013/THM-666(grid-port) -- ALL cluster-only. Feed the grid-port a `G_P^eps`-constrained j (the port gives grid-fraction >= mu - TV/(12V^2); intersect with the G_P^eps grid-fraction >= meas(G_P)-2|P|eps - O(1/V) by the same aliasing bound applied to the G_P indicator's clamp) => the composition closes there too. This would make hrefl fully two-sided modulo the tiny-ruler exact checks.
3. **Lean:** the 6-step chain is interval arithmetic over Q; step-4 core = kps-S112 `observer_of_confined`; steps 2,3,6 are new but mechanical. Target file `LRCComposedRealization.lean` consuming `LRCSmoothBridge` + `LRCDriftEmbed`.
**Guard-rails:** carry the covering precondition (HYP-5690); never fold P into an all-13 cluster (kills the scale separation -- the R3-fold/THM-665-corollary/all-13-drift-embed artifact); the V/s ~ 2.7-2.8 boundary is intrinsic (three independent routes agree) -- do not try to push the wide-regime chain below it, route to the dichotomy instead.
### LEAD (mac-mini-2026-07-09-S65, THM-668/HYP-5720): the SCHUR-BUDGET / live-ruler theorem — the finite-geometry form of the a-priori bridge
**Source:** THM-666 (pair-sum ruler theorem, PROVED: the witness always lives at `t = p/(v_i+v_j)`; the Schur-triple kill rule `(v_i+v_j) | v_l` deletes ruler `(i,j)`); reflection `the-object-is-a-winding-circle-touching-the-medial-axis-macmini-S65`.
**Status:** OPEN — the exact restatement is: **LRC(14) ⟺ for every primitive covering 13-set, some pair-sum modulus `q = v_i+v_j` and multiplier `p` put all 13 residues `v_l·p mod q` in the band `[q/14, 13q/14]`.** Covering ⟹ the constant rulers `q ∈ {2..14}` all fail at `p=1` (zero residue); the conjecture = the 91 pair-sum rulers are never ALL dead-or-band-empty. The kill budget is capped by `E₃(S) ≤ 78 = E₃({1..13})` with equality iff dilated interval (opus-S182's extremal target — the two leads compose: E₃-extremal ⟹ kill budget sub-saturated on covering sets ⟹ need "sub-saturated budget leaves a live banded ruler").
**Why this is the right shape:** it is a FINITE residue-geometry statement per cluster (no equidistribution quantifier, no drift, no grids — native_decide-shaped), and it is EXACT (not sufficient-only): the max provably lives there. The known realization discharges (drift embed, scale separation, continuum bridge) are the `q → 2Vmax`, teeth-slow special cases.
**Next step:** (1) per-cluster statistics of live vs dead vs band-empty rulers over the covering censuses (which q hosts? is the hosting q always in `(Vmax, 2Vmax)`? the [1,18] data shows witnesses at q = 23, 24 ≈ 1.3·Vmax); (2) try to prove: covering + primitive ⟹ some pair-sum q with NO zero residue AND a nonempty band orbit (the multiplier group action on residues mod q); (3) wire to boxeph-S1's LEM-014 P-separated wide regime (their Vmax ≥ C·s leg + this for the window). Cross-link: THM-420 (shell-partner = the mod-(2n−1) special case), opus-S182 (E₃ extremal), klein-S207 (ruler kill by observer = the q=Vmax case of the same rule).
**UPDATE (S65 cont., HYP-5730 — the attack ran; the lead is now a two-domain program):** Four certificates PROVED (C0 window = spread13-as-pair-sum-event; C1 gcd-exact ledger; C2 divisor descent; C3 six-pair prime — statements + proofs in the THM-668 addendum). CENSUS: C1 alone certifies 100% of covering `[1,18]`; C2 alone 100% of random covering cap 60; monad-S2's detuned harmonics fall to C2 (k=23/25); pure counting IS defeatable but only at small scale (defeater live-rulers all at `q ≤ 36`; mechanisms: residue collision `23 ≡ 2 mod 21`, and the p=1 window) — while the open annulus (`r > 13`, all sums > 36) is certificate-saturated (250/250 + hill-climbs stuck at ≥ 38 certified rulers). Blocking census: blocked 13-subsets mod q fall from 100% (q=15) to ~6-7% (q=23-26), lex-first classes all near-intervals. **Refined obligations: (a) exhaust covering `Vmax ≤ ~30` in C (bounded absolute domain); (b) PROVE "covering + all pair sums > Q₀ ⟹ some C1/C2 fires" (divisor abundance / representation-richness — the sole remaining a-priori gap on this route); (c) Lean the four one-page certificates, consumed by kps-S114's `LRCPairSumDispatch.mreach_ge_of_pairsum_band`. klein-S209 (HYP-5731) is stressing the k≥8 mid-band stratum + a second-order C4 hunter ledger.**
### LEAD (death-star-2026-07-09-S1): the equivariant partition function Z_n(x; χ) on the signed pair-cube — make the "two halves of one object" surmise a computation
**Source:** reflection `the-abstract-object-one-equivariant-partition-function-two-time-scales-deathstar-2026-07-09-S1` (synthesizing `the-two-indices-...`, `the-generating-involution-...-kps-S67`, `two-geometries-one-algebra-...`, `the-lonely-runner-is-a-random-round-tournament`).
**Status:** OPEN — a concrete small-n test of the project's deepest surmise. Define Z_n(x; χ) = Σ_{T ∈ {±1}^{C(n,2)}} χ(σ-grade, S_n-orbit) · I(Ω(T), x) on the signed pair-cube and check at n ≤ 5 (exhaustive): (1) the x=2 σ-odd/cut projection reproduces H(T) + Rédei parity; (2) the δ = 1/(n+1) σ-even/cycle reading (pushforward along THM-373's runner phase clock) reproduces the lonely measure p₀; (3) the S_n × Z₂ isotypic split separates the two problems (odd half finitizes, even half is a measure). If confirmed, the LRC(14) residual (multi-scale composition, speeds-ratio > 13) is a TOWER-GLUING statement = the Mode-B (Cayley-Dickson) descent on the tournament side — directional guidance for the composition lemmas (boxeph LEM-014, monad HYP-5717).
**Next step:** any agent — one session: enumerate n=4,5 exhaustively, build both evaluations, test the isotypic split. Caveats in the reflection §5 (the δ-evaluation needs the pushforward made precise; the comparator canonicity is an assumption to test).
### LEAD (klein-2026-07-09-S209, HYP-5732): the AGGREGATED MODULAR SUPPLY route — the coordinate with NO realization leg
**Source:** reflection `the-sixth-coordinate-deletes-the-realization-leg-klein-S209`; HYP-5731 (4 exact studies); THM-668 (attainment); kps `LRCPairSumDispatch` (the Lean consumer, sorry-free).
**Status:** OPEN — proposed architecture, load-bearing pieces identified. LRC(14) covering case ⟸ Σ_{q∈(V,2V]} LM(q) > 0 at Vmax > V0, LM(q) = #{p : all v_l·p mod q in the middle band}. A live (q,p) IS the witness (t = p/q, dispatch consumes) — no drift, no snap, no mid-band dichotomy: the HYP-5692 realization node is DELETED in this coordinate, not solved. Main term (6/7)^13·(3/2)V²; corrections: (i) exact relations n·v=0 = the density-floor decorrelation object (CLOSED in measure form; DISCRETE PORT is the crux — Dirichlet-kernel 1/|j| decay ⟹ reuse moment-LP/box, not absolute sums); (ii) ruler-specific n·v = mq (m≠0): each (n,m) pins ≤ ‖n‖₁ moduli ⟹ total O(V) — cheap. V² supply vs O(V) new-error: the first polynomially-widening margin in any coordinate.
**Data (HYP-5731):** supply abundant on the k≥8 mid-band family (min maxLM = 24; density ≈ (6/7)^13); universal supra-Vmax liveness at V ≳ 91 (2882/2882 moduli incl. adversarial + @91) but FAILS at V ≤ 18 (6148/16947 dead) ⟹ [counting above V0] + [native_decide below V0]. C1∪C4 union certificates adversarially breakable to 0 while supply stays fat — do NOT invest further in low-order union ledgers; the certificate must be the full Fourier ledger or the aggregate.
**Next steps (updated klein-S210 — the port is DONE, THM-671):** the discrete decorrelation bound is the QUINTIC BONFERRONI B5-cert (LM(q) ≥ Σ_{p≠0} f₅(C(p)), exact integers, tight at good moduli, fires 100% of tested instances V=30..280 incl. all adversarial killers). Remaining: (1) THM-671 part 6 — a-priori resolved-modulus supply (m≠0 divisor counting + E3-budget-vs-B5-penalty with LEM-015 rigidity at the top); (2) Lean LRCDiscreteBonferroni.lean (decide-shaped) → LRC14GrandAssembly; (3) the (18,30) covering enumeration (kps-S115 pattern); (4) optional: full moment-LP over exact S_d beats Bonferroni corners. Guard-rails: depth ≤ 3 truncations PROVABLY fail at 13 constraints (B3 = −0.099 iid) — quintic minimum; self-hosting refuted; boundary residues must stay CLOSED-band (equality witnesses are real: 2·{1..13}).

### LEAD (klein-2026-07-09-S208, THM-667/HYP-5692): the MID-BAND (V/14, 9V/14) is the entire open content of LRC(14) — attack it on entanglement-native instruments
**Source:** THM-667 (adaptive-split ladder + localization); reflection `the-lrc14-frontier-is-one-node-with-coordinates-klein-S208`; census HYP-5691.
**Status:** OPEN — this is THE node (one node, five names, HYP-5692). The ladder re-split at (9/14)Vmax makes every covering cluster wide (r̃ ≥ 2.8) and the measure floor closes at every stratum (Lemma A: LRC(≤13) Lipschitz fattening, meas(G_T) ≥ 1/(91·maxT), kills the apex-7 wall; Lemma B: |P̃| = 13−k̃ conservation + floors, margins ≥ +0.047). The ONLY survivors at the realization step are speeds in the mid-band (V/14, 9V/14): too fast to ride the 1/Vmax snap window (need slack ≥ 1/14), too slow to join the confined cluster (co-offset > (5/14)V breaks the aliasing window). Mid-band-free covering sets are CLOSED end-to-end (verified exactly, V=280/420).
**Attack axes (prefer instruments BUILT from scale-entanglement, not separation):** (a) **pair-sum rulers** (THM-668: witness lives at p/(v_i+v_j); mid-band pairs give pair-sums in (15V/14, 9V/7) — an unexplored ruler family; HYP-5720's uniform slack σ ≥ 0.034 is first data); (b) **snap-point counting**: good snap points number ≥ (7/6)·∫W·V per period; can ≤ 5 mid-band danger conditions kill them all? Denjoy–Koksma/three-distance on the perturbed AP of snap points — the wobble is the enemy; (c) **the renormalization tower** (HYP-3901): the mid-band is the multi-scale core; the tower loses ~6 runners/level and terminates but the composed version is unwritten; (d) any factor-4 of THM-665's measured sqrt-cancellation (shrinks the window to the drift threshold).
**Guard-rails:** do NOT re-attack via first moments (∫W ≈ 0.115–0.140 < 1/7 structurally — can never certify the 2/7-robust gap, THM-530); do NOT anchor witnesses at Vmax-ruler points (klein-S207: provably impossible); mind the T-zone Bonferroni budget (|P|=5 admits ONE (13,V/14]-rider; |P|≤3 admits two — exact ledger in lrc14_adaptive_ladder_klein_S208).

### LEAD (opus-2026-07-09-S181/S182, HYP-5682/5683): the a-priori density floor as a SCHUR-TRIPLE / SUM-FREE extremal theorem (REDIRECTED from Freiman/doubling)
**Source:** reflections `the-resonance-sum-is-schur-triples-not-doubling-a-symmetry-match-opus-S182` (the redirect) and `additive-energy-is-necessary-not-sufficient-...-opus-S181`.
**REDIRECT (S182):** the density-floor resonance sum `R` (`L=(6/7)¹³+R`) is a theta-sum over the resonance lattice whose MINIMAL vectors are the height-3 **Schur triples `a+b=c`** (norm 3), NOT the height-4 additive-energy relations `a+b=c+d` (norm 4). Measured: the deficit `(6/7)¹³−L` tracks `E₃=#{a+b=c}` (corr `+0.79`) over `E₂`/doubling (`+0.65`); the sum-free `{1,3,…,25}` (max `E₂`, `E₃=0`) is loose — so `E₃`, not `E₂`, governs (symmetry match: loneliness & `E₃` share the dilation-only invariance group; `E₂` is translation-invariant too, so it can't separate the AP from its translate). **So the a-priori route is a Schur/sum-free theorem, NOT BSG/Freiman-3k−4 (`E₂` tools).**
**Two-step target:** (1) EXTREMAL: `E₃(S) ≤ E₃(interval) = C(k,2)` for nonzero `k`-sets, equality iff `S=c·{1..k}` (the interval maximizes Schur triples — additive-triple analogue of Freiman `|S+S|≥2n−1`); (2) THETA-SUM: `|R| ≤ f(E₃)`, `f(E₃(AP))=(6/7)¹³`, `f` increasing ⇒ `|R|<(6/7)¹³` for every non-interval ⇒ loose. Second-order: the Schur-sublattice DIMENSION/coherence modulates `|R|` (why corr is `0.79` not `1.0`) — the near-AP's aligned 1-D triples give larger `|R|` than a GAP's spread 2-D triples at equal `E₃`.
**Next step:** prove step (1) (interval maximizes Schur triples among nonzero `k`-sets — likely known; check the "maximum number of Schur triples / additive triples" literature, e.g. work bounding `#{a+b=c}`), then make step (2)'s leading-order theta-sum bound explicit from LEM-011's arc coefficients. The old `E₂` framing (BSG → 3k−4) is demoted to the second-order dimension control. Cross-link: opus-2026-06-29 (AP maximizes `E₃,E₄`), mac-mini-S25 (relation-lattice kissing = additive triples), THM-515 (theta-sum), LEM-011 (arc coefficients).

### LEAD (superseded framing, opus-S181 HYP-5682): near-AP branch via BSG → Freiman 3k−4 — see the REDIRECTED Schur-triple lead above
**Source:** reflection `additive-energy-is-necessary-not-sufficient-the-tight-locus-is-resonance-geometry-opus-S181`; the Freiman-neighborhood deep-dive (S181) + the structure-theorem scour.
**Status:** OPEN — the near-AP branch (LEM-012 Dirichlet clustering, `longest-AP ≥ k−6`) currently gets its "the cluster HAS an AP substructure" input from EMPIRICAL census (HYP-2638 Freiman-3k−4 pocket enumeration, verified `k=8,9,10`), not an a-priori theorem. The clean chain that would make it a-priori: **Balog–Szemerédi–Gowers** (high additive energy ⟹ a large subset with small doubling) **→ Freiman 3k−4** (small doubling ⟹ contained in a short AP) **→ LEM-012** (short AP ⟹ good period). BSG / Green–Tao / **polynomial Freiman–Ruzsa (PFR, Gowers–Green–Manners–Tao 2023)** appear in the repo ONLY as parked sidecars (`OPEN-QUESTIONS.md`), never wired in.
**Caveat (S181):** additive energy is necessary but NOT sufficient — 2-D GAPs have high `E` but are loose with short longest-AP, and HYP-2889 shows `E`→structure is not scalar-monotone. So the bridge must go through DOUBLING / Freiman DIMENSION (the resonance-lattice geometry), not the energy scalar: BSG delivers small-doubling (dimension-controlled), which 3k−4 needs, sidestepping the energy-insufficiency.
**Next step:** state the finite-`k=13` BSG constant explicitly (does a `longest-AP ≥ 7` cluster provably contain a small-doubling sub-block of size ≥ some `k'`?), and check whether Freiman 3k−4's AP-length bound `|S+S|−k+1` is short enough to feed LEM-012's `L ≥ k−6` hypothesis. If yes, the near-AP branch becomes a-priori (retires HYP-2638's census). Cross-link: HYP-2637 (Freiman dimension), HYP-2638 (3k−4 pocket), LEM-012/013, THM-527 (spread window).
...
1. **Liu-Zhu 2004 Conjecture 2 in general:** `μ({x,y,y-x,y+x}) = (k+1)m/(4(k+1)m+1)` for `x=2k+1, y=2m+1` coprime, `x >= 3`. **LOWER HALF DONE (opus-S147, THM-659): `μ >= (k+1)m/N` PROVED uniformly** via `A = 2x·B`, `B` = `(k+1)` length-`m` blocks step `2m`; fully symbolic (`(2x)^{-1}M` closed forms, `y ≡ −2mx`). **UPPER HALF reduced to a TILING:** `{0,x,y,x+y}` is a 4-clique of `G_M` (the `x=1` block `{i,i+1,i+2m+1,i+2m+2}` generalized); if `Z_N` tiles into `(k+1)m` such cliques + 1 leftover then Haralambis's single-leftover argument gives `μ <= (k+1)m/N`. Tiling verified on 24 instances (`x+y<=32`); **the remaining step is a UNIFORM tiling construction** — the clique is a `{0,x}+{0,y}` parallelogram, and the tiling should factor through the CRT structure of `N` (recursive combinatorics). Closing that closes the 20-year-open conjecture.
2. **Haralambis's 1977 `|S|=3` rigidity `μ = κ`** (Liu-Robinson Conj. 3; Liu 2008 Problem 2): verified only to `max(D) <= 25` (Liu-Robinson 2020); my S144 window-graph did `max <= 18` (state count ~`2^max` stalls). An `|S|=3`-specific avoiding-set encoding (period ≤ `s·2^s` per CGHRS, or the three-distance structure directly) could push the computational frontier well past 25 — and the slab/combinatorial census suggests `|S|=3` is ALL slab (`μ=κ`, since the 3% `μ>κ` exceptions were all `|S|=4` A.3), which is exactly Haralambis's claim — the mechanism (why 3 differences can't force a combinatorial optimum) is the proof target.
3. **`χ_c(G_GW)` — RESOLVED (kps-S76, THM-658):** `χ_c(G_GW) ≤ 27/2 = 13.5 < 14` (explicit verified quasi-periodic `(27,2)`-coloring, increments `{9,16}`). The circular rung is BLIND at GW; linearization `χ_c = 1/M` FAILS. GENERALIZED to the sandwich `1/μ ≤ χ_c ≤ 1/M` + conjecture `χ_c = 1/M ⟺ μ = M` (easy half proved, 11/11 verified). **New live lead:** the defect half `μ > M ⟹ χ_c < 1/M` (turn a Haralambis separation into a sub-`1/M` variable-speed coloring; GW's two-speed winding is the template) — squarely opus's homomorphism-ladder territory.
Guard-rails: the phenomenon `μ > κ` and `χ_c < 1/κ` are BOTH published (Haralambis; Liu-Zhu punched sets) — novelty is in the tight-instance connection and the exact `x>=3` values, not the existence; cite before any external claim (Cantor-Gordon 1973 / Haralambis 1977 / Liu-Zhu 2004 / Liu 2008 / Liu-Robinson 2020).
**Next step:** any agent — target 1(a) (the explicit lower-bound set) is guess-and-verify from the extracted patterns, one focused session; target 2's encoding upgrade is infrastructure that also speeds every μ computation.
...
1. **χ_c(G_GW) — RESOLVED (kps-S76, THM-658): χ_c ≤ 13.5 < 14**, all graph relaxations BLIND at GW (a verified non-rotation `(27,2)`-coloring). Answer to the decisive rung question: the circular rung is NOT the faithful LRC invariant at the tight locus; only the integer χ = 14 meets 1/M. Generalized: `χ_c = 1/M ⟺ μ = M` (the linearization defect = the μ>M locus).
2. **Prove T_6 AP-maximality** (the deep-resonance-tail law): T_6(E) ≤ T_6(AP_13) = 6/539. The event is finite: 13 phases in ≤ 6 arcs of total width < 1/7, pairwise separated > 1/7 — a 6-block simultaneous-clustering event; union-bound over 6-block partitions of E, per-partition measure governed by the gcd structure of within-block differences (common divisors cluster simultaneously on positive measure; incommensurate only on product-small sets). σ-odd mechanism on an affine-INVARIANT statement — the honest crossing HYP-5117 wanted. (T_4 version FALSE — mod-7-structured families; k=8 T_5 has a flat direction at {1..7, far}.) **S145 localization test:** the T_6 event is NOT uniformly at q ≤ 7 — spread+bump2 carries its T_6 mass near q = 12 rationals, two-block partly near q = 19 (GW: all q=6; AP: all q=6). So the union bound must run the FULL resonance ladder with per-family q-range (large-q windows are narrow, ~1/(q·rates), so the ladder converges — viable but heavier than the 6-partition sketch). Data: inline run in session log S145.
...
### LEAD (monad-explorer-2026-07-07-S11, HYP-5157): THE DOMINANCE LEMMA — finite additive-combinatorial core of the k=8 tail floor via the window-averaged correlation calculus
**Source:** HYP-5157 (universal pair lemma + triple atom law PROVED; exhaustive dominance 170,213 shapes 0 violations; per-shape exact certificates RAY ∈ [0.82, 0.98] vs bar 0.675).
**Status:** UPDATED S13 (HYP-5207/THM-651/THM-654). The k=8 assembly motivation is MOOT — THM-651 closed the k=8 leg with pair data alone. What remains live here: the dominance lemma itself is now the degree-3 frontier for the k ≥ 9 legs. Target 1's m=2 base is PROVED (THM-654, midpoint-rank; renumbered from 652); m ≥ 3 = the per-b transportation LP (per-pattern minima overshoot; kps-S74's majorization mechanism is the hint). Target 2 is DONE-and-weak: U₂ = 131/147 exact but ~8× above true E[V²] — the weight-3 J(m,n) tent-product atoms are the real need. Target 3 is re-scoped by the S13 ratio-mixing no-go: positive multi-tent mixtures cannot beat the pure pair tent (mediant ratios; triple tents ≥ 1.49 vs 0.508), so the k=9/10 path is kps-S73's conditional tent (c ≤ 1.7/1.29 — the small-d per-P exact c-table is computable with the S11 atom engine) or the signed game. k=13: PZ-on-V descent min = AP exact 0.27205 (4.8× bar, S13).
1. **The dominance lemma:** M_m(E) = #{triples of E with reduced max-diff ≤ m} ≤ M_m(AP_k) for all m ≤ 7 (k=8: AP vector (12,26,34,46,50,56); m=2 is the classical "AP maximizes 3-AP count"). Equivalent form: triples contained in an (m+1)-term AP. Per-pattern dominance is FALSE ((1,7): two-block 3 vs AP 1) — the coupling across patterns is essential, so compression must be applied to the cumulative counts. Boundary bookkeeping: q' ∈ {9,...,13,16} atoms exceed θ²/7 by ≤ 8.9% (exact wiggle table in lrc14 results S11) — the Abel summation needs the corrected weight table, not just monotone weights.
2. **Second-moment universal ceiling:** derive the exact weight-2 (two-window) truncation constant U₂(k,θ) for E[V²] (the u,v-averaged Bonferroni S2 — all pair-level terms universal by the same substitution as the pair lemma). Gives the explicit spread-side floor μ ≥ (3/7 − Σ₃(E))²/U₂ with Σ₃(E) the exact spread meter.
3. **The dichotomy assembly (k=8):** spread side = B3 + PZ with explicit constants (needs 1+2); structured side = Σ₃(E) > ε forces additive structure (many small-reduced-diameter triples) → finite classification / census — meets mac-mini-S54's W_q window arithmetic from the moment side, and klein-S156's PZ-on-B (adversarial min 0.7122 at AP) from the bisection side. My V-functional versions: PZ 0.5740 / AT3 0.675608 / RAY 0.8214 at AP₈, all exact rationals; AT3 clears the honest bar by 6·10⁻⁴ — the bar sits exactly at the quadratic/cubic boundary (degree ladder answer for kps-HYP-5147(a): quadratic-in-V fails, cubic clears; grade degree in window variables, not pair marginals).
Also: k=13 residual (diam ≥ 76): PZ-on-V clears the bar 4.8× at the AP and ≥ 0.62 on spread shapes — the uniform k=13 statement needs only inf PZ ≥ 0.0565, weaker than R2; candidate route = the same dichotomy with 4.8× headroom.
**Next step:** any agent — prove 1 at m=2 (classical compression, then extend); or derive 2 (one page of tent algebra); or wide-shape RAY engineering (denominator-managed exact Rayleigh — the pure-Fraction 9-width version stalls on diffs ≥ 64).
...
### LEAD (klein-2026-07-07-S154, HYP-4781): the BISECTION IDENTITY's three sup-bound targets — the k=8 leg as ONE window-avoidance inequality, the far-element bisection step, and the LOO-2/7 hereditary floor
**Source:** HYP-4781 (identity PROVED; reflection the-bisection-identity-factors-the-density-floor-klein-S154). **Status:** OPEN (three concrete targets, each with measured slack).
1. **k=8 window-avoidance (closes the binding HYP-2602 leg, 5.4× slack):** prove `sup_{8-sets E} min_j Bis_j(1/7,E) ≤ 0.325`. Since `μ_{1/7}(7-set)=1` a.e. (PROVED), `μ_{1/7}(E_8)=1−Bis_j` for EVERY j; `Ind_j ≤ 1/7` always, so it suffices to bound the alignment excess `Δ_j ≤ 0.182` for ONE j (observed ≤ ~0.08; midpoint-relation-driven, R3_j the leading term). Tool: Erdős–Turán on the θ-middle-window indicator vs the relation lattice through j (pick j minimizing short-relation involvement — 8 elements, rank-7 lattice, pigeonhole); the kps-S5-wf Weyl identity + kps-S59's weight-≥3 deficit frame are the Fourier engines.
   **STATUS UPDATE (klein-S155, HYP-4791):** the leg is now a COMPOSITE — diam ≤ 11 PROVED (kps-S60 intersected ledger); diam 12..26 VERIFIED exhaustive (min μ = 0.9719, +0.30 margin, ~229k affine classes); far case (dist ≥ ~96·core-diam) has an explicit-constant skeleton (measured Bis ≤ 0.0044); residual = diam ≥ 27 non-far. Lemma-target status (klein-S156, HYP-4801): (i) the PAIR-MASS LAW is **PROVED — THM-638** (general rational threshold, both signs, exhaustively verified; the k=8 Hunter floor μ ≥ 6/49 is now UNCONDITIONAL and diameter-free). (ii) RESCOPED: the 2-vertex/Bonferroni-3 route ≥ 0.197 FAILS at the AP (S3 explodes: Bonf3 = −0.134) but holds ≈ 0.22 on spread shapes — since small-diameter is PROVED (kps-S60 ≤ 11) + VERIFIED (band 12–26), the target is now *Bonf3-endpoint ≥ 0.197 for diam ≥ 27*, whose missing lemma is a TRIPLE-MASS UPPER BOUND (the naive residue-constant triple law is REFUTED — 12/107 classes vary; need m123 ≤ θ³ + θΣG_ij/(q_iq_j) + C/(q1q2q3)). (iii) R ≥ 0.75 at k=8 unchanged (adversarial min 0.913; THM-579-shaped). Or: PZ-on-B two-moment route at the full 0.675 bar (adversarial min 0.7122 at the AP, 5.5% margin). The Hunter-endpoint floor μ ≥ 6/49 (diameter-free, k=8-critical) is rigorous modulo (i).
2. **Far-element bisection step (with kps-S59 closes k=13 up to the spread bulk):** prove `Δ_j → 0` (quantitatively, ≤ C·spread(E_j)/e_j-ish) when `e_j ≫ spread(E_j)` — relations through a far element need huge coefficients, so the window-hit is provably near-independent; then `μ_{1/7}(E) ≥ (6/7)μ_{1/7}(E_j) − o(1)` and recurse (kps's diameter floor on the 12-point remainder). **MEASURED (part 3): `Δ_j·e_j/M ≈ ±0.01–0.03` constant over 200× growth — the law `|Δ_j| ≤ C·M/e_j` holds with tiny C; proof skeleton = crossing count (window slopes ≤ 2M vs sweep rate e_j, O(k²M) linearity pieces).**
3. **LOO-2/7 hereditary floor (diameter-free k=13 companion, 2.9× slack):** prove every 13-set has SOME 12-subset with `μ_{2/7} ≥ m_P` (adversarial min-max = 0.165; single 12-sets go down to ~0.04 but 2/7-smallness is NOT hereditary across all thirteen leave-one-outs — the μ_{2/7}-killers are perforated near-APs and thirteen simultaneous perforation constraints are over-determined; a Freiman-type rigidity). **PARTIAL (part 3, proved modulo roof): `n₂* = 37` exactly, so any 13-set with a leave-one-out of primitive diameter ≤ 36 already has `μ_{1/7} ≥ μ_{2/7}(LOO) ≥ m_P` — "12 tame + one arbitrarily far" done; joint residual with kps-S59: full diam ≥ 76 AND every 12-subset prim-diam ≥ 37.**
Also: **k=12 anomaly cross-check** — the sup of `min_j Bis_j` at k=12 is attained by a NEAR-AP not the AP (0.0755 > 0.0563), matching HYP-2780; any per-k proof must not assume AP extremality at k=12.
**Next step:** any agent — target 1 (sharpest; one inequality, proved base, 5× slack); exact-rational Bis engine as infrastructure (AP side already exact via the Farey roof).
...
**UPDATE (same session, HYP-4807, renumbered from 4797):** target 4 TESTED and is ROBUST — CE ≥ 0.2512 over the full adversarial bank (4.4× m_P), S1 ∈ [1.39,1.89], pairSum ≤ 5.29 (max at AP), and the S1-minimizing adversary RAISES CE (self-stabilizing). Reduced analytic targets: S1 ≥ 1.0 (below) + pairSum ≤ 6.5 (above) on the mod-14-pruned lattice ⟹ CE ≥ 1/14 > m_P. Next: exact-rational CE; the pruned-lattice Fourier bound for S1; hand to opus-S134.
**UPDATE 2 (monad-S2, HYP-4817):** the k=13 leg is now SPLIT BY DIAMETER: diam <= 75 CLOSED (tail-diameter theorem, exact mu(AP_76) >= m_P); residual diam >= 76 has 10x slack (min mu ~0.55). New sharpest leads: (5) Lean-formalize the diameter floor (kps-S59 HYP-4797 = monad HYP-4827, cross-validated by two independent exact engines; 3-line monotonicity + one rational ledger inequality -- biggest available GREEN); (6) spread side diam>=76 via Erdos-Turan (10x budget); (7) E[U] >= 1/14 resummation on the affine lattice (layerwise absolute bounds FAIL, need Selberg-majorant or mac-mini-S41 gap-moment ladder).
...
**UPDATE (death-star-2026-07-19-S59, THM-1284):** the unverified middle N=8..11 is now DONE — first gaps EMPTY (validated census, 0.6M–1.7M families/N, rediscovery gates passed), and the single-far stratum is CLASSIFIED for all N=6..13 (exactly two members: the canonical mediant attainers at N=7, N=13 — the HYP-4516 gate solutions proved unique in-stratum). Populated N in [6,13] = {6} ∪ {N≡1 mod 6}: emptiness is a BAND [8,12], not a 12-specific fact. The arithmetic-not-width reading of this lead is CONFIRMED and sharpened to the band law (see the S59 lead at top; N=31 prediction).
...
- **Status:** structure written; NOT yet citable -- the exact J-K theorem + normalization must be read from the full text (only the abstract mined so far).
- **Next step:** fetch/read the J-K full text; state the accumulation principle in our normalization; check whether their finite characterization already computes the relevant relative spectra near (1/13, 2/25); if yes, extract the certificate; if no, prove the compactness/semicontinuity route self-contained (Chabauty + M-semicontinuity).

**LITERATURE LEAD -- JAIN-KRAVITZ "RELATIVE LONELY RUNNER SPECTRA" for the second-value gap (kind-pasteur-2026-07-05-S14, HYP-4167).** Status: UNINVESTIGATED external lead, HIGH potential. arXiv:2411.12684 (Jain-Kravitz): the RELATIVE Lonely-Runner spectrum (over subtori) has "very rigid arithmetic structure" + "explicit finite characterization" for 2-dim subtori -- the closest existing machinery to the loose-branch second-value gap (G: M>1/13 => M>=2/25). NEXT STEP: read it; test whether the 12-runner max-min gap (1/13,2/25) is a relative-spectrum VOID over a low-dim subtorus in their sense (their finite characterization might hand the gap directly). ALSO (same session, web-checked): Sungkawichai-Trakulthongchai LRC(<=13) (arXiv:2604.23906) is COMPUTATIONAL -- does NOT give extremizer-uniqueness (U) or the second value (G), so the loose branch is NOT citable; but their CONTINUITY lemma (non-tight => interval of witnesses) = opus's interval frame, and their POLYNOMIAL METHOD proves (1..k) mod p (p>k^2+k prime) proper = an AP-residue tool. -> HYP-4167, HYP-4151/4152/4157/4166 (the rigidity crux from four sides), result lrc14_loose_branch_structure_and_literature_kps_S14.md.

**THE GAP CENSUS TO 48 + THE PROOF MAP (mac-mini-2026-07-05-S54, HYP-4117).** Status: the crux's swept frontier.
Spectral gap (1/13, 2/25) EMPTY to height 48: 511,947 full-profile survivors in [25,48] (census C harness lrc_gap_census40_S54.c, ~12 min), all witness-cleared; the DOUBLED-PAIR stratum (310,556 shapes, outside every lift sweep) swept for the first time -- clean. THE PROOF MAP (03-artifacts/drafts/lrc14-proof-map-macmini-S54.md): 5-item finish list, one possibly-mathematical item (the absolute-height mechanism). NEXT LEADS: (i) the mod-575 CRT rigidity route (pinning at 23 AND 25 forces near-full pair systems jointly -- scale-window enumeration at CRT resolution; the census DFS extends); (ii) census B=56..64 (each ~8x cost; the harness scales); (iii) opus-S81 descent off-lift generalization. -> HYP-4117, HYP-4116/MISTAKE-105, kps HYP-4115 ladder, the S53 ledger.

**THE MULTI-LIFT STRATA l=3..6 (mac-mini-2026-07-05-S53, HYP-4109).** Status: l=3 CLOSED (13.3B cells, double-verified); l=4 in flight (6-way C, collect via lrc_l4_postpass_macmini_S53.py on scratchpad l4_part_*.out); l=5/6 boxes+corners clean, chain tails ledgered with sharpened anchors (one C session each -- results/lrc_multilift_ledger_macmini_S53.md). The pyramid method (13u orbit windows) + the witness-family crossing (multi-towers q=14r+1 vs singles 13(r+1), coinciding at 169 = the deep well). 7 atom rows GREEN (LRCLiftFloorRows.lean). OPEN: (i) l=4 verdict collection; (ii) l=5/6 tails; (iii) multi-tower ladder closed form; (iv) feed kps-S7 LRCGapLadder the exact chain fractions. -> HYP-4109, HYP-4103, kps HYP-4112/4110, klein HYP-4107/LRCTeethR, opus HYP-4113.

**THE MULTI-LIFT FLOOR IS 2/25 AND THE LADDER LAW (mac-mini-2026-07-05-S52, HYP-4103).** Status: l=1,2 DONE (S52); l=3 DONE, l=4 in flight, l>=7 DONE opus-S78 (see the S53 entry above).
Lift stratum of hdich (residue-pinned tight candidates W = {r+13k_r}): floor = 2/25 ATTAINED at {1..12}\{4,6} u {17,19} (t = 6/25) -- the global n=12 second value realized INSIDE the lift space; single-lift floor 14/169 ({1..11,168}) is NOT the stratum floor. 14r-ladder closed form M({1..12}\{r} u {14r}) = 14/(13(r+1)) r=7..12, witness congruence (13-r)a == -14 mod 13(r+1), binders (13-r, 14r). Single lifts cutoff-complete to 2016; l=2 complete on structural domains (600k sets); height-1 hypercube complete; spectral gap (1/13, 2/25) EMPTY everywhere swept. OPEN: (i) l=3..6 floor-level domains (fee-bounded T_l = 156(13-l)/(13-2l); expect 2/25); (ii) spread l >= 7 height >= 3 residual (fee dies at 2l >= 13; ratio gate + sieve strain lane); (iii) Fin-12 transcription of klein's lonely_of_window_multi = the fee table's Lean home. -> HYP-4103, MISTAKE-104, THM-618, kps HYP-4102/4105, klein HYP-4099/4104.

**THE INTERVAL-CENSUS UPGRADE (klein-2026-07-02-S112, HYP-4017).** Status: NEW, next Lean session-scale piece; design complete.
The covering band has ZERO tight rows (every covering 13-subset of [1..22] has optimum >= 1/12; uniform margin 1/84; the classical tight (1..13) is non-covering, quarantined on the sieve leg) — kps-S17's length-positivity obstruction vanishes on the covering leg. Full design in 03-artifacts/drafts/interval-census-base-klein-S112.md: (1) parametric-radius kernel gate speedOKr (rho=12); (2) re-emit the W=22 band with OPTIMAL witnesses (already computed by lrc14_band_margin_analysis_klein_S112.py, 17s); (3) interval transport through the normalization wiring (sign/sort/gcd; gcd shrinks intervals by <= W, floor survives); (4) hwindow22_interval: every nonzero |v|<=22 family has a lonely INTERVAL of length >= 1/2002 (primitives LANDED: lonely_interval_of_margin, repeat13_lonely, repeat_lonely_interval in LRCIntervalTransport.lean); (5) the length-invariant census+peel wrapper consuming it. CONSUMER: the analytic middle band (kps-S28 union floor W*(r)=112..513 + F3-sharp/joint rate_core) — the interval base is exactly its input. Remaining formal surface stands at {LRCUpTo13} + {CoveringFarLonely 22} (LRC14CoveringFarSurface.lean / LRCWindowData22.lean).
-> HYP-4017, HYP-3974, f3-sharp-derivation-klein-S106.md, reflection the-extremals-live-on-the-easy-leg.
**arXiv:2607.00876 (Bairaktari-Larsen, "The Binary Tree Mechanism is Optimal for Approximate DP Continual Counting", Jul 2026) -- INVESTIGATED, connection found + USED (opus-2026-07-03-S48, HYP-4013).** Status: INVESTIGATED (structural analogy, not subject overlap -- DP/privacy vs Diophantine covering).
The DP lower bound `Omega(log^{3/2} n)` via HEREDITARY DISCREPANCY of the prefix-sum (lower-triangular counting) matrix is the SAME SHAPE as the LRC floor's hard direction: a LOWER bound (where 2nd-moment gives the WRONG direction) via discrepancy of a triangular accumulation, extremized at the uniform/dyadic config, won by CANCELLATION not magnitude. Three contact points: (i) klein's F3-sharp TELESCOPING (HYP-4011) = the binary-tree exact cancellation that beats the naive sqrt rate; (ii) the counting matrix = the staircase's CUT SPACE (base-path arcs = prefix sums = score hierarchy; the Cut+Cycle GF(2) split); (iii) the ell-infinity max-over-prefixes = the worst-position functional `Q_c` = the `M(S)=max_t` sup. USED IT: the DP lens ("the low-discrepancy instance is the hard one") predicted AND closed (R2) to evidence standard (HYP-4013: AP = min star-discrepancy = min floor, strictly increasing in range). NEXT: try the DP proof's `gamma_2`/factorization-norm machinery for the Schur-convexity of `Q_c` (the one remaining (R2) proof step). -> HYP-4013, HYP-4011, S33 floor proof, reflection `the-lower-bound-is-the-hard-direction-lrc-floor-and-dp-continual-counting-opus-20260703.md`.


**SECOND FABRICATED 'SYNTHESIS' POST (mac-mini-2026-07-05-S52): poke-forum/post_1783279275244.md** -- base64 'Roth + p-adic Littlewood + Monsky synthesis'; SAME fabrication species opus-S65 characterized: 'Pi Unital Flower' guardrail phrase + placeholder SHA e3b0c442... (= empty-string SHA-256) + a nonexistent 'LRC(14) Local-to-Global Rigidity Correspondence'. The cited theorems are real (Roth 1953; EKL for pLC quadratic irrationals; Monsky 2-adic Sperner) but the synthesis is LLM-glue. FLAGGED, not ingested; no action needed unless the owner says otherwise.

**FRANEL/DISCREPANCY + FIBONACCI-mod-7 STRUCTURAL BRIDGE + 3 NEW LEADS (opus-2026-07-04-S65, HYP-4074).** Status: NEW (owner steer: Franel discrepancy + Fibonacci mod 7 period 16). The human 'synthesis' post (poke-forum/post_1783144839182.md) is LLM-FABRICATED (Pi-Unital-Flower guardrails, empty-string SHA-256, MD5('hello')) -- flag it; the ONE real kernel is the base-2 van der Corput discrepancy N D*<=log N/(3 log 2) (Bejian-Faure). RECONCILED WITH FRONTIER: (a) THE DISCREPANCY INVERSION -- LRC-tight = the AP = runners on the (n+1)-th ROOTS OF UNITY = PERFECT equidistribution (M=1/14); golden/Fibonacci speeds (classical discrepancy extremal, badly-approximable) are LRC-LOOSEST (M=0.17). So the LRC extremal is the OPPOSITE end from the golden-ratio discrepancy extremal (extends my S48 HYP-4013 'AP=min star-discrepancy=min floor' with the loose end + roots-of-unity characterization; reconfirms tight-locus=AP HYP-4062). (b) PISANO FINITE-PERIOD BRIDGE: pi(2)=3, pi(7)=16=2^4, pi(14)=48 -- the 'infinite family -> finite period' theme that IS the endgame (my mod-24 check, mac-mini mod-14, kps far-peel); pi(2)=3 == the '3' of vdC's 1/(3 log2) == three-gap g(14)<=3. NEGATIVES CLOSED: Fibonacci speeds loose (not extremal); vdC t-sampling finds witnesses SLOWER than random (engineering witness-finder = dead end). **3 NEW LEADS**: (i) a UNIFORM Pisano-like period P (lcm) mod which ALL confinement/tight-locus checks decide, collapsing the residual to ONE finite computation; (ii) the confinement residual = small-tightener x near-AP corner = a two-comb three-gap/discrepancy problem (can 2 low-freq odd combs avoid all high points of g_E?); (iii) formalize 'LRC-tight <=> perfect equidistribution (roots of unity)' as a discrepancy theorem. PROVED this session: THM-615 Lemma 3 (large tightener => M>=1/12, the loose end of general-U confinement; 1600 families 0 violations). -> THM-615, HYP-4013/opus-S48 (AP=min discrepancy), HYP-4062/kps (tight=AP), HYP-2913 (three-gap), reflection the-discrepancy-inversion-and-the-pisano-finite-period-bridge.
...
(3) **LEDGER = CAP ATLAS + the m=4/5 phase transition.** min over bounded m-cores of meas(L) = cap_{13-m} EXACT (same extremizers). The extremizer family flips phase at m=4/5: {1} u top (Dirichlet) vs odd-unit near-AP; greedy-drop nesting holds within the near-AP phase only. Explain WHY the phases exchange at j=9/10 (= THM-576's cap-pattern break) — likely the point where the Dirichlet box beats the AP comb.
(4) **dim = 16 echo + torsion-tube girth law (apex code).** ham8⊗ham8 tensor code on PSL_2(F_7) has dim EXACTLY 16 = opus's dim H^1 (now explained as 8 Frobenius-21 components x H^1(torus) = 2 on the SURFACE complex — but the deg-8 complex is connected, so its 16 wants its own explanation; |A|+|B| = 16?). Torsion-tube law: par⊗par distance <= 2·(min generator order) (b^3=1 -> weight-6, commuting pairs -> weight-8 codewords); inner distance >= 3 kills them (d_ub jumps 6 -> 936). Next: prove dim formula for ham⊗ham on left-right complexes; test q=11,13 (order-3 vs order-(q±1)/2 torsion).
...
(1) **Second Farey layer = cusp curvature.** ANSWERED (klein-S88, HYP-3843): the tie does NOT break at any interior order -- Lambda_AP == Lambda_GW as FUNCTIONS on (1/15, 1/14], r* = 1/15 exact; curvature separates only at/below the q+q'=15 layer. REPLACED BY: prove the final-window overtaking-emptiness lemma (K=0 on (1/15,1/14) for covering sets; band w-v in (14d,15d]) -- the ladder certificate's (iii); see HYP-3841.
...
**EXTERNAL LEADS -- the biquadratic residual & the Cayley-complex certificate (opus-2026-07-01-S27, from owner links).** Status: ASSESSED.
(1) **Annals 2026, 203-2, p.03 "Good Locally Testable Codes" (Dinur-Evra-Livne-Lubotzky-Mozes), via LEFT-RIGHT CAYLEY COMPLEXES -- HIGH PRIORITY.** Exact machinery for certifying cohomology (HYP-3817) + the Cayley transform (HYP-3813): a Cayley square-complex whose cohomology is a LOCALLY-TESTABLE (locally-certifiable) code. Next: build the covering-min certificate as a cocycle on the left-right Cayley complex of Z/2xZ/2=Gal(Q(sqrt-3,sqrt-7)) (two involutions: antipode iota + QR-negation); sqrt21 = the narrow-class-Z/2 = the nontrivial locally-testable class (HYP-3819). https://annals.math.princeton.edu/2026/203-2/p03
(2) **Cornell CS6840 Algorithmic Game Theory (Fall 2025), Sept 12 -- tangential.** Brouwer/Nash equilibrium existence + SOS-for-games / spectral certificates (arXiv 2511.02264, 2602.21722) = the fixed-point + certificate toolkit (HYP-3814 Brouwer/Borsuk-Ulam). Next: check if the SOS-lower-bound machinery for semirandom Hamiltonians transfers to the LRC moment SDP. https://www.cs.cornell.edu/courses/cs6840/2025fa/
(3) **github Pengbinghui/pipeline-math -- methodological.** GPT+Lean prover-verifier pipeline resolving open problems incl. "tiling complements"; mirrors this repo's multi-agent process + codex Lean (THM-346). Next: a model for formally verifying the covering-min certificate once constructed. https://github.com/Pengbinghui/pipeline-math
...
OPEN-Q-108.
**HYP-3528 residual-pair close-tail.**  HYP-3528 executes the random031
proof-contract router and leaves exactly one open terminal clause:
`residual_pair_close_tail`.  Next proof task: prove that after transport
`(23,93,113)` and branch-boundary lift `(147,169)`, every legal downstream
terminal quotient retaining route sidecar `R` still sees residual pair
`(45,173)` and cannot hide it behind residue buckets, owner counts, sliced-box
volumes, raw counts, or vertical `n*2` gluing.  Treat proof clauses and
sidecar obligations as vertices; the preserved predicate is random031 terminal
...
**Next steps:** half-life ladder prediction at order 128 (HYP-2361), now explicitly excluding Smith form and binary rank because MISTAKE-388/THM-482 prove both persist; 2-adic character condition for the {4,32} dihedral family; classify alternating-chain +2 exceptions; hand trans data to mac-mini's 592 thread (THM-453 Part A bridge).
...
### NEW LEADS (opus-S171 cross-thread mining -- good-period / E_grid resonance side)
- **Port THM-076 Walsh-OCF truncation to the covering master law** (HIGH-INTEREST): the LRC master law What(m)=Σ_{balanced n:n.e=m}∏ĝ_{n_i} is structurally identical to the tournament OCF Walsh factorization THM-076 |I[S]|=2^r(n-2k)!/2^{n-1}, which TRUNCATED CLEANLY (THM-071 telescoping halving). My S171 R_abs<(6/7)^k is that same clean truncation measured on the covering side. Next: transport THM-076's explicit truncation constant => an a-priori R_abs<main theorem with explicit Vmax0 (closes kps-S96 E_grid route). The one difference (tournament had no k/7>1 barely-covers obstruction) is where to look. Source: opus-S171, agent-mined.
- **R2 = metagraph 2nd moment identity** (opus-S171/LEM-007:68): is Var(W)=Σ|What(m)|²=R2=Σr_d² LITERALLY the metagraph Var(H) closed form (THM-589, proved concentration CV²~2/n) or only analogous? THM-589's W(n) is an odd-composition/succession count; proving the identity ports the proved 2/n concentration onto the density floor (or explains the gap). Next: match L(AP) additive energy to NUD(n) recursion.
- **INV-050 Satake NDRTs from almost difference sets** (arXiv:2502.12090): nearly-doubly-regular tournaments from almost difference sets, canonical spectrum under Hardy-Littlewood F -- sits in the Paley/flat-spectrum/difference-set world (THM-162/134) that touches the LRC tight-set extremal. IDENTIFIED, not investigated. Fourier-shaped.
- **Riesz-product certificate for inf L>0** (HYP-2540, THM-515:69-97, Bedert 2025): positive-definite test-measure construction tuned to the relation-lattice additive energy; got 1.857->1.41 by hand, stalled short of <1. This is the named route to inf L>0 and is a pure Fourier/positive-definite construction (my toolkit). Next: execute the certificate with the S171 absolute-decay bound as the majorant.

### LEAD (boxeph-2026-07-12-S21): the LEADER LEDGER — conservation law + climb stopping-times as a witness-side instrument
- **Source:** THM-722 (PROVED) + LEM-025 (PROVED) + HYP-6280; `04-computation/lrc14_leader_ledger_boxeph_S21.py`.
...
## Lead mac-mini-2026-07-19-S124: three small follow-ups from the duty-quantization / Hamming-1 flip-spectrum session (HYP-7970)
- **Source:** `07-reflections/duty-quantization-and-the-measured-hamming1-flip-spectrum-macmini-S124.md`; referee `04-computation/lrc14_dyadic_tower_ladders_macmini_S124.py`.
- **(i) Cross-N flip spectra / the softest-flip law:** measure the Hamming-1 spectrum of the tight locus at N = 7, 19, 25 (script generalizes directly). Conjecture: at N ≡ 1 mod 6 (where F₂ ties) the softest non-tight flip is F₃(N) = 3/(3N+2), giving local isolation radius 1/((N+1)(3N+2)) in closed form — provable-looking via the duty frame + kps's binder gate (5∤3N+2 caveat: at N=31 F₃ degrades to floor, HYP-4592 — so the law needs the gate qualifier; N=31 softest flip would be F₄ or a K-rung: measure it).
- **(ii) EXECUTED (mac-mini-S125, 2026-07-19, HYP-7985) — the Hamming-2 / two-far duty-trading sweep.** VERDICT: the complete two-far stratum (S ⊂ {1..13}, |S|=11, fars ≤ 300, proved-necessary duty+rung filters; 804,119 scanned of 2.75M raw) contains NO family with M ∈ (1/14, 3/41) — no two-far 4/55 realizer with fars ≤ 300 (prior caps: 32/34, boxeph). DISCOVERY: every family was beaten by a witness of denominator q ≤ 41 (the gap-top's own modulus) — the SMALL-WITNESS UNIFORMITY, a theorem candidate whose proof closes the stratum at all heights; the axis-5 escape channel (fars ≡ 0 mod lcm(15..41), all small witnesses dead) is pre-registered as the named hole and is a duty-saturated regime for the S124 frame. NEW FOLLOW-UPS: (ii-a) prove the small-witness law per complement (BAD_q residue-pair analysis, 78 complements, finite) + kill the lcm escape channel separately — this is a lift-level kill theorem per kps-S128c88's redirect; (ii-b) the THREE-FAR stratum (|S|=10; where GW-Hamming-2 with 24 retained lives; the S125 resistant census at removals {10,12} names the first targets).
...
- **(i) Attribution hunt:** two web searches found no announcement; retry in coming days (arXiv listing, expected enormous coverage); add the citation to HYP-8090 + reflection when it lands.
...
- [ ] THM-2017 now proves the leading hyper-Bessel limits at `deg h-r deg b=+-r`. Derive the first two uniform `1/m` corrections as differential operators on those functions; compute their common-zero resultants for primitive charge pairs `p0+q0<=6`.
