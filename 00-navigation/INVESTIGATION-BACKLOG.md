...
- **The lens (verified pieces):** char_A(T)=∏char_A(strong component) (exact n≤6); signed partition = Vandermonde = product of sines on the circle; circulant atoms' eigenvalues = Gauss sums (Paley) / Dirichlet=Chebyshev-U (interval). Reductions = decompositions along group characters; the Dirichlet/Fejér kernel is shared between interval-circulant tournament spectra and the LRC certificate.
