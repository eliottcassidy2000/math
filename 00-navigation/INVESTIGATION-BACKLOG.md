...
**Next steps:**
  1. Specialize the fixed-point-free involutive move theorem to nonzero Boolean masks and derive full unordered THM-346 from THM-350.
...
5. Use the new connectivity bridge to separate quotient purity questions inside the SC top bucket from the lower non-SC buckets.
6. Seek asymptotics for the connected-run covers `c_L` and the top bucket.
7. Add `g`, bucket-transition profile, range-parity, SC/top-bucket flag, and interval-cover features to a future tiling-aware TDA extractor.
