...
### LEAD: the seven-comb wall -- CROSSED BY THE HUNTER TREE BOUND (opus-S312, THM-856); residuals = deficit lemma + near-equal packets
- **RESOLVED into THM-856 (same session):** Hunter/Kounias second-order Bonferroni over a max spanning tree is coercive at exactly m' <= 7.5 (24/169 vs 13/169 at m'=7; dies at 8). Pilot: 4/4 generic radius-7 packets PROVED non-covering by one inequality. Residuals named in THM-856 sec. 6: (1) the pairwise deficit lemma |overlap - (4/169)muE| <= C(E)/x_min => small-lift bound X_0(E) => drop to m'=6; (2) near-equal/consecutive packets (Bezout overlap minimum) => AP-window lemma via THM-778 mechanical words.
...
- **Next step:** for route (ii): compute the exact density of a lifted comb D_{r+13h} restricted to the prefix safe set E as h grows -- equidistribution gives 2/13 * mu(E) asymptotically, but the RATE and its arithmetic dependence on r are the lever; a comb whose residue r resonates with a prefix speed has anomalous restricted density.
