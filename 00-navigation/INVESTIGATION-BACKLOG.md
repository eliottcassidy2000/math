...
- **(ii) The Hamming-2 shell:** two defects can TRADE duties — the first genuinely new phenomenon beyond S124's local rigidity, and exactly where a non-single-far 4/55 realizer would have to live (backlog vii). The duty frame prunes the enumeration: a Hamming-2 family in the gap needs its two defects to jointly cover the duties the removed elements carried AND realize a D≥4 rung pair-sum (s = 55: available pair-sums constrain hard). Small exact sweep: all {1..13} minus two elements plus two ≤ some window with the duty filter — feasible size after pruning.
