...
- **Next step:** prove a maximum-spanning-tree lower bound on the seven labelled edge packets `(reduced ratio, common scale, mod-13 sawtooth, endpoint discrepancy)`. The ideal margin is `11mu(E)/169`; the graph lemma must show enough nondeficient projective edges or classify the exceptional ratio graph. Then use THM-778 mechanical words on genuinely consecutive/AP-window residuals. The separate THM-855 F6 event-walk lead remains open but is not the covering bound.
