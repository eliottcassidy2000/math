...
**Next:** (1) replace the scaffold's hard-coded `pi` interval/Taylor backend with a formally sourced Lean/arb-compatible interval backend; (2) store exact sign certificates for the S162 selected rows; (3) lift them from rows to packet-family templates (AP/GW, K33, petal/two-block, covering, few-apex); (4) use HYP-2979 Ramanujan projectors to pre-split late-q packet families; (5) prove a no-bridge quotient lemma: forgotten coordinates are reconstructible, annihilated by a dual certificate, or routed to a named residual/state-lift bucket.  Namespace: HYP-2981 / T1065.
