...
- **Afterwards:** update HYP-2371 (CONFIRMED/REFUTED) + OPEN-Q-013; the run pins C in R = e(1−C/p−…) to ~3 digits and makes R(43) ≈ 2.628 a second-generation prediction. p=43/47 are NOT feasible with these designs (8.6 TB peak layer / 4e15 mults) — needs new mathematics (see T775 Borel-resummation tangent).
- **Status:** OPEN, handed to compute node.

## LEAD (kind-pasteur-2026-06-10-S1): cubic-lens follow-ups (Threads C/D)
- **(a) First forbidden layer (T771):** c3 is gap-free (THM-462); H omits {7,21}. Compute the c4 spectrum from iso classes n ≤ 9 (c4 is NOT score-determined) — where between degree 3 and H does impossibility first appear? Also the (c3,c4) joint spectrum.
