...
1. **k=8 window-avoidance (closes the binding HYP-2602 leg, 5.4× slack):** prove `sup_{8-sets E} min_j Bis_j(1/7,E) ≤ 0.325`. Since `μ_{1/7}(7-set)=1` a.e. (PROVED), `μ_{1/7}(E_8)=1−Bis_j` for EVERY j; `Ind_j ≤ 1/7` always, so it suffices to bound the alignment excess `Δ_j ≤ 0.182` for ONE j (observed ≤ ~0.08; midpoint-relation-driven, R3_j the leading term). Tool: Erdős–Turán on the θ-middle-window indicator vs the relation lattice through j (pick j minimizing short-relation involvement — 8 elements, rank-7 lattice, pigeonhole); the kps-S5-wf Weyl identity + kps-S59's weight-≥3 deficit frame are the Fourier engines.
   **STATUS UPDATE (klein-S155, HYP-4791):** the leg is now a COMPOSITE — diam ≤ 11 PROVED (kps-S60 intersected ledger); diam 12..26 VERIFIED exhaustive (min μ = 0.9719, +0.30 margin, ~229k affine classes); far case (dist ≥ ~96·core-diam) has an explicit-constant skeleton (measured Bis ≤ 0.0044); residual = diam ≥ 27 non-far. Three lemma-sized targets remain: (i) the PAIR-MASS LAW lattice count (m(q1,q2) = 1/49 + min(r)(7−max(r))/(49q1q2), table-exact — elementary, half page); (ii) 2-vertex Hunter union ≥ 0.197 (pair-intersection uppers); (iii) quasi-independence R ≥ 0.75 at k=8 (adversarial min 0.913; THM-579-shaped) — (i)+(ii)+(iii) closes the residual at the R-route bar. Or: PZ-on-B two-moment route at the full 0.675 bar (adversarial min 0.7122 at the AP, 5.5% margin). The Hunter-endpoint floor μ ≥ 6/49 (diameter-free, k=8-critical) is rigorous modulo (i).
