...
**Next:** (1) build a toy Selberg upper-bound sieve over HYP-2963 packet families with variables `lambda_d`; (2) test whether late q packets `{25,27,41}` from HYP-2979 have stable `G(z)` profiles after endpoint labels are attached; (3) compare Gaussian, Fejer, and compact smoothing transforms by retained packet labels; (4) decide whether the analytic role can become a HYP-2901 Node-3 lemma: after small denominators are killed, `G(z)`-normalization plus declared smoothing forces a twist/Fejer witness or routes to HYP-2908 state-lift.
