...
1. **Liu-Zhu 2004 Conjecture 2 in general:** `μ({x,y,y-x,y+x}) = (k+1)m/(4(k+1)m+1)` for `x=2k+1, y=2m+1` coprime, `x >= 3` (only `x=1` is proved, their Thm 4.3). The window-graph gives BOTH halves per instance; the general proof needs two uniform pieces: (a) LOWER — an explicit combinatorial period-`N=4(k+1)m+1` avoiding set (the optimum is NOT a rotation slab for `x>=3`, verified — so it's a genuine construction, gap alphabet 3-6 values; the extracted patterns in the .out are the data; guess-and-verify symbolically); (b) UPPER (the historically hard half) — a uniform Haralambis-lemma bound `S(N) <= (k+1)m` for any avoiding `S` with `0 in S`, i.e. a uniform potential/partition. For `x=1` Liu-Zhu partition `{0..4m}-{2m}` into `m` blocks `R_i = {i,i+1,i+2m+1,i+2m+2}`; find the `x>=3` analog (the potential range grows faster than `2m(m+1)`, not a single Farey rung — the combinatorial structure IS the obstruction). Settling it closes a 20-year-open conjecture.
