...
- the primitive `n=12` sporadic problem is finite and ramification-stratified;
  its AP-centred common-scale H6 faces are closed through scale seventeen
  (with scale thirteen primitive-impossible).  THM-982's scale-eighteen
  at-most-four-feasible-owner deficit has a frozen primary C++ certificate but
  remains provisional pending independent and cross-build replay, so scale
  eighteen is not yet certified; that face, scales nineteen and higher, the
  finite H5 bank, and non-AP-centred/deep-sheet continuations must be closed or
  replaced by a uniform decreasing invariant;
...
|---|---|---|---|---|
| Ramified component-cover/action lens | prime-power upper sets, literal strict-safe components, labelled future comb progressions, last-speed order, common-sheet fibre, exact shortcut witness | deletion/scale/descent semantics if the legal operation is omitted; common large-prime factors if range zero is misread as prime absence; higher-order union if reduced to pairs | complement-lcm fibre, fibre-Hunter/Kruskal cut, operation category, continuation-congruence proof, endpoint ancestry, `Z/DZ` phase action, cover nerve/Farkas dual | THM-857 solves the scale-one H6 fibre; THM-859 proves exact common-dilation conjugacy and identifies `D` as the failed-congruence ramification degree; THM-860 jointly optimizes all 63 relative cuts, excludes every arithmetic row at scales 1,120, 1,008, and 882 by scalar owner capacity, bounds primitive proper AP-centred H6 by `c<=840`, and gives a finite tree for each language; THM-861 solves `c=2`; THM-862 closes all 1,504 `c=3` contexts; THM-957 closes `c=4`; THM-958 empties `c=5`; THM-960 empties `c=6` and exposes an octahedral affine nerve; THM-962 empties `c=7` by a row-product character; THM-963 empties `c=8` with conflict graph `K3,3`; THM-969 empties `c=9` with intersection graphs `3K2` and `2K2`; THM-970 empties `c=10` after passing to projective owner classes, with intersection graph `K6` minus `C6`; THM-974 empties `c=11`, where 64 supports have pairwise-disjoint owner obligations and the two quadratic cosets have intersection graph `3K2`; THM-976 empties `c=12` with complete owner orthogonality; THM-977 empties `c=14` already at the owner-local gate, where every scalar row misses at least two of fourteen sheets; THM-978 empties `c=15`, where no scalar context is locally feasible at more than four owners; THM-980 empties `c=16`, where no scalar context is locally feasible at more than two owners; THM-981 empties `c=17` after scalar capacity isolates the two quadratic color classes and every owner-local maximum is sixteen of seventeen. The changing faithful vertices—rows, owner obligations, projective owner classes, local sheets, then owner-feasibility vectors—are part of the proof, while completed tournaments are telemetry. THM-860 excludes `c=13` by primitivity; THM-858 bounds the finite H5 ramification base. The next legal common-scale H6 face is `c=18`; THM-982's primary C++ certificate gives the provisional owner-feasibility deficit, but independent and cross-build replays are pending, so it is not part of the proved frontier. |
