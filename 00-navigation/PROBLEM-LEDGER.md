...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. In the reduced `(2,3)` seam, THM-4248 closes exact `M=11` off three walls. On exact-`M=12`, `W=0`, THM-4253 leaves `176/104` classes and `1,512` incidences; THM-4260 excludes all `280` specialized gate classes and THM-4265 proves the wall roots simple. THM-4268/4272 give a non-effective incidence collar across `Lambda=0` via `A_23` and ramification `1,2,4,7`. THM-4279 compresses global-Hom equality on `12Q` to target value plus formal-log coefficients `1,2,4,7`; arbitrary local maps are excluded. | Make the collar effective; separately prove raw Keller descent, cross the three coefficient walls, and prove exact-`M=12` seam entry. JC(2) and DC(2) remain open. |
