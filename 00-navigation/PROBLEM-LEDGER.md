...
|---|---|---|
| LRC(14) | Open with `165` rows. THM-2687 caps the full slope-seven family at `12/13` positive carries even under arbitrary labelwise configuration switching. THM-2682/2684 close every unshifted `D(x)={13x}` three-event chain in the complete THM-2584 rail bank, but THM-2689 proves that zero is an exceptional phase resonance: every nontrivial THM-2657 odometer lift restores positive intrinsic three-event support. None globally permutes the seven clock labels, so every physical escape is necessarily phase-local. No row is excluded. | Build the phase-local present/word/unit fibre over a lawful odometer lift, or prove its first unavoidable sidecar failure; do not retry unshifted `D` selectors or slope-seven configuration switching. In parallel pursue AP-core, Euler/phase, and finite rank-box/star exits. |
