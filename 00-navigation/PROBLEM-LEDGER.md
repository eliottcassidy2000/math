...
|---|---|---|
| LRC(14) | Open with exactly `165` first-depth-one rows. THM-2436 empties every deep-`c_3` type. On `nu_7(c_3)<=M`, THM-2452 gives a complete matched mask; THM-2459 retains supplied root service and drift in four atoms; THM-2460 copies a separately retained word. THM-2458/2462 physically realize the remaining uniform-offset atlas, and THM-2464 realizes every compatible delayed clock cell. THM-2461 proves that temporal word, owner root, deep root, and polarized repair are distinct typed data. No scalar profile is excluded. | Retain drift and service in one delayed semantic word on a common owner/deep-root base, or exclude uniform offset using the full global cover, valuation, joint-clock, and paired-dipole constraints. |
