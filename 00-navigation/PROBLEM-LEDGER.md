...
|---|---|---|
| LRC(14) | Open with exactly `165` first-depth-one rows. THM-2436 empties every deep-`c_3` type. On `nu_7(c_3)<=M`, THM-2445 gives `24` positive partial-endpoint cells and THM-2442 restores the ghost's delayed word. THM-2448 expands every fixed current through the omitted right factors into at most `69` typed cospans (`12` for the ghost), but supplies no positive same-root physicalization. No scalar profile is excluded. | Align a generic cell with its semantic word/repair label, then physicalize a matched cospan or convert a typed transition into a lawful positive repair current. |
