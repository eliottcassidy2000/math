...
|---|---|---|
| LRC(14) | Open with exactly `165` first-depth-one rows. THM-2436 empties every deep-`c_3` type. On `nu_7(c_3)<=M`, THM-2445 gives `24` positive partial-endpoint cells and THM-2442 restores the ghost's delayed word. THM-2452 closes the aggregate omitted-factor debt: one of `128` complete co-shifted Boolean masks copies identically to the bare endpoint, all THM-2448 transitions cancel after full-`X` recombination, and a fresh exact `X` and `91`-unit `m` survive. No scalar profile is excluded. | Identify the complete matched literal atom with its semantic THM-2305 owner/repair word and canonical THM-2401 root orientation, or construct the missing lawful intertwiner. |
