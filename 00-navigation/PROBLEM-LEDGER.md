...
|---|---|---|
| LRC(14) | **OPEN.** The fixed-pool lane has residual `22,647`, max `636`, top `(100,636),(256,636),(294,636),(338,636),(372,636),(384,636)`. THM-4283 realizes the complete 640-row top-signature fibre with twelve common decks and a separately typed 9,006-mask carrier, giving 691 deletions. THM-4286 proves inactive-signature response nonfactorization and two disjoint exact 36-row common-deck nodes, but all 72 rows are already in THM-4283's proof union; on the index-396 node, one normalized exact repair margin itself bijectively ranks the 36 rows. THM-4287 proves that the inherited 8,996-mask carrier closes all three endpoint-637 rows, whereas the repaired carrier has exactly 101 failures on `(100,636)` and `(256,636)`. Independently, a delete-two/add-two deck closes the complete 33-row ideal `I_E(p) subset {275,345}`; its one-row carrier overlap gives 35 new typed deletions. The carrier boundary is nonmonotone. Classifier bases, body covers, common decks, carriers, and typed proof-graph unions remain noninterchangeable; none gives physical entry. The `165` relation rows also remain open: THM-2701/2710 close two terminal languages, while the `BABA` carrier lacks target current. | Resolve the two endpoint-636 hostile rows by quotienting the exact 101 carrier failures, and test recursive full signature ideals with complete common-activity intersections. In parallel, prove physical entry or re-root at an exclusive owner while retaining coefficient-two guard debt, phase, and ancestry. |
...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. In the reduced `(2,3)` seam, THM-4248 closes exact `M=11` off three walls and THM-4226 closes dense `M=13`. THM-4290 now closes the complete exact-`M=12` gate `U*Z*D*Lambda!=0`: the 12-fold deck action makes the special component map factor through `C/<tau^6>`, whose saturated degree is divisible by four, contradicting the only responses `34/42`. This removes all hidden-Hom loci from the interior gate. The earlier `W=0` incidence exhaustion remains an independent audit. At `Lambda=0`, THM-4268/4272 give a non-effective collar and the raw `A_23` contact; THM-4279/4280 give exact formal-log observers and sharp faithful `5Q`; THM-4284/4288 isolate common tangent and the last-blowdown defect. THM-4289 proves the full blowdown ladder and the exact loss `conductor/J_f~=k[[b]]/(b^s)`: ambient Kahler descent detects every missing character, while regular dualizing residue data detect none. An actual equivariant wall map on the named component `C` has degree divisible by four, but the resolved Keller degree/allocation interface is not proved there; other stable components are not covered by that statement. | Audit the `Lambda=0` regular specialization well enough to identify the degree carried by `C` and every additional tail; this could activate THM-4290's intrinsic wall corollary without raw fat-contact descent. In parallel, test the explicit Jacobian-ideal class from THM-4289, cross the remaining `U=0`, `Z=0`, and `D=0` coefficient walls, and prove exact-`M=12` seam entry. JC(2) and DC(2) remain open. |
