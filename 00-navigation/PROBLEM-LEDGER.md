...
|---|---|---|
| LRC(14) | **OPEN.** The fixed-pool outsider-pair lane now has exact residual `177,323`, maximum `688`, uniquely `(520,688)`: THM-4266 gives the learned-carrier descent, THM-4267/4269 close `4:5`/`5:6`, and THM-4270 closes 146 current edges on four more primitive rays. This is not physical entry. Separately, the `165` relation rows remain open: THM-2701/2710 close two terminal languages, while THM-2698/2707 show local recurrence below the semantic gate and the `BABA` carrier still lacks target current. | For the fixed-pool lane, attack `(520,688)` and descend the learned carrier without mistaking carrier resistance for danger. For the global lane, prove physical entry or re-root a recurrent packet at an actual exclusive owner while retaining coefficient-two guard debt, phase, and ancestry. |
