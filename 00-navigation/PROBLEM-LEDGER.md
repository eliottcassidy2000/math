...
|---|---|---|
| LRC(14) | Open with exactly `165` first-depth-one rows. THM-2436 empties every deep-`c_3` type. On the complementary `nu_7(c_3)<=M` branch, THM-2445 partitions the graft into `24` positive partial-endpoint cells. THM-2442 restores a fixed positive delayed word to both subbranches of the unique ownerless ghost; `23` repair/blocker-labelled cells and the omitted right-endpoint masks remain. No scalar profile is excluded. | Align one of the `23` typed cells with its semantic terminal word/repair label, then restore the fully masked endpoint without losing the typed coefficient. |
