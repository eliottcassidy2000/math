...
|---|---|---|
| LRC(14) | **OPEN.** The fixed-pool lane has residual `23,373`, max `644`, top `(220,644),(256,644),(258,644),(294,644),(366,644),(416,644),(512,644)`. THM-4282 builds the complete order-free inactive-signature atlas of THM-4281's residual, proves an exact signature-surgery iff, combines three complementary rebuilt decks with a separately typed 45-mask carrier augmentation, and removes an exact 850-row union. The classifier basis, body covers, common decks, and carrier are explicitly noninterchangeable. THM-4281's earlier frozen deck/carrier union removed `148,099` rows; THM-4278 separately classifies a former minimum-one repair family. None gives physical entry. The `165` relation rows also remain open: THM-2701/2710 close two terminal languages, while the `BABA` carrier lacks target current. | Attack the seven endpoint-644 rows by their full inactive-signature fibres and exact exposed-body response hypergraphs; compare deletion exchanges with the dormant carrier before another global scan. Look for a bounded recursive law for signature fibres across adjacent endpoints, while treating the 216-mask hostile classifier only as a router. Globally, prove physical entry or re-root at an exclusive owner while retaining coefficient-two guard debt, phase, and ancestry. |
...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. In the reduced `(2,3)` seam, THM-4248 closes exact `M=11` off three walls. On exact-`M=12`, `W=0`, THM-4253 leaves `176/104` classes and `1,512` incidences; THM-4260 excludes all `280` specialized gate classes and THM-4265 proves the wall roots simple. THM-4268/4272 give a non-effective incidence collar across `Lambda=0` via the `A_23` contact. THM-4279 gives a four-channel complexified formal-log observer; THM-4280 sharpens the actual integral Hom lattice to either minimal triple `{1,2,4}` or `{2,4,7}`, proves exact ramification spectrum `{1,2,4}` and a sharp faithful `5Q` restriction, and forces degree-`34/42` maps to be nonconstant already on `2Q`. This arithmetic compression fails after scalar extension and on arbitrary local maps. | Exploit the shorter honest-map contact obligation only after proving raw Keller descent or contact-order preservation; make the collar effective, cross the three coefficient walls, and prove exact-`M=12` seam entry. JC(2) and DC(2) remain open. |
