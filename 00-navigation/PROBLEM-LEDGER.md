...
| Tournaments | Join laws and strong-core localization are proved. THM-2221 gives the pinned cut-metric transport term; THM-2242 identifies the exact complement-histogram Wasserstein deficit, permanent/Hall antipode certificate, and dual. The core kernel remains. | Optimize the transport dual jointly with low `G(X)` and unpinned exchange; separately resolve GLMY conventions and the positive-kernel support front. |
