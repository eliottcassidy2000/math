...
|---|---|---|
| LRC(14) | Open with `165` rows. THM-2559 bypasses every blind necklace and produces positive primitive old repair mass; THM-2563 retains both moving blocker--graft dipoles but lacks the fixed-head residue. THM-2565 gives the changed selector a positive stationary same-root future return with floor `rho^2/26`. THM-2568 proves that a complete danger-to-safe common-twist overlap nevertheless cancels in every coarse target character after the full-`X` sum; refined pair drift lives in a `156`-dimensional forgotten-fibre kernel. THM-2569 conditions the paired corner on the stationary diagonal, proving positive common support and floor `7rho^2/913952`, but the Hall root is a spectator and the full-`X` no-go survives root-stratumwise. No row is excluded. | Stop before the annihilating full-`X` pushforward: construct a covariant old head and retain a lawful fixed `X`, normal/jet weight, or physical oriented reference on THM-2569's common packet. |
...
- **Fine structure** [PROVED]: full ℂ*-torus (collision = λ=1 slice of an orbit,
  ...
  discriminant multiplicities and higher image-component counts remain open.
