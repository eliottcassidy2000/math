...
|---|---|---|
| LRC(14) | Open with exactly `165` first-depth-one rows. THM-2426 empties the final subtop-guard lane. On the deep-`c_3`/top-guard side, THM-2430/2431/2432 leave exactly `(1,5,1,8),(2,5,2,9)` at `M=0` and `(2,5,1,8)` at positive depth; `c_3<=M` has a separate noncirculant top graft. No scalar profile is excluded. | Build the punctured common-`91` atlas for the three blocker-bearing types, retaining top-blocker cosets, blocker ownership, cross-base endpoint evolution, and the valuation-zero label. Separately prove owner-conditioned survival for the `c_3<=M` graft and land a signed terminal current. |
