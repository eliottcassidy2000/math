...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. THM-2633/2655 reduce live quartic monodromy to `A4/S4` plus a rank-two `V4` Kummer carrier on the full resolvent normalization. THM-2681 excludes the actual THM-1310 cubic as an exact quartic `S4` Keller resolvent. THM-2685 identifies the quartic even-weight divisor-parity code and maximal unramified quotient; THM-2686 proves coprime invariant Kummer vanishing, so every cyclic cubic normalization has only standard mod-two planes and the `A4` gate reduces to scalar nonvanishing in units or `Cl(R)[2]`. This is not an `A4` exclusion. At degree 22, THM-2671 empties all support-three strata and THM-2683 all support-four strata; only full support remains in the inherited genuine nonsplit branch. | For quartics, compute the THM-2685 boundary matrix, units, and `Cl[2]` on other full cyclic/S3 resolvent normalizations; do not rematch discriminants. For degree 22, attack the full-support four-parameter tail: saturate its terminal projective-toric intersection and stack order `N=12`. Separately address split/even edges, integral raising, branches outside THM-2262, and THM-2240's DC axes. |
