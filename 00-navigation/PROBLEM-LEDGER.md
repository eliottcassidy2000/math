...
|---|---|---|
| LRC(14) | **OPEN.** THM-4326 closes the fixed thirty-label pool after any two positive outsiders by a complete rank-two wall audit. THM-4330 identifies the intrinsic minimum-valuation graph as `K_(m,14-m)`, proves the `1+13` primitive parity root safe for every prescribed runner, and makes `2+12` the first unresolved root type. In a degree-two collision-free seam, every integral positive-rational projective body with at most two labels outside the fixed pool closes, so a counterexample there needs at least three outsiders; re-referencing remains forbidden. | Work the anchored `2+12` split without changing runner: separately attack its two degree-twelve minority anchors and the degree-two bodies that fail both the projective pool sieve and the actual-ratio Haar test. Retain signed relatives, midpoint collisions, scale, and component owners. |
