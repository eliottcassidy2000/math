...
| Mixed binomial additive bases | THM-4026 refutes Sun's `2-4-6-8` conjecture at `896315812331399` by three exhaustive exact routes. THM-4027 proves every residue modulo every modulus is represented, so the failure is global/height-sensitive. THM-4028 proves the ordered summatory count `V X^(25/24)+O(X^(11/12))`: growing mean and complete local support still do not force pointwise coverage. The target has an exact constrained `A^2+3B^2` fibre, low-pair character holes, and a rank-eight combinadic carry state. | Determine the least exception and exceptional-set density; seek a singular-series lower-tail/variance theorem, low-density CRT progressions with bounded lifts, a minimal formalizable prime cover, constrained norm-intersection criteria, and a confluent rank-labelled Pascal carry obstruction. Generalize the necessary-but-not-sufficient reciprocal-degree threshold to other mixed tuples. |
