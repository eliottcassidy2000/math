...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. In the reduced `(2,3)` seam, THM-4248 closes exact `M=11` off three walls. On exact-`M=12`, `W=0`, THM-4253 leaves `176/104` classes and `1,512` incidences; THM-4260 excludes all `280` specialized gate classes and THM-4265 proves the wall roots simple. THM-4268/4272 give a non-effective incidence collar across `Lambda=0` via the `A_23` contact. THM-4279 gives a four-channel complexified formal-log observer. THM-4280 gives integral minimal triples `{1,2,4}` and `{2,4,7}`, ramification spectrum `{1,2,4}`, sharp faithful `5Q`, an exact norm-shell criterion, and sharp `3Q` on every degree `8s^2` shell; degree `34/42` is already visible on `2Q`. This compression fails after scalar extension and on arbitrary local maps. | Exploit the shorter honest-map contact obligation only after proving raw Keller descent or contact-order preservation; make the collar effective, cross the three coefficient walls, and prove exact-`M=12` seam entry. JC(2) and DC(2) remain open. |
