...
| Tournaments | [THM-4463--4466](../05-knowledge/results/tournament_continuation_20260908.md): homogeneous-pair forced-contact floors, checksum output sheets, weighted shear/contact laws and sharp cubic bounds on the stated substitution class. Join and strong-core laws remain proved. THM-2290 makes the endpoint-colour matching kernel hafnian-complete, with universal Pfaffian gauges only at orders `2,4`; THM-2294 gives locally transitive Plucker signs. THM-2501 retains signed-`C4`/Gram energy; THM-3392 gives sharp `Q<=B<=2Q` with ternary-support sidecar. | Resolve residual `G-F_R` on homogeneous swaps; extend the weighted cyclic norm bound beyond its proved grammar. `H>=disc` remains OPEN and is distinct from G. Beyond scalar bipartite norms, retain ties, gains, endpoint colours, support partitions and edge amplitudes. |
