...
|---|---|---|
| LRC(14) | Open. THM-2199 forces rank twelve by explicit height `78*182^13` but does not enumerate the finite locus. In scalar `5+3`, THM-2233 and THM-2239 reduce the exact valuation ledger to `166`: all `165` first-depth-one rows with deepest depth at least five, and `(3,4,5)`. THM-2239 closes `(4,6,8)` for unrestricted normalized cores and identifies the constant time-zero charge `Z_(1,0)=1` as the first-depth-one information loss. | Join THM-2234's private owner/expansion sidecar to the signed charge at time zero; sharpen cross-core incidence on `(3,4,5)`; separately prune THM-2208 and the explicit rank-twelve box. |
...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. THM-2230 computes the exact target-shear response fiber; THM-2240 proves the grade-six Weyl response quotient is not a one-step continuation state; THM-2241 gives the degree-sharp monic tail test and resultant nonproper equation. THM-2245 forces every nonsplit terminal degree-fourteen survivor onto an explicit singular spectral quartic. | Normalize THM-2245's conic and analyze the branch divisor of its residual `T`-double-cover, then apply the retained Keller one-form; retain THM-2240's next-grade gauge axes while testing attached-rung termination; separately classify split/even short edges. |
| Tournaments | Join laws and strong-core localization are proved. THM-2221 gives the pinned cut-metric transport term; THM-2242 identifies the exact complement-histogram Wasserstein deficit, permanent/Hall antipode certificate, and dual, and its prefix tax closes the full pinned split response for transitive common quotients. | Classify genuinely nontransitive `G(X)` beginning with `C_3/C_5`, then optimize unpinned core/context exchange; separately resolve GLMY conventions and the positive-kernel support front. |
