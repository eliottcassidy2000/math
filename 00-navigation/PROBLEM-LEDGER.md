...
|---|---|---|
| LRC(14) | **OPEN.** The fixed-pool lane has residual `22,529`, max `632`, top `(100,632),(256,632),(282,632),(294,632),(306,632),(320,632),(332,632),(366,632),(416,632),(512,632)`. THM-4283--4287 supply the inherited typed carrier/common-deck proof graph. THM-4295 proves that the exact row-aware rank-eight append-only minimum for THM-4287's 101 endpoint-636 failures is `14`; a downstream-optimal 9,020-mask carrier closes all complete live layers through endpoint `633` and leaves `37` failures at `632`. Body `1d106401` at `(256,632)` has no rank-eight response at all, so increasing that append-only carrier budget cannot continue the descent. Three separately typed recursive signature-ideal decks close `36,21,54` rows with exact replacement minima `2,4,4`; with the ten-row carrier node their union deletes `118`. Classifiers, response atlases, body covers, common decks, carriers, and typed proof-graph unions remain noninterchangeable; none gives physical entry. The `165` relation rows also remain open: THM-2701/2710 close two terminal languages, while the `BABA` carrier lacks target current. | Change operation at endpoint `632`: test recursive full signature ideals or a different body-cover rank/consumer against the zero-response witness, while retaining exact activity and overlap sidecars. In parallel, prove physical entry or re-root at an exclusive owner while retaining coefficient-two guard debt, phase, and ancestry. |
