...
|---|---|---|
| LRC(14) | Open with exactly `165` first-depth-one rows. Rank six is forced by height `526`; THM-2302/2305/2306 retain one owner, exact word, root character, fixed/first-collision shell, and signed current service. THM-2304 separates at least `168` deepest boundary currents. THM-2309 gives every owner a full-lattice target-plane pivot, and THM-2310 composes every finite pure-edge path at support level after bounded waits. No profile is excluded. | Land one signed terminal-word current in the correct pure-axis/fork quotient class at the original clock, or prove a nonzero coloured THM-2304 current. Rank, residue, uncoloured connectivity, and delayed support mixing all lose the required phase. |
...
| Reciprocal sequences | Support and indexed profiles differ by collision tax. THM-2352 proves q-adic prefix support has abscissa zero, but indexed plateaux realize every abscissa in every finite cylinder. | Build a support operation calculus and retain multiplicity or an exact terminal sidecar when connecting to LRC residues. |
