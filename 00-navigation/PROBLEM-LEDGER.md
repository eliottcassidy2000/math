...
|---|---|---|
| LRC(14) | Open with `165` rows. THM-2569 gives positive old-head/future-root/paired-dipole common support and floor `7rho^2/913952`, but full-`X` completion still annihilates target characters rootwise. THM-2573/2574 identify the missing Abel tooth mode. THM-2581 rebuilds the owner-clock host on the `{b}`, depth-five fibre; THM-2584 retains its arrival/deep roots and proves an exact positive two-rail path, full joint Fourier support, and a signed endogenous `theta=t-2v` current with every nonzero collision colour. No row is excluded. | Transport the THM-2584 rail/reference onto THM-2569's common packet and prove a typed map from its deepest root to the lawful target co-shift or THM-2334 relation residue before full-`X` annihilation. |
