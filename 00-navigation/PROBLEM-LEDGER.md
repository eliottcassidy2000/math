...
|---|---|---|
| LRC(14) | Open. THM-2051--2169 force finite sparse-relation atlases; THM-2168 leaves only rank-one scalar `5+3` inside the three-blocker `(3,5,0)` profile. THM-2179 gives defect-six/greater body relations, while THM-735 and THM-2184 close fixed-core far cones and endpoint-grid tubes. | Convert the body relation into a phase-preserving far-cone/grid-tube reduction, close scalar `5+3`, and discharge the remaining finite boxes without losing owners, phase, or torsion. |
...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. THM-2181 closes monic depressed quartics. THM-2158/2180 reduce the twice-odd nonmonic quartic to the single pole congruence `V|(4gamma-(beta/V)^2)`. | Force or descend that congruence, then supply the nonmonic terminal monicization/quadratic-member step; separately exclude terminal short edges and optimize Weyl termination. |
...
| Metric composition / flows | THM-2176 identifies the universal Gordian continuation profile, min-plus kernel, and positive interaction cocycle; THM-2177 refutes Goemans' exact unsplittable-flow cost conjecture by a planar `K_3` stable-set facet. | Find positive knot translation catalysis beyond pure bypass, and classify which conflict-graph facets control stronger planar flow gaps. |
