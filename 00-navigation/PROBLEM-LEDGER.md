...
|---|---|---|
| LRC(14) | **OPEN.** THM-4326 closes the fixed thirty-label pool after any two positive outsiders, and THM-4329 completes its thirty-two-label Pascal chart. THM-4330 identifies `K_(m,14-m)`, closes `1+13`, and leaves anchored `2+12` first. On a degree-twelve minority anchor, THM-366 forces `420|h`; half-turn, unit, and divisor tests refine the residual, and an `r_5=5` survivor must satisfy `max(T)<9max(C)`. Joint safe rows defeat all marginal banks; restored endpoint slack closes one while collisions raise its free-sheet count to three. The degree-two projective sieve closes at most two outsiders without re-referencing. THM-4331 gives endpoint escapes and the `1/189 -> 27(C-1)` appender; THM-4332 is the literal boundary. THM-4333 proves `L_3/D>2/27` only on the `181,194`-pair remainder and supplies cofinal third-outsider/two-sheet families. | On the `420|h` minority branch, work exact two-sheet owner covers and equality-wall metric boxes, retaining unit/divisor pruning. Separately test rank four against the forced `7/81` two-appender target on `(50,70)` and `(509,640)`, compute endpoint addresses on the latter, and attack degree-two bodies failing both THM-4330 tests. Retain the prescribed anchor, signed relatives, midpoint collisions, scale, endpoint owners, and residual-pair qualification. |
...
| Jacobian / Dixmier / Poisson | **OPEN.** THM-4290--4327 close the `M=12` interior, corner, and generic endpoint lanes by good-differential order. THM-4334 also closes `Z=beta_11=0`, `U W zeta_3!=0`, including its `A_23` contact; the new cyclic-nine face has genus three and order 34. THM-4328 makes Student visibility even-lossless/odd-zero. | Audit `U=0` with `WZ=0` or either repeated cusp, and `Z=0,U!=0` with `zeta_3=0` or `beta_11=W=0`; the broad primary orders stay positive, but contacts and seam entry remain. |
