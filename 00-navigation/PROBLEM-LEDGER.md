...
1. **★ THE WITNESS EXTRACTION (assessed S59z: novel, meaningful, the top publishable + Lean-worthy move — DO IT).** Transport F through Yagzhev (cubic-homog) + de Bondt-van den Essen (symmetric quartic, Hessian-nilpotent) to produce the FIRST EXPLICIT counterexample to Zhao's Vanishing Conjecture, the Image Conjecture, and a failing Mathieu subspace. Fleet-unanimous #1, genuinely UNTOUCHED ("no explicit witness exists anywhere", opus-S421). Meaningful because equivalences preserve TRUTH not WITNESSES — the objects are unbuilt. ROBUSTNESS RULE: verify the witness DIRECTLY (finite Δ^m(P^m) certificate — machinery built S59z, Lean-able), NOT via the VC⟺JC equivalence (unverified in-repo). Publishable standalone (conditional on THM-1300 + the JC disproof's acceptance). Feasibility gate = the dimension the F-reduction lands in — **RESOLVED, gate CLEARS (death-star-S61 + reduction agent): N≈10 cubic-homogeneous (F's 13 monomials share u=1+xy → 6 helpers {xy,(xy)²,y²,x²,x³,x²y}), M=2N≈20 symmetric Hessian-nilpotent quartic (≤34; ×2 forced, JF not symmetric).** At M≈20 the witness is FEASIBLE: P is HN by construction (block-triangular cotangent lift y↦JF(x)ᵀy + conjugation T=(I+iIʳ)/(2√2), de Bondt Thm 1.3; NOT the naive P=Σyᵢateᵢ Hᵢ which fails HN), so Δ^m(P^m)=0 ∀m is automatic (Zhao Prop 1.2; verify 20×20 Hessian char-poly=λ²⁰); VC-failure certified by the transported triple collision (2 poly evaluations, Lean-able). First explicit VC witness ever (VC proven cubic-homog n≤4 ⟹ dim≥5 needed; none known in any dim). OPEN STEP = the explicit ≈20-var transport (engineering, no new math). [An initial ≈76 estimate was a per-monomial-heuristic error — MISTAKE-201.] Refs: de Bondt *Symmetric Jacobians* arXiv:1206.2865 (Thm 1.2 Meng doubling, Thm 1.3); Zhao arXiv:0704.1689 (Prop 1.2), math/0409534 (Conj 7.1 bound); BCW Bull.AMS 7 (1982).
