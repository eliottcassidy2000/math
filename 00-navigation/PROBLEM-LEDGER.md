...
|---|---|---|
| LRC(14) | **OPEN.** THM-4305 proves exact arbitrary-rank response minimum five for the 145 endpoint-595 failures (mixed ranks eight/nine ten; rank eight 22), closes all three rows with a 9,024-mask carrier, and lowers the typed residual to `20,986`, max `594`. Only two masks are strictly common-inactive on the protected 391 rows. No physical entry. | Find three row-dependent redundant deletions to recover carrier size 9,019; in parallel replay the 25-row endpoint-594 layer and seek an entry/owner bridge. |
