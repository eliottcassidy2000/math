...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. THM-2633/2655 reduce live quartic monodromy to `A4/S4` plus a rank-two `V4` Kummer carrier on the full resolvent normalization. THM-2681 proves the actual THM-1310 cubic's `S3` normalization is `G_m x A^2` on `g!=0`, so it cannot be an exact quartic `S4` Keller resolvent; weaker discriminant/Jelonek shadows and `A4` survive. At degree 22, audited THM-2671 empties all ten support-three strata, and audited THM-2683 closes all five support-four strata by terminal Hensel rank and toric-kernel certificates. Only full support remains in the inherited genuine nonsplit branch. | For quartics, test other full resolvent normalizations and the `A4` cyclic-cubic branch rather than rematching `-L`. For degree 22, attack the full-support four-parameter tail: its terminal `P^4` necessarily meets the four-dimensional projective toric closure in `P^8`, so saturate that intersection and stack order `N=12`. Separately address split/even edges, integral raising, branches outside THM-2262, and THM-2240's DC axes. |
