...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. In the reduced `(2,3)` seam, THM-4248 closes exact `M=11` off three coefficient walls. On exact-`M=12`, `W=0`, THM-4253 leaves `176/104` classes and `864/648=1,512` incidences; THM-4258 compresses each collapse test to three samples. THM-4260 excludes all `280` classes on the specialized gate interior; THM-4265 proves the residual wall roots simple. THM-4268 uses proper relative-map incidence to upgrade this to a non-effective Zariski/formal `W=0` collar. | Make the collar effective by finding equations or quantitative bounds for its complement; separately cross the three coefficient walls and prove exact-`M=12` seam entry. A coherent lift and transverse Jacobian remain an optional explicit route, not a prerequisite for the collar. JC(2) and DC(2) remain open. |
