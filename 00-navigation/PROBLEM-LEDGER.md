...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. In the reduced `(2,3)` seam, THM-4248 closes exact `M=11` off three coefficient walls. On exact-`M=12`, `W=0`, THM-4253 leaves `176/104` classes and `864/648=1,512` incidences. THM-4258 proves that three consecutive difference evaluations, of degrees at most `120/150`, suffice for each collapse test; its conditional two-torsion sidecar has at most 64 geometric patterns. | Freeze the concrete `H_lambda`-to-`h` normalization, then run the `4,536`-row uniform observer table on one canonical node orbit while retaining class/incidence overlap. Separately attack the `M=11` walls, other hidden-Hom locus, and seam entry. |
