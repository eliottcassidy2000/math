...
|---|---|---|
| LRC(14) | Open with `165` rows. THM-2687 caps the full slope-seven family at `12/13` positive carries under arbitrary labelwise switching. THM-2689 proves every nontrivial odometer lift restores intrinsic three-event support but none globally transports clock labels. THM-2691 closes the `13^5+1` central cycle at present depth `6`. THM-2693 finds a fully labelled positive three-event `13^5+2` packet, then proves the inherited delayed sector language empty at four events uniformly over every integer lift and clock word. A positive mixed `D`/slope face remains open. No row is excluded. | Change the delayed grammar or base dynamics, or build a heterogeneous `D`/slope component automaton retaining word, private-unit, and semantic endpoint data. Do not enlarge the carry alphabet inside the inherited delayed word, retry unshifted `D`, the closed central cycle, or slope-seven thirteenfold switching. In parallel pursue AP-core, Euler/phase, and finite rank-box/star exits. |
