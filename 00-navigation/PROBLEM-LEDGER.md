...
| Jacobian / Dixmier / Poisson | **OPEN.** THM-4290--4356 close the staged `M=12` atlas; THM-4357's source pullback leaves the two-parameter plane `S_4339`. The late proofs use critical-series order, saturated exit charts, coefficientwise owner reconstruction, and endpoint-addressed stable graphs. THM-4328 makes Student visibility even-lossless/odd-zero. | Test `S_4339` at row nine, then address seam entry and all-row transport. Retain the exit/collision three-tax tuple, critical-series order, toric owner, and attachment address. |
