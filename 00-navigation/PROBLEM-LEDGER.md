...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. In the reduced `(2,3)` seam, THM-4248 closes exact `M=11` off three coefficient walls. On exact-`M=12`, `W=0`, THM-4253 leaves `176/104` classes and `864/648=1,512` incidences; THM-4258 compresses each collapse test to three samples. THM-4260 instead excludes all `280` classes by a sharp reciprocal-denominator/local-DVR obstruction and proves `S_34=S_42=empty` on the already-specialized gate interior. THM-4265 proves the two unavoidable wall roots are simple and makes the transverse Jacobian the first off-fibre observer. | Construct a coherent `W`-lift and compute its two-by-two transverse Jacobian; separately audit the other coefficient walls and exact-`M=12` seam entry. Retain `W`-order and the leading transverse jet. JC(2) and DC(2) remain open. |
