...
|---|---|---|
| LRC(14) | **OPEN.** THM-4305 proves exact endpoint-595 response minima `5/10/22` (arbitrary/mixed-8-9/rank-eight). THM-4307 uses an exact 37-mask residual retention repair to give a 3,925-mask carrier on the 391-row target; this minimum is only in its fixed `D_350` family. THM-4306 separately closes the 367-row `H_265` ideal with replacement minimum 2. Typed union `2,014`; residual `20,633`; max `594` on 22 rows. No physical entry. | Replay the 22-row endpoint-594 layer against the 3,925 carrier, recurse on other failed singleton ideals, and seek an entry/owner bridge without merging decks. |
