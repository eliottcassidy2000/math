...
| Newton circuit / no-return | THM-3000: one `k`-free cumulant curvature `(3kappa_2^2-2kappa_1kappa_3)/kappa_1^4` is the `d^-2` term of `log(R_k/R_(k-1))` for EVERY fixed edge `k>=2` (Moebius collapse `3C(k,4)-C(k,2)^2/2=k(k-1)(3-2k)/4`); exact third-edge `G_3`; jet law `-(j-1)!C(k-2,j-3)`; hostile forces the GRADED hypothesis `m_j/m_1^j=o(d^(j-3))`. THM-3001: reversal is an exact involution `R*_k=R_(d-k)`, so NO reversal-closed class (positivity, positive real-rootedness, PF-infinity, Hurwitz, strict ULC) can imply global no-return; the bottom edge is `+C(mu)`, the top `-C(mu*)`, so no-return REQUIRES `C(mu)>=0>=C(mu*)`. Log-symmetric root profiles are exactly ratio-palindromic. | Compute `C(mu*_M)` for the first-gap core (its BOTTOM three log jets) -- a positive value refutes family no-return outright. Supply `P_4`/`w_4`, or any `|p_4|=O(M^5)` root-modulus bound, to close the third edge. Prove or refute the two-sign global classifier (census 42/42, needs a discrete-unimodality interior theorem). |
