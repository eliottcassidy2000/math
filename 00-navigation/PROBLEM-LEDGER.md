...
|---|---|---|
| LRC(14) | Open with `165` rows. THM-2680 constructs positive physical `D(x)={13x}` two-event fibres, but THM-2682/2684 close every clock-legal three-event chain in the complete THM-2584 parent rail bank. THM-2687 separately closes every positive thirteen-label intersection in the full THM-2640 slope-seven family, even under arbitrary labelwise configuration switching, by a sharp `12/13` enlarged-envelope cap. The clock-two addendum retains one positive `h=10,c=0` endpoint atom but no target transition. No row is excluded. | Do not add selectors to the empty unshifted `D` chain or retry slope-seven configuration switching. Lift the new odometer-twisted intrinsic clock cycle to labelled rails/present/unit packets, or change parent carrier, boundary semantics, or edge grammar; in parallel pursue AP-core, Euler/phase, and finite rank-box/star exits. |
...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. THM-2633/2655 reduce live quartic monodromy to `A4/S4` plus a rank-two `V4` Kummer carrier on the full resolvent normalization. THM-2681 excludes the actual THM-1310 cubic as an exact quartic `S4` Keller resolvent. THM-2686 proves coprime invariant Kummer vanishing; for every cyclic cubic affine-target normalization, mod-two Kummer cohomology is a sum of standard `F2[C3]` planes, so the `A4` module gate reduces to scalar nonvanishing in units/squares or `Cl(R)[2]`. This is not an `A4` exclusion. At degree 22, audited THM-2671 empties all ten support-three strata, and audited THM-2683 closes all five support-four strata by terminal Hensel rank and toric-kernel certificates. Only full support remains in the inherited genuine nonsplit branch. | For quartics, compute units and `Cl[2]` on other full cyclic resolvent normalizations and finish THM-2685's boundary-residue extension gate; do not rematch discriminants. For degree 22, attack the full-support four-parameter tail: saturate its terminal projective-toric intersection and stack order `N=12`. Separately address split/even edges, integral raising, branches outside THM-2262, and THM-2240's DC axes. |
