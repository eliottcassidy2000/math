...
|---|---|---|
| LRC(14) | **OPEN.** THM-4305 proves exact endpoint-595 response minima `5/10/22` (arbitrary/mixed-8-9/rank-eight). THM-4309 gives a family-relative 3,925-mask carrier on 391 rows; THM-4310 extends it over all 25 endpoint-594 rows. THM-4311 finds the unique endpoint-593 failure, proves response minimum one, and exchanges `0006e281 -> 0036092c` while preserving all 432 rows at size 3,925. THM-4306's `H_265` deck remains separate. Typed union `2,052`; residual `20,595`; max `592` on 35 rows. No physical entry. | Audit the 35-row endpoint-592 failure/response quotient, optimize additions against the full inherited deletion complex, recurse on failed singleton ideals, and seek an entry/owner bridge without merging decks. |
...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. THM-4248 closes exact `M=11` off three walls and THM-4226 closes dense `M=13`. THM-4290/4297/4299 close exact `M=12` away from `U=0`, `Z=0`, and the `D=Lambda=0`, `UZ!=0` corner. There THM-4301 kills separable/horizontal first faces, and THM-4304 proves every reduced repeated carrier is a rational double section; only refinements above those nonreduced sections remain locally. THM-4298 observes the walls in source-normal `G`-rows `6,7,8` but proves no row existence. | Translate each exact double section, retain both prepared `T^1` coordinates and the discriminant valuation, and classify the first refinement splitter; independently derive the actual bracket/Hasse rows through `G`-row eight. `U=0`, `Z=0`, seam entry, JC(2), and DC(2) remain open. |
