...
|---|---|---|
| LRC(14) | **OPEN.** THM-4326/4329 close the fixed-pool chart; THM-4330 leaves anchored `2+12` with `420|h`; THM-4333/4338 close residual ranks three/four; THM-4335 gives exact renewal owners. THM-4347 closes every dyadic eleven-body through height `40`. A max-`72` body escapes fixed clocks `15..43`, while THM-4349 proves that one explicit adaptive clock is nevertheless a complete certificate for each fixed body. THM-4348 reduces one/two-residual third-tooth walls off `7u|h`. THM-4345/4346 identify the anchor residual-wall transducer and the rank-three Brownian / `7`-adic martingale current, while exact hostiles rule out variance-only and cubic-moment-only forcing. | Attack the exact resonance `7u|h` and intersections of at least three wall shadows. In parallel, compress THM-4349's enormous adaptive clock or force its emptiness, and derive an inequality for the nonlinear anchor-conditioned absolute-current histogram rather than another scalar variance bound. |
...
| Jacobian / Dixmier / Poisson | **OPEN.** THM-4290--4356 close the staged `M=12` atlas, culminating in the full `Z=beta_11=zeta_3=W=xi_10=0` endpoint. The late proofs use critical-series order, saturated exit charts, coefficientwise owner reconstruction, and endpoint-addressed stable graphs. THM-4328 makes Student visibility even-lossless/odd-zero. | Address seam entry and all-row/termination transport. Pull every candidate entry slice back through the source-normal row-eight response before opening another abstract boundary atlas; retain the exit/collision three-tax tuple, critical-series order, toric owner, and attachment address. |
