...
|---|---|---|
| LRC(14) | **OPEN.** The fixed-pool lane has residual `24,223`, max `663`, top `(256,663),(366,663),(520,663)`. THM-4281 combines a frozen joint deck common-active on `148,063` post-THM-4277 rows with a nested carrier closure through endpoint 664; exact overlap removal gives `148,099` new proof-graph edges. Its 420-mask body-cover core has the same common set, while the body-redundant mask remains endpoint-essential. THM-4278 separately classifies 72 minimum-one repairs at former top `(520,688)` with zero current deletion and a sharp disconnected exchange hostile; THM-4267/4269/4270 close six rays. This is not physical entry. The `165` relation rows also remain open: THM-2701/2710 close two terminal languages, while the `BABA` carrier lacks target current. | Resolve the three endpoint-663 rows by exact response quotients, but also mine the `24,220` lower complement by first-inactive-mask fibres rather than another saturated layer scan. Test whether the body-redundant/joint-useful mask exposes a missing endpoint coordinate. Globally, prove physical entry or re-root at an exclusive owner while retaining coefficient-two guard debt, phase, and ancestry. |
