...
| Jacobian / Dixmier / Poisson | **OPEN.** THM-4290--4356 close the staged `M=12` atlas; THM-4357 isolates the finite source plane `S_4339`, and THM-4358 kills it at row ten by a delayed prior-depth obstruction. The late proofs use critical-series order, saturated exit charts, coefficientwise owner reconstruction, and endpoint-addressed stable graphs. | Classify the constructible row-eight response image, then test THM-4357's next smallest pullbacks and seam entry. Retain prior-depth functionals and attachment addresses until after the next consumer. |
