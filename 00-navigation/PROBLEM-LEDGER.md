...
|---|---|---|
| LRC(14) | **OPEN.** THM-4326/4329 close the fixed-pool chart. THM-4330 identifies `K_(m,14-m)`, closes `1+13`, and leaves anchored `2+12` first; a degree-twelve minority survivor has `420|h`. THM-4331 gives endpoint escapes and the `1/189 -> 27(C-1)` appender; THM-4332 is the literal boundary. THM-4333 proves `L_3/D>2/27` on the `181,194`-pair remainder. THM-4335 resolves equality capacity into exact owner permutations: component endpoint denominators and pair determinants sharpen the boxes, a new `p=5` hostile loses owners under exact endpoint motion, and minority components admit a deterministic shortest-cover renewal budget. THM-4336 proves `L_4/D>7/81` for the hard controls `(50,70)` and `(509,640)`, yielding direct one-appender LRC(14) cutoffs `6021` and `5295`. | Bound reuse of a fixed `(u,v,q)` owner transition across the `2h` minority components, retaining tooth and sheet addresses. Run the direct-coverage rank-four optimizer on the other `181,192` residual pairs and preserve exact pair qualification; do not extrapolate the two controls. Also attack degree-two bodies failing both THM-4330 entry tests. |
...
| Jacobian / Dixmier / Poisson | **OPEN.** THM-4290--4327 close the `M=12` interior, corner, and generic endpoint lanes by good-differential order. THM-4334 closes `Z=beta_11=0`, `U W zeta_3!=0`; its cyclic-nine face has genus three and order 34. THM-4337 closes `Z=zeta_3=0`, `U beta_11!=0` for arbitrary `W,K,Lambda`; the `W!=0/W=0` face genera are `3+3/6`, graph `b1=11` completes genus 17, and its `A_23` split survives the `K` remainder. THM-4328 makes Student visibility even-lossless/odd-zero. | Audit `U=0` with `WZ=0` or either repeated cusp, and the exact residual `Z=0,U!=0,beta_11=0,W*zeta_3=0`. Stratify the latter cubic edge by Laurent-saturated labelled roots, separating owner exits from interior discriminant collision. Contacts and seam entry remain. |
