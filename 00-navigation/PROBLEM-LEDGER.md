...
| Tournaments | Join laws and strong-core localization are proved. THM-2221 gives the pinned cut-metric term; THM-2242 closes the full transitive response. THM-2249 gives every quotient an explicit forced-pair energy `F_R(X)<=G(X)` and, for `C_3`, proves that only the three rotations have zero energy and every other transport pays the sharp linear floor `phi(N)`. | Classify the residual `G-F_R` on `C_3`, then the zero/first-positive layers for `C_5` and other strong quotients; optimize unpinned core/context exchange separately. |
