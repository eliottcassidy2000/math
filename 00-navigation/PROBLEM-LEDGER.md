...
|---|---|---|
| LRC(14) | **OPEN.** The fixed-pool lane has residual `22,682`, max `637`, top `(100,637),(294,637),(520,637)`. THM-4283 realizes the complete 640-row top-signature fibre with twelve explicit common decks. Its separately typed 9,006-mask carrier closes the 64-row band `638..644`: one new mask repairs endpoint 644, while an exact minimum-nine response repairs the unique failure at endpoint 638. Their 13-row overlap gives an exact 691-row deletion. THM-4286 proves that the old inactive-signature quotient does not determine four audited THM-4282 responses and supplies two disjoint exact 421-mask common-deck nodes on 36 post-THM4282 rows each. All 72 rows already belong to THM-4283's proof union, so THM-4286 contributes no current decrement. Classifier bases, body covers, common decks, carriers, and typed proof-graph unions remain noninterchangeable; none gives physical entry. THM-4287 is **RESERVED / UNPROVED**, not an endpoint-637 descent. The `165` relation rows also remain open: THM-2701/2710 close two terminal languages, while the `BABA` carrier lacks target current. | Scan the repaired carrier from endpoint 637 downward, recomputing essentiality at every consumer. Refine inactive signatures by appended-mask activity and labelled exposed-body response before any recursive closure; seek a recursive law for full signature fibres rather than another arbitrary greedy cover. Prove physical entry or re-root at an exclusive owner while retaining coefficient-two guard debt, phase, and ancestry. |
