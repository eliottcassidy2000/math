...
| Arithmetic-Kakeya forcing certificates (NEW 2026-07-28) | External benchmark: certificate score s proves AK(s); human record 1.67513 (Katz–Tao 2002); target ≤ 1.675 is open. In-repo: exact engine + verifier (`04-computation/ak_forcing_engine.py`); literal-spec rule-(1) proven UNSOUND (score→1 exploit family, 14/9 at n=9, machine-verified); honest equal-suffix game: exact strict certificates at 13/7 ≈ 1.857 ([2,2,2], collective 2-round firing), 15/8, 19/10, 31/16; permissive mode3-v2 reaches 9/5 but lacks the intuitive same-H compilation theorem. Species-flow/illegal-junction and rank/matroid reformulations live in `06-writeups/AK-FORCING-WORKBENCH-klein-S691.md`. The former μ₃ `3+3+1=7` motivation is retracted: that fragment is THM-2473's Keller escape-to-infinity, not vertex identification. | Descend the rung ladder 11/6 → 9/5 → 7/4 (KT99 validation rung) → 5/3 < 1.675 via exact rank bounds, annealing, and min-seed B&B on deep products [2]^k; prove or refute intuitive same-H legality for the mode3-v2 witness; prove or refute the k=1 path floor 2. |
