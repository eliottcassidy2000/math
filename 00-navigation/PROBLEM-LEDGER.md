...
|---|---|---|
| LRC(14) | Open with exactly `165` first-depth-one rows. THM-2436 empties every deep-`c_3` type. On the complete complementary `nu_7(c_3)<=M` branch, hostile-audited THM-2445 partitions the isolated graft into `24` positive partial-endpoint cells: `23` are repair/blocker-labelled and the ghost has source-owner drift or all-six-source completion. No scalar profile is excluded. | Transport one typed THM-2445 cell through the delayed word and fully masked terminal endpoint, retaining target/repair alignment and within-cell carry phase. |
