...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. In the reduced `(2,3)` seam, THM-4248 closes exact `M=11` off three walls and THM-4226 closes dense `M=13`. THM-4290 closes the complete exact-`M=12` gate `U*Z*D*Lambda!=0`: deck equivariance forces degree divisible by four, contradicting `34/42` and removing every hidden-Hom locus from the interior. At `Lambda=0`, THM-4268/4272 give the raw `A_23` collar; THM-4279/4280 give exact formal-log observers; THM-4284/4288 isolate common tangent and the last-blowdown defect. THM-4289 proves the full blowdown ladder and exact `conductor/J_f` loss: ambient Kahler descent detects the missing characters while regular dualizing data do not. THM-4291 finds a genuine genus-five tail with an abstract equivariant degree-42 map, then proves the Keller differential vanishes there to order eight and on every generically separable tail. Repeated-root refinements still block uniform tail extinction and degree allocation to `C`. | Audit the repeated-root Newton faces and every stable tail; if all are Keller-constant, conserve degree onto `C` and activate THM-4290's intrinsic wall corollary. In parallel, test THM-4289's Jacobian-ideal class, cross the `U=0`, `Z=0`, and `D=0` walls, and prove exact-`M=12` seam entry. JC(2) and DC(2) remain open. |
