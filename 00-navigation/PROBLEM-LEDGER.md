...
| Euclidean Kakeya in `R^4` | **OPEN.** General Hausdorff record `3.059849573...`. Borges--Chan--Chen--Liu--Xi--Zhan v1 claims a maximal estimate at `d_0=(159+sqrt(145))/56=3.0543...`, improving the corrected published Katz--Zahl baseline `3.049570924...`; endpoint four-linear theory and sticky `13/4` have extra structure. [THM-4035](../01-canon/theorems/THM-4035-sixty-clock-separation-and-finite-kakeya-spine.md) is FINITE-EXACT only: over `F_61`, three `C_60` clocks chart the nonzero torus of `P^3`, and the same one-clock phase has both all-broad and all-narrow embeddings. | Classify cyclic weight quadruples by minor spectra; complete the boundary direction charts; attach one affine line per direction and compare owner-derived/concurrent/shuffled basepoints. A Euclidean attack must additionally retain shadings, scale, moving plane fields and Wolff nonconcentration. See the [source audit](../05-knowledge/reference/CORE-PAPERS-KAKEYA-4D-2026-08-24.md). |
...
| P-adic zeta / arithmetic irrationality | Long's August 24 draft claims 22 singleton irrationalities, but only the terminal interval margins are `FINITE-EXACT`; the global theorem is an external preprint under specialist audit. THM-4089 proves the exact first-chamber `p=11` wall and a global one-power `p=13,s=3` margin no-go. THM-4091 proves that LCM clearing survives integral reparametrization, literal depth one survives by differentiation, and every higher literal depth fails universally at output degree three. | Audit one complete claimed cell from the global source lattice to the Bost contradiction. For `zeta_13(3)`, stop optimizing the same Hasse codimension: demand an extra determinant saving, wider continuation, or a labelled character-DFT/Pade/Smith selector with decay and nonvanishing. |
| Basic matching logic | The cited arXiv v1 proves one-sort global completeness in its basic fixpoint-free scope. THM-4090 gives a satisfiable two-sort, one-unary theory with semantic consequence but no derivation, making the sort-count boundary exact for that language/system. | Formalize the primitive-rule sort-flow lemma and a finite certificate encoding. Do not identify logical localization/double covers/fixpoints with p-adic localization/geometric covers/interval iteration. |
