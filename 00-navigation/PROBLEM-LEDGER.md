...
|---|---|---|
| LRC(14) | Open with `165` rows. THM-2687 caps the full slope-seven family at `12/13` positive carries under arbitrary labelwise switching. THM-2689 proves every nontrivial odometer lift restores intrinsic three-event support but none globally transports clock labels. THM-2691 then closes the canonical lawful central cycle: its rail cylinders persist at every finite horizon, while the all-event dynamically typed current-present product dies sharply at `H=6`. Other lifts/components and a positive mixed `D`/slope face remain open. No row is excluded. | Change the lawful lift/component or build a heterogeneous `D`/slope component automaton retaining present, word, private-unit, and semantic endpoint data. Do not retry unshifted `D`, the closed central cycle, or slope-seven thirteenfold switching. In parallel pursue AP-core, Euler/phase, and finite rank-box/star exits. |
