...
|---|---|---|
| LRC(14) | Open with exactly `165` first-depth-one rows. THM-2436 empties every deep-`c_3` type. On `nu_7(c_3)<=M`, THM-2445/2442 give `24` word-restored cells and THM-2452 produces a complete matched mask with fresh exact `X` and `91`-unit `m`. THM-2456 classifies the remaining mixed-zero locus as an averaged uniform root offset; THM-2457 gives a sharp common-root co-support graph but proves local bits do not determine the semantic word. No scalar profile is excluded. | Exclude the physically realizable uniform-offset atlas or couple selected drift to a canonical semantic/root edge. |
