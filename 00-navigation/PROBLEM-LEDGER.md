...
|---|---|---|
| LRC(14) | Open with `165` first-depth-one rows. THM-2295/2298 force ranks five/six by `196/526`; THM-2301 gives bright-or-dark banks; THM-2287 gives a `c_1`-anchored rank-six atlas on `120` interiors. THM-2305 refines prescribed return into a pure owner edge or two-target fork while retaining every root character and a bounded signed coefficient. Yet THM-2299's anchored packet has vanishing pair base phase; THM-2303 identifies the lost component current and relative phase. | Derive lawful phase-tree transport or composable incoming/outgoing owner overlap on the same exact word. The no-forced bank is `693`; the fork and other `45` rows need anchor-free structure. |
...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. THM-2247 closes nonsplit degree fourteen. THM-2262 leaves a singular degree-twelve trigonal discriminant at degree eighteen; THM-2297 removes target translation, exposes weights `(2,3,4,5)`, and closes every one-sparse stratum plus the `B=D=0` plane. Every survivor has `(B,D)!=(0,0)`. | Classify the remaining multi-parameter singular trigonal strata while retaining the Keller one-form and Faber sidecar; separately treat split/even short edges and THM-2240's DC continuation axes. |
| Tournaments | Join laws and strong-core localization are proved. THM-2221 gives the pinned cut-metric term; THM-2242 closes the full transitive response. THM-2249 gives every quotient an explicit forced-pair energy `F_R(X)<=G(X)` whose zero set is exactly one scaled automorphism transport. THM-2256 proves a sharp scale dichotomy: zero-cost automorphism/nonautomorphism contact gives bounded first positive energy, while its absence gives a linear gap; transitive quotients have exact gap one and `C_3` has the sharp THM-2249 floor. | Classify the residual `G-F_R` first on bounded-contact quotients, then on `C_3`, `C_5`, and other strong quotients; optimize unpinned core/context exchange separately. |
...
| Mahler `3/2` problem | Open. THM-2228 proves that every binary carry word is a formal `2`-adic orbit, while a positive `Z`-number requires both strict Archimedean suffix tails and eventual stabilization to an ordinary positive integer. These gates are independent; the denominator-`19` family survives every prescribed finite horizon. | Study the intersection of open tail certificates with ordinary stabilization. Any finite-state or carry-only attack must add an unbounded termination/height sidecar; a fixed orbit-time cutoff cannot decide existence. |
...
| Metric composition / flows | THM-2176 gives the Gordian continuation kernel; THM-2191 gives stable localization. THM-2242 makes the kernel embedding isometric. THM-2292 realizes every finite Boolean catalytic defect table in one context, bounds minimal calibration nonfaces by `max(2,d)`, and confines the Brittenham--Hermiller rank-two packet's possible stable nonadditivity to two same-sign chambers. THM-2177 refutes Goemans' exact planar cost conjecture. | Test the two same-sign knot chambers and find explicit stable calibrators; then classify higher-rank signed calibration nerves and unsplittable-flow overload facets. |
