...
|---|---|---|
| LRC(14) | Open with `165` rows. THM-2670 exposes formal clock cycles but no chronology. THM-2680 constructs the canonical physical `D(x)={13x}` two-event fibre product: positive overlaps exist, yet only `10/252` generic clock triples survive versus `146` formal products. THM-2682 proves every three-event chain on the arrival-six carrier is empty because the central return preserves a forbidden repeated shallow clock. The clock-two addendum to THM-2640 retains one positive `h=10,c=0` endpoint atom but no target transition. No row is excluded. | Do not add downstream selectors to the now-empty arrival-six D-chain. Change at least one of arrival stratum, handoff, boundary semantics, or edge grammar; in parallel pursue the independent AP-core, Euler/phase, and finite rank-box/star exits. |
