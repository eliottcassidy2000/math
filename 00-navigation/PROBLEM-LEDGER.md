...
|---|---|---|
| LRC(14) | Open with `165` rows. THM-2532--2540 close cyclic orientation, owner phase, positive all-slope reconstruction, anchor covariance, clock/selector flow, and weighted live boundaries; THM-2538 identifies same-ancestry cross-horizon coupling as the semantic seam. THM-2541 has `169/169` unrestricted `A(q)` support on one typed non-cover control. THM-2542 proves the seven-chart `C_91` bundle is spatial holonomy, not arrival. No row is excluded. | Construct a later target-active Boolean field on the inherited ancestry uniformly for covering rows, and identify a lawful-image restriction that transfers unrestricted `A(q)` support to the all-`91`-unit `B(q)` projector. |
