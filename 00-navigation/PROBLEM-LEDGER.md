...
|---|---|---|
| LRC(14) | **OPEN.** The fixed-pool lane has residual `21,323`, max `626`, top `(100,626),(256,626)`. THM-4287 leaves `22,647`, max `636`. THM-4295 proves the exact row-aware rank-eight append minimum `14`, the body `1d106401` has no rank-eight response, and three recursive ideals have exact replacement minima `2,4,4`. THM-4296 recognizes active deletions as an upset, upgrades that obstruction to exact deletion depth nine, and uses a 14-for-14 exchange plus mixed rank-eight/rank-nine repairs to obtain a 9,019-mask carrier closing 70 rows and leaving 995/10 failures at endpoint 626. Separately, 110 singleton decks, the index-19 deck, and THM-4295's index-294/index-372 decks give 113 common-deck nodes. Their lawful typed join removes 1,324 rows; the two incoming larger ideals add exactly three rows beyond the earlier 1,321-row union. MISTAKE-532 corrects cross-row weakest-margin diagnostics only. Classifiers, body covers, separate common decks, carriers, and proof-graph unions remain noninterchangeable; none gives physical entry. | Quotient the exact 1,005 endpoint-626 carrier failures, beginning with the 995-body `(100,626)` wall and retaining deletion depth as a state coordinate. In parallel, recurse on the 81 remaining failed singleton groups or larger ideals, and prove physical entry or re-root at an exclusive owner while retaining coefficient-two guard debt, phase, and ancestry. |
