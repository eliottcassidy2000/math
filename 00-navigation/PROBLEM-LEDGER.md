...
| Arithmetic-Kakeya forcing certificates (NEW 2026-07-28) | External benchmark: certificate score s proves AK(s); human record 1.67513 (Katz–Tao 2002); target ≤ 1.675 is open. In-repo: exact engine + verifier (`04-computation/ak_forcing_engine.py`); literal-spec rule-(1) proven UNSOUND (score→1 exploit family, 14/9 at n=9, machine-verified); honest equal-suffix game: proved floor score ≥ 1 (rank potential), hand-design plateau 2 REFUTED by search — exact strict certificates at 13/7 ≈ 1.857 ([2,2,2], collective 2-round firing), 15/8, 19/10, 31/16. Species-flow/illegal-junction reformulation + extracted circulation mechanism in `06-writeups/AK-FORCING-WORKBENCH-klein-S691.md`. | Descend the rung ladder 11/6 → 9/5 → 7/4 (KT99 validation rung) → 5/3 < 1.675 via annealing + exact min-seed B&B on deep products [2]^k; implement identification-gluing as mode three (the intuitive definition's merges — leading hypothesis for where the intended 1.75/1.675 certificates live, cf. the owner's μ₃ 3+3+1=7 fragment); prove or refute the k=1 path floor 2. |
