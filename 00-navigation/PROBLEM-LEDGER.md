...
| Mixed binomial additive bases | THM-4026 refutes Sun's `2-4-6-8` conjecture at `896315812331399` by multiple independent exhaustive exact routes. THM-4027 proves every residue modulo every modulus is represented, so the failure is global/height-sensitive. THM-4028 proves the ordered summatory count `V X^(25/24)+O(X^(11/12))`: growing mean and complete local support still do not force pointwise coverage. The target has an exact constrained `A^2+3B^2` fibre, low-pair character holes, and a rank-eight combinadic carry state. THM-4035 adds a hostile at `p=61`: the four role images have sizes `(31,24,26,24)` and the first two already sum to all of `F_61`, so the common `C_60` input clock supplies no local obstruction. | Determine the least exception and exceptional-set density; seek a singular-series lower-tail/variance theorem, low-density CRT progressions with bounded lifts, a minimal formalizable prime cover, constrained norm-intersection criteria, and a confluent rank-labelled Pascal carry obstruction. At `p=61`, retain phase-labelled least-lift cost and fibre variance after residue saturation. Generalize the necessary-but-not-sufficient reciprocal-degree threshold to other mixed tuples. |
