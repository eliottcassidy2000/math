...
|---|---|---|
| LRC(14) | Open with `165` rows. THM-2701 proves the literal unshifted terminal language sharply nilpotent at six; THM-2710 proves the half-shifted language also vanishes by six and that the half phase is invisible at the prescribed clocks `2,4,6`. THM-2698 nevertheless gives a full local half-cycle, and THM-2707 finds `3346` fixed-skeleton packets on one interval forming a complete eleven-partite SCC: recurrence is abundant below the semantic gate. The intrinsic `BABA` carrier exposes the missing integral debt as `A -> 2B`; its conditional unit product has order `168`, but neither supplies target current. No row is excluded. | Re-root a recurrent packet at an actual exclusive owner, build a common-ancestry mapping cone retaining the coefficient-two guard debt, or construct a lawful enriched `C2`/direct macro cospan with one nonnegative transition and a common middle fibre. Treat the exact `C_221` transverse cycle as a local design target until semantic chronology is attached. In parallel pursue AP-core, Euler/phase, and finite rank-box/star exits. |
| Newton circuit / no-return | THM-3000: one `k`-free cumulant curvature `(3kappa_2^2-2kappa_1kappa_3)/kappa_1^4` is the `d^-2` term of `log(R_k/R_(k-1))` for EVERY fixed edge `k>=2`; exact third-edge `G_3`; jet law `-(j-1)!C(k-2,j-3)`; graded hypothesis `m_j/m_1^j=o(d^(j-3))`. THM-3001: reversal is the exact involution `R*_k=R_(d-k)`, so NO reversal-closed class (positivity, positive real-rootedness, PF-infinity, Hurwitz, strict ULC) can imply global no-return; bottom edge `+C(mu)`, top edge `-C(mu*)`, so no-return REQUIRES `C(mu)>=0>=C(mu*)` (necessary, NOT sufficient). THM-3003: reversal IS the antipodal map in centered log-root coordinates, `R_k=R_(d-k)` IFF the log-root measure is symmetric (exact iff), Borsuk-Ulam makes that locus essential, and the jets are 2D MULTIPOLE MOMENTS -- wall-stripping is multipole subtraction, reversal is the Kelvin transform, and the spread ratio bound `kappa=o(d^(1-3/(k+1)))` replaces the jet invoice. THM-3004 REFUTES the two-sign classifier and Newton-ratio unimodality: witness `(n+1)^2(n+3)^2(n+8)`, degree 5, real positive roots, two circuit sign changes with both end curvatures positive. Correct law is a CLUSTER COUNT: `m` separated clusters give exactly `2m-3` sign changes, localized at the band-filling numbers (free-fermion reading: `e_k` is a canonical k-fermion partition function, reversals are band-filling transitions). Classifier true exactly for `m<=2`. MISTAKE-337. | Bound the wall-stripped core's root moduli by `o(M^(5/4))` -- that alone CLOSES the third edge with no `P_4` and no new Macaulay chart. Compute `C(mu*_M)` from the core's BOTTOM three log jets: positive REFUTES family no-return outright. Count the separated scales in the core's root measure (the wall has a five-band multiplicity profile over `-1..-M`, a continuum rather than a few clusters, so `2m-3` may be vacuous there -- check, do not assume). |
...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. For live quartic `A4/S4`, THM-2703/2708 turn the required standard Kummer plane into an exact mod-two boundary/Hermitian-gain nullity gate; THM-2705/2709 classify every affine-linear target plane on every polynomial graph in the reflection quotient, and every survivor is triangular. At degree 22, the inherited genuine nonsplit polynomial branch is empty. In the split even-Faber subchart, THM-2704 proves generic and all-one normalization genus `89` after five universal cusps of total delta `165`. | For quartics, test the boundary gate on geometrically realizable full resolvent normalizations and move beyond linear targets or graph surfaces. For degree 22, classify the exceptional locus that gains at least `89` further delta, becomes reducible/nonflat, or leaves the even chart, while restoring the eleven odd Faber seeds; alternatively prove integral `2`-adic order raising. Separately retain outside reductions and THM-2240's DC axes. |
| Tournaments | Join laws and strong-core localization are proved. THM-2249/2256 give the forced-pair envelope and sharp scale dichotomy. THM-2290 makes the endpoint-colour-selected matching kernel hafnian-complete and limits universal Pfaffian tournament gauges to orders `2,4`; THM-2294 proves the anchored Plucker sign tournament is locally transitive while symmetric character data are colours, not orientations. THM-2501 identifies the first nonconstant even moment of symmetric sign switching with signed-`C4`/Gram energy; skew tournament arcs have zero quadratic form. | Classify residual `G-F_R` on bounded-contact strong quotients, and sharpen the signed-`C4`/Gram relaxation toward the plus-minus extremum before orienting any matching, knot, or handoff relation. Retain ties, gains, endpoint colours, and whole-fibre amplitudes. |
...
| Mahler `3/2` problem | Open. THM-2228 separates strict real tails from ordinary stabilization; THM-2352 puts every indexed abscissa in every carry cylinder and proves termination dense/Haar-null. | Intersect tail certificates with stabilization; finite-state, finite-interaction, and almost-everywhere quotients need an unbounded terminal sidecar. |
...
| Biased-coin critical-run deadline (AMM 12592; NEW 2026-07-30) | THM-2160/2225 prove the Monthly envelopes (`2n`, `max(2,2n-1)`, `2n-2`); THM-3032 (2026-07-31) sharpens the half-tail rule so it DOMINATES the checksum pointwise (profile `2,3,4,6,7,7,8,12,15,...`; `T(n)<=max(n+1,2n-3)` for `n>=5`; floor `n+1` at `n=2^(r+1)-2, 2^(r+1)-1`), and settles the exact optimum for all odd `n` and all `n=2^(r+1)-2` — first open value `T(4) in {5,6}`, with `T(4)=6` optimal over all 1036800 shell-balanced `m=4` rules (Pareto set `{(6,7,7,8),(7,6,7,8)}`). Submission draft: `06-writeups/amm12592-solution.tex` + `amm12592-note-to-editor.txt`. **SLOPE 2 BROKEN (THM-3007/3006, same day):** balanced blocks `[N,N+l)` exist iff `N`,`N+l` are both powers of two (so dyadic shells are FORCED and no balanced block has ratio `<2` — HYP-9061 sec 2b's `rho<2` shell route is closed outright); but block ratio =/= deadline slope, and solving the within-block normal form `sum p_{k,i} C(L_k,j-1-i) = C(m,j)/2` gives exactly fair rules with within-shell ratio `3/2, 14/9, 25/16, <=11/7` at `m=4,8,16,32`, i.e. `T(n)<=(11/7)n` for `4<=n<64`. "Every known rule has `C=2`" is obsolete. **THM-3009 (same day): the answer is neither 2 nor 1.** Deviation form is homogeneous and (A) mod 2 there is NO obstruction ever (`sum_{i subset a}u^i=(1+u)^a` over F_2 collapses the forced pattern to `u^(m-1)` exactly because `m` is dyadic -- so HYP-9061's parity/split-jump/binary-clock picture is a red herring INSIDE a shell); (B) the obstruction is archimedean, visible at `u=-1`: `C(m-1,d) <= sum_k C(a_k,d-L_k)2^(a_k-d+L_k)`, certifying `rho(m) > 1.4000,1.5000,1.5556,1.5610,1.5753,1.5828,1.5887,1.5925,1.5949,1.5962` for `m=4..2048`, hence **`C* > 1.596`** for balanced block schemes. Asymptotic floor `C_arch = 1.5979874...` at binding `delta*=0.6179` (~1/phi), and the finite-m bounds converge to it. So `C*` in `(1.596, 2]` with everything pointing at `~1.598`; the missing half is a uniform CONSTRUCTION (the capacity condition is necessary, not yet sufficient). THM-2966 (spine normal form) makes it an integer Bernstein design problem: `C* = 1+gamma*` for the minimal degree-growth of `sum p^m q W_m + sum q^m p V_m = 1/2`. THM-2967 (pending audit finalization) kills every sublinear excess: `T(n) = n+o(n)` impossible (Polya-Carlson rigidity). Corner-deficit carry mechanics: ratio-2 shells self-annihilate (the classical schemes); below 2, forced half-deficits must cross a capacity desert; the decoded external certificate `(2457/6592)log(8847357/2974400)-log(1285/896)>1/25` is a two-bias gate at the Legendre duals of the binding lattice rays. coinC2 (2026-07-30 evening): certificate context CONFIRMED (fragment re-supplied with the problem statement; rival homes retired) and (27) itself Lean-kernel-certified (`ArtanhSandwich.lean`); THM-2976 binary clock (`beta_M == 0` iff `M+1=2^r`, clock `2^{v_2(M+1)}`, corner-clocked rates = odd unit fractions, top rung `1/3`; Lean-checked); THM-2977 evaluation wall + lane G2 one-bit collapse CLOSE the evaluation reading as a class — (27) must gate a rate/entropy dual (lower-bound reading favored); exact finite-M DP/DFS: parity is the entire finite-M obstruction yet the 9-bias corridor FILLS the envelope box through `M~8-12` at `gamma in {1/3,1/2,3/5,3/4}` — the greedy freeze is a policy artifact, and finite-bias prefix envelopes cannot detect the `gamma<1` obstruction at small M. coinC2 wave 2 (2026-07-31): **C*=2 is no longer favoured**. Lane G5's dyadic checkpoint closure is SUFFICIENT for C*<=1+gamma; THM-3002 gives the normal form (H=1 <=> the epoch's whole imbalance is the middle pair 0^R1^R vs 1^R0^R), the exact capacity identity max|[p^t]Delta|=binom(d,t)2^t with its trichotomy (deficient below gamma=1/2, marginal at 1/2, ample from 3/5; two-ray entropy threshold gamma*~0.598), and the parity lemma showing parity NEVER obstructs a dyadic epoch at any rate. VERIFIED-EXACT: every dyadic epoch through [16,31] closes at gamma=1/2, so a fair extractor exists for all n<=31 with T(n)=n+1+floor(n/2). klein-S428 further shows (27)'s weight is NOT pinned by the inequality (open half-line alpha>0.36747), so 2457/6592 is an output of the source's construction -- the capacity-straddle reading is retracted. | Close all epochs at some gamma<1 (target gamma~3/5, C=8/5) to refute C*=2 outright; find the periodic/inductive orbit of the residual recursion; derive the entropy/max-flow dual and its critical rate; characterize surviving anticipatory witnesses vs greedy (does survival exploit dyadic checkpoints?); hunt the Abel-Dini partial-sum recursion behind `(896,1285)`, `(2974400,8847357)`; deep-M corridor behavior; settle whether `C* = 2`, `= 9049/6592`-adjacent, or `= 1` unattained. |
