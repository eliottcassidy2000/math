...
|---|---|---|
| LRC(14) | Open with `165` rows. THM-2680 constructs positive physical `D(x)={13x}` two-event fibres, but THM-2682/2684 close every clock-legal three-event chain in the complete THM-2584 parent rail bank. Its exact support is the three-tooth envelope `[0,1/28) union [13/28,15/28) union [27/28,1)`; `D` permits only equal-arrival handoffs, and every raw three-return forces a repeated clock. The clock-two THM-2640 addendum retains one positive `h=10,c=0` endpoint atom but no target transition. No row is excluded. | Do not add downstream selectors to the now-empty full-bank `D` chain or retry its projected endpoint phase arrows. Change the parent carrier, handoff, boundary semantics, or edge grammar; in parallel pursue the independent AP-core, Euler/phase, and finite rank-box/star exits. |
