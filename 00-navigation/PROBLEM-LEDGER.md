...
|---|---|---|
| LRC(14) | Open with `165` rows. THM-2687 caps the full slope-seven family at `12/13` positive carries under arbitrary labelwise switching. THM-2689 proves every nontrivial odometer lift restores intrinsic three-event support but none globally transports clock labels. THM-2691 closes the `13^5+1` central cycle at present depth `6`. THM-2693 finds a fully labelled positive three-event `13^5+2` packet, then proves the inherited delayed sector language empty at four events uniformly over every integer lift and clock word. A finer phase digit repairs the minimal carrier but the unchanged `2*13^5=338*13^3` guard makes the full word empty at four for all `13^3` phase policies; slope `14` lasts fourteen states and dies at fifteen. A positive mixed `D`/slope face remains open. No row is excluded. | Create relative phase between the target and high guard, change the delayed grammar/base slope, or build a heterogeneous `D`/slope component automaton retaining word, private-unit, and semantic endpoint data. Do not merely enlarge the carry/common-phase alphabet inside the inherited word, retry unshifted `D`, the closed central cycle, or slope-seven thirteenfold switching. In parallel pursue AP-core, Euler/phase, and finite rank-box/star exits. |
...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. THM-2633/2655 reduce live quartic monodromy to `A4/S4` plus a rank-two `V4` Kummer carrier on the full resolvent normalization. THM-2681 excludes the actual THM-1310 cubic as an exact quartic `S4` Keller resolvent. THM-2685 identifies the quartic even-weight divisor-parity code and maximal unramified quotient; THM-2686 proves coprime invariant Kummer vanishing, so every cyclic cubic normalization has only standard mod-two planes and the `A4` gate reduces to scalar nonvanishing in units or `Cl(R)[2]`. This is not an `A4` exclusion. At degree 22, THM-2671 and THM-2683 empty support three and four, while THM-2692 empties the last full-support chart by an intrinsic degree-seven order-eleven carrier and exact order-twelve closure. Hence every coefficient-support stratum in the inherited polynomial exact-square-prefix, genuine nonsplit branch is empty. | For quartics, compute the THM-2685 boundary matrix, units, and `Cl[2]` on other full cyclic/S3 resolvent normalizations; do not rematch discriminants. For degree 22, move to split/even short edges, integral `2`-adic raising, or branches outside the inherited reduction; do not recompute coefficient-support strata. Separately retain THM-2240's DC axes. |
