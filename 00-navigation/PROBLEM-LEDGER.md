...
|---|---|---|
| LRC(14) | **OPEN.** THM-4326/4329 close the fixed-pool chart. THM-4330 identifies `K_(m,14-m)`, closes `1+13`, and leaves anchored `2+12` first; a degree-twelve minority survivor has `420|h`. THM-4331 gives endpoint escapes and the `1/189 -> 27(C-1)` appender; THM-4332 is the literal boundary. THM-4333 proves `L_3/D>2/27` on the `181,194`-pair remainder. THM-4335 resolves equality capacity into exact owner permutations and a deterministic shortest-cover renewal budget. The addressed-transition audit proves fixed tooth-address reuse one but primitive coprime speed-pair reuse unbounded. THM-4338 closes the complete residual rank-four census with `L_4/D>7/81` using a cubic degree/codegree/triple-incidence majorant and lowers the structured uniform one-appender cutoff to `12,274`. | Prove a third-tooth competition theorem on the `420|h`, `2+12` minority branch; pair-only reuse cannot suffice after addresses are forgotten. Separately attack degree-two bodies failing both THM-4330 entry tests and construct an arbitrary-entry map into the now-complete fixed-pool rank-four terminal. |
...
| Jacobian / Dixmier / Poisson | **OPEN.** THM-4290--4327 close the `M=12` interior, corner, and generic endpoint lanes by good-differential order. THM-4334 closes `Z=beta_11=0`, `U W zeta_3!=0`. THM-4337 closes `Z=zeta_3=0`, `U beta_11!=0` for arbitrary `W,K,Lambda`. THM-4339 closes the clean remaining cubic gate `Z=beta_11=zeta_3=0,U*K*W*(U+W)!=0`, including every interior double/triple root; the only new elliptic tail has order 26, while horizontal conductors simultaneously normalize. THM-4328 makes Student visibility even-lossless/odd-zero. | Audit the proof candidate for the two `U=0` repeated cusps. On the `Z=0` residual, handle `K=0` Laurent root exit, `W=0` degree drop, and the separate `U+W=0` contact, then address seam entry. Keep the exact three-tax tuple `(infinity exit,zero exit,interior Jacobian length)`. |
