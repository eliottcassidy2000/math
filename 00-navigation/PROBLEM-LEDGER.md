...
| Jacobian / Dixmier / Poisson | THM-1300 refutes JC from dimension three; JC(2) and DC(2) remain open. In the reduced `(2,3)` seam, THM-4248 closes exact `M=11` off three walls and THM-4226 closes dense `M=13`. THM-4290 closes the complete exact-`M=12` gate `U*Z*D*Lambda!=0`: deck equivariance forces degree divisible by four, contradicting `34/42` and removing every hidden-Hom locus from the interior. At `Lambda=0`, THM-4268/4272 give the raw `A_23` collar; THM-4289 proves the exact `conductor/J_f` Kahler/dualizing loss. THM-4291 exhibits an abstract equivariant degree-42 genus-five tail but gives its Keller form order eight. THM-4292 proves every exceptional `A_23` tail, including all repeated Newton-Puiseux refinements, is Keller-constant. The inherited `34/42` response degree still lacks a wall-normalization audit. | Normalize the double top-edge root, compute the true wall response degree, put it on `C`, and test the deck eigenlattice. In parallel, cross `U=0`, `Z=0`, and `D=0`, and prove exact-`M=12` seam entry. JC(2) and DC(2) remain open. |
