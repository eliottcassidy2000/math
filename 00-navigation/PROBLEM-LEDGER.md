...
| Jacobian / Dixmier / Poisson | **OPEN.** THM-4290--4327 close the `M=12` interior, corner, and generic endpoints. THM-4334/4337 close beta/zeta owners. THM-4339 closes the clean cubic interior; THM-4340 closes `U=0,WZ!=0`, including both repeated cusps. THM-4342 closes the `K=0` cubic zero exit by saturated strict transforms; its exit depths have genera `15,14,14`. THM-4328 makes Student visibility even-lossless/odd-zero. | On the `Z=0` residual, handle `W=0` degree drop and `U+W=0` contact; on `U=0`, handle `WZ=0`; then address seam entry. Retain the exit/collision three-tax tuple, critical-series order, and toric owner. |
