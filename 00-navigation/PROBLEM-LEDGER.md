...
| Newton circuit / no-return | THM-3000/3001 retain fixed-edge cumulant curvature, all-order jet laws, reversal and the repaired growing-degree hypotheses (MISTAKE-339/341). THM-3003 retains reciprocal rigidity and the cancellation-tax bounds. THM-3004 refutes the two-end classifier; its broad cluster claim remains finite evidence. [Continuing8](../05-knowledge/results/continuing8_20260906_newton_clusters.md) now PROVES exactly `2K-3` changes for every fixed multiplicity profile `m_j>=2` with explicit narrowness/separation. [Exact circuit surjectivity](../05-knowledge/results/continuing8_20260906_newton_universality.md) realizes every positive ratio vector, including every ternary sign word, with simple negative roots and fixed root sum. The second moment fixes `R_1` and prevents unrestricted transfer. [THM-3010 repair](../05-knowledge/results/continuing8_20260906_ballot_repair.md) proves global bronze uniqueness and rational Fuss ratios at every integer order; its general norm/alternation dichotomy is REFUTED. | Characterize the circuit image at fixed `R_1` with the actual factorial/interlacing constraints. For the untouched wall core, retain the proved fixed-edge jet tests and cancellation-tax hypotheses; neither arbitrary circuit realization nor translated/dilated factorial factors preserve its original row. |
