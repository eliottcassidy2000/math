...
|---|---|---|
| LRC(14) | **OPEN.** THM-4305 proves endpoint-595 response minima `5/10/22`. THM-4309--4311 give a 3,925-mask carrier through endpoint 593. THM-4313 closes all 35 endpoint-592 rows by an exact 43-for-43 exchange over the full 467-row target; its complete response-cover bounds are `8..43`, while lower bound 36 is retained-pool-only. THM-4314 closes the 13 endpoint-591 rows without changing the carrier and classifies every deletion through size two: 2 unsafe singletons and 7,869 unsafe pairs. THM-4306's `H_265` deck remains separate. Typed union `2,100`; residual `20,547`; max `590` on 13 rows. No physical entry. | Replay the 13-row endpoint-590 layer unchanged, compute its low-witness hypergraph before optimizing any exchange, and seek an entry/owner bridge without merging decks. |
...
| Jacobian / Dixmier / Poisson | **OPEN.** THM-4248/4226 close `M=11/13`; THM-4290/4297/4299 reduce `M=12` to `U=0`, `Z=0`, and the cubic corner. THM-4301/4304 leave five rational double sections; THM-4307 closes balanced towers. THM-4308's finite row-eight gate plus THM-4312 leaves only `k=1`, with positive formal differential order on both first carriers. No all-row/later closure follows. | Extend to row nine and later `k=1`. `U=0`, `Z=0`, seam entry, `JC(2)`, and `DC(2)` remain open. |
