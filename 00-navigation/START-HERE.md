...
15. **Every counterexample is in a finite labelled code/deck/fan atlas.**
    ...
    THM-2349 supplies a finite delayed marked word/current on all `165` rows, including every repeated and strict resonance arm. THM-2350 identifies its two lawful target characters as balanced target--graft dipoles and rewrites target survival as magnetic Dirichlet energy, without forcing that energy positive. Force two-coordinate or cross-axis mixing, then retain all-`91`-unit visibility and terminal phase.
