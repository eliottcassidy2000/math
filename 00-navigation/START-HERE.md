...
15. **Every counterexample is in a finite labelled code/deck/fan atlas.**
    ...
    THM-2349/2350/2365 supply delayed current, dipoles, and the exact `H`-drift target. THM-2368 has all `169` root-fibre modes but a zero-drift orbit hostile. Hostile-audited THM-2378 empties hard `W=k=1` via the rooted support squeeze `3/4 <= 1/2` and a `5/49` anti-shield floor. Retain the rooted phase/event word for `k=2`, saturated `W=7`, and target incidence; support-only/tournament shadows lose it.
