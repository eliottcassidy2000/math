...
1. **LRC(14) is first open.** THM-2923 leaves `<=6` bodies; THM-2928/2941 empty `k>=4`, cap `k=2,3` at `1742/312`, compress the raw `k=2/3` denominator ledgers to `200.14b/2.55b`, and close the common-level reflected `k=1` diagonal. Other `k<=1` and the rung are open. [Sources](../05-knowledge/reference/CORE-PAPERS.md).
