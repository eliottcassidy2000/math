...
15. **Every counterexample is in a finite labelled code/deck/fan atlas.**
    ...
    THM-2349 supplies a delayed marked current on all `165` rows. THM-2350 identifies balanced target--graft dipoles; THM-2365 makes landing exactly departure from `H(r,s,t)=G(r-t)`. Audited THM-2368 proves all `169` target modes survive on every canonical root fibre, but an exact orbit hostile cancels after chamber integration. Retain the rooted phase/event word: support-only and tournament shadows are insufficient.
