...
height -> inf) -- renormalize to the DIFFERENCE CORE one scale down (HYP-3901): two-scale factorization
...
mirror-balanced singleton dead-component packets, not weak projection graphs:
...
`random_covering_039`, `random_covering_074` and branch-unit rows
`random_covering_001`, `random_covering_062`, `random_covering_086`,
`random_covering_101`.  Companion best-touch refinement: `062`, `086`, and
`101` are the clean `(1,1)` current tests, while `001` is asymmetric with four
isolated dead components and best current `(2,1)`.

Open proof task: prove the finite mirror-singleton current lemma, possibly
with a clean/asymmetric/cover-delta split.  The lemma should retain dead
intervals, blocker-owner pairs, touching gate words, and owner-current
locality rather than collapsing to raw row names, edge-support zero, or gate
...
the count of high safe gaps meeting `C1` is

```text
N(m)=floor((504m-6)/70)-floor((96m-13)/14),
```

and the mirror corridor doubles it:

```text
escapes(m)=2*N(m).
```

Validation against the HYP-3452 component audit through `m=70` gives
...
LTI-414, LTT-314, T1414, OPEN-Q-108.
...
## 🔴 OPEN-Q-JC-FRONTIER (opus-S421): the post-JC target list is live
Read `03-artifacts/drafts/post-jc-frontier-ledger-and-targets-opus-S421.md` (HYP-8185).
Owner priority: assumption-challenging progress on Dixmier bottom rungs (DC1/DC2), explicit
Zhao vanishing/image/Mathieu witnesses (Target 1 -- untouched by anyone, publishable), JC2
weight-descent, classification structure theorem, LRC(14) residuals, unit distance for real.
Sessions should pick a target, log the assumption being challenged, and update the ledger.

## OPEN-Q-108 Ã°Å¸â€Â´ The uniform fattening lemma Ã¢â‚¬â€ the ONE lemma that completes the singular-series proof of LRC(14)
**Status:** OPEN Ã¢â‚¬â€ the isolated crux (kind-pasteur-2026-06-17-S1, THM-523, from the prove/disprove dialectic). By THM-523 (decoupling floor + single-perturbation inf=1/1260 + quantization THM-522), `inf_S L(S)>0 Ã¢Å¸Â¹ C'(14) Ã¢Å¸Â¹ LRC(14)` reduces to: **Ã¢Ë†Æ’ c>0 with meas(G_C) Ã¢â€°Â¥ c for EVERY 12-subset C of distinct positive integers**, where `G_C = [0,1)Ã¢Ë†â€“Ã¢Ë†Âª_{vÃ¢Ë†Ë†C}D_v` is the gap-1/14 lonely set of `C` (`D_v` = `v`'s danger arcs). Equivalently: **the primitive tight locus at n=13 is FINITE** (conjecturally exactly `{AP {1..13}, GoddynÃ¢â‚¬â€œWong T5 {1..11,13,24}}`). KNOWN: the decoupling bound `L(CÃ¢Ë†Âª{w})Ã¢â€°Â¥(6/7)meas(G_C)Ã¢Ë†â€™r/(7w)` handles speeds growing ONE AT A TIME (floor 1/143) and iterates for `k` large entries while the residual `(13Ã¢Ë†â€™k)`-core keeps positive measure; the ONLY uncontrolled regime is `kÃ¢â€°Â¥3` arithmetically-coordinated growing speeds (the drop-6 family minimizes at the large `w=69`). THE LEVER (re-verified): PROVEN LRC(12) gives exactly one 12-subset of `{1..14}` tight at gap 1/13, zero at gap 1/14 Ã¢â‚¬â€ converting the crux from EXISTENCE (`meas(G_C)>0`?) to TRANSVERSALITY (does the isolated gap-1/13 maximizer FATTEN to a uniformly-positive gap-1/14 measure?). LITERATURE: the fixed-n tight-locus classification is "widely open" (PerarnauÃ¢â‚¬â€œSerra arXiv:2409.20160); GoddynÃ¢â‚¬â€œWong's infinite tight family needs nÃ¢â€ â€™Ã¢Ë†Å¾; NO published bound controls the safe-MEASURE (only the gap ÃŽÂº(n)), so this is original. The bounded-speed reduction (Tao Thm 1.3/MSS) makes the compactness scaffold rigorous in principle. Entry: THM-523/522, OPEN-Q-097 (the complementary analytic Abel/Bedert route), 04-computation/tight_locus_*_kps.py. RELATED (opus-2026-06-30-S1, THM-591): the AP tight point `{1..13}` is now fully characterized under OBSERVER perturbation — `M_c(AP)=1/n+c(n-2)/n` is exactly linear, strictly increasing in `c` (the gap fattens away from the homogeneous `c=0`). This is a DIFFERENT lever than the fattening lemma (which perturbs SPEEDS and bounds the lonely MEASURE `meas(G_C)`, not the gap `M_c`), so it does NOT close OPEN-Q-108 — but it shows the AP's gap-direction rigidity exactly, and its avoided-arc-edge method (upper bound `M_c<=env`, now ELEMENTARY: the danger arc's left edge is exactly `-1/q`, so the min-gap-forced runner lands in it) may transfer to bounding `meas(G_C)` near the tight locus. STRONGER (HYP-3749): the avoided-arc argument PROVES `M(S)<=1/n` for any DIFFERENCE-CLOSED `S` (all `|s-s'| in S`), and the only primitive difference-closed `(n-1)`-set is the AP `{1..n-1}` (elementary). So the **AP component of the tight locus is rigorously handled** (difference-closed = dilated APs, all tight); the RESIDUAL is exactly the NON-difference-closed tight sets like Goddyn-Wong `{1..11,13,24}` (`12=13-1` absent) -- the fattening lemma need only rule out finitely many of THOSE, and GW's tightness localizes to covering its absent-difference `12t`-gaps via `{13,24}`.
...
**[GROUNDED 2026-06-29 mac-mini-S12 -> HYP-3545]** The empirical keystone is now verified: `N_tau(T) = #{Ham paths of oriented type tau}` has TOURNAMENT-INDEPENDENT parity = the transitive/descent-set count (Forcade), exhaustive n=4,5 + 3000-sample n=6, ZERO variable-parity types; ALL types odd at n=2^k. This is the Ky-Fan/Borsuk-Ulam odd-count GRADED by type on the type-hypercube `{+,-}^{n-1}`; klein THM-584 (complement = antipodal map of the arc-hypercube Q_d) supplies the Z_2; the signed cycle index (HYP-3540) is its GF; arXiv:2512.09332 (El Sahili-El Zein, arc deletion = Q_d edge) is its edge-stability. The remaining OPEN piece is the topological PROOF that this parity IS a Ky-Fan alternating count for ARBITRARY T (not just transitive). See `07-reflections/one-antipodal-map-the-topological-toolkit-merged.md`.

**[ADVANCED 2026-06-29 mac-mini-S13 -> HYP-3546]** Structural realization VERIFIED: flipping ONE arc (= one EDGE of klein's Q_d) preserves `N_tau(T) mod 2` for every type (exhaustive n=5, all 10240 (tournament, arc-flip) pairs, ZERO violations). Since Q_d is connected by arc-flips, `N_tau mod 2` is CONSTANT = its transitive value = Fan's count in the transitive (= Fan's linear-order) gauge. So **Forcade's theorem IS the Q_d-edge-invariance of the graded Ky Fan count** -- a discrete Z_2-equivariant invariant on klein's antipodal hypercube (THM-584), GF = signed cycle index (THM-587). The ONLY remaining piece for a fully elementary proof is the explicit single-arc-flip involution (the Prescott-Su bistellar/path-following move in this gauge) -- verified to exist, not yet in closed form. Tie-in: the Forcade orders n=2^p are the PERFECT-NUMBER arc-hypercube dimensions d=C(2^p,2) (Mersenne-prime apex) -- see HYP-3546 and `07-reflections/perfect-numbers-are-the-arc-counts-of-forcade-tournaments.md`.
