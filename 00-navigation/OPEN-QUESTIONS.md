...
**OPEN-Q-108 LEAN BOUNDARY (mac-mini-S27 retraction + codex S86g positive-p0 theorem): both NU and p0 routes are viable once the proof consumes only `0 < witnessG2`.** The witness route's event side is now fully concrete + sorry-free: `coverSet` (p0 event), `safeSet` (G_P event), `denseSet`, all measurable (`LRCDenseCovers`); `slowÎ¼`=volume.restrict[0,1) is an `IsProbabilityMeasure`; the general Bonferroni `mu(Aâˆ©B)>=muA+muB-1` (`LRCBonferroniMeasure`); and the concrete floor `witness_floor_concrete : meas(G_P)-p0 <= meas(coverSetá¶œ âˆ© safeSet)` (`LRCWitnessFloorConcrete`; via Bonferroni(coverSetá¶œ,safeSet)+complement identity). `LRCWitnessBonferroni` now has the corrected positive-p0 assembly `lrc14_from_p0_positive_wide_bound_split_nodes`: large clusters need only `0 < delta` with `p0<=cap-delta`, not the false k=8 comparison `witnessMP<=delta`; small clusters can still use the existing `m_P` floor. This is `Verify`-audited alongside `witness_margin_from_p0_wide_bound_shapes` and `large_witness_pos_from_p0_wide_bound_shapes`.

**OPEN-Q-108 KPS S31b concrete-p0 addendum:** `LRCFourteenSkeleton.p0` is now definitionally `DenseCovers.p0 E = (slowÎ¼ (coverSet E)).toReal`, not an opaque carrier.  `LRCP0Concrete` proves `0â‰¤p0`, `p0â‰¤1`, `p0=0` for fewer than six distinct speeds, monotonicity, and `wideBoundConcrete_of_decorrelation`, which packages hp0cap as the concrete cover-event inequality modulo the resonance/decorrelation residual.  This strengthens the p0 route's interface: the remaining `p0â‰¤capâˆ’delta` hypothesis now talks about the actual `coverSet` event consumed by the goodSet/Part-A bridges.

**OPEN-Q-108 S86g strict-cover addendum:** `LRCWitnessFloorConcrete.witness_pos_from_strict_cover_bound` now matches the exact output form of `LRCCoverBound.slowÎ¼_coverSet_lt_cap`: `p0(E)<cap_k` and non-strict `cap_k<=meas(G_P)` imply `0<slowÎ¼((coverSet E)^c âˆ© safeSet P)`.  The p0 positivity route therefore does not need to move strictness onto `hmeasGP` or manufacture a separate delta at the concrete floor layer; the older margin lemma remains the finite-ruler error-budget version.

**OPEN-Q-108 S86g dense-complement addendum:** `LRCDenseCovers.coverSet_compl_subset_denseSet_compl` and `LRCWitnessFloorConcrete.dense_compl_witness_pos_from_strict_cover_bound` now transfer the strict hp0cap floor to `0<slowÎ¼((denseSet E)^c âˆ© safeSet P)` for anchored `0âˆˆE`.  This is a formal max-gap proxy: it proves the p0 lower carrier lies in the complement of the 1/7-dense event.  The remaining readout is not another Bonferroni inequality; it is the cyclic sorted-gap bridge identifying `(denseSet E)^c` with the concrete `goodSet E` carrier used by `witnessG2`.

**OPEN-Q-108 S86g dense-complement margin addendum:** the same proxy now carries the quantitative p0 margin: `dense_compl_witness_margin_from_wide_bound` proves `deltaâ‰¤slowÎ¼((denseSet E)^c âˆ© safeSet P)` from `p0(E)â‰¤cap_kâˆ’delta`, `cap_kâ‰¤meas(G_P)`, and `0âˆˆE`.  This is the interface needed by the finite-ruler error-budget side; the remaining open step is still the `denseSetá¶œ` to `goodSet`/`witnessG2` readout.

**OPEN-Q-108 S86g phase-gap addendum:** `LRCDenseCovers.exists_phase_arc_empty_of_not_dense` now proves the finite cyclic-gap part of the readout: `Â¬Dense17` gives a phase with empty right arc `(0,1/7]` in `Int.fract(c-a)` coordinates.  The new `phaseGapSet` satisfies `(denseSet E)^câŠ†phaseGapSet E`, and `LRCWitnessFloorConcrete` transfers both strict positivity and the p0 `delta` margin to `slowÎ¼(phaseGapSet E âˆ© safeSet P)`.  The remaining formal quotient to `GoodSet.goodSet E` is now specifically the speed-level identity from phase differences to `frac((b-a)x)` plus the finite-list witness packaging.

**OPEN-Q-108 S86g goodSet readout addendum:** the previous quotient is now closed.  `LRCGoodSet.phaseGapSet_subset_goodSet` and `denseSet_compl_subset_goodSet` prove the event readout into concrete `GOOD`; `LRCWitnessFloorConcrete.goodSet_witness_margin_from_wide_bound` gives `deltaâ‰¤slowÎ¼(goodSet E âˆ© safeSet P)` from the p0 margin, and `goodSet_witness_pos_from_strict_cover_bound` gives positivity from strict hp0cap plus non-strict `hmeasGP`.  The witness-floor readout side is now concrete over `GOODâˆ©G_P`; the remaining hard nodes are the analytic p0/cap/Part-A inputs rather than this event quotient.

**OPEN-Q-108 S86g2 shape-readout addendum:** the concrete `GOODâˆ©G_P` carrier now plugs into the shape-indexed proof DAG.  `LRCEventMeasureBridge.shape_goodSet_witness_margin_from_wide_bound` proves `delta s â‰¤ witnessG2 s` from a readout equality `witnessG2 s = slowÎ¼(goodSet(Eof s) âˆ© safeSet(Pof s))`, anchored `0âˆˆEof s`, `p0(Eof s)â‰¤cap sâˆ’delta s`, and `cap sâ‰¤slowÎ¼(safeSet(Pof s))`; the strict-cover positivity analogue is also audited.  Thus the p0 route's remaining large-branch interface can be stated directly on `witnessG2`, leaving hp0cap/hmeasGP and finite-ruler Part A as the hard nodes.

**OPEN-Q-108 S86g2 Part-A goodSet-margin addendum:** `LRCWitnessPartA` now composes the concrete goodSet/safeSet readout with the finite-ruler Part-A budget.  `finite_witness_pos_from_goodSet_margin_shapes` and `finite_witness_pos_from_goodSet_margin_uniform_arc_bound_shapes` turn anchored `Eof`, `p0(Eof)â‰¤capâˆ’delta`, `capâ‰¤slowÎ¼(safeSet Pof)`, `witnessG2=slowÎ¼(goodSet Eofâˆ©safeSet Pof)`, and the finite `rhoK/arcCount/Vmax` error budget into positive finite witness density.  `lrc14_from_finite_partA_goodSet_margin_shapes` packages the same bridge into the conditional LRC14 assembly.  Remaining work is now the concrete node content: hp0cap, hmeasGP, concrete `arcCount/rhoK`, and the finite-ruler approximation inequality.

**OPEN-Q-108 S86g2 concrete-p0 Part-A addendum:** the same Part-A bridge now has wrappers whose margin hypothesis is the named concrete atom `DenseCovers.p0 (Eof s)â‰¤cap sâˆ’delta s`, where `DenseCovers.p0 E=(slowÎ¼(coverSet E)).toReal`.  `finite_witness_pos_from_goodSet_p0_margin_shapes`, its uniform-arc-bound variant, and `lrc14_from_finite_partA_goodSet_p0_margin_shapes` let the KPS S31b concrete p0 output feed the goodSet/safeSet finite-ruler route without unfolding p0 at each use.  This keeps the skeleton-facing hp0cap node aligned with the actual cover event.  Post-pull HYP-2840 reframes hp0cap through `p0â‰¤L_y` and scalar `L_y` extremality; these wrappers are route-neutral and will consume either that Ly margin or the older decorrelation-style margin once formalized.
...
**OPEN-Q-108 STATUS: LEG C (GENUINE-WIDE) EXHAUSTIVELY VERIFIED (claude-opus-2026-06-22-S4, HYP-2825 corrects HYP-2817). ~1.59M genuine-wide doublet configs checked (CORRECTED from 1.16M â€” k=9 was not exhaustive in S3), 0 violations, ALL k=9..12 ALL gaps g=1..4, ALL bounded bases (exhaustive C(14,k-2) enumeration). Three-piece structure CLOSED: (I) frozen room Phi<cap + (II) Tornheim R-tail [M*_rig<=22] + (III) finite window [15,50] [exhaustive, all pass]. THM-527 rho*-CRUX VERIFIED FOR GENUINE-WIDE DOUBLETS (HYP-2826): rho*(P,E_co)>0 AND witness(P,E_co)>0 for ALL tested (k,P,B,g,M); global min rho*=2/147~0.0136>0; global min witness~0.483>0. LRC(14) reduces to: BOUNDED + SINGLE-FAR (THM-563, closed) + THIS LEG-C + L0 glue + Lean. See HYP-2817, HYP-2825, HYP-2826, reflection 07-reflections/lrc14-legC-closed-proof-synthesis-claudeopus-0622-S3.md.**

**OPEN-Q-108 â€” THE WIDE BOUND REDUCES TO CONCENTRATION EXTREMALITY OF L_yK8 (claude-opus-2026-06-22, HYP-2812). [LEAD]**
The cleanest closure of the whole wide region: **`max_E L_yK8` over ALL k-speed configs = `max_BOUNDED L_yK8` = MB < 10cap** (gK8=(10,0,0,1,0,0,10) on the miss-distribution, `L_yK8=10q0+q3+10q6`, `q0=p0`). VERIFIED EXACT over ~100k wide configs (incl. ALL binding families + small-M R-tail bumps): k=10 MB=5.286 vs MW=4.813; k=11 6.032 vs 5.632; k=12 6.641 vs MW=6.286=**E*** (the dichotomy-breaker â€” BELOW MB). **NO wide config exceeds the bounded max.** So gK8's BOUNDED cert is GLOBAL âŸ¹ `p0 <= cap` for all E with NO dichotomy, NO doublet, NO R-tail, NO frozen room â€” the wide leg collapses into the BOUNDED leg. MECHANISM/proof-route: gK8 charges the EXTREME miss-counts q0,q6; both are individually maximized by CONCENTRATED (bounded/slowest) configs (q6=all-missed maximized by slowest speeds; q0=coverage by the tight instance consec), and decorrelation smooths the miss-distribution to the MIDDLE (= the survival-middle-mass currency HYP-2701, now MONOTONE). REMAINING: prove concentration extremality (global extremality of consec under gK8 = a smoothing/majorization lemma on the 7-simplex). The explicit FALLBACK (if concentration extremality resists proof) is the generalized-doublet + Tornheim-R-tail route (HYP-2807/2808: max gw config is a generalized doublet {M,M+g}, R-tail = Mordell-Tornheim double sum |R_g|<~2.9 uniform). â†’ HYP-2812, HYP-2811, HYP-2809, HYP-2807, HYP-2808, HYP-2701, THM-534, THM-538, mac-mini gK8/HYP-2810.
...
**OPEN-Q-108 â€” WIDE REGION UNIFIED BY gK8 + the GENERALIZED-DOUBLET / TORNHEIM-R-TAIL frame (claude-opus-2026-06-22, HYP-2807/2808/2809).**
...
**OPEN-Q-108 â€” CORRECTION TO THE GENUINE-WIDE MARGIN CLAIM (kind-pasteur-2026-06-21-kpswf9, HYP-2805): the consec doublet is NOT the genuine-wide maximizer; robust margin 0.16 FAILS at k=10 (true max 265/588, margin 0.1537), though `p0 < cap` holds everywhere.**
The THM-564 / HYP-2804 closure analyzes the CONSEC doublet `consec_{K-2}âˆª{M,M+1}` (k=10 capâˆ’sup=0.16188 â‰¥ 0.16). But an EXHAUSTIVE genuine-wide search (all (kâˆ’2)-subset bounded bases Ã— adjacent far pairs, filtering on `primitive(FULL E)` NOT `primitive(base)`) finds a HIGHER config: **k=10 `{0,2,4,6,8,10,12,14,15,16}` = 265/588 = 0.45068, margin 0.15372 < 0.16** (and k=9 `{0,4,6,8,10,12,14,15,16}`=321/980, margin 0.1667). The maximizing BASE is DILATED (gcd 2, = 2Â·consec_8) while the full set is primitive (15 odd) â€” so HYP-2804's base-primitivity sweep MISSED it. NET: (i) genuine-wide `p0 < cap_k` STILL HOLDS at every k (the actual LRC requirement; all margins > 0) â€” the leg closes; (ii) the ROBUST 0.16 reframe is UNATTAINABLE at k=10 (margin 0.1537). The frozen-law + correction machinery (HYP-2806) applies to these dilated-base doublets too (base=dilated even, far doublet {15,16}); the closure should target `< cap` at k=10 or fold the dilated bases into the doublet family. ACTION for any all-bounded-bases doublet sweep: filter on `primitive(FULL E)`. Scripts `04-computation/lrc14_genuine_wide_true_maximizer_kpswf9.py`. -> HYP-2805, HYP-2806, THM-564, HYP-2804, HYP-2795.
...
**OPEN-Q-108 â€” GENUINE-WIDE BINDING LEG (THE DOUBLET) VERIFIED-CLOSED via the almost-periodic P/R split (kind-pasteur-2026-06-21-Swf9, THM-564 / HYP-2804; CONVERGES with concurrent kps-S27 HYP-2799/2803 frozen-phase route).**
The genuine-wide leg's BINDING sub-case â€” the doublet maximizer `E_M=consec_{K-2}âˆª{M,M+1}`
(opus HYP-2797) â€” is now VERIFIED-closed with the LRC margin 0.16. The mechanism is the
**doublet analogue of THM-563**: center the deviation at the EXACT frozen plateau `Î¦_frozen`
(not `bvd(base,2)`, which leaves a drift `d_inf` making `MÂ·errorâ†’âˆž`, the HYP-2798 puzzle), then
`g(M)=MÂ·(p0(E_M)âˆ’Î¦)=P(M)+R(M)` exactly (inclusion-exclusion), with `P` exactly period-`7Â·lcm(base)`
(EXACT finite period-max, THM-563 sawtooth) and `R=MÂ·(d2(M)âˆ’d_inf)=O(1/M)` (VERIFIED, Koksma on the
single residual far phase). Closure: `p0â‰¤capâˆ’0.16` for `Mâ‰¥M*=âŒˆ(period-max(P)+sup|R|)/H_KâŒ‰` (TINY:
M*=13,24,55 for K=8,9,10) + an EXACT finite window `[15,M*]`. CLOSED K=8,9 (full period 420) and
K=10 (full period 2940, the BINDING case: `capâˆ’sup_p0=0.16188`); K=11,12 window-verified (non-binding).
The crude `sup_g/15<H_K` FAILS at K=9 (razor-thin) â€” the careful `M*`-cutoff is what closes it.
...
**OPEN-Q-108 â€” LRC14 FRONTIER COMPRESSED AFTER BOUNDED AND SINGLE-FAR CLOSURE (codex-2026-06-21, HYP-2793).**
...
**OPEN-Q-108 â€” SINGLE-FAR CLOSED AS A FINITE PERIODIC MAX (mac-mini-2026-06-21-S6, THM-563).** The
signed-cancellation wall (HYP-2784: absolute bound 125Ã— lossy) is COMBINATORIAL, not analytic.
`wÂ·Î”_w = Î£_j Î£_{endpoints t of A_j} Â±S_j(frac(wÂ·t))` (exact Dedekind/sawtooth identity); the arcs `A_j`
depend ONLY on the base `B`, so `wÂ·Î”_w` is EXACTLY PERIODIC in `w` with period `7Â·lcm(B)`, and
`sup_w Î”_wÂ·w` = a finite exact period-max. For the binding consec bases: period-max = `1, 43/49, 1007/980`
(k=8,9,10), all `< 15Â·margin` âŸ¹ `Î”_w < margin` for ALL `wâ‰¥15` â€” no `w`-window finite check, no Koksma,
no reciprocity. The DILATED case (kps's single-perturbation reduction) closes via the CONTINUOUS period-max
(`contmax < 14Â·margin`: `1.0, 0.895, 1.028`). **COMBINED with kps HYP-2788** (near-cap âŸ¹ single-perturbation
âŸ¹ single-far; genuine-wide âŸ¹ slack floor): the wide region closes via THM-563 + the slack floor, AVOIDING
the joint 2D ET-Koksma. Single-block domination (mine) + kps THM-557 confirm multi-block â‰¤ single-block.
Remaining: period-max â‰¤ 15Â·margin exhaustive over all bounded bases (dangerous k=8,9 verified, worst ratio
~10.8 < 15); kps's dichotomy rigor; HYP-2603 (consec maximizes Plat). â†’ THM-563, THM-546, HYP-2788, HYP-2787.

**OPEN-Q-108 â€” PHASE-CARRIER / MAGIC-RANK FILTER (codex-2026-06-20-S67, HYP-2711/T942).**
...
**OPEN-Q-108 â€” TRUE-WIDE SURVIVAL MIDDLE-MASS GATE (codex-2026-06-20-S64, HYP-2701/T936).**
...
**OPEN-Q-108 â€” RESIDUAL-PROFILE AUTOMATON CONE (codex-2026-06-20-S63, HYP-2698/T934).**
...
residual deletion.  In miss-zeta coordinates `Z_x(A)=Pr(AâŠ†residual)`, independent context clusters satisfy
...
**OPEN-Q-108 â€” ARBITRARY CLUSTER COMPRESSION CONE (codex-2026-06-20-S62, HYP-2697/T933).**
...
`Î£_R w_R q_C(R) â‰¤ Î£_R w_R q_K(R)` on that cone, where `K` is the coherent consecutive block.  Grid scouts
...
**OPEN-Q-108 â€” SINGLE-BLOCK EXTREMALITY: JOINT CARRIER GAP REMAINS (codex-2026-06-20-S61, HYP-2694/T931, THM-557).**
...
proved diagonal-freeze error `|p0({0}âˆª{M..M+m-1})-D_m|â‰¤7*C(m,2)/M`, giving conservative large-M cutoffs
...
**OPEN-Q-108 â€” TRUE-WIDE CAP-FLOOR GATE (codex-2026-06-20-S60, HYP-2695/T930).**
...
**OPEN-Q-108 â€” INCLUSION-EXCLUSION-OVER-N COMPREHENSIVE VIEW + REDIRECT (mac-mini-2026-06-20-S5, HYP-2692).**
The LRC's inclusion-exclusions are one arithmetic skeleton indexed by `6=2Â·3`: N=7 sectors (=THM-534
moment-LP, optimal closes k=8-10, plain Bonferroni fails); N=2 quadratic Ï‡ (QR/NQR, Gauss sum âˆšâˆ’7,
reality, Chebyshev bias ~70% non-universal); N=3 cube root C_3 (Eisenstein); keystone â€” C_3 orbit-sum
of 7th roots = Gaussian period `(âˆ’1+Ï‡âˆšâˆ’7)/2`, the correction's C_3 trace âˆˆ Q(âˆšâˆ’7); N=runners danger
...
(verified):** the lever is the summed **leading-order** residual `R_{s_0}`, `s_0=max(1,7âˆ’|B|)` â€” for the
dangerous true-wide leader (|B|=7) that is `R_1` (one-far, barely-far = THM-546/547 collar machinery,
the best-understood piece, not the dâ‰¥2 lattice); for sparse cores it is `R_3+`. `R_tot=p0âˆ’P_r` stays
within margin in all tested rows. The Q(âˆšâˆ’7) C_3-orbits INDEX the resonance classes the height-weighting
sums over; smallness stays signed archimedean. â†’ THM-534, THM-548, THM-551, HYP-2657, HYP-2684, HYP-2692.

**OPEN-Q-109 â€” The base-HP / grid-symmetric maximizer lemma (the one gap in HYP-2688).**
...
upgrades HYP-2688 to a theorem and certifies the search speedup. â†’ HYP-2688, THM-280, THM-552,
SC-maximizer canon.

**OPEN-Q-108 â€” TRUE-WIDE via DICHOTOMY-FINITE-REDUCTION (kps-2026-06-20-Sx-wf, INDEPENDENT route).**
A second, cluster-decorrelation attack on Region III (complementary to the THM-548 boundary-value picture).
[PROVED] `p0(Î»E)=p0(E)` for `gcd(Î»,7)=1` (THM-531-B, re-verified for p0) + pigeonhole (a cluster of size
`â‰¤6` has `p0=0` alone) âŸ¹ a true-wide set's `â‰¥2`-cluster SHAPE-multiset family is FINITE. [VERIFIED/EXACT]
the correct SHARED-`x` decorrelation engine (RIGOR FIX: independent-`x` convolution under-counts, `17/343` vs
true `23/196`; only shared-`x` matches the `Mâ†’âˆž` limit) gives a worst decorrelated `p0_inf` over the WHOLE
finite family `= [k-1 consec]+[singleton] =` the THM-547 plateau `Qb(k-1)` `= 0.19660/0.36210/0.44789/
0.53125/0.60224 < cap_k` (margin `â‰¥0.132`). So TRUE-WIDE decorrelates to NO WORSE than the (closed) boundary
collar. [REDUCTION] explicit gap cutoff `B=(6/49)Â·fÂ·Vmax/margin = 682/1453/1774/1988/2034` (k=8..12,
conservative; signed Abel shrinks `5-76Ã—`): gaps `>B` CLOSED, gaps in `(14,B]` a finite check gluing to span-14.
[VERIFIED] 0 violations of `p0â‰¤cap_k` on >750 exact true-wide rows (margin `â‰¥0.184`); `max(p0âˆ’p0_inf)~0.02-0.05`.
[SOLE GAP] the multi-cluster ERROR AGGREGATION `p0(E)â‰¤p0_inf+Î£_far (6/49)V_i/g_i` (iterate of the PROVED
single-element THM-546) is verified-numerically but not yet closed-form â€” a pure PRODUCT/decorrelation bound,
...
to-the-boundary-collar-plateau.md`. â†’ HYP-2675, THM-547, THM-546, THM-531, OPEN-Q-108.

**OPEN-Q-108 â€” REGION III (true-wide) BOUNDARY-VALUE ARCHITECTURE (mac-mini-2026-06-20-S3, THM-548).**
The far-element process is a BOUNDARY-VALUE problem (the user's lead). Dictionary: bounded core `B`
= boundary point; far runner `wâ†’âˆž` = radial approach; plateau `Î¦(B)` = boundary function (Fatou,
the one-far limit PROVED to exist with rate `6/49`); two-far curvature `I_B(u,v)` = Bagemihl ambiguity
defect; resonance `mu+nvâ‰ˆ0` = ambiguous point = Freiman small relation. **Exact finite decomposition:**
`p0(BâˆªF) = Î£_{SâŠ†F,|S|â‰¤6} Î”_S(B) = P_r(B) + R(B,F)`, where `P_r(B)=Î£_t prof_t(B)c_t(r)` is the
fully-decorrelated **Fatou boundary value** and `R` the resonance corrections. **VERIFIED this session:**
(a) `Î¦â‚‚(B)=(2pâ‚‚âˆ’pâ‚)/49`; (b) `P_r(B)â‰¤cap_k` with margin GROWING in `r` (0.13â†’0.48 â€” boundary value is
safe); (c) two-far constant `Câ‚‚=13/1372=13/(2Â²Â·7Â³)` (parabolic analogue of one-far `3/49=3/7Â²`) â€” each
Abel order adds one power of the apex prime 7; (d) QR-reality of the PRODUCT kernel (licenses the signed
two-far bound); (e) resonance-gated `|I_Bâˆ’Î¦â‚‚|â‰¤CÂ·V/resdist`, worst case `~0.013 â‰ª` margin â€” two-far
curvature is NEVER cap-threatening; (f) consecutive-pair curvature SATURATES (bounded). **CORRECTION**
(re-verified): the k=9 leader `(0,4,6,8,10,12,14,15,16)` has NEGATIVE curvature `âˆ’13/1470` and dilated
core `2Â·(0,2,3,4,5,6,7)` â€” it is the SCALE-INVARIANT branch, not a positive-synergy exception (HYP-2679
literal premise refuted). **REMAINING (honest):** the ONE genuine analytic gap is the SIGNED magnitude
bound for the `dâ‰¥2` relation lattice (absolute bound proven 5Ã— lossy; needs Poisson/theta keeping
`(âˆ’1)^|T|` + the 7-vanishing); plus the divergent-resonance/stacking risk (sup `w|Î”_w|` grows with #scales,
closure relies on the offsetting tiny plateau â€” computational not yet analytic); plus the unrun finite
checks; plus the upstream glue (HYP-2603). LRC(14) NOT proved. â†’ THM-548, THM-546, THM-547, HYP-2679,
HYP-2678, HYP-2637, HYP-2606.

**OPEN-Q-108 PROGRESS (mac-mini-2026-06-20-S2): 2 of 3 sector-crux regions now CLOSED.** The crux
`p0(E)â‰¤cap_k` (k=8,9,10) splits by spread into three regions: **(I) finite half** `max(E)â‰¤14` â€” PROVED
(kps-S19, 0 violations); **(II) boundary collar** `2nd-largestâ‰¤14, max=w>14` â€” CLOSED (THM-547) via
THM-546 sharpened `|Î”_w|â‰¤(6/49)V(E')/w` for `w>w*=54/90/103` + a feasible finite check `14<wâ‰¤w*`
(k=8 verified: 19100 configs, 0 violations, worst margin 0.155); **(III) true-wide** `2nd-largest>14` â€”
OPEN, the Ruzsa/PlÃ¼nnecke+Freiman program (HYP-2678: `d=1`â†’scale-invariance, `dâ‰¥2`â†’signed dimension
penalty). The signed (6/49) bound (THM-546 S2, from the QR reality HYP-2657, `6=âˆ’1`âˆˆNQR mod 7) is the
shared engine codex's HYP-2676/2677 packet route also adopted. Only region III remains. â†’ THM-546, THM-547,
...
**THE 1/7 PIVOT â€” LRC(14) reduced to ONE lemma (kind-pasteur-2026-06-18-S5, THM-530/HYP-2602):** the residual's correct object is NOT the via-max density Ï*_{2/7} (REFUTED: Î¼_{2/7} has no floor â€” exact k=13 sets with Î¼_{2/7}<1/14; Ï*_{2/7}=0 on admissible (P,E) that are nonetheless lonely) but the GLOBAL-WITNESS density Ï*_{1/7}(P,E)=meas(G_Pâˆ©{maxgap{frac(e_i x)}>1/7}) (a free fast-phase exists âŸº the cluster phases leave a 1/7-gap âŸº M(S)â‰¥1/14). Two-branch floor: **kâ‰¤7 PROVED unconditional** (pigeonhole Î¼_{1/7}=1 âŸ¹ Ï*=meas(G_P)â‰¥m_P=14249/252252); **kâ‰¥8 union bound** Ï*_{1/7}â‰¥meas(G_P)+Î¼_{1/7}(E)âˆ’1>0 contingent ONLY on **HYP-2602 (the 1/7-spread bound): Î¼_{1/7}(E)â‰¥thr_k** (consecutive minimizes; â‰¥0.32 slack; binding k=8; VERIFIED, survived the descent that killed 2/7). With the upstream finite-Vmax/integer glue (THM-527-A), HYP-2602 âŸ¹ LRC(14). This is the single remaining analytic gap. â†’ THM-530, HYP-2602, HYP-2592.  **[CONVERGENCE â€” mac-mini-2026-06-19-S1, 8-angle workflow]:** six independent angles now reduce LRC(14)-S3 to ONE scalar lemma HYP-2607 = THM-534's `consec maximizes the empty-sector moment functional L_y(E)=E[g(N)]`. THM-534 (LP dual, PROVED per-E meas(S7)<=L_y) CLOSES the dangerous rows k=8,9,10 (L_y(consec_8)=2633/7350<cap_8); THM-537 (Beurling/U4 moment LP) converges EXACTLY on it; THM-535 collapses the finite check to those same 3 rows; HYP-2606 proves the ABSOLUTE bound is 5x lossy so the closer must be SIGNED (L_y is). HYP-2607 is NON-separable (component moments fail) â€” in distributional form (k=8): consec maximizes P(N=0)+(1/10)P(N=3)+P(N=6), a convex-order/coupling on the empty-sector count N. â†’ THM-534/535/537, HYP-2606/2607.


**Residual-case S3 OPEN-Q-108 update (kind-pasteur-2026-06-18-S3 / THM-526, HYP-2581, MISTAKE-076):** the LRC(14) covering reduction's last gap (case S3 = covering 13-sets, k=|{v>13}|â‰¥2, spreadâ‰¥13Ã—) advanced. PROVED: the **k=2 slice** (exactly 2 large speeds âŸ¹ Mâ‰¥1/14, via drop-max scaling Vâ‚‚â‰¥51 + bounded core Vmaxâ‰¥63 + finite check â‰¤62 = 4865 sets, worst M=1/12) and **cluster-collapse Lemma A** (window W_K safe for the whole cluster, nonempty iff 13Vminâˆ’Vmax>14Ks; closes single-gap clusters). CORRECTION (MISTAKE-076): the criterion C(S)=âˆƒv:W(S\{v})>1/(7v) is SUFFICIENT but **NOT universal** â€” S\*={1,2,3,5,7,8,9,10,11,12,13,38,42} (covering S3, k=2) has C fail for all v yet M=2/23; so "prove C for all covering S" cannot close LRC(14), and a bounded-speed reduction is REFUTED (S3 infinite: AP family {t,2t,â€¦,12t,V}). The residual is now UNIFIED and asymptotically TIGHT: the criterion margin's Vâ‚€â†’âˆž limit-infimum is EXACTLY 1, lifted to a realized floor >1 by covering+primitivity discreteness â€” no compactness argument; closing it = a uniform positive density floor (Weyl/three-distance, route a) or multi-band CRT placement (route b). NEXT: the rigorous Ï*(Î”,P)â‰¥câ‚€>0 equidistribution lemma. â†’ THM-526, HYP-2581d. **ADVANCE (mac-mini-2026-06-18-S1, THM-527, HYP-2584..2586):** the slow-fast change of variables Ï†=frac(VmaxÂ·Ï„) reformulates route (a) as a clean single-variable **lonely density** Ï*(P,E)=meas{xâˆˆG_P : the k cluster phases {frac(e_i x)} (e_i=Vmaxâˆ’u_i) have circular max-gap >2/7 âŸº fit in a 5/7-arc}, with Ï*>0 âŸ¹ M(S)â‰¥1/14 (PROVED in the w0â†’âˆž limit; Ï_Kâ†’Ï* VALIDATED). **The "no-compactness/asymptotically-tight" obstruction DISSOLVES**: it is about the gap-WIDTH (marginâ†’1), but the proof needs the gap-MEASURE Ï*, which IS compact â€” the extremal cluster has BOUNDED SPREAD (huge spread RAISES Î¼; verified), so the shape space is finite-dimensional/compact and inf Ï* is a positive minimum (no Ï*=0 found; consecutive-cluster exact floor 1/84). PROVED: k=3 (margin 4/3). CORRECTION: consecutive is NOT the globally extremal shape (HYP-2585). The isolated remaining crux = the rigorous uniform floor câ‚€>0 on the compact (P, bounded-spread shape) space. Files: 04-computation/lrc14_{rho_star_limit,threedistance_floor,exact_floor,spread_floor,broadfloor}_macmini_0618s1.py.
**Cluster-size split OPEN-Q-108/109 update (kind-pasteur-2026-06-18-S4 / THM-527, HYP-2581e/f):** the S3 residual sharpened. CONVERGENCE: mac-mini (THM-527 reservation) and kind-pasteur (slow-fast/offset-fit) independently reached the same reformulation [good x âŸº xâˆˆG_P AND the cluster phase-points {frac(e_i x)} leave a circular gap > 1/7 (global witness) / > 2/7 (via-max criterion)]. PIGEONHOLE: maxgapâ‰¥1/m âŸ¹ marginâ‰¥7/mâˆ’1; AUTOMATIC for |L|â‰¤6 (global witness) / |L|â‰¤4 (criterion); |L|â‰¥7/|L|â‰¥5 = the Ï* hard case. PROVED this session: THM-527 (fixed-small-part single-tight-cluster, explicit V0*, global-witness Î¸-sweep); AP-family {1..12,m} (Mâ‰¥2/27, but k=1=S1); ALL-MULT7-LARGE window-collapse (conditional on w_max<14 w_min). The 7-adic angle is REFUTED as the uniform-floor mechanism (floor = small THM-524 binding pairs). REMAINING OPEN = the COORDINATED-GROWTH CORE (kâ‰¥3, no fixed bounded small part, exemplar {t,2t,â€¦,12t,V}), asymptotically tight (M floors at 2/23 from above, criterion-margin limit-inf=1). Finish line = uniform Ï*(Î”,P)â‰¥c0>0 (three-distance/Weyl) OR (THM-524) covering forbids binding denominator D=14qâˆ’r with small r. â†’ THM-527, HYP-2581d.

**Residual-case S3 OPEN-Q-108 update (kind-pasteur-2026-06-18-S3 / THM-526, HYP-2581, MISTAKE-076):** the LRC(14) covering reduction's last gap (case S3 = covering 13-sets, k=|{v>13}|â‰¥2, spreadâ‰¥13Ã—) advanced. PROVED: the **k=2 slice** (exactly 2 large speeds âŸ¹ Mâ‰¥1/14, via drop-max scaling Vâ‚‚â‰¥51 + bounded core Vmaxâ‰¥63 + finite check â‰¤62 = 4865 sets, worst M=1/12) and **cluster-collapse Lemma A** (window W_K safe for the whole cluster, nonempty iff 13Vminâˆ’Vmax>14Ks; closes single-gap clusters). CORRECTION (MISTAKE-076): the criterion C(S)=âˆƒv:W(S\{v})>1/(7v) is SUFFICIENT but **NOT universal** â€” S\*={1,2,3,5,7,8,9,10,11,12,13,38,42} (covering S3, k=2) has C fail for all v yet M=2/23; so "prove C for all covering S" cannot close LRC(14), and a bounded-speed reduction is REFUTED (S3 infinite: AP family {t,2t,â€¦,12t,V}). The residual is now UNIFIED and asymptotically TIGHT: the criterion margin's Vâ‚€â†’âˆž limit-infimum is EXACTLY 1, lifted to a realized floor >1 by covering+primitivity discreteness â€” no compactness argument; closing it = a uniform positive density floor (Weyl/three-distance, route a) or multi-band CRT placement (route b). NEXT: the rigorous Ï*(Î”,P)â‰¥câ‚€>0 equidistribution lemma. â†’ THM-526, HYP-2581d.
...
**Easy-dominates-hard OPEN-Q-108 update (kind-pasteur-2026-06-17-S2 / THM-525, HYP-2573..2576):** the covering case of LRC(14) is LOCALIZED onto OPEN-Q-108 by a non-circular reduction QâŸ¹P (Q=uniform meas(G_C)â‰¥c): a covering 13-set is an easy 12-core C (LRC(12)-lonely) plus a runner wâ‰¡0 mod 14 parked in section 0; STEP-1 closes the non-covering case unconditionally, STEP-2/3 rewrite the covering case via meas(G_C) and the decoupling floor. The reduction reaches NOTHING strictly weaker than Q (the kâ‰¥3 coordinated-growing-speed regime is Q unchanged). NEW SHARPENINGS: (i) the conjectured extremal min meas(G_C)=7/858 confirmed over ~135k cores (drop-6 AP core); (ii) a plateau datum â€” driving 2 coordinated parked speeds â†’âˆž does NOT send Lâ†’0 (L plateaus â‰ˆ0.0238, coordinated cores keep measâ‰¥~0.095â‰«7/858), so the feared "meas(G_C)â†’0" failure mode did not materialize; (iii) a SECOND named sub-gap **G2** (the transversality estimate: w's danger comb, measure 1/7, concentrated near {a/w}, cannot CONTAIN all of G_C) â€” distinct from the uniform-measure GAP A, nonempty in every computed case, no general argument; (iv) the "perfect middle of section 0" is a constructive certificate device (survivor-sufficiency + meas(T_w)=6/7 PROVED), NOT the optimizer (which edge-binds w at 7/89); (v) the naive LRC(12)+Lipschitz lever is REFUTED (safe-arc half-width ~1/v_max). ~105k covering 13-sets, zero counterexamples. THE NEXT STEP that would CLOSE it: a bounded-speed reduction (Tao Thm 1.3 / MSS) making the finite covering-check a certificate up to v_maxâ‰¤Vâ‚€; else a direct attack on G2.
...
**Status codes:** ðŸ”´ CRITICAL (blocks main proof) | ðŸŸ¡ IMPORTANT (needed for paper) | ðŸŸ¢ INTERESTING (worth exploring)

## OPEN-Q-108 ðŸ”´ The uniform fattening lemma â€” the ONE lemma that completes the singular-series proof of LRC(14)
**Status:** OPEN â€” the isolated crux (kind-pasteur-2026-06-17-S1, THM-523, from the prove/disprove dialectic). By THM-523 (decoupling floor + single-perturbation inf=1/1260 + quantization THM-522), `inf_S L(S)>0 âŸ¹ C'(14) âŸ¹ LRC(14)` reduces to: **âˆƒ c>0 with meas(G_C) â‰¥ c for EVERY 12-subset C of distinct positive integers**, where `G_C = [0,1)âˆ–âˆª_{vâˆˆC}D_v` is the gap-1/14 lonely set of `C` (`D_v` = `v`'s danger arcs). Equivalently: **the primitive tight locus at n=13 is FINITE** (conjecturally exactly `{AP {1..13}, Goddynâ€“Wong T5 {1..11,13,24}}`). KNOWN: the decoupling bound `L(Câˆª{w})â‰¥(6/7)meas(G_C)âˆ’r/(7w)` handles speeds growing ONE AT A TIME (floor 1/143) and iterates for `k` large entries while the residual `(13âˆ’k)`-core keeps positive measure; the ONLY uncontrolled regime is `kâ‰¥3` arithmetically-coordinated growing speeds (the drop-6 family minimizes at the large `w=69`). THE LEVER (re-verified): PROVEN LRC(12) gives exactly one 12-subset of `{1..14}` tight at gap 1/13, zero at gap 1/14 â€” converting the crux from EXISTENCE (`meas(G_C)>0`?) to TRANSVERSALITY (does the isolated gap-1/13 maximizer FATTEN to a uniformly-positive gap-1/14 measure?). LITERATURE: the fixed-n tight-locus classification is "widely open" (Perarnauâ€“Serra arXiv:2409.20160); Goddynâ€“Wong's infinite tight family needs nâ†’âˆž; NO published bound controls the safe-MEASURE (only the gap Îº(n)), so this is original. The bounded-speed reduction (Tao Thm 1.3/MSS) makes the compactness scaffold rigorous in principle. Entry: THM-523/522, OPEN-Q-097 (the complementary analytic Abel/Bedert route), 04-computation/tight_locus_*_kps.py.


## OPEN-Q-107 ðŸŸ¢ The Alcuin "+1" as a non-minor-closed correction: forbidden-subgraph set + the cover-internal-edge mechanism (general graphs)
**Status:** OPEN (mac-mini-2026-06-15-S6, THM-520, HYP-2553..2555). Complementary to OPEN-Q-106 (kind-pasteur, Î©-specific). General conflict-graphâ†’tournament map T_G (i<j: arc iâ†’j iff edge else jâ†’i; forward arcs = edges). VERIFIED exact nâ‰¤6: independent set â†” reverse-transitive run (Ï„(G)=nâˆ’largest reverse-transitive run of T_G), #3-cycles=#ordered induced Pâ‚ƒ, #HamPaths(T_G) odd (RÃ©dei shadow). Csorbaâ€“Hurkensâ€“Woeginger: Ï„ â‰¤ Alcuin â‰¤ Ï„+1 (the +1 decided by CHW Lemma 4.3 / Thm 3.1). HEADLINE (THM-520): Ï„ is minor-monotone (âŸ¹ {Ï„â‰¤k} minor-closed âŸ¹ finite Robertson-Seymour obstruction set, the Kuratowski {Kâ‚…,Kâ‚ƒ,â‚ƒ} analogue), but **Alcuin is subgraph-monotone yet NOT minor-monotone â€” it fails ONLY under edge contraction** (smallest witness: contract an edge of Kâ‚‚,â‚„, Alcuin 2â†’3; mechanism = contraction creates an edge INSIDE a minimum vertex cover). So {Alcuinâ‰¤k} is NOT minor-closed. QUESTIONS: (a) prove the contraction mechanism in general (Alcuin jumps iff contraction over-commits a min cover); (b) since Alcuin is subgraph-monotone it HAS a finite forbidden-SUBGRAPH obstruction set â€” compute it for small k (k=1: edgeless nâ‰¥1, Kâ‚,â‚ƒ, â€¦); (c) the +1 has NO clean T_G strong-connectivity signature (HYP-2555 refuted "+1 âŸº non-strong" and "G-Ham-cycle âŸº âˆƒ-order-strong"; the only clean strong-order fact is âˆƒ-order-T_G-strong âŸº G neither empty nor complete, nâ‰¤7) â€” find the right tournament invariant for the +1 or prove it is genuinely order/cover-combinatorial (not a tournament-iso invariant); (d) nâ‰¥7 confirmation. Entry: THM-520, 04-computation/alcuin_tournament_macmini_0615s6.py, CHW SIAM JDM 24(3) (JSTOR 41642576), Robertson-Seymour, Moon 1966.

## OPEN-Q-106 ðŸŸ¢ The forbidden-sub-tournament characterization of "Î©(T) planar" (Kuratowski/Robertsonâ€“Seymour for conflict graphs)
**Status:** OPEN (kind-pasteur-2026-06-16-S1, THM-519, HYP-2551). "`Î©(T)` planar" is a HEREDITARY tournament property (tournament-vertex-deletion = `Î©`-induced-subgraph). In the `Î±(Î©)=1` regime `Î©=K_m`, planar âŸº `mâ‰¤4`, so the minimal obstruction is the `n=5, H=11` tournament (`Î©=Kâ‚…` = 5 pairwise-overlapping odd cycles). (a) Enumerate the minimal "Î©-non-planar" tournaments â€” is the `K_{3,3}`-driven obstruction (needs `Î±(Î©)â‰¥2`) also minimal at some `n`? (b) Is the forbidden set FINITE or infinite? Tournaments are not WQO under sub-tournament order (Chudnovskyâ€“Seymour antichains), so possibly infinite â€” but is it finite within bounded-cutwidth/pathwidth tournaments (where Chudnovskyâ€“Seymour DO get WQO)? (c) CHW prove small-vs-large-boat is poly-decidable for planar graphs; so on planar-`Î©` tournaments, `Alcuin(Î©)` is easy though `Ï„(Î©)=#oddcyclesâˆ’Î½_odd` is hard â€” exploit this. Entry: THM-519, THM-517, Csorbaâ€“Hurkensâ€“Woeginger SIAM JDM 24(3) 2010, `04-computation/alcuin_conflict_graph_kps.py`.

## OPEN-Q-105 ðŸŸ¢ A closed form / Î¸-product for the Burnside core kernel B[m,t], and the unified metagraph-enumerator family
**Status:** OPEN (kind-pasteur-2026-06-15-S7, THM-516, HYP-2538/2539; renumbered from OPEN-Q-103 â€” collision with codex-S12's prime-tail-ladder OPEN-Q-103, a COMPLEMENTARY view of the same kernel). The 1-tail peeling isolates A000568's difficulty in the `n`-independent core kernel `B[m,t]=Î£_{Î¼:|Î¼|=m,â„“=t,odd partsâ‰¥3}2^{e(Î¼)}/z_Î¼`, with `e(Î¼)=C(t,2)+Â½Î£_{d oddâ‰¥3}Ï†(d)M_dÂ²` (positive-definite GCD/Euler-Ï† quadratic form â†’ theta-function exponent; VERIFIED 1113 cores). (a) Does `B[m,t]` have a closed product/Î¸ generating function over part-sizes, diagonalizing the Ï†(d)M_dÂ² cross-coupling (HYP-2538)? The add-a-part recurrence `Î”e=(pâˆ’1)/2+Î£_{d|p}Ï†(d)M'_d` closes only on the divisor-profile state â€” that IS the residual difficulty. (b) Do `G_n(x)` (graphs, A000088), `SC(n)` (base-4), `E_n` (A002854 even graphs) share the same core-kernel compression with their per-part rule (HYP-2539)? (c) Is the `P_n(x)=Î£_{odd-cycle Ïƒ}x^{#edge-orbits}` coefficient triangle (rowsâ†’A000246) a new OEIS sequence? Entry: THM-516, THM-505 (cores = A000009âˆ’3 OCF non-spectral family), codex-S12's THM-514 (the tail-ladder complement), `04-computation/burnside_core_kernel_phi_reframe_kps.py`.
...
## OPEN-Q-096 ðŸŸ¢ â€” The other faces of the master cycle-packing polynomial Î¦

**Source:** monad-explorer-2026-06-15-S5, HYP-2514, reflection `the-master-cycle-packing-polynomial`, THM-505.

The spectrum (Sachs Coefficient Theorem, all-length **signed** `y_k=âˆ’x^{âˆ’k}`) and the OCF
`H` (odd-only **unsigned** fugacity-2 `y_k=2[k odd]`) are two evaluations of one master
disjoint-cycle-packing polynomial `Î¦(T;{y_k}) = Î£_{packings}âˆ y_{|C|}`. Open:

1. **The ALL face is the PERMANENTAL POLYNOMIAL â€” RESOLVED (monad-S6, THM-506, HYP-2515).**
   The all-length unsigned face of Î¦, graded by **vertices**, is `per(xI+A) = Î£_m e_m^unsigned
   x^{nâˆ’m}` (`e_m^unsigned`=#packings on m vtx) â€” the permanental polynomial, the **unsigned
   twin of the char poly** `det(xIâˆ’A)`; the two differ ONLY by the cycle-parity sign
   `(âˆ’1)^#cycâ†’+1` (the det/per dichotomy; det spectral & poly-time, per non-spectral &
   #P-hard). Non-spectral first at n=6 (same wall), splits the same 47 cospectral classes as
   H at n=7 but strictly **finer** (recovers (c6,c7,â€¦) vs H's one functional); **(char,perm)
   determines H iff nâ‰¤7**, breaking at n=8 via the D44â†”D35 trade (within-class rank
   3â†’4). **EVEN FACE RESOLVED (monad-S7, HYP-2517):** the even face's SIGNED form IS a
   clean matrix function â€” the skew char poly `det(xIâˆ’S)`, `S=Aâˆ’Aáµ€`, `=âˆ(xÂ²+Î¼_jÂ²)` with
   coeffs `Î£_W Pf(S[W])Â²` (Coates: odd cycles cancel under reversal) â€” but it is
   **SPECTRAL** (a function of charA; VERIFIED nâ‰¤6 exh, incl. the cospectral-different-H
   ...
   `I(Î©_even,Â·)` (splits 3@n6,46@n7) has NO determinantal home, like H. **The det/per
   ...
   `w_k=1áµ€Aáµ1` are spectral â€” exact closed form `1áµ€adj(xIâˆ’A)1=(âˆ’1)â¿charA(âˆ’xâˆ’1)âˆ’charA(x)`,
   equivalently `F(x)=âˆ(x+1+Î»áµ¢)/âˆ(xâˆ’Î»áµ¢)âˆ’1`, via the matrix-determinant lemma + the
   tournament identity `Aâˆ’J=âˆ’(Aáµ€+I)` (the all-ones perturbation collapses to a
   transpose-shift with FORCED eigenvalues `{âˆ’1âˆ’Î»áµ¢}`, no angle dependence; this is exactly
   why tournaments escape the cospectral walk obstruction `Câ‚„âŠ”Kâ‚` vs `K_{1,4}`). So the
   whole A-affine pencil determinant is now PROVABLY spectral, not just verified â‰¤n7;
   `w_k=C(n,k+1)+spectral cycle corrections` (`w_2=C(n,3)+2câ‚ƒ`, `w_3=C(n,4)+(2nâˆ’3)câ‚ƒ`);
   reciprocity `(1+F(x))(1+F(âˆ’xâˆ’1))=1` centred at `âˆ’1/2` (= fixed point of complementation
   on the spectral axis, same `âˆ’1/2` as THM-055/059/080). STILL OPEN: the
   permanental ROOTS as an invariant; the even/all dimension growth law; the general-n
   carrier deficit of (char,perm); the POINTED walk data `1â‚áµ€(xIâˆ’A)â»Â¹1_b` / `M[a,b]` â€”
   where exactly does the spectral closed form break as we de-contract (handoff, this session).
2. **The signed odd face is "more spectral" â€” RESOLVED at n=7.** `sgn_odd =
   Î£_{odd packings}(âˆ’1)^{#cyc} = I(Î©,âˆ’1) = âˆ’Ï‡Ìƒ(Ind Î©)`, the reduced **Euler characteristic of
   the odd-cycle packing complex**. At n=7 it equals `(1+eâ‚ƒ+eâ‚…+eâ‚†) + (câ‚†âˆ’câ‚‡)` (verified
   3000/3000): non-spectral content is the **1-D** direction `câ‚†âˆ’câ‚‡`, a projection of `H`'s
   2-D `(câ‚†,câ‚‡)`. It splits 16 cospectral classes (iff `Î”(câ‚†âˆ’câ‚‡)â‰ 0`) vs `H`'s 47 (iff
   `Î”(2câ‚†+câ‚‡)â‰ 0`); `47 = 16 + 31`, the 31 gap = classes where `câ‚†~câ‚‡` **covary** and the
   alternating sign `x=âˆ’1 âŠ¥ (1,1)` cancels them. **The fugacity `x` is a dial selecting a
   linear functional of the non-spectral level-sums `Î±_j`.** OPEN: the general-n analogue
   (the Euler-char direction vs the `H` direction in the `âŒŠn/3âŒ‹`-D `Î±_j` space); does some
   `x` minimize the split count (the "most spectral" fugacity)?
3. **General-n Sachs-basis skeleton.** Prove `S = 1 âˆ’ 2eâ‚ƒ âˆ’ 2eâ‚… + 4Â·Î£_{m even â‰¥6}e_m`
   (verified nâ‰¤9) for all `n â‰¤ 11`, and derive the `n=12` refinement when the `(3,3,3,3)`
...
the FIRST non-spectral quantity inside `alpha_1` â€” cospectral `n=7` tournaments realize
`c7 âˆˆ {4,5,10}` at identical `tr(A^k)` (THM-500). So at `n=7` the *uncorrected* trace
...
**FULL ANSWER (monad-explorer-2026-06-14, THM-502 â€” the closed-walk census ladder):**
...
(parts â‰¥3) partitioning k, and each configuration `C` contributes `k/period(C)`. This
...
overlapping 4-cycle pairs, TF = overlapping (triangle,5-cycle) pairs â€” NEW). The
...
count trades 1-for-1 against the overlap count â€” the exact reason corrected trace
vectors do NOT compress `H` past n=6: the corrections ARE the non-spectral
support-geometry. Confirms (via the exhaustive nâ‰¤6 spectral-horizon table) that `c6`
...
*reconstructs `H` exactly*. Substituting the defects (`p33=W6âˆ’c6`, `TF=W8âˆ’c8âˆ’Q44`) into
the OCF `H=I(Î©,2)=Î£2^k Î±_k` gives a clean **spectral skeleton + carrier** split:
n=7 `H = [1+2c3+2c5+4C(c3,2)âˆ’4W6] + 4c6 + 2c7` (PROVED); n=8 adds `+4c8+4Q44`
(equiv. minimal-defect `+4c6+2c7âˆ’4TF`). The fugacity `x=2` sets the weights (`2^level`).
...
non-spectral DOF of `H` is `nâˆ’5` (=0,1,2,3 for n=5..8); the carriers are the simple-cycle
...
**n=9 RESOLVED â€” DIMENSION BREAKS, LINEARITY NEGATIVE (monad-explorer-2026-06-15,
THM-505 extended):** n=9 closed form PROVED & verified 45000/45000:
`H = [skeleton] + 2c7 + 2c9 + 4c6 + 4c8 + 4Q44 + 8Â·T333` (the triple level Î±â‚ƒ=T333 turns
on with weight 8=2Â³; full fugacity form `I(Î©,x)=SKEL(x)+(c7+c9)x+(c6+c8+Q44)xÂ²+T333xÂ³`).
(1) **dim â‰  nâˆ’5 at n=9 â€” it is EXACTLY 6:** nested chain (130000 samples)
`sigâ†’+(c6,c7,c8)â†’+c9â†’+Q44â†’+T333` splits `14804â†’482â†’24â†’1â†’0`. So `(c6,c7,c8,c9)` does NOT
...
so `dim_nonspec(H) = 6 > nâˆ’5 = 4`. BOTH overlap configs Q44 and T333 are INDEPENDENT
carriers, not spectral shadows â€” the dimension JUMPS 3â†’6 at n=9 (n=8 was the last size where
...
linear (nâ‰¤7) â†’ non-polynomial-functional (n=8) â†’ independent-correlations (n=9). The
...
**GROWTH LAW RESOLVED â€” A PARTITION FUNCTION (monad-explorer-2026-06-15-S3):** the dimension
growth law DOES have a closed form. In the basis-independent OCF *packing* basis (expand
`H = Î£_Î» 2^{|Î»|} N_Î»` over length-multisets `Î»` of disjoint odd-cycle packings, parts odd â‰¥3),
`dim_nonspec(H)(n) = #{partitions of sâ‰¤n into odd parts â‰¥3} âˆ’ 3` = `Î£_{sâ‰¤n}[x^s]Î _{k oddâ‰¥3}1/(1âˆ’x^k) âˆ’ 3`
= **1,2,3,5,7,9,12,15,19** for n=6..14 (increment = p_{oddâ‰¥3}(n)). VERIFIED by RANK of the
...
is intrinsically **5, not 6** â€” the trace-basis "6" over-counted because `c8` and `Q44` enter
`H` only via their sum `D35`. `dim â‰¤ #{Î»}âˆ’3` PROVED; equality (no `N_Î»` spectrally pinned)
VERIFIED nâ‰¤11, CONJECTURE general. FILES: 04-computation/ocf_nonspectral_n10_monad.py,
05-knowledge/results/{ocf_nonspectral_n10_n11_monad.out, ocf_nonspectral_n11_monad.out}, reflection
the-non-spectral-dimension-of-H-is-a-partition-function. NEW open: (1) PROVE no `N_Î»` is
spectrally pinned (upgrades the law to a theorem); (2) where does the law first deviate, if
ever â€” does a tightness-pinning kill a carrier at large n (the rank could drop below the
...
sequence is A000009.** One-line GF identity: `Î£_{sâ‰¤n}[x^s]Î _{k oddâ‰¥3}1/(1âˆ’x^k) =
[x^n]Î _{k oddâ‰¥1}1/(1âˆ’x^k) = q(n)` (the cumulative `1/(1âˆ’x)` IS the missing odd part `k=1`),
and `q(n)` = #partitions of `n` into odd `=` distinct parts `=` **A000009**`(n)`. So
`dim(packing) = A000009(n)âˆ’3`, asymptotically `~ exp(Ï€âˆš(n/3))/(4Â·3^{1/4}n^{3/4})`
(super-polynomial). **CORRECTION to the headline:** `A000009(n)âˆ’3` is the non-spectral dim of
the **packing-count vector** `(N_Î»)`, NOT of `H`. `H = I(Î©,2) = 1+Î£_{jâ‰¤âŒŠn/3âŒ‹}2^j Î±_j` depends
only on the **level-sums** `Î±_j=Î£_{|Î»|=j}N_Î»` (it never sees the length-split of a level), and
`Î±_j=0` for `j>âŒŠn/3âŒ‹`. So **`dim_func(H)(n) â‰¤ âŒŠn/3âŒ‹` (LINEAR), PROVED**, `= âŒŠn/3âŒ‹` for nâ‰¥7;
verified n=8 (level-sum rank 2 vs carrier rank 3, H in level-sum span). The fugacity-2
evaluation compresses `exp(âˆšn)â†’n/3`. NEW open: (4) prove the levels `Î±_j` are non-spectrally
independent (âŸ¹ `dim_func(H)=âŒŠn/3âŒ‹` exactly). FILES: 04-computation/ocf_two_dimensions_monad.py,
...
## OPEN-Q-002 -- RESOLVED
**Prove Claim A: H(T) âˆ’ H(Tâˆ’v) = 2Î£_{Câˆ‹v} Î¼(C)**
...
Their formula: ham(DÌ„) = Î£_{Ïƒ âˆˆ S(D), all cycles odd} 2^{Ïˆ(Ïƒ)}.
For tournaments, DÌ„ = D^op (converse) and ham(D^op) = ham(D) by path reversal.
...
**Prior verification record:** nâ‰¤8 exhaustive (THM-015), nâ‰¤10 random sampling, all consistent.
...
The 3-cycle-only formula (per-path identity) fails for nâ‰¥6. The natural generalization summing over all odd cycles overcounts. The maximal-embedding-only formula also fails. Is there a formula of the form Î£_{cycles C, (non-v consecutive in P')} f(C, P') = (inshatâˆ’1)/2 that works for all n?

**Note:** Since OCF/Claim A is now proved (Grinberg-Stanley), this is no longer blocking any main result. Downgraded from ðŸŸ¡ to ðŸŸ¢.
...
## OPEN-Q-006 ðŸŸ¢
**Asymptotic formula for Î£_C Î¼(C)**

The average Type-II contribution per L-cycle window is (L-4)/4, growing linearly with L. Does this yield an asymptotic formula for Î£_C Î¼(C) as a function of the cycle-length distribution of T? What happens for random tournaments as nâ†’âˆž?

---

## OPEN-Q-007 ðŸŸ¡
**Full proof of Fix(Ïƒ) = 2^{mÂ²} for self-evacuating SYT**
...
## OPEN-Q-008 ðŸŸ¢ â€” PARTIALLY RESOLVED
**2-adic tower: what is the 2-adic valuation of H(T)?**

**PARTIALLY RESOLVED (opus-2026-03-05-S13):** v_2(H(T)) = 0 for ALL tournaments (this IS Redei's theorem â€” H(T) is always odd). Verified exhaustively at nâ‰¤6 and sampled at n=7 (5000 tournaments).

The mod-4 structure: H(T) â‰¡ 1 + 2Â·alpha_1 (mod 4) via OCF, where alpha_1 = #odd cycles in Omega(T). At n=3,4 this equals 1+2Â·c_3 (mod 4), but at nâ‰¥5 the relationship breaks because 5-cycles contribute.

**Reformulated question:** What is the distribution of H(T) mod 2^k for kâ‰¥2? Computations show it approaches uniform on odd residues as n grows. The OCF gives H mod 4 via alpha_1 parity, H mod 8 via alpha_1 and alpha_2, etc.
...
- A - D ranges from -12 to +9 (mean â‰ˆ 0)
- Mean(A-B) â‰ˆ -Mean(B-D) is approximate, not exact
...
## OPEN-Q-013 ðŸŸ¡
**Correct formula for H(T_p) for Paley primes p â‰¡ 3 (mod 4)**
...
**Sequence H/|Aut|:** 1, 9, 1729, 6857869865, 62293308207033 â€” no obvious pattern. 3^k pattern breaks catastrophically at p=11. Factorizations are erratic. |Aut(T_p)| = p*(p-1)/2 confirmed for all p (affine QR group).

**ADDENDUM (monad-explorer-2026-06-07, HYP-2306): the "modular significance" of 1729 is REFUTED â€” and the erraticness is now EXPLAINED.** `the-tessellation.md` (Layer 6, opus-S131) read `1729 = r(11) = 7Â·13Â·19 = j(i)+1` as a *modular* fact (completely split in Q(âˆšâˆ’3); appeared at the first genus-1 Paley prime). The sharpest test is **p=19, the next Paley prime, which is ALSO genus 1** (`X_0(11)=X_0(19)=`genus 1, `X_0(23)=`genus 2) â€” *not* the p=23 the reflection guessed. The structure does NOT persist: `r(19)=5Â·7Â·11Â·23Â·774463` has 5,11,23 INERT (â‰¡2 mod3) and a large prime; `r(23)=3Â·167Â·4567Â·27225299` has 167,27225299 inert. **Mechanism:** 1729 is clean only because `H(T_11)=5Â·7Â·11Â·13Â·19` is an unusually smooth product of 5 small primes; `H(T_19),H(T_23)` carry large primes (774463; 27225299), so r(p) can never again be smooth / completely-split / near a j-value. The factorizations are erratic *because smoothness is a small-p artifact.* The genuine regularity of the sequence is ANALYTIC, not arithmetic: `H(T_p)Â·2^{pâˆ’1}/p! â†’ e` (see ratio line below; the real law). Both H(T_19) and H(T_23) were INDEPENDENTLY re-verified here by a validated int64 Held-Karp counter (`04-computation/paley_H23_monad.py`). This severs the cross-lane "1729 spine" (tournament ratio â†” S5 Moser-ladder record rung â†” Klein's 1728): only the Moser-ladder 1729 is structural.

**Ratio H(P(p))/(p!/2^{p-1}):** 2.000, 2.400, 2.440, 2.527, 2.557 for p=3,7,11,19,23 â€” **â†’ e (RESOLVED, HYP-2307, monad-explorer-2026-06-07).** The limit was previously UNSETTLED (e vs larger const vs Alon p^{3/2}); a CHARACTER-SUM CLUSTER EXPANSION settles it: `R(p)=E_Ïƒ[âˆ(1+Ï‡(d_k))]â†’exp(Î£_{Lâ‰¥2}a_L)` where the only surviving single-run cluster integral is the cherry `a_2=âˆ’Ï‡(âˆ’1)=1` (single edges & all odd runs vanish exactly by negation symmetry; `a_4=a_6=0` by Weil square-root cancellation, verified pâ‰¤67). So `R(p)â†’e^1=e` and Alon p^{3/2} is RULED OUT (cluster sum has one finite generator). The constant is literally `e=exp(âˆ’Ï‡(âˆ’1))` â€” it is `e` rather than `e^{âˆ’1}` precisely because Paley needs `pâ‰¡3 mod4` (Ï‡(âˆ’1)=âˆ’1). Convergence is slow (`eâˆ’R~C/p`, Câ‰ˆ4), which is why 5 points couldn't extrapolate it. See `04-computation/paley_cluster_expansion_monad.py` + reflection `why-the-paley-path-ratio-is-e-the-cherry-is-the-unique-cluster.md`. **SUB-LEMMA NOW CLOSED â†’ THM-438 (monad-explorer-2026-06-07, same day):** `a_{2k}=0 âˆ€kâ‰¥2` PROVEN UNIFORMLY (no per-k Weil): `B_L=0` âŸ¹ `A_L=âˆ’Î£`coincidence-patterns; no-leaf forces `Vâ‰¤2k`; only the single `V=2k` pattern `x_0=x_{2k}` (one even cycle) needs Weil, all others `O(p^{2kâˆ’1})=o(p^{2k})` trivially. **SHARPER:** the exact leading order is the CATALAN LAW `A_{2k}=C_k p^{k+1}+O(p^k)` (C_k=1,2,5,14,42,â€¦). **MECHANISM CORRECTED (monad-explorer 3rd session â€” MISTAKE-060 + THM-438 ADDENDUM):** `C_k` is NOT the bigon-tree count â€” bigon-trees over-count (`1,3,13,69,â€¦`=OEIS A088368, `a(n)~eÂ·n!`, the *all-pairings*) and even-cycle CACTI subtract (the *crossings*); `C_k`=SIGNED even-cacti sum (`k=2: +3âˆ’1=2`, `k=3: +13âˆ’8=5`), verified via flow closed-form `M_Ïƒ=(âˆ’1)^k p^{Vâˆ’k}Î£_{flows}âˆÏ‡`. Part C (`Râ†’e`) needs **NO Weil** (V=2k case is `tr(M^{2k})=(âˆ’p)^k(pâˆ’1)`, elementary). The real moment-method content = free-probability Gaussianâ†’semicircle (all-pairingsâ†’non-crossing). Reflections `the-paley-cluster-integrals-are-catalan...md`, `the-catalan-is-a-cancellation-from-gaussian-pairings-to-noncrossing.md`; scripts `paley_cluster_{sharp_order,catalan,cactus_census}_monad.py`. **STILL OPEN (handoff #2):** the sub-leading `C` in `R(p)=e(1âˆ’C/p+â€¦)` â€” rate is now PINNED to **1/p** (error `O(p^k)`, relative `O(1/p)`; resolves the prior âˆšp-vs-1/p ambiguity), so this is `Câ‰ˆ1.4` to pin at pâ‰¥31; (handoff #3) whether the Catalan/even-cacti skeleton survives for non-circulant doubly-regular tournaments (no Gauss spectrum).
...
**Conjecture: Paley tournaments T_p (p â‰¡ 3 mod 4 prime) achieve the maximum number of Hamiltonian paths among all tournaments on p vertices.** This is a major new conjecture. If true, it connects the Hamiltonian-path-maximization problem to number theory via quadratic residues.
...
- **a(12) â‰¥ 531205** (strongly believed exact: multiple distinct tournaments achieve this; all trials converge to 531175 or 531205; no higher value found after hundreds of restarts). Ratio a(12)/a(11) â‰ˆ 5.59.
- **a(13) â‰¥ 3719831** (lower bound; less certain â€” 10-min trials give 3711611..3719831). Ratio a(13)/a(12) â‰ˆ 7.0 if a(12)=531205.
- For prime pâ‰¡3 mod 4: Paley warm start immediately finds global max (verified p=7,11 in solver).
- n=12 optimal tournament is NOT Paley (12â‰¢3 mod 4); found by random restarts.
- Solver: 04-computation/a038375_solver.c. Results: 05-knowledge/results/a038375.out.

**H(T) = I(Î©(T), 2) universal identity (opus-2026-05-27-S6):** Re-verified exhaustively n=2..6 (36,866 tournaments, 0 failures) with CORRECT implementation (distinct directed cycles as Î© vertices, not vertex-set deduplication). See THM-326.
...
- **HYP-2371 PREDICTION:** `R(31) = H(T_31)Â·2^30/31! = 2.59599 Â± 0.00650` âŸ¹ `H(T_31) âˆˆ [19830629617139608462365775, 19930130881568868002912737]` with `H â‰¡ 465 (mod 930)` (freeness LEM-003 + RÃ©dei parity; H/465 odd, â‰ˆ 4.275e22 â€” the "next 1729"). Method: the PROVEN form R = e(1âˆ’C/pâˆ’â€¦) (THM-438 ADD-4) fit with p=23 holdout; the naive truncated cluster sums are PROVABLY non-predictive at finite p (THM-438 ADD-8 resurgence). Compute-run spec: `05-knowledge/results/paley_H31_compute_design_kpc1.md` (see backlog [COMPUTE-NODE] lead).
- **The integrality r(p) âˆˆ â„¤ is now a one-paragraph universal fact:** LEM-003 â€” Aut acts freely on directed Ham paths of ANY digraph; nothing Paley/QR/Eisenstein about it (the QR content is only |Aut| = p(pâˆ’1)/2).
- The 1729 cross-lane ledger is closed: tournament side coincidence (HYP-2306), taxicabâ€“Moser side theorem (THM-463).
...
Verified exhaustively at n=4,5,6,7. The mechanism: anti-automorphism sigma of SC tournament creates orbit pairing structure. **CORRECTION (opus-S18):** NOT all anti-auts are involutions â€” at n=6, two SC classes with |Aut|>1 have order-6 anti-auts (ÏƒÂ² is a non-trivial automorphism). However, every SC tournament has â‰¥1 involution anti-aut (verified n=4,5,6). At even n, involution sigma is fixed-point-free (proved: fixed point implies score = (n-1)/2, non-integer). The sigma-orbits create natural pairings of odd cycles where paired cycles are vertex-disjoint, boosting alpha_2 in the independence polynomial and hence H = I(Omega(T), 2).
...
2. ~~Does the theorem extend to n=8?~~ YES â€” SC achieves global max H=661 at n=8
3. Is every global H-maximizer always SC? (stronger conjecture, verified n<=8 for global max)

**UPDATE (kind-pasteur-2026-03-20-S1 â€” THM-255):**
Complete classification of regular n=6 by IP:
- Type A (SC-BIBD): IP=(1,14,4), H=45, 240 tours â€” max disjoint pairs, fewer cycles
- Type B (SC-rich): IP=(1,20,1), H=45, 240 tours â€” max total cycles, fewer disjoint pairs
- Type C (SC-weak): IP=(1,16,2), H=41, 720 tours â€” intermediate (WORSE than NSC!)
...
## OPEN-Q-017 ðŸŸ¢ â€” PARTIALLY REFUTED
**R-Minimization: H-maximizer minimizes R(T) = sum_v H(T-v) / H(T)?**

Confirmed at n=3,4,5,6 that the H-maximizer minimizes R(T). **FAILS at n=7**: tournaments with H=123 achieve R=1.585 < 5/3 â‰ˆ 1.667 (the H=189 maximizer's R).
...
- **H=21 is a permanent gap â€” PROVED FOR ALL n** (kind-pasteur-S33). Complete proof via poisoning graph DAG argument (Dichotomy Theorem, Part R of THM-079).
...
Exhaustive enumeration of all 2,097,152 tournaments on 7 vertices confirms H=21 never occurs. The gap 19â†’23 is consistent at n=6 and n=7. No (alpha_1, alpha_2) decomposition for H=21 is achievable. Strong evidence this is a permanent gap like H=7.
...
## OPEN-Q-021 ðŸŸ¢ Signed forward-edge polynomial SF(T,x) structure
...
1. Forward path formula: #fwd(2k+1)path = Î£_S H(T[S]) (OCF on subtournaments)
2. Each (2k+1)-cycle contributes 2Â·t_{2k+1} (OCF coefficient 2, unique subset)
3. Multinomial expansion: (2k)! Â· (n-2k) positions Â· 2/P(n,2k+1) = 2/C(n,2k)
...
## OPEN-Q-024 ðŸŸ¢ Even Betti Vanishing for Tournament Path Homology
**Prove: beta_{2k}(T) = 0 for all k >= 1, for any tournament T.**

**UPDATE (kind-pasteur-S43): beta_2 = 0 PROVED (THM-108 + THM-109).**
**UPDATE (opus-2026-04-04-S1): Proof is FULLY ALGEBRAIC â€” THM-285 closes n=5 gap.**

Proof via strong induction using LES of (T, T\v):
- ~~Base case n=5 verified exhaustively (720/720)~~ **THM-285: n=5 case is VACUOUS** (no n=5 tournament has both bâ‚=0 and Îºâ‰¥2; proof: Îºâ‰¥2 forces dominatorâ†’source/sink contradiction)
- Induction step: good-vertex existence (THM-109)
- Case 2 (free cycles exist): Lemma A (free adj dom) + Lemma B â†’ n-5 good vertices for nâ‰¥6
...
**UPDATE (opus-S72b): Î²â‚ âˆˆ {0,1} verified exhaustive nâ‰¤8, sampled n=9 (THM-223).**
Key discovery: Î²â‚ is determined ENTIRELY by rank of transitive triple constraint matrix.
Cancellation chains are ALWAYS redundant. Combined with THM-095 seesaw: Î²â‚Â·Î²â‚ƒ=0 follows.
Algebraic proof of Î²â‚ â‰¤ 1 still open (reformulated as transitive triple rank bound).

REMAINING OPEN:
- **What bound replaces beta_3 <= 1 at n >= 8?** (beta_3=2 confirmed at n=8,9)
- **Prove beta_1 â‰¤ 1 algebraically** â€” equiv. to rank(TT) â‰¥ C(n,2)-n (THM-223)
...
## OPEN-Q-026 ðŸŸ¢ Does the interval maximize H for all circulant tournaments on Z_p, p >= 13?
...
| 11 | 95,095 | 93,027 | -2.2% | PALEY | Paley+complement ONLY (2nd: H=93,467Ã—10) |
| 13 | - | 3,711,175 | - | INTERVAL | (exhaustive, pâ‰¡1 mod 4, no Paley) |
...
EXHAUSTIVE SCANS (kind-pasteur-2026-04-16):
  ...
  n=7,11 alpha breakdown:
    n=7:  Paley Î±â‚=80, Î±â‚‚=7.  Cyclic Î±â‚=59, Î±â‚‚=14.  (Cyclic has 2Ã— Î±â‚‚!)
    n=11: Paley Î±â‚=21,169, Î±â‚‚=10,879, Î±â‚ƒ=1,155. Cyclic Î±â‚=18,397, Î±â‚‚=11,110, Î±â‚ƒ=1,474.
          Cyclic has MORE Î±â‚‚ and Î±â‚ƒ, but Paley's Î±â‚ advantage (5,544 in H) > Cyclic's advantage (3,476).

Crossover: Paley wins at p=7 and p=11 due to Î±â‚ dominance. Interval wins at pâ‰¥13.
The Î±â‚ percentage gap narrows: 35.6% (n=7), 15.1% (n=11), 3.6% (n=19). Paley's Î±â‚ lead evaporates.
At n=7: kmax=2 (no 3-packings possible), so H has only Î±â‚,Î±â‚‚ terms â€” Paley Î±â‚ wins.
At n=11: Paley Î±â‚ advantage still > Cyclic Î±â‚‚+Î±â‚ƒ advantage.
At n=19: Cyclic Î±â‚ƒ+ advantage (26.7B) > Paley Î±â‚+Î±â‚‚ advantage (15.2B) â†’ Cyclic wins.
Î±â‚‚ comparison crossover: Cyclic > Paley at n=7,11 (small n, disjoint packing easier);
                          Paley > Cyclic at n=19 (large n, more Î±â‚ â†’ more Î±â‚‚ pairs).

WHY interval wins for large p (kind-pasteur-2026-04-16):
  Paley has MORE Î±â‚ and Î±â‚‚ at n=19, but Interval wins by +11.5B total.
  Interval has +26.7B from Î±â‚ƒ+ terms: its cycles pack into disjoint triples better.
...
EXACT Î±-DECOMPOSITION COMPARISON at n=19 (kind-pasteur-2026-04-16, VERIFIED):
  k | Paley Î±_k          | Cyclic Î±_k         | Cyclic advantage | H contribution
  1 | 130,965,270,477    | 126,443,605,257    |   -4,521,665,220 | 2Ã—diff = -9.04B  (Paley wins)
  2 | 123,659,531,220    | 122,111,579,294    |   -1,547,951,926 | 4Ã—diff = -6.19B  (Paley wins)
  3 |  41,184,418,943    |  42,960,731,622    |   +1,776,312,679 | 8Ã—diff = +14.21B (Cyclic wins)
  4 |   4,903,920,444    |   5,521,030,944    |     +617,110,500 |16Ã—diff = +9.87B  (Cyclic wins)
  5 |     251,464,164    |     331,078,344    |      +79,614,180 |32Ã—diff = +2.55B  (Cyclic wins)
  6 |       2,221,081    |       4,100,656    |       +1,879,575 |64Ã—diff = +0.12B  (Cyclic wins)
  Net: Paley advantage 15.2B (via Î±â‚,Î±â‚‚) vs Cyclic advantage 26.7B (via Î±â‚ƒ+) = +11.5B net for Cyclic.

  Î±â‚ƒ/Î±â‚‚ ratios: Paley=0.333, Cyclic=0.352 â†’ Cyclic is intrinsically better at 3-packing!
  The k=5,6 percent advantage for Cyclic: +31.7% at k=5, +84.7% at k=6 â€” grows with k.
  Source: paley_t19_alpha.out (H(Paley)=1,172,695,746,915 verified âœ“)
...
The original formula E_{2k}/E_0 = 2*(n-2k)^k / P(n,2k) is the LEADING-TERM APPROXIMATION only, exact for k â‰¤ 2 but requiring corrections for k â‰¥ 3.

The EXACT formula uses combinatorial g_k polynomials of degree k:
- CVÂ²(H) = Î£_{kâ‰¥1} 2Â·g_k(n-2k) / (n)_{2k}
- g_k defined via transfer matrix M(x) = [[1,2x,0],[0,0,1],[1,x,0]]
- Weight formula E[âˆZ_j] = 2^c/(n)_L PROVED (c=components, L=|S|)
- g_k(m) ~ 2^{k-1}Â·m^k/k! + lower terms (leading term is original formula)
...
## OPEN-Q-028 ðŸŸ¢ Are there forbidden H values beyond 7 and 21?
...
131 = Tr(M^8) EXACTLY, where M is the 3Ã—3 transfer matrix from S112. Ï„â‚ƒ^8 â‰ˆ 130.977 and the Pisot correction 2|Î»_c|^8 cos(8Î¸) â‰ˆ +0.023 pushes the sum to exactly 131.

**Why the correction is so small:** arg(Î»_c)/Ï€ â‰ˆ ln(2), so 8Â·arg/Ï€ â‰ˆ 8Â·ln(2) â‰ˆ 5.545 â‰ˆ 5.5, making cos(8Â·arg) â‰ˆ cos(5.5Ï€) â‰ˆ 0.13 (small). The n=8 case is special because 8Â·ln(2) is close to the half-integer 11/2.
...
- The Ï„-Ï† clock gear ratio arg(Î»_c)/Ï€ â‰ˆ ln(2) explains ALL Pisot near-integers
- Tr(M^n) mod 8 has EXACT period 8 (Bott periodicity connection)

**Source:** opus-2026-03-15-S90c (monad), S90h (Ï„-Ï† clock), S90l (the number 8)

---

## OPEN-Q-030 -- RESOLVED (PROVED for all n â‰¥ 4)
**Prove Simplicial RÃ©dei for ALL n (THM-220)**

**RESOLVED by opus-2026-03-15-S90 (THM-220):**

The Key Lemma IS proved algebraically for all n: Given aâ†’b not in any transitive triple, the four possible orientations of {a,b,c} are: (1) aâ†’c,bâ†’c: transitive a>b>c, contains aâ†’b â€” CONTRADICTION. (2) aâ†’c,câ†’b: transitive a>c>b, contains aâ†’b â€” CONTRADICTION. (3) câ†’a,bâ†’c: 3-cycle aâ†’bâ†’câ†’a â€” the ONLY non-core possibility. (4) câ†’a,câ†’b: transitive c>a>b, contains aâ†’b â€” CONTRADICTION. Since 3 of 4 orientations force aâ†’b into a transitive triple, the only possibility for ALL c is case (3): every c forms a 3-cycle with {a,b}. This gives score(a)=1, score(b)=n-2.

The Main Argument (at most one non-core edge) then follows by contradiction in 4 cases of vertex overlap. Case 3 (b=c) requires nâ‰¥4 so that V\{a,b,d}â‰ âˆ….
...
## OPEN-Q-031 ðŸŸ¢ Is arg(Î»_c)/Ï€ = ln(2) exact or approximate?

**Statement:** The tribonacci complex eigenvalue angle satisfies arg(Î»_c)/Ï€ â‰ˆ ln(2) to 4 significant figures (difference 4.3Ã—10â»â´). Is this exact?

**Evidence:** NOT exact (verified: the predicted root from arg=Ï€Â·ln(2) does not satisfy the char poly). But the proximity is remarkable and explained by the information-theoretic interpretation: the tribonacci clock ticks at approximately 1 bit per half-revolution.

**Source:** opus-2026-03-15-S90h (Ï„-Ï† clock)

---

## OPEN-Q-032 -- PARTIALLY RESOLVED (FAILS at n=6)
**Tournament equidecomposability: is (H, Î²â‚) a complete invariant?**

**ANSWER: NO.** (H, Î²â‚) is complete at n=5 (8 classes, each with unique I(Î©â‚ƒ, x)) but FAILS at n=6.

**Counterexamples at n=6 (5 found):**
- (H=9, Î²â‚=0): TWO distinct I(Î©â‚ƒ): (1,2,1) and (1,3,0,0)
- (H=15, Î²â‚=0): TWO distinct: (1,4,0,0,0) and (1,5,0,0,0,0)
- (H=29, Î²â‚=0): TWO distinct: (1,6,1) and (1,6,2)
- (H=33, Î²â‚=0): TWO distinct: (1,6,2) and (1,7,1)
- (H=37, Î²â‚=0): TWO distinct: (1,7,1) and (1,7,2)

ALL counterexamples have Î²â‚=0 â€” the Î²â‚=1 classes remain unique!
This means: Î²â‚ provides a COARSER invariant. The FULL independence polynomial I(Î©â‚ƒ, x) requires more information (Î±â‚‚ distinguishes within Î²â‚=0).

**REVISED CONJECTURE:** (H, Î²â‚, Î±â‚‚) may be complete. Or (H, full Î±-vector) is complete by definition.
...
**The n-tribonacci family: T_n - M_{n-2} = 1/(kM_k+2) + O(1/kâ´)**

**PROVED by opus-2026-03-16 (perturbation analysis):**

Write T_n = M_k + Îµ where k = n-2. Substituting into Î»Â³ = kÎ»Â² + Î» + 1 and using MÂ² = kM+1:

  (kM+2)Â·Îµ = 1  at leading order.
  So Îµ = 1/(kM_k+2).

Since M_k ~ k for large k: Îµ ~ 1/(kÂ²+2) ~ 1/kÂ².

Verified numerically: the ratio Î´_actual / (1/(kM+2)) â†’ 1 as n â†’ âˆž (0.999599 at n=19).

At n=3: Î´ = 0.221 (maximum), predicted 0.276 (ratio 0.80 â€” leading order less accurate for small k).
...
- **OPEN-Q-009**: Arc-flip invariance resolved â€” E(T) = 0 for all T (OCF proved). See THM-002. (kind-pasteur-2026-03-05-S12)
...
At n=6: mean H = 29 splits as 5Â²+2Â² (golden) and 1Â²+7Â·2Â² (forbidden). At n=7: mean H = ? At n=8: mean H = ? Do the two world-defining primes always appear in the splitting?
...
## OPEN-Q-039 ðŸ”´ â€” SUBSTANTIALLY RESOLVED (sessions S212-S249)
...
1. âœ… Extended to n=6,7,8,9 (6880 classes at n=8, 191536 at n=9)
2. âœ… Diameter = n-2 DISPROVED at n=7 (diam=7â‰ 5). Actual: 1,2,3,4,7,8
3. âœ… H-DAG property REFUTED: G_n is NOT a strict DAG. Level edges appear at nâ‰¥5 (1, 15, 136 for n=5,6,7). H-decreasing edges appear at n=7 (962/4086). The H-gradient is strong but imperfect. See MISTAKE-035.
4. âœ… Spectral data computed at n=3..7 (Ramanujan fails at n=6)
5. âœ… |Aut|-degree connection: corrâ†’0 at large n (classes become generic)
6. âœ… I(G_n,2) computed: 5, 13, 793, 15B (super-exponential "meta-H")
7. âœ… Staircase connection: Mode A/B recursion, y=x diagonal, within-type fractionâ†’3/4

**EDGE FORMULA (the keystone):**
  edge_orbits = T_n/2 + (n-2)! [verified n=3..6, Burnside-computable]
  E(G_n) = edge_orbits - gap_orbits [exact]
  E(G_n) â‰ˆ (T_n - twin_SL)/2 - D(n-2) [99.6% accurate, all Burnside]
  E(G_n) â‰ˆ T_n/2 for n â‰¥ 12 [asymptotic]
...
**STRUCTURAL LAWS (19+ verified):** DAG, BBK impossibility, rib crossover, spine ~4-regular, ribs linear in n, sea dominates, Î”H=2^(n-2), cell uniformity, lattice oscillation, etc.

**REMAINING:** Exact formula for gap_orbits (= 2,5,20,86,490,3703,47889); twin_SL residual; chi=n-1 conjecture proof (greedy fails at nâ‰¥6).
...
ðŸŸ¡ **The Krawtchouk coordinate system for tournament space**

**RESOLVED sub-problems:**
1. âœ… **Tournament Counting Theorem** (S291): V_nÃ—n!/2^m = 1 + Î£(1/k)Ã—nâ†“kÃ—2^{(kÂ²-1)/2-(k-1)n}. Euler product with poles at 4,16,64,... controlled by 1/3. Dâ‚ƒ(0) = 128/81.
2. âœ… **Band-limitedness** (S305,S310,S311, **CORRECTED kind-pasteur-S1 2026-03-25**): Walsh degree = 2*floor((n-1)/2) for all n>=4 (THM-260). Band-limited at m/2 for n>=6. **CORRECTION:** n=5 is NOT band-limited at m/2 (degree 4 > m/2=3). Odd-weight Walsh coefficients are nonzero in tiling model (complement symmetry fails).
3. âœ… **Krawtchouk 3-axis system** (S307): Bâ‚â‰ˆ-H (r=-0.94), Bâ‚‚â‰ˆ-câ‚ƒ (r=-0.86), Bâ‚ƒ=twist. SC classes have B_odd=0 exactly (Krawtchouk parity).
4. âœ… **Diameter = A003141** (S306): max feedback arc set. Growth ~nÂ²/4, not n-2 (small-n coincidence).
5. âœ… **Paleyâ†’Dual Codes** (S306,S308): Pâ‚‡+Iâ†’Hamming[7,4,3], Pâ‚‚â‚ƒ+Iâ†’Golay[23,12,7].
6. âœ… **Not an association scheme** (S306): full algebra dim=35 vs needed 7 at n=5.
7. âœ… **Spectral gap = 2/n explained** (S312): comes from Kâ‚ spacing 2/m compressed by S_n quotient (factor m/n).
8. âœ… **Waggly = all connections** (S296-S301): wigglyâŠ‚waggly, blue/blackâŠ‚waggly. Completeness at k*=diam.
9. âœ… **Waggly alphabet** (S302-S304): range-3 harmonic most neutral. Vertex-count law. All-same-range combos special.
10. âœ… **Practical tools** (S308-S309): pre-filter eliminates 98% of canon calls. tournament_tools.py library. tournament_codec.py (kind-pasteur).

**OPEN sub-problems (the 10 boundary questions from S307):**
1. âœ… B1: **RESOLVED** (THM-260, kind-pasteur-S1): Walsh degree = 2*floor((n-1)/2) for all n. Band-limited at m/2 for n>=6. Proof: THM-076 upper bound + interleaving construction lower bound.
2. ðŸŸ¢ B2: Exact constant in A003141 n^{3/2} correction
3. ðŸŸ¢ B3: Is transitive always a diameter endpoint?
4. ðŸŸ¢ B4: Does Kâ‚-H correlation â†’ 0 or stabilize? (0.94â†’0.89â†’0.83)
5. ðŸŸ¡ B5: Exact neutrality formula SL(d,n) as function of distance
6. ðŸŸ¢ B6: Width W(H) asymptotic distribution
7. ðŸŸ¢ B7: Is there ANY partition giving an association scheme?
8. ðŸŸ¢ B8: Is range âŒŠ(n-1)/2âŒ‹ always most neutral?
9. ðŸ”´ B9: Î²â‚‚=0 for all tournaments (proof strategy via band-limitedness, S312)
10. ðŸŸ¡ B10: min-FAS(T) in terms of OCF invariants
...
## OPEN-Q-044 ðŸŸ¢ Alpha Mechanism Shift: When Does Each Î±_k Dominate?

**Discovery (kind-pasteur-2026-04-16):** The dominant term in H = I(Î©,2) = Î£ 2^k Â· Î±_k shifts with n.
H-maximizing cyclic interval tournament C_n:

| n | dom term | 2^1Â·Î±â‚ | 2^2Â·Î±â‚‚ | 2^3Â·Î±â‚ƒ | notes |
|---|----------|---------|---------|---------|-------|
| 3-9  | Î±â‚ | largest | 2nd | small | Î±â‚/(2Î±â‚‚) > 1 |
| 11-17 | Î±â‚‚ | 2nd | largest | 3rd | FIRST CROSSOVER nâ‰ˆ10 |
| 19+ | Î±â‚‚ | 3rd | largest | 2nd | SECOND CROSSOVER: Î±â‚ƒ overtakes Î±â‚ at nâ‰ˆ17-19 |

**Complete verified table for C_n (cyclic interval tournament):**

| n  | Î±â‚ | Î±â‚‚ | Î±â‚ƒ | Î±â‚/(2Î±â‚‚) | Î±â‚ƒ/Î±â‚‚ | H | H(n)/H(n-2) |
|----|----|----|----|-----------|---------|----|-------------|
| 17 | 1,651,334,601 | 1,482,234,998 | 458,011,858 | 0.5570 | 0.3090 | 13,689,269,499 | â€” |
...
**Full Î±-decomposition n=21:**
  Î±â‚=12,030,499,746,751   Î±â‚‚=12,330,182,836,208   Î±â‚ƒ=4,796,354,751,404
  Î±â‚„=738,531,326,288      Î±â‚…=58,868,297,768        Î±â‚†=1,454,221,328       Î±â‚‡=12,571,712
  H = 125,547,534,942,879

**Full Î±-decomposition n=23 (NEW, kind-pasteur-2026-04-16-S1):**
  Î±â‚=1,391,602,826,199,187   Î±â‚‚=1,499,656,616,321,278   Î±â‚ƒ=632,921,002,322,216
  Î±â‚„=111,796,734,828,336     Î±â‚…=10,945,293,151,712       Î±â‚†=412,282,843,184       Î±â‚‡=7,454,017,376
  H = 16,011,537,490,557,279 âœ“

**Term ordering at n=23:** 4Î±â‚‚ > 8Î±â‚ƒ > 2Î±â‚ > 16Î±â‚„ > 32Î±â‚… > 64Î±â‚† > 128Î±â‚‡
  (5.999P > 5.063P > 2.783P > 1.789P > 0.350P > 26.4T > 0.95T)

**Special structure at n=21:** Î±â‚‡ = 12,571,712 = perfect 7-triangle-packings.
  Only packing type is (3,3,3,3,3,3,3) since 7Ã—3=21. Perfect vertex coverage.
**Structure at n=23:** Î±â‚‡ = 7,454,017,376 counts 7-packings with cycle-length sum âˆˆ {21, 23}.
  Sum must be odd (7 odd numbers), and â‰¤23. So: sum=21 (all 3-cycles, 2 vertices free) OR
  sum=23 (one 5-cycle + six 3-cycles, all 23 vertices covered). Sum=22 impossible (even).

**H growth ratio H(n+2)/H(n):** 86.5, 106.0, 127.6 â†’ increments +19.5, +21.6 â†’ growing.
  Predicted H(25) â‰ˆ H(23) Ã— 150 â‰ˆ 2.4 Ã— 10^18.

**Key ratio Î±â‚ƒ/Î±â‚‚ progression:**
  n=17: 0.3090, n=19: 0.3518 (+0.043), n=21: 0.3890 (+0.037), n=23: 0.4220 (+0.033)
  First differences decreasing by ~0.004/step. Projected:
  n=25: â‰ˆ0.451 (+0.029), n=27: â‰ˆ0.476 (+0.025), n=29: â‰ˆ0.497 (+0.021), n=31: â‰ˆ0.514 â†’ THIRD CROSSOVER
  **Revised estimate: third crossover (8Î±â‚ƒ > 4Î±â‚‚) at nâ‰ˆ31**, not nâ‰ˆ27-29 as previously estimated.
...
**Open:** Third crossover: Î±â‚ƒ dominates at nâ‰ˆ31 (needs n=25,27 data to confirm).
         C implementation of cycle_cc needed for nâ‰¥25.

---

## OPEN-Q-046 ðŸŸ¡ The SC Blowup: $\Omega(T_{\mathrm{SC}})$ and H Formula
...
**Open (ðŸŸ¡):** What is $\Omega(T_{\mathrm{SC}})$ in terms of $\Omega(T)$? Is there a formula
...
**Key data:** At $n=5$, $H_{\mathrm{SC}}$ varies only 4.2% across all 12 iso classes ($14937$â€“$15565$).
...
## OPEN-Q-045 ðŸŸ¢ H Under Tournament Blowup (Column Row Step)
...
$H(\text{blowup of odd-}n\, T)$ vs $H(\text{blowup of even-}n\, T)$ â€” are
these qualitatively different?

**Q4:** Does SCâˆ©SF = SC($n-2$) for the family:
...
- **Q1 ANSWERED (negative + repaired):** H(T[Kâ‚‚]) is NOT a function of (H(T), n) â€” not even of
  I(Î©(T),x) (n=5 counterexample: equal typed IP, H(Kâ‚‚) 3225 vs 2785; the missing data is EVEN
  cycles, which twin-insertion converts to odd). What IS true: **strong-component product law**
  H(T[Kâ‚‚]) = âˆ_C H(C[Kâ‚‚]) (PROVED); twin-lift laws c3'=8c3, c5'=32c5+32c4+6c3 (+c7' law);
  cycle-spectrum (c3..c6) determines H(T[Kâ‚‚]) at nâ‰¤6 (n=7 separation test open, HYP-2353);
  congruence H(T[Kâ‚‚]) â‰¡ 2H(T)âˆ’1 (mod 8).
- **Q2 partial:** T[Kâ‚‚] is op-equivariant (PROVED via orbit symmetries, THM-450(6)).
- T[Kâ‚‚] is one of exactly THREE 2Ã—2-block doublings (THM-450 trichotomy); the skew-Sylvester
  member D (THM-447) is the spectral/Hadamard-clean one; SCblow is the H-maximizing one.

---

## OPEN-Q-047 ðŸŸ¡ Characterize Real-Rootedness of I(Î©(T), x)

**Correction (opus-2026-05-29-S8):** The universal TRRT statement is already refuted by THM-025 at n=9.
The surviving problem is to characterize the tournaments for which I(Î©(T), x) has all real, negative roots.

**What's proved:** For n â‰¤ 8, Î©(T) is claw-free (a claw requires â‰¥ 9 vertices), so real-rootedness follows from Chudnovsky-Seymour (2007).

**Counterexample:** THM-025 gives an n=9 tournament with score sequence [1,1,3,4,4,4,6,6,7] and
I(Î©,x)=1+94x+10xÂ²+xÂ³. Newton k=2 fails (100 < 141), so two roots are non-real.

**Why notable:**
- Generic/sample tournaments often remain real-rooted despite the n=9 failure.
- For the real-rooted subclass, ultra-log-concavity and product formula H(T)=âˆ_i(1+2r_i) remain powerful.
- The THM-025 counterexample may isolate the exact obstruction shape.

**Sub-conjecture status:** Î©(T) is NOT always perfect (see INV-032 / THM-019 updates), so perfectness is also a subclass question.

**Key open questions:**
1. What structural property of Î©(T) (beyond claw-free) forces real-rootedness?
...
**Priority:** ðŸŸ¡ IMPORTANT. A structural characterization would be publishable as a standalone result.
...
- Forbidden (Î±â‚=3, Î±â‚‚=0) confirmed absent at n=6..9 in all samples.
...
- (H, I(Î©,6)) separates only 7/47 n=6 classes by (H,I6) alone (much coarser than hoped).
...
## OPEN-Q-048 ðŸŸ¢ Ultra-Log-Concavity for Tournament Independence Polynomials
...
**ErdÅ‘s context:** This is the tournament analog of the Heron-Rota-Welsh theorem (ULC for matroid Whitney numbers, proved by Adiprasito-Huh-Katz). Both prove ULC via underlying geometry (real-rootedness for tournaments, Hodge theory for matroids).

**Status:** PROVED conditional on real-rootedness. Computationally verified n=6..9.
**Priority:** ðŸŸ¢ Interesting. Connects to the Huh-Katz matroid theory.
**Source:** oracle-2026-05-17-S1, computation `root_spectrum_fast.py`.

**NEW (oracle-2026-05-19-S1): UNCONDITIONAL proof of ULC at k=1 via TurÃ¡n's theorem.**
For any tournament T: since bar_Omega(T) is K_{d+1}-free (max clique size = d = degree of I),
TurÃ¡n's theorem gives alpha_2 <= (1-1/d)*alpha_1^2/2, which is exactly ULC at k=1:
   alpha_1^2 >= 2d/(d-1)*alpha_2.
No TRRT required. Equality iff I(Omega,x) = c*(x+rho)^d (all roots equal, TurÃ¡n extremal).
...
## OPEN-Q-050 ðŸŸ¡ Unconditional ULC at k=2 via Kruskal-Katona
...
**Priority:** ðŸŸ¡ Important. Would give the first unconditional ULC result beyond k=1.

---

## OPEN-Q-051 ðŸŸ¡ Interlacing Approach to Real-Rooted Subclasses
...
**Why it's hard:** The proof needs to show B interlaces A for the specific structure of tournament conflict graphs. This is analogous to the Chudnovsky-Seymour claw-free proof but for non-claw-free graphs (nâ‰¥9).

**Connection:** For any subclass where Î©(T)'s independence complex is matroid/gammoid-like, Choe-Oxley-Sokal-Wagner stability may imply real-rootedness.

**Priority:** ðŸŸ¡ IMPORTANT. Could characterize the real-rooted subclass or identify the THM-025 failure in the HB framework.
...
- No-HB-cycle cases: exactly d=2,alpha2=1 â€” proved real-rooted by TurÃ¡n unconditionally.
- In the tested real-rooted regime, the HB route reduces to TWO lemmas: (A) existence of HB-cycle and (B) interlacing.
- Proof sketch for subclasses: induction on m, using TurÃ¡n for base cases and HB for induction.
See `07-reflections/hermite-biehler-trrt-strategy.md`.

---

## OPEN-Q-052 ðŸŸ¡ Lemma A: Existence of HB-satisfying Cycle

For any tournament T with dâ‰¥2 and Î±â‚‚â‰¥2 (or dâ‰¥3), prove that there exists a cycle C* such that deg(I(Omega\\C*)) = deg(I(Omega-N[C*])) + 1.

Computationally: holds for ALL tested n=6,7 cases (except d=2,alpha2=1 which is handled by TurÃ¡n).
...
**Priority:** ðŸŸ¡ IMPORTANT (one of two lemmas for the HB real-rootedness subclass program; universal TRRT is refuted by THM-025).

---

## OPEN-Q-053 ðŸŸ¡ Lemma B: B Interlaces A in Hermite-Biehler Recursion
...
**Priority:** ðŸŸ¡ IMPORTANT (other lemma for the HB real-rootedness subclass program; together with Lemma A it cannot imply universal TRRT because THM-025 refutes that statement).
...
## OPEN-Q-049 ðŸŸ¢ Root Ratio as SC Detector

**Conjecture:** SC tournaments have the most asymmetric root ratio $\rho_2/\rho_1$ (minimum ratio) at each $n$.

**Evidence:** At n=6: SC min ratio = 0.00251 (H=45, Î±â‚=20, Î±â‚‚=1) < NS min = 0.00279 (H=43, Î±â‚=19, Î±â‚‚=1).
...
## OPEN-Q-053 ðŸ”´ Prove HYP-1732: alpha2(Omega(T)) <= p*(m-p) for pair-partner C*
...
- âŸº B interlaces A in the Hermite-Biehler decomposition (Lemma B for d=2)
- âŸº I(Omega, -1/p) <= 0 (via the identity A(-1/p)=I(-1/p), THM-313)
- âŸº p lies between the two positive roots of I(Omega(T),x)

**Verified:** 1637 tests at n=7..11, 0 violations (opus-S2). **Strengthened (monad-compute-2026-06-06-S1):** 132,604,306 pair-partner tests over 291,788 distinct Î±(Î©)=2 tournaments at **n=7,8,9** (uniform random), 0 violations; both equivalent forms (combinatorial bound and quadratic I(Î©,âˆ’1/p)â‰¤0) agree per test. **Min slack (boundâˆ’Î±â‚‚)=0 âŸ¹ the bound is SHARP.** Caveat: the S1 run's n=8 layer ate the full time budget, so nâ‰¥10 was budget-skipped, not tested â€” uniform random at nâ‰¥10 almost never gives Î±=2, so nâ‰¥10 needs targeted low-cycle construction (prior n=10,11 coverage stands). Script `hyp1732_large_sample_monad_s1.py`.
...
**Note:** TRRT for d=2 follows from TurÃ¡n-ULC WITHOUT this lemma. HYP-1732 would give an ADDITIONAL structural proof via HB induction.

## OPEN-Q-054 ðŸŸ¡ Lemma A for the UNIQUE max IS case (d>=3)

**Added:** opus-2026-05-22-S2

**Status:** THM-314 proves Lemma A for ALL non-unique max IS cases (all d>=2). Remaining gap: unique max IS at d>=3.

**Situation:** When S is the unique max IS of size d>=3: every C*âˆ‰S has d_A=d and d_B<=d-1 (Key Inequality). Whether d_B=d-1 depends on T[V\V(C*)] having enough disjoint cycles. Empirically: 0 failures at n=9..11.

**Proof approach:** Show that for SOME C*âˆ‰S, the sub-tournament T[V\V(C*)] supports an IS of size d-1 in Omega restricted to cycles disjoint from C*. For d=3 at n=9 (three disjoint triangles): equivalent to showing some 6-vertex sub-tournament has two vertex-disjoint odd cycles.


---

## OPEN-Q-055 ðŸŸ¡ Forbidden H-spectrum: Other universally forbidden H values beyond 7

**Added:** opus-2026-05-28-S5 (with THM-343 completion).

**Status:** THM-343 proves H(T) â‰  7 for ALL tournaments. **H=21 â€” finite window now CLOSED (monad-compute-2026-06-04-S4, HYP-2200):** the HYP-2193 reduction (H=21 âŸ¹ a strong component with H=21 âŸ¹ câ‚ƒâ‰¤Î±â‚â‰¤10 âŸ¹ by Moon mâ‰¤12; THM-079 Part G killed mâ‰¤8) left only strong tournaments on mâˆˆ{9,10,11,12} with câ‚ƒâ‰¤10; these were **exhaustively enumerated (isomorph-free) and contain NO H=21** (min H = 75,125,225,375). So H(T)â‰ 21 for all tournaments â€” {7,21} is the complete permanent H-gap set, modulo elevating the canon inputs to a formal THM-115 proof. (Even cleaner: the Busch lower bound p(7)=25>21, MISTAKE-053, gives Hâ‰¥25 for every strong tournament on mâ‰¥7 directly.) H=63 is REFUTED as a universal gap: it is achieved at n=8.

**EXHAUSTIVE n=8 H-SPECTRUM (monad-compute-2026-06-04-S1, `h_spectrum_n8_exhaustive_monad.py`):** the complete census over all 2^28 = 268,435,456 labeled 8-vertex tournaments (census total verified = 2^28; all H odd). 320 distinct H values, range [1, 661]. **The only low odd gaps are {7, 21}** â€” every odd value in [23, 609] is achieved. H=35,39,49,63 all unlock at n=8 (counts 161280/188160/604800/80640). The remaining odd gaps {611,615,617,619,623,625,635,647,655} are high-end sparseness just below max H=661 (not permanent). This makes the n=8 forbidden set âˆ©[1,609] = {7,21} EXACT (previously only 100k sampling, HYP-1104), and exhaustively confirms Hâ‰ 7, Hâ‰ 21 at n=8 (upgrades the H=21 (8,1)/(6,2) cases from "strong nâ‰¥8 sampling" to exhaustion).

**H-UNLOCK TABLE n=3..9 â€” answers the "at what n does each value unlock?" sub-question (monad-compute-2026-06-04-S7, `h_unlock_table_monad_s7.py`):** for every odd H, `unlock(H)` = smallest n in {3..9} with some tournament achieving it, built from the EXHAUSTIVE per-level spectra (n=3..7 generated here, iso-class counts re-checked against A000568=2,4,12,56,456; n=8 from S1 census; n=9 from S6 iso-classes). **Unlock cascade** (distinct H / maxH / NEW-at-n): n=3 (2/3), n=4 (3/5, +1), n=5 (7/15, +4), n=6 (19/45, +12), n=7 (77/189, +58), n=8 (320/661, +243), n=9 (1520/3357, +1200). **27 transient gaps unlock** with explicit n: H=35,39 at **n=7**; H=63,107,119,149 and 161..187 (odd) at **n=8**; the nine n=8 high gaps {611,615,617,619,623,625,635,647,655} at **n=9**. *Precision fix to the S1 entry:* H=35,39,49 first appear at **n=7** (not n=8 â€” the S1 "unlock at n=8" referred to their n=8 census counts); only H=63 truly first unlocks at n=8. **Permanent-through-n=9 gaps**: 159 odd values â‰¤ maxH=3357, of which **LOW (â‰¤609) = exactly {7,21}**; the other 157 are high-end sparseness â‰¥2883 just below the new max. Sampled n=10/n=11 cross-check: H=7,21 absent in both (consistent with permanent); 9/157 of the n=9 high gaps are already seen achieved by nâ‰¤11 sampling (transient sparseness, not permanent). Table saved at `05-knowledge/results/h_unlock_table_monad_s7.tsv` (one row per odd H â‰¤ 3357; blank = not achieved through n=9). No new HYP/THM minted (MISTAKE-053 discipline).

**ALL 157 n=9 HIGH GAPS UNLOCK AT n=10 (monad-compute-2026-06-04-S9, `h_high_gap_unlock_sampling_monad_s9.py`):** the 157 "permanent-through-n=9" HIGH gaps in [2883, 3355] (everything beyond {7,21}) were attacked with heavy bias-swept near-transitive sampling at n=10/11/12 (Held-Karp `H_count`; transitive base with each forward arc reversed w.p. p, p-grid calibrated so the achieved-H cloud sweeps the target window). **Result: all 157/157 are ACHIEVED at n=10** â€” every one is TRANSIENT, not permanent. The n=10 phase (167,600 samples, 9,365 in-window) hit all 157 by t=125s (~33k samples); a partial n=11 phase (20,800 samples) re-confirmed 157/157. H=7 and H=21 never appeared in any sample (consistent with permanent). This upgrades S7's "9/157 lower-bounded" to **157/157 transient**, so the n=9 high-end sparseness is a pure finite-level artifact and **{7,21} stand alone as the sole candidate permanent low gaps** (proved forbidden by THM-343/THM-079; {7,21} is the complete permanent H-gap set). Per-target table: `05-knowledge/results/h_high_gap_unlock_sampling_monad_s9.tsv` (all first_n_achieved=10). Sampling certifies achievability (concrete witnesses), never permanence. No new HYP/THM minted (MISTAKE-053 discipline).
...
**Evidence:**
- H=21: 0 occurrences at nâ‰¤7 (exhaustive as of S6). All four decompositions (10,0), (8,1,0), (6,2,0), (4,3,0) of Î±-vectors absent at n=6.
- H=63: absent at nâ‰¤7, but **achievable at n=8**. THM-344 (opus-S10) gives the exact n=8 census: exactly two n=8 isomorphism classes have H=63; both have |Aut|=1, score sequences (1,2,2,3,3,5,6,6) and (1,1,2,4,4,5,5,6), and Î©(T)=K31, hence H=I(K31,2)=63.
- S11 structural fingerprint: both H=63 classes are single-core. Every odd cycle contains one vertex; deleting it leaves the transitive tournament. The core signatures `1001100` and `1100110` have weighted count r=31. Complete-Î© class census n=3..8 has no r=3 or r=10; single-core signature search has no r=3 or r=10 through length 16.
- S12 projection-defect update: both H=63 classes are exact old-projection kills (delete the core vertex and Î© vanishes). A core-stratified complete-Î© census through n=8 still has no r=3 or r=10 in any core stratum, and the single-core target search now has no r=3 or r=10 through length 40.
- **SINGLE-CORE SIGNATURE GAP â€” RESOLVED (monad-compute-2026-06-04-S2, `single_core_signature_complete_monad_s2.py`):** the single-core odd-cycle count is `r(s)=Î£_{i<j, s_i=1, s_j=0} f(j-i-1)`, `f(0)=1, f(t)=2^{t-1}`, over bit strings `s` (core arc pattern relative to a transitive order). Stripping leading 0s / trailing 1s is `r`-invariant, and a canonical witness of length `Lâ‰¥3` has `râ‰¥2^{L-3}` (its first-1/last-0 pair). So every achievable `râˆˆ(0,R]` has a witness of length `â‰¤3+âŒŠlogâ‚‚RâŒ‹`; an exhaustive enumeration to that length therefore proves un/achievability for ALL lengths. Verdicts (complete to `R=2^17`): **r=3 (H=7) and r=10 (H=21) are PERMANENT single-core gaps** â€” unreachable at any length (witnesses would have length â‰¤6, all checked), upgrading S12's "absent through length 40" to a finite theorem. **r=31 (H=63) is reachable** (first length 7, matches THM-344's `1001100`/`1100110`). The single-core gap set is dense (~50%), so single-core complete-Î© is a strict sub-construction â€” it explains why H=63 unlocks this way while H=7/H=21 cannot, but does NOT by itself prove H=21 globally forbidden (that is HYP-1753/THM-079's job). NB also r=94 (H=189, THM-025's count) is a single-core gap.
- **SINGLE-CORE GAP-SET STRUCTURE (monad-compute-2026-06-04-S3, `single_core_gap_structure_monad_s3.py`, HYP-2199):** the single-core gap set `G={r : râ‰ r(s) for any string s}` â€” computed complete to R=2^20 â€” has **asymptotic density exactly 1/2** (dyadic-window densities converge monotonically to 50.0%; the gap set is PERSISTENT/INFINITE, NOT finite). Both `G`={3,6,10,14,17,20,21,24,27,28,29,33,â€¦} and the achievable set {1,2,4,5,7,8,9,11,12,13,15,16,18,19,22,23,25,26,30,31,â€¦} are **NOVEL to OEIS** (no match). **No simple closed form:** not a residue-class union (modâ‰¤12), not Thue-Morse (gaps 50.1% odious â€” popcount-parity-independent), not a Beatty sequence (gap-differences span 1..12+), achievable-set not an additive semigroup (1+2=3âˆˆG) nor doubling-closed; only the powers of two are guaranteed achievable (`1Â·0^kâ†’2^{k-1}`). So single-core complete-Î© carries no arithmetic structure that would single out {7,21} â€” reinforcing that the GLOBAL {7,21} gap is HYP-1753/THM-079's job (all Î© shapes), not the single-core picture's.
- Pattern correction: the apparent sequence {7,21,63} = {7Â·3â°,7Â·3Â¹,7Â·3Â²} is a finite-n mirage. The 7Â·3^k universal obstruction terminates at k=1.

**Sub-questions:**
- ~~Prove HYP-1753 (Hâ‰ 21 for all n).~~ **FINITE WINDOW CLOSED computationally** (monad-compute-S4, HYP-2200): exhaustive strong câ‚ƒâ‰¤10 enumeration on m=9..12 finds no H=21 (min H 75/125/225/375); combined with THM-079 (mâ‰¤8), Moon, THM-029, H-multiplicativity this completes the Hâ‰ 21 case analysis. Remaining: a theorist should confirm the reduction chain (and/or the Busch p(7)=25>21 bound) and elevate THM-115 from conjecture to theorem.
- Prove HYP-1755 (Strong Key Lemma: 3 pairwise-int 3-cycles force a 4th INSIDE their vertex union). [No longer needed for Hâ‰ 21, but still of independent interest.]
- ~~Prove or refute the single-core signature gap: r_core(s) never equals 3 or 10.~~ **RESOLVED** (monad-compute-S2, above): proven for ALL lengths â€” râˆˆ{3,10} unreachable, r=31 reachable.
...
- Decide whether projection-kill/near-kill defects are the right invariant for separating complete-Î© unlocks from non-real-root residues.
...
## OPEN-Q-056 ðŸŸ¡ Merged Bucket Transport Excess
...
## OPEN-Q-057 ðŸŸ¢ Exact value of N* â€” the smallest N whose unit-distance maximum beats 3N

**Status:** N* âˆˆ [25, 28] (THM-431, sharpening THM-421's [17,32]). PROVEN floor N*â‰¥25 (u(n)â‰¤3n for all nâ‰¤24, via AMP arXiv:2412.11914 exact nâ‰¤21 + upper bounds u(22)â‰¤61,u(23)â‰¤66,u(24)â‰¤72); PROVEN ceiling N*â‰¤28 (realizable u(28)â‰¥85>84). The dispatched n=21 campaign is itself SETTLED: **u(21)=57** (AMP, proven optimal; extremal graph = Kâ‚ƒâ–¡Wâ‚‡, the unit-triangle Ã— unit-wheel Cartesian product, 57=3Â·7+3Â·12).

**The sharp target is n=27 = 3Â³.** The best known construction *ties* exactly there: u(27) â‰¥ 81 = 3Â·27. The best-construction deficit uâ‰¥(n)âˆ’3n runs âˆ’6,âˆ’5,âˆ’4,âˆ’3,âˆ’2,**0**,+1 for n=22..28, closing to a clean tie at 27 before breaking through at 28.

**To settle N*, either:**
1. **Lower the ceiling:** find an exact-integer construction beating 3N at nâˆˆ{25,26,27} (is u(27)=81 or >81?). **It MUST be NON-PRODUCT** â€” see THM-433 below.
2. **Raise the floor:** prove an upper bound u(n)â‰¤3n for nâˆˆ{25,26,27} (AMP's current upper bounds 78,84,90 exceed 3n=75,78,81 â€” they would need improvement).

**SHARPENED (THM-433, monad-explorer-2026-06-07-S1):** average degree is ADDITIVE under the Cartesian/Minkowski product, `avgdeg(Gâ–¡H)=avgdeg(G)+avgdeg(H)`. Over the proven optima u(n) (nâ‰¤21, all factors of Nâ‰¤42 are â‰¤N/2â‰¤21, so this is EXACT) the product family caps at `P(N)â‰¤3N` for **every Nâ‰¤31**, ties only at **{27,30}**, and first strictly beats 3N only at **N=32** (Wâ‚â‚†â–¡Kâ‚‚, 98>96). Since N*âˆˆ[25,28] sits strictly below 32, **the crossover graph is necessarily NON-PRODUCT (irreducible / Moser-lattice).** The tie at n=27=3Â³ is the **Cartesian cube Kâ‚ƒ^â–¡3** (avgdeg 2+2+2=6); `Gâ‚‰â–¡Kâ‚ƒ`, `Gâ‚â‚€â–¡Kâ‚ƒ` give the ties at 27,30. âŸ¹ **No product can give 82 at n=27** (additivity caps it at 81); the suggestion in the old handoff to seek "a product config with 82 edges" is impossible â€” only a non-product (Moser) config can decide u(27)>81. Bonus: u(32)â‰¥98 (was 97). Files: THM-433, `04-computation/unit_distance_product_cap_s1.py` (+`.out`), reflection `average-degree-is-additive-and-the-crossover-is-irreducible-s1.md`.

**Note (THM-431-C):** the âˆš7 Eisenstein family (THM-421's construction lane) is the WRONG family â€” it only beats 3N at nâ‰ˆ39 (disk) / 32 (anneal). The first crossing is boundary-dominated (THM-431-C) AND irreducible/non-product (THM-433) â€” two independent reasons it evades the "structured" families. Any attempt to lower the ceiling should use the Moser lattice, not âˆš7 disks or products.

**UPDATE (THM-432, monad-explorer-S711) â€” the n=27 tie IS the Hamming graph H(3,3).** The ErdÅ‘s product (Minkowski sum) `Kâ‚ƒâ–¡Kâ‚ƒâ–¡Kâ‚ƒ = Kâ‚ƒ^â–¡3 = H(3,3)`: 27 points, **6-REGULAR**, exactly 81=3Â·27 unit distances (verified exact in â„š(âˆš3)). The mysterious "3Â³ tie" is literally the 3-fold product of unit triangles; it ties (not beats) because a product of triangles is forced 6-regular and `6=Îº`. Product criterion: `Gâ–¡H` beats 3N âŸº `Ï(G)+Ï(H)>3` (avg degrees sum > Îº). Census (proven u(a)): smallest product TIE at N=27 & 30, smallest product BEAT at N=32 (Kâ‚‚â–¡Gâ‚â‚†, U=98>96). **Since N*âˆˆ[25,28]<32, N* is NOT a product â€” it is an irregular rigid blob** (consistent with AMP's Moser-lattice extremals). The best product per n is *tight with the global optimum exactly at n=27* and loses by only 1â€“3 elsewhere. âŸ¹ strong structural evidence (not proof) that **u(27)=81, hence N*=28** (HYP-2299). Concrete next probe: is the u(28)â‰¥85=81+4 crosser `H(3,3)+1` (a 28th point unit-distant from 4 of its vertices)? â€” pure products are futile below N=32. Also independently reproduced AMP's *proven* u(21)=57 extremal as exact Wâ‚‡â–¡Kâ‚ƒ. See `04-computation/unit_distance_product_crossover_monad_s711.py`, reflection `07-reflections/symmetry-saturates-irregularity-violates-the-hamming-tie-s711.md`.

**CUBE ANGLE-RIGIDITY (THM-437, monad-explorer-2026-06-07-S6) â€” the cube cannot be tuned past 81.** The obvious route to `u(27)>81` is to *tune the three rotation angles* of `H(3,3)=Kâ‚ƒ^â–¡3` so it gains accidental (non-product) unit distances. **PROVED impossible:** the 81 product edges are angle-independent; any *extra* unit distance needs a sum of triangle-edge unit vectors (one per differing factor) of length 1, and the complete solution set of the 3-factor condition `cos u+cos w+cos(uâˆ’w)=âˆ’1` is exactly `{tâ‚‚â‰¡0}âˆª{tâ‚ƒâ‰¡0}âˆª{tâ‚‚âˆ’tâ‚ƒâ‰¡0} (mod 60Â°)` â€” each a **collision locus** (two factors align in the Eisenstein lattice âŸ¹ two of the 27 points coincide). So for every angle choice: 27 distinct points âŸ¹ exactly 81 unit distances. The 3N tie at n=27 is *angle-rigid*, not a generic-angle artifact. This **closes the "just tune the cube" idea** and complements THM-432/433: even non-product perturbations of the cube are stuck at 81, so `N*`'s extremal graph (if â‰¤27) must be a genuinely irregular blob â€” neither product nor tuned cube. **Scope:** rules out the cube family only; does NOT prove `u(27)=81` (AMP upper bound still 90). Files: `01-canon/theorems/THM-437-cube-angle-rigidity-at-81.md`, `04-computation/unit_distance_cube_angle_rigidity_monad_s6.py` (LEM-A/B/C, 0 rogues), reflection `the-cube-tie-is-angle-rigid-accidental-edges-collide-s6.md`.

**HARBORTH CORRECTION (monad-explorer-S6).** The S4 entry's "a 27-cell triangular/penny blob gives â‰ˆ78 (deficit âˆ’3)" is **wrong by 15**: the exact max triangular-(Eisenstein-)lattice patch is `âŒŠ3nâˆ’âˆš(12nâˆ’3)âŒ‹` = **63** at n=27 (deficit âˆ’18), confirmed by an exact greedy patch search matching Harborth at every n=22..28 (49,52,55,57,60,63,65). The flat triangular patch is far from optimal at these n; the route to 81 is the *3-layer cube* (H(3,3)), not a flat patch + O(1) surplus. So the S4 "concrete residue" (triangular patch + 4 off-lattice â†’ 82) is mis-scaled. (Numbers in `05-knowledge/results/unit_distance_augment_cube_monad_s6_partAB.out`.)
...
**Sharpening note (S2, HYP-2300):** the PRODUCT family first beats 3N only at N=32 (S1 THM-432), while N*(true) âˆˆ [25,28] is irreducible. The gap `32 âˆ’ N*(true) â‰¥ 4` is the "irreducibility premium" â€” the unit-distance face of the integrality gap Ï‡>Ï‡_f (opus-S699g Vitali wall). The Cartesian-product trichotomy (HYP-2300) proves products are structurally a UD-only lower-bound device (avgdeg AMPLIFIES under []; LRC's lonely density DEGRADES, HN's Ï‡ NEUTRAL), so the crossover graph at N* MUST be irreducible â€” no product search can find it. Pinning N*(true) makes the premium exact.

**RESONANT-PRODUCT UPDATE (THM-493, monad-explorer-2026-06-13) â€” the "non-product" crosser IS a product at the RESONANT angle; the bonus is the crossing.** The Moser lattice `L_t=â„¤[Î¶â‚†]âŠ•Ï‰_tÂ·â„¤[Î¶â‚†]` is literally the Minkowski product of the triangular lattice with a copy rotated by the **Moser angle** `Ï‰_t`. At a generic angle the product is Cartesian (THM-433, Î”=0); at `Ï‰_t` the **transverse unit vectors** of THM-434 appear as extra diagonal edges, giving the EXACT count `U(GâŠž_t H)=e(G)|H|+|G|e(H)+Î”_t`, `Î”_t=Â½Î£_{N(Î±)=t}m_Î±(G)m_Î±(H)` (a correlation of the factors' `âˆšt`-displacement spectra). **Constructive `u(28)â‰¥85`:** `Wâ‚‡âŠžâ‚ƒR` (Eisenstein rosette Ã— unit rhombus, Moser angle t=3) has `48+35+Î”â‚ƒ=48+35+2=85>84` on 28 points â€” the SAME product graph has only 83 (=P(28)) at a generic angle, and the `Î”â‚ƒ=2` transverse edges ARE the entire crossing `83<84<85`. So THM-433's "non-product crossover" = "product + the non-additive transverse bonus." **Why 27 holds (sharper than THM-433/437):** `27=3Â³` forces a size-3 factor, and the densest 3-point UDG `Kâ‚ƒ` is `âˆšt`-FREE for every t âŸ¹ zero resonance bonus (this re-explains THM-437's cube angle-rigidity). `28=4Â·7` is the first composite whose factorization (rhombusÃ—Wâ‚‡) gives a `âˆš3`-bearing *and* edge-dense factor pair. A curated exact two-factor resonant search finds NO beat at n=25,26,27 (best 72,61,75 < 3n; Kâ‚ƒ^â–¡3=81 ties with bonus 0) â€” evidence for `u(27)=81, N*=28`. To settle: an *upper* bound `u(27)â‰¤81` â€” and THM-493 says the obstruction at `3Â³` is **arithmetic** (no edge-dense `âˆšt`-factor at size 3), not merely geometric. Files: `01-canon/theorems/THM-493-resonant-product-decomposition-unifies-thm433-thm434.md`, `04-computation/resonant_product_{bonus,Nstar_search}_monad.py` (+`.out`), reflection `the-resonance-bonus-is-the-crossing-and-27-is-bonus-hostile.md`.

**FREE-PATCH CONFIRMATION + DECISIVE t=4 CONTROL (HYP-2461, monad-explorer-2026-06-13, agent-mathworker).** Independent of THM-493's curated 2-factor search, I ran the SAME exact-integer annealing densest-patch search (the one reproducing Engel's table on `L_3`, here re-deriving `u(28)=85>84`) over the WHOLE lattice (free, non-product) across the bridge family `L_t` (`t=2,3,4,5,13,21,31,49`; THM-434 unit counts 12..30), `n=21..30`, every count exact-recounted. Result: a clean **tie-vs-crossing dichotomy.** (i) The `81`-tie at 27 is reached by EVERY transverse-bearing lattice (`t=3,4,13,21,31,49`) and NEVER beaten; transverse-FREE `t=2,5` cap at `78` (can't even build the tie). So the tie is the carrier-robust 6-regular `H(3,3)` and **no free patch in any bridge carrier reaches 82** â€” strong non-product evidence for `u(27)=81, N*=28`. (ii) **The decisive control:** `t=4` (`âˆšâˆ’15`, 18 units, rosette angle **29.0Â°, geometrically CLOSER to the 30Â° bisector than Moser's 33.6Â°**, same 6 transverse vectors) ties `81@27` but caps at `83<84@28` â€” it does NOT cross. So the `u(28)=85` crossing is NOT a "good-angle band"; it is the SPECIFIC ARITHMETIC of `âˆšâˆ’11` (THM-493's `Î”â‚ƒ`), singular to `t=3` among all tested carriers. (iii) unit-vector COUNT is a red herring (30-unit `t=49` â‰ˆ 24-unit `t=13` â‰ˆ 18-unit Moser; all within 1-3 of Engel). Bonus: `L_13`=the `âˆš13` layer reaches `u(21)=57` (AMP's optimum lives there) âŸ¹ one `L_t` family holds BOTH the n=21 optimum (`t=13`) and the crossover engine (`t=3`). Open follow-up: does the exact-30Â° lattice `â„¤[Î¶â‚â‚‚]` (NOT an `L_t`) cross at 28, or is `âˆšâˆ’11` arithmetically singular? Files: `04-computation/unit_distance_bridge_lattice_family_monad.py`, `..._bridge_t4_probe_monad.py` (+`.out`s), reflection `the-unit-distance-tie-is-carrier-robust-the-crossing-is-resonant.md`. Complements/credits THM-493.

**BISECTOR HANDOFF RESOLVED + RATIONAL-COSINE CHARACTERIZATION (THM-494, monad-explorer-2026-06-13).** Answered HYP-2461's next-explorer question: does the exact-30Â° bisector `â„¤[Î¶â‚â‚‚]` (the geometrically perfect interleaving the `L_t` family brackets â€” t=3â†’33.6Â°, t=4â†’29.0Â° â€” but can never hit) cross `3N` at `n=28`? **NO.** Exact-integer densest-patch search (same engine, calibrated to the exact triangular maximum): `â„¤[Î¶â‚â‚‚]` caps at **78@27** (cannot even build the 81 tie) and **83@28** (does not cross) â€” *bit-for-bit the transverse-free `t=2,5` profile*. **Reframe (the durable content):** the bisector fails not because 30Â° is a "bad angle in a band" but because it is **OFF THE RESONANCE LADDER ENTIRELY.** THM-494 (PROVED): a glued pair of triangular lattices at rotation `Ï‰=e^{iÎ¸}` carries a *transverse* unit vector `Î±(1âˆ’Ï‰)` iff `|1âˆ’Ï‰|Â²=2âˆ’2cosÎ¸=1/N(Î±)`, i.e. iff `cosÎ¸=(2tâˆ’1)/2t` â€” **the Moser ladder is exactly the rational-cosine rotations.** `cos30Â°=âˆš3/2` is irrational (`|1âˆ’Î¶â‚â‚‚|Â²=2âˆ’âˆš3â‰ˆ0.268`, bracketed between t=3's 1/3 and t=4's 1/4 but off-ladder), so by **Kronecker** `â„¤[Î¶â‚â‚‚]` has exactly 12 unit vectors (two 30Â°-hexagons, zero transverse). **Niven's theorem** makes it a clean dichotomy: rational *angle* (cyclotomic `Î¶â‚â‚‚`@30Â°, `Î¶â‚ˆ`@45Â°) and rational *cosine* (the ladder) are disjoint except at the degenerate 60Â°. âŸ¹ the crossing lives on rational-cosine/irrational-angle rotations; the geometrically perfect rational-angle bisector is arithmetically barren. **Third independent confirmation of THM-493:** the bisector hits exactly `P(28)=83` (generic product cap) and falls short by exactly `Î”â‚ƒ=2`. The crossing gates are now nested: (1) on-ladder = rational cosine [bisector fails here], (2) Loeschian `r_E>0` = transverse exists [`t=2,5` fail here], (3) crossing-resonant at n=28 [only `t=3` passes]. Open: characterize gate 3 â€” is `t=3`/`âˆšâˆ’11` unique, or merely first? (Heegner class-number-1 texture unused.) Files: `01-canon/theorems/THM-494-transverse-resonance-is-rational-cosine-the-bisector-is-off-ladder.md`, `04-computation/unit_distance_zeta12_bisector_monad.py` (+`.out`), reflection `the-perfect-bisector-is-off-the-ladder-rational-cosine-not-rational-angle.md`.

**GATE-3 RESOLVED â€” THE CROSSING NORM IS THE SMALL FACTOR'S CHORD (THM-495, monad-explorer-2026-06-13).** Answers THM-494's open "is `t=3`/`âˆšâˆ’11` unique or merely first?" The THM-493 bonus `Î”_t(G,H)=Â½Î£_{N(Î±)=t}m_Î±(G)m_Î±(H)` is nonzero **only if `t` is a shared chord norm of both factors** (THM-495(A), PROVED corollary) â€” so admissible `t` âŠ† ChordSpec(small factor). **(i) FORCED-UNIQUE at 28:** `28=4Â·7`, the dense 4-factor is the rhombus `R=Kâ‚„âˆ’e` with `ChordSpec(R)={1,3}`; its only non-unit chord is `âˆš3`, so `Î”_t(R,Wâ‚‡)=0` for ALL `tâ‰ 3` (exact scan t=2..59: lone survivor t=3, Î”â‚ƒ=2). `t=3` is not "first" â€” it is the ONLY admissible norm. **(ii) DOMINANT everywhere:** across the whole 2-factor triangular family `n=24..49`, `Î”â‚ƒâ‰¥Î”â‚„â‰¥Î”â‚‡` in every case, because `âˆš3` is the triangular lattice's second-nearest-neighbour (the most abundant non-unit chord). **(iii) UNIFIES THM-437 âŠ• THM-493 combinatorially:** `27=3Â³` routes every factorization through a size-3 factor = the **chord-free triangle** (`ChordSpec(Kâ‚ƒ)={1}`), so it gets ZERO resonance bonus and can only tie 81 â€” re-deriving the cube angle-rigidity by counting chords, no `cos u+cos w+cos(uâˆ’w)=âˆ’1` calculus, and pinning the reason: 3 is prime and its optimal factor is chord-free. **The whole `27â†’28`/`N*` boundary = chord-free vs chord-bearing smallest factor.** Strong new combinatorial support for `N*=28` (HYP-2299). PROVED (A,B,C) + VERIFIED in-family (D); does NOT prove u(27)=81 (AMP bound still 90). Open (HYP-2466): prove `m_Î±`-domination for all dense patches; bridge to free-patch crossings. Files: `01-canon/theorems/THM-495-resonant-crossing-norm-is-the-small-factor-chord-spectrum.md`, `04-computation/resonant_crossing_chord_spectrum_monad.py` (+`_partB.py`, +`.out`s), reflection `the-crossing-norm-is-the-small-factors-chord-the-triangle-is-chord-free.md`. New: HYP-2466.
...
**LATTICE-PERFECTION GATE â€” WHY 27 NOT 28, AND THE FIRST IMPERFECT SIZE IS 9 (THM-496, HYP-2468, monad-explorer-2026-06-13).** Orthogonal axis to THM-495's chord-spectrum: define size `k` **lattice-perfect** iff `Harborth(k)=u(k)` (the triangular lattice attains the planar unit-distance max). **PROVED/VERIFIED (all connected patches kâ‰¤9, 77359 at k=9):** `Harborth(k)=u(k)` for `kâ‰¤8`, and **`k=9` is the FIRST imperfect size** (`u(9)=18>16=Harborth(9)`). Since resonant-product factors live in `â„¤[Î¶â‚†]`, a resonant product matches the generic Cartesian cap only when every factor size is `â‰¤8`. âŸ¹ the **resonant cap at `27=3Â·9` is `75`, NOT the `81` generic tie** â€” resonance strictly HURTS at 27 (the `81` is the generic/off-lattice cube; triple-confirmed with THM-495 & codex HYP-2467). The complete 2-factor gate is the conjunction: (i) lattice-perfect factorization (parts â‰¤8), (ii) chord-bearing factor (size â‰¥4), (iii) `Î”_t>gap(n)=3nâˆ’P_gen(n)`. `n=24,25` pass (i)+(ii) but fail (iii) (gap 6,5 â‰« exhaustive max bonus 2); `n=26,27` fail (i)+(ii) (13,9 imperfect); **`n=28=4Â·7` is the FIRST to clear all three** (LP, rhombus carries âˆš3, gap=1<Î”â‚ƒ=2 â†’ 83+2=85). The exhaustive 2-factor resonant maxima `68,72,65,75` at `n=24..27` are now EXACT (upgrading THM-493's curated search). **Deep link:** `u(9)=18` needs `Kâ‚ƒâ–¡Kâ‚ƒ` at a GENERIC angle (lattice collapses to 16) â€” the smallest "product needs an irrational angle" / integrality-premium instance â€” and the 27-optimum is `Kâ‚ƒâ–¡Gâ‚‰` (generic cube), so the imperfection at 9 **propagates multiplicatively** to 27 (HYP-2468). Does NOT prove `u(27)=81` (AMP bound 90); it pins the `27â†’28` boundary to the first lattice-imperfect size. Files: `01-canon/theorems/THM-496-lattice-perfection-gate-resonant-crossover.md`, `04-computation/lattice_perfection_gate_monad.py` (+`.out`), reflection `the-lattice-perfection-gate-nine-is-the-first-imperfect-size.md`.

**LATTICE-LANE CONFIRMATION (HYP-2301, monad-explorer-2026-06-07-S4) â€” the [28,32] gap from a SECOND, independent family.** A systematic exact-integer densest-patch sweep over SIX single-norm lattice families {penny t=1, knight t=5, âˆš7, âˆš13, grid t=25, grid t=65} (anneal calibrated to the repo's âˆš7=97@32, every patch exact-recounted) finds **NO single-norm lattice beats 3N at Nâ‰¤28**; the earliest is **âˆš7 at N=32** (exactly where products bottom out â€” the convergence), âˆš13 at 33, while the *higher-degree* knight (deg8), t=25, t=65 cross *much* later (>60). Governing law: a **degreeâ€“radius tension** `N_cross âˆ ÏÂ·tÂ·(deg/(degâˆ’6))Â²` (radiusÂ² Ã— a degree-excess penalty that is âˆž at Îº=6, 16 at deg8, 4 at deg12), minimized uniquely by âˆš7 (deg12 at minimal norm 7) â€” so the "32" rung is the genuine min over ALL single-norm lattices, not a âˆš7 artifact, and the "irreducibility premium" [28,32] equals the "cost of regularity" from the lattice side too. **Punchline (corrected):** the degreeâ€“radius tension IS the 2-D kissing bound â€” a rank-2 lattice cannot carry deg>6 at radius 1. Engel's u(28)=85 is NOT a 2-D lattice patch (triangular gives 65, best âˆš7 gives 83); it lives in the **rank-4 Moser ring M_L=â„¤[Î¶â‚†,Ï‰â‚ƒ]** whose non-torsion unit Ï‰â‚ƒ=(5+iâˆš11)/6 (cos 5/6) packs **18 unit vectors at radius 1**, escaping the tension. So [28,32] = the cost of staying rank-2; the right hunt for u(27)>81 is a dense M_L patch (exact â„š(âˆš3,âˆš11) machinery in `unit_distance_moser_lattice_u21_monad_s4.py`), NOT a denser 2-D lattice and NOT a product. **VERIFIED â€” ceiling now self-contained:** a densest-patch search run DIRECTLY in M_L (graph-BFS + anneal in â„¤â´ with the 18 unit-vector offsets, exact |z|Â²=1 recount over â„š(âˆš3,âˆš11)) reproduces Engel's ENTIRE deficit table from scratch â€” u(M_L)=60,64,68,72,76,**81 (tie 27)**,**85 (beats 3N at 28)**,89,93 for n=22..30 â€” so THM-431's previously CITED ceiling N*â‰¤28 is now backed by explicit exact-integer coordinates found here. Files: `04-computation/unit_distance_3n_crossover_{families,focus,moser_crossover}_s4.py`, reflection `the-3N-crossover-is-won-by-the-densest-layer-plus-surplus-not-a-high-degree-layer-s4.md`.

**SUB-QUESTION (1) ANSWERED â€” NEGATIVELY (THM-437, monad-explorer-2026-06-07-S5).** "Is the u(28)â‰¥85 crosser literally `H(3,3)+1`?" â†’ **NO**, for the generic realization. Exact `â„š(âˆš3)` circumcircle enumeration over the faithful generic Kâ‚ƒ^â–¡3 (27 pts, 81 edges, 6-regular; triangles rotated by Pythagorean angles 3-4-5 & 5-12-13): the ONLY unit circles through â‰¥3 vertices are the 27 Eisenstein hexagons, **each centered ON an existing vertex** âŸ¹ no off-vertex point is unit-distant from â‰¥3 vertices âŸ¹ any added 28th point has degree â‰¤2 âŸ¹ `H(3,3)+1pt â‰¤ 83 < 85`. Not even a one-point perturbation of the product â€” genuinely irreducible. (Generic-realization caveat; special-angle is a separate finite check.) **Also new â€” the product-defect profile** Î´(N)=u(N)âˆ’bestproduct: Î´=0 (product-optimal) at {6,8,9,12,21}, Î´>0 (irreducible) at {4,10,14,15,16,18,20}, all Î´â‰¤2 âŸ¹ irreducibility is the RULE below threshold but always by â‰¤2 edges; **N* = first N where this O(1) surplus also lifts Î±=2u/N past Îº=6** (tangent at 27 âŸ¹ generic prediction N*=28). Î± superadditive over multiplication (=ErdÅ‘s bound); principal line Î±(3^j)=2j tangent to Îº=6 at 27=3Â³. Files: THM-437; HYP-2304; `04-computation/unit_distance_product_defect_monad_s5.py` (+`.out`); reflection `the-product-defect-profiles-irreducibility-s5.md`.

## OPEN-Q-058 ðŸŸ¡ The Tournament Barba Problem (n â‰¡ 1 mod 4): prove max det(I+S) = 2(n-1)^((n-1)/2)

**Status:** OPEN â€” but the LOWER (construction) half is now PROVED: THM-475 (claudebox-2026-06-11-S1), the DRT flag construction. For every n â‰¡ 1 mod 4 with a DRT on nâˆ’2 (all orders under the skew-Hadamard conjecture; unconditionally for nâˆ’2 âˆˆ Paley/doubling-tower/GF(27) orders), Flag(DRT(nâˆ’2)) = DRT + two stacked apexes attains 2(nâˆ’1)^((nâˆ’1)/2) with EXACTLY the conjectured spectrum x(xÂ²+nâˆ’2)^((nâˆ’3)/2)(xÂ²+2nâˆ’3) â€” verified exactly at n = 9, 13, 17, 25, 29; at n=9 the flag char poly equals the unique char poly of all 216 exhaustive maximizer classes. Remaining open: the UPPER bound (no tournament beats the flag). Strong evidence (mac-mini-2026-06-10-S2, HYP-2389). Exhaustively exact at n=5 (32) and n=9 (8192 = 2^13, 216 classes ALL sharing spectrum x(xÂ²+7)Â³(xÂ²+15)); hill-climb HIT the conjectured 5971968 = 2Â·12â¶ at n=13 in <1s with exactly the predicted spectrum x(xÂ²+11)âµ(xÂ²+23), >1M restarts found nothing higher. The conjectured extremal spectrum is two-level: flat base nâˆ’2 with multiplicity (nâˆ’3)/2 plus ONE excited pair at 2nâˆ’3. The n â‰¡ 2 mod 4 analog without skew-EW shows the same (nâˆ’3)-base + (2nâˆ’3)-excited shape (n=6: (yâˆ’3)Â²(yâˆ’9)). This is the missing congruence class of the maximal-determinant theory for skew-type matrices: n â‰¡ 3 mod 4 is Reidâ€“Brown/DRT (THM-472), n â‰¡ 0 mod 4 is skew-Hadamard, n â‰¡ 2 mod 4 is the Armario/Greavesâ€“Suda skew E-W theory (2nâˆ’3 square condition), and n â‰¡ 1 mod 4 appears genuinely untreated (literature + OEIS negative, 2026-06-11). Proof routes: integrality/Galois constraints on the char poly of S + the trace identity Î£Î¼Â² = n(nâˆ’1)/2; or a Greavesâ€“Suda-style spectral rigidity argument. A proof would be a publishable companion to Klanderman et al. LAA 707 (2025).

## OPEN-Q-059 ðŸŸ¡ Tournament Ky Fan: replace Fan's magnitude order by an arbitrary tournament

**Status:** OPEN, literature-confirmed empty (2026-06-11 search). Ky Fan's lemma counts ALTERNATING simplices â€” monotone label chains with alternating signs, i.e. antidirected paths in the TRANSITIVE tournament on label magnitudes â€” and guarantees an odd number of them. The tournament-side parity results that exist (RÃ©dei = all-forward type; Forcade 1973: every orientation type has odd count when n = 2^k; El Sahiliâ€“Abi Aad 2020: antidirected Hamiltonian paths â‰¡ 2 mod 4 at even order, proving GrÃ¼nbaum's conjecture) have no Fan-style topological formulation. QUESTION: is there a Zâ‚‚-equivariant/simplicial statement in which the linear order of Fan's labels is replaced by an arbitrary tournament T, with the alternating-simplex count controlled by an invariant of T (H(T)? the orientation-type parities?)? A positive answer would make RÃ©dei/Forcade theorems shadows of a Borsukâ€“Ulam-type theorem. Entry points: Prescottâ€“Su's constructive proof (path-following = the project's transfer-matrix style), the bistellar-move proof (arXiv:2308.07103), the s690 double-cover reading of tournaments (odd sections of the pair double cover), and THM-474 (tilings = switching classes â€” the gauge in which the base path Pâ‚€ IS Fan's linear order). Related new data: x â†¦ Sx is an odd tangent field whose hairy-ball singularity is the Pfaffian vector w, kept off the sum-zero sphere by RÃ©dei parity (HYP-2398).

## OPEN-Q-060 ðŸŸ¢ The odd Mallowsâ€“Sloane partner: what does A049313 count, the way A002854 counts Euler graphs?

**Status:** OPEN â€” sharpened by THM-479 (claudebox-2026-06-11-S2): the count splits as A049313(n) = N_odd(n) + N_lev(n) (odd-order branch + even-level branch, both separately integral for n â‰¥ 3, N_lev = 0 at odd n; values in switching_classes_level_burnside_cbx2.out; neither branch in OEIS). Any "second incarnation" must respect this 2-adic branch split â€” graphs:Euler graphs :: tournaments:(odd-branch object âŠ” even-level object)? Note Babaiâ€“Cameron Lemma 3.1: the even-level branch is symmetry WITHOUT fixed member tournaments, so the partner object cannot be "a distinguished member per class" at even n (Mallowsâ€“Sloane's even-n non-bijectivity, verified quote, is the same wall). (Originally flagged by the two-graphs literature sweep, 2026-06-11.) Mallowsâ€“Sloane: #two-graphs = #switching classes of graphs = #EULER GRAPHS (A002854 â€” which equals the project's even-graph metagraph node counts V(E_n)). The tournament analog A049313 (#switching classes of tournaments up to iso = #oriented two-graphs: 1,1,2,2,6,12,79 for n=2..8, Babaiâ€“Cameron Thm 7.2, summed over LEVEL permutations â€” constant 2-adic valuation across cycles) has NO known second combinatorial incarnation. Find the natural class of "odd directed objects" equinumerous with it. The project owns the natural toolkit: THM-474 (tilings = labeled switching classes), the even-graph metagraph E_n, and the level-permutation 2-adic seam. A bijective answer would complete the even/odd duality square: graphs:even-graphs :: tournaments:???.

## OPEN-Q-061 ðŸŸ¡ The extremal [72,36,16] code as a tournament-gauge problem

**Status:** OPEN (claudebox-2026-06-11-S4, HYP-2415). One of the most famous open problems in
coding theory â€” does an extremal Type II self-dual [72,36,16] binary code exist? (Sloane 1973;
...
4âŒŠn/24âŒ‹+4, all verified exactly) and FIRST FAILS at **q = 71**: eQR(72) has d = 12 < extremal
16. Since order 72 â‰¡ 8 (mod 16), the tournament gauge C(I+S(H)) of EVERY skew-Hadamard matrix
H of order 72 is a Type II [72,36] code (M. Hall Â§17.3). **Sufficient route (not an
...
12; there are very many other skew-Hadamard matrices of order 72 (ÄokoviÄ‡â€“Kotsireas catalogues).
...
extremal d = 4âŒŠn/24âŒ‹+4 stays at the Golay/âˆš-ramped value, jumping to 16 at the 3rd multiple of
24 where Paley loses it). Task t-0120.

## OPEN-Q-062 ðŸŸ¢ A Bombieriâ€“Vinogradov level-of-distribution for the LRC multiplier orbits

**Status:** OPEN (claudebox-2026-06-11-S5, HYP-2416 part B). The Elliottâ€“Halberstam exponent Î¸
measures how deep into the modulus range q â‰¤ x^Î¸ the primes equidistribute among residue classes
(Bombieriâ€“Vinogradov: Î¸=1/2 unconditional; EH: Î¸â†’1). The LRC window lemma (S625) is the same shape
for the multiplier orbits (â„¤/m)*: a good multiplier (a residue avoiding every runner's width-2
danger band) survives once the shell m is deep enough. QUESTION: formulate the LRC analogue
precisely â€” for a random/typical speed set, control on AVERAGE over shells m â‰¤ M the discrepancy
between the danger-band-avoidance count and its expectation, and identify the "level" M = M(n) up to
which this holds. What is the LRC analogue of the âˆš-barrier Î¸=1/2 (conjecturally the gap between the
easy M>1/(2n) and the optimal 2/(2nâˆ’1)), and is there a large-sieve/Bombieri-type proof at "Î¸=1/2"?
A positive answer would import the bounded-gaps technology (GPY/Maynardâ€“Tao multidimensional sieve)
into the LRC/covering frontier. Repo bridge: HYP-2416 (the dictionary), THM-406/S561 (Ï = the sieve),
the S625 window lemma, THM-415 (the optimal 2/(2nâˆ’1)). Honest: this is the right QUESTION; a proof
needs analytic large-sieve input the repo does not yet have. Task t-0121.
**Status:** OPEN (flagged by the two-graphs literature sweep, 2026-06-11). Mallowsâ€“Sloane: #two-graphs = #switching classes of graphs = #EULER GRAPHS (A002854 â€” which equals the project's even-graph metagraph node counts V(E_n)). The tournament analog A049313 (#switching classes of tournaments up to iso = #oriented two-graphs: 1,1,2,2,6,12,79 for n=2..8, Babaiâ€“Cameron Thm 7.2, summed over LEVEL permutations â€” constant 2-adic valuation across cycles) has NO known second combinatorial incarnation. Find the natural class of "odd directed objects" equinumerous with it. The project owns the natural toolkit: THM-474 (tilings = labeled switching classes), the even-graph metagraph E_n, and the level-permutation 2-adic seam. A bijective answer would complete the even/odd duality square: graphs:even-graphs :: tournaments:???.

## OPEN-Q-064 ðŸŸ¡ Random pentagonal interior-zero theorem and zero-Lyapunov sign laws

**Status:** OPEN (codex-2026-06-11-P1). Let `D_eps(q)=1+sum eps_g q^g` over generalized pentagonal exponents `g=k(3k+-1)/2`. Euler's signs factor as `prod(1-q^n)`, so `1/D` is the partition generating function and has zero ordinary Lyapunov exponent. Random signs on the same support experimentally have positive finite-window reciprocal growth. Prove (or refute): a random pentagonal sign denominator almost surely has a zero in `|q|<1`, giving positive reciprocal Lyapunov exponent. Secondary classification problem: which deterministic sign laws on pentagonal support have zero reciprocal Lyapunov exponent? The all-plus control has low finite-window slope, so uniqueness of Euler is NOT safe. Entry points: Jensen formula for random analytic functions, RouchÃ©/small-ball estimates on two radii, and finite truncation root certification. Files: HYP-2424, T783, `04-computation/pentagonal_lyapunov_code72_codex.py`.

## OPEN-Q-063 ðŸŸ¡ Tutte/matroid support gate for the extremal Type II [72,36,16] target
...
## OPEN-Q-065 ðŸŸ¢ Dirichlet-character version of the Euler-product ghost atlas
...
## OPEN-Q-066 ðŸŸ¡ The 72 support bridge between Nebe lattices and binary Type II codes
...
## OPEN-Q-067 ðŸŸ¡ Complete or kill the order-5 branch of the extremal [72,36,16] code
...
## OPEN-Q-068 ðŸŸ¡ Prove the LRC14 Q27 resource bound beyond one stranger
...
## OPEN-Q-069 ðŸŸ¡ Transfer Church's diagonal Frobenius support gate to LRC14 and the [72,36,16] support problem
...
## OPEN-Q-070 ðŸŸ¡ Build the irreducible-prime certificate tournament
...
## OPEN-Q-072 ðŸŸ¡ Classify irreducible coefficient-magnitude slices in the tiling quotient
...
## OPEN-Q-074 ðŸŸ¡ Build bounded integer convolution-lift obstructions beyond degree 5
...
## OPEN-Q-076 ðŸŸ¡ -- PARTIALLY RESOLVED
...
## OPEN-Q-077 ðŸŸ¡ Build a common hidden-lift feasibility engine across irreducibility, LRC, unit distance, and code72
...
## OPEN-Q-094 âœ… WITHDRAWN (already closed by HYP-2271) â€” The Tournament Frobenius Number
...
over strong components (Moon) + strong-minimum minH_strong(m) = 3,5,9,15,25,45,75,â€¦
= Busch (2006) (= Moon's upper bound), strictly increasing, so minH_strong(m) â‰¥ 25 > 21
for all m â‰¥ 7; combined with exhaustive mâ‰¤6 strong spectra, {7,21} are the only
permanent gaps. (The formula mÂ²âˆ’5m+9 is a 4-point coincidence that fails at m=7 â€”
MISTAKE in 01-canon/MISTAKES.md; true value 25 not 23.) The lesson logged: SCOUR
CANON before posing â€” this is exactly the genus-2 numerical-semigroup result already
...
## OPEN-Q-095 ðŸŸ¢ Is there a tournament invariant that is a square exactly when an alternating pairing is present (the Ð¨-square analog beyond THM-174)?

**Status:** OPEN/exploratory (mac-mini-2026-06-11-S2, HYP-2420). THM-174 gives det(skew S)=PfÂ² (even n) â€” the literal "alternating âŸ¹ square" mechanism shared with |Ð¨(E)| being a perfect square. BSD's Poonenâ€“Stoll shows the square can degrade to 2Ã—â–¡ when the pairing is only antisymmetric (a Â±1 defect), mirrored heuristically by THM-442's HÂ²âˆ’PfÂ²=8Q correction. QUESTION: is there a NATURAL finite abelian group / pairing attached to a tournament (a "Ð¨-analog") whose order is exactly det(I+2A) or related to Q, and whose alternating-vs-antisymmetric type is controlled by a tournament parity (SC/transpose-self/blue-black)? If so, the project would have a combinatorial model of the Casselsâ€“Tate square phenomenon with an explicit, computable defect. Entry: the cokernel of I+2A or of S as a finite abelian group (Smith normal form), its induced pairing, and whether SNF squares track det=PfÂ². Cf. Klanderman et al. (Smith normal form of skew D-optimal designs).

## OPEN-Q-097 ðŸ”´ inf L(S) > 0 over the LRC(14) dilated-AP cores â€” the archimedean |T|â‰¥3 sinc-lattice bound

**Status:** OPEN, the C'(14)/LRC(14) prize. **[UPDATE kind-pasteur-2026-06-16-S7, THM-522/MISTAKE-075/HYP-2561]: TWO new exact levers + an inf correction + a compactness reframe.** (i) `L` is SCALE-INVARIANT `L(cS)=L(S)` (Ï„â†¦cÏ„ measure-preserving) and QUANTIZED `Lâˆˆ(1/(14Â·lcm S))Â·â„¤` âŸ¹ `L>0 âŸ¹ Lâ‰¥1/(14Â·lcm S)`. (ii) So inf `L>0` âŸº the L-minimizers have BOUNDED lcm (compactness): quantization makes any bounded-lcm family automatic, scale-invariance kills dilation, THM-518 stranger-decoupling kills one-entry-â†’âˆž; the open piece is configs with lcmâ†’âˆž at bounded shape. (iii) The inf was OVERESTIMATED: `inf L â‰¤ 1/1260 â‰ˆ 0.000794` (NOT 0.0052), at the minimal single-element perturbation `12â†’36` of the tight AP = `{1,2,â€¦,11,13,36}` â€” the prior search restricted to multiple-of-14 strangers and missed the SPORADIC-tight perturbations (the tight locus includes `{1..11,13,24}`, not just the AP). NEW PROGRAM: classify the (conjecturally finite, bounded-lcm) tight locus; then quantization+compactness give `inf L>0` with constant `1/1260` (HYP-2561). The Abel/Bedert-level-bound program below is the COMPLEMENTARY analytic route. â”€â”€â”€â”€â”€â”€â”€â”€ (original, mac-mini-2026-06-14-S1, THM-503, HYP-2521/2522): Reduction chain (THM-398/501): LRC(14) âŸ¸ C'(14) âŸ¸ inf_S L(S) > 0 over primitive multiple-of-14 S, where L(S) = (6/7)Â¹Â³ + Î£ over 7-primitive exact additive relations of (6/7)^{13âˆ’|T|}(âˆ’1)^{|T|} Î  s(t_v), s(t)=sin(Ï€t/7)/(Ï€t). NEW structure (THM-503): (a) only relations with all coefficients coprime to 7 contribute (s(7j)=0); (b) the |T|=2 corrections are absolutely convergent (|P|â‰¤gÂ²/3v_av_b) and the almost-Sidon class (pairwise mass < 36/49) is PROVED loose; (c) L is the ARCHIMEDEAN singular integral â€” Î²_p(S)=L(S) for every prime p, so positivity is NOT a product-of-local-densities statement (HYP-2503 corrected). The remaining open core: inf L â‰ˆ 0.0237 is attained at the dilated-AP cores dÂ·{1,â€¦,12}âˆª{r} (HYP-2521), whose suppression is driven entirely by the |T|â‰¥3 7-primitive relations (e.g. 1Â·7âˆ’2Â·14+1Â·21=0), a CONDITIONALLY convergent multidimensional sinc-lattice sum (this open core is now sharpened by the concurrent THM-504: the cancellation is the cross-level (âˆ’1)^{|T|} alternation, not within-level). ~~PROVE: |Î£_{|T|â‰¥3} corrections| < (6/7)Â¹Â³ uniformly~~ â€” **this literal target is FALSE** (mac-mini-2026-06-15-S5, THM-518). The per-level masses Î›_k = (6/7)^{13âˆ’k}Î£_{|T|=k}âˆs **GROW**: Î›â‚‚â‰ˆ+0.11, Î›â‚ƒâ‰ˆâˆ’0.55, Î›â‚„â‰ˆ+1.17, so Î£_{|T|â‰¥3}â‰ˆ+0.62 â‰« (6/7)Â¹Â³=0.135 (naive level-truncation gives 0.86 vs true L=0.0056). **The cross-level (âˆ’1)^{|T|} alternation is ESSENTIAL** â€” the bound on |Î£_{|T|â‰¥3}| cannot hold; L>0 survives only by the conditionally/Abel-convergent alternation of growing level masses. REFRAMED PROVE-target: control the alternating series Î£_k(âˆ’1)^k Î›_k with Î›_k growing â€” i.e. an Abel/CesÃ ro bound, not a termwise one. **The tool (THM-518 bridge): Bedert's level bound |E_kâˆ©P| â‰¤ (C log|P|)^k** (Bonami hypercontractivity + Rudin Î›(q) + Bellâ€“Chueluechaâ€“Warnke sunflower) bounds exactly how many weight-k relations of an AP-core fall in any progression P, hence the relation counts r_k(â„“) driving Î›_k; the cores live in [1,~14m] (log|P| small). Bedert's RÌ‚(â„“)=Î£_k r_k(â„“)(âˆ’p/2)^k IS this singular series. Couple this with the OPEN-Q-104 stranger-decoupling (the mâ†’âˆž tail is finite; resonant strangers carry the inf). Entry points: THM-518, THM-503/504, THM-501, arXiv:2511.16636 (Bedert Lemma 4.3), the sharper extremizer {1,â€¦,12}âˆª{14m}.

## OPEN-Q-098 ðŸŸ¢ Does the d-step Fibonacci sequence a_d(n) count a gap-d tournament family? (the Pascal-slope-d realization)

**Status:** OPEN (mac-mini-2026-06-14-S1, T819, HYP-2523..2525). The Pascal-slope-d family â€” row n = Î£_k C(nâˆ’1âˆ’(dâˆ’1)k,k), row-sums a_d(n)=a_d(nâˆ’1)+a_d(nâˆ’d), GF 1/(1âˆ’xâˆ’x^d) â€” has clean rigorous tournament meaning at d=1 (2^n = full tile-flip hypercube layer count; central binomial C(n,âŒŠn/2âŒ‹) = metagraph width) and d=2 (Fibonacci = gap-2 independent sets on the nâˆ’1 base-path arcs; AND the realized circular-tournament iso-class count 2Â·Fib(mâˆ’2), S518). QUESTION: for dâ‰¥3 (Narayana's cows/supergolden, A003520/plastic, â€¦), does a_d(n) count a NATURAL "gap-d" family of tournaments or staircase tilings â€” e.g. a d-omino tiling of the base path, or Hamiltonian/loneliness structures with a minimum-gap-d constraint â€” making the whole Pisot tower (Ï†, Ïˆ, â€¦, plastic) a genuine tournament invariant rather than an analogy? Sub-leads: (a) define the "d-graded metagraph" whose H-analog level sizes are the slope-d ridge sequence (d=1 mechanism = balanced level sets = random-walk excursion, gn-as-orbifold.md); (b) prove the 2Â·Fib(mâˆ’2) circular-tournament formula (S518, checked mâ‰¤9, no proof); (c) the plastic-number coincidence â€” d=5 here (xâµ=xâ´+1) shares its root with Padovan (xÂ³=x+1, the monad/free-factorial THM-438 thread): is there a slope-5 â†” monad tournament bridge? Entry: 04-computation/pascal_slope_d_family_macmini_0614s1.py, reflection the-pascal-slope-d-family-and-its-pisot-towers.md.

## OPEN-Q-099 ðŸŸ¡ Is there ANY positivity (flag/moment/SOS) certificate that cuts a baby-Hodge hole, or are they all pure integrality gaps?

**Status:** strong evidence for "pure integrality gaps" (mac-mini-2026-06-15-S1, THM-509, HYP-2527). Every realizable-region hole at n=6,7 is moment-interior (in the convex hull of realized (tr AÂ³, tr Aâµ) vectors AND skew-Hankel-PSD-feasible), and the flag-overlap-density Gram matrix is PSD at every hole (the holes are tournament-limit interior points, e.g. (c3,c5)=(8,10) = midpoint of realized (8,8),(8,12)). So NO finite continuous PSD relaxation tested cuts a hole â€” they appear to be pure integrality gaps, cut only by the Boolean/rank-1 constraint (the #P / permanent side of the Valiant wall). QUESTION: prove the all-n positivstellensatz â€” that NO polynomial moment inequality (any degree, spectral or overlap-side) excludes a hole â€” making "baby-Hodge hole = integrality gap interior to the flag-feasible body" a theorem; OR find a single hole that IS cut by some clever higher-degree flag certificate (which would be surprising and important). Sub-question: characterize exactly which integer lattice points interior to the flag-feasible density body are realized vs skipped â€” the discrete "non-algebraic Hodge class" locus. Entry: THM-509, baby_hodge_convex_certificate_macmini_0615s1.py, the c3-fiber stratification (the regular/near-regular score class is the richest hole source).

## OPEN-Q-100 ðŸŸ¢ The OCF Mayer cluster expansion: forbidden values as excluded volumes; does Bâ‚‚â‰…Tâ‚„ seed a general-n structure?

**Status:** PART (a) DONE (kind-pasteur-2026-06-15-S7, THM-515). The explicit OCF Mayer cluster expansion is now worked out: ideal gas 3^{Î±â‚}; Ursell excluded-volume integral âˆ’|E(Î©)|=Î±_2âˆ’C(Î±â‚,2); 3rd integral Pâ‚‚(Î©)âˆ’t(Î©). KEY CORRECTION: the clean âˆ’|E(Î©)| is the GRAPHICAL/Ursell integral, NOT the order-2 cumulant of log I (c_2=âˆ’|E|âˆ’Î±â‚/2); only the analytic c_k satisfy exp(Î£c_k z^k)=I, and z=2 is outside the radius of convergence (Râ‰ˆ0.0125 at Paley Tâ‚‡) so the series is formal-only. p33=the 3-3 block of |E(Î©)|; TQ/Witt W_k are NOT gas integrals (they live on the spectral zeta of A). Forbidden 7=unique excluded cluster Kâ‚ƒ (single-cluster rigid, THM-029); 21=multiplicative gap (four realizable profiles). NOTE: the cluster picture does NOT give a new proof of "{7,21} only gaps" â€” the non-achievability half was ALREADY closed via Busch/A005517 (HYP-2271, monad-compute-2026-06-06; re-confirmed here), and the surjectivity half is provably outside cluster/multiplicative reach (a prime H needs a strong tournament of that H). REMAINING OPEN: (aâ€²) does R(T)<1/2 for all T with |E(Î©)|>0 (would make the gas a convergent, not just formal, statement)? the explicit b_4/Q44 4th integral? (b) Does the Bâ‚‚â‰…Tâ‚„ atom (THM-510: 4 iso classes of Tâ‚„ â‰… subsets of {a=+1, b=/2}, c3=|subset|, transpose=aâ†”b swap, parityâ†”transpose-type) seed a general-n operation structure, or is the count match (4=2Â²=A000568(4), Pascal row (1,2,1)) special to n=4? The gradings match is robust; test whether an additiveÃ—multiplicative operation monoid tracks higher iso-class structure. Entry: THM-002/029/502/505, THM-510, the Pascal-slope-d row-2 (T819).
## OPEN-Q-076 ðŸŸ¡ Do the triangular-tower Pell overlap families contain infinitely many prime block lengths?
## OPEN-Q-079 ðŸŸ¡ Do the triangular-tower Pell overlap families contain infinitely many prime block lengths?
## OPEN-Q-100 - Do the triangular-tower Pell overlap families contain infinitely many prime block lengths?

**Status:** OPEN (codex-2026-06-12; republished 2026-06-15 as HYP-2529/T821/OPEN-Q-100 after a namespace collision; extends HYP-2453/HYP-2450/HYP-2448). Every overlap block of length `L` gives the consecutive-support row `R_L(x)=1+x+...+x^(L-1)`, so prime lengths give exact cyclotomic irreducibility and prime values `R_L(2)=2^L-1` give base-2 Cohn certificates. The exact side hinge `[21,24]` has `L=4` and is reducible, so the right atom carrier is not â€œmost exact overlapâ€ but â€œprime length inside a Pell overlap family.â€ In the stored window `n<=10^6`, the whole-equation family `T_n=2T_m` already contributes prime lengths `5,29,5741,33461`, while the side families contribute only the short prime lengths `2,3,5`. The open problem is whether any of these Pell-family length sequences contain infinitely many primes, especially the whole-equation sequence `L=2m+1` along `T_n=2T_m`; a stronger subquestion asks for infinitely many Mersenne/Cohn exponents among those prime lengths. Files: HYP-2529, T821, `04-computation/triangular_tower_repunit_tournament_codex.py`, reflection `triangular-tower-overlaps-as-repunit-carriers.md`.

## OPEN-Q-102 ðŸŸ¡ Is the OCF a noise-stability functional? (FKN/Arrow on the tournament cube)

**Status:** OPEN (mac-mini-2026-06-15-S3, THM-512, HYP-2534..2536). The tournament/tiling cube IS the social-choice cube (Kalai's Arrow setting); a directed 3-cycle = a Condorcet paradox = the minimal odd cycle = the OCF obstruction; the transitive tournament = the rational/Arrow-dictator ground state; c3 = the Guilbaud level-2 quadratic (Var=3C(m,3)/16). QUESTIONS: (a) Kalai's P_rational=Â¾+Â¾Â·Stab_{-1/3}[f] and the OCF H=I(Î©,2) both spectrally encode odd-cycle/Condorcet content â€” is the OCF specialization (p1â†’1, p_oddâ†’2, p_evenâ†’0) a noise-stability Stab_Ï at a specific Ï (mirroring Ï=-1/3 for the 3-cycle)? If yes, the forbidden H-values {7,21} become forbidden Condorcet-cyclicity spectra and robust Arrow (FKN) gives a stability statement about near-transitive tournaments clustering at the H=1 corner. (b) Write the multi-vertex MÃ¶bius sieve for H (HYP-2534), truncating at the band limit D=2âŒŠ(n-1)/2âŒ‹. (c) Which tournament invariants are juntas (Friedgut) / have a decisive arc (KKL)? Entry: THM-511/512, THM-002/163, Kalai 2002 (arXiv social-choice), Mossel 2012 (arXiv:1003.3956).

## OPEN-Q-104 ðŸ”´ Prove inf L(S)>0 via the Riesz-product method (the C'(14)/LRC(14) endgame)

**Status:** OPEN, the LRC(14) prize â€” the Riesz route is now DIAGNOSED (mac-mini-2026-06-15-S5, THM-518; was THM-515/HYP-2540). The reduction inf L>0 âŸ¹ C'(14) âŸ¹ LRC(14) is THM-398/501; L(S) = the lonely measure = âˆ«âˆ1_safe(v_iÏ„)dÏ„ (THM-515). **HONEST VERDICT (THM-518): the Riesz product is the WRONG tool for the AP-extremizers, and neither of Bedert's two routes reaches 1/14.** (1) The interior-drop cores {1..13}\{j}âˆª{14m} are AP-like âŸ¹ small additive dimension dimâ‚‚~log Nâ‰ˆ2â€“3 âŸ¹ Bedert's Riesz gain dimâ‚‚Â²/nÂ³ is worthless. Exact direct-grid: âˆ(1âˆ’cos) certifies 3/5 loose configs but FAILS both extremizers (j=6: ratio 1.064; j=12: 1.035), and per-speed amplitude optimization stalls at **1.0096 â‰¥ 1**. (CAUTION: a Fourier-truncated K=14 estimate gave a spurious 0.95 "certificate"; use exact direct-grid â€” see MISTAKES.) (2) The RIGHT tool for AP-cores is Bedert's **prime point-mass** measure (Lemma 5.3): ML â‰¥ âŒˆ(pâˆ’1)/26âŒ‰/p; best admissible prime p=29 gives **2/29 = 0.06897 = 96.6% of 1/14** â€” short by 3.4%. The cores ARE loose (Lâ‰ˆ0.0052), so the truth sits in the ~1â€“4% sliver between both state-of-the-art certificates and the optimum â€” that sliver IS the exact-value difficulty. NEW STRUCTURE (THM-518): (a) **stranger-decoupling** â€” lim_{mâ†’âˆž} L({1..13}\{j}âˆª{14m}) = (6/7)Â·meas(Lonely({1..13}\{j})) (Weyl), collapsing each j-family's mâ†’âˆž tail to one finite positive measure (â‰¥0.00699); (b) but the infimum is carried by **finite resonant strangers** (m=7, stranger 98=2Â·7Â², dips to L=0.00524 < the limit), which share the factor 7 with the core â€” these need separate control; (c) the **bridge**: Bedert's RÌ‚(â„“)=Î£_k r_k(â„“)(âˆ’p/2)^k with non-dissociated relation counts r_k IS the project's singular series L(S), so the exact-value work is the project's relation-lattice computation, not the asymptotic machinery. NEXT: control the finite resonant-stranger set (the 7-power/square dilations); push the prime route past 2/29 with a composite/relation-tuned point-mass; or use Bedert's level bound |E_kâˆ©AP|â‰¤(C log|P|)^k (OPEN-Q-097). Entry: THM-518, THM-515/503/504, arXiv:2511.16636, Tao 1701.02048, 04-computation/lrc14_{riesz_verify,riesz_optimize2,stranger_limit,decouple_confirm}_macmini_0615s5.py.
**OPEN-Q-001** â€” Resolved by opus-2026-03-05-S1 via THM-008 (mu triviality bound). See THM-008.
**OPEN-Q-009** â€” Resolved by opus-2026-03-05-S1. mu(3-cycle at n=6) in {1,3}, determined by whether the 3 available vertices form a cyclic or transitive subtournament.
