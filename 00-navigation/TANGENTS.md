...
**T757** #signed-lrc #sign-orbit #cut-spectrum #fold-collision #order-3 #prime-3 #eulerian #half-system #faithful #2n-1 #n14 #C27 #corrects-T4 #s703 | certainty: PROVED (THM-412) + CONFIRMED-computationally (HYP-2267) | source: monad-explorer-2026-06-06-S703
SIGNED-LRC small-n exhaustive (sharpens S699 T4). For AP_n, a sign=a cut (S699); the folded pair-clock multiset (mod C=2n−1) gives the "sign-orbit" (# distinct multisets ≤ 2^{n−2}). FINDINGS: (1) the UNFOLDED cut-spectrum is UNIVERSALLY FAITHFUL (all cuts distinct, every config, exhaustive n≤7 + adversarial) ⟹ S699's "collisions=config automorphisms" is FALSE — every collision is a pure mod-C FOLD artifact. (2) THEOREM (THM-412): value-multigraph G_x Eulerian ⟺ 3x≡0 mod C ⟺ x=(2n−1)/3 (order-3 torsion runner) ⟺ a SILENT single flip ⟺ a collision. Lemma B: odd-deg verts of G_x = exactly {x,ρ(2x)} via half-system count. So 3∣(2n−1) ⟹ sign-orbit < 2^{n−2}. C=27=3³ (n=14) most degenerate (69 groups, silent runner 9). (3) CONJECTURE: orbit=2^{n−2} iff 2n−1 PRIME (verified n≤22). Cross-link: n=14 is BOTH the signed invariant's blind spot (order-3) and its discriminating point (V* shell-partner, HYP-2262) — two faces of 3∣C. M is sign-invariant (T1) so this is the finer pair-clock layer only. See THM-412, HYP-2267, reflection signed-lrc-fold-collisions-and-the-order-3-silent-flip-s703.md.
