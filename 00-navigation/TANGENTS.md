...
THM-027 PROVED: The n×n transfer matrix M[a,b] = sum_S (-1)^|S| E_a(S) B_b(R) satisfies tr(M) = H(T) for odd n, 0 for even n. PROOF: Bijection between (σ through S ending at a, τ through R starting from a) pairs and Hamiltonian paths; each path P contributes (-1)^{pos(a,P)} to M[a,a]. Summing over a gives sum_{k=0}^{n-1} (-1)^k per path. ALSO: the old claim M = [[1,0],[0,-1]] always is FALSE (MISTAKE-011). M values vary widely with tournament; only SYMMETRY M[a,b] = M[b,a] holds universally.
