...
**T104** #hereditary-maximizer #even-odd-dichotomy #vertex-deletion | certainty: high (exhaustive n=3-7) | source: kind-pasteur-2026-03-06-S18f
HEREDITARY MAXIMIZER CHAIN IS ODD-ONLY. At odd n=3,5,7: EVERY vertex deletion from the global H-maximizer gives the (n-1)-maximizer. At even n=4,6: NO deletion gives the (n-1)-maximizer (H=1 or 3 at n=4, H=11 or 13 at n=6, vs max=3,15). The obstruction at even n is NOT score incompatibility — deletion scores (1,2,2,2,3) at n=6 ARE maximizer scores at n=5, but the subtournament is suboptimal within that class. Score change at odd regular n: delete v → (d-1)^d, d^d (d=(n-1)/2) = self-comp score = maximizer score at n-1. The constant deletion spectrum (all vertices give same H(T-v)) holds for ALL maximizers at n=5,6,7. Two types at n=6: Type A (del=11, IP=[1,14,4]) and Type B (del=13, IP=[1,20,1]).
