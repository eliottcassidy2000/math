...
**T721** #tournaments #enumeration #THM410 #square-blowup #A000568 #Hamiltonian-paths #modular-decomposition | certainty: reserved active scout | source: codex-2026-06-05-S652
Reserved for S652: use THM-410 interval-reversal witnesses and node-square tournament substitution as speedup carriers.  The working idea is that low-`c3` / near-transitive search should use additive interval ledgers, while `n^2`-vertex square blowups should use block path-cover polynomials and macro-word DP instead of raw `2^(n^2)` Held-Karp or unmarked `S_{n^2}` canonicalization.  See HYP-2228.
