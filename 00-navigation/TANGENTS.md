...
The `p0` collapse family is larger than AP under the endpoint-collapse predicate: no open safe gap, unit boundary witnesses, and witness quotient `n`. S602 finds nine collapsed rows in targeted boxes through `n=8`, all two-seed addition chains. The user sporadics `(1,3,4,7)` and `(1,3,4,5,9)` are the unique primitive `{2}` flip floor transversals for `n=5,6`; composite `C=15` adds non-transversal nonunit-shell chains. See HYP-2153 and `07-reflections/lrc-p0-collapse-additive-chains-s602.md`.
