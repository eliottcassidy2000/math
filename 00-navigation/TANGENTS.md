...
**T721** #tournaments #enumeration #THM410 #square-blowup #A000568 #Hamiltonian-paths #modular-decomposition | certainty: finite scout + exact formula validation | source: codex-2026-06-05-S652
THM-410 and square-node substitution give two enumeration speedup carriers.  For a transitive tournament with a reversed matching, `c3=sum(b-a-1)` exactly; the H=21 cap `c3<=10` leaves `188273/2390480` weighted matching states at `n=14`.  For node-squaring, each vertex of an `n`-vertex base becomes an `n`-vertex tournament block; Burnside over base automorphisms gives the square-template sequence `1,1,12,704,2127984` through `n=5`.  Naive `H` multiplicativity fails on cyclic bases, but the exact replacement is `H(B[F_i])=sum_r MacroWords_B(r)*prod_i OrderedPathCovers(F_i,r_i)`: `cycle3[chain3,chain3,chain3]` has direct/formula `H=2721` versus naive product `3`.  See HYP-2228.
