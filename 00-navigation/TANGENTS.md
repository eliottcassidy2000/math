...
Map point sets to tournaments by flipping unit-distance tiles, or by flipping nonunit tiles.  S625 separates graph-level from order-level flop: triangular rows through `n=18` and Moser rows through `n=10` all keep a unit-graph Hamiltonian path; the nonunit complement first has one at `n=6`, fails at the compact `n=7` hexagon because the center is complement-isolated, and reappears from `n=8`; the fixed canonical unit-flip directed all-unit HP first vanishes at `n=7`.  Use snake-base tilings and HP unit-step histograms rather than one binary path label.  See HYP-2201.
