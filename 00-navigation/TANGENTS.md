...
**T084** #interlacing #vertex-deletion #real-roots #induction | certainty: high (computational, 1324/1324) | source: kind-pasteur-2026-03-06-S17
I(Omega(T-v), x) INTERLACES I(Omega(T), x) for all tested tournaments and vertices. 0 failures across n=5 exhaustive (1024) + n=6 (200 random) + n=7 (100 random). If provable, this gives an INDUCTIVE proof of real-rootedness: base case (small n) has real roots; interlacing preserves real-rootedness under vertex addition. This is a MAJOR lead — interlacing is the standard mechanism for proving real roots in combinatorics (e.g., common interlacing for matroids). The key question: does vertex deletion in a tournament induce a "nice" relationship between Omega(T) and Omega(T-v)?
...
**T091** #SC-maximizer-within-score-class #blueself #H-maximization | certainty: high (exhaustive n=4,5,6) | source: kind-pasteur-2026-03-06-S18
WITHIN each score sequence class, the isomorphism class with maximum H is ALWAYS self-converse. Verified exhaustively at n=4 (1 mixed score class, SC max > NSC max), n=5 (all classes with both SC+NSC have SC achieving max), n=6 (3 mixed score classes: (3,3,3,2,2,2) SC max=45 > NSC max=43; (4,3,3,2,2,1) SC max=37 > NSC max=31; (4,4,3,2,1,1) SC max=17 > NSC max=15). The SC advantage in I.P. comes from either more total cycles (alpha_1) or more disjoint pairs (alpha_2). The (3,3,3,2,2,2) class is particularly revealing: all 5 classes have c3=8 identical 3-cycle counts but different 5-cycle counts and disjoint pair counts.

**T092** #score-regularity-H-correlation #variance | certainty: high (exhaustive n=6) | source: kind-pasteur-2026-03-06-S18
Score sequence regularity (low variance) strongly correlates with high H at n=6. Regularity 0.250 (most regular, (3,3,3,2,2,2)) → H=41-45. Regularity 2.917 (transitive, (5,4,3,2,1,0)) → H=1. The correlation is not monotone at intermediate values but the extremes are clear. SC classes cluster at low regularity (regular scores). This may explain the SC maximizer phenomenon: SC tournaments have balanced structure, forcing regular scores, which maximize odd cycle collections.
