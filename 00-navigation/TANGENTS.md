...
Helly logarithms are certificate-search entropy: `Lambda_H(M)=log sum_{h<=H} binom(M,h)`, so fixed-rank Helly costs `H log M`; if the live component count is already `log N` or `loglog N`, the proof pays `loglog` or `logloglog`. S601 reruns the S599 two-block classifier and finds 1113 extracted certificates, all singleton/pair, with zero high-order/live rows. See HYP-2151 and `07-reflections/helly-scale-logarithm-s601.md`.
