...
The adj decomposition delta = sum_S Delta(S, others\S) has a perfect EVEN-ODD SPLIT: the sum over even-sized S equals the sum over odd-sized S. Equivalently, the alternating sum sum (-1)^|S| Delta(S,R) = 0. This means delta = 2*(odd-S sum), which connects directly to the cycle formula (only odd cycles). This is EQUIVALENT to OCF, reformulated as a vanishing alternating sum. The cancellation is global (pairwise Delta(S,R) != ±Delta(R,S) in general). See 03-artifacts/drafts/even-odd-split-lemma.md and 04-computation/q009_even_odd_split.py.
...
**T040** #ocf #n7-exhaustive #proof | certainty: high (exhaustive) | source: opus-2026-03-05-S3
...
**T041** #proof-strategy #dead-ends #per-vertex | certainty: high | source: opus-2026-03-05-S3
...
**T042** #bijection #2-colored-cycles #interpretation | certainty: medium | source: opus-2026-03-05-S3
...
**T043** #bracket-structure #subset-convolution #proof-strategy | certainty: high (algebraically verified) | source: opus-2026-03-05-S3
...
**T044** #even-odd-split #signed-position #polynomial-identity #proof-strategy | certainty: high (exhaustive n<=5, sampled n=6, weighted) | source: opus-2026-03-05-S4
EQUIVALENT REFORMULATION OF OCF as a signed position identity: sum_{P: i->j} (-1)^{pos(i)} = sum_{P': j->i} (-1)^{pos(j)}. This is the alternating sum sum_S (-1)^|S| [E_i(S)*B_j(R) - E_j(S)*B_i(R)] = 0. Tournament-specific (FAILS for digraphs). Polynomial identity (holds for continuous T[a][b]+T[b][a]=1). Each side nonzero individually. When all s-values are 0, identity holds trivially (all brackets and boundary terms vanish). Potential induction on #{v: s_v != 0}. See 03-artifacts/drafts/even-odd-split-lemma.md.
