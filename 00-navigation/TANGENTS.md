...
**T765** #triangular-towers #consecutive-integers #consecutive-squares #A059270 #A059255 #A222716 #pell #endpoint-shells #addition-vs-multiplication #s712 | certainty: exact formulas + bounded crossover classification | source: monad-explorer-2026-06-12-S712
THE TWO TRIANGULAR TOWERS split into one rigid exact bridge plus several infinite Pell truncation families. Additive tower: `A_L(n)=[n^2..n^2+n]`, `A_R(n)=[n^2+n+1..n^2+2n]`, sum `A059270(n)=n(n+1)(2n+1)/2 = 3*sum_{j<=n} j^2` — so the "pure addition" tower already hides squares. Square tower: `B_L(n)=[2n^2+n..2n^2+2n]`, `B_R(n)=[2n^2+2n+1..2n^2+3n]`, and the sum of squares on either block is `A059255(n)=A059270(n)*((12n^2+12n+1)/3)`. USER HITS SHARPENED: `21..24` is the UNIQUE exact raw block shared by both towers, because `B_L(n)=A_R(m)` forces `m=n+1` and then `n^2-2n-3=0`, so `(n,m)=(3,4)`. The softer overlaps are INFINITE Pell families: `10..12` as the right tail of `9..12` and `13..14` as the left prefix of `13..15` come from `m(m+1)=2n(n+1)` i.e. `(2m+1)^2-2(2n+1)^2=-1`, first hits `(2,3),(14,20),(84,119),...`; `25..27` starting `A_L(5)` comes from `m^2=2n^2+2n+1`, i.e. `2m^2-(2n+1)^2=1`, first hits `(3,5),(20,29),(119,169),...`; `36..40` starting `A_L(6)` comes from `m^2=n(2n+1)`, i.e. `(4n+1)^2-8m^2=1`, first hits `(4,6),(144,204),...`. THIRD TOWER: the triangular-number analog `A222716` exists, so the figurate ladder continues even though the power ladder stops above squares (`A234319` per OEIS crossrefs). Open: classify all overlaps, not just endpoint coincidences; test whether the raw-sum crossover `90` is globally unique; push the triangular analog into the same shell/endpoint language.
