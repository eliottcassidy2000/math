...
**T798** #triangular-towers #repunit #cyclotomic #irreducibility #coefficient-rows #tournaments #pell #HYP-2454 #OPEN-Q-076 | certainty: exact finite atlas + nontransitive family tournament | source: codex-2026-06-12
TRIANGULAR-TOWER REPUNIT CARRIER. Every overlap block `[a,a+L-1]` gives the consecutive-support row `x^a(1+x+...+x^(L-1))`, so prime lengths yield exact cyclotomic irreducibility and prime values at `x=2` give Cohn certificates. The exact hinge `[21,24]` has length `4` and is reducible, while the whole-equation Pell family contributes prime lengths `5,29,5741,33461` through `n<=10^6`. The family tournament over window hits, exact hits, support ratio, and prime-length/Mersenne supply is nontransitive (`6` directed 3-cycles; `8` edge flips versus support-only), so exactness/support and atom supply are genuinely different carriers. Files: HYP-2454, OPEN-Q-076, `triangular_tower_repunit_tournament_codex.py`.
**T801** #triangular-towers #repunit #cyclotomic #irreducibility #coefficient-rows #tournaments #pell #HYP-2457 #OPEN-Q-079 | certainty: exact finite atlas + nontransitive family tournament | source: codex-2026-06-12
TRIANGULAR-TOWER REPUNIT CARRIER. Every overlap block `[a,a+L-1]` gives the consecutive-support row `x^a(1+x+...+x^(L-1))`, so prime lengths yield exact cyclotomic irreducibility and prime values at `x=2` give Cohn certificates. The exact hinge `[21,24]` has length `4` and is reducible, while the whole-equation Pell family contributes prime lengths `5,29,5741,33461` through `n<=10^6`. The family tournament over window hits, exact hits, support ratio, and prime-length/Mersenne supply is nontransitive (`6` directed 3-cycles; `8` edge flips versus support-only), so exactness/support and atom supply are genuinely different carriers. Files: HYP-2457, OPEN-Q-079, `triangular_tower_repunit_tournament_codex.py`.
...
**T798** #triangular #power-balance #consecutive-sums #squares #pell #support-gate #72-36-16 #lrc14 #HYP-2454 #OPEN-Q-076 | certainty: exact p=1/p=2 identities + finite bracket scout; open support-transfer program | source: codex-2026-06-12
TRIANGULAR POWER-BALANCE TOWERS. The two prompt towers are exact interval balances for `D_p(C,n)=sum(C-i)^p-sum(C+i)^p`: ordinary sums have center `2T_n`, consecutive-square sums have center `4T_n`. The square tower's unsquared shadow has defect `L2-R2=2T_n` and total `L2+R2=4*S1`. The only full side equality found through the scout is `Q_L(3)=[21,22,23,24]=F_R(4)`; its ordinary shadows are `90=S1(4)` and `78=C(13,2)`, matching the `[72,36,16]` minimum-design `lambda_5`. Power-center scout brackets p>=3 roots between `2pT_n` and `2pT_n+1` for `p<=8,n<=40`. Next: prove the bracket theorem, solve endpoint Pell families, and test the 78/90 shadow as a code/LRC support-ledger address.
