...
## 2026-08-26 current control panel
...
**Complement-clock compiler.** THM-3363's `D=14` terminal is the seed for THM-3366.  On every THM-2928 quotient, covering the unsupported open cells by `r` integer danger combs compiles a hypothetical projected cover into at most `7+r` global clocks (`8+r` when `k=0`); cited LRC through twelve clocks closes `r<=5` (`r<=4` for `k=0`).  The exact pool-14 census closes raw support rows `17,561/19,273/19,198/19,053` for `k=0/1/2/3`.  Keywise intersection with the live refined ledgers removes `298` `k=2` rows / `71,575,318` occurrences and `7` `k=3` rows / `7,648` occurrences, leaving respectively `4,056 / 200,069,517,203` and `1,897 / 2,548,893,834`.  The seven `k=3` hits are exactly `{2,6,8,10,14} union {u}`, odd `u<=13`, at `D=L/2`.  The raw screens overlap all later screens and are not subtractive or physical-cover claims.
**Reflected cone.**  THM-2941's proved cap-aware gain CSP closes `3m>=4D`.  Its global phase-zero atlas is exactly `4/3,3/2,2,5/2,3,4,5,6`.  The one/three-quarter/two-thirds refinements preserve useful finite gain data, but MISTAKE-347 invalidates their split-tail cover; repair must cover both original level orderings.  At cap `5/2`, the finite census still finds one prime-five bridge in each of 65 forced components; this is not presently a cone closure.
**Affine/antipodal and arithmetic controls.** THM-4188 gives the `15/19/23` resonance filtration. THM-4156's full-pool mass already closes every fixed-pool subset by heredity (MISTAKE-520). [THM-4203](../01-canon/theorems/THM-4203-fixed-pool-seventeen-body-depth-eight-haar-completion.md) gives an independent sharp E8 certificate: nontransversals have a disjoint repair, while all `66,468` exceptional transversals through size `17` have positive direct margins. THM-4191's full deck transfers all `C(30,10)=30,045,015` pool bodies per newcomer, sharply using depth seven at q50 and native depth six on 23 exceptions. THM-4201 gives the exactly-one projected profile: 65/69 rows close at depth five and four at depth six with `tau=10/9/10/10`; THM-4190 retains independent zeta margins. [THM-4207](../01-canon/theorems/THM-4207-two-newcomer-sharp-depth-transition-base-surplus-composition-and-variable-pool-chart-number.md) proves the exact `(50,51)` transition `tau(E8)=8` but `tau(E9)>9`, closes all `14,307,150` fixed-pair eleven-body cores and their THM-4150 odd-tail LRC(14) families, and isolates base surplus as the missing marginal-composition coordinate. [THM-4211](../01-canon/theorems/THM-4211-fixed-fifty-cofinal-two-newcomer-haar-tail-and-eighteen-label-chart.md) gives every full-pool nine-body with `{50,r}`, `r>=5682`, every displayed 18-chart nine-body with `{50,r}` for every outsider, and fixed `(49,50)`. [THM-4214](../01-canon/theorems/THM-4214-two-newcomer-pascal-complete-eleven-body-haar-charts.md) glues the zero/one/central-two layers into Pascal charts with lossless ordinal ranks. [THM-4231](../01-canon/theorems/THM-4231-arbitrary-pair-cofinal-depth-six-haar-repair-and-exact-outsider-lift.md) removes the fixed outsider: a symmetric Bonferroni/component deck proves every full-pool nine-body with distinct `q,r>=17548` and hence all `C(32,11)=129,024,480` chart faces. The `5682` and `17548` values are activation-only, and the arbitrary star adds no unproved leaf-leaf pair. Finite pair entry, physical entry, and full LRC(14) remain open.
**Descending banks.**  For `k=3`, torsion/status closes through `297`; the next 45 occupied rows split `1549=659+882+8`, and order-two/four torsion closes the eight `z286` terminals, leaving `275`.  THM-2979 closes the ten `z275` rows by `2033=273+1695+65`; clean-cell density above `1/7` forces an exact order-seven collision in each terminal.  The THM-2941 addendum closes 29 further rows at `274,273,272` by `246=97+149`; height `271` is empty.  THM-2981 closes `423` rows at the fifteen occupied heights `270..247`, THM-3033 closes 200 more through `244`, and THM-3041/3052/3061/3071/3078/3094/3098/3102/3106/3109/3111/3113/3114 empty every projected row through `z_1=226`, reaching the intermediate cap `225`, ledger `374,172`. THM-3139--3251 continue through `z1=219` and cap the projected lane at `216`; THM-3313/3320 close the third and fourth intrinsic-cost prefixes there. THM-3351 closes sixteen more complete families, and THM-3378 leaves ledger/wall/families `372,913/109/12`.  For `k=2`, THM-2970/2972 close through `1679`; THM-2980's monolithic `3,003*80` audit closes all 68 candidate rows at `1600..1679`, and THM-2995's `3,003*20` audit closes all 26 at `1580..1599`.  Their typed joins include literal, torsion, scalar, and translated-section terminals, giving projected cap `1579`.  The earlier `z1736`, `z1732`, and `z1724` packets remain hostile overlap controls.
  ...
  `-- bounded support-at-most-3 code of rank >=11            [THM-2052]
        ...
        `-- rank 11 -> finite two-anchor star atlas; one projective parameter
              ...
              `-- bad deck meets the indexed open 26-disk failure union
                    ...
                    |-- owner words close divisor-complete max<=30 [THM-2066/4075]
