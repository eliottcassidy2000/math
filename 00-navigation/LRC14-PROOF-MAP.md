...
Update through THM-2537: THM-2517's visible diagonal `c=ell` puts the owner at slot `d=-ell`; THM-2532--2537 turn actual owner phase into matched positive walls, an all-slope anchored Gram inverse, clock holonomy, lossless selector flow, and a pointwise selected-wall scalar.  The selected-source Boolean packet retains the terminal word, late owner, all root colours, and re-enters THM-2349's `91`-unit carrier.  Open: force the selected empty head into the target-active terminal/arrival role and align the inherited source/deep coordinate with the clock cut.
### Live interfaces
