...
**Reflected cone.**  Repaired THM-2941 closes `2m>=D`: it recomputes the `561`-body cap-`3` CSP, both orientations of `253` policies, and an assignment-complete `35`-lane/`1,600`-head tail atlas with coupled debt.  Its phase-zero atlas is `4/3,3/2,2,5/2,3,4,5,6`; MISTAKE-347 retains the superseded split-tail sources as hostile controls.  Below that cone, the residual bodies all have same-level graph `K6`, so six distinct levels are forced.  For `m=1`, exact decreasing-label debt rearrangement reduces all `561*30` physical extreme-pair orientations to one pair invoice: the pinned `D=6` theorem and an exact `D=7..44` head join a phase-free periodic-comb tail at `D=45`; only three inherited cells require reselection.  For `m>=2`, the low physical extreme is exactly `m` full teeth.  Summed interval/comb discrepancy, the positive exact base `(m,Q)=(2,46)`, the positive ray increment at `Q=23m`, and positive `Q`-increment close `Q>=23m`, equivalently `D>=22m`.  The base's weakest invoice is `2031150202/35071508756991`; `Q=45` is negative, so `46` is sharp for this coarse invoice at `m=2`.  In the remaining ratio interval, the only primitive low-phase extreme channels are `1:4,1:5,1:6`.  Exact safe-cell trapezoid profiles, `875,160` positive finite rows, and affine tails close all three full rays.  Every other channel has primitive floor `1/105`; a termwise debt envelope closes it at gcd scale `g>=48`, with exactly one hostile surrogate row at `g=47`.  The unresolved primitive lane is therefore `3<Q/m<23`, off `4,5,6`, with `gcd(m,Q)<=47`; it is not yet finite because the coprime primitive coordinates remain unbounded.
