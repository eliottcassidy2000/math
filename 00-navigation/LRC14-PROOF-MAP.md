...
## 2026-08-15 current control panel

**Status: OPEN.** Caps are `1,579/215`; at `z1=216`, THM-3399 takes the THM-3378 baseline `372,913/109/12` to `372,804/0/0`. THM-3352/3355 close matched residues, THM-3360 proves the `1/294` high-pair floor, and same-level edges complete all `3,003` bodies. Other residues/sectors, the rung, and LRC(14) remain open; `q<=25` is false.
**Disconnected-low closure.** The `36,520` complete-multipartite profiles close by THM-3355's weighted horn forest. MISTAKE-377 and THM-3360 repair the affine admissibility gate and prove the physical floor on all `22,890` lanes. Independently, THM-3384 combines a centered superunit continuum, `3,066,273,980` exact bridge masses through `p=698`, and a tail from `699` to prove `I>Dmax/5`. See the [THM-3360 note](../07-reflections/LRC14-disconnected-low-affine-limit-and-tail-certificate-kps-S171.md) and [THM-3384 note](../07-reflections/lrc14-disconnected-low-affine-tail-and-reflected-branch-closure-20260814.md).
...
**Typed transfer sidecars.** Raw drift ratios and denominator/unit passports do not determine located reflected-pair overlap. Nevertheless, four distinct reflected residue rays close projected `k=3` on all `3,003` bodies, five distinct reflected residue rays close projected `k=2` on the active `649`, and [THM-3381](../01-canon/theorems/THM-3381-reflected-residue-affine-phase-transport-and-frozen-tree-stability.md) proves the centered-phase/fixed-tree inequality for noncanonical chambers. See the [scoped transfer](../07-reflections/lrc14-reflected-pair-projected-transfer-and-affine-typed-survivor-20260814.md) and [shift boundaries](../07-reflections/lrc14-reflected-residue-phase-stability-and-tree-obstruction-20260814.md); neither is a live-ledger decrement.
**Complement-clock compiler.** THM-3363's `D=14` terminal is the seed for THM-3366.  On every THM-2928 quotient, covering the unsupported open cells by `r` integer danger combs compiles a hypothetical projected cover into at most `7+r` global clocks (`8+r` when `k=0`); cited LRC through twelve clocks closes `r<=5` (`r<=4` for `k=0`).  The exact pool-14 census closes raw support rows `17,561/19,273/19,198/19,053` for `k=0/1/2/3`.  Keywise intersection with the live refined ledgers removes `298` `k=2` rows / `71,575,318` occurrences and `7` `k=3` rows / `7,648` occurrences, leaving respectively `4,056 / 200,069,517,203` and `1,897 / 2,548,893,834`.  The seven `k=3` hits are exactly `{2,6,8,10,14} union {u}`, odd `u<=13`, at `D=L/2`.  The raw screens overlap all later screens and are not subtractive or physical-cover claims. [THM-3391](../01-canon/theorems/THM-3391-weighted-common-source-cyclic-support-capacity.md) supplies the weighted common-source/equality/coset calculus behind THM-3351/3385 and the THM-3388/3389/3394 sidecars, with no ledger change. [THM-3402](../01-canon/theorems/THM-3402-atomized-sheet-covers-and-constructive-cochain-locus.md) proves the all-q atom/cochain, exact-locus and consecutive-mode iff; finite clutters and `A_C/Gamma_D` body tests remain separate, with no ledger change.
**Projected `z1=216` terminal.** [THM-3399](../01-canon/theorems/THM-3399-complete-z216-projected-wall-by-hybrid-torsion-support-terminal.md) closes all `109` post-THM3378 rows. Row `195` splits `1,565=1,408+157` residual states; only the hostile `157` yield `599` one-high cases, and an independent audit closes the `76/60/1` two-high tail. Thus `372,913/109/12 -> 372,804/0/0` and cap `216->215`. This is projected screening, not a physical cover or LRC(14) conclusion.
...
6. Build an owner-labelled relation-overlap hypergraph. Test directed edges
   only for a real deletion, first-exit, or owner-transfer implication; otherwise retain the signed hypergraph and its rank. Partition resonance cells only after proving THM-2054's missing lifted plateau.
