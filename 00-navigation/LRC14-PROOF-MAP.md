...
**Status: OPEN.** THM-2923 leaves at most six body speeds.  THM-2941 gives wall `h/7` with empty zero excess; THM-2928/2941/2970/2972 and finite-exact boundary referees through the `z1=1642` shell empty aligned `k>=4`, give cap `k=2,3` at `1,641/270`, and empty the common-level reflected `k=1` diagonal.  The remaining censuses, arbitrary `k<=1`, rung, and LRC(14) are open; reserved THM-2980's full band statement is not proved; `q<=25` is false.
...
**Descending banks.**  For `k=3`, torsion/status closes through `297`; the next 45 occupied rows split `1549=659+882+8`, and order-two/four torsion closes the eight `z286` terminals, leaving `275`.  THM-2979 closes the ten `z275` rows by `2033=273+1695+65`; clean-cell density above `1/7` forces an exact order-seven collision in each terminal.  The THM-2941 addendum closes 29 further rows at `274,273,272` by `246=97+149`; height `271` is empty, so the `k=3` cap is `270` and ledger `375,674`.  For `k=2`, THM-2970/2972 first close all `58` rows on `1680..1742`, giving `1679`.  Three canonical all-body atlases then descend to the five `z1656` rows, closed by the first boundary referee.  A complete `1600..1655` atlas checks `168,168` candidate rows and leaves twelve rows at `1650`; status/projected and forced-high arguments close them, including a 167-class final body, giving `1649`.  Singleton shells at `1645` and `1642` then close with twenty exact status certificates and one projected packet; the atlas gaps between them are empty, so the current cap is `1641`.  These are finite-exact boundary inputs, not the reserved full THM-2980 theorem.  The earlier `z1736`, `z1732`, and `z1724` packets remain hostile overlap controls.
