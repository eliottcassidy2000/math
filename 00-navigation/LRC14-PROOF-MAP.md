...
**Status: OPEN.** THM-2923 leaves at most six body speeds.  THM-2941 gives wall `h/7` with empty zero excess; THM-2928/2941 empty aligned `k>=4`, cap `k=2,3` at `1,724/275`, and empty the common-level reflected `k=1` diagonal.  The remaining censuses, arbitrary `k<=1`, rung, and LRC(14) are open; `q<=25` is false.
...
**Descending banks.**  For `k=3`, exact kills are `85=36+49` at `328` and `2,346=702+1,643+1` at `324`; the last unbounded denominator-`3920` ray closes by an [antipodal cell pair](../04-computation/lrc14_j7_k3_z324_antipodal_h_drift_closure_thm2941.py).  At `312`, the inherited high wall reduces the `70` status residuals to `80` finite one-high/two-low cases; two clean cells separated by `d/2` (78 cases) or `d/3` (two cases) close every unbounded high ray.  Common-status/Farkas replay closes `306,302,298` by `137=17+120`.  At `297`, exact capacity/status replay reduces `1,172=271+830+71` states to `73` located one-high/two-low cases, all closed by the torsion-density pigeonhole lemma.  The same mechanism then closes the occupied levels `294,291,288,287,286,285,278,276`: among `1,549=659+882+8` states, the last eight occur at `286` and have a located safe-cell separation of order `2` or `4`.  Thus the current `k=3` cap is `275`, with ledger `375,713`.  For `k=2`, the exact `z1736` split is `929 -> 774 -> 155 -> 286 -> 0` on nine ordinary rows, five forced-high scalar kills, and one projected packet.  At `1732`, the two ordinary bodies give `188=33+155+0` and `24=1+3+20`; all twenty exact packets are projected-empty with minimum margin `903353/7131943`.  The atlas is empty on `1725..1731`, so the cap is `1724` and ledger `2,239,772`.  THM-2970 independently closes the atlas's nineteen forced-high rows and is an orthogonal corroboration, not a dependency of the `1732` replay.
