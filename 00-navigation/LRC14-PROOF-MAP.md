...
## 2026-08-02 current control panel

**Status: OPEN.** THM-2923 leaves at most six body speeds. THM-2928 empties aligned `k>=4`; THM-2995/3114 cap projected `k=2,3` at `1,579/225`; repaired THM-2941 confines reflected-`k=1` certificate failure, inside the `D>=6` stage, to `561` bodies with `m>=2,3<Q/m<6`, `Q/m` off `4,5`, and `gcd(m,Q)<=47`; ray `6` is closed. Arbitrary `k<=1`, the rung, and LRC(14) remain open; `q<=25` is false.
...
**Reflected cone.**  Repaired THM-2941 closes `2m>=D`: it recomputes the `561`-body cap-`3` CSP, both orientations of `253` policies, and an assignment-complete `35`-lane/`1,600`-head tail atlas with coupled debt. THM-3135 proves the simpler standard single-pair uniform H lanes form a DAG; the repaired atlas escapes through lane-conditioned/multi-pair cycles. Its phase-zero atlas is `4/3,3/2,2,5/2,3,4,5,6`; MISTAKE-347 retains the superseded split-tail sources as hostile controls.  Below that cone, the residual bodies all have same-level graph `K6`, so six distinct levels are forced.  For `m=1`, exact decreasing-label debt rearrangement reduces all `561*30` physical extreme-pair orientations to one pair invoice: the pinned `D=6` theorem and an exact `D=7..44` head join a phase-free periodic-comb tail at `D=45`; only three inherited cells require reselection.  For `m>=2`, the low physical extreme is exactly `m` full teeth.  The older two-endpoint-period invoice closes `Q>=23m`; exact safe-cell trapezoid profiles, `875,160` positive finite rows, and affine tails close the full rays `4,5,6`; and the inherited `1/105` floor closes every other channel at gcd scale `g>=48`.  The sharper completion views each low tooth as a circle window of high-period length `W=(QL-b)/(7(mL-a))`: its exact phase-free high-comb mass is `J(W)=floor(W)/7+max(0,{W}-6/7)`.  For physical slope `alpha>=7`, `J(alpha/7)/alpha>=1/91` and the complete debt is at most `11/1107`; for `6<alpha<7`, monotonicity reduces to `Q=6m+1`, where subtracting endpoint debts leaves `(L-2b)/(7((6m+1)L-b))` and `L=14 lcm(E)>=6 sum(E)` strictly pays all four interior debts.  Thus every `Q>6m` closes without a gcd condition.  The unresolved primitive lane is now `3<Q/m<6`, off `4,5`, with `gcd(m,Q)<=47`; ray `6` is its closed boundary, and coprime primitive coordinates remain unbounded.
**Descending banks.**  For `k=3`, torsion/status closes through `297`; the next 45 occupied rows split `1549=659+882+8`, and order-two/four torsion closes the eight `z286` terminals, leaving `275`.  THM-2979 closes the ten `z275` rows by `2033=273+1695+65`; clean-cell density above `1/7` forces an exact order-seven collision in each terminal.  The THM-2941 addendum closes 29 further rows at `274,273,272` by `246=97+149`; height `271` is empty.  THM-2981/3033/3041/3052/3061/3071/3078/3094/3098/3102/3106/3109/3111/3113/3114 empty every projected row through `z_1=226`: cap `225`, ledger `374,172`, with 78 rows at the next layer. THM-3126 independently reconstructs the six `z_1=229` prefix terminals and now serves as a second-path audit. For `k=2`, THM-2970/2972 close through `1679`; THM-2980's monolithic `3,003*80` audit closes all 68 candidate rows at `1600..1679`, and THM-2995's `3,003*20` audit closes all 26 at `1580..1599`.  Their typed joins include literal, torsion, scalar, and translated-section terminals, giving projected cap `1579`.  The earlier `z1736`, `z1732`, and `z1724` packets remain hostile overlap controls.
