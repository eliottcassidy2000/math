...
Update through THM-2539: THM-2517's visible diagonal `c=ell` puts the owner at slot `d=-ell`; THM-2532--2537 turn actual owner phase into matched positive walls, an all-slope anchored Gram inverse, clock holonomy, lossless selector flow, and a pointwise selected-wall scalar.  On the full-support O9c branch, THM-2539 keeps all `72` mixed target/clock channels and produces an ordered positive boundary pair with `216` nonzero root incidences.  Zero phase annihilates that scheduler, and neither a fixed orientation nor canonical selection preserves the mixed spectrum from the proved mechanisms alone.  Open: obtain a branch-uniform positive semantic arrival and align the inherited source/deep coordinate with the clock cut.
### Live interfaces
