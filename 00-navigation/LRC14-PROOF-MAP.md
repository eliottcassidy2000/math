...
**Status: OPEN.** THM-2923 leaves at most six body speeds.  THM-2941 gives wall `h/7` with empty zero excess; THM-2928/2941 empty aligned `k>=4`, cap `k=2,3` at `1,732/297`, and empty the common-level reflected `k=1` diagonal.  The remaining censuses, arbitrary `k<=1`, rung, and LRC(14) are open; `q<=25` is false.
...
**Descending banks.**  For `k=3`, exact kills are `85=36+49` at `328` and `2,346=702+1,643+1` at `324`; the last unbounded denominator-`3920` ray closes by an [antipodal cell pair](../04-computation/lrc14_j7_k3_z324_antipodal_h_drift_closure_thm2941.py).  At `312`, the inherited high wall reduces the `70` status residuals to `80` finite one-high/two-low cases; two clean cells separated by `d/2` (78 cases) or `d/3` (two cases) close every unbounded high ray.  Common-status/Farkas replay then closes `306,302,298` by `137=17+120`, leaving height `297` and ledger `375,765`.  For `k=2`, the all-body atlas and exact splices reach `1736`; its nine ordinary rows close by `929 -> 774 -> 155 -> 286 -> 0`, five HIGH rows have negative forced-wall maxima, and the last HIGH row has one finite-cutoff packet whose projected margin is `849/1547`.  With `1733..1735` scalar-empty, the cap is `1732` and ledger `2,239,774`.
