...
Update through THM-2550: THM-2517's visible diagonal `c=ell` puts the owner at slot `d=-ell`; THM-2532--2537 turn actual owner phase into matched positive walls, clock holonomy, lossless selector flow, and a pointwise selected-wall scalar. THM-2539/2543 give branch-uniform labelled horizontal boundary systems but no semantic vertical edge. THM-2545 identifies the exact second cut: after a genuine later target-root map exists, same-root arrival is forced iff the word-stratified off-diagonal compatibility graph has a weighted Hall deficit. THM-2548 proves that seven-step transport preserves every root-charged `C_91` mode but cannot create that edge; its first cut is directed neutral-gain reachability. THM-2549 proves that chronology itself is already available uniformly: every one of the `165` positive selected-head packets meets a strictly later literal owner-to-word handoff on the same base at one common delay. The obstruction is now action-theoretic. Every proved positive later field on that base is a positive-depth base-thirteen pullback, hence old-target-neutral and old-root-constant; it cannot define an equivariant later root selector and induces only a cemetery Hall table. Conversely, once a specified target-active unit role and its sheet are retained, the top ancestry digit minus its carry recovers the head root pointwise and collapses Hall. Thus the shortest target is to force `k_a` on positive selected-head mass while keeping that digit/carry. THM-2550 is a verified candidate positive control on one typed row (nonzero old owner-loop drift and non-replica cut table), not that ancestry map. THM-2547 is external only after MISTAKE-281 and supplies no physical bridge.
### Live interfaces
