...
**Reflected cone.**  Repaired THM-2941 closes `2m>=D`: it recomputes the `561`-body cap-`3` CSP, both orientations of `253` policies, and an assignment-complete `35`-lane/`1,600`-head tail atlas with coupled debt.  Its phase-zero atlas is `4/3,3/2,2,5/2,3,4,5,6`; MISTAKE-347 retains the superseded split-tail sources as hostile controls.  Below that cone, the residual bodies all have same-level graph `K6`, so six distinct levels are forced.  At `(D,m)=(6,1)`, exact decreasing-label debt rearrangement plus all `561*30` physical extreme-pair orientations closes the slice; the weakest margin is `4942672624/429463642167` on `(1,2,3,4,6,12)`.
