...
THM-2130 intersects `11/13` labels into a sparse exception or `143` determinant; THM-2128/2131 exclude both no-terminal-blocker pencils. THM-2133--2138 empty the scalar `6+1/5+2` tails. THM-2139/2141 close fivefold profiles `(1,5,2)` and `(2,5,1)`, leaving exactly `(1,6,1)`, `(1,7,0)`, `(2,6,0)`, and `(3,5,0)`. THM-2140 gives the one-blocker depth/polar invoice. THM-2148/2168 classify the three-target Fano fibre and force the top two blocker depths; generated-lattice descent then empties all-independent and scalar `4+3`, leaving only rank-one scalar `5+3` inside `(3,5,0)`. THM-2192 forces a unique deepest blocker and eliminates `36/252` unit residue profiles. THM-2198/2204/2205/2207 and THM-2213/2215/2219 exclude every profile through deepest depth four by exact labelled capacities and sparse correlation. THM-2222/2224 force `lambda_1<=5`; THM-2226/2227 reduce the ledger to `452`; THM-2229 retains the positive residual and leaves `240`; THM-2233's exact guard-danger Bellman leaves `224`. THM-2239 closes `58`, leaving `165` first-depth-one rows and `(3,4,5)`; THM-2257/2258 independently close the latter. THM-2286/2288/2289/2291 give bounded exact lifts and delayed blocker-only handoffs on all `165`; THM-2296 reaches prescribed expiration with return or a bounded common atom. THM-2305 refines the return into a pure owner edge or two-target fork, retaining every root character and a bounded signed coefficient on one exact word. THM-2295/2298 force scalar ranks five and six by `196/526`, and THM-2301 makes the resulting septimal/thirteen-adic bright-or-dark address banks exact. On `120` interiors THM-2287 supplies a `c_1`-anchored rank-six flag and exact `21`-pattern atlas; all `165` have its shorter unanchored packet. THM-2299 still kills the pair base phase despite an anchored rank-six flag; THM-2303 identifies the missing terminal-component current and relative phase-tree coordinate. THM-2323/2326/2327 force a marked `91`-unit deepest edge on every positive strict shallow word stratum, and THM-2349 extends the identical marked incidence to all `15` repeated rows after a finite coefficient-dependent shallow restart. THM-2331 puts every prescribed all-unit target address into the atomic expansion. THM-2334/2337 then give the full-word target/first-jet Abel pushforward and a surviving joint fibre, but the target may be zero and the factor-coloured jet is not an address invariant. THM-2340 splits the exact nonzero-target variance into ordered row main effect, column main effect, and two-way interaction, matching the pure-`a`, pure-`b`, and fork loci. THM-2355 gives a quadratic alternative to direct phase tracking: singleton energies plus lawful `13`-twisted pair probes on the nonzero-component support reconstruct the phase tree, while whole target-twist energy alone retains only autocorrelation and admits a real full-support hostile. The next theorem must realize that pair service, an acute grouped-current cone, or another word-matched ANOVA component, then retain a bounded all-unit address. The `120` anchored interiors and `45` unanchored boundary rows still have different relation sidecars, but no longer different marked-current existence.
...
| Rank code / target landing | THM-2295/2298 force ranks five/six; THM-2307 gives the `77/7/1` dual ambiguity. THM-2323/2326/2327 force strict-row `91`-unit edges; THM-2331/2334/2337 retain target fibres, word twists, and the first jet. THM-2340 gives the word ANOVA, while THM-2343/2344 identify zero-only landing with a shifted endpoint convolution inverse. THM-2344 also proves arbitrary nonconstant same-axis word content preserves the bad character. | Break that inverse with at least two independently translated base coordinates or cross-axis coupling, then keep bounded all-unit visibility and THM-2303 phase. Repeated/resonance rows and the other `45` need anchor-free structure. |
