...
THM-2334/2337 give the full-word target/first-jet Abel pushforward, THM-2340 gives the target ANOVA, and THM-2343/2344 identify zero-only landing with a shifted endpoint inverse. THM-2350 identifies the lawful target twists as two balanced target--graft dipoles and makes target survival equivalent to positive magnetic Dirichlet energy; it is uniform on all `165` rows but does not force an edge defect. THM-2355 proves that singleton energies plus lawful `13`-twisted pair probes on a nonzero-component tree reconstruct phase, while whole target-twist energy retains only autocorrelation. THM-2356 gives exact planar graph-chirp tomography, but refined activity need not survive the coarse row sum: the coarse zero-only kernel has dimension `28,393`, and `delta_0(q)B(z)` is only its `169`-dimensional refined-zero subspace. THM-2363's derived coefficient-level detector dominates fixed-word coarse energy by `1/169`, not conversely, and lacks a lawful realization. THM-2354 proves the deep factor-colour current; audited THM-2364 forces a fully mixed deep/blocker-probe colour while collapsing the literal word inside its indicator. Neither retains target/address or terminal endpoint phase. THM-2358 gives the exact real sign cone and a centered-indicator hostile. THM-2378/2381/2382/2385/2417/2426 empty every `M>0`, `nu_7(H)<M<nu_7(c_3)` alternative, so a surviving subtop-guard packet has a target-aligned noncirculant `c_3` graft. On the complementary deep-`c_3`/top-guard side, THM-2430/2431/2432 reduce THM-2427's seven types to three. Hostile-audited THM-2436 then enumerates their punctured common-`91` stalks and uses repeated-step physical rounding, refined by fixed step spectrum at `M=0`, to empty all three. Thus only the `c_3<=M` noncirculant-graft branch survives this valuation split. THM-2441 reduces every fixed rational source/terminal delayed-clock audit on that branch to finitely many classes modulo `7D_0`, with the centred signed endpoint word periodic; the missing global packet and owner-conditioned landing remain. The canonical LRC applications of THM-2388--2419 below the now-empty final lane remain reusable conditional sidecars; transplanting them to that surviving branch is separate, while cross-graph phase/depolarization remains independently open.
...
| Rank code / target landing | THM-2295/2298 force ranks five/six; THM-2307 gives the `77/7/1` dual ambiguity. THM-2323/2326/2327 force strict-row `91`-unit edges; THM-2349 extends the marked triangle and inverse boundary to all `165` rows at finite delayed clocks. THM-2331/2334/2337 retain target fibres, word twists, and the first jet. THM-2340 gives word ANOVA; THM-2343/2344 identify zero-only landing with a shifted endpoint inverse. THM-2350 identifies the lawful balanced dipoles and magnetic target energy but does not force a defect. THM-2356 makes graph tomography exact but exposes a `28,393`-dimensional refined-to-coarse row-sum kernel; its vertical `169`-space is only the refined-zero locus. THM-2363 is a derived detector which dominates coarse energy, not conversely, and lacks a lawful realization. THM-2354 is factor-coloured; THM-2364 proves a mixed deep/blocker-probe extension but collapses the literal word. THM-2358 rules out an automatic real sign cone. Hostile-audited THM-2436's punctured-`91` atlas closes all three deep-`c_3` types by physical repeated-step rounding banks. | The sole surviving `c_3<=M` branch needs owner-conditioned graft survival with top-blocker cosets, blocker-owned septimal fibres, cross-base endpoint evolution, and the labelled valuation-zero speed retained. |
