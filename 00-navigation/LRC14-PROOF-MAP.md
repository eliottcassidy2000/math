...
**Complement-clock terminal.** THM-3363 observes that the canonical `E={1,2,3,4,6,12}`, `D=14` projected carrier has complement `D_1 union D_(14 alpha)`. Any six-drift cover would therefore contradict cited LRC for nine runners. This deletes all 26 denominator shapes on that row without a numerator-height cutoff; `27,239` arbitrary-residue `k=1` body/divisor rows remain.
