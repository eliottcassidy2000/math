...
THM-2334/2337 give the full-word target/first-jet Abel pushforward, THM-2340 gives the target ANOVA, and THM-2343/2344 identify zero-only landing with a shifted endpoint inverse. THM-2350 identifies the lawful target twists as two balanced target--graft dipoles and makes target survival equivalent to positive magnetic Dirichlet energy; it is uniform on all `165` rows but does not force an edge defect. THM-2355 proves that singleton energies plus lawful `13`-twisted pair probes on a nonzero-component tree reconstruct phase, while whole target-twist energy retains only autocorrelation. THM-2356 gives exact planar graph-chirp tomography, but refined activity need not survive the coarse row sum: the coarse zero-only kernel has dimension `28,393`, and `delta_0(q)B(z)` is only its `169`-dimensional refined-zero subspace. THM-2363's derived coefficient-level detector dominates fixed-word coarse energy by `1/169`, not conversely, and lacks a lawful realization. THM-2354 proves the deep factor-colour current; audited THM-2364 forces a fully mixed deep/blocker-probe colour while collapsing the literal word inside its indicator. Neither retains target/address or terminal endpoint phase. THM-2358 gives the exact real sign cone and a centered-indicator hostile. THM-2378/2381/2382/2385/2417/2426 empty every `M>0`, `nu_7(H)<M<nu_7(c_3)` alternative, so a surviving subtop-guard packet has a target-aligned noncirculant `c_3` graft. On the complementary deep-`c_3`/top-guard side, THM-2430/2431/2432 reduce THM-2427's seven types to exactly `(1,5,1,8),(2,5,2,9)` at `M=0` and `(2,5,1,8)` at positive depth. The canonical LRC applications of THM-2388--2419 below the now-empty final lane remain reusable conditional sidecars; transplanting them to a surviving branch is separate, while cross-graph phase/depolarization remains independently open.
