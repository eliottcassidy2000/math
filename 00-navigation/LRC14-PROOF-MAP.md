...
Update through THM-2554: THM-2517's visible diagonal `c=ell` puts the owner at slot `d=-ell`; THM-2532--2540 give matched positive walls, clock holonomy, anchored transport, exact cross-Kakeya reconstruction conditional on a later field, and pointwise selected-wall scalarization, but no live semantic vertical edge. THM-2545 identifies the Hall cut after a genuine later-root map exists. THM-2548 proves that seven-step transport preserves every root-charged `C_91` mode but cannot create that edge; its first cut is directed neutral-gain reachability. THM-2549 proves chronology uniformly: every one of the `165` positive selected-head packets meets a strictly later literal owner-to-word handoff on the same base at one common delay. Every proved positive later field there is a positive-depth pullback, hence old-target-neutral/root-constant, so the inherited table is cemetery-only. A unit role's top sheet digit minus carry recovers the selected head as an old-action ancestry root, but a new semantic intertwiner must identify this with the genuine later root before Hall collapses. THM-2554 reduces any common-torsor equivariant joint law to displacement `d=b-h`; rotation alone permits perfect off-diagonal hostiles, while endpoint-swap plus odd primitive quotient weight would force `d=0`, neither hypothesis being live. THM-2550 is a verified candidate positive control on one typed row, not the ancestry map. THM-2547 remains external after MISTAKE-281.
### Live interfaces
