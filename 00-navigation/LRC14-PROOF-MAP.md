...
**Status: OPEN.** THM-2923 leaves at most six body speeds. THM-2928 empties aligned `k>=4`; THM-2995/3071 cap projected `k=2,3` at `1,579/234`; THM-2941's fixed-owner repair confines reflected-`k=1` certificate failure to `561` bodies, `D>=6,1<=m<2D/3`. Arbitrary `k<=1`, the rung, and LRC(14) remain open; `q<=25` is false.
...
**Reflected cone.**  THM-2941 closes `3m>=2D`.  Its cap-`5/2` replacement keeps one physical pair across a five-interval closed Farey cover, recomputes the `389/172` CSP and both assignment orientations, and couples scale to singleton debt.  Its phase-zero atlas is `4/3,3/2,2,5/2,3,4,5,6`; the unique prime-five edge is a bridge in all 65 forced components.  MISTAKE-347 still invalidates the old one-/three-quarter routes and cap-`3` half-cone.
**Descending banks.**  For `k=3`, torsion/status closes through `297`; the next 45 occupied rows split `1549=659+882+8`, and order-two/four torsion closes the eight `z286` terminals, leaving `275`.  THM-2979 closes the ten `z275` rows by `2033=273+1695+65`; clean-cell density above `1/7` forces an exact order-seven collision in each terminal.  The THM-2941 addendum closes 29 further rows at `274,273,272` by `246=97+149`; height `271` is empty.  THM-2981 closes `423` rows at the fifteen occupied heights `270..247`, THM-3033 closes 200 more through `244`, and THM-3041/3052/3061/3071 empty every projected row through `z_1=235`, including gaps `241` and `238`: cap `234`, ledger `374,780`, with 381 rows at the next occupied layer.  For `k=2`, THM-2970/2972 close through `1679`; THM-2980's monolithic `3,003*80` audit closes all 68 candidate rows at `1600..1679`, and THM-2995's `3,003*20` audit closes all 26 at `1580..1599`.  Their typed joins include literal, torsion, scalar, and translated-section terminals, giving projected cap `1579`.  The earlier `z1736`, `z1732`, and `z1724` packets remain hostile overlap controls.
