...
THM-2130 intersects `11/13` labels into a sparse exception or `143` determinant; THM-2128/2131 exclude both no-terminal-blocker pencils. THM-2133--2138 empty the scalar `6+1/5+2` tails. THM-2139/2141 close fivefold profiles `(1,5,2)` and `(2,5,1)`, leaving exactly `(1,6,1)`, `(1,7,0)`, `(2,6,0)`, and `(3,5,0)`. THM-2140 gives the one-blocker depth/polar invoice. THM-2148/2168 classify the three-target Fano fibre and force the top two blocker depths; generated-lattice descent then empties all-independent and scalar `4+3`, leaving only rank-one scalar `5+3` inside `(3,5,0)`. THM-2192 forces a unique deepest blocker and eliminates `36/252` unit residue profiles. THM-2198/2204/2205/2207 and THM-2213/2215/2219 exclude every profile through deepest depth four by exact labelled capacities and sparse correlation. THM-2222/2224 force `lambda_1<=5`; THM-2226/2227 reduce the ledger to `452`; THM-2229 leaves `240`; THM-2233 leaves `224`; THM-2239's separately centered signed dual closes all first-depth-two rows and `(4,6,8)`; THM-2244's corewise prior-centered odd clause closes `28/29` first-depth-three rows. Exactly `166` rows remain: the `165` first-depth-one profiles and `(3,4,5)`. THM-2234 supplies private mass/expansion but closes no row. Labelled signed guard-hole/owner correlation remains for depth one, while `(3,4,5)` needs a composite or multidirectional score; MISTAKE-238/239/242 still block invalid transfers, and none closes LRC(14).
...
| Rank code / adaptive carry | THM-2196/2199 give finite carry charts, projective finiteness, and rank twelve; THM-2208/2212 reduce each rank-eleven plane to bounded anchors, one cofactor candidate, 91 ruler systems, and a signed equality skeleton; THM-2203 gives the scalar covectors | THM-2198--2229 leave `240`; THM-2233/2239 leave `194`; THM-2244 closes `28/29` first-depth-three rows, leaving exactly `166`: the `165` first-depth-one rows and `(3,4,5)`. THM-2234 gives first-depth-one private expansion. The sharp depth-one target is a separately centered signed owner/incidence join retaining THM-2211 carry and THM-2216/2218/2238 sidecars; `(3,4,5)` requires a composite or multidirectional score. THM-2237 proves the generic lower-moment ceiling exact; THM-2235 forbids a fixed-XOR/odd-sheet antipode shortcut. |
