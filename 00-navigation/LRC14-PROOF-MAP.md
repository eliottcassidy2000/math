...
Independently audited THM-2449 says any rational lawful anchored continuation either fires all `72` mixed source-target colours after one rectangle defect or is exactly delta plus six replicas. THM-2456 identifies that failure as a uniform owner-minus-replica root-density offset: it is trivial on one Boolean chart, but sharp source-exclusive and paired-safe averaged controls survive, so literal truth bits alone do not force a rectangle. THM-2457 turns any supplied common oriented root chart and disjoint semantic pair into a directed `128`-atom co-support graph; a loop is exact-atom service, an edge is two-atom service, and any selected nonzero table admits an at-most-three-atom service coarsening with sharp mass `M_0/16384`. Its exact live strict-profile hostile proves one complete local atom can meet both pure and fork THM-2305 word strata. The remaining obligation is to exclude the physically realizable uniform-offset locus or couple selected drift to a canonical semantic/root edge; ownerless cancellation, missing-factor enumeration, and transition conversion are no longer the issue.
...
| Rank code / target landing | THM-2295/2298 force ranks five/six; THM-2307 gives the `77/7/1` dual ambiguity. THM-2323/2326/2327 force strict-row `91`-unit edges; THM-2349 extends the marked triangle and inverse boundary to all `165` rows at finite delayed clocks. THM-2331/2334/2337 retain target fibres, word twists, and the first jet. THM-2340 gives word ANOVA; THM-2343/2344 identify zero-only landing with a shifted endpoint inverse. THM-2350 identifies the lawful balanced dipoles and magnetic target energy but does not force a defect. THM-2356 makes graph tomography exact but exposes a `28,393`-dimensional refined-to-coarse row-sum kernel; its vertical `169`-space is only the refined-zero locus. THM-2363 is a derived detector which dominates coarse energy, not conversely, and lacks a lawful realization. THM-2354 is factor-coloured; THM-2364 proves a mixed deep/blocker-probe extension but collapses the literal word. THM-2358 rules out an automatic real sign cone. THM-2436 closes every deep-`c_3` type; THM-2445 gives a typed `24`-cell drift/source trichotomy on all `nu_7(c_3)<=M`; THM-2442 supplies the ghost's delayed word; THM-2452 completes a matched literal endpoint atom and reselects a fresh exact `X` and `91`-unit `m`. THM-2456 classifies the exact averaged uniform-offset hostile; THM-2457 makes supplied common-root service a sharp directed atom graph and proves local atoms do not determine pure/fork words. | Exclude the physical uniform-offset atlas or couple selected drift to a canonical semantic/root edge. |
