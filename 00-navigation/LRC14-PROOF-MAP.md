...
Independently audited THM-2449 says any rational lawful anchored continuation either fires all `72` mixed source-target colours after one rectangle defect or is exactly delta plus six replicas. THM-2456 identifies that failure as a uniform owner-minus-replica root-density offset: it is trivial on one Boolean chart, but sharp source-exclusive and paired-safe averaged controls survive. THM-2457 turns any supplied common oriented root chart and disjoint semantic pair into a directed `128`-atom co-support graph. THM-2459 retains one supplied service edge and aggregate endpoint drift in a Boolean union of at most four atoms, with floors `M_0/16384` and `D_0/63001`; THM-2460 copies any independently retained pure/fork word without further endpoint loss. THM-2461 identifies the remaining typing boundary: terminal words are prescribed-time edges, the THM-2305 predecessor root is not THM-2365's deep-probe phase, same-time idempotence is not temporal transport, four of five first-failure roles are target-neutral, and the active THM-2379 repair still freezes its paired blocker. Independently hostile-audited THM-2466 then closes the analytic word-selection seam: after the owner-supported common root base and positive service are supplied, every fixed positive terminal word sufficiently far out retains both observables, with drift/service floors `mu(Q)^2D_0/252004` and `mu(Q)M_0/32768`. THM-2471 constructs the formerly hypothetical owner side directly at THM-2306's first collision: the canonical weighted root service is `169I_r`, all twelve root colours survive, their signed sum is `-I_r`, and their energy/max floors are `I_r^2/12` and `I_r/12`. Its exact Boolean ancestry stalk proves that weighted service is physical, but also that `P_omega(x)`, `P_omega(T^Kx)`, and the THM-2365 deep endpoint are different temporal copies; endpoint-aligned service collapses to the unique owner loop, and a nonowner atom cannot restore owner-supported drift by coarse union. THM-2458/2462 physically realize a thirteen-chart uniform-offset hostile, while THM-2464 shows that linked shallow blockers are forced safe on clean charts but depth at least two and every nonempty joint clock cell remain realizable. The remaining obligation is an off-diagonal temporal owner-to-endpoint/deep intertwiner, a proof that the unique owner-loop drift is nonzero, or a global cover/valuation/semantic-clock/dipole exclusion of uniform offset; owner-root existence, owner-collision service, all-colour restoration, missing-factor enumeration, delayed-word mixing, and local phase compatibility alone are no longer the issue.
...
| Rank code / target landing | THM-2295/2298 force ranks five/six; THM-2307 gives the `77/7/1` dual ambiguity. THM-2323/2326/2327 force strict-row `91`-unit edges; THM-2349 extends the marked triangle and inverse boundary to all `165` rows at finite delayed clocks. THM-2331/2334/2337 retain target fibres, word twists, and the first jet. THM-2340 gives word ANOVA; THM-2343/2344 identify zero-only landing with a shifted endpoint inverse. THM-2350 identifies the lawful balanced dipoles and magnetic target energy but does not force a defect. THM-2356 makes graph tomography exact but exposes a `28,393`-dimensional refined-to-coarse row-sum kernel; its vertical `169`-space is only the refined-zero locus. THM-2363 is a derived detector which dominates coarse energy, not conversely, and lacks a lawful realization. THM-2354 is factor-coloured; THM-2364 proves a mixed deep/blocker-probe extension but collapses the literal word. THM-2358 rules out an automatic real sign cone. THM-2436 closes every deep-`c_3` type; THM-2445/2442 give word-restored typed cells; THM-2452 gives a complete matched endpoint atom. THM-2456 leaves averaged uniform offset; THM-2457 gives the root co-support graph; THM-2459 retains supplied service and drift in four atoms; THM-2460 copies a separately retained word; THM-2461 separates temporal word, owner root, deep root, and polarized repair. THM-2471 supplies canonical first-collision owner-root service and all twelve colours, but proves its source, arrival, and deep endpoint copies are distinct. THM-2458/2462/2464 show that the local hostile survives unrestricted phase realization and every compatible delayed clock cell. | Couple the THM-2471 owner-collision stalk off-diagonally to the THM-2365 endpoint/deep current, prove nonzero unique-owner-loop drift, or exclude uniform offset using the full global cover/valuation/clock/paired-dipole packet. |
